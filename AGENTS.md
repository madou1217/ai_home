...
- Cross-platform for the current interactive CLI path: the backend is real tmux on macOS / Linux / WSL. On **native Windows** `detectTmux()` looks for a tmux-compatible binary — `psmux` (native ConPTY, speaks tmux's CLI) first, then an MSYS2/Cygwin `tmux.exe` (`C:\msys64\usr\bin`, `C:\cygwin64\bin`) or anything named `tmux` on PATH. If none is found, persistence degrades to a plain direct spawn and `sessions` prints an install hint (psmux / MSYS2). The interactive Windows wiring still needs validation on a Windows host; the detached headless lane (WebUI codex app-server 常驻等) is validated: **Windows 上没有 sh**，pane 不再走 `sh -c`，而是 `spawnHeadlessRun` 的 `commandArgv` 原样下发 + 调用方落一个 launcher `.cmd`（`cmd.exe /d /c <launcher>`）。psmux 会把 pane argv 拼成单条 CreateProcess 命令行，空格/嵌套引号不可靠（实测双引号包裹的含空格路径直接失败），所以 launcher 路径必须无空格、裸传。
...
- **SSH 远程拉起/重启 server（Windows）**：sshd 把会话进程塞进带 `KILL_ON_JOB_CLOSE` 的 Job Object，`server restart` spawn 的 detached 子进程在 SSH 会话关闭时被一并回收（status 显示 running 都是假象，换会话就消失）。必须经 `schtasks` 一次性任务拉起（任务在会话外执行）。同时**禁止在 schtasks 里直接跑 node.exe**——控制台子系统会在桌面弹出黑窗；用 autostart 装好的 VBS 隐藏启动器：`schtasks /create /tn <一次性名> /sc once /st 23:59 /tr "wscript.exe \"%AppData%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\com.clawdcodex.ai_home.vbs\"" /ru <user> /f`，`/run` 后验证、再 `/delete`。验证以**另开一条 SSH 会话**查 `/readyz` 为准（确认进程脱离 job 存活）。
...
- **codex config 受管块的清理义务**：set-default 只替换 aih_server 段是不闭环的——旧版按账号命名的 [model_providers.aih_N]（曾内嵌明文 bearer_token，且该字段在现行 codex 已非法）与旧迁移脚手架（openai_base_url + migrated 注释，仅覆盖内建 openai provider、指向已死 relay）必须一并清理（codex-config-sync 的 removeAihManagedCruft，宿主与沙箱两条同步链共用）。凭证走 env_key=OPENAI_API_KEY（aih 启动时注入 env），auth.json 属于用户自己的 codex 登录（ChatGPT OAuth），不要往里写账号 key。现行形态（2026-08-22 起）：宿主受管块用 [model_providers.aih_server.auth] 命令表（scripts/aih-codex-provider-auth.js 三级取 key：OPENAI_API_KEY env 直通 -> AIH_PROVIDER_ACCOUNT_REF 指定账号 -> 默认账号），裸跑 codex 因此闭环；**auth 表与 env_key 在 codex 0.149 互斥（并存直接拒绝加载配置）**，-c 参数不得再传 env_key；auth.json 里存 API key 不能满足 env_key（实测），不要试图用它闭环。
...
- **codex app-server chat 常驻 pane 的认证（2026-09-12 取证结论）：chat 运行时的 codex app-server 用 `-c model_providers.aih_server.*` 把流量钉死在本地网关（投影 config.toml 为 `env_key=OPENAI_API_KEY` 形态），pane env 必须携带网关 client key。** 旧的账号级 relay（`codexRelayProfile.shouldRelayAccount`）只在账号凭证自带 OPENAI_API_KEY 时生效——qodercn 等凭证存 `native_auth` 的账号 env 为空，pane 无 key 出网 → 网关 401 `unauthorized_client`。`codex-app-server-endpoint.js` 现在无条件叠加 `readCodexGatewayConnection` 的 OPENAI_API_KEY/OPENAI_BASE_URL（账号身份由 -c http_headers 的 X-Account-Ref 钉住，key 不出本机），且常驻 pane 复用除 runtimeFingerprint 外还要求 `envSignature`（client key/baseUrl/accountRef 的 sha256 前 16 位）匹配——旧时代缺 key 的 pane readyz 照样 200，不加这个守卫会被无限复用、401 永续。**光叠加 spawn env 在 Windows 上还不够：psmux 的常驻 broker/server 复用其启动时的环境，pane 继承 server 环境，同名变量的 spawn env 被静默忽略（实测用户全局 `OPENAI_API_KEY`/`OPENAI_BASE_URL` 顶掉网关连接串 → 401 在全新 pane 上依旧）。** win32 因此把鉴权/身份变量（OPENAI_API_KEY/OPENAI_BASE_URL/AIH_CODEX_GATEWAY_ACCOUNT_REF/CODEX_HOME 等白名单）经 `new-session -e` 显式投递（tmux-driver `spawnHeadlessRun` 的 `paneEnv`，session env 优先于 server env，空格值实测可用），并给 envSignature 加投递世代后缀强制旧 pane 重建一次；POSIX 维持 fresh-server 进程 env 继承（老 tmux <3.2 不认识 `-e`，不能全局开）。
- **WebUI chat/work 附件链路（2026-09-11 取证结论）：限额/类型的唯一事实来源是 `contracts/chat-attachments.json`（文本 16MB、视频 100MB、图片 10MB、批次 8 个/128MB），前后端分类优先级必须同构（图片 MIME → 视频 MIME → 文本 MIME/扩展名 → 视频扩展名兜底）。** 视频无 harness 直传通道：上传时服务端 ffmpeg/ffprobe 抽 ≤8 张关键帧（`lib/server/chat-video-attachments.js`），发送时帧走既有图片通道、提示词附原视频路径。三个隐藏卡口：①codex app-server 的 turn/start（及排队输入）硬上限 **1,048,576 字符**，chat 模式大文档全文内联必撞；`thread/inject_items` 实测无此上限（1.5M 字符可注入）且注入条目不属任何 turn、不进 thread/read 的 turns（不会在 AIH 时间线产生重复气泡）——超大文档因此在 turn/start 前分段注入历史、turn 只带指路语（`chat-harness-policy.js` 注入三件套 + driver 按 runId 幂等）。②harness 模型目录的 `input_modalities` 来自 models-dev 固定目录投影：provider 后缀变体（`claude-opus-4-6-thinking`、`gemini-3.1-pro-high` 等）查无条目时若默认 text-only 会在本地拦掉图片/帧附件；现为"精确 id → 逐段裁尾回退基座"（`inferBaseModelIds`）+ 未知模型 fail-open `['text','image']`（目录明确 text-only 的保持从严），改投影逻辑必须 bump `chat-harness-gateway.js` 的 `runtimeConfigRevision`（当前 v4）刷新常驻 harness 的 model-catalog。③网关 `/v1` 默认 body 上限已随附件历史放大到 32MB（`server.js DEFAULT_MAX_REQUEST_BODY_BYTES`），否则注入历史后 harness 请求 413。
...
- **kimi 套餐配额数据面（2026-09-11 取证结论）：订阅控制台 `/code/console`「My Quota」页的数据来自官方 Connect RPC `POST https://www.kimi.com/apiv2/kimi.gateway.membership.v2.MembershipService/GetSubscriptionStats`（+ `GetSubscription` 补套餐名/有效期），只认桌面端托管的 kimi.com Web session（nativeAuth.desktopSession），kimi-code CLI token 对它是 401。** `GetSubscriptionStats` 响应即页面全部数据：`ratelimit_code_5h/7d`（{ratio, enabled, reset_time}，聊天窗 ratelimit_5h/7d 不用时整个对象缺席）、`subscription_balance`（`amount_used_ratio` 月度总量、`kimi_code_used_ratio` 其中 Code 部分、`expire_time` 重置日）、`gift_balances[]`（display_name 如 "Invite to Earn Credit"、used ratio、expire_time）、`notice`+`overdrawn`（月度用尽提示）。**proto3 JSON 省略零值：ratio 缺席即 0。** GetSubscription 的 `subscription.goods.title` 是品牌档（Allegretto 等），`next_billing_time` 即页面 "Valid until"，`status=SUBSCRIPTION_STATUS_CANCEL` 表示已取消续费但周期内仍有效。实现：`lib/server/kimi-plan-stats.js`（30s 缓存 + 401 强制轮换 web token 重试一次），路由 `GET /v0/webui/accounts/kimi/:ref/plan-stats`（按需调试入口）；**展示主链路是快照管线**：`kimi-quota-probe.js` 在托管过 desktopSession 的账号上把套餐统计并入 `kimi_oauth_usage` 快照（best-effort，失败不拖累 CLI OAuth 主快照）——月度总量折算成 `bucket=monthly` 的 30day 窗口条目（参与账号级 min 剩余，月度用尽=真实不可用），Gift 余额折算成 `category='gift'` 条目（**不计入 min 剩余**，`lib/account/usage-remaining.js` 与 `web/src/features/accounts/account-state.ts` 两处口径必须同步，否则 Gift 用尽会把健康账号误显示成「已耗尽」），订阅品牌档覆盖 `account.planName` 并落 `account.planSubscription`（账号列「套餐名 + 有效期至」的数据源，`AccountBadges.getKimiPlanSubscription`）。
...
- **qoder/qodercn 的推理通道（2026-09-14 取证结论）：Qoder 没有可直放的 HTTP API——凭证是 WASM 用 machineId 加密的本地 blob（`.auth/user`，`credential_storage_encrypt(userInfoJson, machineId.slice(0,16))`），模型请求还要过 Cosy/sgsdk（`.bin/sgsdk.dll`）签名，属 zcode 同类「客户端侧无解」结论，不要做 relay 逆向。** 上游事实：CLI 打 `https://gateway.qoder.com.cn/model/v1/chat/completions`（全球站 openapi.qoder.sh 系；`QODER_MODEL_SERVER_HOST` 可覆盖），OpenAI chat 形态，Bearer = `security_oauth_token`。aih 的正规通道是原生 CLI headless：`lib/server/qoder-cli-transport.js` 把 `qodercli(cn) --print --output-format stream-json --max-turns 1 --config-dir <账号 runtimeDir>` 桥成 OpenAI Chat Completions（prompt 走 stdin 规避 Windows argv 32K 上限；认证全交 CLI：materializeProviderAuth + PAT 账号 env 注入；图像附件显式占位不静默丢弃）。路由接线：`PROVIDER_FALLBACK_REQUEST_PROTOCOLS` 的 qoder/qodercn → openai_chat（responses-wire harness 经适配器回落），attempt 链注册 `qoderCliTransport`（`upstream-endpoints-transport-qoder-cli.js`，与 openCodeGoTransport 同 Strategy 契约）。CLI 超时下限 120s（冷启动 + 安全初始化 hook 实测 ~1.5s）。**不给 CLI 注入 server/账号代理**：gateway.qoder.com.cn 直连实测可达，经本地代理转发会在 init 后无超时挂死（2026-09-14 对照实验；用户系统级代理仍随 process.env 自然继承）。**上游配额墙形态：result 事件 `is_error` + `errors[0]` 是 `{"pricingUrl":...}` JSON + `error_code:118`**，transport 映射为可读中文文案透出，不做掩盖。

...
*   **Trusted Quota Routing**: `aih` no longer infers quota state from runtime stdout keywords. Quota state is derived from trusted usage snapshots and persisted account status.
...
```
*`auto` selects the next schedulable account at launch time. It does not switch accounts mid-session based on terminal output text.*
...
▶ gemini
  - Account ID: 1  [Active] [Quota Exhausted] (example@gmail.com) 
  - Account ID: 2  [Active] (work@company.com) 
```

### 7. Refresh usage when state looks stale
If an account state looks stale, refresh its usage snapshot and re-check:
```bash

...
func TestPlannerBuildsFromSelectionCredentialAndStrategy(t *testing.T) {
	...
	if !spec.IsValid() ||
		...
		spec.SelectionSource() != accountapp.LaunchSelectionSourceProviderDefault ||
		spec.Binary() != "codex" {
...
func BenchmarkPlannerBuild(benchmark *testing.B) {
	...
	selection, err := accountapp.NewLaunchSelection(account, accountapp.LaunchSelectionSourceProviderDefault)
	if err != nil {
	...
	request := accountapp.LaunchSelectionRequest{ProviderID: codex.ProviderID}

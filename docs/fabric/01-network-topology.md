...
| Node | 暴露本机项目，运行原生 agent runtime | 公司电脑、家里电脑、开发服务器 |
| Relay Node | 为其他节点转发控制流和会话流 | VPS、家里电脑、公司电脑 |
| Agent Runtime | Codex/Claude/AGY/OpenCode 原生进程 | tmux/PTY/GUI bridge |
...
| Agent Runtime | node 本地 runtime store | tmux/PTY/GUI bridge 子进程 | provider 账号引用或短期 account grant | runtime status、session、capabilities |
...
- 输入、审批、控制消息优先级最高。
- PTY frame 可以降帧、合并、丢弃旧帧；semantic event 不允许丢。

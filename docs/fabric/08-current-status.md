...
- Active Todo（后续有新需求先追加到这里，再按顺序推进）：
  ...
  | 9 | pending | M4 远程开发会话：以 server profile -> node -> project -> runtime -> session 的可理解路径替代旧 M4 专用入口路线 | 8.5 Approval/artifact lanes 已完成；`2026-06-28-m4-approval-artifact-lanes.md` 证明 focused 129/129、full 2583/2583、本地默认 `9527` 真实 Codex 会话产生 `artifact_ref=30` 且 30 个 artifact 均通过 artifact lane 读回，approval response idempotency 已覆盖 | 继续 M4 Todo Queue：下一步做 8.6 AWS current real remote session smoke |

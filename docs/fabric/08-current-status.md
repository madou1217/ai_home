...
- Active Todo（后续有新需求先追加到这里，再按顺序推进）：
  ...
  | 8 | done | M3.5 统一 Node 产品模型：把控制面、远程节点、SSH 开发机、节点健康和 transport candidates 收敛到一个 Node Inventory / Node Detail 模型 | `15-unified-node-product-model.md` 已定义对象模型；`2026-06-28-current-aws-node-model-readback.md` 已证明 AWS 授权 registry readback 为 nodes=2、relayNodes=2、projects=2、runtimes=4、transports=2；`2026-06-28-node-inventory-read-model.md` 已证明 Node Inventory/action gating 落地，AWS node 因缺 provider runtime 被正确阻塞 | M3.5 完成；下一步进入 M4 8.4 event store + seq/ack/resume |
  | 9 | pending | M4 远程开发会话：以 server profile -> node -> project -> runtime -> session 的可理解路径替代旧 M4 专用入口路线 | 8.4 Event store + seq/ack/resume 已完成；`2026-06-28-m4-event-store-seq-ack-resume.md` 证明 focused 109/109、full 2576/2576、本地默认 `9527` 真实会话 cursor resume 无重复且 ack accepted | 继续 M4 Todo Queue：下一步做 8.5 approval and artifact lanes |
  | 10 | pending | M5 Recovery：ack/resume、relay failover、audit events、diagnostics export | broker 同 `serverId` 断开恢复已验证；未覆盖 multi-broker/failover/semantic event 不丢 | kill relay/broker 后 3 秒内恢复，session event 可 resume 且不重复 |
  | 11 | pending | WebRTC DataChannel / WebTransport QUIC / Multipath QUIC promotion | WebRTC signaling pass，但 DataChannel open/RTT 未通过；QUIC/WebTransport 未成 promotion evidence | 用 headed browser、手机/跨机、STUN/TURN 和明确 RTT 指标补证；未达 gate 前不设默认 |
...
- 2026-06-28 M3.5 Node Inventory 第一刀：
  ...
  - 真实 AWS registry payload 跑 read model 后，`aws-current-node` 显示 `projectHost=true`、`relayNode=true`、`runtimeHost=false`，四个 start session action 分别阻塞于 `missing_provider_runtime:codex|claude|agy|opencode` 和 `m4_remote_session_action_pending`。
  - `local-mac-remote-node` 显示 `runtimeHost=true`，provider runtimes 为 `codex/claude/agy/opencode`，start session action 只剩 `m4_remote_session_action_pending`。
...
| 8.4 | done | Event store + seq/ack/resume | `2026-06-28-m4-event-store-seq-ack-resume.md`：新增 session event store、`session-ack` / `device-node-session-ack`，events 带 `seq/cursor`，focused 109/109、full 2576/2576、本地默认 `9527` 真实会话 cursor resume 无重复且 ack accepted | AWS current 部署后只补 ack/proxy 能力 smoke；完整远程开发会话仍归 8.6 |
...
8. 扩展 transport candidates：WebTransport/QUIC、旧 WSS relay metrics、multi-relay failover。

...
- Active Todo（后续有新需求先追加到这里，再按顺序推进）：
  ...
  | 9 | pending | M4 远程开发会话：以 server profile -> node -> project -> runtime -> session 的可理解路径替代旧 M4 专用入口路线 | 8.6 AWS current real remote session smoke 已完成；`2026-06-28-m4-aws-real-remote-session-smoke.md` 证明 AWS default `9527` 上真实 node invite、device pair、relay、Codex session start、event polling、`artifact_ref=13`、artifact fetched=12、marker output、`/quit` 和 abort cleanup 均通过；修复了远程 `device-node-session-start` 丢失 `artifactThreshold` 的真实缺口 | 继续 M4 Todo Queue：下一步做 8.7 Mobile/PWA smoke |

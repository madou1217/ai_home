...
不允许 provider-specific payload 直接污染通用 frame。Codex/Claude/AGY/OpenCode 的 slash 和 raw input 优先走 PTY/native bridge。
...
- Native session 成功必须有真实 provider runtime evidence，不能只用管理接口测试替代。

...
通用，明细以 `category:'detail'` 单列、不参与账户级剩余率）。同一账号下 work/code 读到一致余额，
印证同地区共用一份账户级用量。详见同文件 §14。
...
| GW-010 | `GET /v1/blobs/:id` | 读取暂存的图像 blob；Go 已实现内容寻址 LRU blob 仓、GET 路由与 vision guard 入仓（被剥离的图片 + `response_format=url` 两条链路）。覆盖面对齐 Node：索引收录聚合 Provider 命名空间 + 基座回退（三层解析），未命中但属视觉家族保留图片，其余按纯文本处理 | 受限/迁移中 | `lib/server/image-blob-store.js`、`vision-image-guard.js`、`internal/adapters/imageblob`、`internal/transport/http/blobsapi`、`contracts/route-ownership/manifest.json` |
...
| OPS-019 | OAuth 身份迁移计划与恢复验证 | 显式计划摘要确认、独占锁、备份与提交标记保护 apply/recover/rollback；原生 Codex SQLite 指针作为补偿事务参与者，未知引用仍拒绝。完整真实数据副本已通过迁移及回滚；2026-09-18 生产 4 个身份迁移、3 条原生路径读回、服务恢复通过，剩余迁移 0 | 高级/受控维护 | `scripts/oauth-identity-maintenance.js`、`docs/architecture/account-identity-production-acceptance-2026-09-18.md` |

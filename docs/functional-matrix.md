...
| GW-010 | `GET /v1/blobs/:id` | 读取暂存的图像 blob；Go 已实现内容寻址 LRU blob 仓、GET 路由与 vision guard 入仓（被剥离的图片 + `response_format=url` 两条链路）。残留：guard 只对离线模态索引里存在的模型生效，未收录 Provider 失败开放不剥离 | 受限/迁移中 | `lib/server/image-blob-store.js`、`vision-image-guard.js`、`internal/adapters/imageblob`、`internal/transport/http/blobsapi`、`contracts/route-ownership/manifest.json` |

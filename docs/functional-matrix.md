...
| GW-010 | `GET /v1/blobs/:id` | 读取暂存的图像 blob；Go 已实现内容寻址 LRU blob 仓、GET 路由与 vision guard 入仓（被剥离的图片 + `response_format=url` 两条链路）。覆盖面对齐 Node 兜底语义：索引命中为准，未命中但属视觉家族保留图片，其余按纯文本处理；残留是映射覆盖的 Provider 数少于 Node | 受限/迁移中 | `lib/server/image-blob-store.js`、`vision-image-guard.js`、`internal/adapters/imageblob`、`internal/transport/http/blobsapi`、`contracts/route-ownership/manifest.json` |

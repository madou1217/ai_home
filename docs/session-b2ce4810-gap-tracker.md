...
| B32 | codex 协议请求路由到 grok 账号时 500 `protocol_gateway_unavailable` | 2026-09-11 实测(harness 会话 pid 53382 钉 grok 账号 `acct_c7f34088bb80f668f95e`) | ❌ 未修 | **根因已定位**:`protocol-fallback-bridge.js:779` 的 `FALLBACK_PROTOCOL_GATEWAY_RUNNERS` 只注册 `gemini_generate_content`/`openai_responses`/`openai_chat`/`gemini_stream_generate_content`,**缺 `anthropic_messages` runner**;grok 的回落协议正是它,`resolveFallbackProtocolGateway` 取不到运行器 → `:830` 直接 500。已排除「模型不匹配」:用 grok 自己的 `grok-4.6` 复测同样 500。修复需新写 anthropic 网关 runner(新功能,非几行改动),故未在本轮动手 |

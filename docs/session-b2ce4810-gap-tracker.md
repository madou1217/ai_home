...
| B33 | agy 经 `/v1/chat/completions` 收不到图片输入 | 2026-09-12 实测(能力矩阵补测) | ❌ 未修 | claude 同请求同图答 `Blue`,agy 答 `NOIMAGE`;1×1 与 48×48 两种图一致。已排除适配器缺失(`protocol-canonical.js:80` openai→canonical、`:288` canonical→gemini inlineData 两向俱在);**未查明 agy 实际走哪条路径、在哪步丢件**。证据见 `docs/architecture/provider-capability-matrix.md` §3.1 |

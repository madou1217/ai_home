...
| F20 | dsh 2.0 交互与能力吸收清单（分支 Diff 已剔除） | ✅ 行为级已验(§二十七/§二十八) | 保留 11 链路全部 Playwright 行为实测通过:Cmd+K/手册/Cmd+F(本次补真)/灵感胶囊(本次移植 native)/悬浮操作栏/长图分享/ContextMeter/微光扫描/置顶/跨 Tab 同步(B30 修复后复验通过)/首字渲染(实测流式上屏无 UI 停滞,温路径首字 2.7s)。边界:移动端触摸入口已补(§二十九);后续吸收专题在 chat-harness-absorption.md 单列,不属本条目 |
...
| B32 | codex 协议请求路由到 grok 账号时 500 `protocol_gateway_unavailable` | 2026-09-11 实测(harness 会话 pid 53382 钉 grok 账号) | ✅ 已修 | **真因不是缺 anthropic runner**:`PROVIDER_FALLBACK_REQUEST_PROTOCOLS` 只列 codex/kimi/gemini/agy,**漏了 grok**,于是它落到客户端协议通用回落表,首位 `anthropic_messages`——而 grok 在该协议下无任何 provider 路由(实测四种协议均 null),桥接 500。按 `docs/architecture/grok-provider-boundaries.md` §2:grok 的 Build(CLI/OAuth) 与 API Key 边界**都走 api.x.ai/v1 REST、OpenAI 兼容**,故补 `grok: ['openai_chat']`。真实网关实测:`grok-4.6` 由 500 变为 **402 personal-team-blocked:spending-limit**(桥接打通、真到 api.x.ai,是该 team 额度受限)、`gpt-5.5` 变为 **503「no available grok account can serve model gpt-5.5」**(语义正确)。回归测试 2 项 + 全量 6587 项 0 fail |
| B33 | agy 经 `/v1/chat/completions` 收不到图片输入 | 2026-09-12 实测(能力矩阵补测) | ➖ 不适用(按用户决定不再跟进) | claude 同请求同图答 `Blue`,agy 答 `NOIMAGE`;1×1 与 48×48 两种图一致。已排除适配器缺失(`protocol-canonical.js:80` openai→canonical、`:288` canonical→gemini inlineData 两向俱在);**未查明 agy 实际走哪条路径、在哪步丢件**。证据见 `docs/architecture/provider-capability-matrix.md` §3.1。**2026-09-12 复核后收窄并结案(非修复)**:64×64 四象限图经 chat harness 路径实测,`gemini-3.8-flash-high` 与 `gemini-2.5-flash-thinking`(B33 当初失败的同一模型)均 4/4 答对,故**不是模型特异、也不影响 WebUI 用户路径**;请求构建层亦在进程内逐字节排除(normalizeOpenAIContentParts→canonical→canonicalPartsToGeminiParts→addGeminiContent 全链保留 inlineData)。`/v1/chat/completions` 端到端**未复测**——该端点只认 gateway client key,本次取不到。按用户决定不再跟进,标 ➖ 而非 ✅:它不是已修,是已排除。 |
...
## 二十九、检索胶囊移动端触摸入口(2026-09-11)

移动端无键盘,Cmd+F 触达不到。移动端 chat 头部(返回 / 标题 / 操作区)新增搜索钮,
派发与 Cmd+F 相同的 `IN_SESSION_SEARCH_OPEN_EVENT`——两个 surface 的既有监听器直接复用,
零逻辑分叉。配套把检索胶囊在 ≤768px 下改为通栏 + 输入框弹性收缩(原固定 220px 在 390px 溢出)。

**实测(390×844)**:搜索钮可见可点 → 胶囊打开且几何贴合视口(left 12 / right 378 / vw 390)→
「候鸟」会话搜 `候鸟` 命中 `1/4`,Enter/按钮切换匹配正常;「无匹配」态在不含关键词的会话亦正确显示。
`npm run build` exit 0,eslint 0 警告。

## 二十八、三项收尾核查(2026-09-11):全部无需改动

追踪清单归零后,对自己提出的三个候选项逐一核实,结论是**都不需要动代码**——记录下来以免重复排查。

| 候选项 | 核实方式 | 结论 |
|---|---|---|
| 全量测试 2 项红(`account-usage-view.claude`、`server.codex-desktop-hook`) | 用独立 `git worktree` 检出 HEAD 单独跑:**28/28 全过** | 仅存在于并发会话**未提交**的改动中,非已提交回归;CI 测已提交代码,不受影响。**不属本会话,亦不应代改** |
| grok 账号仍不可用 | B32 修好协议桥后实测返回 `402 personal-team-blocked:spending-limit` | 桥接已通、请求真达 api.x.ai;这是该 grok team 的**计费额度**状态,不是网关缺陷 |
| 账号表「剩余额度」列进度条溢出列边界 | 放大截图(`tmp/p5/_quota-col.png`)核对 | **误报**:进度条完全在列内,未越过与「操作」列的分隔线。整页截图缩放下的错觉 |

第三项是本轮第三次「我的观察本身有误」(前两次:容器 font-size 造成的「每页 22px」幻影、把合法的 `--hos-mobile-radius: 18px` 报成圆角异常)。
**规律很稳定:任何"看起来不对"的结论,在动手前都必须用测量或放大核实一次。**

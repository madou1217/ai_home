// Package attemptfailure 把上游低敏分类映射为推理应用层失败结果。
...
// NewClassified 创建不携带 Provider 正文的本地稳定失败。
func NewClassified(
...
// NewTransport 按 Go 错误身份分类，不读取错误文本。
func NewTransport(
	err error,
) (inferencegateway.AttemptFailure, error) {
	classification, classifyErr :=
		sharedfailure.ClassifyTransportError(err)
	if classifyErr != nil {
		return inferencegateway.AttemptFailure{},
			sharedfailure.ErrInvalidClassification
	}
...
// NewIncompleteStream 区分超时、取消、重置和普通断流。
func NewIncompleteStream(
	err error,
) (inferencegateway.AttemptFailure, error) {
	classification, classifyErr :=
		sharedfailure.ClassifyIncompleteStream(err)
	if classifyErr != nil {
		return inferencegateway.AttemptFailure{},
			sharedfailure.ErrInvalidClassification
	}
...
// SafeMessage 返回稳定中文说明，禁止拼接 Provider 原文。
func SafeMessage(kind runtimecore.FailureKind) string {
	switch kind {

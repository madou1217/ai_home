...
var errInvalidPreCommitFailure = errors.New("提交前 Canonical 失败事件无效")
...
// RenderFailed 表示 Canonical 事件无法按 Gemini 合同表达。
...
func (stream *responseStream) Finish(failure streamFailure) {
	...
	if !stream.stream.Committed() {
		writeError(stream.response, failure.status, failure.code, failure.message)
		return
	}

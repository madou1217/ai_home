...
const PROVIDER_API_KEY_ENV = {
  ...
  kiro: [],
  zcode: ['ZCODE_API_KEY'],
  // CodeBuddy 家族：国际站 / 国内站 / WorkBuddy 共用同一组 env 凭据键
  // （同一个 CLI 二进制与同一份 CodeBuddy Code runtime），但账号体系互不相通，
...
// CodeBuddy 家族（国际站 / 国内站 / WorkBuddy）：原生凭据载荷形状相同
// （credentials/oAuth 或共享 .info 的 account + auth），但身份种子前缀按站点区分。
const CODEBUDDY_FAMILY_PROVIDERS = Object.freeze(['codebuddy', 'codebuddycn', 'workbuddy']);

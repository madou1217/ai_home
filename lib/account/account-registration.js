...
function registerAccountIdentity(fs, aiHomeDir, input = {}, deps = {}) {
  ...
  try {
    ...
    const deletionKey = `account:deleted:${accountRef}`;
    if (input.respectDeletion === true
      && db.prepare('SELECT key FROM app_kv WHERE key = ?').get(deletionKey)) {
      const error = new Error('account_deleted_by_user');
      error.code = 'account_deleted_by_user';
      throw error;
    }
    // Explicit user registration is allowed to restore a previously removed
    // identity; an automatic observation is not equivalent to that intent.
    db.prepare('DELETE FROM app_kv WHERE key = ?').run(deletionKey);

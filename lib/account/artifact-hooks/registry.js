...
const { GrokArtifactHookStrategy } = require('./grok');
const { KimiArtifactHookStrategy } = require('./kimi');
const { ZcodeArtifactHookStrategy } = require('./zcode');
...
const DEFAULT_PROVIDER_STRATEGY_FACTORIES = Object.freeze([
  ...
  {
    provider: 'grok',
    create: (options) => new GrokArtifactHookStrategy(options)
  },
  {
    provider: 'kimi',
    create: (options) => new KimiArtifactHookStrategy(options)
  },
  {
    provider: 'zcode',
    create: (options) => new ZcodeArtifactHookStrategy(options)
  }
]);

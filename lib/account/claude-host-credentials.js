...
function createClaudeHostCredentialReconciler(deps = {}) {
  ...
  // Native `claude` leaves CLAUDE_CONFIG_DIR unset. Passing ~/.claude here
  // would address a different, hashed Keychain service than the native CLI.
  ...
  return function reconcileClaudeHostCredentials(record) {
    ...
    return projectDatabaseCredentials(record, credentials, reason);
  };
}

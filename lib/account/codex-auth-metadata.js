...
// Go's. These extra code points close that gap.
...
// Mirrors Go's `firstNonEmpty` (core/accounts/codex/jwt.go:292): return the
// first value whose **Go-trimmed** form is non-empty, and return it *trimmed*.
//
// Both details matter and neither is what `firstNonEmptyString` does:
//   - Go trims with `unicode.IsSpace`, so a NEL-only value counts as empty;
//   - Go returns the trimmed value, so the seed never carries stray whitespace.
// Using the JS-trimming helper here would both pick the wrong candidate and
// emit an untrimmed seed.
...
// `hasControlCharacter` (core/accounts/codex/oauth.go:273): non-empty after a
// Go-style trim, no `:` (it would make the seed's colon-separated format
// ambiguous), no U+FFFD (Go rejects `utf8.RuneError`), no control characters.
//

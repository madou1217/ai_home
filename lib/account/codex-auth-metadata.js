...
function extractCodexMetadata(authJson) {
  ...
  return {
    ...
    upstreamAccountId: resolveCodexWorkspaceFields(authJson).upstreamAccountId,
...
// 工作区字段：Go 能接受的凭据严格按 Go 语义（见 codex-workspace.js），两端发送同一个
// chatgpt-account-id；Go 会整份拒绝的凭据（缺 id_token、工作区冲突等）Go 根本不承接，
// Node 保留旧取值链继续服务并通过 workspaceError 暴露原因，避免团队账号在 Node 侧静默丢头。
function resolveCodexWorkspaceFields(authJson) {
  const workspace = resolveCodexWorkspace(authJson);
  if (workspace.ok) {
    return { upstreamAccountId: workspace.upstreamAccountId, workspaceId: workspace.workspaceId, workspaceError: '' };
  }
  ...
  const accessPayload = decodeJwtPayloadUnsafe(tokens.access_token);
  const idPayload = decodeJwtPayloadUnsafe(tokens.id_token);
  const authClaim = (accessPayload && accessPayload[CODEX_AUTH_CLAIM_NAMESPACE])
    || (idPayload && idPayload[CODEX_AUTH_CLAIM_NAMESPACE]) || {};
  const legacy = String(authClaim.chatgpt_account_id || source.chatgpt_account_id || tokens.account_id || '').trim();
  return { upstreamAccountId: legacy, workspaceId: '', workspaceError: workspace.error };
}

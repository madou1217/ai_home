...
`;
  ...
  function protectNewerKimiHostAuth(nativeAuth, record) {
    const incomingCredentials = nativeAuth && nativeAuth.credentials;
  ...
  function syncPolicyAuthArtifactsToHost(cliName, nativeAuth, record) {
    ...
    if (cliName === 'kimi') {
      const protection = protectNewerKimiHostAuth(nativeAuth, record);
      if (protection) return protection;
    }
    const writtenFiles = [];
    for (const artifact of artifacts) {
      const value = nativeAuth && nativeAuth[artifact.field];

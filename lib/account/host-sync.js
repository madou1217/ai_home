...
function createHostConfigSyncer(deps) {
  ...
  function patchCodexConfigToml(hostGlobalDir, accountId, apiKeyMode) {
    ...
    const providerKey = 'aih';
    const providerName = 'aih codex';
    ...
    if (apiKeyMode) {
      ...
      const providerBlock = `[model_providers.${providerKey}]\nname = "${providerName}"\nbase_url = "${baseUrl}"\nbearer_token = "dummy"\nwire_api = "responses"`;
      
      const blockRegex = new RegExp(`\\[model_providers\\.${providerKey}\\][\\s\\S]*?(?=\\n\\[|$)`);

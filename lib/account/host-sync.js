...
function createHostConfigSyncer(deps) {
  ...
  function patchCodexConfigToml(hostGlobalDir, apiKeyMode, openaiBaseUrl = '', openaiApiKey = '') {
    ...
    if (apiKeyMode) {
      ...
      // auth 命令表形态：scripts/aih-codex-provider-auth.js 三级取 key
      // （OPENAI_API_KEY 直通 -> 账号 ref -> 默认账号），裸跑 codex 闭环，
      // 且与 env_key 互斥（不能同时存在）。
      const providerBlock = getManagedAihProviderBlock({
        ...
        authCommand: {
          ...
          scriptPath: path.join(__dirname, '..', '..', 'scripts', 'aih-codex-provider-auth.js')
        }
      });
  ...
  return function syncGlobalConfigToHost(cliName, accountRef) {
    ...
    if (cliName === 'codex') {
      try {
        ...
        if (apiKeyMode && !(credentialRecord.nativeAuth && credentialRecord.nativeAuth.auth)) {
          const backup = backupHostGlobalConfig('codex', hostGlobalDir, 3);

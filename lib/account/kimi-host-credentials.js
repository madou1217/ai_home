...
function createKimiHostCredentialReconciler(deps = {}) {
  ...
  return function reconcileKimiHostCredentials(accountRef) {
    ...
    const currentCredentials = record.nativeAuth && record.nativeAuth.credentials;
    ...
    const currentDeviceId = String(record.nativeAuth && record.nativeAuth.deviceId || '').trim()
      || readKimiTokenDeviceId(currentCredentials);

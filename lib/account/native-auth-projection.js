...
function hasUsableProviderProjection(provider, artifacts, source) {
  if (!hasRequiredProjectionData(artifacts, source)) return false;
  if (provider === 'kimi') return hasUsableKimiOAuth(source && source.credentials);
  return true;
}
...
function materializeProviderAuth(fs, runtimeDir, provider, options = {}) {
  ...
  if (context.provider === 'kimi' && hasRequiredData) {
    const deviceId = String(nativeAuth.deviceId || '').trim()
      || deriveKimiDeviceId(nativeAuth.credentials);
...
function registerProviderAuthProjection(fs, runtimeDir, provider, options = {}) {
  ...
  writeAccountNativeAuth(fs, aiHomeDir, accountRef, { ...current, ...persistedProjection });
  return {
    registered: true,

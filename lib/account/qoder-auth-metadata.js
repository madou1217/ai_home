...
const { readSubjectAliases } = require('./identity-subject');
...
function extractQoderIdentityFields(authPayload) {
  ...
  return { email, username, uid, token, loginMethod: firstNonEmptyString(payload.login_method, payload.loginMethod) };
}

/** Qoder email/Welcome labels are display data; only the native UID names an OAuth account. */
function buildQoderIdentitySeed(provider, authPayload) {
  const variant = getQoderVariant(provider);
  if (!variant || !authPayload || typeof authPayload !== 'object') return '';
  const fields = extractQoderIdentityFields(authPayload);
  const identity = readSubjectAliases(authPayload, ['uid', 'user_id', 'userId', 'account_id', 'accountId', 'id']);
  return fields.token && identity.valid && identity.subject
    ? `oauth:${variant.provider}:uid:${identity.subject}` : '';
}
...
function extractQoderLoginProjectionMetadata(provider, outputRaw) {
  ...
  const match = output.match(/Login successful!\s+Welcome,\s*([^\r\n\u001b]+)/i);
  const identity = String(match && match[1] || '').trim().replace(/[.!]+$/, '').trim();
  if (!identity) return {};
  return identity.includes('@')
    ? { userInfo: { email: identity.toLowerCase() } }
    : { userInfo: { username: identity.toLowerCase() } };
}
...
function resolveQoderNativeAuthPayload(provider, nativeAuth) {
  ...
  const metadata = source.userInfo && typeof source.userInfo === 'object' ? source.userInfo : null;
  const encrypted = source.credentials;
  const salt = source.keychainSalt;
  if (typeof encrypted === 'string' && encrypted.trim()) {
    const payload = decryptQoderCredentials(encrypted, salt, variant.credentialPrefix);
    if (!payload) return null;
    const recorded = readSubjectAliases(metadata, ['uid', 'user_id', 'userId', 'account_id', 'accountId', 'id']);
    const actual = readSubjectAliases(payload, ['uid', 'user_id', 'userId', 'account_id', 'accountId', 'id']);
    if (!recorded.valid || !actual.valid || (recorded.subject && actual.subject && recorded.subject !== actual.subject)) return null;
    // A console Welcome label cannot replace the credential's UID or token.
...
function summarizeQoderAuth(provider, nativeAuth, options = {}) {
  ...
  if (!payload) {
    ...
    // An opaque official credential may not be locally decodable. Its recorded
    // login label is still valid display metadata, never an enrollment identity.
    const label = extractQoderIdentityFields(nativeAuth && nativeAuth.userInfo);
    const opaque = typeof nativeAuth?.credentials === 'string' && !!nativeAuth.credentials.trim();
    return { configured: opaque, accountName: label.email || label.username || 'Unknown',
      authMode: 'oauth', email: label.email || '' };
  }
  const fields = extractQoderIdentityFields(payload);
  let accountName = 'Qoder Account';
  if (fields.email) accountName = fields.email;
  else if (fields.username) accountName = fields.username;

...
function processQuotaObservation(db, accountRef, provider, obs) {
  ...
  // Detected when current expected reset timestamp has moved past previous expected reset timestamp
  // OR when now has passed previous expected reset timestamp and quota recovered
  ...
  const isCycleRollover = (resetTimestampAdvanced && now >= prevResetAt - TOLERANCE_WINDOW_MS) || naturalResetTimePassed;

  if (isCycleRollover) {
    // Exact physical reset time is the scheduled previousExpectedResetAtMs (e.g. 23:00)
    // even if observed later at 24:00

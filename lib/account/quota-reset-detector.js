...
const { getQuotaResetStrategy } = require('./quota-reset/strategy-registry');
...
function extractQuotaObservations(provider, snapshot) {
  ...
  const strategy = getQuotaResetStrategy(normalizedProvider);
  return strategy.extractObservations(snapshot, capturedAt);
}
...
function processQuotaObservation(db, accountRef, provider, obs) {
  ...
  const normalizedCurrentPlan = strategy.normalizePlanType(planType);
  ...
  if (!prevState) {
    ...
    upsertDetectorState(db, {
      ...
      lastPlanType: normalizedCurrentPlan,
      isArmed: isArmed ? 1 : 0,
  ...
  const prevPlan = strategy.normalizePlanType(prevState.lastPlanType);
  const currentPlan = normalizedCurrentPlan;
  const isPlanUpgraded = strategy.isPlanUpgrade(prevPlan, currentPlan);
  ...
  if (isCycleRollover) {
    ...
    const eventKey = sha256Key(
      `${accountRef}:${normalizedProvider}:${quotaKey}:cycle:${prevResetAt}:${expectedResetAtMs || occurredAtMs}`
    );
    generatedEvent = {
      eventKey,
      accountRef,
    ...
    const eventKey = sha256Key(
      `${accountRef}:${normalizedProvider}:${quotaKey}:rearm:${prevState.rearmGeneration}:${Math.floor(now / 60000)}`
    );
    generatedEvent = {
      eventKey,
      accountRef,

...
function ensureQuotaResetTables(db) {
  ...
  try {
    ...
    if (detectorCols.length > 0 && !detectorCols.includes('last_plan_type')) {
      db.exec(`ALTER TABLE ${DETECTOR_STATE_TABLE} ADD COLUMN last_plan_type TEXT`);
    }
    if (eventCols.length > 0 && !eventCols.includes('previous_plan_type')) {
      db.exec(`ALTER TABLE ${RESET_EVENTS_TABLE} ADD COLUMN previous_plan_type TEXT`);
    }
    if (eventCols.length > 0 && !eventCols.includes('current_plan_type')) {
      db.exec(`ALTER TABLE ${RESET_EVENTS_TABLE} ADD COLUMN current_plan_type TEXT`);
    }
  } catch (_migrationErr) {}
...
function mapEventRow(row) {
  ...
  return {
    ...
    previousPlanType: text(row.previous_plan_type) || null,
    currentPlanType: text(row.current_plan_type) || null
  };
}
...
function mapDetectorStateRow(row) {
  ...
  return {
    ...
    lastPlanType: text(row.last_plan_type) || null,
...
function upsertDetectorState(db, state) {
  db.prepare(`
    INSERT INTO ${DETECTOR_STATE_TABLE} (
      ...
      last_expected_reset_at_ms, last_captured_at_ms, last_plan_type,
      is_armed, rearm_generation, exhausted_at_ms, updated_at_ms
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
function insertResetEvent(db, event) {
  ...
  return db.prepare(`
    INSERT INTO ${RESET_EVENTS_TABLE} (
      ...
      previous_expected_reset_at_ms, detected_at_ms, early_duration_ms,
      exhausted_at_ms, occurred_at_ms, previous_plan_type, current_plan_type
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  `).run(
    ...
    occurredAtMs,
    event.previousPlanType || null,
    event.currentPlanType || null
  );
}

...
function ensureQuotaResetTables(db) {
  ...
  // Auto-migrate columns if tables exist from earlier version
  try {
  ...
  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_quota_reset_events_account_time
      ON ${RESET_EVENTS_TABLE} (account_ref, detected_at_ms DESC, id DESC)
  `);
}
...
function mapEventRow(row) {
  ...
  return {
    ...
    detectedAtMs: Number(row.detected_at_ms) || 0,
...
function insertResetEvent(db, event) {
  return db.prepare(`
    INSERT INTO ${RESET_EVENTS_TABLE} (
      ...
      previous_expected_reset_at_ms, exhausted_at_ms, detected_at_ms, early_duration_ms
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
function listAccountQuotaResetEvents(fs, aiHomeDir, accountRef, options = {}) {
  ...
  return withDatabase(fs, aiHomeDir, (db) => {
    ...
    sql += ' ORDER BY detected_at_ms DESC, id DESC LIMIT ? ';
    params.push(limit);

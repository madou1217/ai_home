...
function readRuntimeState(value) {
  ...
  const nested = value.runtimeState || value.runtime_state;
...
function hasPersistedRuntimeSource(value) {
  ...
  if (Object.prototype.hasOwnProperty.call(value, 'runtime_state')) return true;
...
function formatRuntimeStatusSummary(status, accountId) {
  return `account ${String(accountId || '').trim()} ${formatRuntimeStatusText(status)}`;
}

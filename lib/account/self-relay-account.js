...
const { deleteAccountRef } = require('../server/account-ref-store');
...
function deleteSelfRelayAccounts(options = {}) {
  ...
  providers.forEach((provider) => {
    listAccountCredentialRecords(fs, storageDir, provider).forEach((record) => {
      ...
      deleteAccountUsageSnapshot(fs, storageDir, record.accountRef);
      deleteTransferMetadata(fs, storageDir, record.accountRef);
      const stateDeleted = accountStateService && typeof accountStateService.deleteAccount === 'function'
        ? accountStateService.deleteAccount(record.accountRef)
        : false;
      const accountDeleted = deleteAccountRef(fs, storageDir, record.accountRef, { bestEffort: true });
      if (accountDeleted && typeof fs.rmSync === 'function') {
        [
          resolveAccountRuntimeDir(storageDir, provider, record.accountRef),
          provider === 'codex' ? resolveCodexDesktopRuntimeDir(storageDir, record.accountRef) : ''
        ].filter(Boolean).forEach((runtimeDir) => {

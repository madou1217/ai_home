...
const SUPPORTED_STANDARD_PROVIDERS = Object.freeze(['codex', 'gemini', 'claude', 'agy', 'opencode']);
...
function buildFlatAccountExportFileName(record) {
  ...
  if (provider === 'opencode') {
    const identityKey = buildOpenCodeAuthUniqueKey(record && record.auth);
    const suffix = getPublicAccountRefSuffix(identityKey);
    if (!identityKey || !suffix) return null;
    return {
...
function buildStandardOAuthIdentity(provider, auth) {
  ...
  if (normalizedProvider === 'opencode') return buildOpenCodeAuthUniqueKey(auth);
...
function writeOAuthAccount({ fs, path, provider, accountId, auth, getProfileDir, getToolConfigDir }) {
  ...
  if (provider === 'opencode') {
    writeJsonFile(fs, path, path.join(profileDir, '.local', 'share', 'opencode', 'auth.json'), auth);
  }
}

...
function buildFlatAccountExportFileName(record) {
  ...
  if (provider === 'opencode') {
    // Export preserves an already registered grant; re-enrollment validation is
    // deliberately separate so legacy users never lose the ability to back up.
    if (!record.auth || typeof record.auth !== 'object') return null;
    const suffix = getAccountRefSuffix(record.accountRef);
    if (!suffix) return null;
    return {
  ...
  if (provider === 'kimi') {
    if (!hasUsableKimiOAuth(record && record.auth)) return null;
    const suffix = getAccountRefSuffix(record.accountRef);
    if (!suffix) return null;
    return {
      fileName: `${provider}_auth_${suffix}.json`
...
function buildStandardOAuthIdentity(provider, auth) {
  ...
  if (normalizedProvider === 'kimi') return buildKimiIdentitySeed(auth);
...
function findExistingAccountByIdentity({ fs, aiHomeDir, provider, identityKind, identityPayload }) {
  ...
  if (direct || provider !== 'kimi' || identityKind !== 'oauth') return direct;
...
function writeApiKeyAccount({ fs, aiHomeDir, provider, accountRef, config }) {
  ...
  if (provider === 'codex') {
    ...
    if (config.baseUrl) env.KIMI_BASE_URL = config.baseUrl;
  }
...
function normalizeOAuthAuth(provider, account) {
  ...
  if (provider === 'kimi') {
    return normalizeKimiOAuthAuth(account);
  }
  if (provider === 'opencode') {
...
function importStandardAccountRecords(options = {}) {
  ...
  for (const account of (Array.isArray(records) ? records : [])) {
    try {
      ...
      if (!auth
        || (provider === 'kimi' && !hasUsableKimiOAuth(auth))

...
function normalizeKimiOAuthAuth(account) {
  ...
  const auth = readNestedObject(payload, 'auth');
  const credentials = readNestedObject(payload, 'credentials');
  const value = (...keys) => firstNonEmptyString(
    ...keys.flatMap((key) => [credentials[key], auth[key], payload[key]])
  );
  const numericOrString = (...keys) => {
    for (const key of keys) {
      for (const source of [credentials, auth, payload]) {
...
function buildFlatAccountExportFileName(record) {
  ...
  if (provider === 'kimi') {
    if (!buildKimiIdentitySeed(record && record.auth)) return null;
    const suffix = getAccountRefSuffix(record.accountRef);
    if (!suffix) return null;
    return {
      fileName: `${provider}_auth_${suffix}.json`

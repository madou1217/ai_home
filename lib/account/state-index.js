...
function createAccountStateIndex(options = {}) {
  ...
  function upsertAccountState(provider, accountId, state = {}) {
    ...
    const normalizedRemaining = normalizeRemainingPct(state.remainingPct);
  ...
  function getNextCandidateId(provider, excludedId = '') {
    ...
    const candidates = listStates(p)
      ...
      .filter((row) => row.accountId !== excluded)
      .filter((row) => !isBlockingRuntimeStatus(deriveRuntimeStatus(row)));
    candidates.sort((a, b) => {
      const ar = Number.isFinite(Number(a.remainingPct)) ? Number(a.remainingPct) : -1;
      const br = Number.isFinite(Number(b.remainingPct)) ? Number(b.remainingPct) : -1;
  ...
  function getAccountState(provider, accountId) {
    ...
    return {
      ...
      remaining_pct: normalizeRemainingPct(row.remaining_pct),

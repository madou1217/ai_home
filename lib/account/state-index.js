...
function createAccountStateIndex(options = {}) {
  ...
  const upsertStmt = db.prepare(`
    INSERT INTO account_state (
      provider, account_id, configured, api_key_mode, auth_mode, exhausted, remaining_pct, display_name, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  const upsertRuntimeStateStmt = db.prepare(`
    INSERT INTO account_state (
      provider, account_id, configured, api_key_mode, auth_mode, runtime_state, exhausted, remaining_pct, display_name, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(provider, account_id) DO UPDATE SET
  ...
  const listRowsStmt = db.prepare(`
    SELECT provider, account_id, configured, api_key_mode, auth_mode, runtime_state, exhausted, remaining_pct, display_name, updated_at
    FROM account_state
    WHERE provider = ?
    ORDER BY CAST(account_id AS INTEGER) ASC
  `);

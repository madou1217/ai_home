...
function buildOAuthIdentity(provider, auth) {
  const p = normalizeProvider(provider);
  if (!p) return '';
  // Codex OAuth uses the stable user id, not the email: email changes, and
  // accountRef must not change with it (§8.1). Byte-identical to Go's vector;
  // see docs/architecture/codex-oauth-identity-vector-adr.md.
  if (p === 'codex') return buildCodexOAuthIdentitySeed(auth);
  if (p === 'grok') return buildGrokIdentitySeed(auth && auth.auth || auth);

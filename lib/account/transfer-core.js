...
function normalizeImportProviderAlias(value) {
  ...
  if (provider === 'qodercli' || provider === 'qoder-cli') return 'qoder';
  if (provider === 'qoderclicn' || provider === 'qoder-cli-cn' || provider === 'qoder_cn' || provider === 'qoder-cn') return 'qodercn';
  if (provider === 'moonshot' || provider === 'kimi-code' || provider === 'moonshot-ai') return 'kimi';
  if (
    provider === 'codex'
    || provider === 'claude'
    || provider === 'gemini'
...
function buildOAuthIdentity(provider, auth) {
  ...
  if (p === 'codex') return buildCodexOAuthIdentitySeed(auth);
  if (p === 'claude') {
  ...
  if (p === 'grok') return buildGrokIdentitySeed(auth.auth || auth);
  if (p === 'opencode') return buildOpenCodeIdentitySeed(auth.auth || auth);
  if (p === 'kimi') return buildKimiIdentitySeed(auth.credentials || auth.auth || auth);
  if (p === 'zcode') return buildZcodeIdentitySeed(auth.credentials || auth);
  if (p === 'kiro') return buildKiroIdentitySeed(auth);
  if (['codebuddy', 'codebuddycn', 'workbuddy', 'workbuddycn'].includes(p)) {
    return buildCodebuddyIdentitySeed(p, auth.credentials || auth);
  }
  if (p === 'qoder' || p === 'qodercn') return buildQoderIdentitySeed(p, auth.userInfo || auth);
  if (p === 'agy' || p === 'gemini') {
    const email = normalizeEmailComponent(extractOAuthEmail(p, auth));
    return email ? `oauth:${p}:${email}` : '';
  }
  return '';
}
...
function extractProviderApiKey(p, payload, config, credentials, authCredentials, auth) {
  ...
  if (p === 'kimi') {
    const apiKey = firstNonEmptyString(
      payload.MOONSHOT_API_KEY,
      config.MOONSHOT_API_KEY,
      credentials.MOONSHOT_API_KEY,
      authCredentials.MOONSHOT_API_KEY,
...
function readAccountExportRecord({ provider, accountRef, aiHomeDir, fs }) {
  ...
  if (p === 'kimi') {
    ...
    base.auth = authJson;
    base.meta = {
      userId: String(authJson.user_id || '').trim(),
      deviceId: String(authJson.device_id || '').trim(),
  ...
  if (p === 'qoder' || p === 'qodercn') {
    const {
      resolveQoderNativeAuthPayload,
      extractQoderIdentityFields
    } = require('./qoder-auth-metadata');
    const authJson = resolveQoderNativeAuthPayload(p, nativeAuth) || {};
    const fields = extractQoderIdentityFields(authJson);
    base.auth = authJson;
    base.meta = {
      email: fields.email || '',
      uid: fields.uid || '',
      authMode: fields.loginMethod || 'oauth',
      credentialKind: resolveAccountExportCredentialKind({ ...base, auth: authJson, meta: { email: fields.email } })
    };
    return base;
  }
...
function resolveAccountExportCredentialKind(record) {
  ...
  if (provider === 'kimi') {
    return hasUsableKimiOAuth(auth) ? 'oauth' : '';
  }
  if (provider === 'claude') {

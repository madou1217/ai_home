...
const { hasUsableKimiOAuth } = require('./kimi-auth');
const {
...
function readAccountExportRecord({ provider, accountRef, aiHomeDir, fs }) {
  ...
  if (p === 'kimi') {
    ...
    const authJson = {
      ...credentials,
      ...(nativeAuth.deviceId ? { device_id: nativeAuth.deviceId } : {})
    };
    const expiresAt = Number(authJson.expires_at);
    base.auth = authJson;
    base.meta = {
      ...
      expiresAt: Number.isFinite(expiresAt) && expiresAt > 0
        ? expiresAt * 1000
        : parseJwtExpiryMs(authJson.access_token) || null
    };
    base.meta.credentialKind = resolveAccountExportCredentialKind(base);
    return base;
  }

...
function buildApiKeyIdentity(provider, config) {
  ...
  const prefix = p === 'claude' && extracted.credentialType === 'auth-token' ? 'auth_token' : 'api_key';
  return `${prefix}:${p}:${normalizeBaseUrl(extracted.baseUrl)}:${extracted.apiKey}`;
}
...
function readAccountExportRecord({ provider, accountId, profileDir, configDir, fs }) {
  ...
  if (p === 'gemini') {
    const authJson = parseJsonFileSafe(fs, path.join(profileDir, '.gemini', 'oauth_creds.json')) || {};
    base.auth = authJson;
    base.meta = {
  ...
  if (p === 'claude') {
    const authJson = parseJsonFileSafe(fs, path.join(configDir, '.credentials.json')) || {};
  ...
  if (p === 'agy') {
    const authJson = parseJsonFileSafe(fs, path.join(configDir, 'antigravity-oauth-token')) || {};
  ...
  if (p === 'opencode') {
    const authJson = parseJsonFileSafe(fs, path.join(profileDir, '.local', 'share', 'opencode', 'auth.json')) || {};
    base.auth = authJson;
    base.meta = {
...
function looksLikeAntigravityAccountRecord(value) {
  ...
  if (payload.provider || payload.platform || payload.auth || payload.config || payload.credentials || payload.accountId) return false;

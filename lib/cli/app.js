...
const { writeAccountStatusFile } = require('../account/status-file');
...
function setAccountOperationalStatus(cliName, accountId, status) {
  ...
  const accountStateIndex = getAccountStateIndex();
  const currentState = accountStateIndex.getAccountState(cliName, accountId) || {};
  const profileDir = getProfileDir(cliName, accountId);
  if (currentState && currentState.account_id && typeof accountStateIndex.setStatus === 'function') {
    const updated = accountStateIndex.setStatus(cliName, accountId, normalizedStatus);
    if (updated) writeAccountStatusFile(fs, profileDir, normalizedStatus);
    return updated;
  }
  const liveStatus = checkStatus(cliName, profileDir) || {};
  const updated = accountStateIndex.upsertAccountState(cliName, accountId, {
    ...
    remainingPct: currentState.remaining_pct,
    displayName: String(

...
const { isEnvEnabled } = require('./config/env-flags');
const {
...
const { createPersistentSessionRestore } = require('./services/ai-cli/persistent-session-restore');
const { runNodeCommandRouter } = require('./commands/node-router');
const { runFabricCommandRouter } = require('./commands/fabric-router');
const { runBackgroundSupervisor } = require('./services/background/supervisor-runtime');
const {
  applyAihFrpConfig,
  discoverFrpcConfigPath,
  reconcileAihFrpConfig,
  removeAihFrpConfig
} = require('./services/fabric/frp-config-manager');
const { connectFabricBroker } = require('./services/fabric/broker-connect');
...
const {
  ...
  runServerCommand
} = require('../server/command-handler');
const {
...
// A CLI/server process owns native files beyond an individual DB call. Keep
// its reader lease until exit so maintenance cannot move those files mid-launch.
require('../runtime/account-maintenance-lock').holdAccountProcessLease(fs, AI_HOME_DIR);
...
const { runUpdateCommand } = createUpdateWiring({
  ...
  processObj: process,
  hostHomeDir: HOST_HOME_DIR,
  cliEntryFilePath: process.argv[1],
  log: console.log,
  error: console.error
});
const {
...
const accountArtifactHooks = createAccountArtifactHookWiring({
  ...
  onError: (error, event) => {
    ...
    console.warn(`\x1b[33m[aih:account-hook]\x1b[0m ${provider}#${accountRef}: ${msg}`);
  }
});
...
const {
  ...
  usageSourceKimiOauth: USAGE_SOURCE_KIMI
});
...
const {
  ...
  aiHomeDir: AI_HOME_DIR,
  hostHomeDir: HOST_HOME_DIR,
  enableCodexCliHook: isEnvEnabled('AIH_SERVER_CODEX_CLI_HOOK', true),
  enableCodexDesktopAppHook: isEnvEnabled('AIH_SERVER_CODEX_DESKTOP_HOOK', true),
  // Go Core 迁移期只允许显式 opt-in；默认由 Node 持有公开 9527。
  goCoreEnabled: isEnvEnabled('AIH_GO_CORE_ENABLED', false),
  goCoreBinaryPath: process.env.AIH_GO_CORE_BINARY || undefined,
  goCoreHost: process.env.AIH_GO_CORE_HOST || undefined,
  goCorePort: process.env.AIH_GO_CORE_PORT || undefined,
  goCoreManagementKey: () => process.env.AIH_GO_CORE_MANAGEMENT_KEY || '',
  goCoreClientKey: () => process.env.AIH_GO_CORE_CLIENT_KEY || '',
...
const ptyRuntime = createPtyRuntime(createPtyRuntimeDeps({
  ...
  resolveWindowsNodeShimLaunch: require('../runtime/pty-launch').resolveWindowsNodeShimLaunch,
...
// Post-reboot restore for persistent tmux sessions (registry-driven). Shared
// by `aih ss` (lazy trigger) via globalSessionContext; the server process
// builds its own instance at listen time (see lib/server/server.js).
...
const rootRouterDeps = createRootDispatchWiring({
  ...
  resolveBackgroundServerOptions: () => {
    const stored = readServerConfig({ fs, aiHomeDir: AI_HOME_DIR });
    const parsed = parseServerServeArgs(buildServerArgsFromConfig(stored));
    return applyStoredServerConfigToServeOptions(parsed, () => stored);
  },
  runServerCommand,
  runServerProfileCommand,
  formatServerProfileResult,

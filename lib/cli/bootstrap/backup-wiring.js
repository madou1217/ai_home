...
const { createStandardFormatExportService } = require('../services/backup/standard-format-export');
...
function createBackupExportWiring(deps = {}, factories = {}) {
  ...
  const buildStandardFormatExportService = factories.createStandardFormatExportService || createStandardFormatExportService;
  ...
  const standardFormatExportService = buildStandardFormatExportService({
    fs: deps.fs,
    path: deps.path,
    aiHomeDir: deps.aiHomeDir
  });
  return {
    exportCliproxyapiData: cliproxyapiExportService.exportCliproxyapiData,
    importCliproxyapiCodexAuths: cliproxyapiExportService.importCliproxyapiCodexAuths,
    exportSub2ApiData: standardFormatExportService.exportSub2ApiData,
    exportAntigravityManagerAccounts: standardFormatExportService.exportAntigravityManagerAccounts
  };
}

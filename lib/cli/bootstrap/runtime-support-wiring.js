...
function createStateIndexClientWiring(deps = {}, factories = {}) {
  ...
  const env = deps.env || {};
  const explicitManagementBase = deps.managementBase || env.AIH_SERVER_MANAGEMENT_URL || '';
  const explicitManagementKey = deps.managementKey || env.AIH_SERVER_MANAGEMENT_KEY || '';
  const readStoredConfig = () => {
    if (typeof deps.readServerConfig !== 'function') return {};
    try {
      return deps.readServerConfig() || {};
    } catch (_error) {
      return {};
    }
  };
  const resolveManagementSettings = () => {
    const stored = readStoredConfig();
    return {
      managementBase: explicitManagementBase || buildManagementBaseUrl(stored),
      managementKey: explicitManagementKey || String(stored.managementKey || '').trim()
    };
  };
  const initial = resolveManagementSettings();
  return buildStateIndexClient({
    fetchImpl: deps.fetchImpl,
    managementBase: initial.managementBase,
...
function createAccountArtifactHookWiring(deps = {}, factories = {}) {
  ...
  const syncDefaultAccountAuthToHost = typeof deps.syncGlobalConfigToHost === 'function'
    ? (event) => {
      const result = deps.syncGlobalConfigToHost(event.provider, event.accountRef, { preserveNativeLogin: true });
      if (!result || !result.ok) {
        const reason = result && result.reason ? result.reason : 'unknown_error';
        throw new Error(`default_account_host_sync_failed:${reason}`);
      }
      return result;
    }
    : undefined;
  return buildAccountArtifactHookService({
    ...
    aiHomeDir: deps.aiHomeDir,
    getProfileDir: deps.getProfileDir,
    onDefaultAccountAuthUpdated: deps.onDefaultAccountAuthUpdated || syncDefaultAccountAuthToHost,

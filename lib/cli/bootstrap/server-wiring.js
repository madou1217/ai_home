...
const { createGoCoreSupervisor } = require('../services/server/go-core-supervisor');
...
function createServerWiring(deps = {}, factories = {}) {
  ...
  const buildGoCoreSupervisor = factories.createGoCoreSupervisor || createGoCoreSupervisor;
  ...
  const goCoreSupervisor = deps.goCoreSupervisor || buildGoCoreSupervisor({
    enabled: deps.goCoreEnabled === true,
    fs: deps.fs,
    path: deps.path,
    spawn: deps.spawn,
    ...
    binaryPath: deps.goCoreBinaryPath,
    host: deps.goCoreHost,
    port: deps.goCorePort,
    managementKey: deps.goCoreManagementKey,
    clientKey: deps.goCoreClientKey
  });

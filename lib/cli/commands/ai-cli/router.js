...
const { materializeClaudeKeychainCredentials } = require('../../../account/claude-keychain');
...
//        neither is kicked (cross-machine screen share / pairing).
...
function runAiCliCommandRouter(cmd, args, context = {}) {
  ...
  const startCliPty = (targetCliName, targetId, targetForwardArgs = [], isLoginFlow = false) => {
    ...
    runCliPty(targetCliName, targetId, safeForwardArgs, isLoginFlow);
  };
  ...
  const normalizePointerAccountId = (value) => {
    const normalized = String(value || '').trim();
    return /^\d+$/.test(normalized) ? normalized : '';
  };
  const readAccountPointer = (filePath) => {
    try {
      if (!filePath || !fs.existsSync(filePath)) return '';
      return normalizePointerAccountId(fs.readFileSync(filePath, 'utf8'));
    } catch (_error) {
      return '';
    }
  };
  const clearAccountPointer = (filePath, expectedAccountId = '') => {
    if (!filePath || !fs.existsSync(filePath)) {
      return { accountId: '', changed: false };
    }
    const currentAccountId = readAccountPointer(filePath);
    const expected = normalizePointerAccountId(expectedAccountId);
    if (expected && currentAccountId && currentAccountId !== expected) {
      return { accountId: currentAccountId, changed: false };
    }
    fs.unlinkSync(filePath);
    return { accountId: '', changed: true };
  };
  const resolveProfileDir = (provider, accountId) => (
      typeof getProfileDir === 'function'
      ? getProfileDir(provider, accountId)
      : path.join(PROFILES_DIR, provider, accountId)
  );
  ...
  const listProfileIds = (provider) => {
    return listProviderProfileIds(provider, { fs, profilesDir: PROFILES_DIR });
  };
  ...
  const resolveHomeDiagnosticTargetId = (explicitId = '') => {
    ...
    const defaultId = readAccountPointer(path.join(PROFILES_DIR, cliName, '.aih_default'));
  ...
  const printHomeDiagnostics = (targetId) => {
    ...
    const diagnostic = buildProviderHomeDiagnostics(cliName, profileDir, processImpl.env || {}, {
      ...
      aiHomeDir: path.dirname(PROFILES_DIR)
    });
  ...
  if (idOrAction === 'sessions') {
    ...
    const ids = sessId ? [sessId] : listProfileIds(cliName);
    if (!ids.length) {
    ...
    const collectRows = (collectOnly) => {
      ...
      for (const accId of ids) {
        const result = listPersistentSessions(cliName, accId, {
          processImpl,
          fs,
          spawnSync: context.spawnSync,
          resolveCliPath,
          askYesNo,
          aiHomeDir: PROFILES_DIR ? path.dirname(PROFILES_DIR) : '',
          hostHomeDir: HOST_HOME_DIR,
    ...
    if (interactivePicker && collected.rows.length > 0) {
      const handleSelected = (selected) => {
        if (selected) {
          const entered = enterGlobalPersistentSession(selected, {
            processImpl,
            consoleImpl: console,
            runCliPty: (targetCliName, targetId, forwardArgs, isLoginFlow) => {
              startCliPty(targetCliName, targetId, forwardArgs, isLoginFlow);
            }
          });
          if (!entered) {
      ...
      return;
    } else if (!collected.printedAny && !sessId && ids.length > 0) {
  ...
  if (idOrAction === 'set-default') {
    ...
    if (typeof ensureSessionStoreLinks === 'function') {
      ensureSessionStoreLinks(cliName, targetId);
    }
  ...
  if (idOrAction === 'unset-default') {
    ...
    const defaultPath = path.join(PROFILES_DIR, cliName, '.aih_default');
    try {
      const currentDefaultId = readAccountPointer(defaultPath);
      clearAccountPointer(defaultPath, currentDefaultId);
    } catch (error) {
  ...
  if (idOrAction === 'set-mobile') {
    ...
    const pDir = resolveProfileDir('codex', targetId);
    if (!fs.existsSync(pDir)) {
      console.error(`\x1b[31m[aih] Account ID ${targetId} does not exist.\x1b[0m`);
      processImpl.exit(1);
      return;
    ...
    const aiHomeDir = path.dirname(PROFILES_DIR);
    ...
    const result = service.setDesktopAccountId(targetId);
    if (!result || !result.ok) {
  ...
  if (idOrAction === 'unset-mobile') {
    ...
    const currentMobileId = service.getDesktopAccountId();
    const result = currentMobileId
      ? service.clearDesktopAccountId(currentMobileId)
      : { ok: true, stateFilePath: '', desktopAccountId: '', changed: false };
    if (!result || !result.ok) {
      const reason = result && result.reason ? result.reason : 'unknown_error';
  ...
  if (activeEnv && !idOrAction) {
    ...
    if (!matchedId) {
      ...
      // ✅ 如果是 codex 且有 OPENAI_API_KEY,写入 auth.json
      if (cliName === 'codex' && activeEnv.OPENAI_API_KEY) {
        const codexConfigDir = path.join(getProfileDir(cliName, matchedId), '.codex');
        fs.mkdirSync(codexConfigDir, { recursive: true });
        const authJsonPath = path.join(codexConfigDir, 'auth.json');
        const authData = { OPENAI_API_KEY: activeEnv.OPENAI_API_KEY };
  ...
  if (idOrAction === 'login') {
    ...
    if (mode === 'api_key') {
      ...
      if (Object.keys(newEnv).length > 0) {
        ...
        // ✅ 如果是 codex 且有 OPENAI_API_KEY,写入 auth.json
        if (cliName === 'codex' && newEnv.OPENAI_API_KEY) {
          const codexConfigDir = path.join(getProfileDir(cliName, nextId), '.codex');
          fs.mkdirSync(codexConfigDir, { recursive: true });
          const authJsonPath = path.join(codexConfigDir, 'auth.json');
          const authData = { OPENAI_API_KEY: newEnv.OPENAI_API_KEY };
          fs.writeFileSync(authJsonPath, JSON.stringify(authData, null, 2));
        }
  ...
  // macOS: a native `claude` login stores OAuth in the login Keychain instead of
  // .credentials.json, which makes every file-based reader think the account has
  // no login state. Bridge it to a file once (never overwriting an existing one)
  // so checkStatus / usage / server all see it and aih can self-refresh the token.
  if (cliName === 'claude') {
    try {
      materializeClaudeKeychainCredentials({ profileDir: sandboxDir, fs, path, processObj: processImpl });
    } catch (_e) {
      // best-effort bridge; the read-only keychain fallback still covers status/usage
    }
  }
  ...
  if (!isApiKeyMode && typeof getAccountQuotaState === 'function') {
    const quotaState = getAccountQuotaState(cliName, id, { refreshSnapshot: true });

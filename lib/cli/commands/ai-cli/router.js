...
function runAiCliCommandRouter(cmd, args, context = {}) {
  ...
  const setAccountOperationalStatus = context.setAccountOperationalStatus;
  ...
  const STATUS_ACTIONS = new Set(['up', 'down']);
  const KNOWN_ACTIONS = new Set(['ls', 'set-default', 'set-mobile', 'login', 'auto', ...POLICY_ACTIONS, ...USAGE_ACTIONS, ...CLEANUP_ACTIONS, ...DELETE_ACTIONS, ...DELETE_ALL_ACTIONS, ...COUNT_ACTIONS, ...STATUS_ACTIONS]);
  ...
  const updateAccountStatus = (targetId, nextStatus) => {
    if (!targetId || !/^\d+$/.test(targetId)) {
      console.error(`\x1b[31m[aih] Invalid ID. Usage: aih ${cliName} <id> <up|down>\x1b[0m`);
      processImpl.exit(1);
      return true;
    }
    ...
    if (typeof setAccountOperationalStatus !== 'function') {
      console.error(`\x1b[31m[aih] Account status service unavailable.\x1b[0m`);
      processImpl.exit(1);
      return true;
    }
    const updated = setAccountOperationalStatus(cliName, targetId, nextStatus);
    if (!updated) {
      console.error(`\x1b[31m[aih] Failed to update account status for ${cliName} Account ID ${targetId}.\x1b[0m`);
      processImpl.exit(1);
      return true;
    }
    console.log(`\x1b[32m[Success]\x1b[0m ${cliName} Account ID ${targetId} status set to ${nextStatus === 'down' ? 'down' : 'up'}.`);
    processImpl.exit(0);
    return true;
  };
  ...
  if (idOrAction && STATUS_ACTIONS.has(idOrAction)) {
    if (updateAccountStatus(args[2], idOrAction)) return;
  }
  ...
  if (idOrAction === 'set-mobile') {
    ...
    if (!validation.ok) {
      console.error(`\x1b[31m[aih] Codex App account requires a usable API Key or ChatGPT OAuth account.\x1b[0m`);
      processImpl.exit(1);
      return;
    }
  ...
  if (useBuiltinServerProfile) {
    ...
    runCliPty(cliName, AIH_SERVER_PROFILE_ID, forwardArgs);
    return;
  }
  ...
  if (idOrAction === 'auto') {
    ...
    if (!nextId) {
      console.log(`\x1b[31mNo schedulable accounts found to auto-route. Use 'aih ${cliName} usage' or 'aih ${cliName} login' first.\x1b[0m`);
      processImpl.exit(1);
      return;
    }
  ...
  if (idOrAction && /^\d+$/.test(idOrAction)) {
    ...
    if (idStyleAction && STATUS_ACTIONS.has(idStyleAction)) {
      if (updateAccountStatus(id, idStyleAction)) return;
    }
  ...
  if (!isApiKeyMode && typeof getAccountQuotaState === 'function') {
    ...
    if (quotaState && quotaState.quotaStatus === 'exhausted') {
      ...
      if (ans === false) {
        console.log(`Use 'aih ${cliName} auto' to automatically pick a schedulable account.`);
        processImpl.exit(0);
        return;
      }

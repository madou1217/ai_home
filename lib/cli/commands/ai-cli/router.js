...
const { AI_CLI_CONFIGS, isSupportedAiCli, listSupportedAiClis } = require('../../services/ai-cli/provider-registry');
const {
...
const { reloadCodexDesktopRuntime } = require('../../../server/codex-desktop-runtime-reload');
const {
...
const { prepareDesktopClientProfile } = require('../../services/ai-cli/desktop-client-profile');
const { detectHeadlessInvocation } = require('../../services/pty/headless-invocation');
const {
  syncClaudeDesktopWebSession: defaultSyncClaudeDesktopWebSession
} = require('../../services/ai-cli/claude-desktop-session');
const {
  configureClaudeDesktopMode: defaultConfigureClaudeDesktopMode
} = require('../../services/ai-cli/claude-desktop-inference');
...
//   -R / --aih-resume                                → take over this project's
//        newest existing session, even if it's live elsewhere (e.g. an SSH
//        client is still attached; that client is detached). NOT `--resume`,
//        which is claude's.
//   -M / --aih-mirror                                → attach to this project's
//        newest existing session SHARED — both windows mirror it in real time,
...
function registerApiKeyAccount(fs, aiHomeDir, cliName, cliAccountId, data) {
  ...
  if (!provider || (id && !/^\d+$/.test(id)) || !homeDir || !data || typeof data !== 'object') return null;
  const identitySeed = normalizeIdentitySeed(buildApiKeyIdentity(provider, { config: data }));
  if (!identitySeed) return null;
  const registration = registerAccountIdentity(fs, homeDir, {
    provider,
    ...(id ? { cliAccountId: id } : {}),
    identitySeed
  });
  writeAccountCredentials(fs, homeDir, registration.accountRef, data);
  return registration;
}
...
function runAiCliCommandRouter(cmd, args, context = {}) {
  ...
  const syncClaudeDesktopWebSession = context.syncClaudeDesktopWebSession
    || defaultSyncClaudeDesktopWebSession;
  const configureClaudeDesktopMode = context.configureClaudeDesktopMode
    || defaultConfigureClaudeDesktopMode;
  ...
  const startCliPty = (targetCliName, targetId, targetForwardArgs = [], isLoginFlow = false) => {
    ...
    // A headless run must not repaint the terminal it was called from: no OSC
    // title, no iTerm profile switch, no Warp agent mapping.
    const headlessRun = detectHeadlessInvocation(targetCliName, safeForwardArgs, {
      isLogin: isLoginFlow,
      env: processImpl.env
    }).headless;
    if (!isLoginFlow && !headlessRun) {
  ...
  const parseSetDefaultArgs = (tokens) => {
    ...
    for (let i = 0; i < items.length; i += 1) {
      ...
      if (token === '--desktop-mode') {
        desktopMode = String(items[i + 1] || '').trim().toLowerCase();
        i += 1;
        if (!desktopMode) {
          return { ok: false, error: 'Missing value for --desktop-mode' };
        }
        continue;
      }
      if (token.startsWith('--desktop-mode=')) {
        desktopMode = token.slice('--desktop-mode='.length).trim().toLowerCase();
        if (!desktopMode) {
          return { ok: false, error: 'Missing value for --desktop-mode' };
        }
        continue;
    ...
    if (desktopMode && desktopMode !== 'web' && desktopMode !== 'api') {
      return { ok: false, error: `Invalid desktop mode: ${desktopMode}` };
    }
    return { ok: true, restartClient, forceQuitClient, desktopMode };
  };

  const reportDesktopRestart = (result, forceQuitRequested = false) => {
    if (!result) {
      console.log(`\x1b[33m[aih]\x1b[0m Desktop client restart skipped (unavailable).`);
      return;
    }
    const clientName = result.clientName || cliName;
    if (result.detected && result.restarted) {
      console.log(`\x1b[36m[aih]\x1b[0m ${forceQuitRequested || result.forceQuit ? 'Force-quit and restarted' : 'Restarted'} local ${clientName} desktop client.`);
    } else if (result.launched) {
      console.log(`\x1b[36m[aih]\x1b[0m ${clientName} desktop client was not running; launched it using the learned path.`);
    } else if (result.reason === 'no_saved_path') {
      console.log(`\x1b[33m[aih]\x1b[0m Open ${clientName} desktop app once manually first, then rerun with --restart-client so ai-home can learn and reuse its path.`);
    } else if (result.reason === 'saved_path_missing') {
      console.log(`\x1b[33m[aih]\x1b[0m Learned ${clientName} desktop path is no longer valid. Open the app once manually, then rerun with --restart-client.`);
    } else {
  ...
  const readDefaultProviderSelection = (provider) => {
    if (!supportsAihServerProfile(provider)) return readDefaultCliAccountId(provider);
    if (accountStorageDir && readDefaultProviderProfile(fs, accountStorageDir, provider) === AIH_SERVER_PROFILE_ID) {
      return AIH_SERVER_PROFILE_ID;
    }
    return readDefaultCliAccountId(provider) || AIH_SERVER_PROFILE_ID;
  };
  ...
  const useImplicitProviderProfile = supportsAihServerProfile(cliName) && !idOrAction;

  if (idOrAction === '--restart-client' || idOrAction === '--force-quit-client') {
    const restartArgs = parseSetDefaultArgs(args.slice(1));
    if (!restartArgs.ok || restartArgs.desktopMode) {
      console.error(`\x1b[31m[aih] ${restartArgs.error || 'Unexpected desktop mode'}. Usage: aih ${cliName} --restart-client [--force-quit-client]\x1b[0m`);
      processImpl.exit(1);
      return;
    }
    if (typeof restartDetectedDesktopClient !== 'function') {
      console.error(`\x1b[31m[aih] Desktop client restart is unavailable for ${cliName}.\x1b[0m`);
      processImpl.exit(1);
      return;
    }
    try {
      const restartResult = restartDetectedDesktopClient(cliName, { forceQuit: restartArgs.forceQuitClient });
      reportDesktopRestart(restartResult, restartArgs.forceQuitClient);
      processImpl.exit(restartResult && (restartResult.restarted || restartResult.launched) ? 0 : 1);
    } catch (error) {
      console.error(`\x1b[31m[aih] Desktop client restart failed: ${error.message}\x1b[0m`);
      processImpl.exit(1);
    }
    return;
  }
  ...
  if (idOrAction === 'ls') {
    ...
    const lsResult = listProfiles(cliName, lsArg || null);
    if (lsResult && lsResult.interactive) return;
    processImpl.exit(0);
    return;
  }
  ...
  const resolveHomeDiagnosticTargetId = (explicitId = '') => {
    ...
    const defaultId = readDefaultProviderSelection(cliName);
  ...
  if (idOrAction === 'set-default') {
    const requestedTarget = String(args[2] || '').trim();
    const gatewayDefault = supportsAihServerProfile(cliName)
      && (!requestedTarget || requestedTarget === AIH_SERVER_PROFILE_ID || requestedTarget.startsWith('--'));
    const targetId = gatewayDefault
      ? AIH_SERVER_PROFILE_ID
      : requestedTarget;
    const setDefaultArgs = parseSetDefaultArgs(
      gatewayDefault
        ? args.slice(requestedTarget === AIH_SERVER_PROFILE_ID ? 3 : 2)
        : args.slice(3)
    );
    const desktopModeUsage = cliName === 'claude' ? ' [--desktop-mode <web|api>]' : '';
    const setDefaultUsage = gatewayDefault
      ? `aih ${cliName} set-default [--restart-client] [--force-quit-client]${desktopModeUsage}`
      : `aih ${cliName} set-default <id> [--restart-client] [--force-quit-client]${desktopModeUsage}`;
    if (!gatewayDefault && (!targetId || !/^\d+$/.test(targetId))) {
      console.error(`\x1b[31m[aih] Invalid ID. Usage: ${setDefaultUsage}\x1b[0m`);
      processImpl.exit(1);
      return;
    }
    if (!setDefaultArgs.ok) {
      console.error(`\x1b[31m[aih] ${setDefaultArgs.error}. Usage: ${setDefaultUsage}\x1b[0m`);
      processImpl.exit(1);
      return;
    }
    if (setDefaultArgs.desktopMode && cliName !== 'claude') {
      console.error(`\x1b[31m[aih] --desktop-mode is supported only for Claude Desktop. Usage: ${setDefaultUsage}\x1b[0m`);
      processImpl.exit(1);
      return;
    }
    if (setDefaultArgs.desktopMode && !setDefaultArgs.restartClient) {
      console.error(`\x1b[31m[aih] --desktop-mode requires --restart-client or --force-quit-client. Usage: ${setDefaultUsage}\x1b[0m`);
      processImpl.exit(1);
      return;
    }
    if (gatewayDefault) {
      if (setDefaultArgs.desktopMode) {
        console.error(`\x1b[31m[aih] --desktop-mode is supported only when selecting a Claude account. Usage: ${setDefaultUsage}\x1b[0m`);
        processImpl.exit(1);
        return;
      }
      const syncResult = typeof syncGlobalConfigToHost === 'function'
        ? syncGlobalConfigToHost(cliName, '', { gateway: true })
        : { ok: false, reason: 'host_sync_unavailable' };
      if (!syncResult || !syncResult.ok) {
        const reason = syncResult && syncResult.reason ? syncResult.reason : 'unknown_error';
        console.error(`\x1b[31m[aih] Failed to sync ${cliName} AIH Server profile to host global config (${reason}).\x1b[0m`);
        processImpl.exit(1);
        return;
      }
      try {
        writeDefaultProviderProfile(fs, accountStorageDir, cliName, AIH_SERVER_PROFILE_ID);
        clearDefaultAccountRef(fs, accountStorageDir, cliName);
      } catch (error) {
        console.error(`\x1b[31m[aih] Failed to set ${cliName} AIH Server profile as default (${error.message}).\x1b[0m`);
        processImpl.exit(1);
        return;
      }
      console.log(`\x1b[32m[Success]\x1b[0m Set AIH Server as the default provider for ${cliName}.`);
      if (cliName === 'codex') {
        const desktopReload = reloadCodexDesktopRuntime(createCodexDesktopHookService({
          fs,
          path,
        ...
        if (!desktopReload.ok) {
          console.log(`\x1b[33m[aih]\x1b[0m Codex App runtime reload pending (${desktopReload.reason}).`);
        }
      }
      if (setDefaultArgs.restartClient && typeof restartDetectedDesktopClient === 'function') {
        try {
          reportDesktopRestart(
            restartDetectedDesktopClient(cliName, { forceQuit: setDefaultArgs.forceQuitClient }),
            setDefaultArgs.forceQuitClient
          );
        } catch (error) {
    ...
    if (typeof syncGlobalConfigToHost === 'function') {
      const syncResult = syncGlobalConfigToHost(cliName, targetAccount.accountRef, { restoreGateway: true });
      if (!syncResult || !syncResult.ok) {
        const reason = syncResult && syncResult.reason ? syncResult.reason : 'unknown_error';
    ...
    clearDefaultProviderProfile(fs, accountStorageDir, cliName);
    ...
    const desktopClientConfig = AI_CLI_CONFIGS[cliName] && AI_CLI_CONFIGS[cliName].desktopClient;
    const desktopReloadsHostAuth = !desktopClientConfig || desktopClientConfig.reloadsHostAuth !== false;
    // 这两句文案与 --desktop-mode 都是 Claude Desktop 专属：其它 Provider
    // （如 codebuddy 家族）走到这里只会打印无意义的 Claude 菜单，直接跳过。
    if (setDefaultArgs.restartClient && !desktopReloadsHostAuth && cliName === 'claude') {
      if (setDefaultArgs.desktopMode === 'api') {
        console.log(`\x1b[36m[aih]\x1b[0m Claude Desktop API mode uses the ai-home global routing pool; Account ID ${targetId} selects only the isolated Desktop profile.`);
      } else {
        console.log(`\x1b[36m[aih]\x1b[0m Claude Desktop uses an account-scoped web session; Claude Code OAuth is synced separately.`);
      }
    }
    if (setDefaultArgs.restartClient && typeof restartDetectedDesktopClient === 'function') {
      try {
        const restartOptions = { forceQuit: setDefaultArgs.forceQuitClient };
        const userDataEnvKey = String(desktopClientConfig && desktopClientConfig.userDataEnvKey || '').trim();
        if (userDataEnvKey) {
          ...
          if (!profileDir) throw new Error('desktop_profile_unavailable');
          restartOptions.launchEnv = { [userDataEnvKey]: profileDir };
          if (cliName === 'claude') {
            const desktopMode = setDefaultArgs.desktopMode || 'web';
            restartOptions.prepareLaunch = () => {
              let serverConfig = {};
              try {
                serverConfig = typeof readServerConfig === 'function'
                  ? (readServerConfig() || {})
                  : {};
              } catch (_error) {}
              const modeResult = configureClaudeDesktopMode({
                fs,
                mode: desktopMode,
                accountRef: targetAccount.accountRef,
                profileDir,
              ...
              if (!modeResult || modeResult.ok === false || desktopMode === 'api') {
                return modeResult;
              }
              const sessionResult = syncClaudeDesktopWebSession({
                fs,
                path,
                processObj: processImpl,
                aiHomeDir: accountStorageDir,
              ...
              return {
                ...sessionResult,
                desktopMode,
                desktopModeStatus: modeResult.status
              };
            };
          }
        }
        const restartResult = restartDetectedDesktopClient(cliName, restartOptions);
        const authReloadPurpose = desktopReloadsHostAuth ? ' to reload host auth' : '';
        const launchPreparation = restartResult && restartResult.launchPreparation;
        if (launchPreparation && launchPreparation.status === 'migrated') {
          console.log(`\x1b[36m[aih]\x1b[0m Imported matching Claude web session into the isolated Desktop profile.`);
        } else if (launchPreparation && launchPreparation.status === 'existing') {
          console.log(`\x1b[36m[aih]\x1b[0m Reused the isolated Claude Desktop web session for this account.`);
        } else if (launchPreparation && launchPreparation.status === 'login_required') {
          console.log(`\x1b[33m[aih]\x1b[0m No reusable Claude web session was found; this isolated Desktop profile requires a one-time web login.`);
        } else if (launchPreparation && launchPreparation.status === 'api_configured') {
          console.log(`\x1b[36m[aih]\x1b[0m Configured the isolated Claude Desktop profile for ai-home API mode (${launchPreparation.gatewayBaseUrl}).`);
        }
        if (restartResult && restartResult.detected && restartResult.restarted) {
          if (restartResult.forceQuit) {
            console.log(`\x1b[36m[aih]\x1b[0m Force-quit and restarted local ${restartResult.clientName || cliName} desktop client${authReloadPurpose}.`);
          } else {
            console.log(`\x1b[36m[aih]\x1b[0m Restarted local ${restartResult.clientName || cliName} desktop client${authReloadPurpose}.`);
          }
        ...
        } else if (restartResult && restartResult.reason === 'launch_preparation_failed') {
          const preparationReason = launchPreparation && launchPreparation.reason
            ? launchPreparation.reason
            : 'unknown_error';
          console.log(`\x1b[33m[aih]\x1b[0m Desktop client was not launched because account-scoped session preparation failed (${preparationReason}).`);
  ...
  if (idOrAction === 'unset-default') {
    ...
    if (typeof syncGlobalConfigToHost === 'function') {
      const restoreResult = syncGlobalConfigToHost(cliName, '', { restoreGateway: true });
      if (!restoreResult || !restoreResult.ok) {
        console.error(`\x1b[31m[aih] Failed to restore ${cliName} host config (${restoreResult && restoreResult.reason || 'unknown_error'}).\x1b[0m`);
        processImpl.exit(1);
        return;
      }
    }
    ...
    if (cliName === 'codex') {
      reloadCodexDesktopRuntime(createCodexDesktopHookService({
        fs,
        path,
        processObj: processImpl,
        spawnSync: context.spawnSync,
  ...
  if (idOrAction === 'set-mobile') {
    ...
    const desktopReload = result.changed
      ? reloadCodexDesktopRuntime(service)
      : { ok: true, restarted: false };
    if (!desktopReload.ok) {
      console.log(`\x1b[33m[aih]\x1b[0m Codex App identity saved; runtime reload pending (${desktopReload.reason}).`);
    }
  ...
  if (idOrAction === 'unset-mobile') {
    ...
    if (result.changed) reloadCodexDesktopRuntime(service);
    console.log(`\x1b[32m[Success]\x1b[0m Cleared Codex App account.`);
    processImpl.exit(0);
    return;
  }

  if (useImplicitProviderProfile) {
    forwardArgs = args.slice(1);
    startCliPty(cliName, readDefaultProviderSelection(cliName), forwardArgs, false, forwardArgs);
    return;
  }
  ...
  if (supportsAihServerProfile(cliName) && isNativePassThroughAction(idOrAction)) {
    startCliPty(cliName, readDefaultProviderSelection(cliName), args.slice(1), false, args.slice(1));
    return;
  }
  ...
  if (activeEnv && !idOrAction) {
    ...
    if (!matchedId) {
      ...
      const registration = registerApiKeyAccount(fs, accountStorageDir, cliName, matchedId, activeEnv);
      if (!registration) throw new Error('account_ref_registration_failed');
      matchedId = registration.cliAccountId;
  ...
  if (idOrAction === 'login') {
    ...
    if (mode === 'api_key') {
      console.log(`\n\x1b[36m[aih]\x1b[0m Configuring API Key mode for \x1b[33m${cliName}\x1b[0m`);
      let newEnv = {};
      ...
      if (Object.keys(newEnv).length > 0) {
        const registration = registerApiKeyAccount(fs, accountStorageDir, cliName, '', newEnv);
        if (!registration) throw new Error('account_ref_registration_failed');
        const nextId = registration.cliAccountId;
    ...
    const shouldLogin = createAccount(cliName);
    if (shouldLogin) {
      startCliPty(cliName, '', loginForwardArgs, true, ['login', ...loginForwardArgs]);
    } else {
      console.error(`\x1b[31m[aih]\x1b[0m Unable to prepare login for ${cliName}.`);
      processImpl.exit(1);
    }
    return;
  }
  ...
  if (idNoBrowserOnly) {
    ...
    console.log(`\x1b[33m[Notice]\x1b[0m Account ${id} is already logged in. Starting a new no-browser OAuth login.`);
    const shouldLogin = createAccount(cliName);
    if (shouldLogin) {
      startCliPty(cliName, '', [NO_BROWSER_FLAG], true, [NO_BROWSER_FLAG]);
    } else {
      processImpl.exit(1);
    }
    return;

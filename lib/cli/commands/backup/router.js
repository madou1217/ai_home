...
const KNOWN_IMPORT_PROVIDERS = new Set(['codex', 'gemini', 'claude', 'agy', 'opencode']);
const SUB2API_EXPORT_PROVIDERS = new Set(['codex', 'gemini', 'claude', 'agy']);
...
async function runBackupCommand(cmd, args, deps = {}) {
  ...
  if (cmd === 'export') {
    ...
    if (selectors.length > 0) {
      ...
      consoleImpl.log('\x1b[36m[aih]\x1b[0m Exporting flat account JSON files (provider_email.json / provider_url_ref.json).');
    }

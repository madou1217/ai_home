...
} = require('../services/fabric/profile-pairing');
const {
...
\x1b[33mUsage:\x1b[0m
  ...
  aih fabric profile pair <pair-url|code> [--endpoint URL] [--device-name NAME] [--json]
  aih fabric profile pair-self [--endpoint http://127.0.0.1:9527] [--device-name NAME] [--json]
  aih fabric profile invite [--endpoint http://127.0.0.1:9527] [--device-name NAME] [--json] [90mCreate a browser pairing invite (WebUI auth gate bootstrap)[0m
  ...
  aih fabric provider accounts reauth --provider PROVIDER --account-id ID [--endpoint URL] [--profile-id ID] [--wait-auth-url-ms N] [--json]
  ...
  aih fabric session start <node-id> --provider PROVIDER --prompt TEXT [--project-id ID] [--project-path PATH] [--json]
...
\x1b[33mExamples:\x1b[0m
  ...
  aih fabric profile pair-self --endpoint http://127.0.0.1:9527 --json
  aih fabric profile pair "https://server.example.com/v0/fabric/device-pair?code=..." --json
  ...
  aih fabric provider accounts reauth --provider codex --account-id 1 --json
...
\x1b[33mNotes:\x1b[0m
  ...
  transport readiness reads a paired server profile and never prints the raw device token.
  ...
  transport promotion-gate runs the real aggregate M6 gate and keeps advanced promotion separate from relay fallback. --publish-promotion writes a passing WebRTC promotion back through the remote node token-file over SSH.
  ...
  closure audit aggregates paired server registry, node capabilities, transport status, provider account gates, and an optional real session marker proof.
  ...
  profile pair consumes a real device-pair invite and saves a ready server profile without printing the raw token.
  profile pair-self creates a device invite on the target server, consumes it through the same HTTP device-pair endpoint, and saves the resulting profile locally.
  provider accounts audit is read-only; --endpoint/--profile-id selects a paired server profile, derives the node SSH binding, and never uploads local credentials. provider accounts revalidate requires --yes and clears remote runtime blockers before real session guards.
  provider accounts reauth starts a real remote OAuth job on the paired server profile and returns the job/auth URL; auth-job get/cancel/callback continues that same remote job through the paired device token and never copies local credentials.
  nodes reads the paired server registry and explains each node's server/relay/project/runtime/ssh capabilities.
  session start first checks the node inventory action gate, then calls the protected device-node-session-start API.
  session attach/events/message/slash/stop reuse the paired server profile and protected device-node-session routes.
...
function printFabricError(error, consoleImpl) {
  ...
  if (code === 'ready_server_profile_missing') {
    consoleImpl.error('\x1b[31m[aih] fabric command failed: no paired server profile is ready. Complete Server Setup pairing first.\x1b[0m');
    return;
  }
...
async function runFabricCommandRouter(args = [], deps = {}) {
  ...
  if (action !== 'transport' && action !== 'registry' && action !== 'broker' && action !== 'nodes' && action !== 'session' && action !== 'closure' && action !== 'profile' && action !== 'provider') {
  ...
  if (action === 'profile') {
    const profileArgs = Array.isArray(args) ? args.slice(2) : [];
    const subcommand = String(profileArgs[0] || '').trim();
    if (isHelp(subcommand)) {
      showFabricUsage(consoleImpl);
      processObj.exit(profileArgs.length === 0 ? 1 : 0);
      return profileArgs.length === 0 ? 1 : 0;
    }
    if (subcommand !== 'pair' && subcommand !== 'pair-self' && subcommand !== 'invite') {
      consoleImpl.error(`\x1b[31m[aih] unknown fabric profile command: ${subcommand}\x1b[0m`);
      showFabricUsage(consoleImpl);
      processObj.exit(1);
      return 1;
    }
    const commandArgs = profileArgs.slice(1);
    if (hasHelpArg(commandArgs)) {
      showFabricUsage(consoleImpl);
      processObj.exit(0);
      return 0;
    ...
    try {
      const result = await (deps.runFabricProfilePairingCommand || runFabricProfilePairingCommand)(subcommand, commandArgs, deps);
      if (result && result.json) {
        await writeStdout(processObj, `${JSON.stringify(result)}\n`);
      } else {
        await writeStdout(processObj, `${formatFabricProfilePairingReport(result)}\n`);
      }

...
\x1b[33mUsage:\x1b[0m
  ...
  aih fabric broker connect <server-url> [server-url...] --server-id ID --management-key KEY [--management-key KEY...] [--local-url http://127.0.0.1:9527] [--reconnect-delay-ms 3000] [--max-attempts 0] [--json]
...
\x1b[33mExamples:\x1b[0m
  ...
  aih fabric registry publish http://127.0.0.1:9527 --management-key "$AIH_MANAGEMENT_KEY" --from-server --relay-node --json
  aih fabric registry heartbeat http://127.0.0.1:9527 --management-key "$AIH_MANAGEMENT_KEY" --node-id home-mac --relay-status online --transport relay=online --json
  aih fabric registry agent http://127.0.0.1:9527 --management-key "$AIH_MANAGEMENT_KEY" --node-id home-mac --relay-status online --transport relay=online
  aih fabric registry agent http://127.0.0.1:9527 --management-key "$AIH_MANAGEMENT_KEY" --node-id relay-1 --probe-transport relay=tcp://127.0.0.1:8766 --count 2 --json
  aih fabric registry agent service install http://127.0.0.1:9527 --node-id relay-1 --management-key-file /tmp/management-key --probe-transport relay=tcp://127.0.0.1:8766
  aih fabric broker connect https://server.example.com --server-id home-server --management-key "$AIH_MANAGEMENT_KEY"
...
function printFabricError(error, consoleImpl) {
  ...
  if (code === 'missing_fabric_broker_management_key') {
    consoleImpl.error('\x1b[31m[aih] fabric broker connect failed: pass --management-key or set AIH_MANAGEMENT_KEY.\x1b[0m');
    return;
  }

...
\x1b[33mUsage:\x1b[0m
  ...
  aih node service install <control-url> --node-id ID --token-file FILE --yes [--json]
...
\x1b[33mExamples:\x1b[0m
  ...
  aih node service status --control-url https://control.example.com --node-id nat-node
  aih node service install https://control.example.com --node-id nat-node --token-file ~/.ai_home/fabric/nat-node.token --dry-run
  aih node service uninstall --node-id nat-node --dry-run
  aih node relay connect https://control.example.com --node-id nat-node
  aih node webrtc connect https://control.example.com --node-id nat-node
  aih node relay service install https://control.example.com --node-id nat-node
  aih node webrtc service install https://control.example.com --node-id nat-node
...
\x1b[33mTransport model:\x1b[0m
  ...
  webrtc connect keeps a real DataChannel open for promoted management RPC while still using the Control Plane HTTP port for signaling.
...
function printNodeError(error, consoleImpl) {
  ...
  if (code === 'management_key_required') {
    ...
    if (String(error && error.command || '') === 'webrtc-connect') {
      consoleImpl.error('\x1b[31m[aih] node webrtc connect failed: set server managementKey or pass --management-key matching the joined node secret.\x1b[0m');
      return;
    }
  ...
  if (code === 'missing_webrtc_url' || code === 'invalid_webrtc_url') {
    consoleImpl.error('\x1b[31m[aih] node webrtc connect failed: pass the Control Plane URL, for example https://control.example.com.\x1b[0m');
    return;
  }
  ...
  if (code === 'missing_node_service_control_url') {
    consoleImpl.error('\x1b[31m[aih] node service install failed: pass the Control Plane URL, for example https://control.example.com.\x1b[0m');
    return;
  }
  ...
  if (code === 'missing_node_service_token_file') {
    consoleImpl.error('\x1b[31m[aih] node service install failed: pass --token-file pointing to a local Fabric device token file.\x1b[0m');
    return;
  }
  ...
  if (code === 'fabric_token_file_unreadable') {
    consoleImpl.error(`\x1b[31m[aih] node service install failed: token file is unreadable ${String(error && error.file || '').trim()}.\x1b[0m`);
    return;
  }

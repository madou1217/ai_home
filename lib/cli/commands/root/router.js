  ...
  const preBackupHandlers = {
    ...
    usage: async () => {
      ...
      try {
        ...
        consoleImpl.error('\x1b[90mUsage:\x1b[0m aih usage [stats|models|sessions|session-detail|scan] [--from YYYY-MM-DD] [--to YYYY-MM-DD] [--provider codex|claude|gemini|agy|opencode] [--model MODEL] [--session-id ID] [--no-scan] [--json]');
        processObj.exit(1);
      }
    },

...
function parseTomlBooleanLiteral(value) {
...
function getCodexHooksFeatureFlagState(configText, options = {}) {
  ...
  lines.slice(section.featureIndex + 1, section.sectionEnd).forEach((line) => {
    const match = String(line || '').trim().match(/^([A-Za-z0-9_-]+)\s*=\s*(.+)$/);
    if (!match || !featureNames.has(match[1])) return;
    const value = parseTomlBooleanLiteral(match[2]);
    if (value === null) return;
    activeFlagName = match[1];
    activeValue = value;
  });

  return {
    ...resolved,
    explicit: Boolean(activeFlagName),
    enabled: activeValue !== false,
    disabled: activeValue === false,
    activeFlagName
  };
}

...
function buildFilesystemPlan(fs, aiHomeDir, _provider, mapping) {
    ...
    if (stat.isSymbolicLink()) {
      ...
      if (nativeStoreKind(relative)) {
        try {
          ...
          entry.kind = 'native-sqlite'; entry.logicalDigest = native.before.digest;
          if (native.changes.length) nativeDatabases.push(native);
  ...
  return { edits, links, moves, nativeDatabases, blockers, skipped, inspected, fingerprint: sha256(JSON.stringify(entries)) };
}

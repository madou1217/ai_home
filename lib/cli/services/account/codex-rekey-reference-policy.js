  ...
  try {
    const transformed = transformJsonStrings(text, (value, trail, context) => {
      ...
      if ((!context.isKey && DISPLAY_TEXT_KEYS.has(trail.at(-1)))
        ...
        || (options.historicalFileChange === true && trail[0] === 'item' && trail[1] === 'detail'
          && ['changes', 'paths'].includes(trail[2]))) {
...
function classifyDatabaseText(table, column, value, mapping) {
  ...
  if (column.endsWith('_json') || (table === 'app_kv' && column === 'value')) {
    // timeline-detail-contract.js defines file_change.changes and diff.paths as
    // past tool output. Preserve those original path labels; they are not the
    // native session's active rollout pointer or a future filesystem command.
    let eventKind;
    try { eventKind = JSON.parse(value)?.item?.kind; } catch (_) { /* Strict parser below reports invalid JSON. */ }
    const transformed = transformJsonText(value, mapping, {
      preserveHistory: table.startsWith('chat_runtime_'),
      historicalFileChange: table === 'chat_runtime_events' && column === 'payload_json' && ['file_change', 'diff'].includes(eventKind),

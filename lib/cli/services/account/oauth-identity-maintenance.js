...
function applyMaintenancePlan(plan, options = {}) {
  ...
  try {
    ...
    const afterFiles = buildFilesystemPlan(fs, root, '', new Map(plan.mapping));
    if (afterFiles.edits.length || afterFiles.moves.length || afterFiles.links.length || afterFiles.blockers.length) {

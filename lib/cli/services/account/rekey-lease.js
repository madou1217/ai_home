...
function acquireRekeyLease(aiHomeDir, options = {}) {
  ...
  try {
    if (options.recover === true) {
      ...
      previousToken = readMaintenanceOwner(io, ownerPath, true)?.token || '';
    } else {
    ...
    assertQuiet();
    writeMaintenanceOwner(io, path.join(lockPath, 'owner.json'), { pid: process.pid, token, createdAt: Date.now() });
    syncDirectory(io, parent);
    let released = false;
    return {
      token,

...
const { nativeStoreKind, nativeFingerprint, nativeSchemaFingerprint, planNativeReferences } = require('./rekey-native-policy');
...
function inspectNativeStore(root, relative, mapping) {
  ...
  try {
    ...
    return { source: relative, kind, before, after, schemaDigest: nativeSchemaFingerprint(db), ...references,

...
/** Diagnostic content outside target roots is not rolled back, but schema is still inspected. */
function nativeSchemaFingerprint(db) {
  ...
  return sha256(JSON.stringify([schema, header]));
}
...
module.exports = { nativeStoreKind, isNativeSidecar, nativeFingerprint, nativeSchemaFingerprint, planNativeReferences };

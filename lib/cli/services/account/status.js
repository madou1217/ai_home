...
function createAccountStatusChecker(deps = {}) {
  ...
  return function checkStatus(cliName, accountRef) {
    ...
    try {
      ...
      if (cliName === 'kimi' && !hasKimiApiKey(dbCreds)) {
        configured = hasUsableKimiOAuth(nativeAuth);
        if (configured) {
          accountName = formatKimiOAuthAccountName(nativeAuth, accountRef);
        }
      ...
      // ZCode 共享凭据落在 nativeAuth.credentials（~/.zcode/v2/
      // credentials.json 投影，AES-GCM 信封）。解密失败（跨机器导入未带
      // secret）时只要有令牌条目仍视为已配置，与 kimi 的宽松判定一致。
      if (cliName === 'zcode') {
        const credential = readZcodeOAuthCredential(nativeAuth);
        configured = credential.configured
          || Object.keys(nativeAuth.credentials && typeof nativeAuth.credentials === 'object'
            ? nativeAuth.credentials
            : {}).some((key) => key.startsWith('oauth:') || key === 'zcodejwttoken');
        if (configured) {
          accountName = hasNonEmptyString(credential.userInfo.email)
            ? credential.userInfo.email.trim()
            : hasNonEmptyString(credential.userInfo.name)
              ? credential.userInfo.name.trim()
              : 'ZCode OAuth';
        }
        return { configured, accountName, source: 'app-state.db' };
      }

...
const IDENTITY_PROBE_TIMEOUT_MS = 8000;
const IDENTITY_PROBE_MAX_BUFFER = 64 * 1024;
const IDENTITY_PROBE_PATH = path.join(__dirname, 'claude-web-session-identity-probe.js');
...
function extractClaudeAccountEmail(nativeAuth) {
  return extractClaudeAccountIdentity(nativeAuth).emailAddress;
}

function extractClaudeAccountIdentity(nativeAuth) {
  ...
  return {
    emailAddress: normalizeEmail(account && (
...
function listBrowserCookieSources(fsImpl, pathImpl, Database, hostHomeDir) {
  const sources = [];
...
function decryptAuthCookieRows(rows, sourceKey) {
  const decryptedRows = [];
  try {
    rows.forEach((row) => {
      ...
      decryptedRows.push({ row, value });
    });
    return decryptedRows;
  } catch (error) {
    wipeDecryptedCookieRows(decryptedRows);
    throw error;
  }
}

function encryptAuthCookieRows(decryptedRows, targetKey) {
  return decryptedRows.map(({ row, value }) => ({
    ...row,
    value: '',
...
function wipeDecryptedCookieRows(decryptedRows) {
  (Array.isArray(decryptedRows) ? decryptedRows : []).forEach((entry) => {
    if (entry && Buffer.isBuffer(entry.value)) entry.value.fill(0);
  });
}

function readCookieValues(decryptedRows) {
  const values = {};
  decryptedRows.forEach(({ row, value }) => {
    values[row.name] = value.toString('utf8');
  });
  return {
    sessionKey: String(values.sessionKey || ''),
    lastActiveOrg: String(values.lastActiveOrg || '')
  };
}

function normalizeVerificationResult(result) {
  const status = result && result.status;
  return status === 'match' || status === 'mismatch' || status === 'unavailable'
    ? { status }
    : { status: 'unavailable' };
}

function verifyClaudeWebSessionIdentity(input, options = {}) {
  const accountIdentity = input && input.accountIdentity;
  const cookies = input && input.cookies;
  const probeInput = Buffer.from(JSON.stringify({
    sessionKey: cookies && cookies.sessionKey,
    lastActiveOrg: cookies && cookies.lastActiveOrg
  }));
  const run = options.execFileSync || execFileSync;
  let output = null;
  try {
    output = run(options.nodeExecutable || process.execPath, [IDENTITY_PROBE_PATH], {
      input: probeInput,
      encoding: null,
      maxBuffer: IDENTITY_PROBE_MAX_BUFFER,
      timeout: IDENTITY_PROBE_TIMEOUT_MS,
      stdio: ['pipe', 'pipe', 'ignore']
    });
    const payload = JSON.parse(Buffer.from(output || '').toString('utf8'));
    if (payload && payload.status === 'rejected') return { status: 'mismatch' };
    if (!payload || payload.status !== 'ok') return { status: 'unavailable' };

    const expectedEmail = normalizeEmail(accountIdentity && accountIdentity.emailAddress);
    const actualEmail = normalizeEmail(payload.emailAddress);
    const expectedUuid = String(accountIdentity && accountIdentity.accountUuid || '')
      .trim()
      .toLowerCase();
    const actualUuid = String(payload.accountUuid || '').trim().toLowerCase();
    const organizationUuids = Array.isArray(payload.organizationUuids)
      ? payload.organizationUuids.map((value) => String(value || '').trim().toLowerCase())
      : [];
    const activeOrg = String(cookies && cookies.lastActiveOrg || '').trim().toLowerCase();
    const accountMatches = expectedEmail
      && actualEmail === expectedEmail
      && (!expectedUuid || actualUuid === expectedUuid);
    return {
      status: accountMatches && activeOrg && organizationUuids.includes(activeOrg)
        ? 'match'
        : 'mismatch'
    };
  } catch (_error) {
...
function inspectCookieIdentity(rows, safeStorage, accountIdentity, passwordReader, verifier) {
  let password = null;
  let key = null;
  let decryptedRows = [];
  let keepDecryptedRows = false;
  try {
    password = passwordReader(safeStorage);
    if (!Buffer.isBuffer(password) || password.length === 0) {
      return { status: 'unavailable' };
    }
    key = deriveChromiumKey(password);
    decryptedRows = decryptAuthCookieRows(rows, key);
    const verification = normalizeVerificationResult(verifier({
      accountIdentity,
      cookies: readCookieValues(decryptedRows)
    }));
    if (verification.status !== 'match') return verification;
    keepDecryptedRows = true;
    return { status: 'match', decryptedRows };
  } catch (_error) {
    return { status: 'unavailable' };
  } finally {
    if (!keepDecryptedRows) wipeDecryptedCookieRows(decryptedRows);
...
function syncClaudeDesktopWebSession(options = {}) {
  ...
  const accountIdentity = extractClaudeAccountIdentity(nativeAuth);
  if (!accountIdentity.emailAddress) return failure('account_email_unavailable');
  ...
  const verifier = options.verifySessionIdentity || ((input) => (
    verifyClaudeWebSessionIdentity(input, {
  ...
  if (fsImpl.existsSync(targetPath)) {
    ...
    try {
      if (hasCurrentAuthCookies(existingRows)) {
        const inspection = inspectCookieIdentity(
          existingRows,
          SAFE_STORAGE_DESCRIPTORS.claude,
          accountIdentity,
          passwordReader,
          verifier
        );
        if (inspection.status === 'match') {
          wipeDecryptedCookieRows(inspection.decryptedRows);
        ...
        if (inspection.status === 'unavailable') {
          return failure('existing_session_identity_unverified');
        }
        try {
          updateCookieDatabase(Database, fsImpl, targetPath, []);
        } catch (_error) {
          return failure('existing_session_cleanup_failed');
        }
      }
    } finally {
  ...
  const sources = listBrowserCookieSources(fsImpl, pathImpl, Database, hostHomeDir);
  let source = null;
  let decryptedRows = [];
  ...
  for (const candidate of sources) {
    ...
    if (inspection.status === 'match') {
      source = candidate;
      decryptedRows = inspection.decryptedRows;
      break;
    }
    if (inspection.status === 'unavailable') identityUnavailable = true;
  }
  if (!source) {
    sources.forEach((candidate) => wipeCookieRows(candidate.rows));
    if (identityUnavailable) return failure('browser_session_identity_unverified');
    return {
      ok: true,
      status: 'login_required',
  ...
  try {
    ...
    transformedRows = encryptAuthCookieRows(decryptedRows, targetKey);

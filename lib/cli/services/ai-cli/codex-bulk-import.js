...
function createCodexBulkImportService(options = {}) {
  ...
  function parseCodexRefreshTokenLine(line) {
    ...
    const identityKey = buildOAuthIdentity('codex', authJson);
  ...
  function parseCodexApiKeyRecord(line) {
    ...
    const identityKey = buildApiKeyIdentity('codex', config);
    if (!identityKey) return null;
    return {
  ...
  function buildCodexApiKeyAuth(entry) {
    return {
      OPENAI_API_KEY: String(entry.apiKey || '').trim()
    };
  }
  ...
  function collectExistingCodexAccounts() {
    ...
    for (const entry of entries) {
      ...
      try {
        const record = {
          config: fs.existsSync(path.join(profileDir, '.aih_env.json'))
            ? JSON.parse(fs.readFileSync(path.join(profileDir, '.aih_env.json'), 'utf8'))
            : {},
          auth: fs.existsSync(authPath)
            ? JSON.parse(fs.readFileSync(authPath, 'utf8'))
            : {}
        };
        const identityKey = buildApiKeyIdentity('codex', record);
        if (identityKey && !out.has(identityKey)) {
          const config = extractApiKeyConfig('codex', record);
          out.set(identityKey, {
            credentialKind: 'api-key',
            identityKey,
            apiKey: config.apiKey,
  ...
  function notifyAuthArtifactsChanged(accountId, before, source, reason) {
        ...
        while (true) {
          ...
          try {
            ...
            const authSnapshotBefore = accountArtifactHooks
              ...
              ? accountArtifactHooks.snapshotAccountAuthArtifacts('codex', id)
              : null;
            if (entry.credentialKind === 'api-key') {
              ensureDir(profileDir);
              fs.writeFileSync(path.join(profileDir, '.aih_env.json'), `${JSON.stringify(buildCodexEnvFromApiKey(entry), null, 2)}\n`);
              fs.writeFileSync(path.join(codexDir, 'auth.json'), `${JSON.stringify(buildCodexApiKeyAuth(entry), null, 2)}\n`);
            } else {
            ...
            notifyAuthArtifactsChanged(id, authSnapshotBefore, 'codex_bulk_import', 'imported_credentials_updated');

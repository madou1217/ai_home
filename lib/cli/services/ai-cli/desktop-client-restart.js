...
function createDesktopClientRestartService(deps = {}) {
  ...
  function normalizeLaunchEnv(input) {
    if (!input || typeof input !== 'object' || Array.isArray(input)) return {};
    const env = {};
    Object.entries(input).forEach(([key, value]) => {
      const normalizedKey = String(key || '').trim();
      const normalizedValue = String(value == null ? '' : value);
      if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(normalizedKey)) return;
      if (!normalizedValue || normalizedValue.includes('\0') || Buffer.byteLength(normalizedValue) > 4096) return;
      env[normalizedKey] = normalizedValue;
    });
    return env;
  }

  function buildDesktopLaunchOptions(platformKey, executablePath, launchEnv = {}) {
  ...
  function launchDesktopClient(platformKey, launchRecord, requestedLaunchEnv) {
    ...
    const launchEnv = normalizeLaunchEnv(requestedLaunchEnv);
    try {
      let child = null;
      if (platformKey === 'macos') {
        ...
        child = spawnImpl(
          'open',
          ['-a', appTarget, ...envArgs],
          buildDesktopLaunchOptions(platformKey, executablePath || appTarget, launchEnv)
        );
      } else {
        if (!executablePath) return { ok: false, reason: 'missing_executable_path' };
        child = spawnImpl(executablePath, [], buildDesktopLaunchOptions(platformKey, executablePath, launchEnv));
      }
  ...
  function launchPreparedDesktopClient(platformKey, launchRecord, launchEnv, prepareLaunch) {
    let launchPreparation = null;
    if (typeof prepareLaunch === 'function') {
      try {
        launchPreparation = prepareLaunch();
      } catch (_error) {
        return {
          ok: false,
          reason: 'launch_preparation_failed',
          launchPreparation: { ok: false, reason: 'launch_preparation_exception' }
        };
      }
      if (launchPreparation && launchPreparation.ok === false) {
        return {
          ok: false,
          reason: 'launch_preparation_failed',
          launchPreparation
    ...
    const launchResult = launchDesktopClient(platformKey, launchRecord, launchEnv);
    return launchPreparation
      ? { ...launchResult, launchPreparation }
      : launchResult;
  }
  ...
  function restartDetectedDesktopClient(cliName, options = {}) {
    ...
    const launchEnv = normalizeLaunchEnv(options && options.launchEnv);
    const prepareLaunch = options && typeof options.prepareLaunch === 'function'
    ...
    if (running.detected) {
      ...
      const launchResult = launchPreparedDesktopClient(
        platformKey,
        launchRecord,
        launchEnv,
        prepareLaunch
      ...
      if (!launchResult.ok) {
        return {
          ...
          ...(launchResult.launchPreparation
            ? { launchPreparation: launchResult.launchPreparation }
            : {}),
          cachedPathUpdated
        };
      }

...
function augmentPathEnv(baseEnv, pathEntries, pathImpl = nodePath) {
  ...
  const next = [];
  const seen = new Set();
  // Explicit runtime PATH wins over provider-managed fallback directories.
  // This keeps tests, isolated auth jobs, and per-run runtime tools from being
  // shadowed by an older CLI installed in the host home directory.
  [...current, ...pathEntries].forEach((entry) => {
...
function resolveProviderCliPath(provider, options = {}) {
  ...
  const runtimeToolsDir = String(
    options.runtimeToolsDir || baseEnv.AIH_RUNTIME_TOOLS_DIR || ''
  ).trim();
  const env = augmentPathEnv(baseEnv, [runtimeToolsDir, ...pathEntries], pathImpl);
...
// Provider 安装计划优先由同名 app-installer 模块提供；这里采用延迟加载，
// 避免 app-installers/provider-factory.js 与本 facade 形成初始化循环。
function resolveProviderInstallPlans(provider, pkg, options = {}) {
  if (typeof options.resolveInstallPlans === 'function') {
    const custom = options.resolveInstallPlans(provider, pkg, options);
  ...
  try {
    ...
    if (installer && typeof installer.resolveCliInstallPlans === 'function') {
      const custom = installer.resolveCliInstallPlans(options);
      if (Array.isArray(custom) && custom.length > 0) return custom.filter(Boolean);
    }
  } catch (_error) {
    // 独立安装器不可用时回退到历史策略，保证 CLI 运行时仍可诊断。
  }
  return resolveNativeCliInstallPlans(provider, pkg, options);
}
    ...
    try {
      child = spawnImpl(plan.command, plan.args, {
        ...
        env: {
          ...
          CI: '1',
          NONINTERACTIVE: '1',
          // 有的官方安装脚本只认环境变量来选版本（codex 的 install.sh 第 5 行
          // `RELEASE="${CODEX_RELEASE:-latest}"`）。让 plan 显式携带 env，比把
          // `VAR=值` 拼进 shell 命令串安全（无注入面），且 POSIX/Windows 写法一致。
          ...(plan && plan.env && typeof plan.env === 'object' ? plan.env : {})
        },
...
async function installNativeCliWithProgress(provider, options = {}) {
  ...
  const plans = resolveProviderInstallPlans(normalizedProvider, pkg, {
    path: options.path || nodePath,
    processObj,
    hostHomeDir: resolveHostHome(options),
    resolveNpmInstall,
    resolveInstallPlans: options.resolveInstallPlans
  });

...
/**
 * Resolve a provider's on-disk CLI binary and, when missing, run the official
 * non-interactive install plan (Qoder official installer / Claude Win / npm).
 *
 * CLI interactive path still asks the user; WebUI/server path must auto-close
 * the loop without prompting — otherwise add-account dies on cli_not_found.
 */
...
function resolveProviderCliPath(provider, options = {}) {
  ...
  return resolve(binaryName, resolveOpts)
    || (binaryName !== normalizedProvider ? resolve(normalizedProvider, resolveOpts) : '')
    || '';
}
...
function buildCliNotFoundMessage(provider, ensureResult = {}) {
  ...
  return `未找到 ${binaryName} CLI，自动安装失败（${failedLabels}）。请手动安装后重试。`;
}

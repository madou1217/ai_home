...
const { qoderStrategy } = require('./qoder-strategy');
const { kiroStrategy } = require('./kiro-strategy');
const { grokStrategy } = require('./grok-strategy');
const { kimiStrategy } = require('./kimi-strategy');
const { zcodeStrategy } = require('./zcode-strategy');
const { codebuddyStrategy } = require('./codebuddy-strategy');
...
const STRATEGY_BY_PROVIDER = {
  ...
  qoder: qoderStrategy,
  qodercn: qoderStrategy,
  grok: grokStrategy,
  kimi: kimiStrategy,
  kiro: kiroStrategy,
  zcode: zcodeStrategy,
  codebuddy: codebuddyStrategy,
  // 国内站与国际站共用同一 CLI 二进制与同一隔离机制（HOME + configDir），
  // 差别只在投影根名（.codebuddy-cn）与站点默认值，因此复用同一策略。
  codebuddycn: codebuddyStrategy,
  workbuddy: codebuddyStrategy,
  workbuddycn: codebuddyStrategy
};

...
function resolveAiHomeDir(ctx) {
  if (ctx && ctx.aiHomeDir && typeof ctx.aiHomeDir === 'string') return ctx.aiHomeDir.trim();
  if (ctx && ctx.sandboxDir && ctx.path) {
    // ~/.ai_home/run/auth-projections/opencode/<accountRef> -> ~/.ai_home
    return ctx.path.resolve(ctx.sandboxDir, '..', '..', '..', '..');
  }
  return nodePath.join(nodeOs.homedir(), '.ai_home');
}

function resolveHealthyGatewayModels(ctx) {
  try {
    const fsImpl = (ctx && ctx.fs) || nodeFs;
...
function resolveGatewayProfile(ctx) {
  ...
  // 数字账号：当且仅当 ctx 明确包含 baseEnv.AIH_OPENCODE_GATEWAY_BYOK === '1' 或启动上下文需要注入时
  // 或是运行环境准备时，由 buildAihServerProfileEnv / router 提供。未显式开启时保留纯净原生环境。
  const autoByok = String((env && env.AIH_OPENCODE_GATEWAY_BYOK) || '').trim();
  if (autoByok === '1') {
    try {
      const fsImpl = (ctx && ctx.fs) || nodeFs;
      const aiHomeDir = resolveAiHomeDir(ctx);
...
function buildEnvPatch(ctx) {
  ...
  const isExplicitServer = gateway && gateway.isExplicitServerProfile;
  ...
  Object.assign(set, {
    ...
    ...(isExplicitServer ? { XDG_DATA_HOME: path.join(hostHomeDir, '.local', 'share') } : {}),

...
const GROK_WINDOWS_INSTALL_URL = 'https://x.ai/cli/install.ps1';
const GROK_POSIX_INSTALL_URL = 'https://x.ai/cli/install.sh';
...
const QODER_REGIONS = Object.freeze({
  qoder: Object.freeze({
    ...
    // `qoder` is the desktop editor launcher on Windows and does not support
    // Qoder CLI auth/config isolation flags. The provider id remains `qoder`,
    // but its executable contract is strictly `qodercli`.
...
function buildWindowsPs1Plan(id, label, ps1Url, options = {}) {
  ...
  const pathImpl = resolvePlatformPath(normalizePlatform(processObj), options.path);
...
function buildQoderDirectInstallPlan(region, options = {}) {
  ...
  const pathImpl = resolvePlatformPath(platform, options.path);
...
const kiroInstallStrategy = Object.freeze({
  name: 'kiro',
  matches(provider) {
    return String(provider || '').trim() === 'kiro';
  },
  collectPathEntries(provider, options = {}) {
    const pathImpl = options.path;
    const hostHomeDir = String(options.hostHomeDir || '').trim();
    ...
    if (normalizePlatform(processObj) === 'win32') {
      ...
      const localAppData = String(env.LOCALAPPDATA || '').trim()
        || (hostHomeDir ? pathImpl.join(hostHomeDir, 'AppData', 'Local') : '');
      return localAppData ? [pathImpl.join(localAppData, 'Kiro-Cli')] : [];
    }
    const localBin = joinIf(pathImpl, hostHomeDir, '.local', 'bin');
    return [localBin, '/usr/local/bin'].filter(Boolean);
  },
  resolveInstallPlans(provider, pkg, options = {}) {
    const processObj = options.processObj || process;
    if (normalizePlatform(processObj) === 'win32') {
      return [buildWindowsPs1Plan(
        'kiro_windows_official',
        'Kiro CLI official Windows installer',
        'https://cli.kiro.dev/install.ps1',
        options
      )];
    }
    return [buildPosixCurlBashPlan(
      'kiro_posix_official',
      'Kiro CLI official installer',
      'https://cli.kiro.dev/install'
    )];
  },
  binaryNames: Object.freeze(['kiro-cli'])
});
const grokInstallStrategy = Object.freeze({
  name: 'grok',
  matches(provider) {
    return String(provider || '').trim() === 'grok';
  },
  collectPathEntries(provider, options = {}) {
    const pathImpl = options.path;
    const hostHomeDir = String(options.hostHomeDir || '').trim();
    return pathImpl && hostHomeDir ? [pathImpl.join(hostHomeDir, '.grok', 'bin')] : [];
  },
  resolveInstallPlans(provider, pkg, options = {}) {
    const processObj = options.processObj || process;
    if (normalizePlatform(processObj) === 'win32') {
      const plan = buildWindowsPs1Plan(
        'grok_windows_official',
        'Grok CLI official Windows installer',
        GROK_WINDOWS_INSTALL_URL,
        options
      );
      const commandIndex = plan.args.indexOf('-Command') + 1;
      plan.args[commandIndex] = [
    ...
    return [buildPosixCurlBashPlan(
      'grok_posix_official',
      'Grok CLI official installer',
      GROK_POSIX_INSTALL_URL
    )];
  },
  binaryNames: Object.freeze(['grok'])
});
...
// ZCode CLI ships inside the ZCode desktop app (a bundled node script with a
// `#!/usr/bin/env node` shebang), not as an npm package or standalone
// installer. Discovery only: the resolved .cjs is executed through node by
// buildPtyLaunch's node-script branch.
const zcodeInstallStrategy = Object.freeze({
  name: 'zcode',
  matches(provider) {
    return String(provider || '').trim() === 'zcode';
  },
  collectPathEntries(provider, options = {}) {
    const pathImpl = options.path;
    const hostHomeDir = String(options.hostHomeDir || '').trim();
    ...
    if (normalizePlatform(processObj) === 'win32') {
      ...
      const localAppData = String(env.LOCALAPPDATA || '').trim()
        || (hostHomeDir ? pathImpl.join(hostHomeDir, 'AppData', 'Local') : '');
      if (programFiles) entries.push(pathImpl.join(programFiles, 'ZCode', 'resources', 'glm'));
      if (localAppData) entries.push(pathImpl.join(localAppData, 'Programs', 'ZCode', 'resources', 'glm'));
    } else {
      entries.push('/Applications/ZCode.app/Contents/Resources/glm');
  ...
  binaryNames: Object.freeze(['zcode.cjs', 'zcode'])
});
...
function collectStrategyPathEntries(provider, options = {}) {
  ...
  const processObj = options.processObj || process;
  return strategy.collectPathEntries(provider, {
    ...options,
    path: resolvePlatformPath(normalizePlatform(processObj), options.path)
  }).filter(Boolean);
}
...
function resolveStrategyInstallPlans(provider, pkg, options = {}) {
  ...
  const processObj = options.processObj || process;
  return strategy.resolveInstallPlans(provider, pkg, {
    ...options,
    path: resolvePlatformPath(normalizePlatform(processObj), options.path)
  }).filter(Boolean);
}

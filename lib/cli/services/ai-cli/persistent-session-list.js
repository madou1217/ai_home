...
function formatSessionPickerProjectRow(displayRow, layout = {}, selected = false, animationFrame = 0) {
  const lineWidth = resolveSessionPickerLineWidth(layout.columns);
  const projectPath = formatSessionPickerProjectPath(displayRow.projectPath, layout);
  const visiblePath = truncateCells(projectPath, lineWidth);
  if (selected) return rainbow(visiblePath, animationFrame);
  const slashIndex = visiblePath.lastIndexOf('/');

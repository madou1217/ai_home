...
const { getCliRelayProfile } = require('./relay/cli-relay-profile');
const {
...
function listPersistentSessions(cliName, accountRef, options = {}) {
  ...
  if (aiHomeDir && !options.collectOnly) {
    const providerConfPath = persistentSession.resolveProviderTmuxConfPath(
      resolveAihRunPath(aiHomeDir, 'tmux', 'tmux.conf'),
      cliName
    );
    const confPath = persistentSession.ensureTmuxConf(providerConfPath, fsImpl, {
      cliName,
  ...
  if (cliName === 'codex' && aiHomeDir && cmd.socket) {
    const registry = options.persistentSessionRegistry || persistentSessionRegistry;
    for (const session of decoratedSessions) {
      const nativeSessionId = String(session && session.agentSessionId || '').trim();
      const sessionName = String(session && session.name || '').trim();
      if (!nativeSessionId || !sessionName) continue;
      try {
        registry.bindNativeSession(
          aiHomeDir,
          cmd.socket,
          sessionName,
          nativeSessionId,

...
const SESSION_PICKER_HEADER = '[aih] 选择要进入的持久会话（Enter=进入；旧 UTF-8 行会新开兼容会话，x=关闭选中，X=关闭闲置，↑/↓=选择，q/Esc=退出）';
...
function padSessionDescriptionCells(row, width) {
  const base = String(row && (row.description || row.name) || '').trim();
  const sessionId = normalizeSessionId(row && row.agentSessionId);
  const legacyMarker = isLegacyUtf8RuntimeRow(row) ? ' [旧 tmux UTF-8 运行时]' : '';
  if (!sessionId) return padCells(`${base}${legacyMarker}`, width);

  const suffix = ` (${sessionId})${legacyMarker}`;
  const full = base ? `${base}${base.includes(sessionId) ? legacyMarker : suffix}` : `(${sessionId})${legacyMarker}`;
...
function formatSessionPickerPrefix(row, selected) {
  const cursor = selected ? '>' : ' ';
  const dot = row.live ? SESSION_PICKER_LIVE_ICON : '○';
...
function buildSessionPickerDisplayRows(rows) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (!group) {
      group = { type: 'project', projectPath };
...
function resolveSessionPickerLayout(rows, options = {}) {
  const columns = Math.max(60, Number(options.columns) || SESSION_PICKER_DEFAULT_COLUMNS);
  ...
  const maxTitleForColumns = Math.max(
    SESSION_PICKER_TITLE_MIN_WIDTH,
    columns - prefixWidth
  );
...
function formatSessionPickerRow(row, selected, layout = {}) {
  const prefix = formatSessionPickerPrefix(row, selected);
  const titleWidth = Math.max(SESSION_PICKER_TITLE_MIN_WIDTH, Number(layout.titleWidth) || SESSION_PICKER_TITLE_MAX_WIDTH);
  const title = padSessionDescriptionCells(row, titleWidth);
  return `${prefix}${title}`;
}

function formatSessionPickerProjectRow(displayRow, layout = {}) {
  const columns = Math.max(20, Number(layout.columns) || SESSION_PICKER_DEFAULT_COLUMNS);
  return truncateCells(displayRow.projectPath, columns);
}
...
function renderSessionPickerRows(rows, selectedIndex, write, layout, message = '') {
  const lines = buildSessionPickerDisplayRows(rows).map((displayRow) => {
    if (displayRow.type === 'project') return formatSessionPickerProjectRow(displayRow, layout);
    return formatSessionPickerRow(displayRow.row, displayRow.itemIndex === selectedIndex, layout);
  });
  if (message) lines.push(`${yellow('[aih]')} ${message}`);
...
function selectPersistentSessionRow(rows, options = {}) {
  ...
  try {
    ...
    while (true) {
      ...
      const layout = resolveSessionPickerLayout(items, { columns: stdout.columns });
      if (!headerRendered) {
        write(`\r\x1b[2K${SESSION_PICKER_HEADER}\n`);
        headerRendered = true;
...
function selectPersistentSessionRowAsync(rows, options = {}) {
  ...
  const render = () => {
    ...
    const lines = [
      ...
      ...buildSessionPickerDisplayRows(items).map((displayRow) => {
        if (displayRow.type === 'project') return formatSessionPickerProjectRow(displayRow, layout);
        return formatSessionPickerRow(displayRow.row, displayRow.itemIndex === selectedIndex, layout);
      })
    ];
    if (message) lines.push(`\x1b[33m[aih]\x1b[0m ${message}`);
  ...
  const refreshItems = (preferredTarget = '', reason = '') => {
    if (typeof options.refreshRows !== 'function') return true;
    const nextItems = normalizePickerItems(options.refreshRows());
    const selectedTarget = preferredTarget || (items[selectedIndex] && items[selectedIndex].targetSession) || '';
    const changed = nextItems.map((row) => `${row.accountId}:${row.targetSession}:${row.live}`).join('|')
      !== items.map((row) => `${row.accountId}:${row.targetSession}:${row.live}`).join('|');
    items = nextItems;
    ...
    return !selectedTarget || items.some((row) => row.targetSession === selectedTarget);
  };
...
function printPersistentProjectGroups(rows, log = console.log) {
  ...
  for (const [projectPath, groupRows] of groups) {
    log(dim(projectPath));
    groupRows.forEach((row) => log(formatPersistentSessionRow(row)));
  }
}
...
function listPersistentSessions(cliName, id, options = {}) {
  ...
  const sessions = persistentSession.parseSessionList(res && res.stdout);
  if (!sessions.length) {
  ...
  const view = persistentSession.describeSessionList(decoratedSessions, { cliName, id, cwd });
  ...
  log(dim('  提示（⠿=在用 / ○=空闲）：Enter=进入；旧 UTF-8 行会新开兼容会话，x=关闭选中，X=关闭闲置，-M=镜像，-R=接管。'));
  return { rows, socket: cmd.socket, unavailable: false };
}
...
function buildGlobalSessionGroups(rows) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (!groupsByProject.has(projectPath)) {
      groupsByProject.set(projectPath, { projectPath, rows: [], latestCreated: 0 });
    }
...
function formatGlobalPersistentSessionsByProject(rows) {
  ...
  for (const group of groups) {
    lines.push(dim(group.projectPath));
    for (const row of group.rows) {
      lines.push(formatPersistentSessionRow(row));
    }
  }
  lines.push(dim('  提示（⠿=在用 / ○=空闲）：Enter=进入；旧 UTF-8 行会新开兼容会话，x=关闭选中，X=关闭闲置，-M=镜像，-R=接管。'));
  return lines.join('\n');
}
...
function enterGlobalPersistentSession(selected, context = {}) {
  ...
  if (isLegacyUtf8RuntimeRow(selected)) {

...
function resolveSessionPickerLayout(rows, options = {}) {
  ...
  const titleWidth = availableTitleWidth >= SESSION_PICKER_TITLE_MIN_WIDTH
    ...
    : availableTitleWidth;
  return { columns, lineWidth, titleWidth };
}
...
function formatSessionPickerProjectRow(displayRow, layout = {}) {
  return truncateCells(displayRow.projectPath, resolveSessionPickerLineWidth(layout.columns));
}
...
function buildSessionPickerLines(rows, selectedIndex, layout, message = '') {
...
function selectPersistentSessionRowAsync(rows, options = {}) {
  ...
  const render = () => {
    ...
    const lines = [
      ...
      ...buildSessionPickerLines(items, selectedIndex, layout, message)
    ];

...
//   codex  → resume the exact registry-bound thread id; never guess by cwd
...
function buildRestoreForwardArgs(rawEntry) {
  const entry = typeof rawEntry === 'string' ? { provider: rawEntry } : (rawEntry || {});
  const name = String(entry.provider || '').trim().toLowerCase();
  if (name === 'codex') {
    const threadId = resolveCodexPersistentThreadId(entry);
    return threadId ? ['resume', threadId] : [];
  }
...
function planRestoreActions(entries, probeBySocket, options = {}) {
  ...
  for (const entry of Array.isArray(entries) ? entries : []) {
    ...
    if (
      String(entry.provider || '').trim().toLowerCase() === 'codex'
      && !resolveCodexPersistentThreadId(entry)
    ) {
      plan.unrecoverable.push({ ...entry, unrecoverable: 'codex-thread-unbound' });
      continue;
    }
    plan.restore.push(entry);
  }

  const claimedCodexThreads = new Set(
    plan.alive
      .filter((entry) => String(entry.provider || '').trim().toLowerCase() === 'codex')
      .map(resolveCodexPersistentThreadId)
      .filter(Boolean)
  );
  const winnerByThreadId = new Map();
  for (const entry of plan.restore) {
    if (String(entry.provider || '').trim().toLowerCase() !== 'codex') continue;
    const threadId = resolveCodexPersistentThreadId(entry);
    if (!threadId || claimedCodexThreads.has(threadId)) continue;
    const current = winnerByThreadId.get(threadId);
    if (selectPreferredCodexPersistentOwner([current, entry].filter(Boolean)) === entry) {
      winnerByThreadId.set(threadId, entry);
    }
  }
  const deduplicatedRestore = [];
  for (const entry of plan.restore) {
    if (String(entry.provider || '').trim().toLowerCase() !== 'codex') {
      deduplicatedRestore.push(entry);
      continue;
    }
    const threadId = resolveCodexPersistentThreadId(entry);
    if (!claimedCodexThreads.has(threadId) && winnerByThreadId.get(threadId) === entry) {
      claimedCodexThreads.add(threadId);
      deduplicatedRestore.push(entry);
      continue;
    }
    plan.unrecoverable.push({ ...entry, unrecoverable: 'codex-thread-duplicate' });
  }
  plan.restore = deduplicatedRestore;
  return plan;
}
...
function createPersistentSessionRestore(deps = {}) {
  ...
  function spawnRestoreChild(entry) {
    const forwardArgs = buildRestoreForwardArgs(entry);
    if (entry.provider === 'codex' && forwardArgs.length === 0) {
      throw new Error('restore_codex_thread_unbound');
    }
    ...
    const child = spawn(
      ...
      childArgs,
      {
        ...
        detached: true,
        stdio: 'ignore',
        // Windows: detached restore children must not pop a console window
        // for every restored session after a reboot.
        windowsHide: true
      }
    );
  ...
  function restorePersistentSessions(options = {}) {
    ...
    try {
      ...
      for (const entry of plan.unrecoverable) {
        registry.markEntryUnrecoverable(
          aiHomeDir,
          entry.socket,
          entry.session,
          entry.unrecoverable,

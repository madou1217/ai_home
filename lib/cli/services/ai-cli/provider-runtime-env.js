...
const { reconcileOpenCodeNativeAuth } = require('../../../account/opencode-native-auth');
const { applyWindowsSystemProxy } = require('../../../runtime/windows-system-proxy');
const { normalizeAnthropicSdkBaseUrl } = require('../../../account/anthropic-endpoint');
const {
  CODEX_MANAGED_LAUNCH_ENV,
  markCodexManagedLaunch
} = require('../../../runtime/codex-launch-context');
const {
...
const PROVIDER_HOME_KEYS = Object.freeze([
  ...
  'ZCODE_DATA_BASE_DIR',
  'KIMI_CODE_HOME',
  // CodeBuddy Code 的 configDir 只能由该 env 覆盖（CLI 无 --config-dir 参数），
  // 宿主的 ~/.codebuddy 属于另一个账号，必须在 host-scoped 基座里剥掉。
...
const ACCOUNT_SCOPED_ENV_KEYS = Object.freeze([
  ...
  // 区域选择属于账号身份：宿主的国内站/国际站选择不能泄漏到另一个账号。
  // CLI 与 IDE 各读一个键（2026-09-14 在 WorkBuddy.app 的 app.asar 内实测到
  // ENV_KEY_CODEBUDDY_COPILOT_INTERNET_ENVIRONMENT），两个都必须按账号剥离。
  'CODEBUDDY_INTERNET_ENVIRONMENT',
  'CODEBUDDY_COPILOT_INTERNET_ENVIRONMENT',
  'CODEBUDDY_CONFIG_DIR',
  'WORKBUDDY_CONFIG_DIR',
  'WORKBUDDY_USER_DATA_DIR',
  // 产品配置及 sidecar 引导也能选择宿主身份，不能穿过账号启动边界。
  ...
  'CODEBUDDY_RUNTIME_INSTANCE_ID',
  'CODEBUDDY_API_KEY_DISABLED',
  'WORKBUDDY_AT_REST_ENCRYPTION',
  PROVIDER_ACCOUNT_REF_ENV
]);
...
const ENV_AUTH_KEYS_BY_PROVIDER = Object.freeze({
  ...
  kimi: Object.freeze(['MOONSHOT_API_KEY']),
  // PAT-only Qoder accounts skip native-auth projection when the env token is set.
  qoder: Object.freeze(['QODER_PERSONAL_ACCESS_TOKEN']),
  qodercn: Object.freeze(['QODER_PERSONAL_ACCESS_TOKEN']),
  zcode: Object.freeze(['ZCODE_API_KEY']),
  // API-key 账号完全靠 env 鉴权，沙箱里不需要 .credentials.json 投影；
  // OAuth 账号没有该键，仍会走原生凭据物化。
  // 国内站与国际站共用同一二进制与同一组 env 键，凭据键完全一致。
  codebuddy: Object.freeze(['CODEBUDDY_API_KEY']),
  codebuddycn: Object.freeze(['CODEBUDDY_API_KEY']),
  // WorkBuddy 桌面端内嵌 CodeBuddy Code runtime，账号凭据口径与 codebuddy 相同；
  // 国内站（workbuddycn）与国际站（workbuddy）共用同一组 env 键与同一个凭据键。
  workbuddy: Object.freeze(['CODEBUDDY_API_KEY']),
  workbuddycn: Object.freeze(['CODEBUDDY_API_KEY'])
});
...
function resolveProviderRuntimeScope(provider, projectionDir, baseEnv = {}, options = {}) {
  const candidateDir = normalizePathString(projectionDir);
  ...
  // kimi 的钉账号 relay 与裸网关 profile 都不投影 OAuth 凭据
  // （projectionRequired=false），但都仍需要隔离目录放 relay config.toml：
  // 钉账号用账号沙箱，网关 profile 用网关 runtime 目录；写宿主真实
  // ~/.kimi-code 会污染用户配置并被其中的 oauth 块旁路网关
  // （见 relay/cli-relay-profile.js 契约）。
  const relayKeepsSandbox = (options.authRelayed === true || options.gateway === true)
    && options.relayKeepsAccountSandbox === true;
  return {
    hostHomeDir,
    projectionRequired,
    runtimeDir: projectionRequired || relayKeepsSandbox ? candidateDir : hostHomeDir
  };
}
...
function resolveRuntimeHostHome(_runtimeDir, baseEnv, options = {}) {
  const explicitHostHomeDir = normalizePathString(options.hostHomeDir);
  if (explicitHostHomeDir) return explicitHostHomeDir;
...
function normalizeProxyEnv(envObj, options = {}) {
  const env = applyWindowsSystemProxy(envObj, options);
  const pairs = [
...
function buildProviderLaunchContext(provider, profileDir, baseEnv = {}, options = {}) {
  ...
  const sandboxDir = normalizePathString(profileDir);
  if (!sandboxDir) throw new Error('provider_runtime_profile_dir_invalid');
  ...
  const zcodeDataBaseDir = options.zcodeDataBaseDir || pathImpl.join(sandboxDir, '.zcode');
  ...
  return {
    ...
    launchKind: String(options.launchKind || '').trim().toLowerCase(),
    // relay 标志由调用方显式传入（经 CliRelayProfile 注册表判定），本模块不
    // 自行推导，避免与调用点各自重算导致口径漂移。kimi 的 config 模板按它
    // 二选一（见 launch-profile/kimi-config.js）。
    authRelayed: Boolean(options.authRelayed),
    gateway: Boolean(options.gateway)
  };
}

// 这份 env 的 codex 鉴权已经由 AIH 按 accountRef 选定，全局 codex CLI hook
// 必须直接透传；否则它会回到 `codex set-default` 账号重新推导鉴权，把本账号的
// OPENAI_API_KEY 删掉（OAuth 默认账号）或换成别的账号的 key（跨账号串号）。
// 反向同样重要：非 codex 启动要丢掉继承来的标记，避免它们再拉起 codex 时误透传。
function applyCodexManagedLaunch(env, cliName) {
  if (cliName === 'codex') return markCodexManagedLaunch(env);
...
function buildProviderRuntimeEnv(provider, profileDir, baseEnv = {}, options = {}) {
  ...
  if (accountRef) env[PROVIDER_ACCOUNT_REF_ENV] = accountRef;
  else delete env[PROVIDER_ACCOUNT_REF_ENV];
  applyCodexManagedLaunch(env, launchCtx.cliName);
  ...
  // The Anthropic SDK appends its own `/v1/messages` to ANTHROPIC_BASE_URL.
  // Relays often store the base WITH the version segment (e.g. `.../v1`), which
  // produces `/v1/v1/messages` → 404. Strip the trailing `/v1` before handing
  // the env to the native CLI. The gateway path already normalises this in
  // resolveProviderPath, so only the native CLI env needs fixing.
  if (env.ANTHROPIC_BASE_URL) {
    env.ANTHROPIC_BASE_URL = normalizeAnthropicSdkBaseUrl(env.ANTHROPIC_BASE_URL);
  }

  const normalizedEnv = normalizeProxyEnv(normalizeUtf8LocaleEnv(env, { platform: options.platform }), {
    platform: options.platform,
    execFileSync: options.execFileSync
  });
  return applyAccountEgressProxyEnv(normalizedEnv, launchCtx.cliName, {
    ...options,
    accountRef
  });
}
...
function prepareProviderRuntime(provider, profileDir, baseEnv = {}, options = {}) {
  ...
  const launchCtx = buildProviderLaunchContext(provider, profileDir, scopedBaseEnv, options);
  const fsImpl = options.fs || launchCtx.fs || nodeFs;
  if (launchCtx.cliName === 'kimi' && options.isLogin !== true && options.accountRef) {
    const reconcileKimiHostCredentials = createKimiHostCredentialReconciler({
      fs: fsImpl,
      path: launchCtx.path,
      aiHomeDir: options.aiHomeDir,
      hostHomeDir: options.hostHomeDir || launchCtx.hostHomeDir
    });
    const reconciliation = reconcileKimiHostCredentials(options.accountRef);
    if (reconciliation && reconciliation.ok === false) {
      throw new Error(`kimi_host_auth_reconciliation_failed:${reconciliation.reason || 'unknown_error'}`);
    }
  }
  // OPENCODE_API_KEY 在 opencode 沙箱里会被 strategy 剥掉，只有原生 auth.json 算数。
  // 老账号可能只存了 env key，启动前补齐，否则 CLI 会以匿名身份跑（付费模型必然 401）。
  if (launchCtx.cliName === 'opencode' && options.accountRef) {

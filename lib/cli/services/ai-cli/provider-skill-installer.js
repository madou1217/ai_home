...
const { resolveProviderNativeRoot } = require('../../../runtime/provider-storage-policy');
...
function getSkillLinkPath(ctx) {
  ...
  if (ctx.cliName === 'claude' || ctx.cliName === 'codex') {
    const providerRoot = resolveProviderNativeRoot(ctx.hostHomeDir, ctx.cliName, pathImpl);
    return providerRoot ? pathImpl.join(providerRoot, 'skills', SKILL_NAME) : '';
  }
  return '';
}
...
function getManagedInstructionPath(ctx) {
  ...
  if (ctx.cliName === 'agy') {
    // AGY's fake HOME contains only an alias. The provider-wide instruction
    // source of truth lives in the native host ~/.gemini directory.
    return ctx.hostHomeDir ? pathImpl.join(ctx.hostHomeDir, '.gemini', 'GEMINI.md') : '';
  }
  return '';
}

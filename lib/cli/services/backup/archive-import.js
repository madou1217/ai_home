...
async function extractZipArchive({
  ...
  if (typeof onProgress === 'function') onProgress(5);
  const runFile = execFileSync || nodeChildProcess.execFileSync;
  try {
    runFile('unzip', ['-oq', zipPath, '-d', extractDir], { stdio: 'ignore' });
  } catch (unzipError) {
    try {
      runFile('python3', ['-m', 'zipfile', '-e', zipPath, extractDir], { stdio: 'ignore' });
    } catch (_pythonError) {
...
async function ensureArchiveExtractedByHash({
  ...
  const cacheRoot = aiHomeDir
    ? resolveAihRunPath(aiHomeDir, 'cache', 'import-zip')

...
function normalizeBlockerCode(blocker) {
  ...
  const knownProviders = ['codex', 'claude', 'agy', 'opencode', 'grok', 'qoder', 'qodercn'];
...
function inferProvider(blocker, context = {}) {
  ...
  const known = ['codex', 'claude', 'agy', 'opencode', 'grok', 'qoder', 'qodercn'];
...
function buildCommand(name, context = {}, extra = []) {
  ...
  if (name === 'server-add') {
    return commandLine(['aih server add <name>', '--url', endpoint, '--management-key', '<key>']);
  }
...
function describeBlocker(blocker, context = {}) {
  ...
  if (!raw) {
    ...
    details = {
      ...
      requiresConfirmation: false,
      external: false,
      reason: 'The client cannot read Fabric registry until a Server URL and Management Key are configured.',
      nextAction: 'Add the target Server, then re-run closure audit.',
      command: buildCommand('server-add', context)
    };

...
async function connectFabricBroker(options = {}, deps = {}) {
  ...
  const closed = new Promise((resolve) => {
    socket.once('close', () => resolve({ ok: true, reason: 'closed' }));
    socket.once('error', (error) => resolve({
      ok: false,
      reason: String((error && error.message) || error || 'socket_error')
    }));
  });
...
async function runFabricBrokerConnect(rawArgs = [], deps = {}) {
  ...
  const handle = await connectFabricBroker(options, deps);
  if (options.once) {
    handle.close();
    return {
      ok: true,

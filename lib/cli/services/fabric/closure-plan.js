...
function buildNodePlanItem(summary = {}, options = {}) {
  if (summary.nodeReady) {
    return createPlanItem({
      ...
      owner: 'aih',
      reason: 'The paired profile can read the target node registry.',
  ...
  return createPlanItem({
    id: 'node-registry-pairing',
    domain: 'node',
    status: 'action_required',
    title: 'Pair a ready server profile before reading Fabric nodes',
    owner: 'operator',
    canAutomate: true,
    ...
    commands: [
      commandLine(['aih fabric profile pair <pair-url|code>', '--endpoint', normalizeText(options.endpoint, 2048), '--json']),
...
function buildSelectedProviderPlanItem(summary = {}, providerStates = [], sessionProof = {}, options = {}) {
  ...
  if (!summary.nodeReady) {
    return createPlanItem({
      ...
      blockers: ['ready_server_profile_missing'],
      context: options,
      reason: 'Provider availability depends on the authorized node registry read; fix Server Setup pairing before classifying provider credentials.',
      commands: [
        commandLine(['aih fabric profile pair <pair-url|code>', '--endpoint', normalizeText(options.endpoint, 2048), '--json']),
        buildAuditCommand(options, ['--skip-session', '--json'])
      ],

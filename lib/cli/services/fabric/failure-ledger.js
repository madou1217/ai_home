...
function buildExecutionDecision(failures = [], businessClosure = {}, streamProof = {}, automation = {}, externalPrerequisites = []) {
  ...
  if (automation.blockedByProfileMissing === true) {
    return {
      decision: 'stop_pair_server_profile',
      state,
      canContinueWithoutInput: false,
      reason: 'The client must pair a ready server profile before AIH can read the registry or continue automation.',
      nextCommand: '',
      resumeWhen: ['A ready paired server profile is available and fabric nodes can read the target registry.']
    };
  }

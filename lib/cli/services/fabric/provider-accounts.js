...
function parseReauthArgs(argv = [], env = process.env) {
  ...
  for (let index = 0; index < argv.length;) {
    ...
    if (!isFlag(token) && !options.accountRef) {
      options.accountRef = normalizeText(token, 96);
      index += 1;
      continue;
    }
  ...
  if (!options.accountRef) throw createError('missing_account_ref', 'missing --account-ref');
  if (!isAccountRef(options.accountRef)) throw createError('invalid_account_ref', 'invalid --account-ref');
  return options;
}
...
function buildProviderAccountReauthCommand(provider, accountRef, endpoint = '', extra = []) {
  return commandLine([
    'aih fabric provider accounts reauth',
    '--provider',
    normalizeText(provider, 64).toLowerCase(),
    '--account-ref',
    normalizeText(accountRef, 96),
    ...(endpoint ? ['--endpoint', endpoint] : []),
    ...extra
  ]);
}
...
function firstSampleAccountRef(provider = {}) {
  return normalizeArray(provider.sampleClearableAccountRefs)
    .map((item) => normalizeText(item, 96))
    .find(Boolean) || '';
}

function buildCredentialCommands(action, provider, endpoint, accountRef) {
  const commands = [
  ...
  if (action === 'complete_oauth_reauth' && accountRef) {
    commands.unshift(buildProviderAccountReauthCommand(provider, accountRef, endpoint, ['--json']));
  }
...
function classifyProviderCredentialAction(providerReport = {}) {
  ...
  const accountCount = Number(providerReport.accountCount || 0);
  ...
  if (configured <= 0 && accountCount <= 0) {
    return {
...
function buildProviderCredentialHandoff(providerReport = {}, endpoint = '') {
  const provider = normalizeText(providerReport.provider, 64).toLowerCase();
  const authModes = providerAuthModes(providerReport);
  const accountRef = firstSampleAccountRef(providerReport);
  ...
  return {
    ...
    accountCount: Number(providerReport.accountCount || 0),
    sampleAccountRef: accountRef,
    requiredInput: classification.requiredInput,
    reason: classification.reason,
    commands: buildCredentialCommands(classification.action, provider, endpoint, accountRef)
  };
}
...
function evaluateProviderAccountReauthResponse(profile, options, response) {
  ...
  return {
    ...
    target: {
      ...
      accountRef: normalizeText(options.accountRef, 96),
...
async function runFabricProviderAccountReauth(rawOptions = {}, deps = {}) {
  ...
  if (!options.provider) throw createError('missing_provider', 'missing --provider');
  if (!options.accountRef) throw createError('missing_account_ref', 'missing --account-ref');
  if (!isAccountRef(options.accountRef)) throw createError('invalid_account_ref', 'invalid accountRef');
...
function formatAuditProviderLine(item = {}) {
  ...
  return [
    ...
    `accounts=${Number(item.accountCount || 0)}`,
...
function formatFabricProviderAccountsReport(report = {}) {
  ...
  if (remoteAudit && Object.keys(remoteAudit).length > 0) {
    lines.push(`  remote_audit: db=${summary.dbPresent === true} accounts=${Number(summary.accountCount || 0)} state_rows=${Number(summary.stateRows || 0)} blocked=${Number(summary.runtimeBlocked || 0)} clearable=${Number(summary.clearableRuntimeBlocks || 0)}`);
  ...
  if (report.mode === 'remote-reauth') {
    ...
    lines.push(`  target_account_ref: ${normalizeText(result.targetAccountRef, 96) || normalizeText(target.accountRef, 96)}`);
    if (normalizeText(result.transientAccountRef, 96)) {
      lines.push(`  transient_account_ref: ${normalizeText(result.transientAccountRef, 96)}`);
    }
  ...
  if (report.mode === 'remote-auth-job') {
    ...
    lines.push(`  account_ref: ${normalizeText(job.accountRef, 96) || '-'}`);

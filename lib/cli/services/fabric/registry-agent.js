...
const DEFAULT_RUNTIME_DIAGNOSTIC_PROVIDERS = Object.freeze(listProvidersByCapability('fabricRuntime'));
...
function sleepMs(ms, context = {}) {
  return waitForAbortableDelay(ms, context);
}
...
async function fetchReadyzAccountCounts(endpoint, deps = {}) {
  ...
  try {
    const response = await fetchImpl(readyzUrl(endpoint), {
      method: 'GET',
      signal: deps.signal
    });
...
function isBlockedRegistryError(error) {
...
async function runFabricRegistryAgent(rawArgs = [], deps = {}) {
  ...
  for (;;) {
    ...
    try {
      ...
      if (isBlockedRegistryError(error)) throw error;
    }

    const state = { attempts, failures, lastResult, lastError, lastProbes };
    if ((signal && signal.aborted)
      || (options.count > 0 && attempts >= options.count)
      || (shouldStop && shouldStop(state))) {
      break;
    }

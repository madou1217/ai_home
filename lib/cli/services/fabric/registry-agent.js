...
function canLoadRuntimeAccounts(deps = {}) {
  return deps.fs
    ...
    && typeof deps.getToolAccountIds === 'function'
    && typeof deps.getToolConfigDir === 'function'
...
function sanitizeAvailabilityReasons(reasons) {
  return (Array.isArray(reasons) ? reasons : []).map((item) => ({
    ...
    sampleAccountIds: (Array.isArray(item && item.sampleAccountIds) ? item.sampleAccountIds : [])
      .map((id) => nonEmptyString(id).slice(0, 64))
      .filter(Boolean)
      .slice(0, 5),

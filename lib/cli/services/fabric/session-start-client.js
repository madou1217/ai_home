...
function parseArgs(argv = [], env = process.env) {
  ...
  if (options.accountRef && !isAccountRef(options.accountRef)) {
    throw createError('invalid_account_ref', 'invalid --account-ref');
  }
  return options;
}
...
async function runFabricSessionStartClient(rawOptions = {}, deps = {}) {
  ...
  if (options.accountRef && !isAccountRef(options.accountRef)) {
    throw createError('invalid_account_ref', 'invalid accountRef');
  }

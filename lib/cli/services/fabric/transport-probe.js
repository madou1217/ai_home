...
const { subscribeAbort } = require('../../../runtime/abortable');
...
function createTimeoutSignal(timeoutMs, parentSignal) {
  ...
  const unsubscribeAbort = subscribeAbort(parentSignal, (reason) => controller.abort(reason));
...
function probeTcpTarget(target, options = {}, deps = {}) {
  ...
  return new Promise((resolve) => {
    ...
    unsubscribeAbort = subscribeAbort(deps.signal, () => finish({
      reachable: false,
      status: 'aborted',
...
async function probeHttpTarget(target, options = {}, deps = {}) {
  ...
  const linked = createTimeoutSignal(timeoutMs, deps.signal);
  try {

...
const { resolveNativeAuthIdentitySeed } = require('../../../account/account-identity');
const { prepareKiroIdentityProjection } = require('../../../account/kiro-native-login');
const {
  getPublicAccountRef,
  resolveAccountRef
...
function createUnifiedImportService(options = {}) {
  ...
  function hasProviderAccountMaterial(targetPath, provider) {
    ...
    if (isProviderNativeHomeSource(targetPath, safeProvider)) return true;
  ...
  function isKimiNativeHomeSource(targetPath, provider = '') {
    return String(provider || '').trim().toLowerCase() === 'kimi'
      && isRegularFile(path.join(targetPath, 'credentials', 'kimi-code.json'));
  }

  // 通用「源目录即 provider native 根」检测：auth artifact 去掉 storage
  // policy nativeRoot 后的相对路径在源目录下存在。zcode（~/.zcode 含
  // v2/credentials.json）与 kimi（~/.kimi-code 含 credentials/…）同构。
  function isProviderNativeHomeSource(targetPath, provider = '') {
    const safeProvider = String(provider || '').trim().toLowerCase();
    if (!safeProvider) return false;
    if (isKimiNativeHomeSource(targetPath, safeProvider)) return true;
    const policy = getProviderStoragePolicy(safeProvider);
    if (!policy || policy.nativeRoot.length === 0) return false;
    return getProviderAuthArtifacts(safeProvider).some((artifact) => {
      const relative = artifact.path.slice(policy.nativeRoot.length);
      return relative.length > 0 && isRegularFile(path.join(targetPath, ...relative));
    });
  }

  function shouldRefreshExistingKimiAccount(accountRef, projection) {
    const current = readAccountNativeAuth(fs, aiHomeDir, accountRef);
    const currentCredentials = readKimiOAuthCredentials(current);
    const incomingCredentials = readKimiOAuthCredentials(projection);
    const incomingExpiry = readKimiTokenExpiry(incomingCredentials);
    return incomingExpiry > readKimiTokenExpiry(currentCredentials);
  }
  ...
  function hasDirectProviderCredentials(sourceDir, provider) {
    return collectCredentialRelativePaths(provider)
      .some((relFile) => isRegularFile(path.join(sourceDir, relFile)));
  }

  function listCredentialAccountDirs(sourceDir, provider) {
    if (!directoryExists(sourceDir)) return [];
    if (hasDirectProviderCredentials(sourceDir, provider)) return [{ dir: sourceDir }];
    return fs.readdirSync(sourceDir, { withFileTypes: true })
      .filter((entry) => entry.isDirectory())
      .map((entry) => ({ dir: path.join(sourceDir, entry.name) }))
      .filter((account) => hasDirectProviderCredentials(account.dir, provider))
  ...
  function resolveImportSourceRoot({ extractDir, provider, folderHint }) {
    ...
    if (providerMode) {
      const candidateProviderDirs = [
        ...
        ...(safeProvider === 'kimi' ? [path.join(baseDir, '.kimi-code')] : []),
        baseDir
      ].filter(Boolean);
  ...
  async function importCredentialLayoutProviderSource(sourceDir, provider, parsed, onProgress) {
    ...
    const accounts = listCredentialAccountDirs(sourceDir, safeProvider);
    let imported = 0;
    let duplicates = 0;
    let invalid = 0;
    let failed = 0;
    ...
    for (let index = 0; index < accounts.length; index += 1) {
      ...
      try {
        ...
        if (candidateFiles.length === 0) {
          ...
          const nativeHome = isProviderNativeHomeSource(account.dir, safeProvider);
          const projection = readProviderAuthProjection(fs, account.dir, safeProvider, {
            path,
            nativeHome
          });
          let projectionMetadata;
          let expectedKiroVersions;
          if (safeProvider === 'kiro' && !dryRun) {
            // Token-only native stores need one authenticated identity lookup.
            // Dry-run remains offline and cannot claim verified enrollment.
            const verification = await prepareKiroIdentityProjection(fs, account.dir, {
              path, nativeHome, aiHomeDir,
              request: options.kiroIdentityRequest,
              signal: options.signal
            });
            projectionMetadata = verification.projectionMetadata;
            expectedKiroVersions = verification.expectedKiroVersions;
            Object.assign(projection, projectionMetadata);
          }
          const identity = resolveNativeAuthIdentitySeed(safeProvider, projection);
          if (!identity.identitySeed || identity.degraded) {
            invalid += 1;
          } else {
            // Kimi accounts created before stable user identity support were
            // keyed by a token hash. Match the native user first so a rotated
            // device/token is still reported as the existing account.
            const nativeIdentityMatch = ['kimi', 'kiro', 'grok', 'opencode', 'qoder', 'qodercn', 'zcode'].includes(safeProvider)
              ? findExistingAccountByNativeAuthIdentity(fs, aiHomeDir, safeProvider, projection)
              : null;
            if (nativeIdentityMatch?.ambiguous) {
              throw new Error('ambiguous_existing_identity');
            }
            const accountRef = nativeIdentityMatch
              ? nativeIdentityMatch.accountRef
              : getPublicAccountRef(`unique:${identity.identitySeed}`);
            const existing = nativeIdentityMatch
              || resolveAccountRef(fs, aiHomeDir, accountRef, { bestEffort: true });
            const refreshExisting = Boolean(
              existing
              && !dryRun
              && safeProvider === 'kimi'
              && nativeHome
              && shouldRefreshExistingKimiAccount(existing.accountRef, projection)
            );
            if (existing && !refreshExisting) {
              ...
              const before = accountArtifactHooks
                && typeof accountArtifactHooks.snapshotAccountAuthArtifacts === 'function'
                ? accountArtifactHooks.snapshotAccountAuthArtifacts(safeProvider, accountRef)
                : null;
              const registration = registerProviderAuthProjection(fs, account.dir, safeProvider, {
                aiHomeDir,
                path,
                nativeHome,
                accountRef: refreshExisting ? existing.accountRef : '',
                projectionMetadata, expectedKiroVersions
              });
              if (!registration.registered) {
                invalid += 1;
              } else if (registration.reason === 'existing_account' && !refreshExisting) {
                duplicates += 1;
              } else {
                imported += 1;
                if (before && typeof accountArtifactHooks.notifyDefaultAccountAuthUpdatedIfChanged === 'function') {
  ...
  function resolveFixedProviderSourceDir(baseDir, provider) {
    ...
    const candidates = [
      ...
      ...(safeProvider === 'kimi' ? [path.join(resolvedBaseDir, '.kimi-code')] : []),
      resolvedBaseDir
    ];

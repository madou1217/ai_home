    ...
    for (let index = 0; index < accounts.length; index += 1) {
      ...
      try {
        ...
        if (candidateFiles.length === 0) {
          ...
          const projection = readProviderAuthProjection(fs, account.dir, safeProvider, { path });
          const identity = resolveNativeAuthIdentitySeed(safeProvider, projection);
          if (!identity.identitySeed || identity.degraded) {
            invalid += 1;
          } else {
            const accountRef = getPublicAccountRef(`unique:${identity.identitySeed}`);
            const existing = resolveAccountRef(fs, aiHomeDir, accountRef, { bestEffort: true });
            if (existing) {
              duplicates += 1;
            } else if (dryRun) {
              imported += 1;
              ...
              if (!registration.registered) {
                invalid += 1;
              } else if (registration.reason === 'existing_account') {
                duplicates += 1;
              } else {
                imported += 1;

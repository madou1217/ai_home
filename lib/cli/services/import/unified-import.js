...
const { readKimiTokenExpiry } = require('../../../account/kimi-auth');
...
function createUnifiedImportService(options = {}) {
  ...
  function shouldRefreshExistingKimiAccount(accountRef, projection) {
    ...
    const currentCredentials = current && current.credentials;
    const incomingCredentials = projection && projection.credentials;
    ...
    for (let index = 0; index < accounts.length; index += 1) {
      ...
      try {
        ...
        if (candidateFiles.length === 0) {
          ...
          const nativeHome = isKimiNativeHomeSource(account.dir, safeProvider);

...
function buildNodeBootstrapProbeOptionArgs(options = {}) {
  ...
  if (sshTargets.length === 0 && tcpTargets.length === 0) {
...
function summarizeProbe(results) {
  ...
  return {
    ...
    winrm,
    localManual,
    unreachable: results.length - reachableSsh - authRequiredSsh - localManual - winrm - sshPort
  };
}
...
function buildProbeWarnings(options, tasks) {
  const warnings = [];
  if (!tasks.length) warnings.push('pass --ssh user@host or --tcp host to probe remote bootstrap access');
...
async function runNodeBootstrapProbe(rawArgs = [], deps = {}) {
  ...
  const results = await mapWithConcurrency(tasks, options.concurrency, (task) => {
    return task.type === 'ssh'
      ? probeSshTarget(task.target, options, deps)
      : probeTcpTarget(task.target, options, deps);
  });
  return {
    ok: true,

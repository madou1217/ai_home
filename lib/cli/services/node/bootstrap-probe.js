...
function appendSshConnectionArgs(args, result, options = {}) {
  const identityFile = nonEmptyString(options.sshIdentityFile);
  if (identityFile) args.push('-i', identityFile, '-o', 'IdentitiesOnly=yes');
  if (result.port) args.push('-p', String(result.port));
  return args;
}

function buildSshRemoteRunCommand(result, options = {}) {
  if (!result.bootstrapCommand || !['linux', 'darwin', 'win32'].includes(result.bootstrapTarget)) return '';
  const args = ['ssh'];
  appendSshConnectionArgs(args, result, options);
...
function buildSshInteractiveCommand(result, options = {}) {
...
function buildSshRemoteRunExecution(result, options) {
  ...
  const sshArgs = [];
  appendSshConnectionArgs(sshArgs, result, options);
...
function buildSshBootstrapAction(result, options = {}) {
  if (result.status === 'auth-required') {
    return {
      ...
      targetCommand: buildSshInteractiveCommand(result, options),
  ...
  return attachExecutionDescriptor({
    ...
    remoteRunCommand: buildSshRemoteRunCommand(result, options),
...
function buildSshArgs(target, options, remoteCommand) {
  ...
  appendSshConnectionArgs(args, target, options);

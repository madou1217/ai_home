...
function buildBootstrapPrerequisites(target) {
  if (target === 'win32') {
    return [
      ...
      'outbound HTTPS/WebSocket access to the Server'
    ];
  }
  if (target === 'darwin') {
    return [
...
function buildBootstrapWarnings(options) {
  ...
  if (!options.inviteUrl) {
    warnings.push('invite-url is a placeholder; generate a worker invite on the Server before running join.');
  }
  if (!options.nodeId) {
  ...
  if (transportNeedsEndpoint(options.transportKind) && !options.endpoint) {
    warnings.push(`${options.transportKind} transport needs --endpoint with a Server-reachable http(s) URL.`);
  }
...
function buildTransportGuidance(kind) {
  const guidance = {
    relay: [
      ...
      'Only outbound HTTPS/WebSocket access from node to Server is required.',
    ...
    ssh: [
      ...
      'Pass --endpoint with the HTTP URL the Server can reach through that tunnel.'
    ],
    tailscale: [
    ...
    direct: [
      'Use only when the Server can reach the node server directly.',
  ...
  return [
    ...
    },
    {
      ...
      message: options.controlUrl
        ? 'Server URL is embedded without secrets.'
        : 'Provide a reachable Server URL before running the script.'
    },
    {
...
function buildNodeBootstrapPlan(input = {}, deps = {}) {
  ...
  return {
    ...
    steps: [
      ...
      {
        id: 'join-node',
        title: 'Join this machine to the Server as a worker',
        command: joinCommand
      },
      {
    ...
    security: {
      ...
      notes: [
        'The script intentionally does not include the Server Management Key.',

...
function buildRegistryAgentServiceInstallHint(controlUrl, nodeId) {
  ...
  return `aih fabric registry agent service install ${url} --node-id ${id} --management-key-file '<management-key-file>'`;
}
  ...
  try {
    ...
    return {
      ...
      ...enrichAgentServiceStatus(manager.getStatus(), {
        ...
        nodeId,
        managementKeyImportFile: '<management-key-file>'
      })
    };
  } catch (error) {
    return {
...
function buildNextSteps(report) {
  ...
  const appendRegistryAgentStep = () => {
    if (registryAgent.supported && !registryAgent.running && registryAgentInstall) {
      steps.push(`Install Fabric registry agent after importing the Server Management Key into app-state.db: ${registryAgentInstall}`);
    }
  };
  ...
  if (report.relay.recommended) {
    steps.push(`Join with relay after generating a worker invite on the Server: ${report.relay.commandHint}`);

...
function buildNodeDoctorReport(input = {}, deps = {}) {
  ...
  const cli = inspectCli({
    ...deps,
    processObj: resolveProcessContext(processObj, platform, arch)
  });
  ...
  const service = resolveServiceDiagnostics(platform, options.controlUrl, identity.nodeId, deps);
  const registryAgentService = resolveRegistryAgentServiceDiagnostics(platform, options.controlUrl, identity.nodeId, deps);
  const services = {
    relay: service,

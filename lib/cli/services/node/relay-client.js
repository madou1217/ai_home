...
async function connectRelayOnce(request, deps = {}) {
  ...
  const unsubscribeAbort = subscribeAbort(signal, () => closeRelaySocket(socket));
  // The Server registers the socket before sending relay.hello. Install
  // data handlers first so a request routed immediately after registration
  // cannot fall into the handshake-to-handler gap.
...
function waitForClose(socket, signal) {
  return new Promise((resolve) => {
    ...
    unsubscribeAbort = subscribeAbort(signal, abort);
    if (socket.readyState === 2 || socket.readyState === 3) finish();
  });
}
...
function isRetryableRelayConnectError(error) {
...
async function runRelayLoop(request, deps = {}) {
  ...
  let lastResult = null;
  const signal = deps.signal;
  const connectOnce = typeof deps.connectRelayOnce === 'function' ? deps.connectRelayOnce : connectRelayOnce;
  ...
  while ((!request.maxAttempts || attempts < request.maxAttempts) && !(signal && signal.aborted)) {
    ...
    try {
      ...
      if (!isRetryableRelayConnectError(error) || (request.maxAttempts && attempts >= request.maxAttempts)) {
        throw error;
      }
    ...
    await waitForClose(connection.socket, signal);
    stopHeartbeat();
    if (signal && signal.aborted) break;

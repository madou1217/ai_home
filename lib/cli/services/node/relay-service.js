...
function createNodeRelayServiceManager(input = {}, deps = {}) {
  ...
  const backgroundComponentId = `node-relay:${nodeId}`;
  ...
  function buildConnectComponentArgs() {
    const args = [
      'node',
      'relay',
      'connect',
  ...
  function buildConnectCommandArgs() {
    return [resolveRequiredAihCommandPath(), ...buildConnectComponentArgs()];
  }
  ...
  function createBackgroundLaunchAgent() {
    return createMacosLaunchAgent({
      ...
      launchdPlist: nonEmptyString(deps.backgroundLaunchdPlist)
        || path.join(hostHomeDir, 'Library', 'LaunchAgents', `${BACKGROUND_LAUNCHD_LABEL}.plist`),
  ...
  function installMac() {
    ...
    upsertBackgroundComponent({
      ...
      args: buildConnectComponentArgs()
    }, stateDeps);
...
`;
  ...
  function installWindows() {
    ...
    const legacyScript = path.join(path.dirname(windowsStartupScript), `${label}.cmd`);

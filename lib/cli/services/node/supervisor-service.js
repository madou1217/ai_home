...
const { readRegistryAgentManagementKey } = require('../fabric/registry-agent-management-key-store');
...
function parseNodeSupervisorUninstallArgs(args = []) {
  ...
  for (let index = 1; index < args.length;) {
    ...
    if (token === '--control-url' || token.startsWith('--control-url=')
      || token === '--management-key-file' || token.startsWith('--management-key-file=')
...
function parseNodeSupervisorInstallArgs(args = [], deps = {}) {
  ...
  for (let index = 1; index < args.length;) {
    ...
    if (token === '--management-key-file' || token.startsWith('--management-key-file=')) {
      ...
      options.managementKeyFile = path.resolve(resolveHomePath(next.value, env));
      index += next.consumed;
      continue;
    }
  ...
  if (!options.managementKeyFile && !readRegistryAgentManagementKey(options.nodeId, deps)) {
    const error = new Error('missing_node_service_management_key_file');
    error.code = 'missing_node_service_management_key_file';
    throw error;
  }
...
function buildRegistryAgentInstallArgs(options) {
  ...
  if (options.managementKeyFile) args.push('--management-key-file', options.managementKeyFile);
...
function assertCanInstallNodeSupervisorService(options, deps = {}) {
  ...
  if (!options.managementKeyFile) return;
  try {
    fsImpl.accessSync(options.managementKeyFile, fs.constants.R_OK);
  } catch (_error) {
    const error = new Error('management_key_file_unreadable');
    error.code = 'management_key_file_unreadable';
    error.file = options.managementKeyFile;
    throw error;
  }
}

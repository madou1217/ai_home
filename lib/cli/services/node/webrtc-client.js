...
async function connectWebrtcOnce(request, deps = {}) {
  ...
  await waitForChannelOpen(channel, request.connectTimeoutMs);
  if (request.once) {
    channel.close();
...
function waitForWebrtcReconnectWindow(connection = {}, request = {}) {
  const channel = connection.channel;
  const peerConnection = connection.peerConnection;
...
async function runWebrtcLoop(request, deps = {}) {
  ...
  while (!request.maxAttempts || attempts < request.maxAttempts) {
    ...
    await waitForWebrtcReconnectWindow(connection, request);

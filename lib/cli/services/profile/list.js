...
`);
  ...
  function readLatestIndexedRow(tool, accountId, fallbackRow) {
    ...
    return index.getAccountState(tool, accountId) || fallbackRow;
  }

  function readCodexMobileAccountId() {
    const env = (processObj && processObj.env) || {};
    const envId = String(env.AIH_CODEX_DESKTOP_ACCOUNT_ID || '').trim();
    if (/^\d+$/.test(envId)) return envId;
    const aiHomeDir = path.dirname(profilesDir);
    const statePath = path.join(aiHomeDir, 'codex-desktop-hook-state.json');
    try {
      if (!fs.existsSync(statePath)) return '';
      const parsed = JSON.parse(fs.readFileSync(statePath, 'utf8'));
      const stateId = String(parsed && parsed.desktopAccountId || '').trim();
      return /^\d+$/.test(stateId) ? stateId : '';
    } catch (_error) {
      return '';
    }
  }
  ...
  function listProfiles(filterCliName = null, filterAccountId = null) {
    ...
    tools.forEach((tool) => {
      ...
      if (ids.length === 0) {
        ...
        const mobileId = tool === 'codex' ? readCodexMobileAccountId() : '';
        ...
        while (cursor < ids.length) {
          ...
          batch.forEach((id) => {
            ...
            let quotaStatus = '';
            const row = indexedMap.get(id) || null;
            let operationalStatus = resolveEffectiveAccountStatus(
              row && row.status,
              readAccountStatusFile(fs, pDir)
            );

            const refreshed = refreshIndexedStateForAccount(tool, id, { refreshSnapshot: false });
            if (refreshed) {
              operationalStatus = resolveEffectiveAccountStatus(
                refreshed.status,
                readAccountStatusFile(fs, pDir)
              );
            ...
            const latestRow = readLatestIndexedRow(tool, id, row);
            ...
            if (apiKeyMode) {
              ...
              let baseUrl = '';
              try {
                const envPath = path.join(pDir, '.aih_env.json');
                if (fs.existsSync(envPath)) {
                  const envData = JSON.parse(fs.readFileSync(envPath, 'utf8'));
                  baseUrl = envData.OPENAI_BASE_URL || '';
                }
              } catch (_error) {}
            ...
            const quotaBadge = runtimeBlocked
              ...
              : '';
            const roleBadges = [
              id === defaultId ? '\x1b[32m[★ Default]\x1b[0m' : '',
              tool === 'codex' && id === mobileId ? '\x1b[35m[📱 Mobile]\x1b[0m' : ''
            ].filter(Boolean).join(' ');

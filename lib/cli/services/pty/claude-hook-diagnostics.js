...
const { appendBoundedJsonLine } = require('../../../server/bounded-log-writer');
...
function normalizeDiagnosticRelay(relay) {
  ...
  const gateway = relay.gateway === true;
...
function buildDiagnosticIdentity(input = {}) {
  ...
  const accountRef = normalizeString(input.accountRef);
  const gateway = input.gateway === true;
...
function appendClaudeToolDiagnosticLog(options = {}) {
  ...
  const logPath = pathImpl.join(aiHomeDir, 'logs', 'claude', 'hook-diagnostics.jsonl');
  const entry = buildClaudeToolDiagnosticEntry(options);
  const ok = appendBoundedJsonLine(fsImpl, logPath, entry, { path: pathImpl });
  return { ok, logPath, entry };
}

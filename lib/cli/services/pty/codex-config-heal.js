...
const { healCodexProviderConfig } = require('./codex-provider-config-heal');
...
// healCodexConfigFile 组合入口：一次读取，provider / MCP / projects / hooks.state 自愈后
...
function healCodexConfigFile(configPath, options = {}) {
  ...
  try {
    ...
    const provider = healCodexProviderConfig(original, options);
    const mcp = healCodexMcpServers(provider.config, { platform, fs });
    ...
    if (!provider.changed && !mcp.converted.length && !mcp.removed.length
    ...
    const stamp = new Date().toISOString().replace(/[:.]/g, '-');
    const backupPath = exists ? `${configPath}.aih-bak-${stamp}` : '';
    if (backupPath) fs.writeFileSync(backupPath, original, { encoding: 'utf8', mode: 0o600 });
    if (!exists) fs.mkdirSync(require('node:path').dirname(configPath), { recursive: true });
    fs.writeFileSync(configPath, hooksState.config, { encoding: 'utf8', mode: 0o600 });
    result.changed = true;
    result.provider = { changed: provider.changed };
    ...
    result.backupPath = backupPath;
    if (provider.changed && log) log('[codex-heal] repaired managed provider aih_server configuration');

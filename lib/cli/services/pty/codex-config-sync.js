'use strict';

const { normalizeCodexHooksFeatureFlag } = require('../../config/codex-feature-flags');
const { buildServerBaseUrl } = require('../../../server/server-defaults');
const { normalizeWindowsPathForCodexConfig } = require('../../../runtime/windows-path-encoding');

const AIH_CODEX_PROVIDER_KEY = 'aih_server';
const AIH_CODEX_PROVIDER_NAME = 'AIH Server';
const AIH_CODEX_PROVIDER_BASE_URL = buildServerBaseUrl();
const AIH_CODEX_PROVIDER_WIRE_API = 'responses';
const AIH_CODEX_PROVIDER_ENV_KEY = 'OPENAI_API_KEY';
const CODEX_STARTUP_WARNING_KEY = 'suppress_unstable_features_warning';
const CODEX_STARTUP_UPDATE_CHECK_KEY = 'check_for_update_on_startup';
const NATIVE_API_BASE_URL_MARKER = '# AI Home managed API endpoint for native openai sessions';

function getAihProviderKey() {
  return AIH_CODEX_PROVIDER_KEY;
}

function getManagedAihProviderKeys() {
  return new Set([AIH_CODEX_PROVIDER_KEY]);
}

function getManagedAihProviderHeaders() {
  return new Set(
    Array.from(getManagedAihProviderKeys()).map((key) => `[model_providers.${key}]`)
  );
}

function extractModelProviderName(modelProviderLine) {
  const match = String(modelProviderLine || '').trim().match(/^model_provider\s*=\s*"([^"]+)"\s*$/);
  return match ? String(match[1] || '').trim() : '';
}

function isAihManagedProviderKey(providerKey) {
  const normalized = String(providerKey || '').trim();
  return normalized === AIH_CODEX_PROVIDER_KEY;
}

function extractModelProviderKeyFromHeader(header) {
  const match = String(header || '').trim().match(/^\[model_providers\.([^\]]+)\]\s*$/);
  return match ? String(match[1] || '').trim() : '';
}

function isAihManagedScaffoldComment(line) {
  const trimmed = String(line || '').trim();
  return trimmed === '# This file is managed by ai-home (aih)'
    || trimmed === '# Synced from host config (excluding sensitive fields)'
    || trimmed === '# AI Home managed permissions: Full Access'
    || trimmed === '# API endpoint configuration (migrated from OPENAI_BASE_URL env var)'
    || /^# Codex configuration for account\b/.test(trimmed);
}

function normalizeCodexConfigSyncOptions(options = {}) {
  const normalized = {
    ...options
  };
  const isApiKeyMode = Boolean(options.isApiKeyMode);
  const openaiBaseUrl = String(options.openaiBaseUrl || '').trim();
  normalized.isApiKeyMode = isApiKeyMode;
  normalized.openaiApiKey = String(options.openaiApiKey || '').trim();
  normalized.openaiBaseUrl = openaiBaseUrl;
  normalized.sqliteHome = String(options.sqliteHome || '').trim();
  normalized.codexVersion = String(options.codexVersion || '').trim();
  normalized.forceAihProvider = Boolean(options.forceAihProvider);
  normalized.httpHeaders = options.httpHeaders && typeof options.httpHeaders === 'object'
    ? Object.fromEntries(
      Object.entries(options.httpHeaders)
        .map(([key, value]) => [String(key || '').trim(), String(value || '').trim()])
        .filter(([key, value]) => key && value)
    )
    : {};
  normalized.providerKeyOverride = isAihManagedProviderKey(options.providerKeyOverride)
    ? String(options.providerKeyOverride || '').trim()
    : '';
  return normalized;
}

function getEffectiveAihProviderKey(options = {}) {
  const normalized = normalizeCodexConfigSyncOptions(options);
  return normalized.providerKeyOverride || getAihProviderKey();
}

function escapeTomlBasicString(value) {
  return String(value || '').replace(/\\/g, '\\\\').replace(/"/g, '\\"');
}

function tomlLiteral(value) {
  return `'${String(value || '').replace(/'/g, "''")}'`;
}

// codex 0.149 实测：model_providers 的 auth 命令表与 env_key 互斥（并存直接
// 拒绝加载配置）。宿主受管块用 auth 命令表形态（scripts/aih-codex-provider-auth.js
// --gateway 固定读取指定 AIH 配置中的网关 key），裸跑 codex 因此闭环；
// 不传 authCommand 的调用方（沙箱同步）维持 env_key 形态，由 aih 启动链注入 env。
function getManagedAihProviderBlock(options = {}) {
  const normalized = normalizeCodexConfigSyncOptions(options);
  const providerKey = getEffectiveAihProviderKey(options);
  const authCommand = normalized.authCommand && normalized.authCommand.command
    ? normalized.authCommand
    : null;
  const authLine = authCommand
    ? (normalized.openaiApiKey ? '' : 'bearer_token = "dummy"')
    : (normalized.openaiApiKey
      ? `env_key = "${AIH_CODEX_PROVIDER_ENV_KEY}"`
      : 'bearer_token = "dummy"');
  const headerEntries = Object.entries(normalized.httpHeaders);
  const httpHeadersLine = headerEntries.length > 0
    ? `http_headers = { ${headerEntries.map(([key, value]) => `"${escapeTomlBasicString(key)}" = "${escapeTomlBasicString(value)}"`).join(', ')} }`
    : '';
  const mainBlock = [
    `[model_providers.${providerKey}]`,
    `name = "${AIH_CODEX_PROVIDER_NAME}"`,
    `base_url = "${normalized.openaiBaseUrl || AIH_CODEX_PROVIDER_BASE_URL}"`,
    authLine,
    httpHeadersLine,
    ...(options.supportsWebsockets === true ? ['supports_websockets = true'] : []),
    `wire_api = "${AIH_CODEX_PROVIDER_WIRE_API}"`
  ].filter(Boolean).join('\n');
  if (!authCommand || !normalized.openaiApiKey) return mainBlock;
  return [
    mainBlock,
    '',
    `[model_providers.${providerKey}.auth]`,
    `command = ${tomlLiteral(authCommand.command)}`,
    `args = [${(authCommand.args || [authCommand.scriptPath]).filter(Boolean).map(tomlLiteral).join(', ')}]`,
    'refresh_interval_ms = 300000',
    'timeout_ms = 10000'
  ].join('\n');
}

// applyManagedAihProviderSection 替换受管 provider 段：先剥离可能残留的
// auth 子表段（受管块自身可能带新子表，避免新旧并存重复定义——2026-08-22
// 实证：header 拼接错误曾导致每次 set-default 追加一个 auth 子表，codex 以
// duplicate key 拒绝加载整份配置），再整体替换主段。
function applyManagedAihProviderSection(configText, providerHeader, block) {
  const authHeader = `${String(providerHeader || '').replace(/\]$/, '')}.auth]`;
  return replaceTomlSection(stripNamedSections(configText, [authHeader]), providerHeader, block);
}

function isModelProviderHeader(line) {
  return /^\[model_providers\.[^\]]+\]\s*$/.test(String(line || '').trim());
}

function isProjectHeader(line) {
  return /^\[projects\.[^\]]+\]\s*$/.test(String(line || '').trim());
}

function extractSectionHeader(block) {
  const lines = String(block || '').split('\n');
  for (const line of lines) {
    const trimmed = line.trim();
    if (trimmed) return trimmed;
  }
  return '';
}

function escapeRegExp(value) {
  return String(value || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

function replaceTomlSection(content, header, block) {
  const source = String(content || '');
  const blockRegex = new RegExp(`(?:^|\\n)${escapeRegExp(header)}[\\s\\S]*?(?=\\n\\[|(?![\\s\\S]))`);
  if (blockRegex.test(source)) {
    return source.replace(blockRegex, (match) => {
      const prefix = match.startsWith('\n') ? '\n' : '';
      return `${prefix}${block}`;
    });
  }
  return `${source.trimEnd()}${source.trim() ? '\n\n' : ''}${block}\n`;
}

function upsertRootKeyLine(configText, key, line) {
  const lines = String(configText || '').split('\n');
  const keyPattern = new RegExp(`^${escapeRegExp(key)}\\s*=`);
  let firstSectionIndex = lines.findIndex((candidate) => candidate.trim().startsWith('['));
  if (firstSectionIndex === -1) firstSectionIndex = lines.length;

  let replaced = false;
  const nextLines = [];
  lines.forEach((candidate, index) => {
    const trimmed = candidate.trim();
    if (index < firstSectionIndex && keyPattern.test(trimmed)) {
      if (!replaced) {
        nextLines.push(line);
        replaced = true;
      }
      return;
    }
    nextLines.push(candidate);
  });

  if (!replaced) {
    const insertAt = firstSectionIndex;
    nextLines.splice(insertAt, 0, line);
  }

  return nextLines.join('\n');
}

function syncCodexBuiltinApiBaseUrl(configText, baseUrl = '') {
  // Native App resume can explicitly select `openai` from an old thread.
  // Its API key and endpoint must still belong to the same selected account,
  // even when the App bundle cannot be hooked after an update.
  const lines = String(configText || '').split('\n');
  const endpoint = String(baseUrl || '').trim();
  const cleaned = [];
  let root = true;
  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index];
    if (line.trim().startsWith('[')) root = false;
    if (root && line.trim() === NATIVE_API_BASE_URL_MARKER) {
      if (!endpoint && /^\s*openai_base_url\s*=/.test(lines[index + 1] || '')) index += 1;
      continue;
    }
    cleaned.push(line);
  }
  const content = cleaned.join('\n');
  return endpoint
    ? upsertRootKeyLine(content, 'openai_base_url', `${NATIVE_API_BASE_URL_MARKER}\nopenai_base_url = "${escapeTomlBasicString(endpoint)}"`)
    : content;
}

function enableCodexStartupWarningSuppression(configText) {
  const lines = String(configText || '').split('\n');
  const keyPattern = new RegExp(`^${escapeRegExp(CODEX_STARTUP_WARNING_KEY)}\\s*=`);
  let inFeatures = false;
  const withoutMisplacedKey = lines.filter((line) => {
    const trimmed = line.trim();
    if (trimmed.startsWith('[')) {
      inFeatures = trimmed === '[features]';
      return true;
    }
    return !(inFeatures && keyPattern.test(trimmed));
  }).join('\n');
  return upsertRootKeyLine(
    withoutMisplacedKey,
    CODEX_STARTUP_WARNING_KEY,
    `${CODEX_STARTUP_WARNING_KEY} = true`
  );
}

function disableCodexStartupUpdateCheck(configText) {
  return upsertRootKeyLine(
    configText,
    CODEX_STARTUP_UPDATE_CHECK_KEY,
    `${CODEX_STARTUP_UPDATE_CHECK_KEY} = false`
  );
}

function applyCodexStartupPolicy(configText) {
  return disableCodexStartupUpdateCheck(
    enableCodexStartupWarningSuppression(configText)
  );
}

function hasOwnOption(options, key) {
  return Object.prototype.hasOwnProperty.call(options || {}, key);
}

function buildPreferredAuthMethodLine(accountOnlyConfig, options = {}) {
  const normalized = normalizeCodexConfigSyncOptions(options);
  if (normalized.forceAihProvider || normalized.isApiKeyMode) {
    return 'preferred_auth_method = "apikey"';
  }
  if (hasOwnOption(options, 'isApiKeyMode') && !normalized.isApiKeyMode) {
    return 'preferred_auth_method = "oauth"';
  }
  return accountOnlyConfig && accountOnlyConfig.preferred_auth_method
    ? String(accountOnlyConfig.preferred_auth_method).trim()
    : '';
}

function buildModelProviderLine(accountOnlyConfig, options = {}) {
  const normalized = normalizeCodexConfigSyncOptions(options);
  if (normalized.forceAihProvider || normalized.isApiKeyMode) {
    return `model_provider = "${getEffectiveAihProviderKey(options)}"`;
  }
  if (hasOwnOption(options, 'isApiKeyMode') && !normalized.isApiKeyMode) {
    return 'model_provider = "openai"';
  }
  return accountOnlyConfig && accountOnlyConfig.model_provider
    ? String(accountOnlyConfig.model_provider).trim()
    : '';
}

function extractAccountOnlyConfig(configText) {
  const result = {
    preferred_auth_method: null,
    model_provider: null,
    providers: [],
    model_providers: []
  };

  const lines = String(configText || '').split('\n');
  let activeType = null;
  let activeLines = [];

  const flushActive = () => {
    if (!activeType || activeLines.length === 0) {
      activeType = null;
      activeLines = [];
      return;
    }
    if (activeType === 'providers') {
      result.providers.push(activeLines.join('\n'));
    } else if (activeType === 'model_providers') {
      result.model_providers.push(activeLines.join('\n'));
    }
    activeType = null;
    activeLines = [];
  };

  for (const rawLine of lines) {
    const trimmed = rawLine.trim();

    if (activeType) {
      const startsNewProviders = trimmed === '[[providers]]';
      const startsNewModelProvider = isModelProviderHeader(trimmed);
      const startsOtherSection = trimmed.startsWith('[') && !startsNewProviders && !startsNewModelProvider;

      if (startsNewProviders || startsNewModelProvider || startsOtherSection) {
        flushActive();
      }
    }

    if (!activeType && trimmed.startsWith('preferred_auth_method')) {
      result.preferred_auth_method = trimmed;
      continue;
    }

    if (!activeType && trimmed.startsWith('model_provider')) {
      result.model_provider = trimmed;
      continue;
    }

    if (trimmed === '[[providers]]') {
      activeType = 'providers';
      activeLines = [rawLine];
      continue;
    }

    if (isModelProviderHeader(trimmed)) {
      activeType = 'model_providers';
      activeLines = [rawLine];
      continue;
    }

    if (activeType) {
      activeLines.push(rawLine);
    }
  }

  flushActive();
  return result;
}

function filterHostConfig(configText, options = {}) {
  const lines = String(configText || '').split('\n');
  const filtered = [];
  let skipUntilNextSection = false;
  let inModelProviders = false;

  for (const line of lines) {
    const trimmed = line.trim();

    if (skipUntilNextSection) {
      if (trimmed.startsWith('[')) {
        skipUntilNextSection = false;
      } else {
        continue;
      }
    }

    if (options.excludeAccountOnly) {
      if (trimmed.startsWith('preferred_auth_method') || trimmed.startsWith('model_provider')) {
        continue;
      }
      if (trimmed === '[[providers]]') {
        skipUntilNextSection = true;
        continue;
      }
    }

    if (options.excludeSensitive) {
      if (isModelProviderHeader(trimmed)) {
        inModelProviders = true;
        filtered.push(line);
        continue;
      }

      if (inModelProviders && trimmed.startsWith('[') && !isModelProviderHeader(trimmed)) {
        inModelProviders = false;
      }

      if (inModelProviders) {
        if (
          trimmed.startsWith('bearer_token')
          || trimmed.startsWith('api_key')
          || trimmed.includes('_token =')
          || trimmed.includes('_key =')
        ) {
          continue;
        }
      }
    }

    if (trimmed.startsWith('sandbox_mode')) {
      continue;
    }
    if (isAihManagedScaffoldComment(trimmed)) {
      continue;
    }

    filtered.push(line);
  }

  return filtered.join('\n');
}

function stripNamedSections(configText, headers) {
  const headerSet = headers instanceof Set ? headers : new Set(headers || []);
  if (headerSet.size === 0) return String(configText || '');

  const lines = String(configText || '').split('\n');
  const nextLines = [];
  let skipping = false;

  for (const line of lines) {
    const trimmed = line.trim();
    if (trimmed.startsWith('[')) {
      skipping = headerSet.has(trimmed);
      if (skipping) continue;
    }
    if (!skipping) {
      nextLines.push(line);
    }
  }

  return nextLines.join('\n');
}

function removeManagedAihProviderSection(configText, providerKey = AIH_CODEX_PROVIDER_KEY) {
  const providerHeader = `[model_providers.${providerKey}]`;
  const authHeader = `[model_providers.${providerKey}.auth]`;
  return stripNamedSections(configText, [providerHeader, authHeader]);
}

// 旧版 aih 曾按账号写 [model_providers.aih_<n>]（并可能内嵌明文
// bearer_token——该字段在现行 codex 已不是合法配置，等于把 token 明文
// 躺在 config 里）。现行唯一受管 key 是 aih_server；清理所有遗留命名，
// 否则 `set-default` 永远只替换 aih_server，旧段永远残留（2026-08-22
// 用户审计发现的 aih_2 + yesboss 明文 token）。
const LEGACY_AIH_PROVIDER_KEY_PATTERN = /^aih(?:[-_]\d+)?$/;

function removeAihLegacyProviderSections(configText) {
  const legacyHeaders = extractSections(String(configText || ''), (header) => {
    const match = header.match(/^\[model_providers\.([^\]\s.]+)\]$/);
    return Boolean(match && LEGACY_AIH_PROVIDER_KEY_PATTERN.test(match[1]));
  }).map(extractSectionHeader);
  if (!legacyHeaders.length) return String(configText || '');
  return stripNamedSections(configText, legacyHeaders);
}

// 旧版 aih 迁移逻辑写过 `# API endpoint configuration (migrated from
// OPENAI_BASE_URL env var)` + 根级 openai_base_url。现行 codex 文档：
// openai_base_url 只覆盖内建 openai provider，在 model_provider=aih_server
// 下被忽略；且旧值常指向已停用的 relay。仅清理紧邻受管脚手架注释的键
// （该注释即 aih 派发标记），用户自设的 openai_base_url 不动。
const MIGRATED_BASE_URL_MARKER = '# API endpoint configuration (migrated from OPENAI_BASE_URL env var)';

function removeMigratedOpenaiBaseUrlScaffold(configText) {
  const lines = String(configText || '').split('\n');
  const output = [];
  let skipNextOpenaiBaseUrl = false;
  let changed = false;
  for (const line of lines) {
    const trimmed = line.trim();
    if (trimmed === MIGRATED_BASE_URL_MARKER) {
      skipNextOpenaiBaseUrl = true;
      changed = true;
      continue;
    }
    if (skipNextOpenaiBaseUrl && /^openai_base_url\s*=/.test(trimmed)) {
      skipNextOpenaiBaseUrl = false;
      changed = true;
      continue;
    }
    skipNextOpenaiBaseUrl = false;
    output.push(line);
  }
  return changed ? output.join('\n') : String(configText || '');
}

// removeAihManagedCruft 统一清理 aih 自己的历史遗留（遗留 provider 段 +
// 迁移脚手架），供宿主同步（set-default）与沙箱同步共用。
function removeAihManagedCruft(configText) {
  return removeMigratedOpenaiBaseUrlScaffold(removeAihLegacyProviderSections(configText));
}

function extractSections(configText, predicate) {
  const sections = [];
  const lines = String(configText || '').split('\n');
  let active = null;

  const flush = () => {
    if (!active) return;
    sections.push(active.lines.join('\n').trimEnd());
    active = null;
  };

  for (const line of lines) {
    const trimmed = line.trim();
    if (trimmed.startsWith('[')) {
      flush();
      if (predicate(trimmed)) {
        active = { header: trimmed, lines: [line] };
      }
      continue;
    }
    if (active) active.lines.push(line);
  }

  flush();
  return sections.filter((section) => String(section || '').trim());
}

function mergeSharedProjectSections(baseConfigText, sharedConfigText) {
  const baseText = String(baseConfigText || '');
  const baseHeaders = new Set(extractSections(baseText, isProjectHeader).map(extractSectionHeader));
  const missingSharedProjects = extractSections(sharedConfigText, isProjectHeader)
    .filter((section) => !baseHeaders.has(extractSectionHeader(section)));
  if (missingSharedProjects.length === 0) return baseText;
  return [
    baseText.trimEnd(),
    ...missingSharedProjects
  ].filter((block) => String(block || '').trim()).join('\n\n') + '\n';
}

function stripRootKeys(configText, keys) {
  const keySet = keys instanceof Set ? keys : new Set(keys || []);
  if (keySet.size === 0) return String(configText || '');

  const lines = String(configText || '').split('\n');
  const nextLines = [];
  let inSection = false;

  for (const line of lines) {
    const trimmed = line.trim();
    if (trimmed.startsWith('[')) {
      inSection = true;
      nextLines.push(line);
      continue;
    }
    if (!inSection) {
      const match = trimmed.match(/^([A-Za-z0-9_-]+)\s*=/);
      if (match && keySet.has(match[1])) continue;
    }
    nextLines.push(line);
  }

  return nextLines.join('\n');
}

function hoistModelProviderSections(configText) {
  const lines = String(configText || '').split('\n');
  const rootLines = [];
  const providerSections = [];
  const otherSections = [];
  let activeSection = null;

  const flushSection = () => {
    if (!activeSection) return;
    const target = isModelProviderHeader(activeSection.header)
      ? providerSections
      : otherSections;
    target.push(activeSection.lines.join('\n').trimEnd());
    activeSection = null;
  };

  for (const line of lines) {
    const trimmed = line.trim();
    if (trimmed.startsWith('[')) {
      flushSection();
      activeSection = {
        header: trimmed,
        lines: [line]
      };
      continue;
    }

    if (activeSection) {
      activeSection.lines.push(line);
    } else {
      rootLines.push(line);
    }
  }

  flushSection();

  return [
    rootLines.join('\n').trimEnd(),
    ...providerSections.filter(Boolean),
    ...otherSections.filter(Boolean)
  ]
    .filter((block) => String(block || '').trim())
    .join('\n\n') + '\n';
}

function scopeAccountOnlyConfig(accountOnlyConfig, options = {}) {
  const normalized = normalizeCodexConfigSyncOptions(options);
  const providerKey = getEffectiveAihProviderKey(options);
  const accountModelProvider = extractModelProviderName(accountOnlyConfig && accountOnlyConfig.model_provider);
  const shouldUseAihProvider = Boolean(
    normalized.forceAihProvider
    || normalized.openaiBaseUrl
    || isAihManagedProviderKey(accountModelProvider)
  );
  const scoped = {
    preferred_auth_method: accountOnlyConfig && accountOnlyConfig.preferred_auth_method
      ? accountOnlyConfig.preferred_auth_method
      : null,
    model_provider: accountOnlyConfig && accountOnlyConfig.model_provider
      ? accountOnlyConfig.model_provider
      : null,
    providers: Array.isArray(accountOnlyConfig && accountOnlyConfig.providers)
      ? accountOnlyConfig.providers.slice()
      : [],
    model_providers: []
  };

  if (shouldUseAihProvider) {
    scoped.preferred_auth_method = 'preferred_auth_method = "apikey"';
    scoped.model_provider = `model_provider = "${providerKey}"`;
  }

  const activeModelProvider = extractModelProviderName(scoped.model_provider);
  const modelProviders = Array.isArray(accountOnlyConfig && accountOnlyConfig.model_providers)
    ? accountOnlyConfig.model_providers
    : [];
  const selectedProviders = new Map();
  modelProviders.forEach((block) => {
    const key = extractModelProviderKeyFromHeader(extractSectionHeader(block));
    if (!key) return;
    const selected = key === providerKey
      || (key === activeModelProvider && !isAihManagedProviderKey(key));
    if (selected) selectedProviders.set(key, block);
  });
  scoped.model_providers = Array.from(selectedProviders.values());

  return scoped;
}

function mergeConfigs(hostConfigText, accountOnlyConfig, options = {}) {
  const normalized = normalizeCodexConfigSyncOptions(options);
  const providerKey = getEffectiveAihProviderKey(options);
  let mergedConfig = applyCodexStartupPolicy(
    removeAihManagedCruft(String(hostConfigText || '').trimEnd())
  );

  const preferredAuthMethodLine = buildPreferredAuthMethodLine(accountOnlyConfig, options);
  if (preferredAuthMethodLine) {
    mergedConfig = upsertRootKeyLine(mergedConfig, 'preferred_auth_method', preferredAuthMethodLine);
  }

  const modelProviderLine = buildModelProviderLine(accountOnlyConfig, options);
  if (modelProviderLine) {
    mergedConfig = upsertRootKeyLine(mergedConfig, 'model_provider', modelProviderLine);
  }

  if (normalized.sqliteHome) {
    const sqliteHome = normalizeWindowsPathForCodexConfig(normalized.sqliteHome);
    mergedConfig = upsertRootKeyLine(
      mergedConfig,
      'sqlite_home',
      `sqlite_home = "${escapeTomlBasicString(sqliteHome)}"`
    );
  }

  if (normalized.forceAihProvider || normalized.isApiKeyMode) {
    const providerBlock = getManagedAihProviderBlock({
      openaiBaseUrl: normalized.openaiBaseUrl,
      openaiApiKey: normalized.openaiApiKey,
      providerKeyOverride: normalized.providerKeyOverride,
      httpHeaders: normalized.httpHeaders,
      supportsWebsockets: normalized.supportsWebsockets
    });
    mergedConfig = syncCodexBuiltinApiBaseUrl(mergedConfig, normalized.openaiBaseUrl || AIH_CODEX_PROVIDER_BASE_URL);
    mergedConfig = applyManagedAihProviderSection(mergedConfig, `[model_providers.${providerKey}]`, providerBlock);
  } else if (hasOwnOption(options, 'isApiKeyMode') && !normalized.isApiKeyMode) {
    // OAuth 使用 Codex 内建 openai provider；清掉宿主模板里上一轮 API-key
    // 会话留下的受管 provider/auth 段，避免沙箱继续调用 AIH auth 命令。
    mergedConfig = removeManagedAihProviderSection(mergedConfig, providerKey);
    mergedConfig = syncCodexBuiltinApiBaseUrl(mergedConfig);
  }

  return hoistModelProviderSections(
    normalizeCodexHooksFeatureFlag(`${mergedConfig.trimEnd()}\n`, {
      codexVersion: normalized.codexVersion
    }).content
  );
}

module.exports = {
  AIH_CODEX_PROVIDER_KEY,
  AIH_CODEX_PROVIDER_NAME,
  AIH_CODEX_PROVIDER_BASE_URL,
  AIH_CODEX_PROVIDER_WIRE_API,
  AIH_CODEX_PROVIDER_ENV_KEY,
  CODEX_STARTUP_WARNING_KEY,
  CODEX_STARTUP_UPDATE_CHECK_KEY,
  getAihProviderKey,
  getManagedAihProviderKeys,
  getManagedAihProviderHeaders,
  normalizeCodexConfigSyncOptions,
  escapeTomlBasicString,
  enableCodexStartupWarningSuppression,
  disableCodexStartupUpdateCheck,
  applyCodexStartupPolicy,
  getManagedAihProviderBlock,
  applyManagedAihProviderSection,
  hoistModelProviderSections,
  extractModelProviderName,
  extractModelProviderKeyFromHeader,
  isAihManagedProviderKey,
  scopeAccountOnlyConfig,
  extractAccountOnlyConfig,
  filterHostConfig,
  mergeSharedProjectSections,
  mergeConfigs,
  syncCodexBuiltinApiBaseUrl,
  removeAihManagedCruft,
  removeManagedAihProviderSection,
  removeAihLegacyProviderSections,
  removeMigratedOpenaiBaseUrlScaffold
};

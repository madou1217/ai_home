...
function resolveCodexOwnerRuntimeScope(entry) {
  if (!entry || entry.provider !== 'codex' || !persistentSession.isSafeSessionName(entry.session)) {
    return '';
  }
  const runtimeScope = String(entry.runtimeScope || '').trim();
  try {
    return persistentSession.deriveSocket('codex', runtimeScope) === entry.socket
      ? runtimeScope
      : '';
  } catch (_error) {
    return '';
...
function createPersistentLaunchWrapper(deps = {}) {
  ...
  function maybeWrapPersistentLaunch(launch, ctx) {
    try {
      ...
      const confPath = persistentSession.resolveProviderTmuxConfPath(
        resolveAihRunPath(aiHomeDir, 'tmux', 'tmux.conf'),
        ctx.cliName
      );
      const resolvedConf = persistentSession.ensureTmuxConf(confPath, fs, {
        ...
        tmuxCommand: tmux.command,
        platform: processObj.platform,
        userConfigPath: path.join(aiHomeDir, 'config', 'tmux.conf')
      });
      ...
      const explicitCodexThreadId = ctx.cliName === 'codex'
        ? extractCodexResumeThreadId(ctx.argsToRun)
        : '';
      const persistentSocket = persistentSession.deriveSocket(ctx.cliName, ctx.runtimeScope);
      // A Codex thread owns one writer globally, not once per account socket.
      // The registry address is authoritative: attaching a foreign-socket owner
      // preserves its original account environment without launching a rival.
      const registeredCodexOwners = selectionMode === 'fresh' && explicitCodexThreadId
        ? persistentSessionRegistry.listEntries(aiHomeDir, { fs })
          .filter((entry) => (
            resolveCodexOwnerRuntimeScope(entry)
            && resolveCodexPersistentThreadId(entry) === explicitCodexThreadId
          ))
        : [];
      const registeredCodexOwner = selectPreferredCodexPersistentOwner(registeredCodexOwners);
      let persistentRuntimeScope = ctx.runtimeScope;
      ...
      try {
        ...
        let sessions = enrichNativeWindowsPsmuxSessions(
          tmux,
          ctx,
          persistentSession.parseSessionList(probe && probe.stdout)
        );
        probedSessions = sessions;
        let activeCodexOwner = null;
        if (registeredCodexOwner) {
          const sessionsBySocket = new Map([[persistentSocket, sessions]]);
          const ownerScopesBySocket = new Map();
          for (const entry of registeredCodexOwners) {
            ownerScopesBySocket.set(entry.socket, resolveCodexOwnerRuntimeScope(entry));
          }
          for (const [socket, runtimeScope] of ownerScopesBySocket) {
            if (socket === persistentSocket) continue;
            const ownerListCmd = persistentSession.buildListSessionsCommand({
              cliName: ctx.cliName,
              runtimeScope,
              tmuxCommand: tmux.command
            });
            const ownerProbe = spawnSync(ownerListCmd.command, ownerListCmd.args, {
              encoding: 'utf8',
              env: ctx.envOverrides
            });
            if (!isTrustedSessionProbe(ownerProbe)) {
              throw new Error('persistent owner session probe failed');
            }
            sessionsBySocket.set(socket, enrichNativeWindowsPsmuxSessions(
              tmux,
              { ...ctx, runtimeScope },
              persistentSession.parseSessionList(ownerProbe && ownerProbe.stdout)
            ));
          }
          activeCodexOwner = selectPreferredCodexPersistentOwner(
            registeredCodexOwners.filter((entry) => (
              (sessionsBySocket.get(entry.socket) || []).some((session) => session.name === entry.session)
            ))
          );
          if (activeCodexOwner) {
            persistentRuntimeScope = resolveCodexOwnerRuntimeScope(activeCodexOwner);
            sessions = sessionsBySocket.get(activeCodexOwner.socket) || [];
            probedSessions = sessions;
            requestedSession = activeCodexOwner.session;
          }
        }
        ...
        const intent = activeCodexOwner
          ? 'select'
          : selectionMode === 'fresh' || selectionMode === 'restore'
          ? 'create'
        ...
        if (selectionMode === 'exact') {
          ...
          plannedAction = plan.action;
          announceSessionPlan(ctx.cliName, ctx.cliAccountId, plan, { named: !!label });
        } else if (registeredCodexOwner) {
          persistentRuntimeScope = resolveCodexOwnerRuntimeScope(registeredCodexOwner);
          const plan = { session: registeredCodexOwner.session, action: 'takeover' };
          plannedSession = plan.session;
          plannedAction = plan.action;
          announceSessionPlan(ctx.cliName, ctx.cliAccountId, plan, { named: false });
        } else {
      ...
      if (persistentRuntimeScope === ctx.runtimeScope) {
      ...
      const wrapped = persistentSession.buildTmuxLaunch(persistentInnerLaunch, {
        cliName: ctx.cliName,
        runtimeScope: persistentRuntimeScope,
        cwd,
        label,
        sessionName: plannedSession,
        share: shareTarget,
      ...
      try {
        ...
        const registryEntry = persistentSessionRegistry.writeEntry(aiHomeDir, {
          provider: registeredSession ? registeredSession.provider : ctx.cliName,
          runtimeScope: registeredSession ? registeredSession.runtimeScope : ctx.runtimeScope,
          gateway: registeredSession ? registeredSession.gateway : ctx.gateway === true,
          accountRef: registeredSession ? registeredSession.accountRef : ctx.accountRef,
          ...
          correlationId: String(
            registeredSession && registeredSession.correlationId
            || ctx.envOverrides.AIH_PROVIDER_SESSION_CORRELATION_ID
            || ''
          ).trim(),
          nativeSessionId: String(
            registeredSession && registeredSession.nativeSessionId
            || explicitCodexThreadId
            || ''
          ).trim(),

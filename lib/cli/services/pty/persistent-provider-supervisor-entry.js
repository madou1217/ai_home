...
function createPersistentProviderSupervisorDependencies(context, options = {}) {
  ...
  return {
    ...
    writeError: options.writeError || ((message) => {
      // process.stderr lazily constructs a TTY wrapper. After tmux revokes the
      // pane that constructor can block indefinitely, so write through the
      // already-open descriptor and treat an unavailable pane as best effort.
      try { fsImpl.writeSync(2, String(message || '')); } catch (_error) {}
    }),
    ...
    removeProjection: () => {
      ...
      return removeTransientAuthProjection(
        fsImpl,
        context.runtimeDir,
        context.provider,
        context.accountRef,

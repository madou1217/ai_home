...
module.exports = function createPtyRuntimeLaunchDomain(deps, helpers) {
  ...
  // sing-box sidecar 由本机 Server 进程单点持有。绑定账号的 CLI/headless
  // 冷启动只负责确保 Server 已拉起并等待该账号 endpoint 恢复，绝不在 CLI
  // 进程里再启动第二份 sidecar（否则跨进程内存状态会导致端口冲突）。
  function ensureAccountEgressRuntimeReady(accountRef, options = {}) {
    const normalizedRef = String(accountRef || '').trim();
    if (!normalizedRef || options.isLogin === true || options.gateway === true) {
      return { bound: false, proxyServer: '' };
    }

    const binding = readAccountEgressBinding(fs, aiHomeDir, normalizedRef);
    if (!binding) return { bound: false, proxyServer: '' };

    const resolveRuntime = () => resolveAccountEgressRuntimeProxy({
      fs,
      path,
      aiHomeDir,
    ...
    try {
      serverStatus = serverDaemon?.status?.() || null;
    } catch {}
    const shouldAutostart = String(processObj.env.AIH_SERVER_AUTOSTART || '1') !== '0';
    if (!serverStatus?.running) {
      if (!shouldAutostart || typeof serverDaemon?.start !== 'function') throw lastError;
      try {
        const startPromise = serverDaemon.start([], {
          waitForReady: false,
          readyTimeoutMs: 7000
        ...
        const startError = new Error('account_egress_server_start_failed', { cause: error });
        startError.code = 'account_egress_server_start_failed';
        throw startError;
      }
    }

    const configuredTimeout = Number(processObj.env.AIH_ACCOUNT_EGRESS_READY_TIMEOUT_MS);
    const timeoutMs = Number.isFinite(configuredTimeout) && configuredTimeout > 0
      ? Math.max(100, Math.min(configuredTimeout, 30_000))
      : 8000;

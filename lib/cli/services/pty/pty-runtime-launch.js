...
const { shouldEnableSourceAutoRestart } = require('../../../server/source-auto-restart');
...
const {
  getCliRelayProfile
} = require('../ai-cli/relay/cli-relay-profile');
...
module.exports = function createPtyRuntimeLaunchDomain(deps, helpers) {
  ...
  function ensureLocalAihServerReady(serverConfig, options = {}) {
    ...
    if (
      ...
      && status.stale
      && shouldAutostart
      && shouldEnableSourceAutoRestart(processObj)
  ...
  function resolveLaunchRuntimeScope(cliName, accountRef, isLogin, loginSessionId, gateway = false, accountEnv) {
    ...
    const storedEnv = accountEnv || (accountRef
      ...
      : {});
    // relay 判定经 CliRelayProfile 注册表分发；kimi relay 保留账号沙箱
    // （relayKeepsAccountSandbox），claude relay 回落宿主 HOME（行为不变）。
    const relayProfile = getCliRelayProfile(cliName);
    const authRelayed = Boolean(relayProfile) && relayProfile.shouldRelayAccount({
      provider: cliName,
      accountRef,
    ...
    return resolveProviderRuntimeScope(cliName, projectionDir, processObj.env, {
      ...
      gateway,
      authRelayed,
      relayKeepsAccountSandbox: Boolean(relayProfile && relayProfile.relayKeepsAccountSandbox),
      accountEnv: storedEnv
    });
  }
  ...
  function collectCliPathSearchEntries(cliName) {
    return [
      ...collectNativeCliPathEntries(cliName, { path, hostHomeDir, processObj }),
      ...collectLocalRuntimePathEntries({ path, fs, runtimeRootDir })
    ];
  }

  // 安装后重解析失败时的诊断辅助：在已知候选目录里直接找二进制文件。
  // 找到说明「已安装但不在 PATH/不可执行」，找不到说明安装器落点不在检测范围。
  function findCliBinaryInSearchEntries(cliName, entries) {
    const binaryName = getAiCliBinaryName(cliName) || cliName;
    const candidateNames = [...new Set([
      binaryName,
      ...listProviderBinaryNames(cliName, { processObj }),
      cliName
    ].filter(Boolean))];
    const suffixes = String(processObj.platform || process.platform || '').trim() === 'win32'
      ? ['.exe', '.cmd', '']
      : [''];
  ...
  function resolveCliPathWithRuntimeTools(cliName) {
    ...
    const allEntries = collectCliPathSearchEntries(cliName);

...
function createPtyRuntime(options = {}) {
  ...
  // 仅在独立 Provider 安装器明确需要 npm 兜底时，使用 aih 自带 node 的 npm。
  // 新机器系统 PATH 常无 npm，但 aih 自带 node 运行时旁边就有 npm-cli.js；-g
  // 装到内置 node 的 bin（.node-runtime/<ver>/bin），正好由上面的解析器搜索。
  ...
  function runCliPty(cliName, initialAccountRef, forwardArgs, isLogin = false, runtimeOptions = {}) {
    ...
    noticeLog(`\n\x1b[36m[aih]\x1b[0m 🚀 Running \x1b[33m${cliName}\x1b[0m (${targetLabel})${headlessRun ? ' headless' : ' via PTY'}`);
    ...
    function startBootWave() {
      ...
      // not mix it into the captured answer.
    ...
    function attachOnData(proc, spawnedScope = {}) {
      ...
      proc.onData((data) => {
        ...
        if (!hasReceivedData) {
          ...
          stopBootWave();
          // Only needed to erase the boot spinner, which headless never drew.
          if (!headlessRun) processObj.stdout.write('\r\x1b[K');
        }

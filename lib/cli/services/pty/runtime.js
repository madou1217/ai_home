...
const { buildCodexProviderArgs } = require('../ai-cli/codex-provider-args');
const {
...
function createPtyRuntime(options = {}) {
  ...
  function spawnPty(cliName, cliBin, accountRef, forwardArgs, isLogin, spawnOptions = {}) {
    ...
    if (cliName === 'codex' && usesAuthProjection && !isLogin) {
      ...
      if (typeof ensureSessionStoreLinks === 'function') {
        try {
          ensureSessionStoreLinks(cliName, selectedAccountRef);
        } catch (_error) {}
      }
      try {
    ...
    if (cliName === 'codex') {
      if (
        !isLogin
        && !argsToRun.some((arg) => String(arg || '').startsWith('-c model_provider='))
      ) {
        argsToRun.unshift(...buildCodexProviderArgs(loadedEnv, {
      ...
      var remoteProxy = allowRemoteProxy
    ...
    const finalLaunch = useHeadlessDirect ? launch : maybeWrapPersistentLaunch(launch, {
      ...
      accountRef: selectedAccountRef,
      gateway: isBuiltinServerProfile,
      runtimeScope: runtimeTarget ? runtimeTarget.runtimeScope : '',
  ...
  function runCliPty(cliName, initialAccountRef, forwardArgs, isLogin = false, runtimeOptions = {}) {
    ...
    const initialSessionSync = !isLogin && initialRef
      ...
      && typeof ensureSessionStoreLinks === 'function'
      ? ensureSessionStoreLinks(cliName, initialRef)
      : { migrated: 0, linked: 0 };
    ...
    const cleanupTransientLoginRuntime = () => {
      if (!transientLoginRuntimeDir || transientLoginRuntimeRemoved) return;
      transientLoginRuntimeRemoved = true;
      try {
        fs.rmSync(transientLoginRuntimeDir, { recursive: true, force: true });
      } catch (_error) {
        // The credential database is already authoritative; stale runtime cleanup is best effort.
      }
    };
    ...
    function switchToAccount(targetId, reasonLabel) {
      ...
      setTimeout(() => {
        ...
        if (nextRuntime.projectionRequired && typeof ensureSessionStoreLinks === 'function') {
          try {
            ensureSessionStoreLinks(cliName, activeAccountRef);
          } catch (_error) {}
        }
        outputBuffer = '';

...
function createPtyRuntime(options = {}) {
  ...
  function syncCodexConfigFromHost(accountConfigPath, hostConfigPath, accountId, deps, options = {}) {
    ...
    if (!hostConfigPath || !fs.existsSync(hostConfigPath)) {
      ...
      if (options.isApiKeyMode) {
        const mergedConfig = mergeConfigs('', accountOnlyConfig, accountId, options);
  ...
  function filterHostConfig(configText, options) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // ✅ 跳过 sandbox_mode 字段
      // 这个字段会在 mergeConfigs 中统一设置为 Full Access 模式
  ...
  function mergeConfigs(hostConfigText, accountOnlyConfig, accountId, options = {}) {
    ...
    // ✅ 强制设置权限策略: Full Access
    // 注意: danger-full-access 模式下不需要设置 approvals_reviewer
    // Codex 会自动跳过审批流程
    ...
    // 账号专属 providers 放在最后
    if (accountOnlyConfig.providers && accountOnlyConfig.providers.length > 0) {
      lines.push('');

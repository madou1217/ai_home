...
function createPtyRuntime(options = {}) {
  ...
  function resolveCodexRemoteProxyConfig(forwardArgs, isLogin) {
    ...
    let status = waitForServerStatusReady(serverDaemon, port, 120);
    const shouldAutostart = String(processObj.env.AIH_CODEX_REMOTE_AUTOSTART || '1') !== '0';
    if ((!status || !status.running) && shouldAutostart && typeof serverDaemon.start === 'function') {
      console.log('\x1b[36m[aih]\x1b[0m Codex remote proxy server not running, starting local aih server...');
      try {
        const startPromise = serverDaemon.start(buildServerStartArgs(serverConfig), {
          waitForReady: false,
          readyTimeoutMs: 7000
        });
        if (startPromise && typeof startPromise.catch === 'function') {
          startPromise.catch((error) => {
            ...
            console.warn(`\x1b[33m[aih]\x1b[0m Failed to autostart local server for Codex remote proxy: ${message}`);
          });
        }
      } catch (error) {
        const message = String((error && error.message) || error || 'unknown_error');
      ...
      status = waitForServerStatusReady(serverDaemon, port, 1500);
    }
    if (!status || !status.running || !status.ready) return null;
    const safeHost = host && host !== '0.0.0.0' ? host : '127.0.0.1';
    return {
      remoteUrl: `ws://${safeHost}:${port}`,
      authToken: String(serverConfig.apiKey || '').trim()
    };
  }
  ...
  function syncCodexConfigFromHost(accountConfigPath, hostConfigPath, accountId, deps, options = {}) {
    ...
    if (fs.existsSync(accountConfigPath)) {
      accountOnlyConfig = extractAccountOnlyConfig(fs.readFileSync(accountConfigPath, 'utf8'));
    }
    ...
    if (!hostConfigPath || !fs.existsSync(hostConfigPath)) {
      ...
      // ✅ 即使没有宿主配置,也要写入 API Key 模式的配置
      if (normalizedOptions.isApiKeyMode) {
        const mergedConfig = mergeConfigs('', accountOnlyConfig, accountId, normalizedOptions);
    ...
    // 4. 合并配置
    const mergedConfig = mergeConfigs(safeHostConfig, accountOnlyConfig, accountId, normalizedOptions);
  ...
  function runCliPty(cliName, initialId, forwardArgs, isLogin = false) {
    ...
    function writeUsageStatusLine(lineText) {
      ...
      // ✅ 防止闪烁：只有内容真正变化时才重绘
      const now = Date.now();
      if (text === lastRenderedStatusLine && (now - lastRenderedStatusAt) < 800) {
        return;
      }

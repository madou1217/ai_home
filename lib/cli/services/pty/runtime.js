...
function createPtyRuntime(options = {}) {
  ...
  function syncCodexConfigFromHost(accountConfigPath, hostConfigPath, accountId, deps, options = {}) {
    ...
    const accountBaseConfig = accountConfigText
      ? filterHostConfig(accountConfigText, {
        excludeAccountOnly: true,
        excludeSensitive: false
      })
      : '';
    const hasAccountUserConfig = String(accountBaseConfig || '')
      .split('\n')
      .some((line) => {
    ...
    if (!hostConfigPath || !fs.existsSync(hostConfigPath)) {
      ...
      if (normalizedOptions.isApiKeyMode || normalizedOptions.sqliteHome) {
        const mergedConfig = mergeConfigs(
          hasAccountUserConfig ? accountBaseConfig : '',
          accountOnlyConfig,
          accountId,
          normalizedOptions
        );
    ...
    let sharedProjectConfig = safeHostConfig;
    try {
      const profileRoot = path.dirname(path.dirname(path.dirname(accountConfigPath)));
      if (fs.existsSync(profileRoot)) {
        const siblingIds = fs.readdirSync(profileRoot)
          .filter((entry) => /^\d+$/.test(String(entry || '')));
        siblingIds.forEach((siblingId) => {
          const siblingConfigPath = path.join(profileRoot, siblingId, '.codex', 'config.toml');
          if (siblingConfigPath === accountConfigPath || !fs.existsSync(siblingConfigPath)) return;
          try {
            sharedProjectConfig = mergeSharedProjectSections(
              sharedProjectConfig,
              fs.readFileSync(siblingConfigPath, 'utf8')
            );
          } catch (_error) {}
        });
      }
    ...
    // 4. 合并配置
    const baseConfigForMerge = hasAccountUserConfig
      ? mergeSharedProjectSections(accountBaseConfig, sharedProjectConfig)
      : sharedProjectConfig;
  ...
  function runCliPty(cliName, initialId, forwardArgs, isLogin = false) {
    ...
    let activeForwardArgs = Array.isArray(forwardArgs) ? [...forwardArgs] : [];
    let ptyProc = null;
    ...
    function isCodexResumeForwardArgs(args) {
      ...
      return cliName === 'codex' && String(list[0] || '') === 'resume';
    }

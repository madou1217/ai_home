...
function createPtyRuntime(options = {}) {
  ...
  function resolveCliPathWithRuntimeTools(cliName) {
    ...
    if (preferredEntries.length > 0) {
      ...
      const preferred = resolveCliPath(cliName, { env: { ...baseEnv, PATH: augmentedPath } });
    ...
    if (!cliPath) {
      ...
      if (ans) {
        const installPlans = resolveNativeCliInstallPlans(cliName, pkg, {
          path,
          processObj,
          resolveNpmInstall: resolveBundledNpmInstall
        });
        let installed = false;
        for (const install of installPlans) {
          console.log(`\n\x1b[36m[aih]\x1b[0m Installing \x1b[33m${pkg}\x1b[0m via ${install.label}...`);

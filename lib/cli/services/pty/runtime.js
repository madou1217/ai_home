...
function createPtyRuntime(options = {}) {
  ...
  function spawnPty(cliName, cliBin, accountRef, forwardArgs, isLogin, spawnOptions = {}) {
    ...
    if (cliName === 'codex') {
      markCodexManagedLaunch(envOverrides);
    }
    ...
    if (useHeadlessDirect) {
      let spawned;
      try {
        spawned = spawnHeadlessDirect(finalLaunch, { env: envOverrides });
      } catch (error) {
        cleanupTransientAuthProjection();
        throw error;
      }
  ...
  function resolveCliPathWithRuntimeTools(cliName) {
    ...
    // qodercli) resolve through binaryName. Fallback remains the provider id.
    ...
    if (preferredEntries.length > 0) {
      ...
      const preferred = resolveCliPath(binaryName, { env: { ...baseEnv, PATH: augmentedPath } });
      if (preferred) return preferred;
      if (binaryName !== cliName) {
        const alt = resolveCliPath(cliName, { env: { ...baseEnv, PATH: augmentedPath } });
        if (alt) return alt;
      }
    }
    let resolved = resolveCliPath(binaryName);
    if (resolved) return resolved;
    if (binaryName !== cliName) {
      resolved = resolveCliPath(cliName);
      if (resolved) return resolved;
    }
    ...
    const augmentedPath = [...allEntries, ...(String(baseEnv.PATH || baseEnv.Path || '').split(sep))]
      .filter(Boolean).join(sep);
    return resolveCliPath(binaryName, { env: { ...baseEnv, PATH: augmentedPath } })
      || (binaryName !== cliName
        ? resolveCliPath(cliName, { env: { ...baseEnv, PATH: augmentedPath } })
        : '');
  }
  ...
  function runCliPty(cliName, initialAccountRef, forwardArgs, isLogin = false, runtimeOptions = {}) {
    ...
    if (!cliPath) {
      ...
      const installPrompt = cliName === 'claude' && platform === 'win32'
        ? 'Do you want to automatically install it using the official Windows installer?'
        : (cliName === 'qoder' || cliName === 'qodercn')
          ? 'Do you want to automatically install it using the official Qoder installer?'
          : 'Do you want to automatically install it via npm?';
      ...
      if (!cliPath) {
        console.error(`\x1b[31m[aih] ${binaryName} is still not in PATH after install.\x1b[0m`);
        processObj.exit(1);
      }
    }

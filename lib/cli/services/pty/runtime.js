...
function createPtyRuntime(options = {}) {
  ...
  function shouldOfferWindowsPsmuxInstall(tmux, ctx, isTTY) {
    ...
    if (processObj.platform !== 'win32') return false;
    if (isNativeWindowsPsmuxCodexDisabledByDefault(ctx)) return false;
  ...
  function isNativeWindowsPsmuxCodexExperimentEnabled() {
    return String(processObj.env.AIH_PSMUX_CODEX_EXPERIMENTAL || '') === '1';
  }

  function isNativeWindowsPsmuxCodexDisabledByDefault(ctx) {
    return processObj.platform === 'win32'
      && ctx
      && ctx.cliName === 'codex'
      && !ctx.isLogin
      && !isNativeWindowsPsmuxCodexExperimentEnabled();
  }

  function shouldBypassNativeWindowsPsmuxCodex(tmux, ctx) {
    return isNativeWindowsPsmuxCodexDisabledByDefault(ctx)
      && persistentSession.isNativeWindowsPsmuxCommand(tmux && tmux.command, processObj.platform);
  }
  ...
  function maybeWrapPersistentLaunch(launch, ctx) {
    try {
      ...
      if (shouldBypassNativeWindowsPsmuxCodex(tmux, ctx)) {
        console.log('\x1b[33m[aih]\x1b[0m Windows psmux + Codex 当前存在子 PowerShell 卡死风险，本次使用普通 PTY 启动；如需实验性启用，设置 AIH_PSMUX_CODEX_EXPERIMENTAL=1。');
        return launch;
      }
      ...
      const wrapped = persistentSession.buildTmuxLaunch(persistentInnerLaunch, {
        ...
        sessionName: plannedSession,
        share: shareTarget,
        explicitAttach: nativeWindowsPsmux && (trustedPlan || plannedAction === 'fallback-parallel'),
        attachExisting: ['reattach', 'takeover', 'mirror'].includes(plannedAction),

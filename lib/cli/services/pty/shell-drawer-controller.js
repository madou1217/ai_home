...
function createShellDrawerController(deps = {}) {
  ...
  const isHeadless = typeof isHeadlessRun === 'function' ? isHeadlessRun : () => false;
  ...
  // The drawer must never touch the terminal it has not drawn on. Teardown used
  // to clear the drawer rows unconditionally, which wiped the bottom rows of the
  // screen on exit — and for a non-interactive run (`-p`) the model's answer is
  // exactly what lives there.
  let shellDrawerEverEngaged = false;
  ...
  function destroyShellDrawer() {
    const wasEngaged = shellDrawerEverEngaged;
    shellDrawerVisible = false;
    // Resetting the scroll region is a safe defensive restore for an interactive
    // session; a headless run must leave the terminal completely untouched.
    if (!isHeadless()) applyFullTerminalViewport({ force: true });
    // Clearing rows is only ever correct for rows the drawer actually painted.
    // Doing it unconditionally erased the bottom of the screen on every exit —
    // for a `-p` run that is exactly where the model's answer sits.
    if (wasEngaged) {

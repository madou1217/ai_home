...
const LEGACY_AUTOSTART_LABELS = Object.freeze(['com.aih.server']);
...
function createServerAutostartService(deps = {}) {
  ...
  function resolveAihCommandPath() {
    ...
    if (processObj.platform === 'win32') {
      const found = firstOutputLine(run('where', ['aih'], { encoding: 'utf8' }));
  ...
  function getMacStatus() {
    const installed = Boolean(launchdPlist && fs.existsSync(launchdPlist));
...
`;
  ...
  function uninstallMac() {
    if (launchdPlist && fs.existsSync(launchdPlist)) {
      run('launchctl', ['unload', launchdPlist], { stdio: 'ignore' });
  ...
  function cleanupLegacyMacAutostart() {
    const launchdDir = launchdPlist ? path.dirname(launchdPlist) : '';
    if (!launchdDir) return;
    legacyLabels().forEach((legacyLabel) => {
      const legacyPlist = path.join(launchdDir, `${legacyLabel}.plist`);
      run('launchctl', ['unload', legacyPlist], { stdio: 'ignore' });
...
`;
  ...
  function cleanupLegacyWindowsAutostart() {
    ...
    legacyLabels().forEach((legacyLabel) => {
      const legacyScript = path.join(startupDir, `${legacyLabel}.cmd`);

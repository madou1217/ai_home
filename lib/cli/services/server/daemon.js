...
const {
  ...
  windowsSpawnOptions
} = require('../../../runtime/windows-cmd-launch');
...
function createServerDaemonService(deps = {}) {
  ...
  const goCoreSupervisor = deps.goCoreSupervisor || null;
  ...
  function isPermissionDeniedError(error) {
    ...
    return /access is denied|permission denied|operation not permitted/iu.test(String(error.message || ''));
  }
  ...
  function terminateProcess(pid, options = {}) {
    ...
    try {
      ...
      return {
        stopped: false,
        reason: isPermissionDeniedError(error) ? 'permission_denied' : 'kill_failed',
  ...
  function findListeningPidsByPort(port) {
    ...
    try {
      let pids = [];
      if (processObj.platform === 'win32') {
        const netstatCmd = buildWindowsCmdLaunch(`netstat -ano | findstr LISTEN | findstr :${p}`);
        const out = spawnSync(netstatCmd.file, netstatCmd.args, {
          ...windowsSpawnOptions(netstatCmd),
          encoding: 'utf8',
          windowsHide: true
        });
        if (out.status === 0 && out.stdout) {
  ...
  async function startNode(rawServeArgs, startOptions = {}) {
    ensureDir(aiHomeDir);
    ...
    const child = spawn(processObj.execPath, [startEntry.entryFilePath, 'server', 'serve', ...serveArgs], {
      ...
      env: processObj.env,
      // Windows: detached spawn would otherwise give the server its own
      // visible console window on every start/restart.
      windowsHide: true
    });
    child.unref();
  ...
  /** 启动 Node 公开宿主；只有显式要求时才在其旁边启动私有 Go Core。 */
  async function start(rawServeArgs, startOptions = {}) {
    const result = await startNode(rawServeArgs, startOptions);
    if (startOptions.startGoCore !== true) return result;

    try {
      const goCore = await startGoCore(startOptions.goCoreOptions || {});
      return { ...result, goCore };
    } catch (error) {
      return {
        ...result,
        goCore: {
          ...getGoCoreStatus(),
          state: 'failed',
          error: error.code || 'go_core_start_failed'
        },
        goCoreFailed: true
      };
    }
  ...
  async function restart(rawServeArgs, restartOptions = {}) {
    ...
    const trackedPidOwnsTargetPort = trackedPid > 0
      && findListeningPidsByPort(targetPort).includes(trackedPid);
    if (!trackedPidOwnsTargetPort) resolveStartTarget(targetPort);
  ...
  async function startGoCore(startOptions = {}) {
    if (!goCoreSupervisor || typeof goCoreSupervisor.start !== 'function') {
      return getGoCoreStatus();
    }
    return goCoreSupervisor.start(startOptions);
  }

  async function stopGoCore(stopOptions = {}) {
    if (!goCoreSupervisor || typeof goCoreSupervisor.stop !== 'function') {
      return getGoCoreStatus();
    }
    return goCoreSupervisor.stop(stopOptions);
  }

  function getGoCoreStatus() {
    if (!goCoreSupervisor || typeof goCoreSupervisor.status !== 'function') {
      return { enabled: false, state: 'disabled', pid: 0, endpoint: '', error: '' };
    }
    return goCoreSupervisor.status();
  }
  ...
  function stopServerProcesses(stopOptions = {}, stoppedAutostart = null) {
    ...
    if (!servers.length) {
      ...
      const trackedPidOwnsConfiguredPort = pid > 0
        && findListeningPidsByPort(configuredPort).includes(pid);
      if (trackedPidOwnsConfiguredPort) {
        const result = terminateProcess(pid, { gracefulStopWaitMs });
        if (result.stopped) clearPidState();
        return {
          ...result,
          port: configuredPort,
          entryFilePath

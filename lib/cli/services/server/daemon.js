...
function createServerDaemonService(deps = {}) {
  ...
  async function start(rawServeArgs, startOptions = {}) {
    ...
    if (isAlive(existingPid)) {
      return { alreadyRunning: true, pid: existingPid, started: true, ready: true };
    }
    const existingByPort = findServerPidByPort(targetPort);
    if (existingByPort > 0) {
      try { fs.writeFileSync(pidFile, String(existingByPort)); } catch (_e) {}
      return { alreadyRunning: true, pid: existingByPort, started: true, ready: true };
    }
    if (existingPid) {
      try { fs.unlinkSync(pidFile); } catch (_e) {}
    }
    ...
    const started = await waitForReady(parsed.port, readyTimeoutMs);
    if (!isAlive(child.pid)) {
      const recoveredPid = findServerPidByPort(targetPort);
      if (recoveredPid > 0) {
        try { fs.writeFileSync(pidFile, String(recoveredPid)); } catch (_e) {}
        return { alreadyRunning: true, pid: recoveredPid, started: true, ready: true };
      }
      try { fs.unlinkSync(pidFile); } catch (_e) {}
      return {
        alreadyRunning: false,
    ...
    return { alreadyRunning: false, pid: child.pid, started, ready: started };
  }

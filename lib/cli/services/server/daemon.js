...
function createServerDaemonService(deps = {}) {
  ...
  function listAihServerProcesses() {
    try {
      ...
      return rows.filter((row) => isAlive(row.pid));
    } catch (_e) {
      return [];
    }
  }
  ...
  function isRestartableAutostart(status) {
    if (!status || !status.supported || !status.installed || !status.loaded) return false;
    return status.type === 'launchd' || status.type === 'systemd-user';
  }
  ...
  async function restartLoadedAutostart(targetPort, restartOptions, autostartStatus) {
    ...
    const readyTimeoutMs = Math.max(500, Number(restartOptions.readyTimeoutMs) || 7000);
    const waitForReadyEnabled = restartOptions.waitForReady !== false;
  ...
  async function start(rawServeArgs, startOptions = {}) {
    ...
    if (existingServer && existingServer.port === effectivePort) {
      const drift = getServeConfigDrift(existingServer, serveInputArgs, parsed, effectivePort);
      if (!drift.replace) {
    ...
    if (existingByPort > 0) {
      ...
      const drift = getServeConfigDrift(processSnapshot, serveInputArgs, parsed, effectivePort);
      if (!drift.replace) {

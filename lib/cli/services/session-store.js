...
const { MARKER_FILE: TRANSIENT_AUTH_MARKER_FILE } = require('../../runtime/transient-auth-projection');
...
const SESSION_STORE_ALLOWLIST = {
  ...
  kimi: getProviderSharedEntries('kimi'),
  grok: getProviderSharedEntries('grok'),
  // CodeBuddy 家族：`projects` 是产品级会话存储（不是账号私有状态），显式声明后
  // 每个账号投影里的 projects 都链接到宿主那一份——切换选中账号不会改变可见历史。
  codebuddy: getProviderSharedEntries('codebuddy'),
  codebuddycn: getProviderSharedEntries('codebuddycn'),
  workbuddy: getProviderSharedEntries('workbuddy'),
  workbuddycn: getProviderSharedEntries('workbuddycn')
};

// 会话存储属于**产品而非账号**的 Provider（CodeBuddy 家族，`projects` 是同一台机器上
// WorkBuddy 与 CodeBuddy 共用的一份地区级历史，见 session-reader-codebuddy.js）。
// 这类 Provider 走两条与其它 Provider 不同的规则：
//
//   1. **共享面 = 声明的 sharedEntries，正好等于会话条目**。通用链路对非 opencode 的
//      Provider 是"检测到什么就共享什么"（`shareAllDetectedEntries`），于是会把
//      logs / plugins / local_storage / shell-snapshots 一并链接进宿主目录；对
//      CodeBuddy 家族这既没必要，也会把账号私有状态拖进共享面。
//   2. **绝不把投影里已有的会话迁移进宿主存储**。通用链路在建立链接前会
//      `mergeEntryIntoStore`（move，跨设备时是 copy + unlink）把投影内容搬进宿主。
//      产品级会话不该由 aih 搬运：要么投影侧是空目录（丢弃后建链），要么里面有真实
//      会话文件——那就如实报 unresolved，让上层 fail-closed，由人决定，aih 不复制、
//      不移动、不覆盖。
//
// 结果：账号沙箱里的 <configDir>/projects 是一条指向宿主地区存储的**软链接**
// （一份物理数据，relay 时 CLI 直接写进地区存储），账号切换不会改动它，也不会有
// 任何会话文件被复制或搬走。
const SESSION_STORE_PRODUCT_LEVEL_PROVIDERS = new Set([
  'codebuddy',
  'codebuddycn',
  'workbuddy',
  'workbuddycn'
...
// Kimi <= 0.35 used the projection root itself as KIMI_CODE_HOME. The native
// CLI now expects <projection>/.kimi-code, so these three entries must be
// moved before the normal shared-session reconciliation walks the projection.
// The migration is deliberately limited to known private paths; unknown
// legacy entries remain subject to the existing fail-closed checks.
const KIMI_LEGACY_PROJECTION_ENTRIES = Object.freeze([
  Object.freeze({ legacy: Object.freeze(['config.toml']), current: Object.freeze(['.kimi-code', 'config.toml']) }),
  Object.freeze({ legacy: Object.freeze(['credentials']), current: Object.freeze(['.kimi-code', 'credentials']) }),
  Object.freeze({ legacy: Object.freeze(['device_id']), current: Object.freeze(['.kimi-code', 'device_id']) })
]);
...
const ACCOUNT_OWNED_ENTRY_DENYLIST = {
  ...
  grok: new Set(['auth.json', 'auth.json.lock']),
  agy: new Set([
...
// Provider-created launch shims are disposable process state. Sharing Codex
// tmp leaks native wrapper symlinks into durable state and makes a later
// fail-closed reconciliation reject the whole projection.
const PROVIDER_EPHEMERAL_ENTRIES = {
  codex: new Set(['tmp'])
};
...
function shouldShareToolConfigEntry(cliName, name) {
  ...
  const ephemeral = PROVIDER_EPHEMERAL_ENTRIES[cliName];
  if (ephemeral && ephemeral.has(entryName.toLowerCase())) return false;
...
function collectSharedToolEntryNames(fsImpl, cliName, dirPaths = []) {
  ...
  // 产品级会话存储 Provider（CodeBuddy 家族）：共享面**严格等于**声明的 sharedEntries，
  // 连 `isLikelySessionName` 这类启发式也不参与——否则 history.jsonl / shell-snapshots
  // （名字里带 history / snapshot）会被顺带链接出去。见 SESSION_STORE_PRODUCT_LEVEL_PROVIDERS。
  const allowlistOnly = SESSION_STORE_PRODUCT_LEVEL_PROVIDERS.has(cliName);
  // 通用链路是非 opencode Provider 一律"检测到什么就共享什么"。
  const shareAllDetectedEntries = cliName !== 'opencode' && !allowlistOnly;
  ...
  for (const dirPath of Array.isArray(dirPaths) ? dirPaths : []) {
    for (const name of safeReadDirEntries(fsImpl, dirPath)) {
      ...
      if (allowlistOnly) {
        if (allowlist.includes(name)) candidates.add(name);
        continue;
      }
      if (
...
function createSessionStoreService(options = {}) {
  const {
    ...
    cliConfigs,
    getProfileDir,
    ensureDir: injectedEnsureDir
  } = options;
  const ensureDir = typeof injectedEnsureDir === 'function'
    ? injectedEnsureDir
  ...
  function isConfigAtProjectionRoot(cliName) {
    const config = cliConfigs[cliName] || {};
    return config.configAtProjectionRoot === true;
  }

  function resolveToolConfigRelativeSegments(cliName, options = {}) {
    const config = cliConfigs[cliName] || {};
    const globalFolder = String(config.globalDir || `.${cliName}`).trim() || `.${cliName}`;
    const configSubDir = String(config.configSubDir || '').trim();
    // --config-dir providers (Qoder): projection root IS the config root.
    // Host shared store still lives under globalDir (e.g. ~/.qoder-cn).
    if (isConfigAtProjectionRoot(cliName) && options.forProjection === true) {
      return configSubDir ? [configSubDir] : [];
    }
    return configSubDir ? [globalFolder, configSubDir] : [globalFolder];
  }

  function getProjectionToolConfigDir(cliName, id, runtime = {}) {
    const projectionDir = getProjectionRoot(cliName, id, runtime);
    const segments = resolveToolConfigRelativeSegments(cliName, { forProjection: true });
    return segments.length > 0 ? path.join(projectionDir, ...segments) : projectionDir;
  }

  function getGlobalToolConfigRoot(cliName) {
    const segments = resolveToolConfigRelativeSegments(cliName, { forProjection: false });
    return path.join(hostHomeDir, ...segments);
  }
  ...
  function discardEntryInPlace(srcPath) {
    if (!pathExistsByLstat(srcPath)) return 0;
    fse.removeSync(srcPath);
    return 1;
  }

  function migrateKimiLegacyProjection(projectionRoot) {
    const summary = { migrated: 0, linked: 0, unresolved: [] };
    if (!projectionRoot) return summary;

    const kimiRoot = path.join(projectionRoot, '.kimi-code');
    const kimiRootStat = lstatIfExists(kimiRoot);
    let movedKimiRoot = false;
    if (kimiRootStat && kimiRootStat.isSymbolicLink()) {
      // Kimi 0.36.0 must receive a real account-private home. The stale alias
      // is disposable projection residue and is discarded in place; the
      // target it points to is never inspected or removed.
      summary.migrated += discardEntryInPlace(kimiRoot);
      movedKimiRoot = true;
    }

    const hasLegacyEntry = KIMI_LEGACY_PROJECTION_ENTRIES.some((mapping) => (
      pathExistsByLstat(path.join(projectionRoot, ...mapping.legacy))
    ));
    if (!hasLegacyEntry && !movedKimiRoot) return summary;
    ensureProjectionDirectory(projectionRoot, kimiRoot);

    KIMI_LEGACY_PROJECTION_ENTRIES.forEach((mapping) => {
      const sourcePath = path.join(projectionRoot, ...mapping.legacy);
      if (!pathExistsByLstat(sourcePath)) return;

      const targetPath = path.join(projectionRoot, ...mapping.current);
      // Never merge into a legacy or user-created link. This assertion checks
      // the destination with lstat and therefore cannot follow an external
      // target while preserving the fail-closed projection boundary.
      assertProjectionPathHasNoSymlinks(projectionRoot, targetPath);
      const sourceStat = lstatIfExists(sourcePath);

      if (sourceStat && sourceStat.isSymbolicLink()) {
        // A stale legacy alias is disposable projection residue and is
        // discarded in place; its external target is never read, copied, or
        // removed.
        summary.migrated += discardEntryInPlace(sourcePath);
        return;
      }

      if (!pathExistsByLstat(targetPath)) {
        ensureDir(path.dirname(targetPath));
        fse.moveSync(sourcePath, targetPath, { overwrite: false });
        summary.migrated += 1;
        return;
      }

      summary.migrated += mergeEntryIntoStore(
        'kimi',
  ...
  // Kimi session index views. The sessions directory itself stays provider-
  // shared (one physical copy, linked from every projection), but the kimi CLI
  // filters index entries by a plain string-prefix check against its own
  // KIMI_CODE_HOME/sessions (isPathInside), so a shared index makes other
  // accounts' sessions visible yet unresumable (session.not_found). aih keeps
  // a canonical index in the host store (sessionDir in host path form) and
  // regenerates each projection's private session_index.jsonl as a view: same
  // entries, sessionDir rewritten to that projection's own sessions path.
  // Only index metadata is rewritten; session data is never copied.
  function toSlashPath(value) {
    const resolved = String(value || '');
    return processObj.platform === 'win32' ? resolved.replace(/\\/g, '/') : resolved;
  }

  function parseKimiSessionIndexLine(line) {
    let parsed = null;
    try {
    ...
    if (parsed.deleted === true) return { sessionId, deleted: true };
    const sessionDir = String(parsed.sessionDir || '').trim();
    const workDir = String(parsed.workDir || '').trim();
    if (!sessionDir || !workDir || !path.isAbsolute(sessionDir)) return null;
    return { sessionId, sessionDir, workDir };
  }

  function readKimiSessionIndexRecords(indexFilePath) {
    if (!pathExistsByLstat(indexFilePath)) return [];
    let content = '';
    try {
    ...
    return content.split(/\r?\n/)
      ...
      .filter(Boolean)
      .map((line) => ({ line, record: parseKimiSessionIndexLine(line) }));
  }

  // Canonicalizes any sessionDir shaped like
  // <aiHome>/run/auth-projections/kimi/<ref>/.kimi-code/sessions/<rel> — works
  // even for projections that no longer exist on disk.
  function relFromKimiProjectionsBase(kimiProjectionsBase, resolvedDir) {
    if (!kimiProjectionsBase || !isWithinDirectory(kimiProjectionsBase, resolvedDir)) return null;
    const segments = path.relative(path.resolve(kimiProjectionsBase), resolvedDir).split(path.sep);
    if (segments.length < 4 || segments[1] !== '.kimi-code' || segments[2] !== 'sessions') return null;
    return segments.slice(3).join(path.sep);
  }

  function canonicalizeKimiSessionRecord(record, sessionRoots, kimiProjectionsBase) {
    const resolved = path.resolve(record.sessionDir);
    if (path.basename(resolved) !== record.sessionId) return null;
    for (const root of sessionRoots) {
      if (!isWithinDirectory(root, resolved)) continue;
      return {
        sessionId: record.sessionId,
        rel: toSlashPath(path.relative(path.resolve(root), resolved)),
        workDir: record.workDir
      };
    }
    const rel = relFromKimiProjectionsBase(kimiProjectionsBase, resolved);
    if (rel === null) return null;
    return { sessionId: record.sessionId, rel: toSlashPath(rel), workDir: record.workDir };
  }

  function writeFileIfChanged(filePath, content) {
    const exists = pathExistsByLstat(filePath);
    if (exists && fs.readFileSync(filePath, 'utf8') === content) return false;
    if (!exists && content === '') return false;
    ensureDir(path.dirname(filePath));
    const tempPath = `${filePath}.aih-sync-${Date.now()}`;
    try {
  ...
  function syncKimiSessionIndexView(projectionRoot) {
    const summary = { migrated: 0, unresolved: [] };
    if (!projectionRoot) return summary;

    const kimiHome = path.join(projectionRoot, '.kimi-code');
    const viewIndexPath = path.join(kimiHome, 'session_index.jsonl');
    const viewSessionsDir = path.join(kimiHome, 'sessions');
    const storeSessionsRoot = path.join(hostHomeDir, '.kimi-code', 'sessions');
    const storeIndexPath = path.join(hostHomeDir, '.kimi-code', 'session_index.jsonl');

    // The view must be a plain file owned by aih. Legacy shared links (symlink
    // or Windows hard link into the host index) are detached; foreign links
    // fail closed.
    const viewStat = lstatIfExists(viewIndexPath);
    let detachedSharedLink = false;
    if (viewStat && viewStat.isDirectory()) {
      summary.unresolved.push(path.join('.kimi-code', 'session_index.jsonl'));
      return summary;
    }
    if (viewStat && viewStat.isSymbolicLink()) {
      if (!pointsToManagedTarget(viewIndexPath, storeIndexPath)) {
        summary.unresolved.push(path.join('.kimi-code', 'session_index.jsonl'));
        return summary;
      }
      fs.unlinkSync(viewIndexPath);
      detachedSharedLink = true;
    } else if (
      viewStat
      && pathExistsByLstat(storeIndexPath)
      && isSameFilesystemEntry(viewIndexPath, storeIndexPath)
    ) {
      fs.unlinkSync(viewIndexPath);
      detachedSharedLink = true;
    }
    ...
    const sessionRoots = [
      path.resolve(storeSessionsRoot),
      path.resolve(viewSessionsDir)
    ];
    const aiHome = getAiHomeDir();
    const kimiProjectionsBase = aiHome
      ? path.join(aiHome, 'run', 'auth-projections', 'kimi')
      : '';
    const canonical = new Map();
    const keptLines = [];
    // Unparseable and foreign lines stay in the canonical file verbatim;
    // canonicalizable entries are deduplicated by sessionId (latest wins) and
    // normalized to host path form.
    readKimiSessionIndexRecords(storeIndexPath).forEach(({ line, record }) => {
      if (!record) {
        keptLines.push(line);
        return;
      }
      if (record.deleted) {
        canonical.delete(record.sessionId);
        return;
      }
      const entry = canonicalizeKimiSessionRecord(record, sessionRoots, kimiProjectionsBase);
      if (!entry) {
        keptLines.push(line);
        return;
      }
      canonical.set(record.sessionId, entry);
    });
    // A view just detached from the shared index is identical to the canonical
    // file, so only merge a view that was already private: it may carry
    // sessions this account created since the last sync.
    if (!detachedSharedLink) {
      readKimiSessionIndexRecords(viewIndexPath).forEach(({ record }) => {
        if (!record) return;
        if (record.deleted) {
          canonical.delete(record.sessionId);
          return;
        ...
        const entry = canonicalizeKimiSessionRecord(record, sessionRoots, kimiProjectionsBase);
        if (entry) canonical.set(record.sessionId, entry);
      });
    }

    const canonicalLines = [
      ...keptLines,
      ...Array.from(canonical.values()).map((entry) => JSON.stringify({
        sessionId: entry.sessionId,
        sessionDir: toSlashPath(path.join(storeSessionsRoot, entry.rel)),
        workDir: entry.workDir
      }))
    ];
    if (writeFileIfChanged(storeIndexPath, canonicalLines.length > 0 ? `${canonicalLines.join('\n')}\n` : '')) {
      summary.migrated += 1;
    }

    // The view lists every canonical session whose data exists, with sessionDir
    // in this projection's path form so kimi's prefix check accepts it. While
    // a legacy private sessions directory is still being merged back into the
    // store, data may physically live under the view root, so check both.
    const viewLines = [];
    canonical.forEach((entry) => {
      const storeStat = statIfExists(path.join(storeSessionsRoot, entry.rel));
      const viewStatForEntry = statIfExists(path.join(viewSessionsDir, entry.rel));
      if (!storeStat?.isDirectory() && !viewStatForEntry?.isDirectory()) return;
      viewLines.push(JSON.stringify({
        sessionId: entry.sessionId,
        sessionDir: toSlashPath(path.join(viewSessionsDir, entry.rel)),
        workDir: entry.workDir
      }));
    });
    if (writeFileIfChanged(viewIndexPath, viewLines.length > 0 ? `${viewLines.join('\n')}\n` : '')) {
      summary.migrated += 1;
    }
    ...
    entries.forEach((entry) => {
      ...
      if (entry.isDirectory()) {
        ...
        if (nestedStoreStat && !statIfExists(nestedStorePath)?.isDirectory()) {
          migrated += discardEntryInPlace(nestedSrcPath);
          return;
        }
  ...
  function mergeEntryIntoStore(cliName, entryName, srcPath, storePath) {
    ...
    if (srcStat.isDirectory()) {
      ...
      if (!storeStat || !storeStat.isDirectory()) {
        return discardEntryInPlace(srcPath);
      }
  ...
  function migrateAndLinkSessionEntry(cliName, entryName, srcPath, storePath) {
    ...
    if (pathExistsByLstat(srcPath)) {
      ...
      if (srcStat.isSymbolicLink()) {
        ...
        migrated += 1;
      } else if (SESSION_STORE_PRODUCT_LEVEL_PROVIDERS.has(cliName)) {
        // 产品级会话存储：**绝不迁移**。空目录可以安全丢弃后建链；里面有真实会话
        // 文件就如实报 unresolved，让上层 fail-closed —— aih 不复制、不移动、不覆盖。
        if (srcStat.isDirectory() && safeReadDirEntries(fs, srcPath).length === 0) {
          fse.removeSync(srcPath);
          migrated += 1;
        } else {
          return { migrated, linked, unresolved: true };
  ...
  function classifyPrivateProjectionPath(cliName, relativeSegments) {
    ...
    if (
      candidate.length === 1
      && candidate[0] === TRANSIENT_AUTH_MARKER_FILE.toLowerCase()
    ) {
      return { isPrivate: true, containsPrivate: false };
    }
    const privatePaths = [
  ...
  function reconcileProjectionResources(cliName, accountRef, runtime = {}) {
    const projectionRoot = getProjectionRoot(cliName, accountRef, runtime);
    const summary = { migrated: 0, linked: 0, unresolved: [] };
    if (!projectionRoot) return summary;
    const handledToolRoot = normalizeProjectionSegments(
      resolveToolConfigRelativeSegments(cliName, { forProjection: true })
    );
  ...
  function prepareProviderRuntimeHome(cliName, accountRef, runtime = {}) {
    const summary = { migrated: 0, linked: 0, unresolved: [] };
    const projectionRoot = getProjectionRoot(cliName, accountRef, runtime);
  ...
  function ensureSessionStoreLinks(cliName, accountRef, runtime = {}) {
    ...
    if (cliName === 'kimi') {
      const legacyResult = migrateKimiLegacyProjection(projectionRoot);
      migrated += legacyResult.migrated;
      linked += legacyResult.linked;
      unresolved.push(...legacyResult.unresolved);

      // A login runtime is a disposable credential workspace, not a provider
      // account projection. Historical login runtimes were built as fake HOME
      // trees containing links for unrelated host files (Library, .config,
      // projects, etc.). Reconcile only Kimi's known legacy private entries
      // here; walking that fake HOME would reject unrelated links and can
      // never improve Kimi session persistence.
      if (explicitProjection) {
        const result = { migrated, linked };
        if (unresolved.length > 0) {
          result.unresolved = Array.from(new Set(unresolved.filter(Boolean)));
        }
        return result;
      }

      const viewResult = syncKimiSessionIndexView(projectionRoot);
      migrated += viewResult.migrated;
      unresolved.push(...viewResult.unresolved);
    }
    ...
    } else if (isConfigAtProjectionRoot(cliName)) {
      // Qoder-style: --config-dir owns the whole account tree. Auth artifacts stay
      // account-private under the projection root; do not bridge the projection
      // root into the host globalDir (would mark `.qoder` / whole tree unresolved).
    ...
    // Config-at-root providers already own the projection root as private config;
    // walking it with reconcileProjectionResources would reclassify auth files and
    // empty roots as unresolved (.qoder / credentials).
    if (!isConfigAtProjectionRoot(cliName)) {

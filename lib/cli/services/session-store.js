...
function createSessionStoreService(options = {}) {
  ...
  function migrateKimiLegacyProjection(projectionRoot, migrationOptions = {}) {
    ...
    if (kimiRootStat && kimiRootStat.isSymbolicLink()) {
      // Kimi 0.36.0 must receive a real account-private home. Preserve only
      // the alias itself before rebuilding the directory; never inspect or
      // remove the target it points to.
      summary.migrated += preserveConflictingEntry(kimiRoot, {
        ...migrationOptions,
        relativePath: path.join('legacy-kimi', '.kimi-code')
      });
      movedKimiRoot = true;
    }
    ...
    KIMI_LEGACY_PROJECTION_ENTRIES.forEach((mapping) => {
      ...
      const relativePath = path.join('legacy-kimi', ...mapping.legacy);
      const sourceStat = lstatIfExists(sourcePath);

      if (sourceStat && sourceStat.isSymbolicLink()) {
        // A stale legacy alias is moved as a link into the private conflict
        // store; its external target is never read, copied, or removed.
        summary.migrated += preserveConflictingEntry(sourcePath, {
          ...migrationOptions,
          relativePath
        });
        return;
  ...
  function ensureSessionStoreLinks(cliName, accountRef, runtime = {}) {
    ...
    if (cliName === 'kimi') {
      const legacyResult = migrateKimiLegacyProjection(projectionRoot, migrationOptions);

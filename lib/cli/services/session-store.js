...
const ACCOUNT_OWNED_ENTRY_DENYLIST = {
  ...
  agy: new Set(['antigravity-oauth-token'])
};
...
function collectSharedToolEntryNames(fsImpl, cliName, dirPaths = []) {
  ...
  const shareAllDetectedEntries = true;
...
function createSessionStoreService(options = {}) {
  ...
  function ensureSessionStoreLinks(cliName, id, options = {}) {
    ...
    if (processObj.platform === 'darwin' && hostHomeDir) {
      ...
      const guestKeychainsDir = path.join(guestLibraryDir, 'Keychains');
      const hostKeychainsDir = path.join(hostHomeDir, 'Library', 'Keychains');
      if (fs.existsSync(hostKeychainsDir)) {
        if (!isManagedLink(guestKeychainsDir, hostKeychainsDir)) {
          try {
            fse.removeSync(guestKeychainsDir);
          } catch (_e) {}
          createManagedLinkSafe(hostKeychainsDir, guestKeychainsDir, true);
        }
      }
    }

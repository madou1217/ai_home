...
const CLAUDE_API_CREDENTIAL_CONFIG_DIR = '.claude-api-credential';
const CLAUDE_SHARED_STATE_ENTRIES = [
  'history.jsonl',
  'projects',
  'shell-snapshots',
  '.claude.json',
...
const ACCOUNT_OWNED_ENTRY_DENYLIST = {
  ...
  claude: new Set(['.credentials.json', 'credentials.json']),
...
function createSessionStoreService(options = {}) {
  ...
  function getToolConfigDir(cliName, id) {
    ...
    return configSubDir
      ? path.join(getProfileDir(cliName, id), globalFolder, configSubDir)
      : path.join(getProfileDir(cliName, id), globalFolder);
  }
  ...
  function getAiHomeDir() {
    const normalizedProfilesDir = String(profilesDir || '').trim();
    if (normalizedProfilesDir && path.basename(normalizedProfilesDir) === 'profiles') {
      return path.dirname(normalizedProfilesDir);
    }
    return hostHomeDir ? path.join(hostHomeDir, '.ai_home') : '';
  }
  ...
  function mergeUniqueJsonlLines(storePath, srcPath) {
    let storeLines = [];
    let srcLines = [];
    try {
      storeLines = fs.existsSync(storePath)
        ? fs.readFileSync(storePath, 'utf8').split(/\r?\n/).filter(Boolean)
        : [];
      srcLines = fs.existsSync(srcPath)
    ...
    const seen = new Set(storeLines);
    const merged = storeLines.slice();
    ...
    if (merged.length === storeLines.length && fs.existsSync(storePath)) return false;
    ensureDir(path.dirname(storePath));
    fs.writeFileSync(storePath, merged.length ? `${merged.join('\n')}\n` : '', 'utf8');
    return true;
  }

  function mergeClaudeCustomApiKeyResponses(targetValue, sourceValue) {
    const out = {
    ...
    ['approved', 'rejected'].forEach((key) => {
      ...
      [
        ...
        sourceValue && Array.isArray(sourceValue[key]) ? sourceValue[key] : []
      ].forEach((items) => {
        items.forEach((item) => {
  ...
  function mergeClaudeProjectState(targetProjects, sourceProjects) {
    const merged = {
      ...(sourceProjects && typeof sourceProjects === 'object' && !Array.isArray(sourceProjects)
        ? sourceProjects
        : {})
    };
    if (targetProjects && typeof targetProjects === 'object' && !Array.isArray(targetProjects)) {
      Object.entries(targetProjects).forEach(([projectPath, targetProject]) => {
        const sourceProject = merged[projectPath];
        if (
          sourceProject
          && typeof sourceProject === 'object'
          && !Array.isArray(sourceProject)
          && targetProject
          && typeof targetProject === 'object'
          ...
          merged[projectPath] = targetProject;
        }
      });
    }
    return merged;
  ...
  function mergeClaudeConfigJson(storePath, srcPath) {
    const storeConfig = safeReadJsonFile(storePath);
    const srcConfig = safeReadJsonFile(srcPath);
    if (!storeConfig && !srcConfig) return false;
    if (!storeConfig && srcConfig) {
      writeJsonFile(storePath, srcConfig);
      return true;
    }
    if (!srcConfig) return false;

    const merged = {
      ...srcConfig,
      ...storeConfig
    };
    merged.customApiKeyResponses = mergeClaudeCustomApiKeyResponses(
      storeConfig && storeConfig.customApiKeyResponses,
      srcConfig && srcConfig.customApiKeyResponses
    );
    merged.projects = mergeClaudeProjectState(
      storeConfig && storeConfig.projects,
      srcConfig && srcConfig.projects
    );

    if (JSON.stringify(merged) === JSON.stringify(storeConfig)) return false;
  ...
  function mergeEntryIntoStore(cliName, entryName, srcPath, storePath) {
    ...
    try {
      ...
      if (cliName === 'claude' && entryName === '.claude.json') {
        const changed = mergeClaudeConfigJson(storePath, srcPath);
        fse.removeSync(srcPath);
        return changed ? 2 : 1;
      }

      if (cliName === 'claude' && entryName === 'history.jsonl') {
        const changed = mergeUniqueJsonlLines(storePath, srcPath);
        fse.removeSync(srcPath);
        return changed ? 2 : 1;
      }
  ...
  function ensureClaudeApiCredentialLegacyLinks(id, storeRoot) {
    const legacyDir = path.join(getProfileDir('claude', id), CLAUDE_API_CREDENTIAL_CONFIG_DIR);
    if (!pathExistsByLstat(legacyDir)) return { migrated: 0, linked: 0 };
    if (!ensureToolConfigDir(legacyDir)) return { migrated: 0, linked: 0 };
    return ensureConfigDirSessionStoreLinks(
      'claude',
      legacyDir,
      storeRoot,
      CLAUDE_SHARED_STATE_ENTRIES
    );
  }
  ...
  function listCodexDesktopRuntimeHomes() {
    ...
    const runtimeRoot = aiHomeDir ? path.join(aiHomeDir, 'codex-desktop-runtime') : '';
    ...
    return entries
      .filter((entry) => entry && entry.isDirectory() && /^app-server-\d+$/.test(entry.name))
  ...
  function ensureSessionStoreLinks(cliName, id, options = {}) {
    ...
    if (processObj.platform === 'darwin' && hostHomeDir) {
      const guestProfileDir = getProfileDir(cliName, id);
    ...
    if (cliName === 'agy') {
      const parentToolConfigDir = path.join(getProfileDir(cliName, id), '.gemini');
    ...
    if (cliName === 'claude') {
      const legacyResult = ensureClaudeApiCredentialLegacyLinks(id, storeRoot);

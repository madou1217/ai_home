...
function buildAiHomeTerminalTitle(provider) {
  const providerMeta = getProviderMeta(provider);
  const fallbackMeta = getProviderMeta('');
  const icon = String(fallbackMeta.terminalIcon || '').trim();
  const label = String(providerMeta.label || provider || 'AI').trim();
  return `${icon ? `${icon} ` : ''}AIH ${label}`.trim();
}
...
function activateTitleProviderBadge(provider, strategy, options = {}) {
  ...
  const title = String(options.title || buildProviderTerminalTitle(normalizedProvider)).trim();
  const sequence = buildTerminalTitleSequence(title);
...
function prepareCurrentTerminalProviderIcon(provider, options = {}) {
  ...
  if (terminal === 'warp') {
    if (activateWarpAgentCommandMapping(provider, { ...options, processImpl })) return { applied: true, terminal };
    if (activateTitleProviderBadge(provider, strategy, {
      ...options,
      processImpl,
      title: buildAiHomeTerminalTitle(provider)

...
// Category mapping for apps/tools
const APP_CATEGORIES = {
  cli: ['claude', 'codex', 'gemini', 'agy', 'opencode', 'grok', 'qoder', 'qodercn', 'kimi', 'kiro'],
  desktop: ['claude-desktop', 'codex-desktop', 'gemini-desktop'],
  ide: ['vscode', 'cursor', 'windsurf'],
  agents: ['opencode', 'agy', 'hermes-agent']
};
...
function getConfigFormat(configPath) {
  const extension = nodePath.extname(String(configPath || '')).toLowerCase();
...
async function listManagedApps(options = {}) {
  ...
  for (const providerId of cliList) {
    ...
    if (APP_CATEGORIES.agents.includes(providerId)) {
  ...
  // Add Desktop & IDE Apps (Claude Desktop, ChatGPT Desktop, VS Code)
  const desktopApps = [
    {
      id: 'claude-desktop',
      name: 'Claude Desktop',
      provider: 'claude',
      configProvider: 'claude-desktop',
      type: 'desktop',
      categories: ['ALL', 'Desktop'],
      binaryName: 'Claude.app',
      pkg: '',
    ...
    },
    {
      ...
      type: 'desktop',
      categories: ['ALL', 'Desktop'],
      binaryName: 'ChatGPT.app',
      pkg: '',
      defaultModel: 'gpt-5.5',
      supportedModels: ['gpt-5.5', 'gpt-5-turbo', 'o3', 'o1']
    },
    {
      id: 'vscode',

...
const APP_CATEGORIES = Object.freeze({
  cli: Object.freeze(listInstallableAiClis()),
...
async function listManagedApps(options = {}) {
  ...
  const cliList = listInstallableAiClis();
  const apps = [];
  ...
  for (const providerId of cliList) {
    ...
    const label = String((def.presentation && def.presentation.label) || def.name || cliConfig.name || providerId).trim();
    apps.push({
      id: providerId,
      // CLI 与 Desktop 是合同中的两种不同客户端形态；名称显式带出 CLI，
      // 避免在“全部”视图里把 Gemini CLI 误认为 Gemini Desktop。
      name: label.endsWith(' CLI') ? label : `${label} CLI`,
      provider: providerId,
      clientId: providerId,
      clientName: label,
      type: 'cli',
  ...
  desktopApps.push({
    ...
    clientId: 'vscode',
    clientName: 'Visual Studio Code',
    // VS Code 当前由 Codex 扩展接入；后续接入其他 Provider 只需扩展
    // integrationProviders，前端图标组件会自动叠加对应 Provider 标识。
    integrationProviders: ['codex'],

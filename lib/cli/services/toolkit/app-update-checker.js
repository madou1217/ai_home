  ...
  return new Promise((resolve) => {
    ...
    try {
      child = spawnImpl(command, args, {
        encoding: 'utf8',
        windowsHide: true,
        // npm 在 Windows 上是 npm.cmd，不开 shell 直接 EINVAL（见 requiresCmdShell）。
        // 这条路径被 listManagedApps 共用，失败的表现是「应用管理」页永远显示没有新版。
        // winget.exe / brew 不以 .cmd 结尾，判据天然不会误开 shell。
        shell: requiresCmdShell(
          command,
          (options.processObj && options.processObj.platform) || process.platform
        ),

...
function writeAtomicFile(fsImpl, pathImpl, targetPath, content, allowCreate = false) {
  ...
  try {
    ...
    if (!allowCreate && !fsImpl.existsSync(targetPath)) throw configNotFoundError();
...
function readManagedAppConfig(appId, options = {}) {
  ...
  if (!exists) {
    if (options.allowMissingTarget === true) {
      return {
        ok: true,
...
function saveManagedAppConfig(appId, content, options = {}) {
  ...
  const displayTarget = resolveTarget(appId, options);
  const target = canonicalizeExistingTarget(displayTarget, fsImpl);
  if (!fsImpl.existsSync(target.targetPath) && options.allowMissingTarget !== true) {
    throw configNotFoundError();
  }

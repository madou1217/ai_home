...
const CONFIG_APP_ALIASES = Object.freeze({
  claude: 'claude',
...
function isWritableTarget(fsImpl, pathImpl, targetPath) {
  ...
  return canAccess(fsImpl, pathImpl.dirname(targetPath), nodeFs.constants.W_OK);
}
...
function buildUnixCopyCommand(sourcePath, targetPath, mode) {
  const parentPath = nodePath.dirname(targetPath);
  return [
    `/bin/mkdir -p -- ${quoteShellArg(parentPath)}`,
    `/usr/bin/install -m ${String(mode || '600')} -- ${quoteShellArg(sourcePath)} ${quoteShellArg(targetPath)}`
  ].join(' && ');
}
...
function runElevatedCopy(sourcePath, targetPath, mode, options = {}) {
  ...
  if (platform === 'win32') {
    ...
    return runWindowsElevated([
      ...
      `Copy-Item -LiteralPath ${pathLiteral(sourcePath)} -Destination ${pathLiteral(targetPath)} -Force`
    ].join('; '), options);
  }
  return runUnixElevated(buildUnixCopyCommand(sourcePath, targetPath, mode), platform, options);
}
...
function writeAtomicFile(fsImpl, pathImpl, targetPath, content) {
  ...
  try {
    ...
    if (stat && Number.isFinite(stat.mode)) mode = stat.mode & 0o777;
  } catch (_error) {}

  const tempPath = `${targetPath}.aih-edit-${process.pid}-${Date.now()}`;
  let renamed = false;
  try {
    fsImpl.writeFileSync(tempPath, content, { encoding: 'utf8', mode });
    if (typeof fsImpl.chmodSync === 'function') {
...
function readManagedAppConfig(appId, options = {}) {
  const fsImpl = options.fs || nodeFs;
  const target = resolveTarget(appId, options);
  ...
  try {
    ...
    if (!isElevationError(error)) throw error;
...
function saveManagedAppConfig(appId, content, options = {}) {
  ...
  try {
    ...
    try {
      const result = runElevatedCopy(temp.tempPath, target.targetPath, '600', options);

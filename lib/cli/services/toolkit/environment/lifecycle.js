...
const { getEnvironmentTool } = require('./catalog');
const { renderPlanCommand } = require('./plan-builders');
const { probeEnvironmentTool, resolveHostHome, resolvePlatform } = require('./probe');
...
function resolveEnvironmentToolPlans(toolId, action, options = {}) {
  ...
  const tool = getEnvironmentTool(toolId);
  ...
  const rawPlans = platform === 'macos'
    ? resolveMacosEnvironmentPlans(tool.id, normalizedAction, lifecycleOptions)
    : platform === 'windows'
      ? resolveWindowsEnvironmentPlans(tool.id, normalizedAction, lifecycleOptions)
      : resolveLinuxEnvironmentPlans(tool.id, normalizedAction, lifecycleOptions);
...
function planEnvironmentToolAction(input = {}, options = {}) {
  ...
  const tool = getEnvironmentTool(input.toolId);
  const probe = probeEnvironmentTool(tool, options);

...
function probeCommand(command, args = ['--version'], options = {}) {
  const normalizedCommand = String(command || '').trim();
  if (!normalizedCommand) {
    return { installed: false, version: '', executablePath: '', managedVersions: [] };
  }
  const result = execCommand(normalizedCommand, args, options);
  return {
    installed: result.ok,
    version: firstLine(result.value),
    executablePath: result.ok ? resolveExecutablePath(normalizedCommand, options) : '',
    managedVersions: []
  };
}
...
function probeEnvironmentTool(tool, options = {}) {
  ...
  if (typeof tool.detect === 'function') return tool.detect(options);
  ...
  return probeCommand(command, tool.probe && tool.probe.args || ['--version'], options);
}

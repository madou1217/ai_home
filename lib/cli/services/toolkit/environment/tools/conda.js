...
function buildPosixCleanupPlan(options = {}) {
...
module.exports = defineEnvironmentToolAdapter({
  ...
  buildPlans(action, options = {}) {
    ...
    if (options.platform === 'macos') {
      ...
      if (action === 'uninstall') return [brew, buildPosixCleanupPlan(options)];
      return [brew, installer];
    }
    if (action === 'update') return [buildPosixUpdatePlan(options), installer];
    if (action === 'uninstall') return [buildPosixCleanupPlan(options)];
    return [installer];
  }
});

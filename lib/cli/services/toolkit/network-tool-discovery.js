...
const FRP_ROLES = Object.freeze(['frpc', 'frps']);
const NETWORK_TOOL_IDS = Object.freeze(['frpc', 'frps', 'cloudflared']);
...
const NETWORK_TOOL_SPECS = Object.freeze({
  ...
  frps: Object.freeze({
    ...
    configDirFlags: [],
    configExtensions: FRP_CONFIG_EXTENSIONS,
    environmentKeys: ['AIH_FRPS_CONFIG', 'FRPS_CONFIG'],
    defaultCwdNames: ['frps.ini']
  }),
  cloudflared: Object.freeze({
    id: 'cloudflared',
    configFlags: ['--config'],
    configDirFlags: [],
    // 只接受 YAML 配置，明确排除证书 PEM 与隧道凭据 JSON。
    configExtensions: ['yml', 'yaml'],
    environmentKeys: ['AIH_CLOUDFLARED_CONFIG', 'CLOUDFLARED_CONFIG'],
    defaultCwdNames: []
  })
});
...
function standardConfigCandidates(spec, options = {}) {
  ...
  if (spec.id === 'cloudflared') {
    if (home) candidates.push(pathImpl.join(home, '.cloudflared', 'config.yml'));
    if (platform === 'win32') {
      const systemRoot = env.SystemRoot || env.SYSTEMROOT || 'C:\\Windows';
      candidates.push(pathImpl.join(systemRoot, 'System32', 'config', 'systemprofile', '.cloudflared', 'config.yml'));
      ...
      candidates.push('/etc/cloudflared/config.yml');
      candidates.push('/usr/local/etc/cloudflared/config.yml');
    }
...
function selectEntryTier(spec, entries, sourceForEntry, options = {}) {
  ...
  const authoritative = entries.length > 0 && (
    spec.id !== 'cloudflared'
    || entries.some((entry) => entryDeclaresConfig(spec, entry, options))
  );
...
function selectConfig(spec, matchingProcesses, matchingStartup, options = {}) {
  const tokenManagedCloudflared = spec.id === 'cloudflared'
    && matchingProcesses.concat(matchingStartup).some((entry) => (
      extractFlagValues(entry, ['--token'], spec.id, options).present
    ));
  if (tokenManagedCloudflared) {
    return { path: '', source: '', count: 0, ambiguous: false, state: 'token-managed' };
  }

...
function encodeSSLink(node) {
  ...
  let link = `ss://${userinfoB64}@${formatServerHost(node.server)}:${node.port}`;
  if (node.plugin) {
    let pluginOptions = node.pluginOpts || '';
    if (pluginOptions && typeof pluginOptions === 'object' && !Array.isArray(pluginOptions)) {
      pluginOptions = Object.entries(pluginOptions).map(([key, value]) => (
        value === true ? key : `${key}=${String(value)}`
      )).join(';');
    }
    link += `/?plugin=${encodeURIComponent(node.plugin + (pluginOptions ? `;${pluginOptions}` : ''))}`;
  }
  if (node.name) {

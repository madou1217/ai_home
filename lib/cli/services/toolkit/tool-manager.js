...
const TOOLKIT_TOOL_CATEGORIES = Object.freeze([
  ...
  Object.freeze({
    id: 'network-access',
    label: '网络接入与隧道',
    description: '管理反向隧道和 Cloudflare Edge Tunnel。'
  })
]);
...
const TOOL_DEFINITIONS = Object.freeze([
  ...
  Object.freeze({
    ...
    capabilities: ['detect', 'version', 'config-edit'], runtimeInspectable: true,
    config: { name: 'frps.toml', format: 'toml' }
  }),
  Object.freeze({
    ...
    capabilities: ['detect', 'version', 'config-edit', 'config-validate'], runtimeInspectable: true,
    config: { name: 'config.yml', format: 'yaml' }
  })
]);
...
function listManagedTools(options = {}) {
  ...
  const tools = TOOL_DEFINITIONS.map((definition) => {
    ...
    const executablePath = resolvedCommand

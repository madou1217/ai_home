...
function createUsageAccountRuntimeService(options = {}) {
  ...
  function hasTrustedUsageRemaining(cache) {
    return getUsageRemainingPercentValues(cache).length > 0;
  }
  ...
  function syncExhaustedStateFromUsage(cliName, id) {
    ...
    if (!cache || !hasTrustedUsageRemaining(cache)) {
  ...
  function refreshIndexedStateForAccount(cliName, id, refreshAccountOptions = {}) {
    ...
    if (configured && !apiKeyMode && isUsageManagedCli(cliName)) {
      if (!cache || !hasTrustedUsageRemaining(cache)) {
        clearExhausted(cliName, accountId);
      } else if (isUsageSnapshotExhausted(cache)) {
        markExhaustedFromUsage(cliName, accountId);
      } else {
        clearExhausted(cliName, accountId);
      }
    }
    const exhausted = hasTrustedUsageRemaining(cache) ? isExhausted(cliName, accountId) : false;

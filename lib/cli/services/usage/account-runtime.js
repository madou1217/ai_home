...
function createUsageAccountRuntimeService(options = {}) {
  ...
  function refreshIndexedStateForAccount(cliName, id, refreshAccountOptions = {}) {
    ...
    if (accountStateService && typeof accountStateService.syncAccountBaseState === 'function') {
      accountStateService.syncAccountBaseState(cliName, accountId, {
    ...
    clearRecoverableAgyAuthInvalidBlock({
      provider: cliName,
      accountId,
      runtimeStatus: deriveRuntimeStatus(getIndexedAccountState(cliName, accountId)),
  ...
  function refreshStaleIndexedAccountStates(cliName, limit = usageIndexBgRefreshLimit) {
    ...
    const staleIds = accountQueryService && typeof accountQueryService.listStaleAccountIds === 'function'
      ? accountQueryService.listStaleAccountIds(cliName, staleBefore, limit)
      : [];
    staleIds.forEach((id) => {
  ...
  function getToolAccountIds(cliName) {
    ...
    if (accountStateService && typeof accountStateService.pruneMissing === 'function') {
      accountStateService.pruneMissing(cliName, fsIds);
    }
    return fsIds;
  }

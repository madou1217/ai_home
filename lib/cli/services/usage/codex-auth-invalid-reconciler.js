...
// A rejected refresh grant does not invalidate an access token that is still
// within its signed lifetime.  This is common while an OpenAI subscription is
// being changed: the refresh endpoint can briefly reject the old grant even
// though the existing bearer remains usable.  Keep the account schedulable in
// that window and let the next normal refresh attempt observe the new plan.
function hasUsableAccessToken(authJson, nowMs = Date.now()) {
  const accessToken = getAccessToken(authJson);
  if (!accessToken) return false;
  const expiresAt = resolveTokenExpiresAt(authJson);
  return Number.isFinite(expiresAt) && expiresAt > Number(nowMs) + 30_000;
}
...
function createCodexAuthInvalidReconciler(options = {}) {
  ...
  function clearRuntimeBlock(accountRef, evidence = 'token_refresh_success') {
  ...
  async function refreshOrRetain(job) {
    ...
    if (isSessionInvalidRefreshFailure(result)) {
      if (hasUsableAccessToken(authJson)) {
        clearRuntimeBlock(job.accountRef, 'access_token_still_valid');
        return {
          action: 'kept',
          accountRef: job.accountRef,
          reason: 'refresh_rejected_access_token_still_valid'
        };
      }

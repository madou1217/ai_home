...
// 把桌面端套餐统计（GetSubscriptionStats，见 lib/server/kimi-plan-stats.js）折算成
// Remaining 条目并入快照：月度订阅总量 -> bucket=monthly 的 30day 窗口条目；
// Gift 余额 -> category='gift' 条目（不计入 min(remainingPct)，Gift 用尽≠账号不可用，
// 过滤发生在 lib/account/usage-remaining.js）。订阅品牌档（Allegretto 等）同时覆盖
// planName 并落到 account.planSubscription（账号列展示套餐名 + 有效期）。
function mergeDesktopPlanStatsIntoSnapshot(snapshot, planStats, capturedAt) {
  if (!snapshot || !planStats || typeof planStats !== 'object') return snapshot;
  const account = snapshot.account && typeof snapshot.account === 'object' ? { ...snapshot.account } : {};
  const total = planStats.total;
  if (total && Number.isFinite(Number(total.usedRatio))) {
    const usedRatio = Math.max(0, Math.min(1, Number(total.usedRatio)));
    const resetAtMs = Number(total.resetAtMs) || 0;
    snapshot.entries.push({
      bucket: 'monthly',
      windowMinutes: 43200,
      window: 'month',
      remainingPct: Math.round((1 - usedRatio) * 1000) / 10,
      resetIn: resetAtMs ? formatResetInFromIso(new Date(resetAtMs).toISOString(), capturedAt) : '',
      resetAtMs
    });
  }
  const gifts = Array.isArray(planStats.gifts) ? planStats.gifts : [];
  for (const gift of gifts) {
    if (!gift || !Number.isFinite(Number(gift.usedRatio))) continue;
    const usedRatio = Math.max(0, Math.min(1, Number(gift.usedRatio)));
    snapshot.entries.push({
      bucket: String(gift.name || '').trim() || 'Gift',
      windowMinutes: 0,
      window: 'gift',
      remainingPct: Math.round((1 - usedRatio) * 1000) / 10,
      resetIn: '',
      resetAtMs: Number(gift.expireAtMs) || 0,
      category: 'gift'
    });
  }
  const plan = planStats.plan && typeof planStats.plan === 'object' ? planStats.plan : null;
  if (plan) {
    const name = String(plan.name || '').trim();
    if (name) account.planName = name;
    account.planSubscription = {
      name,
      status: String(plan.status || '').trim(),
      validUntilMs: Number(plan.validUntilMs) || 0,
      resetAtMs: Number(plan.resetAtMs) || 0
    };
  }
...
function createKimiQuotaProbe(options = {}) {
  ...
  // 桌面端套餐统计是可选增强：账号托管过 desktopSession 时才并入，失败不影响主快照。
  const fetchPlanStats = options.fetchPlanStats !== undefined
    ? options.fetchPlanStats
    : require('../../../server/kimi-plan-stats').fetchKimiPlanStats;
  const proxyOptions = {
  ...
  async function probe(accountRef, probeTimeoutMs) {
    ...
    try {
      ...
      if (nativeAuth.desktopSession && typeof fetchPlanStats === 'function') {
        try {
          const planResult = await fetchPlanStats({
            fs,
            aiHomeDir,
            accountRef,
            timeoutMs,
            ...
            noProxy: proxyOptions.noProxy
          });
          if (planResult && planResult.ok && planResult.stats) {
            mergeDesktopPlanStatsIntoSnapshot(snapshot, planResult.stats, now());
          }
        } catch (_error) {
          // 桌面端套餐统计失败不影响 CLI OAuth 主快照。
        }
      }
...
module.exports = {
  ...
  __private: {
    ...
    mergeDesktopPlanStatsIntoSnapshot
  }
};

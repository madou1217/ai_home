...
function createKimiQuotaProbe(options = {}) {
  ...
  async function resolveAccessToken(accountRef, credentials, probeTimeoutMs) {
  ...
  async function probeKimiUserInfo(baseUrl, accessToken, timeoutMs, deviceHeaders) {
    try {
  ...
  async function probe(accountRef, probeTimeoutMs) {
    ...
    const tokenResult = await resolveAccessToken(accountRef, credentials, timeoutMs);
    ...
    try {
      ...
      const payload = await res.json().catch(() => null);
      const identity = await probeKimiUserInfo(baseUrl, accessToken, timeoutMs, deviceHeaders);

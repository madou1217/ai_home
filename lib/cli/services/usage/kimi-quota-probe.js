...
function toUsageEntry(raw, fallbackName, windowMinutes, capturedAt) {
  ...
  const remainingPct = Number.isFinite(limit) && limit > 0
    ? Math.max(0, Math.min(100, (1 - (Number.isFinite(used) ? used : 0) / limit) * 100))
    : null;
  ...
  return {
    bucket: String(fallbackName || raw.name || '') || window,
    windowMinutes,
    window,
    remainingPct,
...
function createKimiQuotaProbe(options = {}) {
  ...
  async function resolveAccessToken(accountRef, env, credentials, probeTimeoutMs) {
    const accessToken = String(credentials.access_token || credentials.accessToken || '').trim();
    const expiresAtMs = Number(credentials.expires_at) > 0
      ? Number(credentials.expires_at) * 1000
      : null;
    ...
    const refreshed = await refreshKimiAccessToken(refreshAccount, { force: !accessToken }, {
      fs,
      aiHomeDir,
      fetchWithTimeout,
  ...
  async function probeKimiUserInfo(baseUrl, accessToken, timeoutMs) {
    try {
  ...
  async function probe(accountRef, probeTimeoutMs) {
    ...
    if (String(env.MOONSHOT_API_KEY || '').trim()) {
      // API Key 模式没有托管配额端点，Remaining 对这类账号不适用。
      return { error: 'api_key_mode_not_applicable' };
    }
    const credentials = record.nativeAuth && record.nativeAuth.credentials;
    if (!credentials || typeof credentials !== 'object') return { error: 'missing_oauth_credentials' };

    const baseUrl = String(env.KIMI_CODE_BASE_URL || '').trim().replace(/\/+$/, '')
      || DEFAULT_KIMI_CODE_BASE_URL;
    const tokenResult = await resolveAccessToken(accountRef, env, credentials, timeoutMs);
    if (!tokenResult.accessToken) return { error: tokenResult.error || 'token_unavailable', auth: true };

    try {
      const res = await fetchWithTimeout(`${baseUrl}/usages`, {
        headers: {
      ...
      const payload = await res.json().catch(() => null);
      const identity = await probeKimiUserInfo(baseUrl, tokenResult.accessToken, timeoutMs);

...
function createModelUsageAccountingService(options = {}) {
  ...
  async function printScan(parsed) {
    ...
    const result = modelUsageService.scan({ provider });
    if (parsed.json) {
      log(JSON.stringify({ ok: true, command: 'scan', result }, null, 2));
      return;
    }
    log(`\x1b[36m[aih]\x1b[0m 模型用量扫描完成: files=${result.files}, records=${result.records}, prompts=${result.prompts}, skipped=${result.skipped}`);
    Object.entries(result.providers || {}).forEach(([name, item]) => {
      const reason = item.reason ? ` (${item.reason})` : '';
      log(`  - ${name}: files=${item.files}, records=${item.records}, prompts=${item.prompts}, skipped=${item.skipped}${reason}`);
    });
  }

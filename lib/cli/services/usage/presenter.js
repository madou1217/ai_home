...
const { formatCreditUsageLines } = require('./credit-format');
...
const { resolveAccountPlanLabel } = require('../profile/account-plan-badge');
...
function createUsagePresenterService(options = {}) {
  ...
  const resolveAccountScopeOverride = options.resolveAccountScope;
  ...
  function resolveAccountScope(provider, selector) {
    if (typeof resolveAccountScopeOverride === 'function') {
      const resolved = resolveAccountScopeOverride(provider, selector);
      return resolved
        && resolved.provider === provider
        && isAccountRef(resolved.accountRef)
        ? resolved
        : null;
    }
    const value = String(selector || '').trim();
    const account = isAccountRef(value)
      ? resolveAccountRef(fs, aiHomeDir, value, { bestEffort: true })
      : resolveAccountRefByCliId(fs, aiHomeDir, provider, value, { bestEffort: true });
    return account && account.provider === provider ? account : null;
  }
  ...
  function syncIndexedBaseState(accountRef, cliName, state) {
    if (!accountStateService || typeof accountStateService.syncAccountBaseState !== 'function') return false;
    return accountStateService.syncAccountBaseState(accountRef, cliName, state);
  }

  function isApiKeyAccount(provider, accountRef) {
    const credentials = readAccountCredentials(fs, aiHomeDir, accountRef);
    if (provider === 'codex') return Boolean(String(credentials.OPENAI_API_KEY || '').trim());
    if (provider === 'claude') {
      return Boolean(String(credentials.ANTHROPIC_API_KEY || credentials.ANTHROPIC_AUTH_TOKEN || '').trim());
    }
    if (provider === 'gemini') {
      return Boolean(String(credentials.GEMINI_API_KEY || credentials.GOOGLE_API_KEY || '').trim());
    }
    if (provider === 'kimi') return Boolean(String(credentials.MOONSHOT_API_KEY || '').trim());
    return false;
  }
  ...
  function formatUsageLabel(cliName, id, accountName) {
    ...
    const scope = resolveAccountScope(cliName, id);
    if (!scope) return '';
    const cache = readUsageCache(cliName, scope.accountRef);
  ...
  function formatAccountPlanBadge(cliName, id, formatOptions = {}) {
    const scope = resolveAccountScope(cliName, id);
    if (!scope) return '';
    const apiKeyMode = typeof formatOptions.apiKeyMode === 'boolean'
      ? formatOptions.apiKeyMode
      : isApiKeyAccount(cliName, scope.accountRef);
    if (apiKeyMode) return '';
    const cache = readUsageCache(cliName, scope.accountRef);
    const account = cache && cache.account && typeof cache.account === 'object'
      ? cache.account
      : null;
    const label = resolveAccountPlanLabel({
      provider: cliName,
      planType: account && account.planType,
      rateLimitTier: account && account.rateLimitTier
    });
  ...
  function formatCodexProbeErrorHint(id) {
    ...
    if (lower.includes('panic') || lower.includes('null object') || lower.includes('app_server_exit_')) {
      const installCommand = processObj.platform === 'win32'
        ? 'irm https://chatgpt.com/codex/install.ps1 | iex'
        : 'curl -fsSL https://chatgpt.com/codex/install.sh | sh';
      return `Codex app-server crashed while probing usage. Upgrade codex and retry (\`${installCommand}\`).`;
    }
  ...
  function formatUsageSnapshotLines(cache) {
    ...
    const creditLines = formatCreditUsageLines(cache);
    return creditLines || [JSON.stringify(cache)];
  }
  ...
  function printAgyUsagePreflight(id) {
    ...
    const scope = resolveAccountScope('agy', id);
    const report = scope ? buildAgyUsagePreflight('agy', scope.accountRef) : null;
    if (!report) {
    ...
    console.log(`  - runtimeDir: ${report.runtimeDir}`);
    console.log(`  - configDir: ${report.configDir}`);
    console.log(`  - nativeAuth: ${formatBoolean(report.nativeAuthPresent)}`);
    console.log(`  - accessToken: nativeAuth=${formatBoolean(report.nativeAccessTokenPresent)}, env=${formatBoolean(report.envAccessTokenPresent)}, selected=${report.selectedTokenSource || 'none'}`);
  ...
  function formatAgyPreflightRow(report) {
    const pieces = [
      `token=${formatBoolean(report.nativeAuthPresent && report.nativeAccessTokenPresent)}`,
  ...
  function printAllAgyUsagePreflights() {
    ...
    const reports = ids
      .map((id) => {
        const scope = resolveAccountScope('agy', id);
        const report = scope ? buildAgyUsagePreflight('agy', scope.accountRef) : null;
        return report ? { ...report, cliAccountId: id } : null;
      })
      .filter(Boolean);
    ...
    reports.forEach((report) => {
      const hasToken = !!(report.nativeAuthPresent && report.nativeAccessTokenPresent);
      ...
      if (hasToken && report.refreshTokenPresent && !report.refreshDue) {
        directReady += 1;
        if (!recommended) recommended = report.cliAccountId;
      }
      console.log(`  - Account ID ${report.cliAccountId}: ${formatAgyPreflightRow(report)}`);
    });
  ...
  function printUsageSnapshot(cliName, id, queryOptions = {}) {
    ...
    const scope = resolveAccountScope(cliName, id);
    if (!scope) {
      console.log(`\x1b[90m[aih]\x1b[0m Account ID ${id} was not found.`);
      return;
    }
    const { accountName } = checkStatus(cliName, scope.accountRef);
    if (isApiKeyAccount(cliName, scope.accountRef)) {
    ...
    let cache = readUsageCache(cliName, scope.accountRef);
    cache = ensureUsageSnapshot(cliName, scope.accountRef, cache, { forceRefresh: noCache });
    if (!cache) {
      console.log(`\x1b[90m[aih]\x1b[0m No cached usage snapshot for ${cliName} Account ID ${id}.`);
      const hint = getUsageNoSnapshotHint(cliName, scope.accountRef);
      if (hint) {
    ...
    const noNumericHint = getCodexNoNumericUsageHint(cache, scope.accountRef);
    if (noNumericHint) {
  ...
  async function printUsageSnapshotAsync(cliName, id, queryOptions = {}) {
    ...
    if (typeof ensureUsageSnapshotAsync === 'function') {
      cache = await ensureUsageSnapshotAsync(cliName, scope.accountRef, cache, { forceRefresh: noCache });
    } else {
      cache = ensureUsageSnapshot(cliName, scope.accountRef, cache, { forceRefresh: noCache });
    }
  ...
  function buildUsageProbePayload(cliName, id) {
    const scope = resolveAccountScope(cliName, id);
    if (!scope) return { cliName, id, status: 'pending' };
    const { configured, accountName } = checkStatus(cliName, scope.accountRef);
    if (!configured) {
    ...
    let cache = readUsageCache(cliName, scope.accountRef);
    cache = ensureUsageSnapshot(cliName, scope.accountRef, cache);
    ...
    if (!cache) {
      return {
        ...
        status: 'no_snapshot',
        label,
        hint: getUsageNoSnapshotHint(cliName, scope.accountRef)
      };
    }
  ...
  async function buildUsageProbePayloadAsync(cliName, id, probeOptions = {}) {
    ...
    let cache = readUsageCache(cliName, scope.accountRef);
    if (typeof ensureUsageSnapshotAsync === 'function') {
      cache = await ensureUsageSnapshotAsync(cliName, scope.accountRef, cache, probeOptions);
    } else {
      cache = ensureUsageSnapshot(cliName, scope.accountRef, cache);
    }
    const label = accountName && accountName !== 'Unknown' ? accountName : 'OAuth';
  ...
  async function printAllUsageSnapshots(cliName, scanOptions = {}) {
    ...
    const worker = async () => {
      while (true) {
        ...
        const accountRef = scope.accountRef;
        ...
        try {
          ...
          const operationalStatus = resolvePersistedOperationalStatus(accountRef);
          if (payload.status === 'pending') {
            syncIndexedBaseState(accountRef, cliName, {
          ...
          if (payload.status === 'probe_error') {
            ...
            incrementCount(probeErrorCounts, normalizeProbeErrorForSummary(payload.error || readProbeErrorForSummary(cliName, accountRef)));
          ...
          if (payload.status !== 'ok') {
            const quotaState = typeof getAccountQuotaState === 'function'
              ? getAccountQuotaState(cliName, accountRef, { refreshSnapshot: false })
              : null;
            syncIndexedBaseState(accountRef, cliName, {
              status: operationalStatus,
              configured: true,
            ...
            skippedUnknown += 1;
            incrementCount(probeErrorCounts, normalizeProbeErrorForSummary(readProbeErrorForSummary(cliName, accountRef)));

...
function createUsageSnapshotService(options = {}) {
  ...
  // CodeBuddy 家族四支（codebuddy / codebuddycn / workbuddy / workbuddycn）共用一个探针：
  // 同地区两支是同一个账号、同一个余额接口，只是 endpoint 与共享凭据文件不同，
  // 由 probe 内部按 provider 解析（见 lib/account/codebuddy-billing.js）。
  const codebuddyQuotaProbe = createCodebuddyQuotaProbe({
    fs,
    aiHomeDir,
    readAccountCredentialRecord,
    resolveAccountRef,
  ...
  const codexDomain = createCodexUsageSnapshotDomain(
    { fs, path, spawn, spawnSync, processObj, aiHomeDir, resolveCliPath, usageSnapshotSchemaVersion, usageSourceCodex, getProfileDir, writeUsageCache, accountArtifactHooks, resolveAccountEgressRequestOptions, accountEgressDeps },
  ...
  async function refreshAgyUsageSnapshotAsync(cliName, id) {
    ...
    const timeoutMs = Number.isFinite(timeoutMsRaw)
      ...
      : 8000;
    const accountRef = String(id || '').trim();
    const accountRequestOptions = await resolveProviderAccountEgressRequestOptions({
      account: { provider: 'agy', accountRef },
      provider: 'agy',
      options: {
        proxyUrl: processObj.env.AIH_SERVER_PROXY_URL || processObj.env.HTTPS_PROXY || processObj.env.HTTP_PROXY,
        noProxy: processObj.env.AIH_SERVER_NO_PROXY || processObj.env.NO_PROXY || processObj.env.no_proxy
    ...
    const quotaFetchImpl = accountRequestOptions.bound
      ? (url, init) => fetchWithTimeoutImpl(url, init, timeoutMs, {
          proxyUrl: accountRequestOptions.options.proxyUrl,
          noProxy: accountRequestOptions.options.noProxy
        })
      : fetchWithImpl;
...
`;
  ...
  // 家族四支共用一个探针与一份缓存读判定（kind/source 相同），只有 cliName 不同——
  // cliName 决定 cache 的存放键与 trusted 校验分支。
  async function refreshCodebuddyUsageSnapshotAsync(cliName, id, probeTimeoutMs) {
    const accountRef = String(id || '').trim();
    if (!isAccountRef(accountRef)) return null;
    const result = await codebuddyQuotaProbe.probe(accountRef, probeTimeoutMs);
    if (!result || !result.snapshot) {
      setProbeError(cliName, accountRef, (result && result.error) || 'probe_failed');
      return null;
    }
    writeUsageCache(cliName, accountRef, result.snapshot);
    setProbeError(cliName, accountRef, '');
    clearRuntimeStateForVerifiedSnapshot(cliName, accountRef, result.snapshot);
    return result.snapshot;
  }

  async function probeCodebuddyUsageAsyncWithCachePolicy(cliName, id, cache, refreshOptions = {}) {
    const forceRefresh = !!(refreshOptions && refreshOptions.forceRefresh);
    if (!forceRefresh && !isUsageSnapshotStale(cache)) return cache;
    const refreshed = await refreshCodebuddyUsageSnapshotAsync(
      cliName,
      id,
      Number(refreshOptions && refreshOptions.probeTimeoutMs) || null
    );
    return refreshed || cache || null;
  }
  ...
  async function ensureUsageSnapshotAsync(cliName, id, cache, refreshOptions = {}) {
    ...
    if (CODEBUDDY_FAMILY_PROVIDERS.includes(cliName)) {
      return probeCodebuddyUsageAsyncWithCachePolicy(cliName, id, cache, refreshOptions);
    }
    if (cliName === 'codex') {

...
`;
    ...
    try {
      ...
      if (!parsedOutput || parsedOutput.ok !== true) {
        setProbeError(cliName, id, parsedOutput && (parsedOutput.error || parsedOutput.detail) ? `${parsedOutput.error || ''} ${parsedOutput.detail || ''}`.trim() : 'probe_not_ok');
        return null;
      }
...
`;
    ...
    const envOverrides = {
      ...
      AIH_CODEX_SQLITE_HOME: resolveCodexSqliteHome({ path, profileDir: sandboxDir }),
      ...
      try {
        child = spawnProcess(codexBin, ['app-server', '--listen', 'stdio://'], {
          ...
          env: {
            ...
            ...(codexSqliteHome ? { CODEX_SQLITE_HOME: codexSqliteHome } : {}),
            HOME: sandboxDir,
            USERPROFILE: sandboxDir
          },
          stdio: ['pipe', 'pipe', 'pipe'],

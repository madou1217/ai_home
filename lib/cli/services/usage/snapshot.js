...
const { buildProviderRuntimeEnv } = require('../ai-cli/provider-runtime-env');
...
function createUsageSnapshotService(options = {}) {
  ...
  function clearRecoveredRuntimeState(cliName, id, snapshot, evidence = 'verified_usage_success') {
    ...
    if (!cliName || !/^\d+$/.test(accountId)) return false;
    const baseState = {
      configured: true,
      apiKeyMode: false,
  ...
  function markCodexAuthInvalidFromUsageProbe(cliName, id, reason) {
    ...
    return accountStateService.recordRuntimeFailure(
      ...
      buildAuthInvalidRuntimeState(reason || 'auth_invalid_reauth_required'),
      {
        configured: true,
        apiKeyMode: false,
        displayName: String(snapshotAccount.email || snapshotAccount.accountId || '').trim()
      }
    );
  }
  ...
  function getIndexedCodexRuntimeStatus(cliName, id) {
    ...
    try {
      ...
      const state = index.getAccountState(cliName, id);
  ...
  function readProfileEnvJson(profileDir) {
    return readJsonFileSafe(path.join(profileDir, '.aih_env.json'));
  }

  function readAgyOAuthTokenPayload(configDir) {
    return readJsonFileSafe(path.join(configDir, 'antigravity-oauth-token'));
  }
  ...
  function readAgyAccessTokenForSandbox(cliName, id, options = {}) {
    ...
    const env = readProfileEnvJson(profileDir);
    const envToken = sanitizeAccessToken(env && (env.AGY_ACCESS_TOKEN || env.GOOGLE_OAUTH_ACCESS_TOKEN) || '');
    const configDir = getToolConfigDir(cliName, id);
    const oauth = readAgyOAuthTokenPayload(configDir);
    ...
    return {
      accessToken,
      tokenSource: accessToken === oauthToken ? 'antigravity-oauth-token' : '.aih_env.json',
      configDir,
      profileDir,
      email: readAgyEmailForSandbox(configDir),
      tokenExpiresAt: parseResetAtMsFromIso(token.expiry)
    };
  }

  function readAgyEmailForSandbox(configDir) {
    const emailPath = path.join(configDir, 'email.cache');
    if (!fs.existsSync(emailPath)) return '';
    try {
      return String(fs.readFileSync(emailPath, 'utf8') || '').trim();
    } catch (_error) {
      return '';
    }
  }
  ...
  function buildAgyUsagePreflight(cliName, id) {
    ...
    return {
      ...
      profileDir,
      configDir,
      tokenFileExists: !!oauth,
      envAccessTokenPresent: !!envToken,
      oauthAccessTokenPresent: !!oauthToken,
      refreshTokenPresent: !!refreshToken,
      selectedTokenSource: auth && auth.tokenSource || '',
      emailPresent: !!readAgyEmailForSandbox(configDir),
        ...
        if (refreshedAuth && refreshedAuth.accessToken && refreshedAuth.accessToken !== auth.accessToken) {
          try {
            const snapshot = await fetchAgyCodeAssistQuotaSnapshot({
              ...
              profileDir: refreshedAuth.profileDir,
  ...
  function refreshGeminiUsageSnapshot(cliName, id) {
    ...
    const oauthPath = path.join(sandboxDir, '.gemini', 'oauth_creds.json');
    if (!fs.existsSync(oauthPath)) return null;

    let oauthPayload;
    try {
      oauthPayload = JSON.parse(fs.readFileSync(oauthPath, 'utf8'));
    } catch (_err) {
      return null;
    }
...
`;
    ...
    const codexSqliteHome = resolveCodexSqliteHome({ path, profileDir: sandboxDir });
  ...
  async function refreshCodexTokenForSandbox(cliName, id) {
    ...
    const readAuth = () => {
      try {
        ...
        return { authJson: null, tokens: null };
      }
    };
  ...
  function getClaudeUsageAuthForSandbox(cliName, id) {
    ...
    if (fs.existsSync(credentialsPath)) {
      ...
      oauth = payload && (payload.claudeAiOauth || payload.claude_ai_oauth);
    }
    // macOS: a native `claude` login keeps OAuth tokens in the login Keychain
    // instead of .credentials.json. Fall back to it so usage probes can run.
    if (!oauth || !(oauth.accessToken || oauth.access_token)) {
      const keychainCreds = readClaudeKeychainCredentials({ configDir: claudeConfigDir });
      const keychainOauth = keychainCreds && (keychainCreds.claudeAiOauth || keychainCreds.claude_ai_oauth);
      if (keychainOauth && (keychainOauth.accessToken || keychainOauth.access_token)) {
        oauth = keychainOauth;
      }
    }
    {

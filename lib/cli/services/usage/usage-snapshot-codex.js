...
module.exports = function createCodexUsageSnapshotDomain(deps, helpers) {
  ...
  function normalizeCodexResetCreditSummary(value) {
  ...
  function parseCodexRateLimits(rateLimits, capturedAt, source, rateLimitResetCredits) {
    ...
    const resetCredits = normalizeCodexResetCreditSummary(rateLimitResetCredits);
    return {
  ...
  function parseCodexAccountFallback(account, capturedAt, source, authJson) {
    const snapshot = buildCodexMetadataFallbackSnapshot({
      schemaVersion: usageSnapshotSchemaVersion,
      capturedAt: capturedAt || Date.now(),
      source: source || usageSourceCodex,
    ...
    const livePlanType = String(account && (account.planType || account.plan_type) || '').trim();
    if (!livePlanType || !snapshot.account) return snapshot;
    snapshot.account = { ...snapshot.account, planType: livePlanType };
    const email = String(snapshot.account.email || '').trim();
    if (Array.isArray(snapshot.entries) && snapshot.entries.length > 0) {
      snapshot.entries = snapshot.entries.map((entry, index) => index === 0
        ? { ...entry, window: [`plan:${livePlanType}`, email].filter(Boolean).join(' ') }
        : entry);
    }
    return snapshot;
  }
  ...
  function mergeCodexSnapshotAccount(snapshot, account, authJson) {
    ...
    // `snapshot.account` and the provider account payload are live responses.
    // Keep their plan tier ahead of the JWT metadata cached at login time so a
    // subscription change (for example Plus -> Pro) is reflected immediately.
    const livePlanType = String(
      (account && (account.planType || account.plan_type))
      || (snapshot.account && snapshot.account.planType)
      || ''
    ).trim();
    snapshot.account = {
      ...(snapshot.account || {}),
      ...preferredAccount,
      planType: livePlanType || preferredAccount.planType || ''
    };
    return snapshot;
  }
...
function startCodexAppServer() {
  if (process.platform === 'win32') {
    ...
    // 本脚本是独立探针、无法 require 仓库模块；此处必须带
    // windowsVerbatimArguments，否则 libuv 把 line 里的 " 转义成 \" 后
    // cmd.exe 会解析失败（知识归属见 lib/runtime/windows-cmd-launch.js）。
  ...
  while ((idx = stdoutBuf.indexOf('\\n')) >= 0) {
    ...
    if (msg && msg.id === 'aih_rate') {
      if (msg.result && msg.result.rateLimits) {
        finish({
          ok: true,
          rateLimits: msg.result.rateLimits,
          rateLimitResetCredits: msg.result.rateLimitResetCredits
        });
      } else {
...
`;
    ...
    try {
      ...
      if (parsedOutput.rateLimits) {
        parsed = parseCodexRateLimits(
          parsedOutput.rateLimits,
          Date.now(),
          usageSourceCodex,
          parsedOutput.rateLimitResetCredits
        );
  ...
  function getCodexUsageProxyOptions() {
    return {
  ...
  function resolveCodexUsageRequestOptions(accountRef) {
    return resolveProviderAccountEgressRequestOptions({
      account: { provider: 'codex', accountRef },
      provider: 'codex',
      options: getCodexUsageProxyOptions(),
      deps: {
        fs,
        aiHomeDir,
        processObj,
  ...
  async function fetchWithOptionalCustomFetch(url, init, timeoutMs, requestOptionsResult = null) {
    ...
    if (requestOptionsResult?.bound && typeof fetchWithTimeoutImpl === 'function') {
    ...
    return fetchWithTimeoutImpl(url, init, timeoutMs, requestOptions ? {
      proxyUrl: requestOptions.proxyUrl,
      noProxy: requestOptions.noProxy
    } : undefined);
  }
  ...
  async function refreshCodexTokenForSandbox(cliName, id) {
    ...
    const refreshWithCurrentAuth = async () => {
      ...
      const requestOptionsResult = await resolveCodexUsageRequestOptions(accountRef);
      if (!requestOptionsResult?.ok || !requestOptionsResult.options) return false;
      ...
      try {
        ...
        }, timeoutMs, requestOptionsResult);
  ...
  async function fetchCodexDirectUsage(url, init, timeoutMs, requestOptionsResult) {
    return fetchWithOptionalCustomFetch(url, init, timeoutMs, requestOptionsResult);
  }
  ...
  function extractResetCreditsFromDirectPayload(payload) {
    if (!payload || typeof payload !== 'object') return null;
    ...
    return payload.rateLimitResetCredits
      ...
      || (result && (result.rateLimitResetCredits || result.rate_limit_reset_credits))
      || null;
  }
  ...
  async function refreshCodexUsageSnapshotFromDirectApiAsync(cliName, id, timeoutOverrideMs = null) {
    ...
    const requestOptionsResult = await resolveCodexUsageRequestOptions(String(id || '').trim());
    if (!requestOptionsResult?.ok || !requestOptionsResult.options) {
      setProbeError(cliName, id, 'account_egress_unavailable');
      return null;
    }
    ...
    try {
      ...
      return buildCodexSnapshotFromProbePayload(cliName, id, {
        ok: true,
        rateLimits,
        rateLimitResetCredits: extractResetCreditsFromDirectPayload(payload),

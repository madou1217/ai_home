...
const CAPABILITY_ALIASES = Object.freeze({
  ...
  fabricRuntime: 'fabric_runtime',
  gatewayProfile: 'gateway_profile',
  sessionHistory: 'session_history',
  accountSessionStore: 'account_session_store',
  usageScan: 'usage_scan'
});
...
function normalizeDefinition(definition = {}) {
  ...
  // family/site 缺省时按"单站产品"补：family == id、site == global。
  // 合同生成器已保证显式值存在，这里的缺省只防御手工构造的合同切片。
  const family = normalizeProviderId(definition.family) || id;
  const site = String(definition.site || '').trim().toLowerCase() === 'cn' ? 'cn' : 'global';
  return deepFreeze({
    id,
    family,
    site,
...
// getProviderHeadlessConfig 返回原生 CLI 的非交互调用声明副本；null 表示该
// Provider 没有 headless 入口（例如 gemini），只能走 PTY 交互路径。
function getProviderHeadlessConfig(providerRaw) {
  const definition = getProviderDefinition(providerRaw);
  return definition && definition.cli && definition.cli.headless
    ? cloneJson(definition.cli.headless)
    : null;
}
...
/** 站点展示名；未知站点返回空串（调用方据此不渲染标记）。 */
const SITE_LABELS = Object.freeze({ global: '国际站', cn: '国内站' });

// getProviderFamily 读取 Provider 的产品族标识；未知 Provider 回退为自身。
function getProviderFamily(providerRaw) {
  const definition = PROVIDER_DEFINITIONS_BY_ID[normalizeProviderId(providerRaw)];
  return definition ? definition.family : normalizeProviderId(providerRaw);
}

// getProviderSite 读取 Provider 归属的站点。
function getProviderSite(providerRaw) {
  const definition = PROVIDER_DEFINITIONS_BY_ID[normalizeProviderId(providerRaw)];
  return definition ? definition.site : 'global';
}

// getProviderSiteLabel 返回站点的用户可见名称。
function getProviderSiteLabel(siteRaw) {
  return SITE_LABELS[String(siteRaw || '').trim().toLowerCase()] || '';
}

// listProviderFamilies 返回按产品顺序聚合的产品族，成员按站点顺序（国际站在前）。
function listProviderFamilies() {
  const buckets = new Map();
  PROVIDER_DEFINITIONS.forEach((definition) => {
    if (!buckets.has(definition.family)) buckets.set(definition.family, []);
    buckets.get(definition.family).push(definition);
  });
  return Array.from(buckets.values()).map((definitions) => {
    const providers = definitions.slice().sort((left, right) => (
      (left.site === right.site ? 0 : left.site === 'global' ? -1 : 1)
    ));
    const named = providers.find((definition) => definition.site === 'global') || providers[0];
    return {
      family: named.family,
      label: named.presentation.label,
      providers: providers.map((definition) => definition.id),
      sites: providers.map((definition) => definition.site),
      multiSite: providers.length > 1
    };
  });
}

// getProviderMenuLabel 返回列表/菜单里展示单个 Provider 的名称。
//
// 多站点产品必须带站点，否则同族的两个 Provider 看起来一模一样；单站产品直接用
// 族名，避免出现"Codex · 国际站"这种零信息量的后缀。
function getProviderMenuLabel(providerRaw) {
  const definition = PROVIDER_DEFINITIONS_BY_ID[normalizeProviderId(providerRaw)];
  if (!definition) return getProviderMeta(providerRaw).label;
  const members = listProviderFamilies().find((family) => family.family === definition.family);
  if (!members || !members.multiSite) return definition.presentation.label;
  const siteLabel = getProviderSiteLabel(definition.site);
  return siteLabel ? `${members.label} · ${siteLabel}` : members.label;
}

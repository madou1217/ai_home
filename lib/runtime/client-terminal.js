...
function resolveTerminalExecutable(definition, context = {}) {
  ...
  const localAppData = String(env.LOCALAPPDATA || (hostHomeDir
    ...
    : '')).trim();
  return findFirstExisting((platformConfig.paths || []).map((candidate) => {
...
function buildWindowsTerminalLaunch(command, title, context = {}) {
  ...
  // windowsHide 必须为 false：spawn 的 CREATE_NO_WINDOW 会把 WT 新窗口创建成
  // 隐藏窗口（进程链正常但用户「看不到任何反应」，2026-08-22 对照实验实锤）。
  return {
...
function resolvePackageManager(platform, context = {}) {
  ...
  for (const candidate of candidates) {
    const executable = findOnPath(candidate.names, resolved) || findFirstExisting(
      platform === CLIENT_PLATFORMS.MACOS
        ? ['/opt/homebrew/bin/brew', '/usr/local/bin/brew', `${readHostHome(resolved)}/.homebrew/bin/brew`]
        : platform === CLIENT_PLATFORMS.WINDOWS
          ? [
              path.join(String(env.LOCALAPPDATA || readHostHome(resolved) || 'C:\\Users\\Public'), 'Microsoft', 'WindowsApps', 'winget.exe'),
              path.join(String(env.ProgramFiles || 'C:\\Program Files'), 'WindowsApps', 'winget.exe')
            ]
          : ['/usr/bin/flatpak', '/usr/local/bin/flatpak', `${readHostHome(resolved)}/.local/bin/flatpak`],
      fs
    );
...
const TERMINAL_DEFINITIONS = Object.freeze({
  ...
  [TERMINAL_IDS.WARP]: defineClientTerminalAdapter({
    ...
    platforms: {
      macos: {
        ...
        paths: [
          ...
          '{hostHomeDir}/Applications/Warp.app/Contents/MacOS/stable'
        ]
      },
      windows: {
        ...
        paths: [
          ...
          '{hostHomeDir}/AppData/Local/Programs/Warp/Warp.exe'
        ]
      },
      linux: {
        ...
        paths: [
          ...
          '{hostHomeDir}/.local/bin/warp-terminal'
        ]
      }
    },
    supports: () => true,
    ...
    buildPlans: (context) => {
      if (context.platform === CLIENT_PLATFORMS.LINUX) return [];
      const packageId = context.platform === CLIENT_PLATFORMS.WINDOWS ? 'Warp.Warp' : 'warp';
      return buildPackagePlans(resolvePackageManager(context.platform, context), packageId, 'Warp');
    }
  }),

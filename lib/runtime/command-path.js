...
function selectWindowsProbeResult(lines, env) {
  ...
  return candidates.find((candidate) => supportedExtensions.has(path.win32.extname(candidate).toLowerCase()))
    || candidates[0]
    || '';
}
...
function resolveWithProbe(cmdName, platform, env, spawnSyncImpl, report) {
  if (platform === 'win32') {
    const probe = spawnSyncImpl('where.exe', [cmdName], {
      encoding: 'utf8',
    ...
    const lines = String(probe.stdout || '')
      ...
      .map((line) => line.trim())
      .filter(Boolean);
    const resolved = selectWindowsProbeResult(lines, env);
    if (resolved) {
  ...
  const probe = spawnSyncImpl('sh', ['-lc', `command -v "${safe}"`], {
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'ignore'],
    env
  });
...
function resolveCommandPathDetailed(cmdName, options = {}) {
  ...
  const resolvedByProbe = resolveWithProbe(normalized, platform, env, spawnSyncImpl, report);
  if (resolvedByProbe) {
    return {

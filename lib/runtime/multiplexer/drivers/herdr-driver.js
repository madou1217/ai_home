...
class HerdrDriver extends BaseMultiplexerDriver {
  ...
  install(options = {}) {
    ...
    let result;
    try {
      result = spawnSyncImpl(installPlan.command, installPlan.args, {
    ...
    const refreshed = this.detect({ ...options, spawnSync: spawnSyncImpl });
    return {
  ...
  listSessions(socket, options = {}) {
    ...
    return spawnImpl('herdr', args, { encoding: 'utf8' });
  }
  ...
  spawnHeadlessRun(options = {}) {
    ...
    const result = spawnImpl('herdr', herdrArgv, {
      env: options.env || process.env,
      stdio: 'ignore'
    });
    if (result.status !== 0) {
  ...
  hasRun(socket, options = {}) {
    const spawnImpl = options.spawnSyncImpl || spawnSync;
    try {
      const result = spawnImpl('herdr', ['status', '--session', `${socket}-${RUN_SESSION_NAME}`], { stdio: 'ignore' });
  ...
  killRun(socket, options = {}) {
    const spawnImpl = options.spawnSyncImpl || spawnSync;
    try {
      spawnImpl('herdr', ['kill', '--session', `${socket}-${RUN_SESSION_NAME}`], { stdio: 'ignore' });
  ...
  sendInput(socket, text, options = {}) {
    ...
    try {
      ...
      const sent = spawnImpl('herdr', args, { stdio: 'ignore' });

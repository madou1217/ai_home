...
class HerdrDriver extends BaseMultiplexerDriver {
  ...
  spawnHeadlessRun(options = {}) {
    ...
    // 与 tmux driver 对齐：commandArgv 直接给出 pane 进程数组，跳过 sh -c（Windows 无 sh）。
    const commandArgv = Array.isArray(options.commandArgv) && options.commandArgv.length > 0
      ? options.commandArgv.map(String)
      : null;
    const shellCommand = String(options.shellCommand || '').trim();
    if (!socket || (!commandArgv && !shellCommand)) {

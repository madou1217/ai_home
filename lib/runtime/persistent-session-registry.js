...
// session/cwd/label/forward-args/native-session identity). It never stores credentials, and it does
...
const UNRECOVERABLE_REASONS = Object.freeze([
  ...
  'codex-thread-duplicate'
]);

function normalizeMetadataId(value) {
  const text = String(value || '').trim();
  if (!text || text.length > 512 || text.includes('\0')) return '';
  return text;
}
...
function readEntryFile(filePath, fsImpl, options = {}) {
  try {
...
function normalizeEntry(raw = {}) {
  return {
    ...
    correlationId: normalizeMetadataId(raw.correlationId),
    nativeSessionId: normalizeMetadataId(raw.nativeSessionId),
    supervisorManaged: raw.supervisorManaged === true,
...
function writeEntry(aiHomeDir, rawEntry = {}, options = {}) {
  ...
  if (!isUsableEntry(entry)) {
    if (options.strict === true) {
      const error = new Error('persistent_session_registry_entry_invalid');
      error.code = 'persistent_session_registry_entry_invalid';
      throw error;
    }
    return null;
  }
  ...
  if (!filePath) {
    if (options.strict === true) {
      const error = new Error('persistent_session_registry_path_invalid');
      error.code = 'persistent_session_registry_path_invalid';
      throw error;
    }
    return null;
  }
  try {
    ...
    entry.updatedAt = now;
    entry.correlationId = entry.correlationId || existing.correlationId;
    entry.nativeSessionId = entry.nativeSessionId || existing.nativeSessionId;
...
function bindNativeSession(aiHomeDir, socket, session, nativeSessionId, options = {}) {
  const normalizedNativeSessionId = normalizeMetadataId(nativeSessionId);
  if (!normalizedNativeSessionId) return null;
  const filePath = entryFilePath(aiHomeDir, socket, session);
  if (!filePath) return null;
  const fsImpl = options.fs || fs;
  const existing = normalizeEntry(readEntryFile(filePath, fsImpl, options) || {});
  if (!isUsableEntry(existing)) return null;
  return writeEntry(aiHomeDir, {
    ...existing,
    nativeSessionId: normalizedNativeSessionId
  }, options);
}

function bindNativeSessionByCorrelation(
  aiHomeDir,
  provider,
  correlationId,
  nativeSessionId,
  ...
  const normalizedCorrelationId = normalizeMetadataId(correlationId);
  const normalizedNativeSessionId = normalizeMetadataId(nativeSessionId);
  if (!normalizedProvider || !normalizedCorrelationId || !normalizedNativeSessionId) return null;
  const match = listEntries(aiHomeDir, options).find((entry) => (
    entry.provider === normalizedProvider
    && entry.correlationId === normalizedCorrelationId
  ));
...
function listEntries(aiHomeDir, options = {}) {
  ...
  for (const file of files) {
    ...
    const entry = normalizeEntry(readEntryFile(path.join(dir, file), fsImpl, options) || {});
...
module.exports = {
  ...
  writeEntry,
  bindNativeSession,
  bindNativeSessionByCorrelation,
  touchEntry,

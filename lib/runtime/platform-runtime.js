...
function collectCommandCandidates(commandName, options = {}) {
  ...
  const names = platform === 'win32'
    ? [commandName, `${commandName}.ps1`, `${commandName}.cmd`, `${commandName}.bat`, `${commandName}.exe`]
    : [commandName];
  const candidates = [];
  const seen = new Set();
...
function resolveHighestVersionedCommandPath(commandName, options = {}) {
  ...
  const spawnSyncImpl = options.spawnSyncImpl || spawnSync;
  const versionProbeTimeoutMs = Math.max(
    1000,
    Number(options.versionProbeTimeoutMs)
      || (String(commandName || '').trim() === 'claude' ? 5000 : 2000)
  );
  let best = null;
...
function shouldResolveHighestVersionedCommand(commandName, env) {
  const normalized = String(commandName || '').trim();
  if (normalized === 'codex') {
    return String((env || process.env).AIH_CODEX_RESOLVE_LATEST || '1') !== '0';
  }
  if (normalized === 'claude') {
    return String((env || process.env).AIH_CLAUDE_RESOLVE_LATEST || '1') !== '0';
  }
  if (normalized === 'opencode') {
    return String((env || process.env).AIH_OPENCODE_RESOLVE_LATEST || '1') !== '0';
  }
  return false;
}
...
function resolveCliPath(commandName, options = {}) {
  const normalized = String(commandName || '').trim();
  const env = options.env || process.env;
  if (shouldResolveHighestVersionedCommand(normalized, env)) {
    const latestPath = resolveHighestVersionedCommandPath(normalized, options);
...
module.exports = {
  ...
  shouldResolveHighestVersionedCommand,
  resolveCliPath
};

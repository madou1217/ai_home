...
//   ~/Library/Application Support/CodeBuddyExtension/Data/Public/auth/<authentication.id>.info
// 它**不受 CODEBUDDY_CONFIG_DIR 约束**（CLI 用 os.homedir() 定位），所以策略里必须
// 声明成 HOME 相对路径；账号沙箱里同样的相对路径就是被隔离的 HOME。
//
// 文件名由**发行版**决定，不是由站点决定（2026-09-15 实测 2.151.0 两个发行版）：
//   - WorkBuddy.app 内嵌 CLI：product.json 的 authentication.id = `workbuddy-desktop`
//   - 独立分发（npm 包 / 官方 install.sh 的两个站点脚本）：`Tencent-Cloud.coding-copilot`
//     （国内站 install.sh 从 myqcloud COS 取的是**同一个包**，内嵌的 authentication
//     块仍是这一个，不带站点差异；站点只由 token 的 realm 决定）
// 所以**同一个 .info 文件在同发行版下可承载不同站点的 token**——这是厂商的文件布局，
// 不是 aih 能改的事实。两个 Provider 各自声明自己那支 CLI 真正读的那一个文件。
...
// 国内站（copilot.tencent.com / workbuddy.cn）：`codebuddycn` 与 `workbuddy` 共用。
...
// 国际站（www.codebuddy.ai）：`codebuddy` 用。实测 `codebuddy` 的 CLI（独立分发的
// @tencent-ai/codebuddy-code）与 CodeBuddy.app IDE 是同一个 authentication.id，
// 本机该文件里的 token realm 是 `https://www.codebuddy.ai/auth/realms/copilot`。
//
// ⚠️ 该文件**站点不可归因**：国内站独立分发的 CLI 也用这个 id，所以它里面可能换成
// 一枚国内站 token（realm 变成 copilot.tencent.com）。因此国内站 Provider 刻意
...
const PROVIDER_STORAGE_POLICIES = Object.freeze({
  ...
  //   - 主站登录态存于 HOME 下 CodeBuddyExtension 的共享 .info，与国内站 CLI
  //     是同一个文件（见 CODEBUDDY_CN_SHARED_AUTH_PATH），因此两个 Provider
  //     声明同一个 auth artifact = 同一个账号。~/.workbuddy/credentials/ 是
  //     连接器令牌，与主站登录无关，不参与共享。

...
// quoteForCmd 的产物要经 node-pty 的 MSVCRT 转义（" -> \"）后再交给 cmd.exe
// 解析，而 cmd.exe 不认识 \"（2026-08-22 codex -c 引号事故）。因此 cmd 包装行
// 里不允许出现 "：key=value 这类参数必须保持裸文本（= 在 cmd 中不是特殊字符）。
...
function collectCmdSetMap(cmdBody) {
  const values = new Map();
  const re = /^[\s@]*set\s+"?([A-Za-z_][A-Za-z0-9_]*)=([^"\r\n]*)"?/gim;
  let match;
  while ((match = re.exec(String(cmdBody || '')))) {
...
function resolveCmdEnvToken(token, shimDir, cmdEnv, env = {}) {
  const text = stripOuterQuotes(token);
  const variableOnly = text.match(/^%([A-Za-z_][A-Za-z0-9_]*)%$/);
  if (variableOnly) {
    const key = variableOnly[1].toUpperCase();
    const value = cmdEnv.get(key) || env[key] || env[variableOnly[1]] || '';
    return expandCmdEnvValue(value, shimDir, env);
  }
  return resolveCmdShimToken(text, shimDir);
}

function hasFirstArgBranchBeforeLine(lines, lineIndex, firstArg) {
  const arg = String(firstArg || '').trim();
  if (!arg) return false;
  const escaped = arg.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const branchRe = new RegExp(`\\bif\\s+/I\\s+"%~1"=="${escaped}"\\b`, 'i');
  for (let i = 0; i < lineIndex; i += 1) {
    if (branchRe.test(lines[i])) return true;
  }
  return false;
}

function findDelegatedCmdShimTarget(cmdBody, shimDir, args = [], env = {}) {
  const lines = String(cmdBody || '').split(/\r?\n/);
  const cmdEnv = collectCmdSetMap(cmdBody);
  const firstArg = Array.isArray(args) && args.length ? String(args[0] || '').trim() : '';
  for (let i = 0; i < lines.length; i += 1) {
    const line = lines[i];
    if (!/%\*/.test(line)) continue;
    const match = line.match(/^\s*(?:call\s+)?(?:"([^"]+)"|([^\s"]+))\s+%\*/i);
    if (!match) continue;
    if (hasFirstArgBranchBeforeLine(lines, i, firstArg)) return '';
    const target = resolveCmdEnvToken(match[1] || match[2], shimDir, cmdEnv, env);
    const ext = path.win32.extname(target).toLowerCase();
    if (ext === '.cmd' || ext === '.bat') return target;
  }
  return '';
}
...
function resolveWindowsNodeShimLaunch(cliBin, args, options = {}) {
  ...
  const visitKey = shimPath.toLowerCase();
  ...
  try {
    ...
    if (!targetScript) {
      const delegatedShim = findDelegatedCmdShimTarget(body, shimDir, args, options.env || {});
      if (delegatedShim) {
        return resolveWindowsNodeShimLaunch(delegatedShim, args, {
          ...options,
...
function buildPtyLaunch(cliBin, args, options = {}) {
  ...
  // Windows cannot CreateProcess a Node script directly (zcode's CLI ships as
  // zcode.cjs with a `#!/usr/bin/env node` shebang). Route it through node.
  if (ext === '.js' || ext === '.cjs' || ext === '.mjs') {
    const nodeExec = String(options.nodeExecPath || process.execPath || 'node').trim() || 'node';
    return { command: nodeExec, args: [String(cliBin), ...normalizedArgs] };
  }
...
// resolveWindowsUpstreamSpawn 把可能指向 .cmd/.bat 垫片的上游 CLI 二进制解析为
// 可直接 child_process.spawn 的目标：npm node 垫片解析成 node + 脚本（argv
// 直传、引号语义正确，且 Node 直接 spawn .cmd 会抛 EINVAL——CVE-2024-27980
// 防护）；其余 .cmd/.bat 回退 buildPtyLaunch 的 cmd.exe 包装（spawn 时必须
// 携带返回的 windowsVerbatimArguments，见 quoteForCmd 的转义说明）。
// 只有显式传入 platform 'win32' 才启用解析；其余平台与未指定平台原样透传。
function resolveWindowsUpstreamSpawn(cliBin, args = [], options = {}) {
  const platform = String(options.platform || '');
  const normalizedArgs = Array.isArray(args) ? args.map((x) => String(x)) : [];
  if (platform !== 'win32') {
    return {
      command: String(cliBin || ''),
      args: normalizedArgs,
      envPatch: {},
  ...
  const shimLaunch = resolveWindowsNodeShimLaunch(cliBin, normalizedArgs, options);
  if (shimLaunch) {
    return {
      command: shimLaunch.command,
      args: shimLaunch.args,
      envPatch: shimLaunch.envPatch || {},
      windowsVerbatimArguments: false
    };
  }

  const launch = buildPtyLaunch(cliBin, normalizedArgs, { platform });
  return {

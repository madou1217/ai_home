...
function createAccountAppLauncher(deps = {}) {
  ...
  // 全程 best-effort：任何失败只意味着该次启动仍需手动登录。
  function seedKimiManagedDesktopSession(accountRef, userDataDir) {
    try {
      let session = tryReadKimiDesktopSession(accountRef);
      if (!session) return;
      const adopted = adoptKimiTokensImpl(userDataDir);
      if (adopted && adopted.refreshToken && adopted.refreshToken !== session.refreshToken) {
        writeDesktopSession(fsImpl, aiHomeDir, accountRef, { ...session, ...adopted });
        session = { ...session, ...adopted };
      }
      seedKimiTokenStoreImpl({
        userDataDir,
  ...
  function launchDesktop(provider, account, profileDir, cliConfig) {
    ...
    if (provider === 'kimi' && account.accountRef) {
      seedKimiManagedDesktopSession(account.accountRef, userDataDir);
    }
  ...
  function resolveUnscopedCliPath(provider, cliConfig, platformKey, platformPath) {
    ...
    return findOnPath([binaryName], getBaseEnv(), fsImpl, platformPath, platformKey);
  }
  ...
  function launchAccountApp(input = {}) {
    ...
    try {
      if (kind === APP_KIND_CLI) return launchCliTerminal(provider, account, terminalId);

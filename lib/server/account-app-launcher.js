...
const { readAccountCredentials, readAccountCredentialRecord } = require('./account-credential-store');
const {
  readDesktopSession,
  writeDesktopSession
  ...
  return (Array.isArray(processes) ? processes : [])
    .filter((proc) => {
      ...
      return Boolean(targetApplicationName)
        && parseZcodeDesktopApplicationName(commandLine).toLowerCase() === targetApplicationName;
    })
    .map((proc) => proc.pid)
...
// 并兼容 macOS ZCode 改写后的 application name，供 app-entries 批量匹配账号运行态。
  ...
  try {
    ...
    return processes
      ...
      .map((proc) => {
        ...
        const applicationName = parseZcodeDesktopApplicationName(proc.commandLine);
        return {
          pid: proc.pid,
...
function createAccountAppLauncher(deps = {}) {
  ...
  const resolveZcodeCredentialSecretImpl = deps.resolveZcodeCredentialSecret
    || resolveZcodeCredentialSecret;
  ...
  const readCredentialRecordImpl = deps.readAccountCredentialRecord || readAccountCredentialRecord;
  const seedKimiTokenStoreImpl = deps.seedKimiDesktopTokenStore || seedKimiDesktopTokenStore;
  const adoptKimiTokensImpl = deps.adoptKimiDesktopTokensFromProfile || adoptKimiDesktopTokensFromProfile;
  const hasKimiTokenStoreImpl = deps.hasKimiDesktopTokenStore || hasKimiDesktopTokenStore;
  // 凭证库读取失败（DB 缺失/损坏）不阻塞启动，仅跳过托管注入。
  function tryReadKimiDesktopSession(accountRef) {
    try {
      return readDesktopSession(readCredentialRecordImpl(fsImpl, aiHomeDir, accountRef));
    } catch (_error) {
      return null;
    }
  }
  ...
  function prepareKimiManagedDesktopSession(accountRef, userDataDir, platformKey) {
    try {
      let session = tryReadKimiDesktopSession(accountRef);
      const adopted = adoptKimiTokensImpl(userDataDir);
      if (adopted && adopted.refreshToken) {
        if (!session || adopted.refreshToken !== session.refreshToken) {
          writeDesktopSession(fsImpl, aiHomeDir, accountRef, { ...(session || {}), ...adopted });
        }
        session = { ...(session || {}), ...adopted };
      }
      const hasExistingStore = hasKimiTokenStoreImpl(userDataDir, { fs: fsImpl, path: pathImpl });
      ...
      if (!session) {
        return hasExistingStore
          ...
          : { ready: false, error: 'kimi_desktop_session_required' };
      }
      const seeded = seedKimiTokenStoreImpl({
        userDataDir,
        accessToken: session.accessToken,
        refreshToken: session.refreshToken,
        userId: session.userId
      });
      if (seeded && seeded.seeded) return { ready: true, source: 'managed' };
      if (hasExistingStore) {
  ...
  function launchDesktop(provider, account, profileDir, cliConfig) {
    ...
    const desktopApplicationName = provider === 'zcode'
      ? buildZcodeDesktopApplicationName(account && account.accountRef)
      : '';

    // Every Desktop client uses its account-specific user-data directory for
    // instance identity. AGY is deliberately not special here: its native
    // Keychain is shared, so the launch environment must force the provider's
    // per-account file fallback instead of closing unrelated clients.
    ...
    if (runningPids.length > 0 && provider !== 'kimi') {
    ...
    if (provider === 'zcode') {
      ...
      env[ZCODE_CREDENTIAL_SECRET_ENV] = resolveZcodeCredentialSecretImpl(getBaseEnv());
      ...
      // --user-data-dir 仍保持不变；macOS 主进程会把命令行改写为 application name，
      // 因此单实例检测和 app-entries 同时支持 user-data-dir 与应用名身份。
      // 注意：多个实例都注册 zcode:// deep-link，多账号同时开着时 OAuth 回调
      // 可能落到错误的实例，建议只开当前要登录的那一个账号的 Desktop。
      env[ZCODE_DESKTOP_APPLICATION_NAME_ENV] = desktopApplicationName;
    }
    ...
    const isAgyMacos = provider === 'agy' && platformKey === 'macos';
    const launchFile = isAgyMacos ? '/usr/bin/open' : resolved.executablePath;
    const launchArgs = isAgyMacos
      ? ['-n', '-a', resolved.bundlePath, '--args', `--user-data-dir=${userDataDir}`]
      : [`--user-data-dir=${userDataDir}`];
    // kimi 桌面托管登录：启动前把托管 session 种进隔离 profile 的 token 仓。
    let kimiSession = null;
    if (provider === 'kimi' && account.accountRef) {
      kimiSession = prepareKimiManagedDesktopSession(account.accountRef, userDataDir, platformKey);
      if (!kimiSession.ready) {
        return fail(kimiSession.error, {
          reason: kimiSession.reason,
          ...(runningPids.length > 0 ? { pids: runningPids } : {})
        });
      }
    }
    if (runningPids.length > 0) {
      // Kimi 的 token 仓只在主进程启动时读取。如果这次刚把托管
      // session 写入 profile，必须只重启该账号的旧实例；已有可用
      // profile 时则保持同账号单实例，不做无意义重启。
      if (!kimiSession || kimiSession.source !== 'managed') {
        return { ok: true, status: 'already_running', pids: runningPids };
      }
      const killed = closeDesktopPids(runningPids, platformKey);
      if (killed.length !== runningPids.length) {
        return fail('kimi_desktop_restart_failed', { pids: runningPids });
      }
    }
    const child = spawnDetached(launchFile, launchArgs, {
      env,
      cwd: profileDir
    });
  ...
  function closeDesktop(provider, account, profileDir, cliConfig) {
    ...
    const runningPids = findRunningDesktopPids(userDataDir, platformConfig, platformKey, {
      execFileSync: execFileSyncImpl,
      applicationName: provider === 'zcode'
        ? buildZcodeDesktopApplicationName(account && account.accountRef)
        : ''
    });
  ...
  function launchCliTerminal(provider, account, terminalId = DEFAULT_TERMINAL_ID) {
    ...
    const child = spawnDetached(terminal.file, terminal.args, { env: terminalEnv });
    return {
      ok: true,
      pid: child && child.pid,

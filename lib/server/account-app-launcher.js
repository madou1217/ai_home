...
const { launchWindowsInteractiveCommand } = require('../runtime/windows-interactive-launch');
const {
...
const APP_STATUS_LAUNCH_READY = 'launch_ready';
...
function resolveDesktopExecutable(platformKey, platformConfig, context) {
  ...
  if (platformKey === 'windows') {
    const resourcesDir = pathImpl.dirname(onPath);
    if (pathImpl.basename(resourcesDir).toLowerCase() === 'resources') {
      const appDir = pathImpl.dirname(resourcesDir);
      const appEntry = execNames
        .map((name) => pathImpl.join(appDir, name))
        .find((candidate) => fsImpl.existsSync(candidate));
      if (appEntry) return { executablePath: appEntry, bundlePath: '' };
    }
  }
  return { executablePath: onPath, bundlePath: '' };
}
...
function createAccountAppLauncher(deps = {}) {
  ...
  const launchWindowsInteractive = deps.launchWindowsInteractive || launchWindowsInteractiveCommand;
  ...
  function buildAccountLaunchEnv(provider, profileDir, account, egress, egressPrepared) {
    ...
    const options = {
      ...
      ...(egressPrepared === true ? { accountEgress: egress || null } : {})
    };
  ...
  function buildDesktopLaunchContext(
    provider,
    account,
    profileDir,
    userDataDir,
    ...
    return {
      ...
      applicationName: '',
      platformKey,
      egressPrepared: egressPrepared === true,
      // 账号专属出口的解析结果，由调用方预先解析后经 launchAccountApp 传入。
      // 启动器本身不认识 proxy-pool：专用监听解析属于异步 IO，而本层启动链保持
      // 同步；provider 策略只消费已经解析好的结果。
      egress: egress && typeof egress === 'object' ? egress : null,
      fs: fsImpl,
  ...
  function launchDesktop(
    ...
    const ctx = buildDesktopLaunchContext(
      provider,
      account,
      profileDir,
      userDataDir,
    ...
    // 账号出口保存编排只需要知道现有进程是否必须重启。该只读探针必须早于
    // Kimi 等托管登录策略，不能因为检查运行态而读取或改写凭据、profile。
    if (inspectDesktopRunning === true) {
      return runningPids.length > 0
        ? { ok: true, status: 'already_running', pids: runningPids }
        : { ok: true, status: 'not_running', pids: [] };
    }
    ...
    // 异步启动编排先用同步预检确认「确实需要 spawn」，再解析账号出口。
    // 预检必须停在任何投影准备、profile 写入和进程创建之前，避免资格失败、
    // 已运行实例或未安装客户端触发无意义的代理探测与 Proxy Pool 副作用。
    if (deferDesktopSpawn === true) {
      return {
        ok: true,
        status: APP_STATUS_LAUNCH_READY,
        executable: resolved.executablePath
      };
    }

    const env = buildAccountLaunchEnv(
      provider,
      profileDir,
      account,
      egress,
    ...
    // 仅对真正即将 spawn 的实例同步 provider 原生 profile。已有实例早退、安装缺失、
    // 托管登录失败或重启失败时都不改磁盘，避免配置已变而进程仍沿用旧状态。
    const profile = strategy.prepareLaunchProfile(ctx) || { ready: true };
    if (!profile.ready) {
      return fail(profile.error || 'desktop_profile_prepare_failed', {
        ...(profile.reason ? { reason: profile.reason } : {}),
        ...(profile.egressWarning ? { egressWarning: profile.egressWarning } : {})
      });
    }
    // 已解析安装包的派生准备（默认原样返回）放在所有资格/会话/profile 检查之后，
    // 只对真正即将 spawn 的实例执行。ZCode 在这里生成账号投影内的 CoW 影子 App；
    // env 装饰和 spawn 统一消费最终 resolved，不能再引用原始 bundle。
    const preparedResolved = strategy.prepareResolvedLaunch(resolved, ctx) || { ready: true, resolved };
    if (!preparedResolved.ready) {
      return fail(preparedResolved.error || 'desktop_resolved_launch_prepare_failed', {
        ...(preparedResolved.reason ? { reason: preparedResolved.reason } : {})
      });
    }
    const launchResolved = preparedResolved.resolved || resolved;
    strategy.decorateResolvedLaunchEnv(env, launchResolved, ctx);
    const spawnPlan = decorateAccountEgressChromiumPlan(
      strategy.resolveSpawnPlan(launchResolved, ctx),
      provider,
      egressPrepared ? egress : null
    );
    const child = spawnDetached(spawnPlan.file, spawnPlan.args, {
      env,
      cwd: profileDir,
      ...(platformKey === 'windows' ? { windowsHide: false } : {})
    });
    const egressWarning = String(profile.egressWarning || '').trim();
    return {
      ok: true,
      status: 'launched',
      pid: child && child.pid,
      executable: launchResolved.executablePath,
  ...
  // zcode 桌面端运行期会在沙箱 profile 里刷新 token（持有 refresh_token）但不回写
  // DB；关闭（或发现已退出）时把活凭据捕获回 DB 快照，服务端探测才不会继续用过期
  // JWT。捕获失败绝不影响关闭结果。
  function captureZcodeAuthOnClose(provider, account, profileDir) {
    if (provider !== 'zcode' || !account || !account.accountRef || !profileDir) return;
    try {
      captureProviderAuth(fsImpl, profileDir, 'zcode', {
        path: pathImpl,
        aiHomeDir,
        accountRef: account.accountRef,
        processObj
  ...
  function closeDesktop(provider, account, profileDir, cliConfig) {
    ...
    if (runningPids.length === 0) {
      captureZcodeAuthOnClose(provider, account, profileDir);
  ...
  // 归一化 WebUI 传入的项目目录：支持 ~ 前缀展开；空值保持缺省行为
  // （终端继承 Server 工作目录）。
  function normalizeCliWorkdir(input) {
    ...
    if (text === '~' || text.startsWith('~/') || text.startsWith('~\\')) {
      ...
      if (!home) return text;
      return text === '~' ? home : pathImpl.join(home, text.slice(2));
    }
    return text;
  }
  ...
  function isExistingDirectory(candidate) {
    try {
      return Boolean(fsImpl.statSync(candidate).isDirectory());
    } catch (_error) {
      return false;
    }
  }

  // 终端适配器统一从命令串启动（macOS 经 AppleScript do script，env 不会
  // 透传到新窗口），工作目录只能拼进命令：posix 用 cd '…'，Windows 沿用
  // 现有 set "…" 同层的引号形态 cd /d "…"。
  function buildWorkdirCdPrefix(workdir, platformKey) {
    const quoted = quoteShellToken(workdir, platformKey);
    return platformKey === 'windows' ? `cd /d ${quoted}` : `cd ${quoted}`;
  }
  ...
  function launchCliTerminal(provider, account, terminalId = DEFAULT_TERMINAL_ID, workdir = '') {
    const cliAccountId = String(account && account.cliAccountId || '').trim();
    if (account && !/^\d+$/.test(cliAccountId)) return fail('account_not_found');

    const workdirPath = normalizeCliWorkdir(workdir);
    if (workdirPath && !isExistingDirectory(workdirPath)) {
      return fail('workdir_not_found', { provider, kind: APP_KIND_CLI, workdir: workdirPath });
    }
    ...
    if (workdirPath) command = `${buildWorkdirCdPrefix(workdirPath, platformKey)} && ${command}`;
    ...
    if (platformKey === 'windows') {
      // WebUI Server 常驻在 Session 0；直接 spawn WindowsTerminal/WT 只会在
      // Session 0 激活 AppX IPC，窗口和 Codex 子进程不会进入用户桌面。通过
      // 当前登录用户的交互计划任务交接，再由 `start ... cmd.exe` 创建可见
      // conhost，Codex 仍沿用完整 aih 启动链路和账号 env 标记。
      const interactive = launchWindowsInteractive(command, {
        fs: fsImpl,
        path: platformPath,
        env: terminalEnv,
        processObj,
        ...
        nonce: deps.interactiveLaunchNonce
      });
      if (!interactive || interactive.ok !== true) {
        return fail('terminal_launch_failed', {
          provider,
          kind: APP_KIND_CLI,
          ...(interactive && interactive.message ? { message: interactive.message } : {})
        });
      }
      return {
        ok: true,
        ...
        accountRef: String(account && account.accountRef || ''),
        launchMode: 'interactive-task',
        taskName: interactive.taskName
      };
    }
    // 真实 AppX 宿主直接以独立 argv 启动并显窗；仅在无法解析宿主、只能使用
    // wt.exe AppExecutionAlias 时，终端适配器才返回隐藏 cmd.exe + start 的兼容规格。
    // 两条路径都由统一的 Windows spawn 规格消费，避免命令参数在外层 shell 合并。
    ...
    try {
      if (kind === APP_KIND_CLI) {
        return action === APP_ACTION_CLOSE
          ...
          : launchCliTerminal(provider, account, terminalId, input.workdir);
      }
      ...
      return action === APP_ACTION_CLOSE
        ...
        : launchDesktop(
          ...
          cliConfig,
          input.egress,
          Object.prototype.hasOwnProperty.call(input, 'egress'),
          input.deferDesktopSpawn === true,
          input.inspectDesktopRunning === true
        );
    } catch (error) {

...
function createAccountAppLauncher(deps = {}) {
  ...
  function launchAccountApp(input = {}) {
    ...
    if (kind === APP_KIND_DESKTOP && !cliConfig.desktopClient) return fail('desktop_not_supported');
...
// desktop = manifest 声明了 desktopClient 且能解析到可执行文件；
// cli = CLI 二进制在 PATH 上可解析（zcode 没有面向用户的 CLI 二进制，因此隐藏）。
...
function createAppEntryDetector(deps = {}) {
  ...
  function detect() {
    ...
    for (const definition of listProviderDefinitions()) {
      ...
      const desktop = Boolean(platformConfig && resolveDesktopExecutable(platformKey, platformConfig, {
        fs: fsImpl,
        path: platformPath,
        env,
      ...
      const cli = Boolean(binaryName && findOnPath([binaryName], env, fsImpl, platformPath, platformKey));
  ...
  function detectCapabilities() {
    ...
    for (const definition of listProviderDefinitions()) {
      ...
      capabilities[definition.id] = {
        desktop: Boolean(platformConfig),
        cli: Boolean(cliConfig.binaryName || definition.id)
      };
    }
    return capabilities;
  }

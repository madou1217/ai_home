...
function compareAndSwapAccountNativeAuth(fs, aiHomeDir, accountRef, snapshot, data) {
  const normalizedRef = normalizeAccountRef(accountRef);
  if (!normalizedRef) return false;
  const expected = snapshot && isPlainObject(snapshot.nativeAuth)
    ? snapshot.nativeAuth
    : null;
  const expectedUpdatedAt = Number(snapshot && snapshot.nativeAuthUpdatedAt);
  if (!expected || !Number.isFinite(expectedUpdatedAt) || expectedUpdatedAt < 0) return false;
  const normalizedValue = isPlainObject(data) ? data : {};
  const now = Math.max(Date.now(), expectedUpdatedAt + 1);
  const result = withCredentialDatabase(fs, aiHomeDir, (db) => db.prepare(`
    UPDATE ${ACCOUNT_CREDENTIAL_TABLE}
    SET native_auth_json = ?, native_auth_updated_at = ?, updated_at = ?
    WHERE account_ref = ?
      AND native_auth_updated_at = ?
      AND native_auth_json = ?
  `).run(

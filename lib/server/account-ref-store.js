...
function deleteAccountRef(fs, aiHomeDir, accountRef, deps = {}) {
  ...
  try {
    ...
    inTransaction = true;
    const account = db.prepare('SELECT provider FROM account_refs WHERE account_ref = ?').get(normalizedRef);
    // Independent App files can outlive account deletion. Preserve the user's
    // removal intent so the background observer cannot resurrect that account.
    // A later explicit registration clears this marker in its own transaction.
    if (account && ['codex', 'codebuddy', 'codebuddycn', 'workbuddy', 'workbuddycn'].includes(account.provider)) {
      const now = Date.now();
      db.prepare(`INSERT INTO app_kv (key, value, updated_at) VALUES (?, ?, ?)
        ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`)
        .run(`account:deleted:${normalizedRef}`, JSON.stringify({ deletedAt: now }), now);
      if (account.provider !== 'codex') {
        const hasCredentials = db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='account_credentials'").get();
        const row = hasCredentials && db.prepare('SELECT native_auth_json FROM account_credentials WHERE account_ref=?').get(normalizedRef);
        if (row) {
          let auth; try { auth = JSON.parse(row.native_auth_json); } catch (_) {}
          const identity = require('../account/account-identity').resolveNativeAuthIdentitySeed(account.provider, auth);
          if (identity.identitySeed && !identity.degraded) {
            const canonical = getPublicAccountRef(`unique:${identity.identitySeed}`);
            db.prepare(`INSERT INTO app_kv (key, value, updated_at) VALUES (?, ?, ?)
              ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at`)
              .run(`account:deleted:${canonical}`, JSON.stringify({ deletedAt: now }), now);
          }
        }
      }
    }

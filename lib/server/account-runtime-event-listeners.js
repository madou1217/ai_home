...
function removeSessionAffinityForAccount(state, provider, accountId) {
  ...
  map.forEach((entry, key) => {
    if (String(entry && entry.accountId || '') === String(accountId)) {
      map.delete(key);
      removed += 1;
    }
  });
...
// 需求：listener 需要按 provider/accountId 找到当前内存账号，但不能知道调用方上下文。
function findRuntimeAccount(state, provider, accountId) {
  const accounts = Array.isArray(state && state.accounts && state.accounts[provider])
    ? state.accounts[provider]
    : [];
  return accounts.find((account) => String(account && account.id || '') === String(accountId)) || null;
}
...
function createRuntimeStateIndexListener(deps = {}) {
  ...
  return function runtimeStateIndexListener(event) {
    if (accountStateService && typeof accountStateService.clearRuntimeBlock === 'function' && event.runtimeState === null) {
      return accountStateService.clearRuntimeBlock(event.provider, event.accountId, {
        ...(event.baseState || {}),
...
function applyRuntimeStateToMemoryAccount(state, event) {
  const account = findRuntimeAccount(state, event.provider, event.accountId);
...
function createServerPoolSyncListener(deps = {}) {
  ...
  return function serverPoolSyncListener(event) {
    ...
    if (isBlockingRuntimeStatus(resolveEventRuntimeStatus(event))) {
      removeSessionAffinityForAccount(state, event.provider, event.accountId);

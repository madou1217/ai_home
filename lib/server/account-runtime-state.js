...
// failure streak reaches the policy threshold. Policies can also opt out of
// account state entirely (for example endpoint/network reachability failures),
// while timeout policies retain a threshold so one slow request leaves the
// account routable. Model-scoped failures (429 / quota / capacity) are delegated
// to markProxyAccountFailure with { scope:'model' } and never touch account-wide
// state.
//
...
function applyAccountFailurePolicy(account, policy, options = {}) {
  ...
  // such as a timeout policy) defer the account-wide cooldown until consecutive
  // failures pile up, so one blip never empties the pool.

...
function syncPersistedRuntimeState(account, provider, accountStateIndex) {
  ...
  if (!provider || !accountId) return account;
  const row = accountStateIndex.getAccountState(provider, accountId);
  ...
  if (Object.prototype.hasOwnProperty.call(row, 'runtime_state')) {
    return replacePersistedAccountRuntimeState(account, row.runtime_state);
  }
  return account;
}

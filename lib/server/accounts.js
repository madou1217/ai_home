...
// zcode 网关池只收 API-key 账号：参与推理调度（anthropic 协议透传，x-api-key 鉴权）。
// OAuth 计划账号自 2026-08-19 起不再进入推理池（relay 已取消：Start Plan 推理
// 准入门由 Z.ai 服务端活动窗口控制，405/3012，官方桌面端同待遇，客户端侧无解）。
// OAuth 账号仍由账号管理 / 桌面启动 / billing 用量展示使用（这些链路直接按
// accountRef 读凭据，不经过本池）。jwtToken 过期只能重新 login 导入。
...
function loadZcodeServerAccounts(deps) {
  ...
  listProviderCredentialRecords(deps, 'zcode').forEach((record) => {
    ...
    if (!apiKey) {
      // OAuth 计划账号不进推理池（见上方注释）。模型探测/用量采集按 accountRef
      // 直接读凭据，不经过本池：zcodeJwtToken 对应 GET zcode-plan billing/balance

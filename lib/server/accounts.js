...
function loadKimiServerAccounts(deps) {
  ...
  listProviderCredentialRecords(deps, 'kimi').forEach((record) => {
    ...
    const nativeAuth = effectiveRecord.nativeAuth && (effectiveRecord.nativeAuth.credentials || effectiveRecord.nativeAuth.auth);
    const oauthToken = nativeAuth && String(nativeAuth.access_token || nativeAuth.accessToken || '').trim();
    const refreshToken = nativeAuth && String(nativeAuth.refresh_token || nativeAuth.refreshToken || '').trim();
    const accessToken = apiKey || oauthToken;
    ...
    const deviceId = apiKeyMode
      ? ''
      : String(effectiveRecord.nativeAuth && effectiveRecord.nativeAuth.deviceId || '').trim()
        || readKimiTokenDeviceId(nativeAuth);
    out.push({
      accountRef,
      email: '',
      provider: 'kimi',

...
const { ZCODE_DEFAULT_BASE_URL } = require('../account/provider-api-base-url');
...
// - OAuth 计划账号：仅入池做模型目录探测，不参与推理调度。实证（2026-08）：
//   GET https://api.z.ai/api/coding/paas/v4/models 用 Bearer zai accessToken 免验证码
//   返回模型列表；而计划推理端点 zcode.z.ai/api/v1/zcode-plan/anthropic 每请求强制
//   阿里云验证码（实测 400 {"code":3007,"msg":"captcha verify failed"}），网关未实现
//   验证码求解前 relay 不可用，故 schedulableStatus 置为 oauth_relay_unsupported。
// OAuth 计划账号没有 refresh token；jwtToken 过期只能重新 login 导入。
const ZCODE_OAUTH_MODELS_BASE_URL = 'https://api.z.ai/api/coding/paas/v4';
const ZCODE_OAUTH_SCHEDULABLE_STATUS = 'oauth_relay_unsupported';
...
function loadZcodeServerAccounts(deps) {
  ...
  listProviderCredentialRecords(deps, 'zcode').forEach((record) => {
    ...
    if (!apiKey) {
      // 模型探测以 zcodeJwtToken 为准（桌面端同款：GET zcode-plan billing/balance
      // 拿套餐实际可用模型）；accessToken（zai OAuth token）只是 paas 探测的回退凭据，
      // 它会过期且无 refresh token，过期后 api.z.ai 实测 401。jwtToken 过期只能重新 login 导入。
      ...
      out.push({
        ...
        openaiBaseUrl: ZCODE_OAUTH_MODELS_BASE_URL,
        ...
        quotaStatus: 'not_applicable',
        quotaReason: '',
        schedulableStatus: ZCODE_OAUTH_SCHEDULABLE_STATUS,
        schedulableReason: 'zcode OAuth 计划推理需桌面端验证码，网关仅用于模型探测',
        cooldownUntil: 0,
        consecutiveFailures: 0,
        successCount: 0,
        failCount: 0,

...
// - OAuth 计划账号：参与推理调度。推理端点 zcode.z.ai/api/v1/zcode-plan/anthropic
//   每请求强制阿里云验证码（实测 400 {"code":3007,"msg":"captcha verify failed"}），
//   由 lib/server/zcode-captcha-bridge.js 把验证码挑战桥到 WebUI 求解后带
//   X-Aliyun-Captcha-Verify-Param 头重发（upstream-endpoints.js 的 3007 拦截分支）。
//   模型探测：GET https://api.z.ai/api/coding/paas/v4/models 用 Bearer zai accessToken
//   免验证码返回模型列表（balance 接口失败时的回退，见 http-utils.js）。

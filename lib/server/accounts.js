...
function readTokenFromAgyConfig(fs, configDir) {
  if (!configDir) return '';
  const tokenPath = path.join(configDir, 'antigravity-oauth-token');
  const oauth = parseJsonFileSafe(tokenPath, fs);
  const token = String(oauth && oauth.token && oauth.token.access_token || '').trim();
  return token;
}
...
function loadAgyServerAccounts(deps) {
  ...
  ids.forEach((id) => {
    ...
    const oauthToken = readTokenFromAgyConfig(fs, configDir);
    const accessToken = envToken || oauthToken;

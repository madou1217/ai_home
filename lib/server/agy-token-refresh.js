...
function redactKnownOauthSecrets(value, secrets = []) {
  ...
  for (const secret of secrets) {
    ...
    if (encoded !== normalized) text = text.split(encoded).join('[redacted]');
    text = text.split(normalized).join('[redacted]');
  }
  return sanitizeResponseText(text, 320);
}

function normalizeOAuthErrorCode(value, secrets = []) {
  const code = redactKnownOauthSecrets(value, secrets);
  if (!code || code.includes('[redacted]')) return '';
...
function persistAgyOAuthSnapshot(account, tokens, nowMs, deps = {}) {
  ...
  if (Number.isFinite(tokens.expiresAt) && tokens.expiresAt > 0) {
    ...
    // 与 expiry 同步更新毫秒字段：Go 按 expires_at_ms / refreshed_at_ms 判断过期与凭据代际，
    // 只改 expiry 会让同步后的 Go 一直认为 token 早已过期（agy 请求全部 401）。
    next.token.expires_at_ms = Math.trunc(tokens.expiresAt);
  }
  next.token.refreshed_at_ms = Math.trunc(nowMs);
  ...
  materializeProviderAuth(
    context.fs,
    resolveAccountRuntimeDir(context.aiHomeDir, 'agy', context.accountRef),
    'agy',
    {
...
async function refreshAgyAccessToken(account, options = {}, deps = {}) {
  ...
  const knownOauthSecrets = [refreshToken, account.accessToken];
  const refreshTask = (async () => {
    try {
      ...
      knownOauthSecrets.push(...credentialCandidates.map((candidate) => candidate.clientSecret));
      let payload = null;
      ...
      for (const { clientId, clientSecret } of credentialCandidates) {
        ...
        if (!response.ok) {
          ...
          const code = normalizeOAuthErrorCode(oauthError.code, knownOauthSecrets);
      ...
      return {
        ...
        detail: redactKnownOauthSecrets(
          ...
          [...knownOauthSecrets, account.accessToken]
        )
      };
    } finally {

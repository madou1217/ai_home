...
function writeAgyUsageSnapshot(fs, profileDir, snapshot) {
  if (!fs || !profileDir || !isTrustedAgyUsageSnapshot(snapshot)) return false;
  const filePath = path.join(profileDir, '.aih_usage.json');
...
async function refreshAgyUsageSnapshotForAccount(input = {}) {
  ...
  const work = fetchAgyCodeAssistQuotaSnapshot({
    ...
    .then((snapshot) => {
      ...
      writeAgyUsageSnapshot(fs, account.profileDir, snapshot);

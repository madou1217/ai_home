  ...
  const work = (async () => {
    const accountRequestOptions = await resolveProviderAccountEgressRequestOptions({
      ...
      deps: {
        ...
        processObj: input.processObj,
        accountEgressDeps: input.accountEgressDeps,
        resolveAccountEgressRequestOptions: input.resolveAccountEgressRequestOptions
      }
    });
    if (!accountRequestOptions?.ok || !accountRequestOptions.options) return null;
    const fetchImpl = accountRequestOptions.bound
      ? createQuotaFetchImpl(input.fetchWithTimeout, accountRequestOptions.options, timeoutMs)
      : (input.fetchImpl || createQuotaFetchImpl(input.fetchWithTimeout, accountRequestOptions.options, timeoutMs));

...
// accountKey(profileDir) -> entry
//   entry: { child, pid, port, accountKey, agyBin, baseEnv, projectId, lastUsed }
const pool = new Map();
let evictTimer = null;
...
async function adopt({ accountKey, child, agyBin, baseEnv, projectId }) {
  const key = String(accountKey || '').trim();
  if (!key || !child || !pidAlive(child.pid)) return false;
  ...
  // 同账号已有旧暖机 LS：用新的替换（新进程同样能服务该账号所有会话）。
  ...
  if (typeof child.onExit === 'function') {
    child.onExit(() => {
      ...
      if (cur && cur.child === child) pool.delete(key);
    });
  }

  enforceCap();
  ensureEvictTimer();
  log('adopted', key, 'pid', child.pid, 'port', port);
  return true;
}

function getLive(accountKey) {
  const key = String(accountKey || '').trim();
...
function hasWarm(accountKey) {
  return !!getLive(accountKey);
}
...
async function runWarmResume({ accountKey, conversationId, prompt, transcriptPath }) {
  const entry = getLive(accountKey);
  ...
  try {
    ...
    // LS 可能已崩/端口失效——剔除该暖机 LS，让调用方回退到冷启动。
    log('send-message failed, dropping warm LS:', error && error.message);
    killEntry(entry);
    pool.delete(entry.accountKey);

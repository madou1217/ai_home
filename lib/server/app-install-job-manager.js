...
function isCliInstallSupported(provider, installer = getAppInstaller(provider)) {
  ...
  return Boolean(definition && definition.clients && definition.clients.cli
...
function createAppInstallJobManager(options = {}) {
  ...
  async function installDesktop(provider, installOptions = {}) {
    ...
    for (const plan of plans) {
      ...
      if (attempt.ok && verifyDesktopInstall) {
        try {
          ...
          verification = null;
          attempt.error = `安装命令已完成，但应用校验异常：${String(error && error.message || error || 'unknown_error')}`;
        }
        if (!verification && !attempt.error) {
          attempt.error = '安装命令已完成，但未检测到目标 Desktop 应用。';
        }
  ...
  function start(input = {}) {
    ...
    if (target.kind === 'cli' && !isCliInstallSupported(target.provider)) {
    ...
    const job = {
      id,
      source: 'app-install',
      taskName: `${target.provider} ${target.kind === 'desktop' ? 'Desktop' : 'CLI'} 安装`,
      ...target,
      status: 'queued',
      phase: 'queued',
  ...
  function canInstall(input = {}) {
    ...
    return isCliInstallSupported(target.provider, installer);
  }

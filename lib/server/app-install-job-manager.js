...
const { getIdeClient } = require('../cli/services/toolkit/ide-client-registry');
const {
  openSseStream,
  writeSseJson,
  broadcastSseJson,
...
function actionLabel(action) {
  return ({ install: '安装', update: '更新', uninstall: '卸载' })[normalizeAction(action)] || '操作';
}
...
function createAppInstallJobManager(options = {}) {
  ...
  const verifyDesktopInstall = typeof options.verifyDesktopInstall === 'function'
    ...
    : null;
  const resolveDesktopRecord = typeof options.resolveDesktopRecord === 'function'
    ? options.resolveDesktopRecord
    : verifyDesktopInstall;
  const resolveCliPath = typeof options.resolveCliPath === 'function'
    ? options.resolveCliPath
    : null;
  ...
  function resolveLifecyclePlans(provider, kind, action, installOptions = {}) {
    ...
    if (installer && typeof installer.resolveLifecyclePlans === 'function') {
      ...
      if (action !== 'install' && kind === 'cli' && resolveCliPath) {
        try { installedPath = resolveCliPath(provider, { ...options, ...installOptions }); } catch (_error) {}
      }
      if (action !== 'install' && kind === 'desktop' && resolveDesktopRecord) {
        try { installedRecord = resolveDesktopRecord(provider, { ...options, ...installOptions }); } catch (_error) {}
      }
    ...
    if (plans.length === 0) {
      return {
        ...
        error: action === 'install' && kind === 'desktop'
          ...
          : '应用生命周期配置异常，请刷新资源清单后重试。',
        errorCode: 'app_lifecycle_contract_violation'
      };
    }
  ...
  function start(input = {}) {
    ...
    if (!normalizeAction(input.action)) return { ok: false, error: 'invalid_lifecycle_action' };
    ...
    const job = {
      ...
      status: 'queued',
      phase: 'queued',
      progress: { percent: 0, label: `等待${actionLabel(target.action)}任务开始` },
      attempts: [],
      result: null,
      error: '',
      createdAt: timestamp,
  ...
  function cancelJob(jobId) {
    ...
    job.phase = 'cancelled';
    job.error = `${actionLabel(job.action)}任务已取消。`;
    job.finishedAt = now();

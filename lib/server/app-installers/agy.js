...
module.exports = createProviderInstaller({
  ...
  cli: {
    ...
    binaryNames: ['agy'],
    cleanupHomeFiles: ['.local/bin/agy']
  },
  desktop: {
    macos: {
      plans: [buildAntigravityPosixDesktopPlan()],
      cleanupHomeTrees: ['Applications/Antigravity.app']
    },
    windows: {
      plans: [buildAntigravityWindowsDesktopPlan()],
      windowsDisplayNames: ['Antigravity']
    },
    linux: {
      plans: [buildAntigravityPosixDesktopPlan()],
      cleanupHomeFiles: ['.local/bin/agy'],
      cleanupHomeTrees: ['.local/opt/antigravity']
    }
  }
});

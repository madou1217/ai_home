...
const { collectBundledCliPathEntries } = require('./codebuddy-bundle-cli');
 ...
 * 收集 CLI 可能的安装目录。
 *
 * 第一优先级是 **WorkBuddy.app 内嵌的 CLI**（见 codebuddy-bundle-cli.js）：装了
 * WorkBuddy 桌面端就已经有一份国内站 CLI，因此解析阶段即可命中，不会触发任何安装
 * ——这就是国内侧的零安装闭环。之后的条目才是独立安装的落点（官方脚本用
 * ~/.local/bin，与国内站同落点）。
 *
 * **为什么内嵌件必须排在前面（不只是版本一致）**：两个发行版的 product.json 里
 * `authentication.id` 不同，因而读的是**不同的凭据文件**（2026-09-15 实测 2.151.0）：
 *   - WorkBuddy.app 内嵌件 → `workbuddy-desktop.info`（= 桌面端写入的那份，站点干净）
 *   - 独立分发件（npm / 官方 install.sh） → `Tencent-Cloud.coding-copilot.info`
 *     （与**国际站**同 id，文件站点不可归因）
 * 所以让独立分发件抢先，会让国内站账号落到国际站那支 CLI 的凭据文件上，闭环静默失效。
 * 由此得到的更新通道：桌面端存在时 **WorkBuddy.app 就是国内站 CLI 的更新源**。
 * 这是刻意选择，不是顺序疏漏。
 *
 * @param {object} [options]
 * @returns {string[]}
...
function collectCliPathEntries(options = {}) {
  ...
  const entries = [
    ...collectBundledCliPathEntries(options),

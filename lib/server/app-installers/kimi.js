...
function collectCliPathEntries(options = {}) {
  ...
  // 官方 install.sh 与 uv tool install 的默认落点
  const entries = [adapter.path.join(home, '.local', 'bin')];
  // kimi-code 自带 bin 目录与 bun 全局安装落点
  entries.push(adapter.path.join(home, '.kimi-code', 'bin'));
  entries.push(adapter.path.join(home, '.bun', 'bin'));
  if (normalizePlatform(options) !== CLIENT_PLATFORMS.WINDOWS) {
    // npm 全局安装落在当前 node 的 bin 目录；再补 Homebrew/系统常见前缀
    const processObj = options.processObj && typeof options.processObj === 'object' ? options.processObj : null;
    const execPath = processObj ? String(processObj.execPath || '').trim() : '';
    if (execPath) entries.push(adapter.path.dirname(execPath));
...
module.exports = createProviderInstaller({
  ...
  desktop: {
    macos: {
      ...
      cleanupHomeTrees: ['Applications/Kimi.app']
    },
    windows: {
      resolveInstallPlans: resolveDesktopInstallPlans,
      windowsDisplayNames: ['Kimi']
    }
  },
  cli: {
    ...
    binaryNames: ['kimi'],
    cleanupHomeFiles: ['.local/bin/kimi']
  }
});

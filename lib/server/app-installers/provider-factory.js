...
function createProviderInstaller({ provider, desktop = {}, cli = {}, lifecycle = {} }) {
  ...
  function resolveLifecyclePlans(action, options = {}) {
    ...
    if (kind === 'desktop') return resolveDesktopInstallPlans(options);
    if (!hasCli) return [];
    if (action === 'uninstall' && typeof cli.resolveUninstallPlans === 'function') {
      return cli.resolveUninstallPlans(options) || [];
    }
    // Official CLI installers are intentionally idempotent. Until a provider
    // exposes a verified uninstall strategy, update reuses its official plan
    // and uninstall remains an explicit empty capability rather than guessing.

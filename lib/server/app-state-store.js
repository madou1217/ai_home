...
const { assertAccountMaintenanceAvailable } = require('../runtime/account-maintenance-gate');
const { acquireAccountMaintenanceLock, activateRequestedProcessLease } = require('../runtime/account-maintenance-lock');
...
function getDatabaseSyncCtor(deps = {}) {
  if (Object.prototype.hasOwnProperty.call(deps, 'DatabaseSync')) {
    ...
    if (deps.strict === true && typeof DatabaseSync !== 'function') {
      throw new Error('app_state_database_unavailable');
    }
    return DatabaseSync;
  }
  try {
    ...
    if (deps.strict === true) throw error;
    return null;
  }
}
...
/** An explicitly injected REAL SQLite driver must not bypass maintenance. */
function usesNativeSQLite(DatabaseSync) {
  try {
    const Native = require('node:sqlite').DatabaseSync;
    return DatabaseSync === Native || DatabaseSync.prototype instanceof Native;
...
function openAppStateDatabase(fs, aiHomeDir, deps = {}) {
  ...
  const realDatabase = !Object.hasOwn(deps, 'DatabaseSync') || usesNativeSQLite(DatabaseSync);
  if (realDatabase) activateRequestedProcessLease(fs, aiHomeDir);
  // Real SQLite connections participate in the OS-backed read lease. Existing
  // injected test drivers keep their own I/O boundary; no production flag bypass.
  const lease = realDatabase ? acquireAccountMaintenanceLock(fs, aiHomeDir) : null;
  let db;
  try {
    assertAccountMaintenanceAvailable(fs, aiHomeDir);
    db = new DatabaseSync(dbPath);
    const close = db.close.bind(db);
    // A failed close can leave a live connection. Keep exclusion until the
    // native close succeeds; releasing it in finally would allow overlap.
    db.close = () => { close(); lease?.close(); };
    ...
    if (db) {
      try { db.close(); } catch (_closeError) { /* Retain exclusion until process exit. */ }
    } else lease?.close();
    throw error;
  }
}

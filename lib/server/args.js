...
function parseServerServeArgs(rawArgs) {
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    ...
    if (arg === '--provider') {
      const val = String(tokens[i + 1] || '').trim().toLowerCase();
      if (!['codex', 'gemini', 'claude', 'agy', 'auto'].includes(val)) throw new Error('Invalid --provider value');
      out.provider = val;
      i += 1;
      continue;
    }
  ...
  if (!['codex', 'gemini', 'claude', 'agy', 'auto'].includes(out.provider)) out.provider = 'auto';

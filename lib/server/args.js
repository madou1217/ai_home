...
function parseServerServeArgs(rawArgs) {
  ...
  const out = {
    ...
    // Provider CLI 自动升级。检查默认开，**应用默认关**：静默改用户全局环境（npm -g 会连带
    // 改掉用户自己终端里的 codex）必须是一次显式选择，见 provider-session-hook-autoinstall.js
    // 头注释记下的那次回退。用 AIH_SERVER_PROVIDER_CLI_AUTO_UPGRADE_APPLY=1 打开。
    providerCliAutoUpgrade: String(process.env.AIH_SERVER_PROVIDER_CLI_AUTO_UPGRADE || '1').trim() !== '0',
    providerCliAutoUpgradeApply: String(process.env.AIH_SERVER_PROVIDER_CLI_AUTO_UPGRADE_APPLY || '0').trim() === '1',
  ...
  out.providerCliAutoUpgradeApply = out.providerCliAutoUpgradeApply === true;
  {

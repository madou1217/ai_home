...
function parseServerServeArgs(rawArgs) {
  ...
  const out = {
    ...
    // Historical transcript scans are synchronous; opt in explicitly so a
    // large local Codex history can never starve the HTTP event loop at boot.
    modelUsageScan: String(process.env.AIH_SERVER_MODEL_USAGE_SCAN || '0').trim() !== '0',

...
class ChatRuntimePublishingStore {
  ...
  branchSession(command) {
    const child = this.store.branchSession(command);
    this.publishSince(child.sessionId, 0);
    return child;
  }
  readHistorySeed(sessionId) { return this.store.readHistorySeed(sessionId); }
  recordNativeResponseItem(sessionId, evidence) { return this.store.recordNativeResponseItem(sessionId, evidence); }
  readNativeResponseItem(sessionId, itemId) { return this.store.readNativeResponseItem(sessionId, itemId); }
  markNativeHistoryCoverage(sessionId, evidence) { return this.store.markNativeHistoryCoverage(sessionId, evidence); }
  ...
  updateRuntimeBinding(sessionId, patch, replacement) {
    return this.publishKnown(sessionId, () => this.store.updateRuntimeBinding(sessionId, patch, replacement));
  }
  ...
  setGoal(sessionId, goal, options) {
    return this.publishKnown(sessionId, () => this.store.setGoal(sessionId, goal, options));
  }

  clearGoal(sessionId) {
    return this.publishKnown(sessionId, () => this.store.clearGoal(sessionId));
  }
  ...
  getRetrySubmission(sessionId, sourceTurnId) {
    return this.store.getRetrySubmission(sessionId, sourceTurnId);
  }

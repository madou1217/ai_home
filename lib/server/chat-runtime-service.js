...
class ChatRuntimeService {
  ...
  async dispatchCommand(sessionId, command = {}) {
    ...
    if (isChatSession(session) && command.type === 'turn.submit') {
      const content = String(command.payload && command.payload.content || '').trim();
      this.store.updatePolicy(sessionId, {
        ...(session.policy.title === '新对话' && content ? { title: content.slice(0, 50) } : {}),
        ...(command.payload && command.payload.model ? { model: command.payload.model } : {})
      });
    }

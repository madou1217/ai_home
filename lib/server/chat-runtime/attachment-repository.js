...
class AttachmentRepository {
  ...
  cloneInTransaction(sourceId, childId, ids) {
    return normalizeAttachmentIds(ids).map((id) => {
      const row = this.context.db.prepare(`SELECT * FROM chat_runtime_attachments
        WHERE session_id = ? AND attachment_id = ?`).get(sourceId, id);
      if (!row) throw new ChatRuntimeError('chat_attachment_not_found', 404);
      const childAttachmentId = this.context.idFactory('attachment');
      this.context.db.prepare(`INSERT INTO chat_runtime_attachments
        (attachment_id, session_id, file_path, name, mime_type, created_at) VALUES (?, ?, ?, ?, ?, ?)`)
        .run(childAttachmentId, childId, row.file_path, row.name, row.mime_type, this.context.clock());

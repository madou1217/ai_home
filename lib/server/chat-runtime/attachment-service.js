...
const { documentFromUpload, persistChatDocuments } = require('../chat-document-attachments');
...
class ChatRuntimeAttachmentService {
  ...
  upload(sessionId, input = {}) {
    ...
    const filePaths = uploads.flatMap((upload) => upload.document
      ? persistChatDocuments([upload.document], persistenceOptions)
      : this.persistImages([upload.dataUrl], persistenceOptions));
...
function normalizeUploads(value) {
  ...
  return value.map((upload) => {
    ...
    if (!SUPPORTED_IMAGE_MIME_TYPES.has(mimeType)) {
      try {
        const document = documentFromUpload({ name, mimeType, dataUrl });
        return { name, mimeType: document.mimeType, dataUrl, document };
      } catch (error) {
        throw new ChatRuntimeError('chat_attachment_mime_unsupported', 422, { mimeType, message: error.message });
      }
    }

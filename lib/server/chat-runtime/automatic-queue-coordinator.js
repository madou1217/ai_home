...
class AutomaticQueueCoordinator {
  ...
  async interveneAtToolBoundary(action) {
    ...
    if (session?.activeTurn?.runId !== action.runId
      || !['running', 'waiting_input'].includes(session.state)) return null;
  ...
  async dispatchAfterTurn(action) {
    ...
    const session = this.store.getSession(action.sessionId);
    if (this.closed || session?.state !== 'idle' || session.policy?.queueControl?.paused
      || session.policy?.queueControl?.lastRunId !== action.runId
      || !hasQueuedPolicy(this.store, action.sessionId, action.policy)) {
      return null;
    }
    return this.dispatchCommand(action.sessionId, {
      commandId: automaticCommandId(action),
      type: 'queue.dispatch',
      payload: { policy: action.policy, afterRunId: action.runId }
    });
  }
...
function interventionCommand(action, item) {
  return {
    ...
    type: 'turn.intervene',
    payload: { mode: 'steer_current', content: item.payload.content, expectedRunId: action.runId }
  };
}

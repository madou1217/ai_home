...
function listCommands(session = {}, driver = {}) {
  const snapshot = record(session.capabilitySnapshot);
  const commands = [{ id: 'turn.submit', type: 'turn.submit' }, { id: 'turn.retry', type: 'turn.retry' }];
  if (session.policy?.workspaceMode === 'chat'
    || driver.historyBranch === true) {
    commands.push(...['session.fork', 'turn.regenerate'].map(commandEntry));
  }
  ...
  if (supports(snapshot, 'session.goal')) {
    commands.push(...['session.goal.set', 'session.goal.get', 'session.goal.clear'].map(commandEntry));
  }

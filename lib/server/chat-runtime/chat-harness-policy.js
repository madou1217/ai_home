...
function budgetHistoryItems(items, contextWindow, percent = 60) {
  ...
  // DeepSeek Harness keeps the leading system/developer boundary while
  // compacting the conversational tail. Treat that prefix as an atomic unit so
  // a seed never resumes with the transcript but without its instructions.
  const pinned = leadingInstructionItems(source);
  const pinnedTokens = pinned.reduce((sum, item) => sum + estimateTextTokens(JSON.stringify(item)), 0);
  const units = historyUnits(source.slice(pinned.length));
  ...
  const kept = [...pinned, ...keptUnits.reverse().flat()];
  ...
  const paired = [];
  let seenUser = pinned.some(isUserHistoryItem);
  ...
  return { items: kept, truncated: kept.length !== source.length,
    appliedTokens, budgetTokens: budget, pinnedTokens };
}

function leadingInstructionItems(items) {
  const pinned = [];
  for (const item of items) {
    if (!isInstructionHistoryItem(item)) break;
    pinned.push(item);
  }
  return pinned;
}

function isInstructionHistoryItem(item) {
...
function isUserHistoryItem(item) {
  return String(item && item.role || '').trim().toLowerCase() === 'user';
}

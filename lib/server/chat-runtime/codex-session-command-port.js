...
function createCodexHandlers(port, options = {}) {
  return Object.freeze({
    ...
    'turn.intervene': ({ command, store, sessionId }) => {
      const expected = command.payload.expectedRunId;
      const session = expected ? store.getSession(sessionId) : null;
      if (expected && (session.activeTurn?.runId !== expected
        || !['running', 'waiting_input'].includes(session.state))) {
        throw new ChatRuntimeError('chat_queue_boundary_superseded', 409);
      }
      return port.intervene(command.payload);
    },

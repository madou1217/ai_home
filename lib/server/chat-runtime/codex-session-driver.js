...
class CodexSessionDriver {
  ...
  async readBoundedNativeSeed(threadId, active) {
    ...
    try {
      ...
      // Paginated Codex app-servers (0.154+) no longer expose full turns
      // through thread/read. Older proxies surface that mismatch as
      // `list_turns is not supported yet`. Overflow recovery must still be
      // able to create a clean thread and retry the current turn; history is
      // optional here because the durable AIH timeline remains authoritative.
      if (isUnsupportedThreadRead(error)) return [];
      throw error;
    }
...
function isUnsupportedThreadRead(error) {

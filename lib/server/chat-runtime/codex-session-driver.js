...
class CodexSessionDriver {
  ...
  startTurn(context = {}) {
    ...
    const { appendDocumentPathsToPrompt } = require('../chat-document-attachments');
    const imagePaths = attachmentPaths.filter((file) => guessAttachmentMimeType(file));
    const prompt = appendDocumentPathsToPrompt(text(context.command && context.command.payload.content),
      attachmentPaths.filter((file) => !guessAttachmentMimeType(file)));

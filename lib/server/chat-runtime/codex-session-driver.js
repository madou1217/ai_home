...
class CodexSessionDriver {
  ...
  compactThread(context = {}) {
    ...
    this.requireContext(context);
    if (this.active) throw new ChatRuntimeError('chat_turn_already_active', 409);
    if (!this.nativeThreadId) throw new ChatRuntimeError('codex_native_session_missing', 422);
  ...
  async launch(active) {
    ...
    const result = await this.client.request('turn/start', chatTurnParams(protocol.buildTurnStartParams({

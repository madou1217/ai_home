...
const { APP_SERVER_PINS_GATEWAY_PROVIDER } = require('../codex-app-server-endpoint');
const {
...
const { readCodexHistorySeed } = require('./codex-history-seed');
const { nativeHistoryParams } = require('./codex-native-history-policy');
const { CodexWorkBranch } = require('./codex-work-branch');
const { CodexSessionGoalSync } = require('./codex-session-goal-sync');
...
class CodexSessionDriver {
  constructor(options = {}) {
    ...
    this.readNativeResponseItem = options.readNativeResponseItem;
      ...
      (options.clientFactory || acquireAppServerClient)(clientOptions({
        ...
        accountIdentityValidator: options.accountIdentityValidator
          || createCodexAppServerAccountIdentityValidator({
            ...
            gatewayPinnedProvider: APP_SERVER_PINS_GATEWAY_PROVIDER
          })
    ...
    this.historyBranch = new CodexWorkBranch(this, options);
    ...
    this.goals = new CodexSessionGoalSync({ client: this.client,
      readSession: () => this.currentSession(),
      publish: (message, active) => this.bridge.forwardNotification(message, providerEventContext(active)).persisted
    });
    this.active = null;
    ...
    this.recovery = new CodexTurnRecovery({
      ...
      syncGoal: (active, client) => this.goals.refresh(active, client),
  ...
  async acquireThread(active) {
    if (this.nativeThreadId) {
      ...
      await this.goals.refresh(active);
      markTrace(active, 'sessionBound', { runtimeId: this.runtimeScope });
      return;
    }
    const params = nativeHistoryParams(chatThreadParams(protocol.buildThreadStartParams({
      approvalMode: active.approvalMode,
  ...
  bind(active) {
    this.client.bindTurn(active.nativeThreadId, {
      ...
      onReconnectResume: (response, historyClient) => this.recovery.reconnect(active, response, historyClient),
  ...
  observe(active, message, serverRequest) {
    ...
    if (!serverRequest && !this.goals.accepts(message)) return;
    ...
    this.goals.observe(observed.mapped, observed.persisted);
  ...
  async retryAfterContextOverflow(active) {
    ...
    const replacement = { threadId: previousThreadId, nativeTurnId: active.nativeTurnId,
    ...
    // Replace the bound thread and clear its old turn anchor atomically. The
    // same AIH run will acquire a new native turn only after this succeeds.
  ...
  async readBoundedNativeSeed(threadId, active) {
    if (!threadId) return [];
    const policy = this.getSessionPolicy() || this.session.policy;
    return readCodexHistorySeed(this.client, threadId, {
      excludeTurnId: active.nativeTurnId,
      contextWindow: active.threadConfig?.model_context_window || policy?.contextState?.contextWindow,
      percent: policy?.documentBudgetPercent,
      initialHistory: this.initialHistory,
      readNativeItem: this.readNativeResponseItem
    });
  }

  resumeParams(currentApprovalMode, active) {
    const params = nativeHistoryParams(chatThreadParams(protocol.buildThreadResumeParams({

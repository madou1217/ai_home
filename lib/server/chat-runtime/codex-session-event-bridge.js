...
class CodexSessionEventBridge {
  constructor(options = {}) {
    ...
    this.nativeResponseItemSink = options.nativeResponseItemSink;
    this.nativeHistoryCoverageSink = options.nativeHistoryCoverageSink;
    this.nativeThreadItemSink = options.nativeThreadItemSink;
    this.userMessageLinks = new CodexUserMessageLink();
  ...
  forward(message, context, client, toolOrder) {
    const rawMessageId = this.userMessageLinks.observe(message);
    let captured = Promise.resolve();
    if (['item/started', 'item/completed'].includes(message.method) && this.nativeThreadItemSink) {
      const { threadId, turnId, item } = message.params || {};
      if (threadId && turnId && item?.id) {
        const write = this.writeChain.then(() => this.nativeThreadItemSink({ threadId, turnId, item, rawMessageId }));
        this.writeChain = write.catch(() => this.nativeHistoryCoverageSink?.({ threadId, turnId, boundary: 'gap' })).catch(() => {});
        captured = write;
      }
    }
    const result = this.forwardMapped(message, context, client, toolOrder);
    return { ...result, persisted: Promise.all([captured, result.persisted]) };
  }

  forwardMapped(message, context, client, toolOrder) {
    ...
    if (TURN_EVENTS.has(mapped.type) && message.params?.threadId && providerTurnId) {
      persisted = this.markHistoryCoverage(message.params.threadId, providerTurnId,
        mapped.type === 'turn.started' ? 'started' : 'completed');
    }
    if (shouldRoute) {
      let flushed = Promise.resolve();
      if (TERMINAL_TURN_EVENTS.has(mapped.type) && toolOrder) {
        flushed = toolOrder.flush((ordered) => this.routeToolEvent(ordered, context));
      }
      const scheduled = toolOrder
        ? toolOrder.schedule(event, (ordered) => this.routeToolEvent(ordered, context))
        : this.routeToolEvent(event, context);
      persisted = Promise.all([persisted, flushed, scheduled]);
    }
  ...
  forwardRawToolOrder(message, context, toolOrder) {
    ...
    let rawPersisted = Promise.resolve();
    if (message.method === 'rawResponseItem/completed' && !params.item?.id && params.turnId) {
      rawPersisted = this.markHistoryCoverage(params.threadId, params.turnId, 'gap');
    }
    if (message.method === 'rawResponseItem/completed' && params.item?.id
      && this.nativeResponseItemSink) {
      const write = this.writeChain.then(async () => {
        try {
          return await this.nativeResponseItemSink({
            threadId: params.threadId, turnId: params.turnId || '', item: params.item
          });
        } catch (error) {
          if (this.nativeHistoryCoverageSink && params.turnId) await this.nativeHistoryCoverageSink({
            threadId: params.threadId, turnId: params.turnId, boundary: 'gap'
          });
          throw error;
        }
      });
    ...
    // Capture raw call/output and still advance the ordering coordinator. An
    // early return here would strand typed events waiting for their raw result.
    const ordered = toolOrder.observe(message, (event) => this.routeToolEvent(event, context));
    const persisted = Promise.all([rawPersisted, ordered]);
    return { event, mapped, persisted, providerTurnId: '' };
  }

  createToolOrder() { return new CodexToolOrderCoordinator(); }

  markHistoryCoverage(threadId, turnId, boundary) {
    if (boundary === 'gap') this.userMessageLinks.reset(threadId, turnId);
    if (!this.nativeHistoryCoverageSink) return this.writeChain;
    const write = this.writeChain.then(() => this.nativeHistoryCoverageSink({ threadId, turnId, boundary }));
    this.writeChain = write.catch(() => {});
    return write;
  }
  ...
  flushToolOrder(toolOrder) {
    ...
    const flush = toolOrder.flush((ordered) => this.routeToolEvent(ordered));
  ...
  routeToolEvent(event, context = {}) {
    const item = event && event.payload && event.payload.item;
    const kind = item && item.kind;
    const isTool = ['tool', 'shell', 'file_change'].includes(kind);
    if (!isTool || !this.extensions) return this.route(event);
    // These hooks bracket event persistence, not native tool execution. Keep
    // awaited hooks inside the same write chain so they cannot reorder items.
    const before = event.type === 'timeline.item.started'
      ? () => this.extensions.runToolHook('beforeToolCall', structuredClone(event), context) : null;
    const after = event.type === 'timeline.item.completed'
      ? () => this.extensions.runToolHook('afterToolCall', structuredClone(event), context) : null;
    return this.route(event, before, after);
  }

...
const { readCodexSessionHistory, projectCodexSessionHistory } = require('./codex-session-history');
...
class CodexSessionHistorySync {
  ...
  importRecovered(response, recoveryAnchor) {
    if (typeof this.historySink !== 'function') return { imported: 0, skipped: true };
    return this.historySink(projectCodexSessionHistory(response, {
      threadId: this.getThreadId(), runtimeId: this.runtimeId, recoveryAnchor
    }));
  }
}

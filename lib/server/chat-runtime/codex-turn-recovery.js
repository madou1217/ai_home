...
const { hydrateCodexHistoryResponse } = require('./codex-history-pages');
...
class CodexTurnRecovery {
  ...
  async attach(active, pendingInteractions) {
    ...
    const response = await this.client.request(
      'thread/resume',
      nativeHistoryParams(protocol.buildThreadResumeParams({
        approvalMode: active.approvalMode,
        threadId: active.nativeThreadId
      }))
    );
    const snapshot = await this.restoreSnapshot(active, response);
    if (this.syncGoal) await this.syncGoal(active, this.client);
  ...
  async restoreSnapshot(active, response, historyClient = this.client) {
    // Mark the known running turn before any fallible paging. Even a failed
    // resume must not allow later terminal notifications to certify the gap.
    if (active.nativeTurnId && typeof this.bridge?.markHistoryCoverage === 'function') {
      await this.bridge.markHistoryCoverage(active.nativeThreadId, active.nativeTurnId, 'gap');
    }
    const hydrated = await hydrateCodexHistoryResponse(historyClient, response, { threadId: active.nativeThreadId });
    const snapshot = recoveredTurnSnapshot(hydrated, {
      nativeTurnId: active.nativeTurnId,
      clientUserMessageId: active.clientUserMessageId || active.context.runId
    });
    if (snapshot.id !== active.nativeTurnId && typeof this.bridge?.markHistoryCoverage === 'function') {
      await this.bridge.markHistoryCoverage(active.nativeThreadId, snapshot.id, 'gap');
    }
    ...
    if (this.importRecoveredHistory) await this.importRecoveredHistory(hydrated, {
  ...
  async reconnect(active, response, historyClient) {
    if (this.getActive() !== active || active.settled) return;
    await this.bridge.writeChain;
    if (this.getActive() !== active || active.settled) return;
    const snapshot = await this.restoreSnapshot(active, response, historyClient);
    if (this.syncGoal) await this.syncGoal(active, historyClient);

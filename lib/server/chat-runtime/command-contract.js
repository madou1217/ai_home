...
const PAYLOAD_NORMALIZERS = Object.freeze({
  ...
  'turn.retry': normalizeTurnRetry,
  'turn.regenerate': normalizeMessageOperation,
  'session.fork': normalizeMessageOperation,
  ...
  'session.goal.set': normalizeGoalSet,
  'session.goal.clear': normalizeGoalClear,
...
function normalizeGoalSet(payload) {
  const unknown = Object.keys(payload).filter((key) => !['objective', 'tokenBudget'].includes(key));
  if (unknown.length > 0) {
    throw new ChatRuntimeError('chat_goal_payload_invalid', 422, { unknown });
  }
  payload.objective = requiredText(payload.objective, 'chat_goal_objective_required');
  if (payload.objective.length > 16000) {
    throw new ChatRuntimeError('chat_goal_objective_too_large', 422);
  }
  if (payload.tokenBudget !== undefined && payload.tokenBudget !== null) {
    if (!Number.isSafeInteger(payload.tokenBudget) || payload.tokenBudget < 1) {
      throw new ChatRuntimeError('chat_goal_token_budget_invalid', 422);
    }
  } else payload.tokenBudget = null;
  return payload;
}

function normalizeGoalClear(payload) {
  if (Object.keys(payload).length > 0) {
    throw new ChatRuntimeError('chat_goal_payload_invalid', 422, {
...
function normalizeMessageOperation(payload) {
  if (Object.keys(payload).some((key) => key !== 'sourceItemId')) {
    throw new ChatRuntimeError('chat_message_operation_payload_invalid', 422);
  }
  return { sourceItemId: requiredText(payload.sourceItemId, 'chat_source_item_required') };
}

function normalizeTurnRetry(payload) {
  if (Object.keys(payload).some((key) => key !== 'sourceTurnId')) {
    throw new ChatRuntimeError('chat_retry_payload_invalid', 422);
  }
  return { sourceTurnId: requiredText(payload.sourceTurnId, 'chat_retry_source_turn_required') };
}
...
function normalizeTurnIntervene(payload) {
  if (payload.expectedRunId !== undefined) payload.expectedRunId = requiredText(payload.expectedRunId, 'chat_run_id_required');
...
function normalizeQueueDispatch(payload) {
  if (payload.afterRunId !== undefined) payload.afterRunId = requiredText(payload.afterRunId, 'chat_run_id_required');

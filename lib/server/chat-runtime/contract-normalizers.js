...
const { normalizeMessageMetrics } = require('./message-metrics-contract');
...
function normalizeEventPayload(type, value, eventTurnId) {
  ...
  if (type === 'session.goal.updated') {
    if (payload.goalSource !== undefined && !['aih', 'native'].includes(payload.goalSource)) {
      throw new ChatRuntimeError('chat_goal_source_invalid', 422);
    }
    return { goal: normalizeGoal(payload.goal),
      ...(payload.goalSource ? { goalSource: payload.goalSource } : {}) };
  }
  if (type === 'session.goal.cleared') {
    return payload.goalId === undefined ? {} : { goalId: requiredText(payload.goalId, 'chat_goal_id_invalid') };
  }
  if (type === 'turn.metrics.updated') return { metrics: normalizeMessageMetrics(payload.metrics),
    ...(payload.totals === undefined ? {} : { totals: normalizeMessageMetrics(payload.totals) }) };
...
function normalizeGoal(value) {
  const goal = cloneRecord(value, 'chat_goal_invalid');
  const status = requiredText(goal.status, 'chat_goal_status_invalid');
  if (!GOAL_STATUSES.has(status)) throw new ChatRuntimeError('chat_goal_status_invalid', 422, { status });
  const normalized = {
    threadId: requiredText(goal.threadId, 'chat_goal_thread_invalid'),
    objective: requiredText(goal.objective, 'chat_goal_objective_invalid'),
    status,
    tokensUsed: nonNegativeInteger(goal.tokensUsed, 'chat_goal_tokens_used_invalid'),
    timeUsedSeconds: nonNegativeInteger(goal.timeUsedSeconds, 'chat_goal_time_used_invalid'),
    createdAt: nonNegativeInteger(goal.createdAt, 'chat_goal_created_at_invalid'),
    updatedAt: nonNegativeInteger(goal.updatedAt, 'chat_goal_updated_at_invalid')
  };
  if (goal.tokenBudget !== null && goal.tokenBudget !== undefined) {
    normalized.tokenBudget = nonNegativeInteger(goal.tokenBudget, 'chat_goal_token_budget_invalid');
  } else normalized.tokenBudget = null;
  return normalized;
}
...
function normalizeSnapshot(input = {}) {
  ...
  ['runtimeBinding', 'capabilitySnapshot', 'activeTurn', 'failedTurn'].forEach((key) => {

...
const { TurnMetricsRepository } = require('./turn-metrics-repository');
const { projectChatContextInTransaction } = require('./chat-context-state');
const { createToolHistoryIntegrityGuard } = require('./tool-history-integrity');

class EventRepository {
  constructor(context) {
    this.context = context;
    this.turnMetrics = new TurnMetricsRepository(context, this);
  }
  ...
  appendInTransaction(sessionId, draft = {}, options = {}) {
    const { measureTurn = true } = options;
    ...
    const toolHistoryIntegrity = options.toolHistoryIntegrity
      || createToolHistoryIntegrityGuard(this.context, sessionId);
    toolHistoryIntegrity.assert([event]);
    db.prepare(`
    ...
    if (measureTurn) this.turnMetrics.observeInTransaction(event);
    if (measureTurn && (event.type === 'session.goal.updated' || event.type === 'session.goal.cleared'
      || event.type === 'turn.metrics.updated' || event.type === 'timeline.item.started'
      || event.type === 'timeline.item.completed' || event.type === 'turn.failed' || event.type === 'turn.interrupted'
      || event.type === 'run.lost')) {
      projectChatContextInTransaction(this.context, this, event);
    }
    return event;
  }

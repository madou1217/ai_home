...
class FailedTurnRepository {
  ...
  read(sessionId) {
    ...
    `).get(sessionId);
    if (!latest || latest.type !== 'turn.failed' || !latest.turn_id) return null;
  ...
  requireSubmission(sessionId, sourceTurnId) {
    ...
    if (session?.state !== 'idle' || latest?.failure.turnId !== sourceTurnId || !latest.command) {

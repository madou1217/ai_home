...
class RecoveryRepository {
  constructor(context, events, sessions) {
  ...
  begin(sessionId) {
    return withTransaction(this.context.db, () => {
      ...
      const activeTurn = { ...session.activeTurn, state: 'recovering',
        interruptRequested: Boolean(session.activeTurn.interruptRequested || session.state === 'interrupting') };
  ...
  complete(sessionId, input = {}) {
    return withTransaction(this.context.db, () => {
      ...
      const state = session.activeTurn.interruptRequested ? 'interrupting'
        : interactions.length > 0 ? 'waiting_input' : 'running';
      const activeTurn = {
        ...session.activeTurn,
        state,
  ...
  fail(sessionId, error = {}) {
    return withTransaction(this.context.db, () => {
      ...
      const terminal = { type: 'run.lost', turnId: activeTurn?.turnId, runId: activeTurn?.runId,
        at: this.context.clock(), source: runtimeSource(session), payload: { error: normalizeError(error) } };
      if (activeTurn) {
        this.events.turnMetrics.finishInTransaction(sessionId, terminal);
        settleTurnTimeline(this.context, this.events, sessionId, terminal);
      }
      this.storage.failAcceptedCommands(sessionId, restartFailure());
      ...
      this.pauseQueue(session);
      if (activeTurn) this.events.appendInTransaction(sessionId, terminal);
  ...
  convergeOrphans(session) {
    ...
    if (lost) {
      this.pauseQueue(session);
      this.appendRunEvent(session, null, 'run.lost', { error: restartFailure() });
    }
  ...
  pauseQueue(session) {
    this.sessions.updatePolicyInTransaction(session.sessionId, { queueControl: {
      ...session.policy.queueControl, paused: true, reason: 'run_lost'
    } });
  }

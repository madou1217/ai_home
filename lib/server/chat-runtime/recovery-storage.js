...
class RecoveryStorage {
  ...
  failAcceptedCommands(sessionId, result) {
    // Message operations commit an immutable child before launching work. They
    // can resume by command ID; ordinary in-flight commands remain fail-closed.
    this.context.db.prepare(`
      ...
      WHERE session_id = ? AND status = 'accepted'
        AND type NOT IN ('session.fork', 'turn.regenerate')

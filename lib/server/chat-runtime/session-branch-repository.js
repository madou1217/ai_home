...
class SessionBranchRepository {
  ...
  create(command) {
    return withTransaction(this.context.db, () => {
      ...
      const { legacySessionId: _legacy, lineage: _lineage, archived: _archived,
        contextState: _context, queueControl: _queue, ...policy } = source.policy;
      ...
      for (const item of responseItems.filter((entry) => entry.type === 'reasoning')) {
        this.store.nativeResponseItems.recordInTransaction(sessionId, {
          threadId: source.runtimeBinding.nativeSessionId || 'inherited-seed', item
        });
      }

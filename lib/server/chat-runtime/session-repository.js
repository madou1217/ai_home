...
class SessionRepository {
  ...
  createInTransaction(input) { return this.insertInTransaction(sessionDraft(input, this.context)); }
  ...
  list(filters = {}) {
    ...
    addFilter(clauses, values, "json_extract(policy_json, '$.workspaceMode')", filters.workspaceMode);
    addFilter(clauses, values, 'execution_account_ref', filters.executionAccountRef);
  ...
  updateRuntimeBinding(sessionId, patch = {}, replacement) {
    return withTransaction(this.context.db, () => {
      ...
      if (replacement) {
        if (session.activeTurn?.runId !== replacement.runId
          || session.activeTurn?.nativeTurnId !== replacement.nativeTurnId
          || session.runtimeBinding.nativeSessionId !== replacement.threadId
          || !patch.nativeSessionId || patch.nativeSessionId === replacement.threadId) {
          throw new ChatRuntimeError('chat_native_thread_replacement_stale', 409);
        }
        const { nativeTurnId: _oldAnchor, ...activeTurn } = session.activeTurn;
        this.updateState(sessionId, session.state, activeTurn);
      }
      this.context.db.prepare(`
  ...
  updatePolicy(sessionId, patch = {}) {
    return withTransaction(this.context.db, () => this.updatePolicyInTransaction(sessionId, patch));
  }

  updatePolicyInTransaction(sessionId, patch = {}) {
    const session = this.require(sessionId);

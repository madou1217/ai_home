...
class SessionResolutionCoordinator {
  ...
  async resolve(input = {}) {
    ...
    if (text(input.sessionId)) {
      const session = this.store.getSession(text(input.sessionId));
      if (!session) throw new ChatRuntimeError('chat_session_not_found', 404);
      if (isChatSession(session) || session.provider !== sessionInput.provider
        || session.executionAccountRef !== sessionInput.executionAccountRef
        || session.projectPath !== sessionInput.projectPath
        || (sessionInput.runtimeBinding.nativeSessionId
          && sessionInput.runtimeBinding.nativeSessionId !== session.runtimeBinding.nativeSessionId)) {
        throw new ChatRuntimeError('chat_session_identity_mismatch', 409);
      }
    ...
    const existing = sessionInput.runtimeBinding.nativeSessionId
      ...
      : null;
    if (existing) {
      if (isChatSession(existing)) throw new ChatRuntimeError('chat_session_requires_chat_identity', 409);
  ...
  async rebind(existing, input) {
    if (isChatSession(existing)) throw new ChatRuntimeError('chat_session_requires_chat_identity', 409);
    if (existing.policy.lineage) throw new ChatRuntimeError('chat_branch_account_mismatch', 409);

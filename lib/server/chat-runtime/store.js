...
const { FailedTurnRepository } = require('./failed-turn-repository');
...
const { SessionBranchRepository } = require('./session-branch-repository');
const { settleTurnTimeline } = require('./timeline-settlement');
const { NativeResponseItemRepository } = require('./native-response-item-repository');
const { BranchOperationRepository } = require('./branch-operation-repository');
const { WorkBranchRepository } = require('./work-branch-repository');
const { NativeThreadItemRepository } = require('./native-thread-item-repository');
...
class ChatRuntimeStore {
  constructor(context) {
    ...
    this.nativeResponseItems = new NativeResponseItemRepository(context, this.sessions);
    this.attachments = new AttachmentRepository(context, this.sessions);
    this.commands = new CommandRepository(context);
    this.failedTurns = new FailedTurnRepository(context);
    this.queue = new QueueRepository(context, this.events);
    this.interactions = new InteractionRepository(context, this.events);
    this.recovery = new RecoveryRepository(context, this.events, this.sessions);
    this.timelineImports = new TimelineImportRepository(context, this.events);
    this.branches = new SessionBranchRepository(this);
    this.branchOperations = new BranchOperationRepository(this);
    this.workBranches = new WorkBranchRepository(this);
    this.nativeThreadItems = new NativeThreadItemRepository(this);
  }

  createSession(input) { return this.sessions.create(input); }
  branchSession(command) { return this.branches.create(command); }
  readHistorySeed(sessionId) { return this.branches.read(sessionId); }
  recordNativeResponseItem(sessionId, evidence) { return this.nativeResponseItems.record(sessionId, evidence); }
  readNativeResponseItem(sessionId, itemId) { return this.nativeResponseItems.read(sessionId, itemId); }
  markNativeHistoryCoverage(sessionId, evidence) { return this.nativeResponseItems.markCoverage(sessionId, evidence); }
  ...
  updateRuntimeBinding(sessionId, patch, replacement) {
    return this.sessions.updateRuntimeBinding(sessionId, patch, replacement);
  }
  ...
  setGoal(sessionId, goal, options = {}) {
    return withTransaction(this.context.db, () => {
      const session = this.sessions.require(sessionId);
      const policy = { ...session.policy,
        contextState: { ...(session.policy.contextState || {}), goal: structuredClone(goal),
          ...(options.source ? { goalSource: String(options.source) } : {}) } };
      this.context.db.prepare(`
        UPDATE chat_runtime_sessions SET policy_json = ?, updated_at = ? WHERE session_id = ?
      `).run(JSON.stringify(policy), this.context.clock(), sessionId);
      const event = this.events.appendInTransaction(sessionId, {
        type: 'session.goal.updated',
        source: runtimeSource(session),
        payload: { goal, ...(options.source ? { goalSource: options.source } : {}) }
      });
      return { goal, event, session: this.sessions.require(sessionId) };
    });
  }
  clearGoal(sessionId) {
    return withTransaction(this.context.db, () => {
      const session = this.sessions.require(sessionId);
      const contextState = { ...(session.policy.contextState || {}) };
      delete contextState.goal;
      delete contextState.goalSource;
      const policy = { ...session.policy, contextState };
      this.context.db.prepare(`
        UPDATE chat_runtime_sessions SET policy_json = ?, updated_at = ? WHERE session_id = ?
      `).run(JSON.stringify(policy), this.context.clock(), sessionId);
      const event = this.events.appendInTransaction(sessionId, {
        ...
        source: runtimeSource(session),
        payload: { goalId: session.policy.contextState?.goal?.threadId }
      });
      return { goal: null, event, session: this.sessions.require(sessionId) };
    });
  }
  ...
  beginTurn(sessionId, input = {}) {
    return withTransaction(this.context.db, () => {
      if (input.resumeQueue) {
        const control = this.sessions.require(sessionId).policy.queueControl;
        if (control?.paused) this.sessions.updatePolicyInTransaction(sessionId, {
          queueControl: { ...control, paused: false }
        });
      }
      if (input.policy && Object.keys(input.policy).length > 0) {
        this.sessions.updatePolicyInTransaction(sessionId, input.policy);
      }
  ...
  getRetrySubmission(sessionId, sourceTurnId) {
    return this.failedTurns.requireSubmission(sessionId, sourceTurnId);
  }
  ...
  settleTurn(sessionId, input = {}) {
    return withTransaction(this.context.db, () => {
      this.events.turnMetrics.finishInTransaction(sessionId, input.event);
      const outcomeUnknown = settleTurnTimeline(this.context, this.events, sessionId, input.event);
      this.sessions.updateState(sessionId, 'idle', null);
      const wasPaused = this.sessions.require(sessionId).policy.queueControl?.paused;
      this.sessions.updatePolicyInTransaction(sessionId, { queueControl: {
        paused: Boolean(wasPaused) || outcomeUnknown || input.event.type !== 'turn.completed', lastRunId: input.event.runId,
        reason: outcomeUnknown ? 'tool_outcome_unknown' : input.event.type === 'turn.interrupted' ? 'user_stop'
          : input.event.type === 'turn.failed' ? 'turn_failed' : 'completed'
      } });
      ...
      const queue = input.queue
        ...
        : null;
      const draft = outcomeUnknown && input.event.type === 'turn.failed'
        ? { ...input.event, payload: { ...input.event.payload, retryable: false, outcomeUnknown: true } }
        : input.event;
      const event = this.events.appendInTransaction(sessionId, draft);
      return { event, queue, session: this.sessions.require(sessionId) };
    });
  }
  ...
  getSnapshot(sessionId) {
    ...
    const failedTurn = this.failedTurns.get(sessionId);
    if (failedTurn) snapshot.failedTurn = failedTurn;
...
function runtimeSource(session) {
  return {
    provider: String(session && session.provider || 'unknown'),
    runtimeId: String(session && session.runtimeBinding && session.runtimeBinding.runtimeId || 'unbound')
  };
}

...
function normalizeTimelineDetail(kind, input) {
  ...
  if (['message', 'reasoning'].includes(kind) && detail.metrics !== undefined) {
    detail.metrics = normalizeMessageMetrics(detail.metrics);
  }
...
function validateShell(detail) {
  ...
  optionalIntegerField(detail, 'exitCode');
  // Codex ThreadItem.commandExecution uses an opaque string ID (96883599).
  // Preserve older AIH numeric records while accepting native IDs losslessly.
  if (typeof detail.processId === 'string') requiredTextField(detail, 'processId');
  else optionalNonNegativeIntegerField(detail, 'processId');
}

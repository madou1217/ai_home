...
class TurnLifecycle {
  ...
  submit(command, queue, trace) {
    ...
    let providerRun;
    try {
      const context = this.driverContext(run, command);
      providerRun = command.type === 'slash.execute'
        ? this.driver.compactThread(context) : this.driver.startTurn(context);
    } catch (error) {
  ...
  retry(command, trace) {
    this.ensureCanSubmit();
    const source = this.store.getRetrySubmission(this.sessionId, command.payload.sourceTurnId);
    return this.submit({ ...command, type: 'turn.submit', payload: source.payload,
      submissionCommandId: source.commandId }, null, trace);
  }
  ...
  createRun(command, queue, trace, imagePaths) {
    ...
    const run = {
      ...
      runId,
      startedAt: Date.now(),
      submissionCommandId: !queue && command.type === 'turn.submit'
        ? command.submissionCommandId || command.commandId : undefined,
  ...
  begin(run, command) {
    ...
    this.store.beginTurn(this.sessionId, {
      activeTurn,
      resumeQueue: command.type === 'turn.submit',
      ...(command.type === 'turn.submit' ? { policy: {
        ...(session.policy.workspaceMode === 'chat' && session.policy.title === '新对话' && content
      ...
      event: this.eventDraft('turn.queued', run, {
        state: 'starting',
        ...(run.submissionCommandId ? { submissionCommandId: run.submissionCommandId } : {}),
        activeTurn
      })
    });
  }
  ...
  activeTurn(run, state) {
    ...
    const activeTurn = {
      ...
      ...(existing.startedAt || run.startedAt ? { startedAt: existing.startedAt || run.startedAt } : {}),
      ...(existing.firstTokenAt === undefined ? {} : { firstTokenAt: existing.firstTokenAt }),
      ...(existing.metrics ? { metrics: existing.metrics } : {}),
      ...(existing.usageTotals ? { usageTotals: existing.usageTotals } : {}),
      ...(existing.nativeTurnId || run.nativeTurnId
        ? { nativeTurnId: existing.nativeTurnId || run.nativeTurnId }
        : {}),
      state
    };
    if (state === 'interrupting' || run.interruptRequested) activeTurn.interruptRequested = true;
    return activeTurn;
  }

...
class TurnMetricsRepository {
  ...
  finishInTransaction(sessionId, terminal) {
    ...
    `).all(sessionId, active.turnId);
    const items = projectTimeline(rows.map((entry) => ({ type: entry.type, payload: JSON.parse(entry.payload_json) })));
    ...
    if (!target) return;
    const status = terminal.type === 'turn.failed' ? 'failed'

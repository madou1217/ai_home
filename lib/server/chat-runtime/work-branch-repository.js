...
class WorkBranchRepository {
  ...
  capture(command) {
    ...
    if (!target || target.kind !== 'message' || target.status !== 'completed'
      || target.detail.phase === 'interaction_answer' || !['user', 'assistant'].includes(target.detail.role)) {
  ...
  existing(command) {
    const child = this.store.getSession(branchSessionId(command.commandId));
  ...
  commit(command) {
    return withTransaction(this.context.db, () => {
      ...
      const inheritedGoal = projection.policy.contextState && projection.policy.contextState.goal;
      const inheritedGoalSource = projection.policy.contextState && projection.policy.contextState.goalSource;
      const sessionId = branchSessionId(command.commandId);
      this.store.sessions.createInTransaction({ sessionId, provider: source.provider,
        ...
        policy: { ...policy, contextState: inheritedGoal ? { goal: inheritedGoal,
          ...(inheritedGoalSource ? { goalSource: inheritedGoalSource } : {}) } : {}, title: `${policy.title || '工作会话'} · ${command.type === 'turn.regenerate' ? '重新生成' : '分支'}`,

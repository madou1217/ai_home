...
function resolveTokenExpiryMs(account) {
  // Refresh Token 可能比 Access Token 更早到期；错过前者会失去续期能力。
  const deadlines = [account && account.tokenExpiresAt, account && account.refreshTokenExpiresAt]
    .map(Number)
...
function persistClaudeOAuthSnapshot(account, tokens, nowMs, deps = {}) {
  ...
  if (Number.isFinite(tokens.refreshTokenExpiresAt) && tokens.refreshTokenExpiresAt > 0) {
    nextOauth.refreshTokenExpiresAt = tokens.refreshTokenExpiresAt;
    if (Object.prototype.hasOwnProperty.call(nextOauth, 'refresh_token_expires_at')) {
      nextOauth.refresh_token_expires_at = tokens.refreshTokenExpiresAt;
    }
  }
...
async function refreshClaudeAccessToken(account, options = {}, deps = {}) {
  ...
  const refreshTask = (async () => {
    try {
      ...
      // 官方响应给出相对秒数；未提供时保留原期限，不推测或延长 grant 的寿命。
      const refreshLifetime = payload.refresh_token_expires_in;
      const refreshLifetimeSeconds = typeof refreshLifetime === 'number'
        || (typeof refreshLifetime === 'string' && refreshLifetime.trim())
        ? Number(refreshLifetime)
        : NaN;
      const refreshTokenExpiresAt = Number.isFinite(refreshLifetimeSeconds) && refreshLifetimeSeconds >= 0
        ? nowMs + refreshLifetimeSeconds * 1000
        : readClaudeOauthCredential({ credentials }).refreshTokenExpiresAt;
      ...
      account.refreshTokenExpiresAt = refreshTokenExpiresAt || null;

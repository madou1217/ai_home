...
const DEFAULT_CLAUDE_OAUTH_TOKEN_URL = 'https://api.claude.ai/api/oauth/token';
...
function resolveTokenExpiryMs(account) {
  ...
  const expiryIso = parseIsoTimestampMs(oauth.expiry);
...
function persistClaudeOAuthSnapshot(account, tokens, nowMs) {
  const configDir = String(account && account.configDir || '').trim();
  if (!configDir) return false;

  const credentialsPath = path.join(configDir, '.credentials.json');
  const current = readJsonFileSafe(credentialsPath);

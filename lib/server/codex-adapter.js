...
function convertOpenAIResponsesToCodexPayload(requestJson, forcedModel, options = {}) {
  ...
  Object.entries(source).forEach(([key, value]) => {
    ...
    payload[key] = key === 'input' ? sanitizeCodexResponsesInput(value, options) : value;
  });
...
function sanitizeCodexResponsesInput(input, options = {}) {
  ...
  // 过滤掉不可持久化的无内容 reasoning 项，或清除 store: false 下无法跨请求引用的 item id
  const filtered = [];
  input.forEach((item) => {
    ...
    // store: false 或无状态池化网关下，上游不持久化服务端 item；客户端在下一轮输入中若传了历史 item.id，
    // 上游会报 404: Item with id 'rs_...' not found. Items are not persisted when store is set to false.
    // 因此在输入中安全清除历史 reasoning / message 的 id 引用，避免上游 404
    if (sanitizedItem.type === 'reasoning') {
      delete sanitizedItem.encrypted_content;
      delete sanitizedItem.id;
      // 如果 reasoning 项既没有 summary 也没有 content，则无需在无状态 input 中空转传入
      const hasSummary = Array.isArray(sanitizedItem.summary) && sanitizedItem.summary.length > 0;
      const hasContent = Array.isArray(sanitizedItem.content) && sanitizedItem.content.length > 0;
      if (!hasSummary && !hasContent) return;
    }
    filtered.push(sanitizedItem);
  });
  return filtered;
}

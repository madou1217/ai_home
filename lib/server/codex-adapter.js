...
function convertOpenAIResponsesToCodexPayload(requestJson, forcedModel, options = {}) {
  ...
  Object.entries(source).forEach(([key, value]) => {
    ...
    payload[key] = key === 'input' ? sanitizeCodexResponsesInput(value) : value;
  });
...
function sanitizeCodexResponsesInput(input) {
  ...
  return input.map((item) => {
    ...
    if (!expectedPrefix) return item;
    const id = toPlainText(item.id || '').trim();
    if (!id || id.startsWith(expectedPrefix)) return item;
    const { id: _invalidId, ...withoutInvalidId } = item;
    return withoutInvalidId;
  });
}

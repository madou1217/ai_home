'use strict';

const crypto = require('node:crypto');
const { createCodexChatStreamConverter, mapUsageFromCodexResponse } = require('./codex-chat-stream-converter');
const { forwardCodexResponseStream } = require('./codex-response-stream');
const { createDownstreamAbortContext } = require('./upstream-stream-forwarder');
const { isLoopbackUrl } = require('./http-utils');
const { resolveProxyConfig } = require('./http-utils-utils');
const { classifyUpstreamFailure, describeError } = require('./upstream-failure-policy');
const { applyAccountFailurePolicy } = require('./account-runtime-state');
const { runWithAccountAttempts } = require('./request-orchestrator');
const {
  applyProviderProtocolParameterStrategy
} = require('./provider-request-parameter-strategy');
const {
  applyModelCatalogSettingsToEntries
} = require('./model-catalog-settings-store');
const { selectPoolAccountsForModel } = require('./model-account-pool-selector');
const {
  buildNoAvailableAccountResponse,
  hasUnavailableReason
} = require('./account-availability');
const { writeUnavailableAccountResponse } = require('./upstream-failure-replay');
const { appendUpstreamFailureDiagnosticLog } = require('./diagnostic-log');
const { projectCodexModelCatalog } = require('./codex-model-metadata-projection');
const {
  fetchCodexModelCatalogForAccount,
  fetchCodexModelsForAccount,
  parseCodexModelCatalog,
  parseCodexModelsResponse,
  resolveCodexClientVersion,
  resolveCodexUpstreamBaseUrl
} = require('./codex-model-client');
const {
  resolveAccountEgressRequestOptions: resolveAccountEgressRequestOptionsDefault
} = require('./account-egress-request-options');

const HOP_BY_HOP_HEADERS = new Set([
  'connection',
  'keep-alive',
  'proxy-authenticate',
  'proxy-authorization',
  'te',
  'trailer',
  'transfer-encoding',
  'upgrade',
  'proxy-connection'
]);

const TRANSIENT_RETRY_DELAY_MS = 250;

function buildCodexModelListEntries(ids, settings, options = {}) {
  return applyModelCatalogSettingsToEntries(
    (Array.isArray(ids) ? ids : []).map((id) => ({ id, provider: 'codex' })),
    settings,
    { providerMode: options && options.provider }
  );
}

function buildCodexAccountModelListEntries(accountModels, settings, options = {}) {
  const entries = [];
  Object.entries(accountModels && typeof accountModels === 'object' ? accountModels : {}).forEach(([accountRef, models]) => {
    (Array.isArray(models) ? models : []).forEach((id) => {
      entries.push({ id, provider: 'codex', accountRef });
    });
  });
  if (entries.length < 1) return [];
  return applyModelCatalogSettingsToEntries(
    entries,
    settings,
    { providerMode: options && options.provider }
  );
}

function hasAccountModelEntries(accountModels) {
  return Object.values(accountModels && typeof accountModels === 'object' ? accountModels : {})
    .some((models) => Array.isArray(models) && models.length > 0);
}

function buildCodexNativeModelCatalog(modelIds, catalogByAccount) {
  const bySlug = new Map();
  Object.values(catalogByAccount && typeof catalogByAccount === 'object' ? catalogByAccount : {})
    .forEach((models) => {
      (Array.isArray(models) ? models : []).forEach((model) => {
        const slug = toPlainText(model && model.slug || '').trim();
        if (slug && !bySlug.has(slug)) bySlug.set(slug, model);
      });
    });
  return projectCodexModelCatalog(modelIds, Array.from(bySlug.values()));
}

// 收窄逻辑本身住在 model-account-pool-selector：codex 和 upstream-endpoints 曾各持一份
// 拷贝并各自漂移，这里只保留 codex 侧的入参适配。
function selectCodexAccountsForRequestModel(pool, model, state, options) {
  const requestedModel = toPlainText(model || '').trim();
  if (!requestedModel || !Array.isArray(pool) || pool.length < 1) {
    return { pool, filtered: false, accountRefs: [] };
  }
  return selectPoolAccountsForModel({
    pool,
    provider: 'codex',
    model: requestedModel,
    state,
    options: options || {}
  });
}

function shouldSkipForwardHeader(headerName) {
  const key = String(headerName || '').toLowerCase();
  return key === 'host'
    || key === 'authorization'
    || key === 'content-length'
    || HOP_BY_HOP_HEADERS.has(key);
}

function normalizeHeaderValue(value) {
  if (Array.isArray(value)) {
    return value.map((item) => String(item || '').trim()).filter(Boolean).join(', ');
  }
  return String(value == null ? '' : value).trim();
}

function isSafeHeaderValue(value) {
  return !/[\u0000-\u0008\u000A-\u001F\u007F]/.test(String(value || ''));
}

function sanitizeAccessToken(rawToken) {
  const token = String(rawToken || '').trim();
  if (!token) return '';
  if (/[\r\n\0]/.test(token)) return '';
  return token;
}

function describeUpstreamError(error) {
  return describeError(error);
}

// Network-failure diagnostics must name the EFFECTIVE egress, not env var
// names: the server normally proxies via its saved proxy_url, so pointing
// operators at AIH_SERVER_PROXY_URL sends them hunting in the wrong place.
// Mid-stream socket cuts additionally explain why no automatic retry happened
// (2026-09-11 UND_ERR_SOCKET forensics: relay/proxy cut long SSE streams).
function withNetworkHint(detail, codexBaseUrl, networkContext = {}) {
  const upstream = String(codexBaseUrl || '').trim();
  const context = networkContext || {};
  const parts = [String(detail || '').trim()];
  if (context.phase === 'mid-stream') {
    const elapsedMs = Number(context.elapsedMs);
    const after = Number.isFinite(elapsedMs) && elapsedMs > 0 ? ` after ${(elapsedMs / 1000).toFixed(1)}s` : '';
    parts.push(`hint: upstream connection dropped mid-stream${after}; partial output was already delivered, so the request was not retried`);
  } else {
    parts.push('hint: check codex upstream reachability');
  }
  if (upstream) {
    parts.push(`codex_upstream=${upstream}`);
  }
  // Proxy URLs may embed userinfo; never leak credentials into client-facing
  // diagnostics — strip them while keeping host/port visible.
  const proxyUrl = String(context.proxy && context.proxy.url || '')
    .trim()
    .replace(/^([a-z][a-z0-9+.-]*:\/\/)[^/@]*@/i, '$1');
  parts.push(proxyUrl ? `proxy=${proxyUrl}` : 'proxy=none (direct)');
  return parts.filter(Boolean).join(' | ');
}

function isTransientRetryPolicy(policy) {
  if (!policy || !policy.retryable || !policy.shouldRetryAnotherAccount) return false;
  if (Number(policy.failureThreshold) < 2) return false;
  return ['network_error', 'timeout', 'service_unavailable'].includes(String(policy.kind || ''));
}

function isTransportFailurePolicy(policy) {
  return ['network_error', 'timeout'].includes(String(policy && policy.kind || ''));
}

function applyFailurePolicyToAccount(account, policy, markProxyAccountFailure, defaultThreshold, model = '') {
  // Shared with the upstream-endpoints path: transport reachability failures
  // may opt out of account state entirely, while timeout policies can retain a
  // bounded streak before cooling.
  return applyAccountFailurePolicy(account, policy, {
    markProxyAccountFailure,
    defaultThreshold,
    model
  });
}

function parseCodexModels(options) {
  const raw = String((options && options.codexModels) || '').trim();
  if (!raw) return [];
  const out = raw
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean);
  return out;
}

function resolveCodexModel(requestedModel, options, state) {
  const requested = toPlainText(requestedModel || '').trim();
  if (requested) return requested;

  const cachedModels = Array.isArray(state && state.modelsCache && state.modelsCache.ids)
    ? state.modelsCache.ids.filter((item) => Boolean(toPlainText(item || '').trim()))
    : [];
  if (cachedModels.length > 0) return cachedModels[0];

  const configuredModels = parseCodexModels(options);
  if (configuredModels.length > 0) return configuredModels[0];

  return '';
}

function toPlainText(value) {
  if (typeof value === 'string') return value;
  if (value == null) return '';
  return String(value);
}

function collectTextFromMessageContent(content) {
  if (typeof content === 'string') return content;
  if (!Array.isArray(content)) return '';
  const parts = [];
  content.forEach((item) => {
    if (!item || typeof item !== 'object') return;
    if (item.type === 'text' && typeof item.text === 'string') {
      parts.push(item.text);
    }
  });
  return parts.join('\n');
}

function mapMessagePart(role, part) {
  if (!part || typeof part !== 'object') return null;
  if (part.type === 'text') {
    return {
      type: role === 'assistant' ? 'output_text' : 'input_text',
      text: toPlainText(part.text || '')
    };
  }
  if (part.type === 'image_url') {
    const imageUrl = part.image_url && typeof part.image_url === 'object'
      ? toPlainText(part.image_url.url || '')
      : toPlainText(part.image_url || '');
    if (!imageUrl) return null;
    return {
      type: 'input_image',
      image_url: imageUrl
    };
  }
  return null;
}

function mapMessagesToCodexInput(messages) {
  const input = [];
  if (!Array.isArray(messages)) return input;
  messages.forEach((message) => {
    if (!message || typeof message !== 'object') return;
    const role = String(message.role || 'user').trim().toLowerCase();
    if (role === 'tool') {
      const callId = toPlainText(message.tool_call_id || '').trim();
      if (!callId) return;
      input.push({
        type: 'function_call_output',
        call_id: callId,
        output: collectTextFromMessageContent(message.content)
      });
      return;
    }

    const codexRole = role === 'system' ? 'developer' : role;
    const content = [];
    if (typeof message.content === 'string') {
      content.push({
        type: codexRole === 'assistant' ? 'output_text' : 'input_text',
        text: message.content
      });
    } else if (Array.isArray(message.content)) {
      message.content.forEach((part) => {
        const mapped = mapMessagePart(codexRole, part);
        if (mapped) content.push(mapped);
      });
    }
    if (content.length === 0) {
      content.push({
        type: codexRole === 'assistant' ? 'output_text' : 'input_text',
        text: ''
      });
    }
    input.push({
      type: 'message',
      role: codexRole,
      content
    });

    if (codexRole === 'assistant' && Array.isArray(message.tool_calls)) {
      message.tool_calls.forEach((toolCall) => {
        if (!toolCall || toolCall.type !== 'function') return;
        const fn = toolCall.function && typeof toolCall.function === 'object' ? toolCall.function : {};
        input.push({
          type: 'function_call',
          call_id: toPlainText(toolCall.id || ''),
          name: toPlainText(fn.name || ''),
          arguments: toPlainText(fn.arguments || '')
        });
      });
    }
  });
  return input;
}

function mapToolsToCodex(tools) {
  if (!Array.isArray(tools)) return [];
  const out = [];
  tools.forEach((tool) => {
    if (!tool || tool.type !== 'function') return;
    const fn = tool.function && typeof tool.function === 'object' ? tool.function : {};
    const name = toPlainText(fn.name || '').trim();
    if (!name) return;
    out.push({
      type: 'function',
      name,
      description: toPlainText(fn.description || ''),
      parameters: fn.parameters && typeof fn.parameters === 'object'
        ? fn.parameters
        : { type: 'object', properties: {} }
    });
  });
  return out;
}

function mapResponseFormatToCodexTextFormat(responseFormat) {
  if (!responseFormat || typeof responseFormat !== 'object') return null;
  const type = String(responseFormat.type || '').trim();
  if (type === 'text') {
    return { format: { type: 'text' } };
  }
  if (type !== 'json_schema') return null;
  const schema = responseFormat.json_schema && typeof responseFormat.json_schema === 'object'
    ? responseFormat.json_schema
    : null;
  if (!schema) return null;
  return {
    format: {
      type: 'json_schema',
      name: toPlainText(schema.name || '').trim() || 'json_schema',
      strict: Boolean(schema.strict),
      schema: schema.schema && typeof schema.schema === 'object' ? schema.schema : {}
    }
  };
}

function convertOpenAIChatToCodexPayload(requestJson, forcedModel) {
  const model = toPlainText(forcedModel || requestJson && requestJson.model || '').trim();
  const codexPayload = {
    // Codex backend requires stream=true; non-stream clients are emulated by adapter side aggregation.
    stream: true,
    store: false,
    instructions: '',
    input: mapMessagesToCodexInput((requestJson && requestJson.messages) || []),
    parallel_tool_calls: true,
    reasoning: {
      effort: toPlainText(requestJson && requestJson.reasoning_effort || '').trim() || 'medium',
      summary: 'auto'
    },
    include: ['reasoning.encrypted_content']
  };
  if (model) codexPayload.model = model;

  const tools = mapToolsToCodex(requestJson && requestJson.tools);
  if (tools.length > 0) codexPayload.tools = tools;
  const textFormat = mapResponseFormatToCodexTextFormat(requestJson && requestJson.response_format);
  if (textFormat) codexPayload.text = textFormat;
  if (requestJson && requestJson.text && typeof requestJson.text === 'object') {
    const verbosity = toPlainText(requestJson.text.verbosity || '').trim();
    if (verbosity) {
      codexPayload.text = { ...(codexPayload.text || {}), verbosity };
    }
  }
  return codexPayload;
}

function convertOpenAIResponsesToCodexPayload(requestJson, forcedModel, options = {}) {
  const source = requestJson && typeof requestJson === 'object' ? requestJson : {};
  // store:false 时上游不持久化任何 response/item：previous_response_id 指向的历史
  // response 与 input 内携带的类型化 item id（rs_/msg_/fc_ 等）都无法在 store 中解析，
  // 透传必然触发 404 'Item with id rs_... not found'。出站前剥掉这些引用，对话历史
  // 以客户端随请求内联的 input 为准（codex CLI 在 store:false 链路上本就全量内联）。
  const storeDisabled = source.store === false;
  const payload = {};
  Object.entries(source).forEach(([key, value]) => {
    if (value === undefined) return;
    if (key === 'provider') return;
    if (storeDisabled && key === 'previous_response_id') return;
    payload[key] = key === 'input'
      ? sanitizeCodexResponsesInput(value, { ...options, dropUnpersistedItemIds: storeDisabled })
      : value;
  });
  const model = toPlainText(forcedModel || source.model || '').trim();
  if (model) payload.model = model;
  if (options.forceStream) payload.stream = true;
  return applyProviderProtocolParameterStrategy(payload, {
    provider: 'codex',
    protocol: 'openai_responses',
    model
  });
}

function sanitizeCodexResponsesInput(input, options = {}) {
  if (!Array.isArray(input)) return input;
  const idPrefixesByType = {
    message: 'msg_',
    reasoning: 'rs_',
    function_call: 'fc_',
    function_call_output: 'fco_',
    custom_tool_call: 'ctc_',
    custom_tool_call_output: 'ctco_'
  };
  return input.map((item) => {
    if (!item || typeof item !== 'object' || Array.isArray(item)) {
      return item;
    }
    const sanitizedItem = { ...item };
    const expectedPrefix = idPrefixesByType[item.type];
    if (expectedPrefix) {
      const id = toPlainText(item.id || '').trim();
      // store:false 下上游不持久化任何 item，即使前缀合法的 id（如历史 rs_ item）
      // 也无法解析，引用即 404；此时与「前缀不匹配的外来 id」同等处理：剥离。
      if (id && (options.dropUnpersistedItemIds === true || !id.startsWith(expectedPrefix))) {
        delete sanitizedItem.id;
      }
    }
    // OpenAI Responses 协议多账号池化/换号切换时：上一个账号生成的 reasoning.encrypted_content
    // 无法被新换的账号私钥解密，上游会直接抛 400 invalid_encrypted_content: The encrypted content could not be verified.
    if (sanitizedItem.type === 'reasoning') {
      delete sanitizedItem.encrypted_content;
    }
    return sanitizedItem;
  });
}

// 请求是否携带账号绑定的加密思考链：previous_response_id 链上的 reasoning item 或
// input 内联的 encrypted_content 都只能用生成它的账号私钥解密，换号必然 400
// invalid_encrypted_content。携带时选号必须硬性优先会话粘性绑定的账号。
function requestCarriesEncryptedReasoning(requestJson) {
  const source = requestJson && typeof requestJson === 'object' ? requestJson : {};
  if (toPlainText(source.previous_response_id || '').trim()) return true;
  const input = Array.isArray(source.input) ? source.input : [];
  return input.some((item) => item && typeof item === 'object' && !Array.isArray(item)
    && Boolean(toPlainText(item.encrypted_content || '').trim()));
}

function buildCodexAccountDiagnosticFields(account, upstreamUrl) {
  const apiKeyMode = Boolean(account && (account.apiKeyMode || account.authType === 'api-key'));
  return {
    accountRef: account && account.accountRef || undefined,
    accountEmail: account && account.email || undefined,
    accountAuthType: account && account.authType || (apiKeyMode ? 'api-key' : undefined),
    apiKeyMode,
    openaiBaseUrl: apiKeyMode ? toPlainText(account && account.openaiBaseUrl || '').trim() : '',
    upstreamUrl: toPlainText(upstreamUrl || '').trim() || undefined
  };
}

function parseOpenAIErrorMessage(rawText) {
  try {
    const parsed = JSON.parse(String(rawText || ''));
    const message = toPlainText(parsed && parsed.error && parsed.error.message || '').trim();
    if (message) return message;
    const detail = toPlainText(parsed && parsed.detail || parsed && parsed.message || '').trim();
    if (detail) return detail;
  } catch (_error) {}
  return toPlainText(rawText || '').trim();
}

function writeOpenAIResponsesError(res, statusCode, rawText, fallbackDetail) {
  let payload = null;
  try {
    const parsed = JSON.parse(String(rawText || ''));
    if (parsed && typeof parsed === 'object' && parsed.error && typeof parsed.error === 'object') {
      payload = parsed;
    }
  } catch (_error) {
    payload = null;
  }
  if (!payload) {
    const message = parseOpenAIErrorMessage(rawText) || toPlainText(fallbackDetail || '').trim() || `upstream_${statusCode}`;
    payload = {
      error: {
        message,
        type: statusCode === 404 ? 'not_found_error' : 'invalid_request_error',
        param: null,
        code: null
      }
    };
  }
  res.statusCode = statusCode;
  res.setHeader('content-type', 'application/json; charset=utf-8');
  res.end(JSON.stringify(payload));
}

function writeOpenAIResponsesSafetyError(res, statusCode) {
  res.statusCode = statusCode;
  res.setHeader('content-type', 'application/json; charset=utf-8');
  res.end(JSON.stringify({
    error: {
      message: 'The configured upstream rejected this request under its safety policy',
      type: 'permission_error',
      param: null,
      code: 'upstream_safety_rejected'
    }
  }));
}

function buildCodexFailureErrorPayload(failure) {
  const payload = failure && failure.payload && typeof failure.payload === 'object'
    ? failure.payload
    : {};
  const source = payload.error
    || payload.response && payload.response.error
    || payload.last_error
    || payload.response && payload.response.last_error
    || payload;
  const message = toPlainText(source && source.message || failure && failure.detail || '').trim();
  const type = toPlainText(
    source && source.type && source.type !== 'error'
      ? source.type
      : payload.code
    || 'invalid_request_error'
  ).trim();
  return {
    error: {
      message,
      type,
      param: source && source.param != null ? source.param : null,
      code: source && source.code != null ? source.code : null
    }
  };
}

function extractNativeCompletedResponse(rawText) {
  const events = parseSseEvents(rawText);
  const completed = events.find((item) => item && item.type === 'response.completed' && item.response);
  if (completed && completed.response && typeof completed.response === 'object') {
    return hydrateCompletedResponseOutput(completed.response, events);
  }
  try {
    const parsed = JSON.parse(String(rawText || ''));
    if (parsed && typeof parsed === 'object') return parsed;
  } catch (_error) {}
  return null;
}

function collectOutputItemsFromDoneEvents(events = []) {
  return events
    .filter((event) => event && event.type === 'response.output_item.done' && event.item && typeof event.item === 'object')
    .map((event, index) => ({
      index: Number.isFinite(Number(event.output_index)) ? Number(event.output_index) : index,
      item: event.item
    }))
    .sort((left, right) => left.index - right.index)
    .map((entry) => entry.item);
}

function collectOutputText(output = []) {
  return (Array.isArray(output) ? output : [])
    .filter((item) => item && item.type === 'message' && Array.isArray(item.content))
    .flatMap((item) => item.content)
    .filter((part) => part && part.type === 'output_text')
    .map((part) => toPlainText(part.text || ''))
    .join('');
}

function hydrateCompletedResponseOutput(response, events = []) {
  const source = response && typeof response === 'object' ? response : {};
  const currentOutput = Array.isArray(source.output) ? source.output : [];
  const output = currentOutput.length > 0 ? currentOutput : collectOutputItemsFromDoneEvents(events);
  if (output.length === currentOutput.length && source.output_text) return source;
  const outputText = source.output_text || collectOutputText(output);
  return {
    ...source,
    output,
    ...(outputText ? { output_text: outputText } : {})
  };
}

function parseSseEvents(rawText) {
  const lines = String(rawText || '').split(/\r?\n/);
  const events = [];
  let dataLines = [];
  const flush = () => {
    if (dataLines.length === 0) return;
    const data = dataLines.join('\n').trim();
    dataLines = [];
    if (!data || data === '[DONE]') return;
    try {
      events.push(JSON.parse(data));
    } catch (_error) {}
  };
  lines.forEach((line) => {
    if (!line.trim()) {
      flush();
      return;
    }
    if (line.startsWith('data:')) {
      dataLines.push(line.slice(5).trimStart());
    }
  });
  flush();
  return events;
}

function stringifyCompact(value) {
  if (typeof value === 'string') return value;
  if (value == null) return '';
  try {
    return JSON.stringify(value);
  } catch (_error) {
    return String(value);
  }
}

function extractCodexFailureDetailFromPayload(payload) {
  if (!payload || typeof payload !== 'object') return '';
  const type = toPlainText(payload.type || '').trim();
  const status = toPlainText(
    payload.status
    || payload.response && payload.response.status
    || ''
  ).trim();
  const error = payload.error
    || payload.response && payload.response.error
    || payload.last_error
    || payload.response && payload.response.last_error
    || null;
  const failed = type === 'response.failed'
    || type === 'error'
    || status === 'failed'
    || Boolean(error);
  if (!failed) return '';
  return stringifyCompact(error || payload);
}

function isCodexInvalidRequestPayload(payload) {
  if (!payload || typeof payload !== 'object') return false;
  const nestedError = payload.error
    || payload.response && payload.response.error
    || payload.last_error
    || payload.response && payload.response.last_error
    || null;
  const code = toPlainText(
    payload.code
    || nestedError && (nestedError.code || nestedError.type)
    || ''
  ).trim();
  return code === 'invalid_request_error' || code === 'invalid_request';
}

function extractCodexFailure(rawText) {
  const events = parseSseEvents(rawText);
  for (const event of events) {
    const detail = extractCodexFailureDetailFromPayload(event);
    if (detail) {
      return {
        detail,
        payload: event,
        requestError: isCodexInvalidRequestPayload(event)
      };
    }
  }
  try {
    const payload = JSON.parse(String(rawText || ''));
    const detail = extractCodexFailureDetailFromPayload(payload);
    if (detail) {
      return {
        detail,
        payload,
        requestError: isCodexInvalidRequestPayload(payload)
      };
    }
  } catch (_error) {}
  return null;
}

function extractCodexFailureDetail(rawText) {
  return extractCodexFailure(rawText)?.detail || '';
}

function extractUsageFromOpenAIChunks(chunks) {
  if (!Array.isArray(chunks)) return null;
  for (let index = chunks.length - 1; index >= 0; index -= 1) {
    const usage = chunks[index] && chunks[index].usage;
    if (usage && typeof usage === 'object') return usage;
  }
  return null;
}

function extractCodexUsageFromRawResponse(rawText) {
  const events = parseSseEvents(rawText);
  const completed = events.find((item) => item && item.type === 'response.completed') || null;
  if (completed && completed.response && typeof completed.response === 'object') {
    return completed.response.usage || null;
  }
  try {
    const parsed = JSON.parse(String(rawText || ''));
    const response = unwrapCodexResponseEnvelope(parsed);
    return response && typeof response === 'object' ? response.usage || null : null;
  } catch (_error) {
    return null;
  }
}

function recordCodexModelUsage(deps, payload = {}) {
  if (!deps || typeof deps.recordModelUsage !== 'function') return;
  const usage = payload.usage && typeof payload.usage === 'object' ? payload.usage : null;
  if (!usage) return;
  try {
    deps.recordModelUsage({
      provider: 'codex',
      accountRef: payload.accountRef,
      requestId: payload.requestId,
      sessionId: payload.sessionId,
      model: payload.model,
      usage,
      sourceKind: 'server_codex_proxy',
      timestampMs: payload.timestampMs
    });
  } catch (_error) {
    // best effort accounting; never fail a successful upstream response
  }
}

function convertCodexSseToOpenAIChunks(rawText, requestedModel) {
  const converter = createCodexChatStreamConverter(requestedModel);
  return parseSseEvents(rawText).flatMap((event) => converter.event(event));
}

function mapOutputMessageAndToolCalls(response) {
  const output = Array.isArray(response && response.output) ? response.output : [];
  const toolCalls = [];
  let content = '';
  let reasoning = '';
  output.forEach((item) => {
    if (!item || typeof item !== 'object') return;
    if (item.type === 'message' && Array.isArray(item.content)) {
      item.content.forEach((part) => {
        if (!part || typeof part !== 'object') return;
        if (part.type === 'output_text') {
          content += toPlainText(part.text || '');
        }
      });
      return;
    }
    if (item.type === 'reasoning' && Array.isArray(item.summary)) {
      item.summary.forEach((part) => {
        if (!part || typeof part !== 'object') return;
        if (part.type === 'summary_text') {
          reasoning += toPlainText(part.text || '');
        }
      });
      return;
    }
    if (item.type === 'function_call') {
      toolCalls.push({
        id: toPlainText(item.call_id || ''),
        type: 'function',
        function: {
          name: toPlainText(item.name || ''),
          arguments: toPlainText(item.arguments || '')
        }
      });
    }
  });
  return { content, reasoning, toolCalls };
}

function unwrapCodexResponseEnvelope(payload) {
  if (!payload || typeof payload !== 'object') return null;
  if (payload.response && typeof payload.response === 'object') return payload.response;
  return payload;
}

function convertCodexResponseToOpenAICompletion(payload, requestedModel) {
  const response = unwrapCodexResponseEnvelope(payload);
  if (!response || typeof response !== 'object') return null;
  const msg = mapOutputMessageAndToolCalls(response);
  const usage = mapUsageFromCodexResponse(response) || {
    prompt_tokens: 0,
    completion_tokens: 0,
    total_tokens: 0
  };
  const finishReason = msg.toolCalls.length > 0 ? 'tool_calls' : 'stop';
  return {
    id: toPlainText(response.id || `chatcmpl-${Date.now()}`),
    object: 'chat.completion',
    created: Number(response.created_at) || Math.floor(Date.now() / 1000),
    model: toPlainText(response.model || requestedModel || ''),
    choices: [
      {
        index: 0,
        message: {
          role: 'assistant',
          content: msg.content || '',
          reasoning_content: msg.reasoning || undefined,
          tool_calls: msg.toolCalls.length > 0 ? msg.toolCalls : undefined
        },
        finish_reason: finishReason
      }
    ],
    usage
  };
}

function generateSessionId() {
  try {
    if (typeof crypto.randomUUID === 'function') return crypto.randomUUID();
  } catch (_error) {}
  return `${Date.now()}-${Math.random().toString(16).slice(2)}`;
}

async function handleCodexModels(ctx) {
  const { options, state, res, deps } = ctx;
  const { buildOpenAIModelsList, fetchWithTimeout, modelCatalogSettings } = deps;
  const nativeCatalogResponse = ctx.responseFormat === 'codex';
  const now = Date.now();
  const ttl = Math.max(1000, Number(options.modelsCacheTtlMs) || 300000);
  const configuredModels = parseCodexModels(options);
  if (state
    && state.modelsCache
    && state.modelsCache.updatedAt > 0
    && now - state.modelsCache.updatedAt < ttl
    && Array.isArray(state.modelsCache.ids)
    && state.modelsCache.ids.length > 0) {
    const nativeModels = buildCodexNativeModelCatalog(
      state.modelsCache.ids,
      state.modelsCache.catalogByAccount
    );
    if (nativeCatalogResponse && nativeModels.length > 0) {
      res.statusCode = 200;
      res.setHeader('content-type', 'application/json; charset=utf-8');
      res.setHeader('x-aih-models-format', 'codex');
      res.end(JSON.stringify({ models: nativeModels }));
      return;
    }
    if (nativeCatalogResponse) {
      // Legacy caches only contain ids. Probe once so a Codex client receives
      // the full ModelInfo contract instead of an incompatible OpenAI list.
    } else {
      const accountEntries = buildCodexAccountModelListEntries(
        state.modelsCache.byAccount,
        modelCatalogSettings || state.modelCatalogSettings,
        options
      );
      const payload = buildOpenAIModelsList(hasAccountModelEntries(state.modelsCache.byAccount) ? accountEntries : buildCodexModelListEntries(
        state.modelsCache.ids,
        modelCatalogSettings || state.modelCatalogSettings,
        options
      ));
      res.statusCode = 200;
      res.setHeader('content-type', 'application/json; charset=utf-8');
      res.end(JSON.stringify(payload));
      return;
    }
  }
  const pool = (state && state.accounts && Array.isArray(state.accounts.codex))
    ? state.accounts.codex
    : [];
  const probeCount = Math.max(1, Math.min(8, Number(options.modelsProbeAccounts) || 2));
  const candidates = pool
    .filter((account) => Date.now() >= Number(account.cooldownUntil || 0))
    .slice(0, probeCount);
  const modelSet = new Set();
  const byAccount = {};
  const catalogByAccount = {};
  let firstError = '';
  const probeTimeout = Math.min(5000, Number(options.upstreamTimeoutMs) || 45000);
  const settled = await Promise.allSettled(
    candidates.map((account) => fetchCodexModelCatalogForAccount({
      options,
      account,
      fetchWithTimeout,
      timeoutMs: probeTimeout
    }))
  );
  settled.forEach((result, index) => {
    const account = candidates[index];
    const accountRef = String(account && account.accountRef || '').trim();
    if (result.status === 'fulfilled') {
      if (accountRef) {
        byAccount[accountRef] = result.value.ids.slice().sort();
        catalogByAccount[accountRef] = result.value.models.slice();
      }
      result.value.ids.forEach((id) => modelSet.add(id));
      return;
    }
    if (accountRef) {
      byAccount[accountRef] = [];
      catalogByAccount[accountRef] = [];
    }
    if (!firstError) {
      firstError = toPlainText(result.reason && result.reason.message || result.reason).trim();
    }
  });
  const discoveredIds = Array.from(modelSet).sort();
  let finalIds = discoveredIds;
  let source = 'discovered';
  if (finalIds.length === 0) {
    const staleIds = Array.isArray(state && state.modelsCache && state.modelsCache.ids)
      ? state.modelsCache.ids.filter((item) => Boolean(toPlainText(item || '').trim()))
      : [];
    if (staleIds.length > 0) {
      finalIds = staleIds;
      source = 'stale-cache';
    } else if (configuredModels.length > 0) {
      finalIds = configuredModels;
      source = 'configured';
    }
  }
  if (finalIds.length === 0) {
    res.statusCode = 503;
    res.setHeader('content-type', 'application/json; charset=utf-8');
    res.end(JSON.stringify({
      ok: false,
      error: 'models_unavailable',
      detail: firstError || 'no_models_discovered'
    }));
    return;
  }
  if (state && state.modelsCache) {
    state.modelsCache = {
      updatedAt: now,
      ids: finalIds,
      byAccount,
      catalogByAccount,
      sourceCount: discoveredIds.length > 0 ? candidates.length : 0
    };
  }
  const nativeModels = buildCodexNativeModelCatalog(finalIds, catalogByAccount);
  if (nativeCatalogResponse) {
    res.statusCode = 200;
    res.setHeader('content-type', 'application/json; charset=utf-8');
    res.setHeader('x-aih-models-source', source);
    res.setHeader('x-aih-models-format', 'codex');
    if (nativeModels.length < 1) {
      res.setHeader('x-aih-models-fallback', 'metadata-unavailable');
    }
    res.end(JSON.stringify({ models: nativeModels }));
    return;
  }
  const accountEntries = buildCodexAccountModelListEntries(
    byAccount,
    modelCatalogSettings || state.modelCatalogSettings,
    options
  );
  const payload = buildOpenAIModelsList(hasAccountModelEntries(byAccount) ? accountEntries : buildCodexModelListEntries(
    finalIds,
    modelCatalogSettings || state.modelCatalogSettings,
    options
  ));
  res.statusCode = 200;
  res.setHeader('content-type', 'application/json; charset=utf-8');
  res.setHeader('x-aih-models-source', source);
  if (discoveredIds.length === 0) {
    res.setHeader('x-aih-models-fallback', '1');
    if (firstError) {
      const safeReason = toPlainText(firstError).replace(/[^\x20-\x7E]/g, ' ').trim();
      if (safeReason) {
        res.setHeader('x-aih-models-fallback-reason', safeReason.slice(0, 120));
      }
    }
  }
  res.end(JSON.stringify(payload));
}

async function handleCodexChatCompletions(ctx) {
  const {
    options,
    state,
    req,
    res,
    requestJson,
    routeKey,
    requestStartedAt,
    cooldownMs,
    requestMeta,
    deps
  } = ctx;

  const {
    chooseServerAccount,
    pushMetricError,
    writeJson,
    fetchWithTimeout,
    markProxyAccountFailure,
    markProxyAccountSuccess,
    appendProxyRequestLog,
    refreshCodexAccessToken,
    resolveAccountEgressRequestOptions: resolveAccountEgressRequestOptionsDep,
    recordModelUsage,
    accountActivity,
    accountStateIndex,
    waitForTransientRetry = (delayMs) => new Promise((resolve) => setTimeout(resolve, delayMs))
  } = deps;

  const requestedAccountRef = String(
    req
    && req.headers
    && (req.headers['x-account-ref'] || req.headers['X-Account-Ref'])
    || ''
  ).trim();
  const codexAccounts = Array.isArray(state.accounts.codex) ? state.accounts.codex : [];
  const basePool = requestedAccountRef
    ? codexAccounts.filter((account) => String(account && account.accountRef || '') === requestedAccountRef)
    : codexAccounts;
  const requestedModel = toPlainText(requestJson && requestJson.model || '').trim();
  const resolvedModel = resolveCodexModel(requestedModel, options, state);
  const nativeResponsesMode = String(requestMeta && requestMeta.clientProtocol || '').trim() === 'openai_responses';
  if (!resolvedModel) {
    writeJson(res, 400, {
      ok: false,
      error: 'invalid_request',
      detail: 'model is required and no discoverable codex model is available'
    });
    return;
  }
  const modelPoolSelection = selectCodexAccountsForRequestModel(basePool, resolvedModel, state, options);
  const pool = modelPoolSelection.pool;
  const sessionKey = (requestMeta && (requestMeta.sessionKey || requestMeta.sessionId)) || '';
  const projectPath = (requestMeta && requestMeta.projectPath) || '';
  const projectDirName = (requestMeta && requestMeta.projectDirName) || '';
  if (modelPoolSelection.filtered && basePool.length > 0 && pool.length < 1) {
    state.metrics.totalFailures += 1;
    pushMetricError(state.metrics, routeKey, 'codex', {
      message: 'no_available_account',
      error: 'no_available_account',
      model: resolvedModel,
      sessionId: sessionKey,
      projectPath,
      projectDirName
    });
    writeJson(res, 503, {
      ok: false,
      error: 'no_available_account',
      detail: `no available codex account can serve model ${resolvedModel}`,
      availability: {
        provider: 'codex',
        model: resolvedModel,
        total: basePool.length,
        available: 0
      }
    });
    return;
  }
  const baseMaxAttempts = Math.min(
    Math.max(1, Number(options.maxAttempts) || 3),
    Math.max(1, pool.length)
  );
  const authRetryBudget = (typeof refreshCodexAccessToken === 'function' && pool.length > 0) ? 1 : 0;
  // Reserve one bounded slot for a transient retry of the same account. The
  // orchestrator consumes it only after all untried accounts are unavailable,
  // so normal account rotation remains the first recovery path.
  const transientRetryBudget = pool.length > 0 ? 1 : 0;
  const maxAttempts = baseMaxAttempts + authRetryBudget + transientRetryBudget;
  const forcedRefreshRetryUsed = new Set();
  const recordedTransientFailures = new Set();
  // 携带加密思考链的请求：能不解密就不换号——会话粘性绑定账号即使仅处于
  // （账号,模型）软冷却也优先于其他账号；必须换号时（绑定账号已排除/硬失败）
  // 才由 sanitizeCodexResponsesInput 剥离 encrypted_content 降级重试。
  const preserveEncryptedReasoningAffinity = nativeResponsesMode && requestCarriesEncryptedReasoning(requestJson);
  // 认证失败的定向回退偏好（如 OAuth 账号 401/403 后优先 api-key 账号），由
  // 失败策略在重试过程中置位，仅影响本请求后续的选号排序，不做硬过滤。
  let retryPreferAuthType = '';
  const selectAccountForAttempt = (attemptPool, selectionState, cursorKeyArg, selectionOptions = {}) => (
    typeof chooseServerAccount === 'function'
      ? chooseServerAccount(attemptPool, selectionState, cursorKeyArg, {
        ...selectionOptions,
        preserveEncryptedReasoningAffinity,
        preferAuthType: retryPreferAuthType || selectionOptions.preferAuthType || '',
        accountStateIndex
      })
      : null
  );
  let lastError = '';
  let lastFailurePolicy = null;
  const orchestration = await runWithAccountAttempts({
    pool,
    maxAttempts,
    chooseServerAccount: selectAccountForAttempt,
    selectionState: state,
    cursorState: state.cursors,
    cursorKey: 'codex',
    provider: 'codex',
    model: resolvedModel,
    strategy: state.strategy,
    sessionKey: (requestMeta && requestMeta.sessionKey) || '',
    // Last-resort: serve a soft model-cooled account rather than 503'ing when the
    // alias preflight found nothing cleanly routable (flowed via requestMeta).
    allowModelCooled: Boolean(requestMeta && requestMeta.allowModelCooled),
    accountActivity,
    onAttempt: async (account, control) => {
      lastFailurePolicy = null;
      const resolveRequestOptions = typeof resolveAccountEgressRequestOptionsDep === 'function'
        ? resolveAccountEgressRequestOptionsDep
        : resolveAccountEgressRequestOptionsDefault;
      let accountRequestOptions;
      try {
        accountRequestOptions = await resolveRequestOptions({
          fs: deps.fs,
          aiHomeDir: deps.aiHomeDir || options.aiHomeDir,
          processObj: deps.processObj,
          provider: 'codex',
          accountRef: account.accountRef,
          options,
          deps
        });
      } catch (error) {
        accountRequestOptions = {
          ok: false,
          error: 'account_egress_unavailable',
          egressError: String(error?.message || error || 'unknown')
        };
      }
      if (!accountRequestOptions?.ok || !accountRequestOptions.options) {
        lastError = [
          String(accountRequestOptions?.error || 'account_egress_unavailable'),
          String(accountRequestOptions?.egressError || '')
        ].filter(Boolean).join(':');
        control.setLastError(lastError);
        return { action: 'retry_next' };
      }
      const attemptOptions = accountRequestOptions.options;
      if (typeof refreshCodexAccessToken === 'function') {
        try {
          await refreshCodexAccessToken(account, {
            force: false,
            timeoutMs: attemptOptions.upstreamTimeoutMs,
            proxyUrl: attemptOptions.proxyUrl,
            noProxy: attemptOptions.noProxy
          }, {
            fetchWithTimeout
          });
        } catch (_error) {}
      }

      const accessToken = sanitizeAccessToken(account.accessToken);
      if (!accessToken) {
        markProxyAccountFailure(account, 'invalid_access_token', cooldownMs, options.failureThreshold);
        lastError = `invalid_access_token_account_${account.accountRef}`;
        control.setLastError(lastError);
        return { action: 'retry_next' };
      }

      const codexBaseUrl = resolveCodexUpstreamBaseUrl(options, account);
      if (isLoopbackUrl(codexBaseUrl, options.port)) {
        lastError = 'infinite_loop_detected';
        control.setLastError(lastError);
        return { action: 'break' };
      }
      const codexUrl = `${codexBaseUrl}/responses`;
      const codexPayload = nativeResponsesMode
        ? convertOpenAIResponsesToCodexPayload(requestJson || {}, resolvedModel, {
          forceStream: !(account && (account.apiKeyMode || account.authType === 'api-key'))
        })
        : convertOpenAIChatToCodexPayload(requestJson || {}, resolvedModel);
      const clientWantsStream = Boolean(requestJson && requestJson.stream);
      const logRetryFailure = (policy, data = {}) => {
        appendUpstreamFailureDiagnosticLog({
          options,
          appendProxyRequestLog,
          requestId: requestMeta && requestMeta.requestId,
          route: routeKey,
          provider: 'codex',
          account,
          attempt: control.attempt + 1,
          maxAttempts,
          requestedModel,
          effectiveModel: resolvedModel,
          streamRequested: clientWantsStream,
          upstreamUrl: codexUrl,
          durationMs: Date.now() - requestStartedAt,
          policy,
          ...data
        });
      };
      const applyRequestFailurePolicy = (policy) => {
        if (isTransportFailurePolicy(policy)) {
          const key = [account.accountRef, resolvedModel, policy.kind].join('\u0000');
          if (recordedTransientFailures.has(key)) return;
          recordedTransientFailures.add(key);
        }
        applyFailurePolicyToAccount(
          account,
          policy,
          markProxyAccountFailure,
          options.failureThreshold,
          resolvedModel
        );
      };
      const retryOutcomeForPolicy = async (policy, rawFailureBody = '') => {
        lastFailurePolicy = policy;
        if (policy.shouldRetryAnotherAccount && policy.preferRetryAuthType) {
          retryPreferAuthType = String(policy.preferRetryAuthType || '').trim();
        }
        if (!policy.shouldRetryAnotherAccount) {
          lastError = policy.detail;
          control.setLastError(lastError);
          state.metrics.totalFailures += 1;
          pushMetricError(state.metrics, routeKey, 'codex', {
            message: lastError,
            error: policy.failureReason || policy.kind || 'upstream_failed',
            accountRef: account.accountRef,
            sessionId: sessionKey,
            projectPath,
            projectDirName,
            model: resolvedModel
          });
          const statusCode = Number(policy.clientStatusCode) || 502;
          if (nativeResponsesMode) {
            if (policy.kind === 'safety_rejected') {
              writeOpenAIResponsesSafetyError(res, statusCode);
            } else {
              writeOpenAIResponsesError(
                res,
                statusCode,
                policy.shouldPassthroughToClient ? rawFailureBody : '',
                lastError
              );
            }
          } else {
            writeJson(res, statusCode, {
              ok: false,
              error: 'upstream_failed',
              detail: lastError
            });
          }
          return { action: 'return' };
        }
        if (!isTransientRetryPolicy(policy)) return { action: 'retry_next' };
        const deferred = control.deferSameAccountRetry();
        if (deferred && isTransportFailurePolicy(policy)) {
          await waitForTransientRetry(TRANSIENT_RETRY_DELAY_MS);
        }
        return { action: 'retry_transient' };
      };

      const downstream = createDownstreamAbortContext(req, res);
      let streamStarted = false;
      const startStream = () => {
        if (streamStarted) return;
        res.statusCode = 200;
        res.setHeader('content-type', 'text/event-stream; charset=utf-8');
        res.setHeader('cache-control', 'no-cache');
        res.setHeader('connection', 'keep-alive');
        res.setHeader('x-aih-server-account-ref', account.accountRef);
        if (account.email) res.setHeader('x-aih-server-account-email', account.email);
        res.setHeader('x-aih-effective-model', resolvedModel);
        streamStarted = true;
        if (typeof res.flushHeaders === 'function') res.flushHeaders();
      };
      try {
        const headers = {};
        Object.entries(req.headers || {}).forEach(([k, v]) => {
          const key = String(k || '').toLowerCase();
          if (shouldSkipForwardHeader(k)) return;
          const normalized = normalizeHeaderValue(v);
          if (!normalized) return;
          if (!isSafeHeaderValue(normalized)) return;
          headers[key] = normalized;
        });
        headers.authorization = `Bearer ${accessToken}`;
        headers['content-type'] = 'application/json';
        headers.accept = codexPayload.stream ? 'text/event-stream' : 'application/json';
        // Local proxies may strip response metadata. Avoid compressed bytes
        // without their encoding header on this always-streaming wire API.
        if (codexPayload.stream) headers['accept-encoding'] = 'identity';
        headers.connection = 'keep-alive';
        const clientVersion = resolveCodexClientVersion(options);
        if (clientVersion) headers.version = clientVersion;
        headers.session_id = generateSessionId();
        headers['user-agent'] = headers['user-agent'] || (clientVersion ? `codex_cli_rs/${clientVersion}` : 'codex_cli_rs');
        headers.originator = headers.originator || 'codex_cli_rs';
        if (account.upstreamAccountId) headers['chatgpt-account-id'] = account.upstreamAccountId;
        headers['x-aih-account-ref'] = account.accountRef;
        headers['x-aih-account-email'] = account.email || '';

      const upstreamRes = await fetchWithTimeout(codexUrl, {
        method: 'POST',
        headers,
        signal: downstream.signal,
        body: JSON.stringify(codexPayload)
      }, attemptOptions.upstreamTimeoutMs, {
        proxyUrl: attemptOptions.proxyUrl,
        noProxy: attemptOptions.noProxy
      });

      let streamed = null;
      let rawText = '';
      const upstreamContentType = String(upstreamRes.headers?.get?.('content-type') || '').trim();
      if (upstreamRes.ok && clientWantsStream && upstreamRes.body
          && typeof upstreamRes.body.getReader === 'function'
          // Codex requests stream:true. Missing proxy headers must not turn
          // reasoning into a whole-response buffer and trip WebUI's deadline.
          && (!upstreamContentType || /text\/event-stream/i.test(upstreamContentType))) {
        try {
          streamed = await forwardCodexResponseStream({
            response: upstreamRes, res, signal: downstream.signal,
            timeoutMs: attemptOptions.upstreamTimeoutMs,
            model: resolvedModel, native: nativeResponsesMode,
            detectFailure: extractCodexFailure, start: startStream
          });
        } catch (error) {
          // Before any event reaches the client, retain the existing structured
          // error/retry policy. Once exposed, an answer must never be replayed.
          if (streamStarted || !error.codexFailure) throw error;
          rawText = JSON.stringify(error.codexFailure.payload);
        }
      } else {
        rawText = await upstreamRes.text();
      }
      if (upstreamRes.status === 401 || upstreamRes.status === 403) {
        const accountRef = String(account.accountRef || '');
        const allowRefreshRetry = (
          typeof refreshCodexAccessToken === 'function'
          && !forcedRefreshRetryUsed.has(accountRef)
        );
        if (allowRefreshRetry) {
          let refreshResult = null;
          try {
            refreshResult = await refreshCodexAccessToken(account, {
              force: true,
              timeoutMs: attemptOptions.upstreamTimeoutMs,
              proxyUrl: attemptOptions.proxyUrl,
              noProxy: attemptOptions.noProxy
            }, {
              fetchWithTimeout
            });
          } catch (_error) {
            refreshResult = null;
          }
          if (refreshResult && refreshResult.ok && refreshResult.refreshed) {
            forcedRefreshRetryUsed.add(accountRef);
            control.retrySameAccount();
            return { action: 'retry_same' };
          }
        }
        const policy = classifyUpstreamFailure({
          provider: 'codex',
          statusCode: upstreamRes.status,
          headers: upstreamRes.headers,
          body: rawText,
          detail: `upstream_${upstreamRes.status}_account_${account.accountRef}`,
          account,
          defaultCooldownMs: cooldownMs
        });
        applyRequestFailurePolicy(policy);
        logRetryFailure(policy, {
          status: upstreamRes.status,
          upstreamStatus: upstreamRes.status,
          upstreamHeaders: upstreamRes.headers,
          upstreamBody: rawText
        });
        lastError = policy.detail;
        control.setLastError(lastError);
        return retryOutcomeForPolicy(policy, rawText);
      }
      if (upstreamRes.status >= 400 && upstreamRes.status < 500 && upstreamRes.status !== 429) {
        const detail = `upstream_${upstreamRes.status}: ${toPlainText(rawText).slice(0, 320)}`;
        const policy = classifyUpstreamFailure({
          provider: 'codex',
          statusCode: upstreamRes.status,
          headers: upstreamRes.headers,
          body: rawText,
          detail,
          defaultCooldownMs: cooldownMs
        });
        if (
          policy.kind === 'safety_rejected'
          || policy.shouldMarkFailure
          || policy.shouldRetryAnotherAccount
        ) {
          applyRequestFailurePolicy(policy);
          logRetryFailure(policy, {
            status: upstreamRes.status,
            upstreamStatus: upstreamRes.status,
            upstreamHeaders: upstreamRes.headers,
            upstreamBody: rawText
          });
          lastError = policy.detail;
          control.setLastError(lastError);
          return retryOutcomeForPolicy(policy, rawText);
        }
        state.metrics.totalFailures += 1;
        pushMetricError(state.metrics, routeKey, 'codex', {
          message: detail,
          error: 'invalid_request',
          accountRef: account.accountRef,
          sessionId: sessionKey,
          projectPath,
          projectDirName,
          model: resolvedModel
        });
        if (options.logRequests) {
          appendProxyRequestLog({
            at: new Date().toISOString(),
            requestId: requestMeta && requestMeta.requestId,
            route: routeKey,
            provider: 'codex',
            ...buildCodexAccountDiagnosticFields(account, codexUrl),
            status: upstreamRes.status,
            error: detail,
            durationMs: Date.now() - requestStartedAt
          });
        }
        if (nativeResponsesMode) {
          writeOpenAIResponsesError(res, upstreamRes.status, rawText, detail);
          return { action: 'return' };
        }
        writeJson(res, upstreamRes.status, { ok: false, error: 'invalid_request', detail });
        return { action: 'return' };
      }
      if (!upstreamRes.ok) {
        const detail = `upstream_${upstreamRes.status}: ${toPlainText(rawText).slice(0, 320)}`;
        const policy = classifyUpstreamFailure({
          provider: 'codex',
          statusCode: upstreamRes.status,
          headers: upstreamRes.headers,
          body: rawText,
          detail,
          defaultCooldownMs: cooldownMs
        });
        applyRequestFailurePolicy(policy);
        logRetryFailure(policy, {
          status: upstreamRes.status,
          upstreamStatus: upstreamRes.status,
          upstreamHeaders: upstreamRes.headers,
          upstreamBody: rawText
        });
        lastError = policy.detail;
        control.setLastError(lastError);
        return retryOutcomeForPolicy(policy, rawText);
      }

      const responseFailure = extractCodexFailure(rawText);
      if (responseFailure) {
        const responseFailureDetail = responseFailure.detail;
        const policy = classifyUpstreamFailure({
          provider: 'codex',
          statusCode: responseFailure.requestError ? 400 : 0,
          error: new Error(responseFailureDetail),
          // The wire body can be an SSE envelope. Pass the already-decoded
          // event so failure classification reads only structured error codes
          // and never searches free-form transcript text.
          body: responseFailure.payload,
          detail: responseFailureDetail,
          defaultCooldownMs: cooldownMs
        });
        if (policy.shouldPassthroughToClient && !policy.shouldRetryAnotherAccount) {
          state.metrics.totalFailures += 1;
          pushMetricError(state.metrics, routeKey, 'codex', {
            message: responseFailureDetail,
            error: 'invalid_request',
            accountRef: account.accountRef,
            sessionId: sessionKey,
            projectPath,
            projectDirName,
            model: resolvedModel
          });
          if (options.logRequests) {
            appendProxyRequestLog({
              at: new Date().toISOString(),
              requestId: requestMeta && requestMeta.requestId,
              route: routeKey,
              provider: 'codex',
              ...buildCodexAccountDiagnosticFields(account, codexUrl),
              status: policy.clientStatusCode,
              error: responseFailureDetail,
              durationMs: Date.now() - requestStartedAt
            });
          }
          const failurePayload = JSON.stringify(buildCodexFailureErrorPayload(responseFailure));
          if (nativeResponsesMode) {
            writeOpenAIResponsesError(res, policy.clientStatusCode, failurePayload, responseFailureDetail);
          } else {
            writeJson(res, policy.clientStatusCode, {
              ok: false,
              error: 'invalid_request',
              detail: responseFailureDetail
            });
          }
          return { action: 'return' };
        }
        applyRequestFailurePolicy(policy);
        logRetryFailure(policy, {
          status: policy.clientStatusCode || 503,
          upstreamStatus: upstreamRes.status,
          upstreamHeaders: upstreamRes.headers,
          upstreamBody: rawText,
          upstreamError: responseFailureDetail
        });
        lastError = policy.detail;
        control.setLastError(lastError);
        return retryOutcomeForPolicy(policy, rawText);
      }

      if (streamed) {
        recordCodexModelUsage({ recordModelUsage }, {
          accountRef: account.accountRef,
          requestId: requestMeta && requestMeta.requestId,
          model: resolvedModel,
          usage: streamed.usage,
          timestampMs: Date.now()
        });
        res.end();
      } else if (nativeResponsesMode) {
        res.statusCode = 200;
        res.setHeader('x-aih-server-account-ref', account.accountRef);
        if (account.email) res.setHeader('x-aih-server-account-email', account.email);
        res.setHeader('x-aih-effective-model', resolvedModel);
        if (clientWantsStream) {
          const nativeUsage = extractCodexUsageFromRawResponse(rawText);
          recordCodexModelUsage({ recordModelUsage }, {
            accountRef: account.accountRef,
            requestId: requestMeta && requestMeta.requestId,
            model: resolvedModel,
            usage: nativeUsage,
            timestampMs: Date.now()
          });
          res.setHeader('content-type', 'text/event-stream; charset=utf-8');
          res.setHeader('cache-control', 'no-cache');
          res.setHeader('connection', 'keep-alive');
          res.end(rawText);
        } else {
          const nativeResponse = extractNativeCompletedResponse(rawText);
          if (!nativeResponse) {
            markProxyAccountFailure(account, 'invalid_codex_response', cooldownMs, options.failureThreshold);
            logRetryFailure({
              kind: 'invalid_codex_response',
              failureReason: 'invalid_codex_response',
              detail: 'invalid_codex_response',
              clientStatusCode: 502,
              retryable: true
            }, {
              status: 502,
              upstreamStatus: upstreamRes.status,
              upstreamHeaders: upstreamRes.headers,
              upstreamBody: rawText
            });
            lastError = 'invalid_codex_response';
            control.setLastError(lastError);
            return { action: 'retry_next' };
          }
          res.setHeader('content-type', 'application/json; charset=utf-8');
          recordCodexModelUsage({ recordModelUsage }, {
            accountRef: account.accountRef,
            requestId: requestMeta && requestMeta.requestId,
            model: nativeResponse.model || resolvedModel,
            usage: nativeResponse.usage,
            timestampMs: Date.now()
          });
          res.end(JSON.stringify(nativeResponse));
        }
      } else if (clientWantsStream) {
        const chunks = convertCodexSseToOpenAIChunks(rawText, resolvedModel);
        recordCodexModelUsage({ recordModelUsage }, {
          accountRef: account.accountRef,
          requestId: requestMeta && requestMeta.requestId,
          model: resolvedModel,
          usage: extractUsageFromOpenAIChunks(chunks),
          timestampMs: Date.now()
        });
        res.statusCode = 200;
        res.setHeader('content-type', 'text/event-stream; charset=utf-8');
        res.setHeader('cache-control', 'no-cache');
        res.setHeader('connection', 'keep-alive');
        res.setHeader('x-aih-server-account-ref', account.accountRef);
        if (account.email) res.setHeader('x-aih-server-account-email', account.email);
        res.setHeader('x-aih-effective-model', resolvedModel);
        chunks.forEach((chunk) => {
          res.write(`data: ${JSON.stringify(chunk)}\n\n`);
        });
        res.write('data: [DONE]\n\n');
        res.end();
      } else {
        const events = parseSseEvents(rawText);
        const completed = events.find((item) => item && item.type === 'response.completed') || null;
        let parsed = completed;
        if (!parsed) {
          try {
            parsed = JSON.parse(rawText);
          } catch (_error) {
            parsed = null;
          }
        }
        const completion = convertCodexResponseToOpenAICompletion(parsed, resolvedModel);
        if (!completion) {
          markProxyAccountFailure(account, 'invalid_codex_response', cooldownMs, options.failureThreshold);
          logRetryFailure({
            kind: 'invalid_codex_response',
            failureReason: 'invalid_codex_response',
            detail: 'invalid_codex_response',
            clientStatusCode: 502,
            retryable: true
          }, {
            status: 502,
            upstreamStatus: upstreamRes.status,
            upstreamHeaders: upstreamRes.headers,
            upstreamBody: rawText
          });
          lastError = 'invalid_codex_response';
          control.setLastError(lastError);
          return { action: 'retry_next' };
        }
        res.statusCode = 200;
        res.setHeader('content-type', 'application/json; charset=utf-8');
        res.setHeader('x-aih-server-account-ref', account.accountRef);
        if (account.email) res.setHeader('x-aih-server-account-email', account.email);
        res.setHeader('x-aih-effective-model', resolvedModel);
        recordCodexModelUsage({ recordModelUsage }, {
          accountRef: account.accountRef,
          requestId: requestMeta && requestMeta.requestId,
          model: completion.model || resolvedModel,
          usage: completion.usage,
          timestampMs: Date.now()
        });
        res.end(JSON.stringify(completion));
      }

      markProxyAccountSuccess(account, { model: resolvedModel });
      state.metrics.totalSuccess += 1;
      if (options.logRequests) {
        appendProxyRequestLog({
          at: new Date().toISOString(),
          requestId: requestMeta && requestMeta.requestId,
          route: routeKey,
          provider: 'codex',
          accountRef: account.accountRef,
          status: 200,
          streamTransport: streamed ? 'upstream_sse' : 'buffered',
          upstreamContentType,
          ...(streamed ? {
            firstEventMs: streamed.firstEventAt - requestStartedAt,
            ...(streamed.firstContentAt ? { firstContentMs: streamed.firstContentAt - requestStartedAt } : {})
          } : {}),
          durationMs: Date.now() - requestStartedAt
        });
      }
        return { action: 'return' };
      } catch (error) {
        if (downstream.isDisconnected()) return { action: 'return' };
        const policy = classifyUpstreamFailure({
          provider: 'codex',
          error,
          body: error.codexFailure && error.codexFailure.payload,
          statusCode: error.codexFailure && error.codexFailure.requestError ? 400 : 0,
          defaultCooldownMs: cooldownMs
        });
        if (policy.kind === 'timeout') state.metrics.totalTimeouts += 1;
        applyRequestFailurePolicy(policy);
        logRetryFailure(policy, {
          status: policy.clientStatusCode || 502,
          upstreamError: error
        });
        lastError = policy.detail;
        if (policy.kind === 'network_error' || policy.kind === 'timeout') {
          lastError = withNetworkHint(policy.detail, resolveCodexUpstreamBaseUrl(options, account), {
            phase: streamStarted ? 'mid-stream' : 'pre-response',
            elapsedMs: Date.now() - requestStartedAt,
            proxy: resolveProxyConfig(codexUrl, {
              proxyUrl: attemptOptions.proxyUrl,
              noProxy: attemptOptions.noProxy
            })
          });
        }
        control.setLastError(lastError);
        if (streamStarted) {
          state.metrics.totalFailures += 1;
          pushMetricError(state.metrics, routeKey, 'codex', {
            message: lastError, accountRef: account.accountRef, model: resolvedModel,
            sessionId: sessionKey, projectPath, projectDirName
          });
          const payload = policy.kind === 'safety_rejected'
            ? { message: 'The configured upstream rejected this request under its safety policy', type: 'upstream_safety_rejected', code: 'upstream_safety_rejected' }
            // lastError carries the enriched mid-stream/effective-proxy hint;
            // bare policy.detail would strip the actionable context.
            : { message: lastError, type: 'upstream_error', code: policy.failureReason || policy.kind || 'upstream_failed' };
          res.end(`${nativeResponsesMode ? 'event: error\n' : ''}data: ${JSON.stringify({ ...(nativeResponsesMode ? { type: 'error' } : {}), error: payload })}\n\n`);
          return { action: 'return' };
        }
        return retryOutcomeForPolicy(policy);
      } finally {
        downstream.dispose();
      }
    }
  });

  if (orchestration.kind === 'returned') return;
  const transientAttemptsEnded = (
    orchestration.kind === 'attempts_exhausted'
    || orchestration.kind === 'no_account'
  ) && isTransientRetryPolicy(lastFailurePolicy);
  if (transientAttemptsEnded) {
    const unavailable = buildNoAvailableAccountResponse('codex', pool, { model: resolvedModel });
    const preserveTransportFailure = isTransportFailurePolicy(lastFailurePolicy);
    const allAccountsUnavailable = Number(unavailable.payload.availability.available) < 1;
    const useUnavailableAccount = allAccountsUnavailable && !preserveTransportFailure;
    const statusCode = useUnavailableAccount
      ? unavailable.statusCode
      : Number(lastFailurePolicy.clientStatusCode) || 503;
    const errorCode = useUnavailableAccount
      ? unavailable.payload.error
      : 'upstream_temporarily_unavailable';
    const payload = useUnavailableAccount
      ? unavailable.payload
      : { ok: false, error: errorCode, detail: lastError || lastFailurePolicy.failureReason };
    state.metrics.totalFailures += 1;
    pushMetricError(state.metrics, routeKey, 'codex', {
      message: errorCode,
      error: errorCode,
      model: resolvedModel,
      sessionId: sessionKey,
      projectPath,
      projectDirName
    });
    if (options.logRequests) {
      appendProxyRequestLog({
        at: new Date().toISOString(),
        requestId: requestMeta && requestMeta.requestId,
        route: routeKey,
        provider: 'codex',
        status: statusCode,
        error: lastError || errorCode,
        durationMs: Date.now() - requestStartedAt
      });
    }
    writeJson(res, statusCode, payload);
    return;
  }
  if (orchestration.kind === 'no_account') {
    state.metrics.totalFailures += 1;
    pushMetricError(state.metrics, routeKey, 'codex', {
      message: 'no_available_account',
      error: 'no_available_account',
      model: resolvedModel,
      sessionId: sessionKey,
      projectPath,
      projectDirName
    });
    const unavailable = buildNoAvailableAccountResponse('codex', pool, { model: resolvedModel });
    writeUnavailableAccountResponse(res, writeJson, unavailable);
    return;
  }
  if (
    orchestration.kind === 'attempts_exhausted'
    && hasUnavailableReason(pool, 'auth_invalid_reauth_required')
  ) {
    state.metrics.totalFailures += 1;
    pushMetricError(state.metrics, routeKey, 'codex', {
      message: 'no_available_account',
      error: 'no_available_account',
      model: resolvedModel,
      sessionId: sessionKey,
      projectPath,
      projectDirName
    });
    const unavailable = buildNoAvailableAccountResponse('codex', pool, { model: resolvedModel });
    if (options.logRequests) {
      appendProxyRequestLog({
        at: new Date().toISOString(),
        requestId: requestMeta && requestMeta.requestId,
        route: routeKey,
        provider: 'codex',
        status: unavailable.statusCode,
        error: lastError || 'no_available_account',
        durationMs: Date.now() - requestStartedAt
      });
    }
    writeUnavailableAccountResponse(res, writeJson, unavailable);
    return;
  }

  state.metrics.totalFailures += 1;
  pushMetricError(state.metrics, routeKey, 'codex', {
    message: lastError,
    error: lastError || 'upstream_failed',
    attemptedAccountRefs: Array.from(orchestration.attemptedAccountRefs || []),
    sessionId: sessionKey,
    projectPath,
    projectDirName,
    model: resolvedModel
  });
  if (options.logRequests) {
    appendProxyRequestLog({
      at: new Date().toISOString(),
      requestId: requestMeta && requestMeta.requestId,
      route: routeKey,
      provider: 'codex',
      status: 502,
      error: lastError,
      durationMs: Date.now() - requestStartedAt
    });
  }
  writeJson(res, 502, { ok: false, error: 'upstream_failed', detail: lastError });
}

module.exports = {
  handleCodexModels,
  handleCodexChatCompletions,
  __private: {
    parseCodexModels,
    parseCodexModelCatalog,
    parseCodexModelsResponse,
    fetchCodexModelCatalogForAccount,
    fetchCodexModelsForAccount,
    buildCodexNativeModelCatalog,
    resolveCodexUpstreamBaseUrl,
    resolveCodexClientVersion,
    resolveCodexModel,
    buildCodexAccountDiagnosticFields,
    convertOpenAIChatToCodexPayload,
    convertOpenAIResponsesToCodexPayload,
    sanitizeCodexResponsesInput,
    convertCodexSseToOpenAIChunks,
    convertCodexResponseToOpenAICompletion,
    extractCodexFailureDetail,
    extractCodexFailure,
    extractNativeCompletedResponse,
    parseSseEvents
  }
};

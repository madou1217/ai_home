...
const { narrowPoolByModelCatalog } = require('./model-pool-narrowing');
const {
...
function selectCodexAccountsForRequestModel(pool, model, state, options) {
  ...
  // 优先用全局正排/倒排索引 O(1) 查：
  // 1. 查出真正支持 requestedModel 的可调度账号列表（过滤掉不支持该模型的中转/Ollama等账号）
  // 2. 将候选账号池 narrow 到该模型对应的账号列表，防止轮询尝试不支持该模型的账号
  // 3. allowNoAccountVerdict: false 保障冷启动或缓存未收录时回退放行整池，由上游错误说话
  ...
  if (modelIndex && modelIndex.builtAt > 0) {
    return narrowPoolByModelCatalog({
      ...
      accountRefs: findRoutableAccountsForModel(modelIndex, requestedModel, 'codex'),
  ...
  // 回退路径：若全局倒排索引尚未构建，走基于 capability index 的传统过滤逻辑
  ...
  return narrowPoolByModelCatalog({
    ...
    provider: 'codex',
    model: requestedModel,
    accountRefs: listAvailableAccountRefsForModelProvider(index, requestedModel, 'codex'),
...
async function handleCodexChatCompletions(ctx) {
  ...
  if (transientAttemptsEnded) {
    ...
    state.metrics.totalFailures += 1;
    pushMetricError(state.metrics, routeKey, 'codex', errorCode);
    if (options.logRequests) {
      appendProxyRequestLog({
        at: new Date().toISOString(),
        requestId: requestMeta && requestMeta.requestId,

...
const { mapCodexTokenUsage } = require('./chat-runtime/codex-token-usage-adapter');
const {
  mapCodexPlanUpdate
...
const METHOD_MAPPERS = new Map([
  ['thread/tokenUsage/updated', (_id, params) => mapCodexTokenUsage(params)],
  ['thread/goal/updated', (_id, params) => mapGoalUpdated(params)],
  ['thread/goal/cleared', (_id, params) => mapGoalCleared(params)],
...
function mapGoalUpdated(params) {
  return intent('session.goal.updated', params.turnId, undefined, {
    goal: normalizeGoal(params.goal, params.threadId)
  });
}

function mapGoalCleared(params) {
  return {
    type: 'session.goal.cleared',
    payload: { ...(params.threadId ? { goalId: String(params.threadId) } : {}) }
  };
}

function normalizeGoal(value, fallbackThreadId) {
  const goal = asRecord(value);
  return {
    threadId: String(goal.threadId || fallbackThreadId || ''),
    objective: String(goal.objective || ''),
    status: String(goal.status || ''),
    tokenBudget: goal.tokenBudget === null || goal.tokenBudget === undefined ? null : Number(goal.tokenBudget),

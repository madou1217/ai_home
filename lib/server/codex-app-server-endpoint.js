...
// 本模块启动的 app-server **一律**被钉在本机网关上：下方 env 构造无条件用 gatewayEnv 覆盖
// OPENAI_BASE_URL/OPENAI_API_KEY，于是 buildCodexProviderArgs 必然吐出 -c model_provider=aih_server，
// 与 gateway/per-account、OAuth/api-key 都无关。后果是 account/read 恒为
// { account: null, requiresOpenaiAuth: false } —— 这一点身份校验方必须知道，否则会拿
// 「codex 自报 chatgpt 登录」的判据去量一个按契约就不会自报账号的进程。
// 事实写在这里而不是散在调用点：将来这里若改成有条件钉，只需要改这一处。
const APP_SERVER_PINS_GATEWAY_PROVIDER = true;
...
// psmux 常驻 server 的 broker 环境会覆盖 spawn env 里的同名变量（实测：Windows 用户全局
// OPENAI_API_KEY=yesboss-* 顶掉网关 client key → /v1/responses 401 unauthorized_client）。
// win32 下鉴权与身份变量必须经 new-session -e 显式投递（session env 优先于 server env）。
// 投递世代盐：旧代码创建的 pane 没有 -e 投递，签名加世代后缀强制其失效重建。
const PANE_ENV_DELIVERY_GENERATION = 'paneenv1';
const CODEX_APP_SERVER_PANE_ENV_KEYS = [
  'OPENAI_API_KEY',
  'OPENAI_BASE_URL',
...
function codexAppServerPaneEnv(env) {
  const paneEnv = {};
  for (const key of CODEX_APP_SERVER_PANE_ENV_KEYS) {
    if (typeof env[key] === 'string' && env[key]) paneEnv[key] = env[key];
  }
  return paneEnv;
}

function appServerEnvSignature(env = {}, options = {}) {
  ...
  return String(options.platform || '') === 'win32'
    ? `${digest}-${PANE_ENV_DELIVERY_GENERATION}`
    : digest;
}
...
async function ensureCodexAppServerEndpoint(options = {}) {
  ...
  const envSignature = appServerEnvSignature(gatewayEnv, { platform });
  const existing = readAppServerState(aiHomeDir, runtimeScope);
  ...
  // `-c` 传不了 provider 显示名，缺 name 的受管表会让 codex 拒绝加载配置、app-server 秒退。
  ...
  const spawned = spawnDetachedTmuxRun({
    ...
    cwd: os.homedir(),
    env,
    // win32：psmux broker 环境会顶掉同名 spawn env，鉴权/身份变量必须走 -e 显式投递。
    ...(platform === 'win32' ? { paneEnv: codexAppServerPaneEnv(env) } : {}),
    multiplexerBinding
  });
  if (!spawned.ok) {
...
module.exports = {
  APP_SERVER_PINS_GATEWAY_PROVIDER,

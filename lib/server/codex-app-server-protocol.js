...
// confirm/plan(codex 无 plan 语义,降级 confirm) → untrusted + workspace-write(S3 实证组合)。
function buildApprovalPolicyParams(_approvalMode) {
...
function mapServerNotification(message, state) {
  ...
  if (method === 'error') {
    const messageText = normalizeString(params.message) || 'codex_app_server_error';

...
function buildFastThreadReadResponse(payload, deps = {}) {
  // Resume establishes a live writer and returns opaque pagination cursors.
  // Only the upstream app-server can acknowledge it; a rollout tail is not
  // a complete resume response, even when its recent messages look valid.
  ...
  const params = payload.params && typeof payload.params === 'object' && !Array.isArray(payload.params)
    ? payload.params
    : {};
  if (params.includeTurns !== true) return null;
...
module.exports = { durationToMs, truncateText, normalizeCommandSource, normalizeCommandStatus, createEmptyTurn, normalizeStaleInProgressTurns, parseRecentCodexRolloutTurns, readRecentRolloutTurns, buildFastThreadReadResponse };

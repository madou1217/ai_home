'use strict';
// Thread resume domain, maintained here after the one-time split.

const { isNativeOAuthConfig, hasResumeTransportOverride } = require('./codex-native-transport-policy');
...
function reconcileSelectedThreadConfig(payload, deps = {}) {
  ...
  const nativeOAuth = isNativeOAuthConfig(currentConfig);
  // Reading a thread must never pretend its live transport already changed.
  if (nativeOAuth && payload.method !== 'thread/resume') return null;
  const currentModel = nativeOAuth ? '' : currentConfig.model;
  if (!nativeOAuth && !isAihManagedProvider(currentProvider)) return null;
  ...
  for (const stateDbPath of listCodexStateDbPaths(fs, codexHome)) {
    ...
    try {
      ...
      const row = db.prepare('SELECT model_provider, model, cwd FROM threads WHERE id = ?').get(threadId);
      const persistedProvider = String(row && row.model_provider || '').trim();
      const persistedModel = String(row && row.model || '').trim();
      if (nativeOAuth && (!row || (persistedProvider !== 'openai' && !isAihManagedProvider(persistedProvider))
        || hasResumeTransportOverride(fs, payload, payload.params && payload.params.cwd || row.cwd, codexHome))) return null;

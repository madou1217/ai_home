...
const { parseCodexTransportConfig } = require('./codex-native-transport-policy');
// Shared proxy helpers, maintained here after the one-time split.
...
function readCurrentCodexConfig(fs, codexHome, args = []) {
  ...
  try {
    ...
    return parseCodexTransportConfig(content, args);
  } catch (_error) {
...
function readCurrentCodexRuntimeConfig(fs, codexHome, deps = {}) {
  const processObj = deps.processObj || process;
  // 配置来自子进程实际加载的 CODEX_HOME；sqlite_home 仅决定历史存储位置。
  const runtimeHome = String(processObj.env && processObj.env.CODEX_HOME || '').trim()
    ? resolveCodexHome(deps)
    : '';
  if (runtimeHome && runtimeHome !== codexHome) {
    const runtimeConfig = readCurrentCodexConfig(fs, runtimeHome, deps.forwardArgs);
    if (hasCodexRuntimeConfig(runtimeConfig)) return runtimeConfig;
  }
  const explicitStateHome = String(processObj.env && processObj.env.CODEX_SQLITE_HOME || '').trim();
  if (explicitStateHome) {
    const explicitConfig = readCurrentCodexConfig(fs, codexHome, deps.forwardArgs);
  ...
  if (hostCodexHome && hostCodexHome !== codexHome) {
    const hostConfig = readCurrentCodexConfig(fs, hostCodexHome, deps.forwardArgs);
    if (hasCodexRuntimeConfig(hostConfig)) return hostConfig;
  }
  return readCurrentCodexConfig(fs, codexHome, deps.forwardArgs);
}

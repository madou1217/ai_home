...
const { isNativeOAuthConfig } = require('./codex-native-transport-policy');
...
function runCodexAppServerStdioProxy(argv, deps = {}) {
  ...
  if (!state.enabled) {
    ...
    const passthroughTarget = resolveWindowsUpstreamSpawn(parsed.upstream, parsed.forwardArgs, {
      platform: processObj && processObj.platform,
      env: processObj.env || process.env,
      fsImpl: fs
    });
    const child = spawnImpl(passthroughTarget.command, passthroughTarget.args, {
      stdio: 'inherit',
      env: { ...(processObj.env || process.env), ...passthroughTarget.envPatch },
      windowsVerbatimArguments: passthroughTarget.windowsVerbatimArguments === true
    });
    forwardExitCode(child, processObj);
    return child;
  }

  const nativeOAuth = isNativeOAuthConfig(readCurrentCodexRuntimeConfig(
    fs, resolveCodexStateHome({ ...deps, processObj }), { ...deps, processObj, forwardArgs: parsed.forwardArgs }
  ));
  const remoteControlProxy = nativeOAuth ? null : startRemoteControlProxyProcess(fs, state, writeTrace, {
    ...deps,
    processObj
  });
  ...
  // Windows 上游可能是 npm .cmd 垫片：直接 spawn 会 EINVAL，先解析为可直连目标。
  const upstreamTarget = resolveWindowsUpstreamSpawn(parsed.upstream, parsed.forwardArgs, {
    platform: processObj && processObj.platform,
  ...
  const child = spawnImpl(upstreamTarget.command, upstreamTarget.args, {
    ...
    env: { ...spawnEnvResult.env, ...upstreamTarget.envPatch },
  ...
  const desktopAccountSyncEnabled = !nativeOAuth && (deps.enableDesktopAccountSync === true || processObj === process);
  ...
  const stdinPump = createLinePump((line) => {
    ...
    const threadConfigReconcile = reconcileSelectedThreadConfig(parsedPayload, {
      fs,
      processObj: { ...processObj, env: spawnEnvResult.env },
      forwardArgs: parsed.forwardArgs,
      DatabaseSync: deps.DatabaseSync
    });

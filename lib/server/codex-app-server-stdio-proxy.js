...
const { buildRemoteControlProcessObject } = require('./codex-remote-control-account-context');
...
function runCodexAppServerStdioProxy(argv, deps = {}) {
  ...
  const remoteControlProcessObj = buildRemoteControlProcessObject(
    processObj,
    state,
    parsed.stateFile,
    deps
  ...
  const remoteControlProxy = startRemoteControlProxyProcess(fs, state, writeTrace, {
    ...deps,
    processObj: remoteControlProcessObj
  });

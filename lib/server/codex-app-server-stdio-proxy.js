...
function createCodexDesktopAccountSyncController(options = {}) {
  ...
  let activeAccountRef = String(options.initialState && options.initialState.desktopAccountRef || '').trim();
  ...
  for (const stateDbPath of listCodexStateDbPaths(fs, codexHome)) {
    ...
    try {
      ...
      for (const row of dbRows) {
        ...
        if (isCodexSubagentThread(row, spawnedChildIds)) continue;
...
function patchThreadListVisibilityResponse(line, context = {}, options = {}) {
  ...
  // A cursor (request or response) represents a precise upstream page. Only
  // remove hidden child threads here; inserting or sorting current DB rows
  // would advance the cursor while silently dropping older sessions.
  if (context.cursor || parsed.result.nextCursor) {

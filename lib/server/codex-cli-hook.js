...
// cmd 垫片里承载「被包住的那个真二进制」的变量名。导出是因为它是个**跨模块契约**：
// upgrade-channel 判渠道时必须穿透这层 hook 才能看到真正的安装渠道，而 cmd 的
// `set "NAME=值"` 写法没有 POSIX 那种 `exec '路径'` 的自解释性 —— 垫片里 NODE_BIN /
// HELPER 也都是存在的真实文件，靠「取第一个存在的引号串」会解析到 node.exe 上去。
const WRAPPER_UPSTREAM_VAR = 'UPSTREAM';
...
function buildWindowsCmdWrapperScript(options = {}) {
  ...
  return [
    ...
    `REM ${WRAPPER_MARKER}`,
    'setlocal',
    `set "${WRAPPER_UPSTREAM_VAR}=${cmdSetValue(upstreamBinaryPath)}"`,

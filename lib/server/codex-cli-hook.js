...
function buildWindowsPowerShellWrapperScript(options = {}) {
  ...
  return [
    ...
    '$forwardArgs = @($args)',
    'if ($forwardArgs.Count -gt 0 -and $forwardArgs[0] -eq "resume") {',
    ...
    '  exit $LASTEXITCODE',
    '}',
    'if ($forwardArgs.Count -gt 0 -and $forwardArgs[0] -eq "app-server") {',
...
function buildPowerShellUpstreamDelegateScript(upstreamBinaryPath) {
  return [
    '#!/usr/bin/env pwsh',
    '# aih-codex-upstream-delegate',
    `$upstream = ${powershellQuote(upstreamBinaryPath)}`,
...
function buildCmdUpstreamDelegateScript(upstreamBinaryPath) {
  return [
    '@echo off',
    'REM aih-codex-upstream-delegate',
    `call "${cmdSetValue(upstreamBinaryPath)}" %*`,
    'exit /b %ERRORLEVEL%',
    ''
  ].join('\r\n');
}

function buildUpstreamDelegateScriptForTarget(targetBinaryPath, upstreamBinaryPath) {
  const ext = path.extname(String(targetBinaryPath || '')).trim().toLowerCase();
  if (WINDOWS_POWERSHELL_SHIM_EXTENSIONS.has(ext)) {
    return buildPowerShellUpstreamDelegateScript(upstreamBinaryPath);
  }
  if (WINDOWS_CMD_SHIM_EXTENSIONS.has(ext)) {
    return buildCmdUpstreamDelegateScript(upstreamBinaryPath);
  }
  return buildUpstreamDelegateScript(upstreamBinaryPath);
}
...
function createCodexCliHookService(deps = {}) {
  ...
  function writeUpstreamDelegate(destinationPath, upstreamBinaryPath) {
    fs.writeFileSync(destinationPath, buildUpstreamDelegateScriptForTarget(destinationPath, upstreamBinaryPath), 'utf8');

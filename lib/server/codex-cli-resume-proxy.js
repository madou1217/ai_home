...
function wireClientToUpstream({ client, upstream, cwd }) {
  ...
  upstream.on('message', (data, isBinary) => {
    sendWhenOpen(client, data, { binary: isBinary });
  });
...
function startCodexCliResumeCwdProxy(remoteConfig, options = {}) {
  const remoteUrl = normalizeString(remoteConfig && remoteConfig.remoteUrl);
  ...
  const host = normalizeString(options.host) || '127.0.0.1';

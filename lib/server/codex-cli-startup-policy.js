...
function buildCodexDefaultCliArgs(fs, env, args, authMode, options = {}) {
  const policyArgs = authMode === 'apikey' && env.AIH_CODEX_GATEWAY_ACCOUNT_REF && !hasCodexModelProviderArg(args)
    ? buildCodexProviderArgs(env) : buildCodexStartupPolicyArgs();
  const explicitTransport = args.some(arg => ['--remote', '--profile', '-p', '--oss', '--local-provider'].includes(arg)
    || ['--remote=', '--profile=', '--local-provider=', '-cmodel_provider='].some(prefix => String(arg).startsWith(prefix)));
  if (authMode === 'oauth' && !hasCodexModelProviderArg(args) && !explicitTransport) {
    // Explicit invocation override also wins over a resumed thread's old
    // provider; the inactive aih_server registration remains on disk.
    policyArgs.push('-c', 'model_provider=openai');
  }
  if (authMode === 'apikey') {

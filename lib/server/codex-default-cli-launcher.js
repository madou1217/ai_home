...
function buildCodexDefaultCliEnv(fs, options = {}) {
  ...
  const apiKey = record && record.provider === 'codex'
    ...
    : '';
  if (apiKey) {
    Object.assign(env, readCodexGatewayConnection(fs, aiHomeDir, accountRef).env);
    return { env, aiHomeDir, accountRef, authMode: 'apikey' };
  }
...
function runCodexDefaultCli(upstream, args, options = {}) {
  ...
  const spawnTarget = resolveWindowsUpstreamSpawn(upstreamBinary, cliArgs, {
  ...
  const child = spawnImpl(spawnTarget.command, spawnTarget.args, {
    stdio: 'inherit',
    env: { ...runtime.env, ...spawnTarget.envPatch },

...
function buildApiKeyIdentity(artifacts) {
  if (!artifacts) return null;
  const apiKey = String(
    artifacts.envJson.OPENAI_API_KEY
    || artifacts.authJson.OPENAI_API_KEY
    || ''
  ).trim();
  ...
  return {
    ...
    apiKey,
    baseUrl: String(artifacts.envJson.OPENAI_BASE_URL || artifacts.authJson.OPENAI_BASE_URL || '').trim(),
...
function buildCodexDesktopAccountIdentity(artifacts, options = {}) {
  return buildApiKeyIdentity(artifacts) || buildChatGptIdentity(artifacts, options);
}
...
function validateCodexDesktopAccount(fs, options = {}) {
  ...
  if (!identity) {
    return { ok: false, code: 'missing_codex_desktop_auth' };
  }
  return {
    ok: true,
    accountId,
...
function buildCodexDesktopRuntimeAuth(identity) {
  ...
  if (identity.authType === CODEX_DESKTOP_AUTH_TYPES.API_KEY) {
    return { OPENAI_API_KEY: identity.apiKey };
  }

...
function resolveCodexDesktopBundlePath(fs, aiHomeDir, hostHomeDir, options = {}) {
  ...
  const candidates = [
    ...
    ...(options.allowSystemApplications === false
      ? []
      : CODEX_DESKTOP_BUNDLE_NAMES.map((bundleName) => path.join('/Applications', bundleName)))
  ];
  for (const candidate of candidates) {
    if (!candidate) continue;
    // 缓存路径同样受宿主边界约束，不能绕过隔离 HOME 的系统目录限制。
    if (options.allowSystemApplications === false) {
      const relative = path.relative(path.resolve(hostHomeDir), path.resolve(candidate));
...
function createCodexDesktopHookService(deps = {}) {
  ...
  function resolvePaths() {
    ...
    // A missing host home is ambiguous: falling through to the global
    // /Applications scan lets an isolated test/runtime rewrite the user's
    // real ChatGPT.app bundle. Require the caller to provide the host scope
    // explicitly before any bundle discovery or mutation is attempted.
    if (!hostHomeDir) {
      return {
        bundlePath: '',
        targetBinaryPath: '',
        upstreamBinaryPath: '',
        reason: 'host_home_required'
      };
    }
    const processHomeDir = String(
      processObj.env && (processObj.env.AIH_HOST_HOME || processObj.env.HOME || processObj.env.USERPROFILE)
      || ''
    ).trim();
    const allowSystemApplications = Boolean(
      processHomeDir
      && pathImpl.resolve(processHomeDir) === pathImpl.resolve(hostHomeDir)
      // HOME/AIH_HOST_HOME 在测试和投影中可同时被重写，必须再核对 OS 用户目录。
      && pathImpl.resolve(hostHomeDir) === pathImpl.resolve((deps.userInfo || os.userInfo)().homedir)
    );
    const bundlePath = resolveCodexDesktopBundlePath(fs, aiHomeDir, hostHomeDir, {
      allowSystemApplications
    });
    if (!bundlePath) {
      return {
        bundlePath: '',
  ...
  function activate() {
    ...
    if (!paths.bundlePath) {
      return { ok: true, supported: true, enabled: false, reason: paths.reason || 'codex_app_not_found' };
    }
    const previous = readState();
  ...
  function ensureInstalled() {
    ...
    const paths = resolvePaths();
    if (!paths.bundlePath) {
      const reason = paths.reason || 'codex_app_not_found';
      writeState(false, paths, { reason });
      return {
        ok: true,
        supported: true,
        enabled: false,

...
function filterCodexModelEntries(payload) {
...
function parseCodexModelsResponse(payload) {
  return filterCodexModelEntries(payload)
    .map((model) => toPlainText(model.slug || model.id || model.model || '').trim());
}

function parseCodexModelCatalog(payload) {
  if (!Array.isArray(payload && payload.models)) return [];
  return filterCodexModelEntries(payload)
    .filter((model) => Boolean(toPlainText(model.slug || '').trim()));
}

async function fetchCodexModelCatalogForAccount(ctx) {
  ...
  if (account && account.upstreamAccountId) {
    headers['chatgpt-account-id'] = account.upstreamAccountId;
  }
  ...
  return {
    ids: parseCodexModelsResponse(parsed),
    models: parseCodexModelCatalog(parsed)
  };
}

async function fetchCodexModelsForAccount(ctx) {
  const catalog = await fetchCodexModelCatalogForAccount(ctx);
  return catalog.ids;
}

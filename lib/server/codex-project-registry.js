...
function removeTomlTable(content, tableHeader) {
  ...
  return {
    content: output.join(lineEnding),
    removed
  };
}

function removeCodexProjectRegistration(projectPath, options = {}) {
  const normalizedProjectPath = normalizeProjectPath(projectPath);
  if (!normalizedProjectPath) {
    return { ok: false, updated: false, reason: 'missing_project_path' };
  }
  ...
  const projectHeader = `[projects."${escapeTomlBasicString(normalizedProjectPath)}"]`;
  const currentContent = String(nodeFs.readFileSync(configPath, 'utf8') || '');
  const next = removeTomlTable(currentContent, projectHeader);
  if (next.removed) nodeFs.writeFileSync(configPath, next.content, 'utf8');
  return {
    ok: true,
    updated: next.removed,
    configPath,
    projectPath: normalizedProjectPath
  };
}

...
} = require('./runtime-account-egress-request-options');
...
async function proxyHttpRequest(req, res, options) {
  ...
  const resolveRequestOptions = typeof options.resolveRuntimeAccountEgressRequestOptions === 'function'
    ? options.resolveRuntimeAccountEgressRequestOptions
    : resolveRuntimeAccountEgressRequestOptions;
  const accountRequestOptions = await resolveRequestOptions({
    fs: options.fs,
    ...
    resolveAccountEgressRuntimeProxy: options.resolveAccountEgressRuntimeProxy
  });
  if (!accountRequestOptions?.ok || !accountRequestOptions.options) {
    ...
    res.statusCode = 503;
    res.end('account_egress_unavailable');
    return;
  }
  ...
  try {
    ...
    if (accountRequestOptions.bound === true) {
      const proxyFetch = typeof options.proxyFetch === 'function'
        ? options.proxyFetch
        : fetchWithTimeout;
      upstreamResponse = await proxyFetch(target, requestInit, 30_000, {
        proxyUrl: accountRequestOptions.options.proxyUrl,
        noProxy: accountRequestOptions.options.noProxy
      });
    } else {
      const directFetch = typeof options.fetchImpl === 'function' ? options.fetchImpl : fetch;
      upstreamResponse = await directFetch(target, requestInit);
    }
  } catch (error) {
    writeTrace({
      direction: 'http_from_chatgpt',
...
function attachWebSocketProxy(server, options) {
  ...
  server.on('upgrade', async (req, socket, head) => {
    ...
    const connectWebSocket = typeof options.createAccountWebSocket === 'function'
      ? options.createAccountWebSocket
    ...
    try {
      upstream = await connectWebSocket({
        ...
        processObj: options.processObj,
        provider: 'codex',
        account: { accountRef: options.accountRef },
        ...
        WebSocket,
        resolveAccountEgressRequestOptions: (input) => resolveRequestOptions({
          ...input,
          fs: options.fs,
          aiHomeDir: options.aiHomeDir,
          processObj: options.processObj,
      ...
      try {
        socket.write(accountEgressFailed
...
function startCodexRemoteControlProxy(options = {}) {
  ...
  const server = http.createServer((req, res) => {
    proxyHttpRequest(req, res, {
      ...
      accountRef: options.accountRef,
      resolveAccountEgressRuntimeProxy: options.resolveAccountEgressRuntimeProxy,
      resolveRuntimeAccountEgressRequestOptions: options.resolveRuntimeAccountEgressRequestOptions,
  ...
  attachWebSocketProxy(server, {
    ...
    resolveAccountEgressRuntimeProxy: options.resolveAccountEgressRuntimeProxy,
    resolveRuntimeAccountEgressRequestOptions: options.resolveRuntimeAccountEgressRequestOptions,
    createAccountWebSocket: options.createAccountWebSocket
  });
...
if (require.main === module) {
  ...
  const server = startCodexRemoteControlProxy({
    ...
    accountRef: process.env.AIH_REMOTE_CONTROL_ACCOUNT_REF,
    aiHomeDir: process.env.AIH_REMOTE_CONTROL_AI_HOME
  });
  server.on('listening', () => {

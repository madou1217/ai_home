...
function normalizeCredit(value) {
  ...
  const rawExpiresAt = value.expiresAt ?? value.expires_at;
  ...
  return {
    ...
    expiresAt,
    // 缺省 / null = 永不过期；给了值却解析不出来才算明细不可信。
    expiryMalformed: rawExpiresAt !== undefined && rawExpiresAt !== null && expiresAt === null
  };
}
...
function normalizeResetCreditInventory(value) {
  ...
  let hasMalformedAvailableExpiry = false;
  for (const rawCredit of rawCredits) {
    ...
    const { expiryMalformed, ...normalized } = credit;
    if (normalized.status === 'available' && expiryMalformed) hasMalformedAvailableExpiry = true;
  ...
  return {
    availableCount,
    // 协议：credits 为 null 表示只知道数量；列表可能被上游截断（长度 < availableCount）。
    // expiresAt 为 null 表示永不过期，排序时放最后，不算明细缺失。
    detailsComplete: !hasDuplicateCreditId
      && !hasMalformedAvailableExpiry
...
function selectNextResetCredit(credits, now = Date.now()) {
  ...
  return (Array.isArray(credits) ? credits : [])
    .filter((credit) => credit
      ...
      && (credit.expiresAt === null || credit.expiresAt > currentTime))
    .slice()

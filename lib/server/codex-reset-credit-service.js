...
// creditId 为空表示上游选卡：必须省略字段（协议里 null/缺省才触发上游选择）。
function buildConsumeParams(operationId, creditId) {
  const params = { idempotencyKey: operationId };
  if (text(creditId)) params.creditId = text(creditId);
  return params;
}
...
function createCodexResetCreditService(options = {}) {
  ...
  async function fetchInventory(accountRef) {
    ...
    // 明细完整时本地挑最早过期卡；上游只给数量（credits=null 或列表被截断）时，
    // 按协议省略 creditId，由上游选择下一张可用卡。
    const selectionMode = detailsComplete
      ? 'local'
      : (supported && inventory.availableCount > 0 ? 'upstream' : 'none');
    const selectableCount = selectionMode === 'local'
      ? persistedAvailableCount
      : (selectionMode === 'upstream' ? inventory.availableCount : 0);
    return {
      accountRef,
      supported,
      availableCount: inventory.availableCount,
  ...
  async function list(accountRef) {
    ...
    return {
      ...snapshot,
      nextCreditId: snapshot.selectionMode === 'local'
  ...
  async function executeConsume(input) {
    ...
    const selected = snapshot.selectionMode === 'local'
      ? selectNextResetCredit(snapshot.credits, snapshot.capturedAt)
      : (snapshot.selectionMode === 'upstream' ? { creditId: '' } : null);
    if (!selected) {
    ...
    try {
      result = await withTimeout(
        consumeCredit(accountRef, buildConsumeParams(operationId, selected.creditId)),
        consumeTimeoutMs
      );
    } catch (error) {
  ...
  async function executeReconcile(input) {
    ...
    try {
      result = await withTimeout(
        consumeCredit(accountRef, buildConsumeParams(operationId, operation.creditId)),
        consumeTimeoutMs
      );
    } catch (error) {
      const unknown = markResetCreditOperationUnknown(fsImpl, aiHomeDir, {

...
const { createResponseCommitGate } = require('./codex-response-recovery');
...
async function forwardCodexResponseStream({ response, res, signal, timeoutMs, model, native, detectFailure, start }) {
  ...
  const commitGate = createResponseCommitGate();
  let completed = false;
  let usage = null;
  let idleError = null;
  let firstEventAt = 0;
  ...
  try {
    for await (const event of parseSseJsonStream(chunks)) {
      ...
      // created/in_progress is not answer output. Hold that small preamble so
      // a terminal quota rejection can reach the existing account attempt loop.
      const exposed = output.length ? commitGate.push(event, output.join('')) : [];
      if (exposed.length) {
        start();
        for (const frame of exposed) {
          signal.throwIfAborted();

...
function reloadAccountTokensFromAuthSnapshot(account) {
  const authPath = String(account && account.codexAuthPath || '').trim();
  if (!authPath) return false;
  ...
  const refreshToken = sanitizeAccessToken(tokens.refresh_token || tokens.refreshToken);
  if (accessToken) account.accessToken = accessToken;
  if (idToken) account.idToken = idToken;
  if (refreshToken.startsWith('rt_')) account.refreshToken = refreshToken;
...
async function refreshCodexAccessToken(account, options = {}, deps = {}) {
  ...
  reloadAccountTokensFromAuthSnapshot(account);
  ...
  const authPath = String(account.codexAuthPath || '').trim();
  const shouldLockAuthPath = authPath && fs.existsSync(authPath);

  account._lastRefreshAttemptAt = nowMs;
  const doRefresh = async () => {
    reloadAccountTokensFromAuthSnapshot(account);
    const currentRefreshToken = String(account.refreshToken || '').trim();
    if (!currentRefreshToken.startsWith('rt_')) {
      return { ok: false, refreshed: false, reason: 'missing_refresh_token' };
    }
  ...
  const refreshTask = (async () => {
    try {
      if (shouldLockAuthPath) {
        const locked = await withAuthRefreshLock(fs, path, authPath, doRefresh, {
          timeoutMs: Math.max(timeoutMs, 30_000)
        });
        if (!locked.acquired) {
          return { ok: false, refreshed: false, reason: 'refresh_lock_timeout' };
        }
        return locked.value;
      }

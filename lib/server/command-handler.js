...
async function runServerCommand(args, deps) {
  ...
  if (action === 'stop') {
    ...
    try {
      const goCore = typeof serverDaemon.goCoreStatus === 'function'
        ? serverDaemon.goCoreStatus()
        : null;
      if (goCore && goCore.enabled === true && typeof serverDaemon.stopGoCore === 'function') {
        await serverDaemon.stopGoCore();
      }
    } catch (error) {
      console.error(`\x1b[31m[aih] Go Core stop failed: ${error.message}\x1b[0m`);
      return 1;
    }
  ...
  if (action === 'start' || !action || action.startsWith('-')) {
    ...
    try {
      ...
      const goCore = typeof serverDaemon.goCoreStatus === 'function'
        ? serverDaemon.goCoreStatus()
        : null;
      if (goCore && goCore.enabled === true && typeof serverDaemon.startGoCore === 'function') {
        startOptions.startGoCore = true;
      }
      const result = await serverDaemon.start([], startOptions);
      if (result.goCoreFailed) {
        console.error(`\x1b[31m[aih] Go Core start failed: ${result.goCore.error || 'unknown'}\x1b[0m`);
        return 1;
      }
  ...
  if (action === 'restart') {
    const backgroundRestart = isBackgroundRestart(processObj);
    if (args.length > 2 && !backgroundRestart) {
    ...
    try {
      ...
      const goCore = typeof serverDaemon.goCoreStatus === 'function'
        ...
        : null;
      if (goCore && goCore.enabled === true && typeof serverDaemon.startGoCore === 'function') {
        restartOptions.startGoCore = true;
      }
      const result = await serverDaemon.restart(
        backgroundRestart ? args.slice(2) : [],
        restartOptions
      );
      if (result.goCoreFailed) {
        console.error(`\x1b[31m[aih] Go Core start failed: ${result.goCore.error || 'unknown'}\x1b[0m`);

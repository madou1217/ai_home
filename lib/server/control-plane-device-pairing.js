...
const DEFAULT_DEVICE_SCOPES = Object.freeze(['all']);
...
function fullAccessScopes() {
  return DEFAULT_DEVICE_SCOPES.slice();
}
...
function buildDevicePairUrl(invite, code) {
  ...
  if (!endpoint) return '';
  const url = new URL('/v0/fabric/device-pair', endpoint);
...
function buildDeviceWebPairUrl(invite, code) {
  ...
  parsed.pathname = `${basePath}/ui/server-setup`;
  parsed.search = '';
  parsed.hash = '';

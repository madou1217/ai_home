...
function storeSessionCommandAck(command, ack) {
  sessionCommandAcks.set(commandAckKey(command), {
...
async function executeRemoteDevelopmentSessionCommand(payload = {}, deps = {}) {
  ...
  const ack = command.type === 'stop'
    ? executeStopCommand(command, deps)
    : await executeInputCommand(command, deps);
  return storeSessionCommandAck(command, ack);
}

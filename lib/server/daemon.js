...
const { DEFAULT_SERVER_PORT, normalizeServerPort } = require('./server-defaults');
...
function createServerDaemonController(opts) {
  ...
  const defaultPort = normalizeServerPort(options.defaultPort, DEFAULT_SERVER_PORT);
  ...
  function readProcessCommand(pid) {
    ...
    try {
      if (process.platform === 'win32') {
        const out = spawnSync('wmic', ['process', 'where', `ProcessId=${pid}`, 'get', 'CommandLine'], { encoding: 'utf8' });
  ...
  function scheduleBackgroundReadyProbe(pid, port, timeoutMs) {
    ...
    const tick = () => {
      if (!isProcessAlive(pid)) {
        if (readServerPid() === pid) {
          clearPidFile();
        }
        return;
      }
      waitForServerReady(port, 150).then((ready) => {
        if (ready) return;
  ...
  async function start(rawServeArgs, startOptions = {}) {
    ...
    if (isProcessAlive(existingPid)) {
      return { alreadyRunning: true, pid: existingPid, started: true, ready: true, appliedConfig };
    }
    const existingByPort = findServerPidByPort(targetPort);
    if (existingByPort > 0) {
      try { fs.writeFileSync(pidFile, String(existingByPort)); } catch (e) {}
      return { alreadyRunning: true, pid: existingByPort, started: true, ready: true, appliedConfig };
    }
    ...
    if (!waitForReadyEnabled) {
      scheduleBackgroundReadyProbe(child.pid, targetPort, readyTimeoutMs);
      return {
    ...
    const started = await waitForServerReady(parsed.port, readyTimeoutMs);
    ...
    return { alreadyRunning: false, pid: child.pid, started, ready: started, failed: !started, appliedConfig };
  }
...
`;
  ...
  async function restart(rawServeArgs, restartOptions = {}) {
    const stopped = stop(restartOptions.stopOptions);
    const started = await start(rawServeArgs || [], restartOptions.startOptions);
    return {
      stopped,
      started,

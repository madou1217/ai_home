...
const { getEnvironmentTool } = require('../cli/services/toolkit/environment/catalog');
...
const { probeEnvironmentTool } = require('../cli/services/toolkit/environment/probe');
const {
  attachSseWatcher,
  broadcastSseJson,
  openSseStream,
...
function createEnvironmentToolJobManager(options = {}) {
  ...
  const probeTool = typeof options.probeTool === 'function'
    ? options.probeTool
    : (toolId) => probeEnvironmentTool(getEnvironmentTool(toolId), options);

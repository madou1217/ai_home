...
function sanitizeDiagnosticReasons(value) {
  ...
  return input.map((item) => {
    ...
    const out = {
      ...
      sampleAccountIds: normalizeStringList(source.sampleAccountIds, [], 64).slice(0, 5)
    };
...
function buildHeartbeatBootstrapInput(payload = {}, nodeInput = {}, nodeId = '', ownerDeviceId = '', deps = {}) {
  ...
  return {
    node: {
      ...
      id: nodeId,
      roles,
      ownerDeviceId: ownerDeviceId || normalizeId(nodeInput.ownerDeviceId || payload.ownerDeviceId),
...
function heartbeatFabricNode(input = {}, deps = {}) {
  ...
  if (!existing) {
    return registerFabricNode(
      buildHeartbeatBootstrapInput(payload, nodeInput, nodeId, ownerDeviceId, deps),
      deps
    );
  }

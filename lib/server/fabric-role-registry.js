...
const { listProvidersByCapability, providerSupports } = require('../provider-catalog');

const FABRIC_REGISTRY_KEY = 'fabric:registry';
...
const FABRIC_RUNTIME_PROVIDERS = Object.freeze(listProvidersByCapability('fabricRuntime'));
...
const FABRIC_NODE_FIELDS = Object.freeze([
  'id',
  'name',
  'roles',
  'platform',
...
const FABRIC_RELAY_NODE_FIELDS = Object.freeze([
  'id',
  'nodeId',
  'enabled',
  'capacityClass',
...
function sanitizeDiagnosticReasons(value) {
  const input = Array.isArray(value) ? value : [];
  return input.map((item) => {
    const source = item && typeof item === 'object' ? item : {};
    const reason = normalizeText(source.reason, 160);
    ...
    const out = {
      ...
      sampleAccountRefs: normalizeStringList(source.sampleAccountRefs, [], 64).slice(0, 5)
    };
    if (Number.isFinite(retryAt) && retryAt > 0) {
      out.retryAt = Math.floor(retryAt);
    }
    return out;
...
function projectObjectList(value, fields) {
...
function normalizeRegistry(input) {
  ...
  return {
    ...
    nodes: projectObjectList(source.nodes, FABRIC_NODE_FIELDS),
    relayNodes: projectObjectList(source.relayNodes, FABRIC_RELAY_NODE_FIELDS),
    ...
    runtimeDiagnostics: Array.isArray(source.runtimeDiagnostics) ? source.runtimeDiagnostics : [],
    networkMeasurements: Array.isArray(source.networkMeasurements) ? source.networkMeasurements : []
  };
}

function readFabricRegistry(deps = {}) {
  return normalizeRegistry(readJsonValue(deps.fs, deps.aiHomeDir, FABRIC_REGISTRY_KEY));
}

function writeFabricRegistry(registry, deps = {}) {
  const normalized = normalizeRegistry(registry);
  if (!writeJsonValue(deps.fs, deps.aiHomeDir, FABRIC_REGISTRY_KEY, normalized)) {
    throw new Error('fabric_registry_write_failed');
  }
  return normalized;
}
...
function normalizeProvider(value) {
  ...
  return providerSupports(provider, 'fabricRuntime') ? provider : '';
}
...
function sanitizeRuntimeDiagnostic(input = {}, nodeId, deps = {}) {
  const source = input && typeof input === 'object' ? input : {};
  const provider = normalizeProvider(source.provider);
  if (!provider) return null;
  const cli = source.cli && typeof source.cli === 'object' ? source.cli : {};
  const accounts = source.accounts && typeof source.accounts === 'object' ? source.accounts : {};
  const id = normalizeId(source.id) || normalizeId(`${nodeId}-${provider}-runtime-diagnostic`);
  if (!id) return null;
  const now = nowMs(deps);
  const accountTotal = Number(accounts.total);
  const unavailable = Number(accounts.unavailable);
  const hasSchedulable = Object.prototype.hasOwnProperty.call(accounts, 'schedulable');
  const hasAvailable = Object.prototype.hasOwnProperty.call(accounts, 'available');
  const schedulableAccounts = hasSchedulable
    ? Number(accounts.schedulable)
    : (hasAvailable ? Number(accounts.available) : accountTotal);
  return {
    id,
    nodeId,
    provider,
    cli: {
      command: normalizeText(cli.command || provider, 64),
      available: cli.available === true,
      path: normalizeText(cli.path || '', 2048),
      error: normalizeText(cli.error || '', 160)
    },
    accounts: {
      total: Number.isFinite(accountTotal) && accountTotal > 0 ? Math.floor(accountTotal) : 0,
      schedulable: Number.isFinite(schedulableAccounts) && schedulableAccounts >= 0 ? Math.floor(schedulableAccounts) : 0,
      available: Number.isFinite(schedulableAccounts) && schedulableAccounts >= 0 ? Math.floor(schedulableAccounts) : 0,
      unavailable: Number.isFinite(unavailable) && unavailable >= 0
        ? Math.floor(unavailable)
        : Math.max(0, (Number.isFinite(accountTotal) ? Math.floor(accountTotal) : 0) - (Number.isFinite(schedulableAccounts) ? Math.floor(schedulableAccounts) : 0)),
      reasons: sanitizeDiagnosticReasons(accounts.reasons),
      source: normalizeText(accounts.source || '', 64),
      error: normalizeText(accounts.error || '', 160)
    },
    checkedAt: numberOrZero(source.checkedAt) || now,
    updatedAt: now
  };
}
...
function isActiveFabricPromotion(promotion, deps = {}) {
  if (!promotion || promotion.remoteRequestReady !== true) return false;
  const expiresAt = Number(promotion.expiresAt || 0);
  return !expiresAt || expiresAt > nowMs(deps);
}

function findExistingFabricPromotion(registry, nodeId, input = {}, deps = {}) {
  const existing = findNodeTransport(registry && registry.transports, nodeId, input);
  const promotion = existing && existing.promotion && typeof existing.promotion === 'object'
    ? existing.promotion
    : null;
  return isActiveFabricPromotion(promotion, deps) ? promotion : null;
}

function sanitizeFabricTransport(input = {}, node, deps = {}, options = {}) {
  const source = input && typeof input === 'object' ? input : {};
  ...
  const hasPromotionInput = Object.prototype.hasOwnProperty.call(source, 'promotion');
  const promotion = hasPromotionInput
    ? normalizeTransportPromotion(source.promotion)
    : normalizeTransportPromotion(undefined, options.previousPromotion);
...
function hasRelayHeartbeatIntent(payload = {}, transportInputs = []) {
  const relayInput = payload.relayNode || payload.relay || null;
  if (relayInput && typeof relayInput === 'object') return true;
  if (normalizeText(payload.relayStatus, 32)) return true;
  return transportInputs.some((transport) => {
    return normalizeFabricTransportKind(transport && transport.kind) === 'relay';
  });
}

function buildHeartbeatBootstrapInput(payload = {}, nodeInput = {}, nodeId = '', deps = {}) {
  const transportInputs = Array.isArray(payload.transports) ? payload.transports : [];
  const baseRoles = normalizeRoleList(nodeInput.roles || (nodeInput.role ? [nodeInput.role] : ['node']));
  const roles = hasRelayHeartbeatIntent(payload, transportInputs)
    ? Array.from(new Set(baseRoles.concat('relay-node')))
    : baseRoles;
  const relayInput = payload.relayNode || payload.relay || (
    normalizeText(payload.relayStatus, 32) ? { status: normalizeText(payload.relayStatus, 32) } : null
  );
  const now = nowMs(deps);
  return {
    node: {
    ...
    relayNode: relayInput || undefined,
    transports: transportInputs,
    runtimeDiagnostics: Array.isArray(payload.runtimeDiagnostics) ? payload.runtimeDiagnostics : []
  };
}
...
function findLegacyPromotion(nodeId, transport, deps = {}) {
  if (!transport || !nodeId) return null;
  const transportId = normalizeId(transport.id);
  const kind = normalizeTransportKind(transport.kind);
  const existing = listNodeTransports(nodeId, deps).find((entry) => {
    if (transportId && normalizeId(entry && entry.id) === transportId) return true;
    return kind && normalizeTransportKind(entry && entry.kind) === kind;
  });
  return existing && existing.promotion && typeof existing.promotion === 'object'
    ? existing.promotion
    : null;
}

function isActiveRuntimeRpcPromotion(promotion, deps = {}) {
  if (!promotion || promotion.remoteRequestReady !== true) return false;
  if (normalizeText(promotion.mode, 64) !== 'management-rpc') return false;
  const expiresAt = Number(promotion.expiresAt || 0);
  return !expiresAt || expiresAt > nowMs(deps);
}

function selectLegacyPromotion(fabricPromotion, existingPromotion, deps = {}) {
  if (
    isActiveRuntimeRpcPromotion(existingPromotion, deps)
    && !isActiveRuntimeRpcPromotion(fabricPromotion, deps)
  ) {
    return existingPromotion;
  }
  return fabricPromotion || existingPromotion || null;
}

function mirrorLegacyNode(node, transports, deps = {}, options = {}) {
  ...
  transports.forEach((transport) => {
    ...
    const existingPromotion = options.preserveExistingPromotion === true
      ? findLegacyPromotion(node.id, transport, deps)
      : null;
    const promotion = selectLegacyPromotion(transport.promotion || null, existingPromotion, deps);
    ...
    if (transport.kind === 'webrtc' && !normalizeText(transport.endpoint, 2048)) {
      delete legacyTransport.score;
      delete legacyTransport.lastError;
      delete legacyTransport.trustLevel;
    }
    upsertRemoteTransport(legacyTransport, deps);
  });
}
...
function serializeFabricRegistry(registry) {
  ...
  return {
    ...normalized,
    nodeInventory: buildFabricNodeInventory(normalized),
    counts: {
...
function registerFabricNode(input = {}, deps = {}) {
  ...
  const runtimeDiagnosticInputs = Array.isArray(payload.runtimeDiagnostics) ? payload.runtimeDiagnostics : [];
  const relayInput = payload.relayNode || payload.relay || {};
  ...
  const runtimes = runtimeInputs
    ...
    .filter(Boolean);
  const runtimeDiagnostics = runtimeDiagnosticInputs
    .map((diagnostic) => sanitizeRuntimeDiagnostic(diagnostic, node.id, deps))
    .filter(Boolean);
  ...
  const transports = transportInputs
    .map((transport) => sanitizeFabricTransport(transport, node, deps, {
      previousPromotion: findExistingFabricPromotion(registry, node.id, transport, deps)
    }))
    .filter(Boolean);
  ...
  const next = normalizeRegistry({
    ...
    runtimeDiagnostics: mergeWithoutNode(registry.runtimeDiagnostics, node.id).concat(runtimeDiagnostics),
...
function heartbeatFabricNode(input = {}, deps = {}) {
  ...
  if (!existing) {
    return registerFabricNode(
      buildHeartbeatBootstrapInput(payload, nodeInput, nodeId, deps),
      deps
    );
  }
  ...
  const next = normalizeRegistry({
    ...
    runtimeDiagnostics: runtimeDiagnostics.length > 0
      ? mergeWithoutNode(registry.runtimeDiagnostics, node.id).concat(runtimeDiagnostics)
      : registry.runtimeDiagnostics,
    networkMeasurements: appendNetworkMeasurements(registry, node, updatedTransports, deps)
  });
  writeFabricRegistry(next, deps);
  mirrorLegacyNode(node, updatedTransports, deps, { preserveExistingPromotion: true });

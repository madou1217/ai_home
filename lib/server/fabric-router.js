...
const { handleFabricRouteProofRequest } = require('./fabric-route-proof');
...
async function handleFabricRequest(ctx) {
  ...
  const handledRouteProof = await handleFabricRouteProofRequest(ctx);
  if (handledRouteProof) return true;

...
function createGoCoreGatewayForwarder(deps = {}) {
  ...
  function shouldDefer(entryId, headers, transport) {
    if (!deferToNode) return false;
    return Boolean(deferToNode({ entryId, transport, pinnedAccountRef: firstHeader(headers[PINNED_ACCOUNT_HEADER]) }));
  }
  ...
  function tryHandleHttp(req, res, ctx = {}) {
    ...
    if (shouldDefer(entryId, req.headers, 'http')) return false;
  ...
  function tryHandleUpgrade(req, socket, head, ctx = {}) {
    ...
    if (!entryId) return false;
    if (shouldDefer(entryId, req.headers, 'websocket')) return false;

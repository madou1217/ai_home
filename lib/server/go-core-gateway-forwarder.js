...
function createGoCoreGatewayForwarder(deps = {}) {
  ...
  async function shouldDefer(entryId, headers, transport, model) {
    if (!deferToNode) return false;
    return Boolean(await deferToNode({
      entryId, transport, model, pinnedAccountRef: firstHeader(headers[PINNED_ACCOUNT_HEADER])
    }));
  }
  ...
  async function tryHandleHttp(req, res, ctx = {}) {
    ...
    const model = bufferedBody ? readRequestModel(ctx.pathname, bufferedBody) : '';
    if (await shouldDefer(entryId, req.headers, 'http', model)) {
    ...
    const forwardHeaders = buildForwardRequestHeaders(req.headers, target, ctx.requestId);

...
function reloadAccountTokensFromAuthSnapshot(account, deps = {}) {
  ...
  const profile = readGrokAuthProfile(authJson);
  const accessToken = sanitizeAccessToken(profile.accessToken);
  const refreshToken = sanitizeAccessToken(profile.refreshToken);
  if (!accessToken && !refreshToken) return false;

  if (accessToken) account.accessToken = accessToken;
  if (refreshToken) account.refreshToken = refreshToken;
  if (Number.isFinite(profile.tokenExpiresAt) && profile.tokenExpiresAt > 0) {
    account.tokenExpiresAt = profile.tokenExpiresAt;
  }
  if (profile.email) account.email = profile.email;
  if (profile.oauthClientId) account.oauthClientId = profile.oauthClientId;
  return true;
}

...
function readRequestBody(req, options = {}) {
  ...
  // Go Core 转发判定已读完请求体后交还 Node 时，复用已缓冲的体（流已被消费）。
  if (req && Buffer.isBuffer(req.aihBufferedBody)) {
    if (enforceLimit && req.aihBufferedBody.length > maxBytes) {
      const err = new Error('request_body_too_large');
      err.code = 'request_body_too_large';
      return Promise.reject(err);
    }
    return Promise.resolve(req.aihBufferedBody);
  }

...
function mapAgyModelToUpstream(model) {
  ...
  if (Object.prototype.hasOwnProperty.call(agyModelMap, normalized)) {
    return agyModelMap[normalized];
  }
...
function sanitizeSchemaForGemini(schema) {
  ...
  for (const [key, value] of Object.entries(schema)) {
    ...
    if (key === 'type' && Array.isArray(value)) {
      const types = value.filter((t) => typeof t === 'string' && t.trim().toLowerCase() !== 'null');
      if (types.length > 0) {
        result[key] = types[0];
      } else {
        result[key] = 'string';

...
function buildGeminiAccountSessionMapKey(account, externalKey) {
...
function buildGeminiCodeAssistSessionState(options, account, requestJson) {
  ...
  if (isGeminiCodeAssistSessionId(externalKey)) {
    ...
    const found = findGeminiSessionMapEntry(map, [
      mapKey,
      buildGeminiAccountSessionMapKey(account, externalKey)
    ]);
...
async function fetchModelsForAccount(options, account, timeoutMs = 8000) {
  ...
  if (provider === 'claude' && isClaudeAuthTokenAccount(account)) {
    return [];
  }
  if (
    provider === 'codex'
  ...
  const claudeApiKey = provider === 'claude'
    && (account.apiKeyMode || String(account.authType || '').trim().toLowerCase() === 'api-key');
  const headers = {};
  if (claudeApiKey) {
  ...
  if (provider === 'claude') {
    ...
    if (!account.apiKeyMode && account.authType !== 'api-key') {
  ...
  if (!res.ok) {
    ...
    throw new Error(`HTTP ${res.status} ${sanitizeResponseText(text, 160)}`.trim());
  }

...
function mapAgyModelToUpstream(model) {
  const normalized = String(model || '').trim().toLowerCase();
  if (normalized.includes('3.5 flash') || normalized.includes('3.5-flash')) {
    if (normalized.includes('low')) return 'gemini-2.5-flash';
    if (normalized.includes('medium')) return 'gemini-2.5-flash';
    if (normalized.includes('high')) return 'gemini-3-flash-preview';
    return model;
  }
  if (normalized.includes('3.1 pro') || normalized.includes('3.1-pro')) {
    if (normalized.includes('preview')) return 'gemini-3.1-pro-preview';
    if (normalized.includes('low')) return 'gemini-2.5-pro';
    if (normalized.includes('high')) return 'gemini-3.1-pro-preview';
    return model;
  }
  if (normalized.includes('claude') || normalized.includes('sonnet') || normalized.includes('opus')) {
    if (normalized.includes('opus')) return 'gemini-3.1-pro-preview';
    if (normalized.includes('thinking') || normalized.includes('sonnet')) return 'gemini-2.5-pro';
    return model;
  }
  if (normalized.includes('gpt-oss') || normalized.includes('gpt_oss') || normalized.includes('gpt oss')) {
    if (normalized.includes('medium')) return 'gemini-2.5-pro';
    return model;
  }
  return model;
}
...
async function buildGeminiCodeAssistGenerateContext(options, account, requestJson, timeoutMs) {
  ...
  const provider = String(account && account.provider || '').trim().toLowerCase();
  if (provider === 'agy') {
    model = mapAgyModelToUpstream(model);
  }
...
async function fetchGeminiCodeAssistChatCompletion(options, account, requestJson, timeoutMs = 8000) {
  ...
  const originalModel = requestJson && requestJson.model;

...
async function fetchModelsForAccount(options, account, timeoutMs = 8000) {
  ...
  const nativeCliModels = await discoverNativeCliModels(options, account, timeoutMs);
  ...
  if (
    ...
    const planModels = await fetchZcodePlanBalanceModels(options, account, timeoutMs);
    if (planModels) return planModels;
    // zcodeJwtToken 缺失或 balance 接口不可用 → 分支内部回退 paas 探测
    //（accessToken 仍新鲜时 https://api.z.ai/api/coding/paas/v4/models 可用）。
    // 不再 fall through 到通用探测：OAuth 账号的 openaiBaseUrl 是推理端点
    //（…/zcode-plan/anthropic），通用探测会拼出错误的 /models URL。
    return fetchZcodePaasModels(options, account, timeoutMs);
  }

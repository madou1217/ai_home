...
async function readImageGenerationResponseText(response, options = {}) {
  ...
  // Native image endpoints can omit both content-type and content-encoding.
  // Keep bytes intact until the shared decoder handles the compression, and
  // bound the decoded JSON as well as the bytes received over the network.
  const decode = (bytes) => {
    try {
      const text = decodeResponseBuffer(bytes, readHeader(response, 'content-encoding'), { maxOutputLength: maxBytes });
      if (Buffer.byteLength(text) > maxBytes) throw responseTooLarge(maxBytes);
      return text;
    } catch (error) {
      if (error.code === 'ERR_BUFFER_TOO_LARGE') throw responseTooLarge(maxBytes);
      throw error;
    }
  };
  try {
    ...
    if (body && typeof body.getReader === 'function') {
      ...
      const bytes = await readBoundedChunks({
        next: () => reader.read(),

...
async function handleImageGenerations(ctx) {
  ...
  const account = chooseServerAccount(pool, state, `image-gen:${provider}`, {
    provider,
    model: request.model,
    sessionKey: String(requestMeta && requestMeta.sessionKey || '').trim()
  });
  if (!account) {
    fail(new ImageGenerationError(503, 'no_available_account', `no healthy ${provider} account can serve model ${request.model}`));
    return true;
  }

  // 3. Resolve the strategy and gate capability.

...
function addDiscoveredEntries(entries, state, provider, account, now) {
  const apiProfile = isApiKeyAccount(account) ? getAccountImageApiProfile(account) : null;
  if (isApiKeyAccount(account) && !apiProfile) return;
  collectAccountModelIds(state, provider, account).forEach((id) => {
    ...
    if (apiProfile && apiProfile.models.length > 0 && !apiProfile.models.some((spec) => spec.id === id)) return;
    const capabilities = apiProfile
    ...
    addEntryAccount(
      ...
      'discovered',
      capabilities,
      apiProfile ? apiProfile.qualityOptions : getNativeImageQualityOptions(provider, id),
      now
    );
  });
}
  ...
  Object.entries(accountsByProvider).forEach(([providerRaw, accounts]) => {
    ...
    (Array.isArray(accounts) ? accounts : []).forEach((account) => {
      ...
      if (!isApiKeyAccount(account)) {
        addNativeEntries(entries, provider, account, now);
      } else {
        const profile = getAccountImageApiProfile(account);
        if (profile) profile.models.forEach((spec) => {
          addEntryAccount(
            ensureEntry(entries, provider, spec), account, 'configured',
            profile.capabilities, profile.qualityOptions, now
          );
        });
      }

...
function adaptKimiChatRequestBuffer(buffer) {
  ...
  const needsStreamUsage = source.stream === true && source.stream_options?.include_usage === undefined;
  ...
  if (!hasEffort && !hasBudget && !defaultBudget && !needsStreamUsage) return buffer;
  ...
  if (needsStreamUsage) result.stream_options = { ...source.stream_options, include_usage: true };

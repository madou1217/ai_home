...
async function callAuthService(deps, method, payload, accessToken) {
  ...
  const requestOptionsResult = await resolveProviderAccountEgressRequestOptions({
    account: {
      provider: 'kimi',
      accountRef: String(deps.accountRef || '').trim()
    },
    provider: 'kimi',
    options: {
  ...
  if (!requestOptionsResult?.ok || !requestOptionsResult.options) {
    const failure = describeAccountEgressFailure(requestOptionsResult);
  ...
  try {
    const url = `${resolveAuthBaseUrl(deps)}/${AUTH_SERVICE}/${method}`;
    const init = {
      method: 'POST',
      headers: {
    ...
    const res = requestOptionsResult.bound
      ? await resolveFetchWithTimeout(deps)(url, init, timeoutMs, {
          proxyUrl: requestOptionsResult.options.proxyUrl,
          noProxy: requestOptionsResult.options.noProxy
        })
...
async function ensureDesktopSessionAccessToken(fsImpl, aiHomeDir, accountRef, deps = {}) {
  ...
  const refreshed = await refreshDesktopSessionToken({
    ...deps,
    fs: deps.fs || fsImpl,
    aiHomeDir: deps.aiHomeDir || aiHomeDir,
    accountRef: deps.accountRef || accountRef

...
//（ConfirmLoginQRCode / RefreshToken 均实测拒绝 CLI token）。
// 唯一可行入口是官方扫码登录 RPC（auth.kimi.com 的 account.gateway.v1.AuthService）：
//   CreateLoginQRCode → 用户用微信扫码确认 → GetLoginQRCodeStatus(SUCCESS)
//   → 拿到 web access_token + refresh_token → 托管在 nativeAuth.desktopSession，
//   打开桌面 App 时注入隔离 profile 的 localStorage。
...
async function getDesktopLoginQRCodeStatus(deps = {}, code) {
  ...
  if (state === QR_STATUS.SUCCESS) {
    result.accessToken = String(data.access_token || '').trim();
    result.refreshToken = String(data.refresh_token || '').trim();
    result.userId = String(data.user_id || '').trim();
...
async function refreshDesktopSessionToken(deps = {}, refreshToken) {
  ...
  return {
    ok: true,
    accessToken,
    refreshToken: String(data.refresh_token || '').trim() || normalized
  };
}

...
async function refreshKimiAccessToken(account, options = {}, deps = {}) {
  ...
  // 刷新前先吸收 host 侧（kimi-code CLI 在 projection 里自行续期）可能更新的
  // grant：kimi 每次刷新都轮换 refresh_token，若 CLI 刚轮换过而 DB 还是旧快照，
  // 直接拿旧 refresh_token 撞 OAuth 端点只会吃到 invalid_grant 并被 24h 抑制。
  if (typeof deps.reconcileHostCredentials === 'function') {
    try {
      const reconciliation = deps.reconcileHostCredentials(accountRef);

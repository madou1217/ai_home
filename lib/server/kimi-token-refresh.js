...
async function refreshKimiAccessToken(account, options = {}, deps = {}) {
  ...
  const previousAccessToken = String(account.accessToken || '').trim();
  const previousRefreshToken = String(account.refreshToken || '').trim();
  ...
  // 本地凭据已吸收并投影到运行账号后，才应用调用方的请求抑制策略。
  if (!force && typeof deps.shouldSkipRefresh === 'function' && deps.shouldSkipRefresh(account)) {
    return { ok: true, refreshed: false, reason: 'suppressed' };
  }
  if (!force && Number.isFinite(expiresAtMs) && expiresAtMs - nowMs > skewMs) {
    const recovered = Boolean(previousRefreshToken && previousRefreshToken !== refreshToken
      && previousAccessToken !== account.accessToken);
    return { ok: true, refreshed: recovered, reason: recovered ? 'credentials_adopted' : 'not_due' };
  }
  ...
  // Another CLI/server can rotate the single-use grant while OAuth is in flight.
  // Reconcile once after rejection/CAS loss and adopt only a fresh new generation
  // belonging to this account; never retry the rejected grant or overwrite a winner.
  const adoptConcurrentCredentials = () => {
    for (const reconcile of reconcilers) {
      try { reconcile(accountRef); } catch (_error) {}
    }
    const record = readAccountCredentialRecord(fsImpl, aiHomeDir, accountRef);
    if (!record || record.provider !== 'kimi') return null;
    const next = readKimiOAuthCredentials(record.nativeAuth);
    const nextRefresh = readKimiRefreshToken(record.nativeAuth);
    const nextAccess = String(next && next.access_token || '').trim();
    const nextExpiry = resolveKimiTokenExpiryMs(next);
    if (!nextRefresh || nextRefresh === refreshToken || !nextAccess
      || nextAccess === credentials.access_token || !nextExpiry || nextExpiry <= Date.now()) return null;
    return {
      ok: true, refreshed: true, reason: 'credentials_adopted',
      accessToken: nextAccess, refreshToken: nextRefresh, expiresAtMs: nextExpiry,
      tokenType: next.token_type, deviceId: resolveKimiOAuthDeviceId(record.nativeAuth)
    };
  };
  ...
  refreshState.promise = (async () => {
    try {
      ...
      if (status !== 200 || !data || typeof data.access_token !== 'string' || !data.access_token) {
        ...
        const recovered = adoptConcurrentCredentials();
      ...
      if (!persisted) {
        return adoptConcurrentCredentials() || { ok: false, refreshed: false, reason: 'stale_credentials' };
      }

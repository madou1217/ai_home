...
function sortManagementAccounts(accounts) {
  return (Array.isArray(accounts) ? accounts.slice() : []).sort((left, right) => {
    ...
    return String(left && left.accountRef || '').localeCompare(String(right && right.accountRef || ''));
  });
}
...
function notifyManagementWatchers(ctx, notifyOptions = {}) {
  const { state, options, deps } = ctx;
  const liveState = getManagementLiveState(state);
  const { changed, payload } = captureManagementSnapshot(state, options, deps);
  if ((!changed && !notifyOptions.force) || liveState.watchers.size === 0) return false;
  broadcastSseJson(liveState.watchers, payload, {
    onWatcherRemoved: () => {
      if (liveState.watchers.size === 0) {
        stopManagementWatchPoller(liveState);
...
function emitManagementLiveEvent(ctx, payload) {
  if (!payload || typeof payload !== 'object') return false;
  const liveState = getManagementLiveState(ctx.state);
  if (liveState.watchers.size === 0) return false;
  broadcastSseJson(liveState.watchers, payload, {
    onWatcherRemoved: () => {
      if (liveState.watchers.size === 0) {
        stopManagementWatchPoller(liveState);

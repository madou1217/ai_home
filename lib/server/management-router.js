...
async function handleManagementRequest(ctx) {
  ...
  if (method === 'GET' && pathname === '/v0/management/accounts') {
    writeJson(res, 200, buildManagementAccountsPayload(state, { fs, getProfileDir, getToolConfigDir }));
    return true;
  }
  ...
  if (method === 'POST' && pathname === '/v0/management/cooldown/clear') {
    const canPersistRuntimeClear = accountStateIndex && typeof accountStateIndex.upsertRuntimeState === 'function';
    SUPPORTED_SERVER_PROVIDERS.forEach((provider) => {
      ...
      accounts.forEach((a) => {
        ...
        if (!accountId) return;
        accountStateIndex.upsertRuntimeState(provider, accountId, null, {
          configured: true,

...
async function runUsageScanJob(liveState, job, modelUsageService, onTokenUsageUpdated) {
  job.status = 'running';
  ...
  try {
    ...
    if (typeof onTokenUsageUpdated === 'function') {
      try {
        await refreshAccountTokenUsage(modelUsageService, onTokenUsageUpdated, { generatedAt: Date.now() });
      } catch (_error) {
        // Token cache refresh is best effort; the scan itself remains successful.
      }
    }
...
function startUsageScanJob(state, modelUsageService, options = {}) {
  ...
  Promise.resolve()
    .then(() => runUsageScanJob(liveState, job, modelUsageService, options.onTokenUsageUpdated))
    .catch(() => {});

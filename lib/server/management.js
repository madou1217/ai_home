...
const { readTrustedUsageSnapshot, getMinRemainingPctFromUsageSnapshot } = require('./accounts');
...
function buildManagementAccountsPayload(state, deps = {}) {
  const { fs, getProfileDir, getToolConfigDir } = deps;
  ...
  return {
    ...
    accounts: allAccounts.map((a) => ({
      ...
      planType: (() => {
        ...
        if (apiKeyMode) return 'api-key';
        if (provider !== 'codex' || !fs || typeof getToolConfigDir !== 'function') return '';
        try {
          const authPath = path.join(getToolConfigDir(provider, a.id), 'auth.json');
          if (!fs.existsSync(authPath)) return '';
          const authJson = JSON.parse(fs.readFileSync(authPath, 'utf8'));
          return String(extractCodexMetadata(authJson).planType || '').trim();
        } catch (_error) {
          return '';
        }
      })(),
      ...
      remainingPct: (() => {
        ...
        if (rawRemaining !== null && rawRemaining !== undefined && rawRemaining !== '') {
          const runtimeRemaining = Number(rawRemaining);
          if (Number.isFinite(runtimeRemaining)) return runtimeRemaining;
        }
        if (!fs || typeof getProfileDir !== 'function') return null;
        const snapshot = readTrustedUsageSnapshot({ fs, getProfileDir }, a.provider || 'codex', a.id);
        return getMinRemainingPctFromUsageSnapshot(snapshot);
      })(),
      usageSnapshot: (() => {
        if (!fs || typeof getProfileDir !== 'function') return null;
        return normalizeAccountUsageSnapshot(readTrustedUsageSnapshot({ fs, getProfileDir }, a.provider || 'codex', a.id));
      })(),

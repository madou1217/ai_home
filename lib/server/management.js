...
function normalizeMetricErrors(errors) {
  return (Array.isArray(errors) ? errors : []).map((item) => {
    ...
    ['account' + 'Key', 'account_key', 'account_ref'].forEach((field) => {
    ...
    const accountId = String(
      (source.accountId || source.account_id || source.aiHomeAccountId)
      || ''
    ).trim();
    // 管理接口统一给前端读 message；保留 error 字段兼容状态页和旧脚本。
    return {
      ...publicItem,
      provider,
      accountRef,
...
function buildManagementAccountsPayload(state, deps = {}) {
  const { fs, getProfileDir, accountStateIndex } = deps;

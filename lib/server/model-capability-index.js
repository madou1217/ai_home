...
function isAccountModelAllowedByQuota(provider, account, modelId) {
  ...
  // The model catalog and quota snapshot are independent upstream views. New
  // models can appear in fetchAvailableModels before they receive a quota
  // bucket, so a missing bucket is unknown rather than evidence that the
  // account cannot serve the model. Only an explicit exhausted bucket blocks
  // the account-model pair.
  if (!quotaModel) return true;

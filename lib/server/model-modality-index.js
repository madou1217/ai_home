...
// 同步前新上线的模型在本地没有元数据,探测结果是唯一真实来源。按归一化模型 id
// 索引,注册时使对应缓存条目失效;resetModelModalityCache 一并清空。
...
function registerProbedModelModalities(descriptors) {
  (Array.isArray(descriptors) ? descriptors : []).forEach((descriptor) => {
    ...
    if (!id || !modalities) return;
    probedModalityOverrides.set(id, modalities);
    // 缓存键含 provider 前缀(provider|id)和裸 id 两种形态,都要失效。
    for (const key of modalityCache.keys()) {
      if (key === id || key.endsWith(`|${id}`)) modalityCache.delete(key);
    }
  });
}
...
function computeModelModalities(modelId, deps = {}) {
  const override = probedModalityOverrides.get(normalizeModelId(modelId));
...
function getModelModalities(modelId, deps = {}) {
  ...
  const cacheKey = provider ? `${provider}|${id}` : id;

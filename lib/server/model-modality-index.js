...
//
// OpenAI entries are version-ranged rather than enumerated on purpose: an
// enumerated table rots as soon as a new major ships. The previous
// `gpt-(4o|4[.-]1|5)` judged the already-published `gpt-6-astra` as
// text-only, so a vision request for it was rejected upstream instead of
// being sent. The range is validated against the shipped snapshot:
// `^gpt-[4-9]` covers gpt-4.x/5.x/6.x (all image-input in the snapshot) while
// still excluding `gpt-3.5-turbo` and `gpt-oss-*` (text-only there).

 ...
 * @param {string[]} [input.knownAccountRefs]   目录中绑定该模型的全部账号 ref（包括额度耗尽账号）
 * @param {string[]} [input.blockedAccountRefs]  provider 策略明确判定额度耗尽的账号 ref
...
function narrowPoolByModelCatalog(input) {
  ...
  const knownRefs = Array.isArray(knownAccountRefs)
    ? knownAccountRefs.map(normalizeText).filter(Boolean)
    : [];
  const blockedRefs = Array.isArray(blockedAccountRefs)
    ? blockedAccountRefs.map(normalizeText).filter(Boolean)
    : [];
  ...
  // A provider-owned quota strategy is a hard, model-scoped routing fact. Do
  // not let a stale provider catalog or the defensive "unknown catalog" path
  // put an explicitly exhausted account back into the upstream pool.
  if (refs.length < 1 && knownRefs.length > 0 && blockedRefs.length > 0) {
    const known = new Set(knownRefs);
    const blocked = new Set(blockedRefs);
    const knownInPool = poolAccountRefs.filter((accountRef) => known.has(accountRef));
    const blockedInPool = knownInPool.filter((accountRef) => blocked.has(accountRef));
    if (knownInPool.length > 0 && blockedInPool.length === knownInPool.length) {
      return { pool: [], filtered: true, accountRefs: [], blockedByUsage: true };
    }
  }
...
function excludeAccountsWithoutModel(input) {
  ...
  const isProviderImageModel = imageModelBelongsToProvider(provider, modelId);
  const kept = accounts.filter((account) => {
    ...
    // 原生生图模型(如 agy/gemini 的 gemini-3.1-flash-image)属于专用生图能力，
    // 文本探测目录通常不包含它；不能仅凭常规文本探测目录不含生图模型而误排除有效账号。
    if (isProviderImageModel) return true;

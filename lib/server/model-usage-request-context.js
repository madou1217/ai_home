...
function recordAccountPinDiagnostic(appendLog, input = {}) {
  ...
  // Pin validation is gateway-local and finishes before the request body is read,
  // so the recorded operation is non-inference and non-streaming by definition.
  const entry = {
    at: new Date().toISOString(),

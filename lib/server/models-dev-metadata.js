...
function inferBaseModelIds(modelId) {
  ...
  if (/^glm-/i.test(stripped)) candidates.push(`zhipu/${stripped}`);
...
function inferModelsDevProviderIds(aihProvider, modelId) {
  ...
  // kimi OAuth(api.kimi.com/coding)模型挂在 kimi-for-coding provider 目录;
  // api-key(api.moonshot.cn)模型对应 moonshotai-cn/moonshotai 目录。
  if (provider === 'kimi') candidates.push('kimi-for-coding', 'moonshotai-cn', 'moonshotai');
  // 请求路径(vision guard、capability filter)只有模型 id、没有 provider 上下文,
  // kimi coding 端点的模型 id 需要按前缀定位到 kimi-for-coding provider 目录。
  if (/^kimi-for-coding/i.test(stripped) || /^k3(?:-|$)/i.test(stripped)) {
    candidates.push('kimi-for-coding');
  }
  // zcode 的 GLM 模型在 models.dev 挂在智谱目录；z.ai 计划端点的模型同源。
  // 无 provider 上下文的请求路径按 glm- 前缀定位；opencode 网关模型
  // （opencode-go/glm-*)归属 opencode 目录，不混入智谱候选。
  if (provider === 'zcode' || (!provider && /^glm-/i.test(stripped))) {
    candidates.push('zhipu', 'z-ai');
  }

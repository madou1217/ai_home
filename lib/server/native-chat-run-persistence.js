...
function getNativeRunStoreDir(options = {}) {
  ...
  if (!aiHomeDir) return '';
  return resolveAihRunPath(aiHomeDir, 'fabric', 'native-runs');
}
...
function buildRunMeta(run = {}) {
  const gateway = run.gateway === true;
  return {
    runId: normalizeText(run.runId, 160),
    provider: normalizeText(run.provider, 64),
    ...(gateway
      ? { gateway: true }
      : { accountRef: normalizeText(run.accountRef, 96) }),
...
function readPersistedNativeChatRunEvents(runId, options = {}) {
  ...
  const identity = meta.gateway === true
    ? { gateway: true }
    : { accountRef: normalizeText(meta.accountRef, 96) };
  return {

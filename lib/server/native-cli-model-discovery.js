...
async function discoverNativeCliModels(options = {}, account = {}, timeoutMs = 8000) {
  ...
  if (options.aiHomeDir != null && typeof options.aiHomeDir !== 'string') {
    throw new Error('native_cli_model_discovery_invalid_ai_home');
  }
  const aiHomeDir = normalizePathString(options.aiHomeDir)
    || path.join(nodeOs.homedir(), '.ai_home');
  ...
  if (options.hostHomeDir != null && typeof options.hostHomeDir !== 'string') {
    throw new Error('native_cli_model_discovery_invalid_host_home');
  }
  const hostHomeDir = normalizePathString(options.hostHomeDir) || path.dirname(aiHomeDir);
  const runtimeEnv = {

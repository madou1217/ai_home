...
// server 启动时收养 detached multiplexer native run 的孤儿：
// 上一个 server 进程死掉（部署重启/崩溃）时,backend 里的 CLI run 还活着。
...
function adoptWebUiNativeRuns(options = {}) {
  ...
  for (const manifest of listRunManifests(aiHomeDir, { fs })) {
    const multiplexerBinding = resolveRunMultiplexerBinding({
      ...
      multiplexerType: manifest.multiplexer
    });
    if (!multiplexerBinding.available) {
      skipped += 1;
      log(`[aih] skipped webui native run ${manifest.runId}: multiplexer ${manifest.multiplexer} unavailable`);
      continue;
    }
    const runOptions = { ...options, multiplexerBinding };
    const alive = hasRunSession(manifest.socket, runOptions);
    if (!alive) {
      ...
      cleanupRunSocket(manifest.socket, runOptions);
      finalized += 1;
      continue;
    }
    ...
    const handle = {
      ...
      multiplexer: manifest.multiplexer,
      adopted: true,
      getActivePrompt() { return null; },
      writeInput(input, writeOptions = {}) {
        const raw = String(input || '');
        if (!raw) return;
        sendRunKeys(manifest.socket, raw, { ...runOptions, appendNewline: writeOptions.appendNewline !== false });
      },
      resize() { /* headless 无需尺寸 */ },
      abort() {
        killRunServer(manifest.socket, runOptions);
      }
    };
  ...
  return { adopted, finalized, skipped };
}

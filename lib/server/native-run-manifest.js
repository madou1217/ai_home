...
// webUI native run 的磁盘清单（detached multiplexer run 的收养依据）。
//
// 每个 run 一个 JSON（<aiHomeDir>/run/webui-runs/<runId>.json）+ 一个输出日志
// （<aiHomeDir>/logs/webui-runs/<runId>.log）。server 重启后据清单收养仍在 multiplexer 里跑的 run：重新注册进
// run registry（/chat/runs 可见、可 abort/回写输入），tail 日志到完成再走统一收尾。
// 只存寻址元数据（provider/account/sessionId/multiplexer/namespace/日志路径），绝不存凭据——
...
const RUN_MULTIPLEXERS = new Set([
  MULTIPLEXER_TYPE.TMUX,
  MULTIPLEXER_TYPE.HERDR
]);
...
function normalizeRunMultiplexer(value) {
  const normalized = String(value || '').trim().toLowerCase();
  if (!normalized) return MULTIPLEXER_TYPE.TMUX;
  return RUN_MULTIPLEXERS.has(normalized) ? normalized : '';
}
...
function normalizeManifest(raw = {}) {
  ...
  return {
    ...
    multiplexer: normalizeRunMultiplexer(raw.multiplexer),
...
function isCanonicalRunLogPath(aiHomeDir, entry) {
  const expected = entry && runLogPath(aiHomeDir, entry.runId);
  if (!expected || !entry.logPath) return false;
  return path.resolve(entry.logPath) === path.resolve(expected);
}

function isUsableManifest(aiHomeDir, entry) {
  ...
  return Boolean(
    ...
    && entry.multiplexer
    && entry.socket
    && isCanonicalRunLogPath(aiHomeDir, entry)
  );
}
...
function writeRunManifest(aiHomeDir, rawEntry = {}, options = {}) {
  ...
  const logPath = runLogPath(aiHomeDir, entry.runId);
  if (!filePath || !isUsableManifest(aiHomeDir, entry)) return null;
...
function readRunManifest(aiHomeDir, runId, options = {}) {
  ...
  try {
    ...
    return isUsableManifest(aiHomeDir, entry) ? entry : null;
  } catch (_error) {
    return null;
  }
}
...
function listRunManifests(aiHomeDir, options = {}) {
  ...
  for (const name of names) {
    try {
      ...
      if (isUsableManifest(aiHomeDir, entry)) entries.push(entry);
    } catch (_error) { /* 坏条目跳过 */ }
  }
  return entries;
}

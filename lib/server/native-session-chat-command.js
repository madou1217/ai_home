...
function resolveNativeRunMultiplexer(provider, options = {}) {
  if (String(process.env.AIH_WEBUI_TMUX_RUNS || '').trim() === '0') return null;
  if (!TMUX_RUN_PROVIDERS.has(String(provider || '').toLowerCase())) return null;
  if (options.interactiveCli) return null;
...
function shouldUseTmuxNativeRun(provider, options = {}) {
  return Boolean(resolveNativeRunMultiplexer(provider, options));
}
...
function buildResumeCommand(provider, options = {}) {
  ...
  if (['codebuddy', 'codebuddycn', 'workbuddy', 'workbuddycn'].includes(normalizedProvider)) {
    // CodeBuddy Code（@tencent-ai/codebuddy-code）的会话恢复入口：
    //   -r / --resume <sessionId> 续指定会话；-p/--print 非交互打印后退出。
    // 与 qoder 同构：headless 走 --print + stream-json，交互式只带 prompt 位置参数。
    // 会话文件在 <configDir>/projects/<project>/<sessionId>.jsonl，与 Claude 同布局，
    // 且地区内的两个数据根都会探测（见 session-reader-codebuddy）。
    const args = [];
    if (model) args.push('--model', model);
    if (interactiveCli) {
      args.push('--resume', sessionId);
  ...
  if (normalizedProvider === 'agy') {
    if (model && isImageGenerationModel(model)) {
      const error = new Error(`agy CLI does not support image generation model ${model}`);
      error.code = 'agy_image_model_unsupported';
      throw error;
    }
...
function buildStartCommand(provider, options = {}) {
  ...
  if (['codebuddy', 'codebuddycn', 'workbuddy', 'workbuddycn'].includes(normalizedProvider)) {
    // 新建会话：--session-id 固定 uuid（AIH 侧预生成，便于随后按文件定位并续聊），
    // headless 仍走 -p/--print + stream-json。
    const args = [];
    if (model) args.push('--model', model);
    if (interactiveCli) {
      if (options.sessionId) args.push('--session-id', normalizeString(options.sessionId));
...
module.exports = { isOfficialNativeSessionProvider, pushClaudeHeadlessStreamArgs, detectImageMediaType, buildClaudeStreamJsonUserMessage, claudeUsesStreamJsonInput, shellSingleQuote, resolveNativeRunMultiplexer, shouldUseTmuxNativeRun, resolveRunsAiHomeDir, buildResumeCommand, buildStartCommand, applyProviderConfigDirArgs };

...
// （resolveNativeCliLaunch）探测一次；探测到 cli_not_found 才触发自动安装（installNativeCliWithProgress，
// 与 CLI/accounts 页复用同一份 strategy registry），经 onProgress 把每个安装计划的进度回传给调用方——
// webui-chat-routes 把它转成 SSE `cli-install-progress` 事件，落在既有"正在处理中"状态位（不新增 UI 组件）。

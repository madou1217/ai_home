...
function buildResumeCommand(provider, options = {}) {
  ...
  if (normalizedProvider === 'agy') {
    ...
    // 回复由 transcript 读取(completeOnTranscriptUpdate)。`--conversation <id>` 续会话。
...
function spawnNativeSessionStream(options = {}) {
  ...
  if (!gateway && runtime.projectionRequired && typeof options.ensureSessionStoreLinks === 'function') {
    try {
      options.ensureSessionStoreLinks(provider, accountRef);
    } catch (_error) {}
  }
  ...
  if (provider === 'codex') {
    args.unshift(...buildCodexProviderArgs(env, { force: gateway }));
  }
  ...
  if (
    ...
    && agyWarmPool.hasWarm(accountRef)
  ...
  child.onData((chunk) => {
    ...
    if (interactiveCli) {
      ...
      // agy headless(--print):stdout 就是流式模型正文,直接作为 delta 事件推给前端 → 真流式、
      // 首字不再等整轮。最终干净正文仍由 transcript(completeOnTranscriptUpdate)定稿,覆盖流式期
      // 偶发的 agy 状态前言。

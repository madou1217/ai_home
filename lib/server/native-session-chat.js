...
function spawnNativeSessionStream(options = {}) {
  ...
  const beforeSessionIds = (!isResume && provider === 'claude' && sessionParams.projectDirName)
    ? listClaudeProjectSessionFiles(sessionParams.projectDirName).map((item) => item.id)
    : [];
  let lineBuffer = '';
  let pendingTerminal = '';
  let flushTimer = null;
...
async function runNativeSessionPrompt(options = {}) {
  ...
  if (!runResult.ok) {
    const classifiedFailure = classifyNativeSessionFailure(
      provider,
      normalizeString(runResult.stderr) || normalizeString(runResult.stdout)
    );
    const error = new Error(classifiedFailure.message || `native_session_failed_exit_${runResult.exitCode}`);
    error.code = classifiedFailure.code || 'native_session_failed';
    error.retryAnotherAccount = classifiedFailure.retryAnotherAccount === true;

...
function spawnNativeSessionStream(options = {}) {
  ...
  const generatedSessionId = !requestedSessionId && ['gemini', 'claude', 'grok', 'qoder', 'qodercn', 'codebuddy', 'codebuddycn', 'workbuddy', 'workbuddycn'].includes(provider)
  ...
  let launch;
  try {
    launch = (options.resolveNativeCliLaunch || resolveNativeCliLaunch)(provider, {
      claudeCliJsPath: options.claudeCliJsPath,
      hostHomeDir: options.hostHomeDir || env.AIH_HOST_HOME,
      env
    });
  } catch (error) {
    cleanupTransientAuthProjection();

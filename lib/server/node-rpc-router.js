...
function normalizeAccountIdSegment(value) {
  const accountId = String(value || '').trim();
  return /^[A-Za-z0-9._:-]{1,128}$/.test(accountId) ? accountId : '';
}
...
function enrichProviderAccountReauthResult(result, job, targetAccountId) {
  ...
  const jobAccountId = String((serialized && serialized.accountId) || source.accountId || '').trim();
  if (jobAccountId && jobAccountId !== targetAccountId) {
    enriched.transientAccountId = jobAccountId;
  }
  if (serialized) {
...
async function handleDeviceProviderAccountReauthRequest(ctx) {
  ...
  const accountId = normalizeAccountIdSegment(payload.accountId || payload.account_id);
  ...
  await handleReauthAccountRequest({
    ...ctx,
    pathname: `/v0/webui/accounts/${provider}/${accountId}/reauth`,
    res: relayRes,
    fs: ctx.deps.fs,
    accountStateIndex: ctx.deps.accountStateIndex,
    accountStateService: ctx.deps.accountStateService,
    getToolAccountIds: ctx.deps.getToolAccountIds,
    getProfileDir: ctx.deps.getProfileDir,
    getToolConfigDir: ctx.deps.getToolConfigDir,
  ...
  if (statusCode >= 200 && statusCode < 300 && body && body.ok === true) {
    ...
    ctx.deps.writeJson(ctx.res, 200, {
      ...
      result: enrichProviderAccountReauthResult(body, job, accountId)
    });
    return true;
  }
  ctx.deps.writeJson(ctx.res, statusCode, {
...
async function handleNodeSessionStartRequest(ctx) {
  ...
  try {
    ...
    const result = starter(payload, {
      getProfileDir: ctx.deps.getProfileDir,
      resolveSessionAccountId: ctx.deps.resolveSessionAccountId,

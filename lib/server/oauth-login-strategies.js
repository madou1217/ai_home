...
const GEMINI_STRATEGY = {
  ...
  updateProgress({ job, hints, deps }) {
    ...
    if (!/Do you trust the files in this folder\?/i.test(logs)
    ...
    if (!ptyProcess || typeof ptyProcess.write !== 'function') {
      deps.appendJobLog(job, '检测到 Gemini 文件夹信任提示，但当前 PTY 不支持自动输入。');
      job._geminiFolderTrustSelected = true;
      deps.setAuthProgressState(job, deps.states.AWAITING_FOLDER_TRUST);
      return;
    }

    job._geminiFolderTrustSelected = true;
    deps.setAuthProgressState(job, deps.states.AWAITING_FOLDER_TRUST);
    deps.appendJobLog(job, '检测到 Gemini 文件夹信任提示，自动选择 2. Trust parent folder。');
    ptyProcess.write('2\r');
    deps.setAuthProgressState(job, deps.states.FOLDER_TRUST_SELECTED);
  },
...
// ZCode: the CLI's poll-based login endpoint (`zcode.z.ai/api/v1/oauth/cli/init`)
// was retired upstream (404 "OAuth response is not valid JSON"), so aih drives
// the desktop OAuth flow itself — chat.z.ai authorize accepts a loopback
// redirect_uri, which gives same-machine auto-capture plus the paste fallback.

...
//   ensureOpenCodeServe(账号级 multiplexer 常驻,socket=aih-ocserve-<accountRef>,端口按账号确定性派生)
...
const { MULTIPLEXER_TYPE } = require('../runtime/multiplexer');
...
function serveStatePath(aiHomeDir, runtimeScope) {
  const dir = resolveAihRunPath(aiHomeDir, 'opencode-serve');
  return dir ? path.join(dir, `${runtimeScope}.json`) : '';
}

function readServeState(aiHomeDir, runtimeScope, fsImpl = nodeFs) {
  try {
    const state = JSON.parse(fsImpl.readFileSync(serveStatePath(aiHomeDir, runtimeScope), 'utf8'));
...
function writeServeState(aiHomeDir, runtimeScope, state, fsImpl = nodeFs) {
  const statePath = serveStatePath(aiHomeDir, runtimeScope);
  const tempPath = `${statePath}.${process.pid}.${Date.now()}.tmp`;
  try {
    fsImpl.mkdirSync(path.dirname(statePath), { recursive: true });
    fsImpl.writeFileSync(tempPath, JSON.stringify(state, null, 2), 'utf8');
    fsImpl.renameSync(tempPath, statePath);
  } catch (error) {
    try { fsImpl.unlinkSync(tempPath); } catch (_cleanupError) { /* best-effort */ }
    throw codedError('opencode_serve_state_write_failed', 'opencode serve backend state 写入失败', error);
  }
}

function storedServeMultiplexer(state) {
  const raw = state && state.multiplexer;
  const stored = raw == null ? '' : String(raw).trim().toLowerCase();
  if (!stored) return MULTIPLEXER_TYPE.TMUX;
  if (stored === MULTIPLEXER_TYPE.TMUX || stored === MULTIPLEXER_TYPE.HERDR) return stored;
  throw codedError(
    'opencode_serve_multiplexer_unknown',
    `opencode serve state 记录了未知 multiplexer backend:${stored}`
  );
}
...
function buildServeState({ runtimeScope, port, socket, multiplexer, startedAt }) {
  return {
    runtimeScope,
    port,
    socket,
    multiplexer,
    startedAt: positiveInteger(startedAt, Date.now())
  };
}

function resolveStoredServeBinding(options, multiplexer) {
  return resolveRunMultiplexerBinding({
    ...options,
...
function resolveCurrentServeBinding(options) {
  const currentOptions = {
    ...options,
    multiplexerBinding: null
  };
  delete currentOptions.multiplexerType;
  delete currentOptions.multiplexer;
  return resolveRunMultiplexerBinding(currentOptions);
}
...
async function ensureServeNow(options, target) {
  ...
  const aiHomeDir = resolveServeAiHomeDir(options);
  const existingState = readServeState(aiHomeDir, runtimeScope, fsImpl);
  const existingMultiplexer = existingState ? storedServeMultiplexer(existingState) : '';
  const port = positiveInteger(existingState && existingState.port, servePortForAccount(runtimeScope));
  const socket = normalizeString(existingState && existingState.socket) || serveSocketForAccount(runtimeScope);
  const baseUrl = `http://127.0.0.1:${port}`;
  const probe = clientFactory({ baseUrl, timeoutMs: 3000 });

  if (await healthOk(probe)) {
    const multiplexer = existingMultiplexer || MULTIPLEXER_TYPE.TMUX;
    writeServeState(aiHomeDir, runtimeScope, buildServeState({
      runtimeScope,
      port,
      socket,
      multiplexer,
      startedAt: existingState && existingState.startedAt
    }), fsImpl);
    return { baseUrl, port, socket, multiplexer, reused: true };
  }

  // state 绑定的是旧资源的唯一 backend 身份。先固定该 binding，再完成判活/清理；
  // 当前 auto 选择只影响下一次 spawn，绝不能拿新 backend 去清理旧资源。
  if (existingState) {
    const existingBinding = resolveStoredServeBinding(options, existingMultiplexer);
    if (!existingBinding.available) {
      throw codedError(
        'opencode_serve_multiplexer_unavailable',
        `opencode serve 的 ${existingMultiplexer} backend 不可用,无法安全管理已有进程`
      );
    }
    if (hasRunSession(socket, { multiplexerBinding: existingBinding })) {
      cleanupRunSocket(socket, { multiplexerBinding: existingBinding, fs: fsImpl });
    }
  } else {
    // state 上线前的 serve 只可能由 tmux 创建；无 state 的遗留资源仅按 tmux 探测。
    const legacyBinding = resolveStoredServeBinding(options, MULTIPLEXER_TYPE.TMUX);
    if (legacyBinding.available && hasRunSession(socket, { multiplexerBinding: legacyBinding })) {
      cleanupRunSocket(socket, { multiplexerBinding: legacyBinding, fs: fsImpl });
    }
  }

  const multiplexerBinding = resolveCurrentServeBinding(options);
  if (!multiplexerBinding.available) {
    throw codedError(
      'opencode_serve_multiplexer_unavailable',
      `opencode serve 的 ${multiplexerBinding.name} backend 不可用,无法启动常驻进程`
    );
  }
  ...
  const resolveNativeCliLaunchImpl = options.resolveNativeCliLaunchImpl || resolveNativeCliLaunch;
  const launch = resolveNativeCliLaunchImpl('opencode', { env });
  const logPath = serveLogPath(aiHomeDir, runtimeScope, fsImpl);
  ...
  const spawnedState = buildServeState({
    runtimeScope,
    port,
    socket,
    multiplexer: spawned.multiplexer || multiplexerBinding.name,
    startedAt: Date.now()
  });
  try {
    writeServeState(aiHomeDir, runtimeScope, spawnedState, fsImpl);
  } catch (error) {
    cleanupRunSocket(socket, { multiplexerBinding, fs: fsImpl });
    throw error;
  }
  ...
  while (Date.now() < deadline) {
    if (await healthOk(probe)) {
      return { baseUrl, port, socket, multiplexer: spawnedState.multiplexer, reused: false };
    }

...
function getOpenCodeAuthPath(account) {
  const explicit = String(account && account.authPath || '').trim();
  if (explicit) return explicit;
  const profileDir = String(account && account.profileDir || '').trim();
  return profileDir ? path.join(profileDir, '.local', 'share', 'opencode', 'auth.json') : '';
}
...
function resolveOpenCodeGoApiKey(options, account, deps = {}) {
  ...
  const authPath = getOpenCodeAuthPath(account);
  const auth = parseJsonFileSafe(authPath, deps.fs || fs);

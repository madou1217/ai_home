...
function listProviderAccountIds(fsImpl, aiHomeDir, provider) {
  const root = normalizeText(aiHomeDir);
  const normalizedProvider = normalizeProvider(provider);
  if (!root || !normalizedProvider) return [];
  try {
    return fsImpl.readdirSync(path.join(root, 'profiles', normalizedProvider))
      .filter((name) => /^\d+$/.test(String(name || '').trim()))
...
function listPersistentSessionsForAccount(provider, accountId, tmux, options = {}) {
  const spawnSyncImpl = options.spawnSync || defaultSpawnSync;
...
function collectPersistentSessionRunKeys(projects, options = {}) {
  ...
  for (const provider of collectProjectProviders(projects)) {
    const accountIds = listProviderAccountIds(fsImpl, aiHomeDir, provider);
    for (const accountId of accountIds) {
      const rawSessions = listPersistentSessionsForAccount(provider, accountId, tmux, {
        ...options,
        fs: fsImpl
      });

...
} = require('./protocol-canonical-response');
const {
...
function convertOpenAIResponsesToOpenAIChat(payload) {
  ...
  const reasoningEffort = toPlainText(source.reasoning && source.reasoning.effort || '').trim();
...
function convertOpenAIResponsesToAnthropicMessages(payload) {
  ...
  // 映射 reasoning → thinking 参数。DeepSeek 等第三方端点默认开启
  // extended thinking，要求历史消息中的 thinking 块被原样传回。
  // 当前适配器不生成 thinking 块，所以：
  // - 有 reasoning.effort  → 显式 enabled + budget → 上游负责生成 thinking
  // - 有工具调用、无 reasoning → disabled → 避免 "thinking must be passed back" 400
  // - 纯文本、无 reasoning → 不注入 thinking 参数，用上游默认值
  const reasoning = source.reasoning && typeof source.reasoning === 'object' ? source.reasoning : null;
  const hasToolCalls = messages.some(function (msg) {
    return Array.isArray(msg.content) && msg.content.some(function (c) { return c.type === 'tool_use'; });
  });
  ...
  if (reasoning && reasoning.effort) {
    out.thinking = { type: 'enabled', budget_tokens: mapReasoningEffortToThinkingBudget(reasoning.effort) };
...
function mapReasoningEffortToThinkingBudget(effort) {
  ...
  if (key === 'high') return 4096;
  if (key === 'medium') return 2048;
  return 1024;
}
...
function applyProtocolAdapterPath(input = {}) {
  ...
  const payload = path.reduce((currentPayload, adapter) => {
    const fn = resolveNamedProtocolAdapter(adapter.requestAdapter, PROTOCOL_REQUEST_ADAPTER_FUNCTIONS);
    if (typeof fn !== 'function') {
      throw new Error(`unsupported_protocol_request_adapter:${adapter.requestAdapter || adapter.id}`);
    }
  ...
  return {
    ...
    payload,
    adapters: path.map((adapter) => adapter.id)
  };
}

function applyProtocolRequestAdapterPath(input = {}) {
  return applyProtocolAdapterPath(input);
}

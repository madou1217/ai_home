...
function parseOpenAIChatSseToCanonicalEvents(rawText, fallbackModel) {
  ...
  flush();
  const stop = events.findLast((event) => event.type === 'message_stop');
  if (stop && usage) stop.usage = usage;
  return events;
}
...
function renderOpenAIResponsesSseFromCanonicalEvents(events, fallbackModel) {
  const chunks = [];
  const renderer = createOpenAIResponsesRenderer((chunk) => chunks.push(chunk), fallbackModel);

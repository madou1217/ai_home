...
function createStreamingProtocolResponse(res, options = {}) {
  ...
  return {
    ...
    headers,
    // Preserve the downstream lifecycle through the protocol adapter so pending
    // upstream reads and response-open requests can be cancelled.
    once: typeof res.once === 'function' ? res.once.bind(res) : undefined,
    off: typeof res.off === 'function' ? res.off.bind(res) : undefined,
    removeListener: typeof res.removeListener === 'function' ? res.removeListener.bind(res) : undefined,
    get destroyed() { return Boolean(res.destroyed); },
    get body() {
      return rawBody || bufferedOutput;
    },
    get writableEnded() {
      return writableEnded || Boolean(res.writableEnded);
    },
...
const PROVIDER_FALLBACK_REQUEST_PROTOCOLS = Object.freeze({
  ...
  kimi: Object.freeze(['openai_chat']),
  // grok 的 Build(CLI/OAuth) 与 API Key 两个边界都走 api.x.ai/v1 REST，OpenAI 兼容
  // （见 docs/architecture/grok-provider-boundaries.md §2）。缺这一条时，grok 会落到
  // 客户端协议的通用回落表，其首位是 anthropic_messages——而 grok 在
  // anthropic_messages 下没有任何 provider 路由，桥接因此 500。
  grok: Object.freeze(['openai_chat']),
  // qoder/qodercn 的推理由原生 CLI headless transport 承接（openai_chat 语义），
  // responses-wire（chat harness）经协议适配回落到这里。
  qoder: Object.freeze(['openai_chat']),
  qodercn: Object.freeze(['openai_chat']),
...
async function runClientProtocolViaProviderProtocolRoute(ctx) {
  ...
  const nativeDirect = Boolean(providerPlan && providerPlan.nativeDirect);
  const routeRes = nativeDirect
    ? res
    : wantsStream

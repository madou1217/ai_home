...
const { createOpenAIResponsesRenderer } = require('./protocol-responses-stream-renderer');
...
function createStreamParserOptions(sourceProtocol, targetProtocol) {
  return {
    requireExplicitStop: normalizeProtocolId(targetProtocol) === 'openai_responses',
...
function createCanonicalParser(sourceProtocol, emit, fallbackModel, options = {}) {
  ...
  if (source === 'openai_chat') {
    return createOpenAIChatParser(emit, fallbackModel, options);
  }
...
function createOpenAIChatParser(emit, fallbackModel, options = {}) {
  const state = {
    buffer: '',
    dataLines: [],
    started: false,
  ...
  const emitStop = () => {
    if (state.stopped || !state.finishReason) return;
    state.stopped = true;
  ...
  const flushFrame = () => {
    ...
    if (choice.finish_reason && !state.stopped) {
      ...
      state.finishReason = resolveOpenAIChatFinishReason(choice.finish_reason, {
        hasToolCalls: state.toolCalls.size > 0
      });
      // OpenAI-compatible streams may send a separate choices:[] usage frame
      // after finish_reason. Preserve it before emitting the terminal response.
      if (chunk.usage) emitStop();
    }
  };
  ...
  return {
    write(chunk) {
      ...
      flushFrame();
      emitStop();
      if (state.started && !state.stopped && !options.requireExplicitStop) {
        state.stopped = true;

...
//     升级写新目录、只切指针，从不 unlink 运行中的 exe；回滚是本地切指针，离线且瞬时。
//     Windows 上结构性免疫 npm 那种 EPERM。
...
// rollbackable：回滚是否无需重新下载（standalone 的旧 release 目录还在本地）。
const CHANNEL_CAPABILITIES = Object.freeze({
  [CHANNELS.STANDALONE_RELEASE]: { pinnable: true, offlineRollback: true },
  [CHANNELS.NPM_GLOBAL]: { pinnable: true, offlineRollback: false },
  [CHANNELS.VENDOR_SELFUPDATE]: { pinnable: false, offlineRollback: false },
  [CHANNELS.HOMEBREW]: { pinnable: false, offlineRollback: false },
  [CHANNELS.UNKNOWN]: { pinnable: false, offlineRollback: false }
});
...
function detectCliChannel(input = {}) {
  ...
  return {
    ...
    offlineRollback: capabilities.offlineRollback,
    shadowedNpmInstall,
    evidence
  };
}

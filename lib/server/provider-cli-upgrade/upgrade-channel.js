...
const { WRAPPER_MARKER, WRAPPER_UPSTREAM_VAR } = require('../codex-cli-hook');
...
// 因此一个判断同时覆盖两种垫片。
//
// Windows 上同一个 hook 长成 cmd 批处理，写法没有 exec 那种自解释性：
//   REM aih-codex-cli-hook
//   set "UPSTREAM=…\codex.aih-original.cmd"   ← 真正被包住的那个
//   set "NODE_BIN=…\node.exe"                 ← 也是真实存在的文件
//   set "HELPER=…\codex-app-server-stdio-proxy.js"
// 「取第一个存在的引号串」在这里有两种错法：引号里带着 `UPSTREAM=` 前缀所以一个都不
// 存在（实测 2026-09-14 真机就是这样，codex 因此落进 unknown），或者退一步去掉前缀之后
// 解析到 node.exe 上去。所以 cmd 形态必须按变量名精确取 UPSTREAM，不靠出现顺序。
const SHIM_MAX_BYTES = 4096;
const QUOTED_PATH = /['"]([^'"\n]+)['"]/g;
const CMD_UPSTREAM_ASSIGNMENT = new RegExp(`^\\s*set\\s+"${WRAPPER_UPSTREAM_VAR}=([^"\\n]+)"`, 'mi');
...
function followAihShim(fsImpl, target) {
  try {
    ...
    const upstream = CMD_UPSTREAM_ASSIGNMENT.exec(text);
    if (upstream && upstream[1] !== target && fsImpl.existsSync(upstream[1])) return upstream[1];

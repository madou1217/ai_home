...
function createProviderUpgradeDeps(options = {}) {
  ...
  // 渠道根目录探测里有一次 `npm root -g` 子进程，按实例缓存：没必要每轮每个 provider 各付一次。
  //
  // 这个缓存有一条已知失效条件，写在这里而不是假装没有：nvm（含 Windows 的 nvm4w）执行
  // `nvm use` 会整体改写 <nvm>\nodejs 这个目录链接的指向，全局 CLI 集合随之整批更换。
  // 实测 2026-09-14 的真机就是这个形状：d:\nvm4w\nodejs → …\nvm\v22.23.1。
  // server 跑着的时候用户切了 node 版本，缓存里的 npmGlobalRoot 就指向旧版本的目录，
  // 于是升级写进去的东西对新版本而言凭空消失。重启 server 即可恢复正确 ——
  // 之所以先只记录不修，是因为自动失效需要一个可靠的变更信号（目录 mtime 不够），
  // 而这条路径的真实触发频率还没有数据支撑值不值得那份复杂度。

...
function probeNpmGlobalRoot(options = {}) {
  ...
  try {
    ...
    const result = spawnSyncImpl(command, ['root', '-g'], {
      encoding: 'utf8',
      windowsHide: true,
      // Windows 上 npm 是 .cmd，不开 shell 会直接 EINVAL（见 requiresCmdShell）。
      // 这里失败是无声的：回落到按 execPath 推导，而推导值未必等于真实 prefix，
      // 于是渠道判定拿着一个错的根去比对，结果是「什么都不属于 npm 渠道」。

...
const { collectPostInstallActions, runPostInstallActions } = require('./upgrade-post-install');
 ...
 *   reinstallCodexCliHook()          -> {ok, error}  可选；未注入时善后动作按跳过处理
 *   log(entry)                       -> void
 *
 * config.applyEnabled === false 时只检查不改动（阶段一）。
 */
async function runProviderUpgradeCycle(provider, ledger, deps = {}, config = {}) {
  ...
  // 阶段一（只检查、不改动）。闸门照常观测并把证据记进账本——这正是阶段一要采集的真实数据；
  // 唯独 consecutiveQuiescentTicks 归零。否则打开 apply 的那一刻，会拿着攒了几天的计数
  // 立刻开装，等于整道闸门从未存在过。要的是「开关一开，重新数两拍」。
  if (config.applyEnabled === false) {
    return {
      ledger: commit({
        ...carry,
        ...
        lastDeferReason: quiescence.busy ? 'busy' : 'apply_disabled'
      }),
      ...outcome(STATES.HEALTHY, 'apply_disabled', {
        targetVersion: decision.targetVersion,
        busy: Boolean(quiescence.busy),
  ...
  // 官方安装脚本会覆盖 aih 的 CLI hook 垫片，装完必须补回来。补不回来 = 版本升上去了
  // 但 aih 对该 provider 的接管没了，这种半吊子状态比升级失败更糟，按验证失败处理走回滚。
  const postInstall = await runPostInstallActions(collectPostInstallActions(planResult.plans), deps);
  const verdict = postInstall.ok
    ? await deps.verify(provider, target)
    : { verdict: VERDICTS.FAIL, detail: `post_install_failed:${postInstall.failedAction}` };
  ...
  // 回滚同样会被安装脚本覆盖 hook，同样要补回来。
  const rolledPostInstall = await runPostInstallActions(collectPostInstallActions(rollbackPlans.plans), deps);
  if (!rolledPostInstall.ok) return broken('rollback_post_install_failed', rolledPostInstall.failedAction);

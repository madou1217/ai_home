...
function shouldRejectEmptyRemoteModels(provider, account) {
  return isCodexOauthAccount(provider, account);
}

function buildEmptyRemoteModelsError(provider, account) {
  if (isCodexOauthAccount(provider, account)) return 'empty_codex_models_catalog';
...
function shouldProbeAccountModels(provider, account) {
  ...
  if (normalizedProvider === 'claude' && isClaudeAuthTokenAccount(account)) return false;

...
async function dispatchProviderProtocolRoute(ctx) {
  ...
  // zcode 验证码桥是对象而非函数，createProviderProtocolRouteDeps 的白名单不会带它；
  // 显式透传，保证协议适配路径上的 zcode OAuth 账号也能触发 3007 → 验证码重发。
  if (ctx.deps && ctx.deps.zcodeCaptchaBridge) routeDeps.zcodeCaptchaBridge = ctx.deps.zcodeCaptchaBridge;

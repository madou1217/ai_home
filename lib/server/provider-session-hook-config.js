...
const { SESSION_FILE_CAPABLE_PROVIDERS } = require('../sessions/session-reader');
const {
...
// provider 是否用独立的 hooks.json 承载(而非和其它设置混在一个 settings.json 里)。
const HOOKS_JSON_TARGET_PROVIDERS = new Set(['codex', 'agy', 'grok']);
...
const DEFAULT_EVENTS_BY_PROVIDER = Object.freeze({
  ...
  // 官方文档确认(docs.x.ai/build/features/hooks,与 AI Home 实际安装的 x.ai/cli/install.sh 是同一个
  // CLI,不是同名的 superagent-ai/grok-cli 社区项目):JSON hooks,事件名与 claude 一致。
  grok: Object.freeze(['SessionStart', 'UserPromptSubmit', 'Stop', 'StopFailure', 'SessionEnd'])
});
...
function getProviderSessionSyncMode(providerRaw) {
  const provider = normalizeProvider(providerRaw);
  if (isProviderSessionHookSupported(provider)) return 'hook';
  if (SESSION_FILE_CAPABLE_PROVIDERS.includes(provider)) return 'polling';
  return 'unavailable';
}

...
function buildProviderSessionHookSenderCommand(options = {}) {
  ...
  const nodeCommand = normalizeText(options.nodeCommand) || '/usr/bin/env node';
  ...
  return [
    nodeCommand,
    quoteShellArg(senderScriptPath),
    MANAGED_HOOK_MARKER,
    '--provider',
    quoteShellArg(provider),
    '--event',
    quoteShellArg(eventName),
    '--url',
    quoteShellArg(receiverUrl)
  ].join(' ');
}
...
function isManagedCommand(command, provider) {
  ...
  return text.includes(`--provider '${provider}'`) || text.includes(`--provider ${provider}`);
}

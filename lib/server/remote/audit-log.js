...
function getRemoteAuditLogPath(aiHomeDir) {
  ...
  return root ? path.join(root, REMOTE_AUDIT_LOG_FILE) : '';
}
...
function appendRemoteAuditEvent(event, deps = {}) {
  ...
  const existed = typeof fs.existsSync === 'function' && fs.existsSync(filePath);
  ...
  try {
    ...
    if (!existed && typeof fs.chmodSync === 'function') {

...
const REMOTE_INVITES_KEY = 'remote:invites';
...
function readInviteStore(deps = {}) {
  const stored = readJsonValue(deps.fs, deps.aiHomeDir, REMOTE_INVITES_KEY);
  return {
    version: 1,
    invites: Array.isArray(stored && stored.invites) ? stored.invites : []
  };
}
...
function writeInviteStore(store, deps = {}) {
  ...
  if (!writeJsonValue(deps.fs, deps.aiHomeDir, REMOTE_INVITES_KEY, payload)) {
    throw new Error('remote_invite_store_write_failed');
  }
}

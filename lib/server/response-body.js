...
function tryDecode(buffer, decoder, options) {
  try {
    const out = decoder(buffer, options);
    return isMostlyText(out) ? out : null;
  } catch (error) {
    // A caller's decoded-size limit is a hard boundary, not a failed format
    // guess. Do not retry another decoder or return compressed text after it.
    if (options && options.maxOutputLength && error.code === 'ERR_BUFFER_TOO_LARGE') throw error;
    return null;
  }
}
...
function decodeResponseBuffer(buffer, encoding = '', options = {}) {
  const raw = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer || []);
  if (raw.length < 1) return '';

  for (const decoder of listDecodersForEncoding(encoding)) {
    const decoded = tryDecode(raw, decoder, options);

...
function parseServerConfigArgs(rawArgs = []) {
...
function printConfigUsage() {
  console.log('\x1b[90mUsage:\x1b[0m aih server config [set] [--host <ip>] [--port <n>] [--client-key <key>] [--management-key <key>] [--open-network|--local-only]');
}

async function runServerConfigCommand(args, deps = {}) {
  let parsed;
  try {
    parsed = parseServerConfigArgs(args);
  } catch (e) {

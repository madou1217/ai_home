...
const { initWebUiModelsCache } = require('./webui-model-cache');
...
function createProxyServerState(options, deps) {
  ...
  SUPPORTED_SERVER_PROVIDERS.forEach((provider) => {
    const optionName = `${provider}MaxConcurrency`;
    executors[provider] = createProviderExecutor(provider, options[optionName], options.queueLimit);
  });
  return {
    ...
    webUiModelsCache: initWebUiModelsCache(),
...
function printProxyServeStartup(options, state, requiredClientKey, requiredManagementKey) {
  ...
  if (options.codexClientVersion) {
    console.log(`  codex_client_version: ${options.codexClientVersion}`);
  }
  ...
  if (options.agyBaseUrl) {
    console.log(`  agy_base_url: ${options.agyBaseUrl}`);
  }
  if (options.proxyUrl) {

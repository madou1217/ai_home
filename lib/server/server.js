...
const { shouldDeferGoRouteToNode } = require('./go-core-route-deferral');
...
async function startLocalServer(options, deps) {
  ...
  const goCoreHost = (typeof deps.createGoCoreHost === 'function' ? deps.createGoCoreHost : createGoCoreHost)({
    ...
    requiredClientKey,
    writeJson,
    deferToNode: ({ entryId, pinnedAccountRef }) => shouldDeferGoRouteToNode({
      entryId,
      pinnedAccountRef,
      state,
      accountStateIndex,
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (goCoreHost.tryHandleHttp(req, res, { method, pathname, requestId })) return;

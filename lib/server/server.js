...
async function startLocalServer(options, deps) {
  ...
  if (aiHomeDir && String((processObj.env && processObj.env.AIH_SERVER_ALLOW_MULTI) || '').trim() !== '1') {
    const lock = await acquireServerInstanceLock({
      fs,
      processObj,
      aiHomeDir,
      host: options.host,
    ...
    if (!lock.ok) {
      if (lock.reason === 'already_running') {
        throw new Error(`another aih server is already running (pid=${lock.pid}); refusing to start a second instance. Use 'aih server restart', stop it first, or set AIH_SERVER_ALLOW_MULTI=1 to override`);
      }
      throw new Error(`port ${options.port} is already in use (held on ${lock.host}); refusing to start a second gateway. Stop the other instance or set AIH_SERVER_ALLOW_MULTI=1 to override`);
    }
  }
  ...
  try {
    const pruned = pruneStaleAccountRuntimeProjections({ fs, path, aiHomeDir });
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (pathname.startsWith('/v0/webui/')) {
        // WebUI 数据面鉴权门：同机 loopback 自动信任，远程客户端统一使用 Management Key。

...
const { createZcodeCaptchaBridge } = require('./zcode-captcha-bridge');
...
async function startLocalServer(options, deps) {
  ...
  // zcode OAuth 计划账号的验证码桥：3007 拦截（upstream-endpoints）→ accounts
  // watch SSE 广播 → WebUI 跑阿里 Captcha SDK → verify param 回传后重发上游。
  // broadcast 返回监听者数；0 表示没有 WebUI 在线，桥会立即失败而不是挂到超时。
  const zcodeCaptchaBridge = deps.zcodeCaptchaBridge || createZcodeCaptchaBridge({
    fetchWithTimeout,
    proxyUrl: options.proxyUrl,
    noProxy: options.noProxy,
    broadcast: (payload) => {
      const listeners = countAccountsWatchListeners({ state });
      if (listeners > 0) emitAccountsLiveEvent({ state }, payload);
      return listeners;
    }
  });

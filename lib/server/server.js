...
const { createAccountWebSocket } = require('./account-websocket-egress');
const { peekZcodeSingBoxRuntime } = require('./zcode-sing-box-runtime');
...
async function startLocalServer(options, deps) {
  ...
  // Account-local 127.0.0.1 proxy endpoints outlive Server upgrades/restarts.
  // Rebuild every persisted account endpoint without restarting provider clients.
  setImmediate(async () => {
    if (lifecycleStopping) return;
    try {
      ...
      if (Number(result.restored) > 0) {
        console.log(`\x1b[36m[aih]\x1b[0m Account egress restore: rebuilt ${result.restored} sidecar endpoint(s)`);
      }
      if (Number(result.failed) > 0) {
        console.warn(`\x1b[33m[aih]\x1b[0m Account egress restore: ${result.failed} endpoint(s) failed`);
      }
    } catch (error) {
      console.warn(`\x1b[33m[aih]\x1b[0m Account egress restore failed: ${String((error && error.message) || error)}`);
    }
  });
  ...
  server.on('upgrade', async (req, socket, head) => {
    ...
    // WS 长连接按「选中账号 + 连接存活」计 in-flight，语义与 HTTP attempt 对齐。
    let wsActivityEnd = null;
    ...
    try {
      ...
      // WS 长连接按「选中账号 + 连接存活」计 in-flight，语义与 HTTP attempt 对齐。
      const wsAccountRef = String(account.accountRef || '');
      if (accountActivity && typeof accountActivity.begin === 'function') {
        accountActivity.begin('codex', wsAccountRef);
      }
      wsActivityEnd = () => {
        if (!wsActivityEnd) return;
        if (accountActivity && typeof accountActivity.end === 'function') {
          accountActivity.end('codex', wsAccountRef);
        }
        wsActivityEnd = null;
      };
      ...
      // 创建到上游的 WebSocket 连接。账号绑定出口时通过本机 sidecar 的
      // HTTP CONNECT 隧道；绑定不可用必须 fail-closed，不能静默直连。
      const connectAccountWebSocket = typeof deps.createAccountWebSocket === 'function'
        ? deps.createAccountWebSocket
        : createAccountWebSocket;
      const upstream = await connectAccountWebSocket({
        provider: 'codex',
        account,
        upstreamUrl,
      ...
      wss.handleUpgrade(req, socket, head, (client) => {
        ...
        client.on('error', (error) => {
          ...
          if (wsActivityEnd) wsActivityEnd();
          upstream.close();
        });
      ...
      if (wsActivityEnd) wsActivityEnd();
      const accountEgressFailed = error?.code === 'account_egress_unavailable';
      const statusLine = accountEgressFailed
        ? '503 Service Unavailable'
        : '502 Bad Gateway';
      socket.write(`HTTP/1.1 ${statusLine}\r\nContent-Type: application/json\r\n\r\n`);
      socket.write(JSON.stringify(accountEgressFailed
        ? { ok: false, error: 'account_egress_unavailable' }
        : { ok: false, error: 'upstream_failed', detail: errorMsg }));
      socket.destroy();
    }
  });
  ...
  function stopServer(signal = 'manual') {
    ...
    let accountEgressRuntime = null;
    try {
      const peekAccountEgressRuntime = typeof deps.peekAccountEgressRuntime === 'function'
        ? deps.peekAccountEgressRuntime
        : peekZcodeSingBoxRuntime;
      accountEgressRuntime = peekAccountEgressRuntime({ aiHomeDir });
    } catch {}

    const cleanupTasks = [
      invokeLifecycle(tokenRefreshDaemon, 'stop'),
      invokeLifecycle(accountEgressRuntime, 'stop'),

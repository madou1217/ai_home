...
function buildSessionEventPayload(session, event = {}) {
  ...
  // native run 生命周期/交互 prompt 事件的透传字段：detached 重连的 webUI 靠 runId 定位
  // input/abort 回写通道，靠 prompt/promptId 恢复/收起 PlanChoiceDock。
  const runId = normalizeText(event.runId || event.run_id);
  const promptId = normalizeText(event.promptId || event.prompt_id);
  if (runId) payload.runId = runId;
  if (promptId) payload.promptId = promptId;
  if (event.prompt && typeof event.prompt === 'object') payload.prompt = event.prompt;
  if (event.retryStatus && typeof event.retryStatus === 'object') payload.retryStatus = event.retryStatus;
  return payload;
}

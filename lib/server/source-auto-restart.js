...
function shouldEnableSourceAutoRestart(processObj) {
  ...
  // 常驻网关承载现有 HTTP/WS 请求；源码编辑不能默认中断这些连接。
  // 自动重载仅供显式启用的开发环境使用，普通运行由 status 提示 stale。
  return envValue(processObj, 'AIH_SERVER_SOURCE_AUTO_RESTART') === '1';
}

const BACKGROUND_RESTART_ENV = 'AIH_SERVER_BACKGROUND_RESTART';
...
const SENSITIVE_SERVE_OPTIONS = new Set([
  '--api-key',
  '--client-key',
  '--management-key'
]);

function stripSensitiveServeArgs(args) {
  const source = Array.isArray(args) ? args : [];
  const safe = [];
  for (let index = 0; index < source.length; index += 1) {
    const token = String(source[index] || '');
    const inlineIndex = token.indexOf('=');
    const key = inlineIndex > 0 ? token.slice(0, inlineIndex) : token;
    if (SENSITIVE_SERVE_OPTIONS.has(key)) {
      if (inlineIndex < 0) index += 1;
      continue;
    }
    safe.push(token);
  }
...
function buildRestartServeArgs(options = {}, processObj) {
  ...
  return argvArgs.length > 0
    ? appendRestartServeArgsFromOptions(stripSensitiveServeArgs(argvArgs), options)
...
function startServerSourceAutoRestart(options = {}, deps = {}) {
  ...
  let timer = null;
  const enabled = shouldEnableSourceAutoRestart(processObj);
  ...
  function checkOnce() {
    ...
    if (!enabled) {
      // 保留启动指纹，status 持续显示待加载源码，不伪装成已经更新。
      return { stale: true, reason: 'source_changed', restart: { requested: false, reason: 'disabled' } };
    }

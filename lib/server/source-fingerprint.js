...
function samePath(path, processObj, left, right, fs) {
  const a = normalizePath(path, realpathIfPossible(fs, normalizePath(path, left)));
  const b = normalizePath(path, realpathIfPossible(fs, normalizePath(path, right)));
  if (!a || !b) return false;
...
function getSourceFreshness(options = {}) {
  ...
  } else if (recordedEntry && currentEntryPath && !samePath(path, processObj, recordedEntry, currentEntryPath, fs)) {
    stale = true;

...
function resolveEntryProjectRoot(path, candidateEntryFilePath) {
  const normalized = normalizePath(path, candidateEntryFilePath);
  if (!normalized) return '';
  if (path.basename(normalized) !== 'app.js') return '';
...
function isExistingAiHomeEntry(fs, path, candidateEntryFilePath) {
  const rootDir = resolveEntryProjectRoot(path, candidateEntryFilePath);
  if (!rootDir) return false;
  if (!fs.existsSync(candidateEntryFilePath)) return false;
  return readPackageName(fs, path, rootDir) === 'ai_home';
}

function parseServerEntryFilePathFromCommand(command) {
  const cmd = String(command || '').trim();
  if (!cmd || !/\bserver\b/.test(cmd) || !/\bserve\b/.test(cmd)) return '';
  const match = cmd.match(/(?:^|\s)(?:"([^"]*[\/\\]lib[\/\\]cli[\/\\]app\.js)"|'([^']*[\/\\]lib[\/\\]cli[\/\\]app\.js)'|([^\s"']*[\/\\]lib[\/\\]cli[\/\\]app\.js))(?:\s|$)/);
...
function getSourceFingerprintPaths(fs, path, candidateEntryFilePath) {
  const rootDir = resolveEntryProjectRoot(path, candidateEntryFilePath);
  if (!rootDir) return [];
  return SOURCE_FINGERPRINT_FILES
    .map((relativePath) => ({
      relativePath,
...
function getSourceFingerprintFilePath(path, aiHomeDir) {
  return path.join(aiHomeDir, SOURCE_FINGERPRINT_FILE);
}

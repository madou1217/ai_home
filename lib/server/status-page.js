    ...
    async function tick() {
      try {
        ...
        document.getElementById('accountTable').innerHTML =
          '<thead><tr><th>id</th><th>provider</th><th>email</th><th>runtime</th><th>until</th><th>ok/fail</th><th>last error</th></tr></thead><tbody>' +
          list.map((a) => {
            const until = Number(a.runtimeUntil || 0) > Date.now() ? fmtTs(a.runtimeUntil) : '-';
            return '<tr><td class="mono">' + esc(a.id) + '</td><td>' + esc(a.provider || '-') + '</td><td>' + esc(a.email || '-') + '</td><td>' + esc(a.runtimeStatus || '-') + '</td><td>' + esc(until) + '</td><td>' + esc(a.successCount || 0) + '/' + esc(a.failCount || 0) + '</td><td class="mono">' + esc(a.runtimeReason || a.lastError || '-') + '</td></tr>';
          }).join('') +
          '</tbody>';

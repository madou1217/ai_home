...
const { createCodebuddyNativeCredentialSync } = require('../account/codebuddy-credential-sync');
...
function createTokenRefreshDaemon(state, options, deps) {
  ...
  const startupSkewMs = Math.max(
    skewMs,
    Number(options.tokenStartupRefreshBeforeExpiryMs) || skewMs
  );
  ...
  async function refreshAccountToken(account, provider, isStartup = false) {
    ...
    // Kimi must reconcile CLI-rotated credentials before comparing the rejected
    // grant signature; suppressing here would also block local recovery.
    if (provider !== 'kimi' && shouldSuppressInvalidRefresh(account)) return;
        ...
        if (isUnrecoverableTokenRefreshFailure(result) && !hasAccessToken(account)) {
          ...
          if (typeof logInfo === 'function') {
            logInfo(
              ...
              + 'access token is still present, waiting for upstream auth evidence'
            );
          }
        }
        return;
  ...
  // Observe independent native App logins even when no AIH CLI/desktop wrapper
  // launched them. The existing pool loader re-evaluates credential revisions;
  // do not force accounts healthy or clear unrelated quota/network state here.
  const nativeCredentialSync = createCodexNativeCredentialSync({ fs, aiHomeDir, hostHomeDir });
  nativeCredentialSync.start({
    intervalMs: options.nativeCredentialSyncIntervalMs,
    onUpdated: () => {
      if (typeof reloadRuntimePool === 'function') reloadRuntimePool();
    },
    onError: (reason) => {
      if (typeof logWarn === 'function') logWarn(reason);
    }
  });

  const codebuddyCredentialSync = createCodebuddyNativeCredentialSync({ fs, aiHomeDir, hostHomeDir });
  codebuddyCredentialSync.start({
    intervalMs: options.nativeCredentialSyncIntervalMs,
    onUpdated: () => { if (typeof reloadRuntimePool === 'function') reloadRuntimePool(); },
    onError: reason => { if (typeof logWarn === 'function') logWarn(reason); }
  });
  ...
  return {
    ...
    getStats: () => ({
      ...
      codebuddyCredentialSync: codebuddyCredentialSync.getStats(),
      hostProviderHeal: hostProviderHeal.getState()
    }),
    forceRefresh: () => {
      nativeCredentialSync.scan();
      codebuddyCredentialSync.scan();

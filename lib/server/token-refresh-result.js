...
function hasUnrecoverableRefreshTokenEvidence(result) {
  ...
  if (/refresh[ _-]?token[^a-z0-9]{0,12}(?:(?:has|had)[ _-]+been|was|is)?[ _-]*(?:already[ _-]+|previously[ _-]+)?reused/.test(evidence)) return true;
  if (/refresh[ _-]?token[^a-z0-9]{0,12}(?:(?:has|had)[ _-]+(?:already[ _-]+|previously[ _-]+)?been|was|is)[ _-]+(?:already[ _-]+|previously[ _-]+)?used/.test(evidence)) return true;

    ...
    try {
      ...
      return {
        attempt: sanitizeAttempt({
          ...
          label: plan.label,
          ok,
          error: ok ? '' : `${plan.label} 已执行，但资源状态校验未达到预期。`
        }),
        observed
      };
    } catch (error) {
      return {
        attempt: sanitizeAttempt({
          ...
          label: plan.label,
          ok: false,
          error: `${plan.label} 已执行，但资源状态校验失败：${String(error && error.message || error)}`
        }),
        observed: null
      };
    }
  ...
  async function run(job) {
    ...
    try {
      for (let index = 0; index < job.plans.length; index += 1) {
        ...
        job.phase = 'completed';
        job.result = {
          installed: Boolean(verified.observed && verified.observed.installed),

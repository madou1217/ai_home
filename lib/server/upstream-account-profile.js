...
// on concrete details: which chat/image wire API the endpoint speaks
// and which extra request headers it demands. These are properties of the account's endpoint, not of the
...
const IMAGE_API_ENV_KEY = 'AIH_IMAGE_API';

// Preserve unknown explicit dialects so image routing can fail closed instead
// of silently sending a request with the wrong protocol.
function normalizeImageApi(value) {
  return String(value == null ? '' : value).trim().toLowerCase();
}

function resolveAccountImageApi(account) {
  return normalizeImageApi(account && account.upstreamImageApi) || 'openai';
}
...
function resolveUpstreamProfileFromEnv(env) {
  ...
  const imageApi = normalizeImageApi(source[IMAGE_API_ENV_KEY]);
  return {
    ...(imageApi ? { upstreamImageApi: imageApi } : {}),
...
function buildUpstreamProfileEnv(profile) {
  ...
  const env = {};
  const imageApi = normalizeImageApi(source.imageApi != null ? source.imageApi : source.upstreamImageApi);
  if (imageApi) env[IMAGE_API_ENV_KEY] = imageApi;

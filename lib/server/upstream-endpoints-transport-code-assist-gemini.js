...
// Code Assist（cloudcode-pa）把标准 Gemini 响应包在 {response, traceId, metadata} 里；
// 对 Gemini 客户端必须还原成标准 generateContent 结构（已是标准结构时原样返回）。
function toPublicGeminiGenerateContent(payload) {
...
const {
  createCodeAssistHelpers,
  writeCodeAssistFailure
} = require('./upstream-endpoints-code-assist-helpers');
        ...
        try {
          ...
          if (streamMode && typeof fetchGeminiCodeAssistGenerateContentStream === 'function') {
            ...
            for await (const piece of nonEmptyUpstreamStream) {
              ...
              res.write(`data: ${JSON.stringify(toPublicGeminiGenerateContent(piece))}\n\n`);
            }
            res.end();
          ...
          if (streamMode) {
            ...
            res.setHeader('x-aih-server-account-ref', account.accountRef);
            if (account.email) res.setHeader('x-aih-server-account-email', account.email);
            res.write(`data: ${JSON.stringify(toPublicGeminiGenerateContent(payload))}\n\n`);
            res.end();
          } else {
            attemptMutable.streamTransport = 'non_stream';
            const raw = Buffer.from(JSON.stringify(toPublicGeminiGenerateContent(payload)));
            res.statusCode = 200;
          ...
          writeCodeAssistFailure(writeJson, res, attemptMutable.finalStatusCode, attemptMutable.lastError, policy);
          return { action: 'return' };
        }
      } else {

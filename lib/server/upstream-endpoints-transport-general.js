...
const { adaptKimiChatRequestBuffer } = require('./kimi-chat-request');
...
const generalPassthroughTransport = {
  ...
  async run(ctx) {
    ...
    if (provider === 'kimi' && /\/chat\/completions\/?$/.test(upstreamPath)) {
      forwardBody = adaptKimiChatRequestBuffer(forwardBody);
    }

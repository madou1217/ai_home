...
function resolveProviderPath(provider, reqUrl, upstreamBase) {
  ...
  if ((provider === 'gemini' || provider === 'agy' || provider === 'claude' || provider === 'codex' || provider === 'grok') && rawPath.startsWith('/v1/')) {
    return rawPath.slice(3);
  }
  if ((provider === 'gemini' || provider === 'agy' || provider === 'claude' || provider === 'codex' || provider === 'grok') && rawPath === '/v1') {
    return '/';
  }
  return rawPath;
}

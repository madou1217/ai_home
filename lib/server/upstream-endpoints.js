...
async function handleUpstreamPassthrough(ctx) {
  ...
  if (provider === 'gemini' && !(state.geminiSessionIdMap instanceof Map)) {
    state.geminiSessionIdMap = new Map();
  }
  const geminiRequestOptions = provider === 'gemini'
    ? {
        ...options,
        sessionKey: String(requestMeta && requestMeta.sessionKey || '').trim(),
        geminiSessionIdMap: state.geminiSessionIdMap,
  ...
  const orchestration = await runWithAccountAttempts({
    ...
    onAttempt: async (account, control) => {
      ...
      const accountGeminiRequestOptions = provider === 'gemini'
        ? {

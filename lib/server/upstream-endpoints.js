...
const { buildNoAvailableAccountPayload } = require('./account-availability');
...
async function handleUpstreamPassthrough(ctx) {
  ...
  if (orchestration.kind === 'no_account') {
    ...
    writeJson(res, 503, buildNoAvailableAccountPayload(provider, pool, {
      excludeIds: orchestration.attemptedIds
    }));
    return;
  }

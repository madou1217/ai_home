...
function resolveProviderUpstream(options, provider, account) {
  ...
  if (provider === 'grok') {
    const fromAccount = String(account && account.openaiBaseUrl || '').trim();
    return (fromAccount || 'https://api.x.ai/v1').replace(/\/+$/, '');
  }
  if (provider === 'kimi') {
    const fromAccount = String(account && account.openaiBaseUrl || '').trim();
    return (fromAccount || 'https://api.moonshot.cn/v1').replace(/\/+$/, '');
  }
  if (provider === 'kiro') {
    const fromAccount = String(account && account.openaiBaseUrl || '').trim();
    return (fromAccount || 'https://q.us-east-1.amazonaws.com').replace(/\/+$/, '');
  }
...
function selectAccountsForRequestModel(pool, provider, requestJson, state, options) {
  ...
  // 优先用倒排索引 O(1) 查，无索引时回退到 buildModelCapabilityIndex
  var modelIndex = state && state.modelAccountIndex;
  if (modelIndex && modelIndex.builtAt > 0) {
    var accountRefs = findRoutableAccountsForModel(modelIndex, model);
    if (accountRefs.length > 0) {
      var allowed = new Set(accountRefs);
      var poolByRef = {};
      pool.forEach(function (account) {
        var ref = getAccountRef(provider, account);
        if (ref) poolByRef[ref] = account;
      });
      return {
        pool: accountRefs.map(function (ref) { return poolByRef[ref]; }).filter(Boolean),
        model: model,
        filtered: true,
        accountRefs: accountRefs
      };
    }
    // 倒排索引无此模型 → 索引可能不完整（后台尚未探测到），退回到 unchecked
    return { pool: pool, model: model, filtered: false, accountRefs: [], unchecked: true };
  }

  // 回退路径
  var index = buildModelCapabilityIndex(state, options || {});
  var providerModels = index.providerModels && index.providerModels.get(provider);
  if (!(providerModels instanceof Set) || providerModels.size < 1) {
    return { pool: pool, model: model, filtered: false, accountRefs: [], unchecked: true };
  }
  var accountRefsOld = listAvailableAccountRefsForModelProvider(index, model, provider);
  if (accountRefsOld.length < 1) {
    if (Array.from(providerModels).some(function (modelId) { return modelIdsMatch(modelId, model); })) {
      return { pool: pool, model: model, filtered: false, accountRefs: [], providerCatalogOnly: true };
    }
    return { pool: [], model: model, filtered: true, accountRefs: [] };
  }
  var allowedOld = new Set(accountRefsOld);
  return {
    pool: pool.filter(function (account) { return allowedOld.has(getAccountRef(provider, account)); }),
    model: model,
    filtered: true,
    accountRefs: accountRefsOld
  };
}
...
async function handleUpstreamPassthrough(ctx) {
  ...
  // here; every dispatch path below rebuilds its upstream body from requestJson,
  // so mutating it in place is enough. Vision-capable targets are a no-op.
  const visionGuard = guardNonVisionImagePayload(requestJson);
  ...
  const orchestration = await runWithAccountAttempts({
    ...
    onAttempt: async (account, control) => {
      ...
      const responseOpenTimeoutMs = streamRequested
        ? Math.max(STREAM_RESPONSE_OPEN_TIMEOUT_MS, Number(options.upstreamTimeoutMs) || 0)
        : options.upstreamTimeoutMs;

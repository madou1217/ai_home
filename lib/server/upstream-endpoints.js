...
} = require('./anthropic-server-tool-compat');
...
async function handleUpstreamPassthrough(ctx) {
  ...
  const orchestration = await runWithAccountAttempts({
    ...
    onAttempt: async (account, control) => {
      ...
      let forwardBody = ['GET', 'HEAD'].includes(method) ? undefined : bodyBuffer;
      // Third-party Anthropic-compatible upstreams (GLM/Zhipu, DashScope, ...)
      // reject official-only server tools (e.g. advisor_20260301) with a strict
      // serde 400 that kills the whole request. Strip them before forwarding;
      // the official endpoint keeps them so advisor stays functional there.
      if (
        provider === 'claude'
        && isClaudeMessagesPath(upstreamPath)
        && !isOfficialAnthropicBaseUrl(upstreamBase)
        && forwardBody
        && forwardBody.length > 0
      ) {
        try {
          const parsedBody = JSON.parse(forwardBody.toString('utf8'));
          const { tools, removed } = stripOfficialOnlyServerTools(parsedBody && parsedBody.tools);
          if (removed.length > 0) {
            forwardBody = Buffer.from(JSON.stringify({ ...parsedBody, tools }));
            console.log(`[aih] Stripped official-only server tools for third-party anthropic upstream (account: ${account.id}): ${removed.join(', ')}`);
          }
        } catch (_parseError) {
          // Not JSON we can rewrite (or malformed) — forward the original body.
        }
        if (headers['anthropic-beta']) {
          const beta = sanitizeAnthropicBetaHeader(headers['anthropic-beta']);
          if (beta.removed.length > 0) {
            if (beta.value) headers['anthropic-beta'] = beta.value;

...
async function handleUpstreamPassthrough(ctx) {
  ...
  const refreshProviderAccessToken = provider === 'codex'
    ? refreshCodexAccessToken
    : (provider === 'claude' ? refreshClaudeAccessToken : null);
      ...
      if (upstreamRes.status === 401 || upstreamRes.status === 403) {
        ...
        if (allowRefreshRetry) {
          ...
          if (!isUnrecoverableTokenRefreshFailure(refreshResult)) {
            const refreshFailureDetail = `token_refresh_${normalizeTokenRefreshFailureReason(refreshResult)}`;
            const policy = {
              ...
              failureReason: refreshFailureDetail,
              detail: refreshFailureDetail,
              scope: 'account',
            ...
            lastError = refreshFailureDetail;
            finalStatusCode = 503;
            control.setLastError(lastError);

...
function selectAccountsForRequestModel(pool, provider, requestJson, state, options) {
  ...
  if (modelIndex && modelIndex.builtAt > 0) {
    // 必须带 provider：倒排索引是全 provider 共用的，中转账号常把别家的裸模型 id
    // 收进自己的目录。不带 provider 查会拿到别家账号的 ref，映射回本池必然为空，
    // 于是把「别家有这个模型」误报成「本 provider 没有可用账号」。
    return Object.assign(
      ...
      narrowPoolByModelCatalog({
        ...
        // 倒排索引查不到绑定只能说明索引不完整（后台尚未探测到），
        // 这条路径永远不下「没有账号能服务」的终判——保持原有语义。
...
async function handleUpstreamPassthrough(ctx) {
  ...
  const orchestration = await runWithAccountAttempts({
    ...
    onAttempt: async (account, control) => {
      if (transientPoolRetryUsed && Date.now() - requestStartedAt >= requestBudgetMs) {
      ...
      const attemptUpstreamTimeoutMs = transientPoolRetryUsed
        ? Math.max(
            ...
            Math.min(
              ...
              requestBudgetMs - (Date.now() - requestStartedAt)
            )
          )

...
async function handleV1Request(ctx) {
  ...
  if (method === 'GET' && pathname === '/v1/models' && options.backend === 'codex-adapter') {
    ...
    if (providerMode === 'gemini' || providerMode === 'claude' || providerMode === 'agy') {
    ...
    const agyResult = await collectModelIdsFromHandler(handleUpstreamModels, {
      options: { ...options, provider: 'agy' },
      state: {
        ...state,
        modelsCache: { updatedAt: 0, ids: [], byAccount: {}, sourceCount: 0 }
      },
    ...
    const mergedList = mergeGatewayModelEntries([
      ...
      ...agyResult.ids.map((id) => ({ id, provider: 'agy', source: 'remote' }))
    ], aliases);

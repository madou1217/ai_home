...
} = require('./model-usage-request-context');
...
async function handleV1Request(ctx) {
  ...
  const modelCatalogRequest = method === 'GET' && pathname === '/v1/models';
  let pinnedAccountRef = readPinnedAccountRef(routingReq && routingReq.headers);
  let pinnedAccount = null;
  if (pinnedAccountRef) {
    if (!isAccountRef(pinnedAccountRef)) {
      if (options.logRequests) {
        recordAccountPinDiagnostic(appendProxyRequestLog, {
          requestMeta,
          method,
          pathname,
          status: 400,
          error: 'invalid_account_ref'
        });
      }
    ...
    if (!pinnedAccount) {
      if (options.logRequests) {
        recordAccountPinDiagnostic(appendProxyRequestLog, {
          ...
          method,
          pathname,
          healed: modelCatalogRequest,
          status: 404,
          error: 'unknown_account_ref'
        });
      }
  ...
  if (options.logRequests) {
    const configuredProvider = String(options && options.provider || '').trim().toLowerCase();
    recordModelUsageRequestContext(appendProxyRequestLog, {
      requestMeta,
      requestJson,
      model: extractGeminiModelFromPath(pathname),
      provider: pinnedAccount && pinnedAccount.provider
        || (modelCatalogRequest ? 'gateway' : '')
        || readHeaderValue(routingReq && routingReq.headers, 'x-provider')
        || (configuredProvider !== 'auto' ? configuredProvider : ''),
      clientProtocol,
      method,
      pathname
    });

...
const { resolvePinnedAccount, readPinnedAccountRef } = require('./pinned-account');
...
async function handleV1Request(ctx) {
  ...
  if (pinnedAccountRef) {
    ...
    // 钉选可用 = 在运行池中,且持久化生命周期为 up(规则见 pinned-account.js,Go 转发决策共用)。
    const { resolvedPin, persistedPin, usable: pinUsable } = resolvePinnedAccount(state, accountStateIndex, pinnedAccountRef);
    if (pinUsable) {

...
async function handleV1Request(ctx) {
  ...
  if (pinnedAccountRef) {
    ...
    if (!pinnedAccount) {
      // 运行池只装可调度账号,被停用/凭据失效的账号不在其中。此前这里一律报
      // unknown_account_ref,于是「这个号停用了」被说成「你给的引用不存在」——
      // 会话钉死在死账号上时,客户端只会看到一串 404 而无从下手。
      // 先问持久化状态:确实没有这条记录才是「未知」。
    ...
    // A pinned request bypasses normal pool selection, so it must perform the
    // same durable lifecycle check explicitly. Otherwise a CLI delete/down
    // leaves a stale in-memory account able to send its old key upstream.
    if (accountStateIndex && typeof accountStateIndex.getAccountState === 'function') {
      const persisted = accountStateIndex.getAccountState(pinnedAccountRef);
      if (!persisted) {
        writeJson(res, 404, { ok: false, error: 'unknown_account_ref' });
        return true;
      }
      if (persisted.status !== 'up') {
        // 账号存在、只是不可调度(最常见是 OAuth 失效待重登)。此前这里也报
        // unknown_account_ref,把「这个号要重新登录」说成「你给的引用不存在」,
        // 直接把排查引向错误方向。403 而非 5xx:重试无用,得先处理账号本身。
        const runtimeStatus = String(
          deriveAccountRuntimeStatus(persisted.runtimeState).status || ''
        ).trim();
        writeJson(res, 403, {
          ok: false,
          error: 'pinned_account_unavailable',
          accountRef: pinnedAccountRef,
          provider: pinnedAccount.provider,
          status: String(persisted.status || ''),
          reason: runtimeStatus || 'account_not_up',
          // 两种成因给两种可执行指引:凭据失效要重登;账号被停用/删除则本会话
          // 绑死在死账号上,只能换号或重新启用——网关刻意不静默换号,否则等于
          // 拿已停用账号的凭据继续打上游。
          hint: runtimeStatus === 'auth_invalid'
            ? '该账号登录态已失效,请重新登录后再用它发起请求'
            : '该账号已停用或被删除;本会话绑定的就是它,请重新启用该账号,或改用其它账号新建会话'
        });
        return true;

...
async function handleV1Request(ctx) {
  ...
  if (pinnedAccountRef) {
    ...
    if (pinUsable) {
      ...
      const hasSchedulableAlternative = candidatePools.some((provider) => {
      ...
      if (hasSchedulableAlternative) {
        ...
        if (state && state.metrics) {
          pushMetricError(state.metrics, pathname, fallbackProvider || 'unknown', {
            ...
            pinnedAccountRef,
            detail: persistedPin
              ? `pinned account not schedulable (status=${String(persistedPin.status || 'unknown')}), fell back to pool`
              : 'pinned account unknown to the persisted store, fell back to pool'
          });
        }

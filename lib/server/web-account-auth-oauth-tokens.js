'use strict';
// GENERATED BY web-account-auth-split script — 一次性拆分已完成，本文件此后按普通源文件维护。
...
// CodeBuddy 家族：四支客户端的共享凭据都在 HOME 下 CodeBuddyExtension，不在
// configDir（== nativeRoot）内，所以不能按 nativeRoot 段裁剪后拼接，直接按
// HOME 相对路径拼 runtimeDir。文件名随 Provider 不同（见 storage policy 同名常量）：
// 国内站两支（codebuddycn / workbuddycn）共用 `workbuddy-desktop.info`，
// 国际站两支各用各的（`Tencent-Cloud.coding-copilot.info` / `workbuddy-desktop-ai.info`）。
function resolveCodebuddySharedAuthPath(relativePath) {
  return (job) => {
    const runtimeDir = String((job && job.runtimeDir) || '').trim();
...
const OAUTH_ARTIFACT_PATH_OVERRIDES = Object.freeze({
  ...
  codebuddy: resolveCodebuddySharedAuthPath(CODEBUDDY_INTL_SHARED_AUTH_PATH),
  codebuddycn: resolveCodebuddySharedAuthPath(CODEBUDDY_CN_SHARED_AUTH_PATH),
  workbuddy: resolveCodebuddySharedAuthPath(CODEBUDDY_AI_SHARED_AUTH_PATH),
  workbuddycn: resolveCodebuddySharedAuthPath(CODEBUDDY_CN_SHARED_AUTH_PATH)
});

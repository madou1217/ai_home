...
// ZCode desktop's own Z.ai OAuth client. The CLI's poll-based login
// (`oauth/cli/init`) was retired upstream (404), but the desktop flow —
...
// writes the encrypted credentials.json the CLI itself reads.
...
function createAuthJobManager(options = {}) {
  ...
  // provider access token；两者缺一即视为交换失败。写盘格式与 ZCode CLI

...
function hasOpenCodeAuthTokens(profileDir, fs) {
  const authPath = path.join(String(profileDir || ''), '.local', 'share', 'opencode', 'auth.json');
  if (!authPath || !fs.existsSync(authPath)) return false;
  try {
    const auth = JSON.parse(fs.readFileSync(authPath, 'utf8'));
...
function getOauthArtifactPath(job) {
  if (!job || (!job.configDir && !job.profileDir)) return '';
  ...
  if (job.provider === 'opencode') return path.join(String(job.profileDir || ''), '.local', 'share', 'opencode', 'auth.json');
  return '';
}
...
function hasOauthCompletionArtifacts(job, fs) {
  ...
  else if (job.provider === 'opencode') completed = hasOpenCodeAuthTokens(job.profileDir, fs);
...
function createAuthJobManager(options = {}) {
  ...
  function startNativeOauthJob(provider, nativeOauth, accountId, profileDir, configDir, existingAccountId, previousAccountState = null) {
    ...
    appendJobLog(job, `授权任务：provider=${provider} account=${accountId}${existingAccountId ? ' reauth=true' : ''}`);
    jobs.set(jobId, job);
  ...
  function startOauthJob(provider, authMode, jobOptions = {}) {
    ...
    try {
      prepareProviderRuntime(normalizedProvider, profileDir, processObj.env, {
        path,
        fs,
    ...
    const envOverrides = buildProviderRuntimeEnv(normalizedProvider, profileDir, processObj.env, {
      path,
      platform: processObj.platform
    });

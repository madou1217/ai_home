...
function getOauthArtifactPath(job) {
  ...
  if (job.provider === 'agy') return path.join(String(job.configDir || ''), 'log');
  return '';
}
...
function hasOauthCompletionArtifacts(job, fs) {
  ...
  else if (job.provider === 'agy') completed = readAgyAuthMetadata(fs, path, job.profileDir).configured;
...
function submitAgyAuthorizationCode(job, rawCallbackUrl) {
  if (!job || job.provider !== 'agy') return { ok: false, code: 'callback_not_supported', job };
  const parsed = parseAuthorizationCodeInput(rawCallbackUrl, job.redirectUri);
  if (!parsed) {
    appendJobLog(job, 'Antigravity 授权码解析失败。');
    return { ok: false, code: 'invalid_authorization_code', job };
  }
  if (parsed.error) {
    job.error = parsed.error;
    appendJobLog(job, `OAuth provider 返回错误：${compactLogText(parsed.error)}`);
    finalizeJob(job, 'failed', parsed.error, 1);
    return { ok: false, code: 'oauth_provider_error', job };
  }
  if (job.oauthState && parsed.state && parsed.state !== job.oauthState) {
    appendJobLog(job, 'Antigravity 授权码 state 校验失败。');
    return { ok: false, code: 'invalid_callback_state', job };
  }
  if (!parsed.code) {
    appendJobLog(job, 'Antigravity 授权码为空。');
    return { ok: false, code: 'invalid_authorization_code', job };
  }

  const ptyProcess = job._ptyProcess;
  if (!ptyProcess || typeof ptyProcess.write !== 'function') {
    appendJobLog(job, 'Antigravity 授权码无法写回：当前 PTY 不支持自动输入。');
    return { ok: false, code: 'authorization_code_forward_unavailable', job };
  }

  appendJobLog(job, '收到 Antigravity 授权码，已写回 CLI 等待原生登录完成。');
  ptyProcess.write(`${parsed.code}\r`);
...
function createAuthJobManager(options = {}) {
  ...
  function startManualCallbackCodexOauthJob(accountId, profileDir, configDir, existingAccountId) {
    ...
    const job = {
      ...
      _reauthTargetId: existingAccountId || ''
    };
  ...
  function startOauthJob(provider, authMode, jobOptions = {}) {
    ...
    const tempAccountId = getNextAccountIdFromIds(getToolAccountIds(normalizedProvider));
    const accountId = tempAccountId;
    const { profileDir, configDir } = ensureAccountSandbox({
      fs,
    ...
    if (normalizedProvider === 'codex') {
      const sqliteHome = resolveCodexSqliteHome({ path, profileDir });
  ...
  async function completeBrowserOauthCallback(jobId, rawCallbackUrl) {
    ...
    if (job.provider === 'agy') {
      return { ...submitAgyAuthorizationCode(job, rawCallbackUrl), job: refreshJobState(job) };
    }
    if (job._manualCallbackOauth) {

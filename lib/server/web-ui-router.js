...
const { resolveStatusAfterOauthSuccess } = require('../account/account-recovery-state');
...
const { handleWebUiChatRuntimeRequest } = require('./webui-chat-runtime-routes');
const { handleWebUiChatSessionsRoutes } = require('./webui-chat-sessions-routes');

const { handleWebUiTerminalRequest } = require('./webui-terminal-routes');
const { handleWebUiClientTerminalRoutes } = require('./webui-client-terminal-routes');
const { handleWebUiTaskRoutes } = require('./webui-task-routes');
...
const { handleProviderCliUpgradeRoutes } = require('./webui-provider-cli-upgrade-routes');
const {
...
const { handleFileTreeRequest } = require('./webui-file-tree-routes');
const { handleGitSummaryRequest, handleGitDiffRequest } = require('./webui-git-routes');
const {
...
} = require('./webui-session-lifecycle-routes');
const {
...
} = require('./webui-frp-config-routes');
const {
  handleWebUiOutboundRelayRoutes
} = require('./webui-outbound-relay-routes');
const {
...
const {
  ...
  matchEgressRoute
} = require('./webui-zcode-egress-routes');
const {
...
const { handleWebUiEnvironmentRoutes } = require('./webui-environment-routes');
const { handleWebUiImageStudioRoutes } = require('./webui-image-studio-routes');
const { handleWebUiToolkitRoutes } = require('./webui-toolkit-routes');
const { handleAppInstallJobRoutes } = require('./webui-app-install-routes');
const {
  handleWebUiCodexResetCreditRoutes
} = require('./webui-codex-reset-credit-routes');
const {
  handleWebUiAccountQuotaResetRoutes
} = require('./webui-account-quota-reset-routes');
const {
...
async function handleOauthJobFinishedStateSync(deps, state, job) {
  ...
  const effectiveStatus = resolveStatusAfterOauthSuccess(previousState, {
  ...
  if (job.provider === 'codex') {
    ...
    try {
      const invalidation = invalidateCodexAppServerEndpoint({
      ...
      appendOauthJobSyncLog(
        job,
        invalidation && invalidation.invalidated
          ? '状态同步：已失效当前账号的 Codex app-server，后续连接将使用新凭据。'
          : '状态同步：当前账号没有运行中的 Codex app-server。'
      );
    } catch (error) {
      appendOauthJobSyncLog(
        job,
        `状态同步：Codex app-server 失效失败：${String(error && (error.code || error.message) || 'unknown_error')}`
      );
    }
  }
  const runtimeAccounts = deps.loadServerRuntimeAccounts(withAccountQueryListFns({
    fs: deps.fs,
    // 漏掉 aiHomeDir 会让凭据目录读不到任何账号，reload 直接把账号池换成空池。
    aiHomeDir: deps.aiHomeDir,
...
function cleanupAuthJobArtifacts(job, deps, state) {
  ...
  // A cancellation/completion notification may arrive immediately after kill().
  // Wait for the PTY exit callback before moving or deleting its fake HOME.
  if (job._ptyProcess) return;
  const { fs, aiHomeDir, accountStateIndex, loadServerRuntimeAccounts, getToolConfigDir, getProfileDir, checkStatus, applyReloadState } = deps;
  ...
  if (runtimeDir) {
    try {
      ...
      if (!error || error.code !== 'ENOENT') {
        job.resourceReconciliationError = String((error && error.message) || error || 'unknown_error');
        return;
      }
    }
  }
  if (runtimeExists) {
    try {
      reconcileProviderResources(
        deps.ensureSessionStoreLinks,
        job.provider,
        job.accountRef || `login-${job.id || 'transient'}`,
...
async function handleWebUIRequest(ctx) {
  ...
  if (await handleWebUiChatRuntimeRequest(routeCtx)) return true;
  if (await handleWebUiChatSessionsRoutes(routeCtx)) return true;
  ...
  if (pathname === '/v0/webui/server-routes/frp' || pathname.startsWith('/v0/webui/server-routes/frp/')) {
    const handled = await handleWebUiFrpConfigRoutes(routeCtx);
    if (handled) return true;
  }

  if (pathname === '/v0/webui/server-routes/relays') {
    const handled = await handleWebUiOutboundRelayRoutes(routeCtx);
    if (handled) return true;
  }
  ...
  if (pathname === '/v0/webui/studio/image' || pathname.startsWith('/v0/webui/studio/image/')) {
    const handled = await handleWebUiImageStudioRoutes(routeCtx);
    if (handled) return true;
  }
  ...
  // WebUI Toolkit routes: /v0/webui/toolkit/*
  if (pathname === '/v0/webui/terminals' || pathname.startsWith('/v0/webui/toolkit/terminals')) {
    const handled = await handleWebUiClientTerminalRoutes(req, res, method, pathname, routeCtx);
    if (handled) return true;
  }

  // 全局后台任务队列：页面导航/刷新只会重新订阅，不会影响服务端任务。
  if (pathname === '/v0/webui/tasks' || pathname === '/v0/webui/tasks/watch') {
    const handled = await handleWebUiTaskRoutes(routeCtx);
    if (handled) return true;
  }

  if (pathname === '/v0/webui/toolkit/environments' || pathname.startsWith('/v0/webui/toolkit/environments/')) {
    const handled = await handleWebUiEnvironmentRoutes(routeCtx);
    if (handled) return true;
  }

  // WebUI Toolkit routes: /v0/webui/toolkit/*
  if (pathname.startsWith('/v0/webui/toolkit')) {
    const handled = await handleWebUiToolkitRoutes(req, res, method, pathname, routeCtx);
    if (handled) return true;
  }

  // 统一的 CLI/Desktop 安装任务：确认后异步执行，并通过 SSE 推送进度。
  if (pathname === '/v0/webui/app-install' || pathname.startsWith('/v0/webui/app-install/jobs')) {
    const handled = await handleAppInstallJobRoutes(routeCtx);
    if (handled) return true;
  }
  ...
  // GET /v0/webui/provider-cli-upgrade - 只读展示 provider CLI 自动升级的账本与调度状态
  if (pathname === '/v0/webui/provider-cli-upgrade') {
    const handled = await handleProviderCliUpgradeRoutes(routeCtx);
    if (handled) return true;
  }
  ...
  const accountQuotaResetHandled = await handleWebUiAccountQuotaResetRoutes(routeCtx);
  if (accountQuotaResetHandled) return true;

  const codexResetCreditHandled = await handleWebUiCodexResetCreditRoutes(routeCtx);
  if (codexResetCreditHandled) return true;
  ...
  if (method === 'POST' && pathname.match(/^\/v0\/webui\/accounts\/add\/jobs\/([^/]+)\/install$/)) {
    const handled = await handleConfirmAddJobInstallRequest(routeCtx);
    return handled;
  }
  ...
  // GET /v0/webui/app-entries - 宿主机实测的 Provider 桌面/CLI 入口可用性
  if (method === 'GET' && pathname === '/v0/webui/app-entries') {
    const handled = await handleListAppEntriesRequest(routeCtx);
    return handled;
  }

  // POST /v0/webui/accounts/kimi/:accountRef/desktop-session/start|poll - kimi 桌面版托管扫码登录
  if (method === 'POST' && pathname.match(/^\/v0\/webui\/accounts\/([^/]+)\/([^/]+)\/desktop-session\/start$/)) {
    const handled = await handleKimiDesktopSessionStartRequest(routeCtx);
    return handled;
  }
  if (method === 'POST' && pathname.match(/^\/v0\/webui\/accounts\/([^/]+)\/([^/]+)\/desktop-session\/poll$/)) {
    const handled = await handleKimiDesktopSessionPollRequest(routeCtx);
    return handled;
  }

  // GET /v0/webui/accounts/kimi/:accountRef/plan-stats - kimi 套餐配额（月度总量/5h/7d/Gift）
  if (method === 'GET' && pathname.match(/^\/v0\/webui\/accounts\/([^/]+)\/([^/]+)\/plan-stats$/)) {
    const handled = await handleKimiPlanStatsRequest(routeCtx);
    return handled;
  }

  // POST /v0/webui/accounts/:provider/:accountRef/open-app - 打开该账号的 Desktop 应用或 CLI 终端
  if (method === 'POST' && pathname.match(/^\/v0\/webui\/accounts\/([^/]+)\/([^/]+)\/open-app$/)) {
    const handled = await handleOpenAccountAppRequest(routeCtx);
    return handled;
  }
  ...
  // GET|POST /v0/webui/accounts/:provider/:accountRef/egress - 账号专属出口绑定
  // （用于验证/规避可能存在的出口 IP 维度限制）
  if (matchEgressRoute(method, pathname)) {
    const handled = await handleZcodeEgressRequest(routeCtx);
    return handled;
  }
  ...
  // GET /v0/webui/fs/tree - 浏览已登记项目内的目录
  if (method === 'GET' && pathname === '/v0/webui/fs/tree') {
    return handleFileTreeRequest(routeCtx);
  }
  ...
  // GET /v0/webui/fs/access - 使用同一策略预检媒体文件，避免浏览器直接加载后只得到模糊 403
  if (method === 'GET' && pathname === '/v0/webui/fs/access') {
    const handled = await handleCheckFileAccessRequest(routeCtx);
    return handled;
  }

  // POST /v0/webui/fs/trust - 按服务端派生的 scope 持久化可信目录
  if (method === 'POST' && pathname === '/v0/webui/fs/trust') {
    const handled = await handleTrustFilePathRequest(routeCtx);
    return handled;
  }
  ...
  // GET /v0/webui/git/summary - 获取只读 Git 工作区状态
  if (method === 'GET' && pathname === '/v0/webui/git/summary') {
    return handleGitSummaryRequest(routeCtx);
  }

  // GET /v0/webui/git/diff - 获取单文件只读 diff
  if (method === 'GET' && pathname === '/v0/webui/git/diff') {
    return handleGitDiffRequest(routeCtx);
  }
  ...
  // GET /v0/webui/sessions/lifecycle-capabilities - 原生会话生命周期能力
  if (method === 'GET' && pathname === '/v0/webui/sessions/lifecycle-capabilities') {
    return handleGetSessionLifecycleCapabilitiesRequest(routeCtx);
  }
  ...
  // POST /v0/webui/chat/cli-install-confirmations/:confirmationId - 安装缺失 CLI 前的一次性确认
  if (method === 'POST' && pathname.match(/^\/v0\/webui\/chat\/cli-install-confirmations\/([^/]+)$/)) {
    const handled = await handleCliInstallConfirmationDecisionRequest(routeCtx);
    return handled;
  }

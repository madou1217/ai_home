...
} = require('./webui-account-egress-catalog-routes');
...
async function handleWebUIRequest(ctx) {
  ...
  if (pathname.startsWith('/v0/webui/account-egress/catalog')) {
    const handled = await handleWebUiAccountEgressCatalogRoutes(req, res, method, pathname, routeCtx);
    if (handled) return true;
  }

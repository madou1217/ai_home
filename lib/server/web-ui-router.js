...
async function handleWebUIRequest(ctx) {
  ...
  // 电子书 AI 伴读免密 API 代理：/docs/*/api/chat 或 /docs/api/chat
  if (pathname === '/docs/api/chat' || /^\/docs\/[^/]+\/api\/chat$/.test(pathname)) {
    return handleHarnessBookCopilotRoute(routeCtx);
  }

  // 电子书静态文件服务：/docs/
  if (pathname.startsWith('/docs/')) {
    return serveDocsFile(pathname, res, { fs, options });
  }
...
/**
 * 提供 Harness 电子书静态文件与阅读器服务
 */
/**
 * 提供通用电子书 AI 伴读免密 API 代理
 */
async function handleHarnessBookCopilotRoute(routeCtx) {
  const { req, res, method, pathname } = routeCtx;
  if ((pathname !== '/docs/api/chat' && !/^\/docs\/[^/]+\/api\/chat$/.test(pathname)) || method !== 'POST') return false;

  // 标记该请求为电子书伴读免密请求，重写 URL 为 /v1/chat/completions 后由 server.js handleV1Request 统一消费
  req.__isHarnessBookCopilot = true;
  req.url = '/v1/chat/completions';
  return false; // 返回 false 让外层 server.js 进入 handleV1Request 处理
}

function serveDocsFile(pathname, res, { fs }) {
  const docsRootDir = path.join(__dirname, '../../docs');
  const relPath = pathname.replace(/^\/docs\//, '');
  ...
  let filePath = path.join(docsRootDir, normalizedPath);

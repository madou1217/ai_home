...
async function handleWebUIRequest(ctx) {
  ...
  if (method === 'GET' && pathname === '/v0/webui/models') {
    try {
      ...
      // 2. 如果某个 provider 缺少模型，从 state.accounts 拉取
      if (state && state.accounts) {
        const promises = [];

        for (const provider of SUPPORTED_SERVER_PROVIDERS) {
          if (result[provider] && result[provider].length > 0) continue; // 已有模型

          const providerAccounts = state.accounts[provider] || [];
          // 找第一个有 token 的账号
          let account = providerAccounts.find(a => a.accessToken);

          // API Key 模式的 Codex 账号：从 auth.json 读 OPENAI_API_KEY
          if (!account && provider === 'codex') {
            const accountIds = getToolAccountIds('codex');
            for (const id of accountIds) {
              try {
                const configDir = getToolConfigDir('codex', id);
                ...
                if (fs.existsSync(authPath)) {
                  ...
                  if (apiKey) {
                    ...
                    const envData = fs.existsSync(envPath) ? JSON.parse(fs.readFileSync(envPath, 'utf8')) : {};
                    account = {
                      provider: 'codex',
                      accountId: id,
                      accessToken: apiKey,
                      openaiBaseUrl: envData.OPENAI_BASE_URL || '',
                      profileDir
                    };
                    break;
                  }
          ...
          promises.push(
            fetchModelsForAccount(options, account, 8000)
              .then(models => {
                const ids = models.map(m => typeof m === 'string' ? m : (m.id || '')).filter(Boolean);
                ids.forEach(id => { if (registry) addModelToRegistry(registry, provider, id); });
                result[provider] = ids.sort();
              })
              .catch(() => { /* provider 拉取失败，跳过 */ })
          );
        }

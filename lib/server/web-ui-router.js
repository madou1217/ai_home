...
const { handleWebUiZcodeCaptchaRoutes } = require('./webui-zcode-captcha-routes');
...
async function handleWebUIRequest(ctx) {
  ...
  // 电子书 AI 伴读免密 API 代理：/docs/harness-book/api/chat
  if (pathname === '/docs/harness-book/api/chat') {
    return handleHarnessBookCopilotRoute(routeCtx);
  }

  // 电子书静态文件服务：/docs/harness-book/
  if (pathname.startsWith('/docs/harness-book/')) {
    return serveHarnessBookFile(pathname, res, { fs, options });
  }
  ...
  // zcode OAuth 计划账号的阿里云验证码桥（pending 恢复 + verify param 回传）。
  if (pathname.startsWith('/v0/webui/zcode-captcha')) {
    const handled = await handleWebUiZcodeCaptchaRoutes(routeCtx);
    if (handled) return true;
  }
 ...
 */
/**
 * 提供 Harness 电子书 AI 伴读免密 API 代理
 */
async function handleHarnessBookCopilotRoute(routeCtx) {
  const { req, res, method, pathname } = routeCtx;
  if (pathname !== '/docs/harness-book/api/chat' || method !== 'POST') return false;
...
function serveHarnessBookFile(pathname, res, { fs }) {
  const docsDir = path.join(__dirname, '../../docs/harness-book');
  const relPath = pathname.replace(/^\/docs\/harness-book\//, '');
  ...
  let filePath = path.join(docsDir, normalizedPath);
  ...
  if (!fs.existsSync(filePath)) {
    ...
    res.end('Harness book file not found');
    return true;
  }

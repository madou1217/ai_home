...
function readFastAccountPresence(provider, record, authMetadata = null, options = {}) {
  ...
  if (provider === 'kimi') {
    // kimi 的 OAuth 凭证在 native_auth_json 的 $.credentials 下；refresh_token 存在即视为已配置
    // （access_token 分钟级过期，由 refresh 守护按需续期），与 checkStatus 的判定保持一致。
    const creds = nativeAuth.credentials || nativeAuth.auth || {};
    return {
      configured: Boolean(
        String(creds.refresh_token || '').trim() || String(creds.access_token || creds.accessToken || '').trim()
      ),
      apiKeyMode: false
    };
  }

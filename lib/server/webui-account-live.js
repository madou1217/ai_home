...
function readAccountTokenUsage(ctx, accountRef) {
  ...
  const value = allUsage instanceof Map ? allUsage.get(key) : allUsage[key];
  return value
    ? normalizeAccountTokenUsageValue(value, ACCOUNT_TOKEN_USAGE_DIMENSIONS)
    // 没有任何记录的账号也要给全维度零值，避免前端少一根柱子。
    : Object.fromEntries(ACCOUNT_TOKEN_USAGE_DIMENSIONS.map((dimension) => [dimension, 0]));
}
...
// 绕过 10 分钟级 model-usage 扫描，让 WebUI 伤害数字随请求实时弹出。
function emitAccountTokenConsumedEvent(state, payload = {}) {
  if (!state) return;
  const provider = String(payload.provider || '').trim().toLowerCase();
  const accountRef = String(payload.accountRef || '').trim();
  ...
  emitAccountsEvent(liveState, {
    ...
    occurredAt: Number(payload.timestampMs) || Date.now()
  });
}

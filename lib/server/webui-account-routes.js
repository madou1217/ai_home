...
async function handleOpenAccountAppRequest(ctx) {
  ...
  const launcher = createAccountAppLauncher({
    ...
    resolveCliPath: (cliProvider) => resolveProviderCliPath(cliProvider, {
      fs: ctx.fs,
      path: ctx.path || ctx.deps && ctx.deps.path,
      processObj: ctx.processObj || ctx.deps && ctx.deps.processObj || process,
      env: ctx.env || ctx.deps && ctx.deps.env,
      hostHomeDir: ctx.deps && ctx.deps.hostHomeDir
    })
  });
  const result = launcher.launchAccountApp({ provider, accountRef, kind, action, terminalId });
  if (!result.ok) {
    ...
    if (isInstallMissing) {
      ...
      const installAvailable = typeof manager.canInstall === 'function'
    ...
    const statusCode = result.error === 'account_not_found' ? 404
      : (result.error === 'account_unconfigured' || result.error === 'account_auth_invalid' ? 409 : 400);
    writeJson(ctx.res, statusCode, {
      ok: false,
      error: result.error,
...
function matchRunningDesktopAccounts(ctx, instances) {
  ...
  for (const record of Array.isArray(records) ? records : []) {
    ...
    if (runningDirs.has(normalizeDir(path.join(profileDir, 'electron-user-data')))) {
      running.push(accountRef);
    }
  }
  return running;
}
...
async function handleListAppEntriesRequest(ctx) {
  ...
  try {
    ...
    runningAccounts = matchRunningDesktopAccounts(ctx, instances);
  } catch (_error) {
    runningAccounts = [];
  }
  ctx.writeJson(ctx.res, 200, { ok: true, entries, capabilities, runningAccounts });
  return true;
}

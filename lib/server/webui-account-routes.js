...
const { resolveCodexDesktopHookService, queueCodexDesktopAccountSync, setCodexDesktopAccount, clearCodexDesktopAccount, resolveAccountAppEligibility, sharedAppEntryDetector, getAppEntryDetector, matchRunningDesktopAccounts } = require('./webui-account-routes-desktop');
...
async function handleOpenAccountAppRequest(ctx) {
  ...
  const launcher = createAccountAppLauncher({
    ...
    processObj: ctx.processObj || ctx.deps && ctx.deps.processObj,
    env: ctx.env || ctx.deps && ctx.deps.env,
    execFileSync: ctx.execFileSync || ctx.deps && ctx.deps.execFileSync,
    spawn: ctx.spawn || ctx.deps && ctx.deps.spawn,
    ...
    resolveCliPath: (cliProvider) => resolveProviderCliPath(cliProvider, {
      ...
      env: ctx.env || ctx.deps && ctx.deps.env,
      resolveNativeCliPath: ctx.deps && ctx.deps.resolveNativeCliPath,
  ...
  if (!result.ok) {
    ...
    const statusCode = result.error === 'account_not_found' ? 404
      : (result.error === 'account_unconfigured'
        || result.error === 'account_auth_invalid'
        || ACCOUNT_APP_STATE_CONFLICT_ERRORS.has(result.error)
        ? 409
        : (result.error === 'agy_desktop_process_probe_failed' ? 503 : 400));
    const message = ACCOUNT_APP_ERROR_MESSAGES[result.error]

  ...
  try {
    ...
    if (provider === 'codex') {
      ...
      if (validation.ok) {
        ...
        desktopAccount = {
          eligible: true,
          synced: Boolean(result && result.ok),
          changed: Boolean(result && result.changed),
          hotSyncQueued: Boolean(result && result.sync && result.sync.queued),
    ...
    writeJson(ctx.res, 200, { ok: true, provider, accountRef, account, desktopAccount });
    return true;
  } catch (error) {
    writeJson(ctx.res, 500, {
      ok: false,
...
async function handleClearDefaultAccountRequest(ctx) {
  ...
  try {
    ...
    writeJson(ctx.res, 200, { ok: true, provider, accountRef, account });
    return true;
  } catch (error) {
    writeJson(ctx.res, 500, {
      ok: false,

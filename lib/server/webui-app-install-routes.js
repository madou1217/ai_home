...
const { findIdeClientRecord, getIdeClient } = require('../cli/services/toolkit/ide-client-registry');
...
const { getAppEntryDetector } = require('./webui-account-routes-desktop');
...
function createManagerFromContext(ctx = {}) {
  const deps = ctx.deps || {};
  const findInstalledDesktopRecord = (provider, installOptions = {}) => {
    const processObj = ctx.processObj || deps.processObj || process;
    const platform = resolveClientPlatform({
      platform: installOptions.platform || ctx.platform || deps.platform,
      processObj
    });
    if (getIdeClient(provider)) {
      return findIdeClientRecord(provider, {
        fs: ctx.fs || deps.fs,
        pathImpl: resolvePlatformPath(platform, ctx.path || deps.path),
        env: ctx.env || deps.env || processObj.env || {},
        platform,
        hostHomeDir: ctx.hostHomeDir || deps.hostHomeDir
      });
    }
    return findDesktopClientRecord(provider, {
      fs: ctx.fs || deps.fs,
  ...
  const resolveDesktopRecord = typeof deps.verifyDesktopInstall === 'function'
    ? deps.verifyDesktopInstall
    : findInstalledDesktopRecord;
  const resolveCliPath = (provider, lifecycleOptions = {}) => resolveProviderCliPath(provider, {
    ...lifecycleOptions,
    fs: ctx.fs || deps.fs,
    path: ctx.path || deps.path,
  ...
  const verifyCliLifecycle = typeof deps.verifyCliLifecycle === 'function'
    ? deps.verifyCliLifecycle
    : (provider, action, lifecycleOptions = {}) => {
      const cliPath = resolveCliPath(provider, lifecycleOptions);
      return action === 'uninstall' ? !cliPath : Boolean(cliPath);
    };
  return createAppInstallJobManager({
    ...
    invalidateAppEntries: typeof deps.invalidateAppEntries === 'function'
      ? deps.invalidateAppEntries
      : () => {
        const detector = getAppEntryDetector(ctx);
        if (detector && typeof detector.invalidate === 'function') detector.invalidate();
      },
    verifyDesktopInstall: resolveDesktopRecord,
    resolveDesktopRecord,
    resolveCliPath,
    verifyCliLifecycle
  });
}
...
async function handleAppInstallJobRoutes(ctx) {
  ...
  if (method === 'POST' && pathname === '/v0/webui/app-install') {
    ...
    if (!started.ok) {
      writeError(ctx, started.error === 'confirmation_required' ? 428 : 400, started.error, '无法创建安装任务。');
      return true;
    }

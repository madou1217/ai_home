...
function readCodexArchivedSessionProjectPath(fs, sessionFilePath) {
  let fd;
  try {
    ...
    for (const line of chunk.split('\n')) {
      ...
      try {
        ...
        const cwd = record && record.payload && record.payload.cwd;
        if (cwd) return String(cwd);
      } catch (_error) {}
    }
    const match = chunk.match(/"cwd"\s*:\s*("(?:\\.|[^"\\])*")/);
    if (match && match[1]) return String(JSON.parse(match[1]));
    return '';
  } catch (_error) {
    return '';
  } finally {
  ...
  try {
    ...
    if (fs.existsSync(archivedDir)) {
      ...
      for (const entry of fs.readdirSync(archivedDir, { withFileTypes: true })) {
        ...
        const projectPath = readCodexArchivedSessionProjectPath(fs, filePath);
        archived.push({
          id: sessionId,
          title,
          provider: 'codex',

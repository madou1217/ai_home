...
function buildApiProxyMessages(messages, images, options = {}) {
  ...
  const { normalizeChatDocuments, appendDocumentText } = require('./chat-document-attachments');
  const documents = normalizeChatDocuments(options.documents);
  if (documents.length) {
    const lastUser = list.findLast((message) => message.role === 'user');
    if (lastUser) lastUser.content = appendDocumentText(lastUser.content, documents);
    else list.push({ role: 'user', content: appendDocumentText(options.prompt, documents) });
  }

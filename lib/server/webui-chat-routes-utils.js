...
function humanizeUpstreamError(rawText, { status, model, provider } = {}) {
  ...
  switch (code) {
    case 'no_available_account':
      // zcode OAuth 计划账号只做账号管理/桌面启动/用量，不做推理 relay（既定事实），
      // 此时引导「补全凭据」是误导；只有 zcode API Key 账号才能参与推理。
      if (providerLabel.toLowerCase() === 'zcode') {
        return 'zcode OAuth 账号不支持推理调用，请改用 zcode API Key 账号或其他 provider。';
      }
      return (

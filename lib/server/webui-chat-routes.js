...
const { buildApiProxyAttachmentImages } = require('./chat-attachment-api-proxy');
...
async function handleGetChatAttachmentRequest(ctx) {
  ...
  const contentType = guessAttachmentMimeType(resolvedPath) || guessVideoMimeType(resolvedPath);
  if (!contentType) {
...
// GET /v0/webui/chat/runs?sessionId=&provider=&projectDirName= —— 列出注册表中仍在跑的 run
// （native 会话与 api-proxy 纯聊天，后者带 mode/contentSnapshot）。
...
async function handleNativeChatRunListRequest(ctx) {
  ...
  const runs = listRuns()
    ...
    .map((run) => ({
      ...
      // 链路区分：native-session（默认，可带 activePrompt 交互卡）/ api-proxy（纯聊天 relay，
      // 只携带已累积内容快照，恢复时不走 native 交互卡逻辑）。
      mode: normalizeString(run.mode) || 'native-session',
      contentSnapshot: (typeof run.getContentSnapshot === 'function'
        ? String(run.getContentSnapshot() || '')
        : ''),
...
// 首 token 前事件缓冲的兜底阈值：缓冲初衷是压住 agy/gemini PTY 暖机噪音，
// 但模型长时间只输出 thinking 时浏览器 eventstream 收不到任何数据(UI 一直"思考中")。
// 缓冲超过 TTL 或条数上限就把已缓冲事件直写 SSE 并停止后续缓冲。
const PRE_FIRST_TOKEN_BUFFER_TTL_MS = 2500;
const PRE_FIRST_TOKEN_BUFFER_MAX_EVENTS = 20;

function createPreFirstTokenBuffer(writeSse, options = {}) {
  const ttlMs = Number(options.ttlMs) > 0 ? Number(options.ttlMs) : PRE_FIRST_TOKEN_BUFFER_TTL_MS;
  const maxEvents = Number(options.maxEvents) > 0 ? Number(options.maxEvents) : PRE_FIRST_TOKEN_BUFFER_MAX_EVENTS;
  const buffered = [];
  let visible = false;
  let flushTimer = null;
  ...
  const flush = () => {
    ...
    if (visible) return;
    visible = true;
    buffered.forEach((item) => writeSse(item));
    buffered.length = 0;
  };
  return {
    push(eventPayload) {
      // 已可见(首 token 已出或兜底已触发)：后续事件直写，不再进缓冲。
      if (visible) {
        writeSse(eventPayload);
        return;
      }
      buffered.push(eventPayload);
      if (buffered.length >= maxEvents) {
        flush();
        return;
      }
      // TTL 从首个缓冲事件起算；unref 避免定时器拖住进程退出。
      if (!flushTimer) {
        flushTimer = setTimeout(flush, ttlMs);
...
// 流式结束时持久化到 chatStore 的指标：字段名与前端读取口径一致
// (ChatMessageMetrics: durationMs/ttftMs/outputTokens/tokensPerSec；StatsLine 另读 inputTokens)。
// 上游未给 usage 时不编造 token 数，只保留真实计时指标。
function buildChatTurnMetrics({ durationMs, ttftMs, usage }) {
  const metrics = { durationMs, ttftMs };
  const outputTokens = Math.max(0, Number(usage && (usage.completion_tokens != null
    ? usage.completion_tokens
    : usage.output_tokens)) || 0);
  const inputTokens = Math.max(0, Number(usage && (usage.prompt_tokens != null
    ? usage.prompt_tokens
    : usage.input_tokens)) || 0);
  if (outputTokens > 0) {
    metrics.outputTokens = outputTokens;
    if (durationMs > 0) metrics.tokensPerSec = Math.round(outputTokens / (durationMs / 1000));
  }
  if (inputTokens > 0) metrics.inputTokens = inputTokens;
  return metrics;
}

function resolveChatStoreHostHome(ctx) {
  return ctx.hostHomeDir || (ctx.deps && ctx.deps.hostHomeDir);
}

// Gateway requests normally use the account pool. Once a pure-chat session has
// an accountRef recorded, keep sending that pin so subsequent turns stay on the
// same execution account instead of being rebalanced by the pool.
function readPinnedChatSessionAccountRef(ctx, sessionId, provider) {
  const id = normalizeString(sessionId);
  if (!id) return '';
  try {
    const session = require('./webui-chat-store').readChatSession(
      id,
      resolveChatStoreHostHome(ctx)
    );
    if (!session || (normalizeString(session.provider).toLowerCase()
      && normalizeString(session.provider).toLowerCase() !== normalizeString(provider).toLowerCase())) {
      return '';
    }
    const accountRef = normalizeString(session.accountRef);
    if (!isAccountRef(accountRef)) return '';
    // CLI changes may leave the live pool stale. Drop an obsolete pin before
    // gateway selection so the conversation can move to an enabled account.
    const accountStateIndex = ctx.deps && ctx.deps.accountStateIndex;
    if (accountStateIndex && typeof accountStateIndex.getAccountState === 'function') {
      ...
      if (!persisted || persisted.status !== 'up') return '';
    }
    const normalizedProvider = normalizeString(provider).toLowerCase();
    const pool = ctx && ctx.state && ctx.state.accounts && ctx.state.accounts[normalizedProvider];
    if (Array.isArray(pool)) {
      const account = pool.find((item) => normalizeString(item && item.accountRef) === accountRef);
      if (!account) return '';
      if (String(account.schedulableStatus || '').trim() && account.schedulableStatus !== 'schedulable') return '';
      if (deriveAccountRuntimeStatus(account).status !== 'healthy') return '';
    }
    return accountRef;
  } catch (_error) {
    return '';
...
function readUpstreamAccountRef(response) {
  const headers = response && response.headers;
  const value = headers && typeof headers.get === 'function'
    ? headers.get('x-aih-server-account-ref')
    : '';
  return isAccountRef(value) ? normalizeString(value) : '';
}

function setUpstreamAccountRefHeader(res, accountRef) {
  if (res && typeof res.setHeader === 'function' && isAccountRef(accountRef)) {
    res.setHeader('x-aih-server-account-ref', accountRef);
  }
}

// api-proxy 纯聊天 run 的注册句柄（F6 断线韧性）：与 native run 同表注册进 run store，
// 刷新后 GET /chat/runs 能列出在途 run 并带已累积内容快照（getContentSnapshot）。
// mode: 'api-proxy' 用于区分 native 链路——无 activePrompt、不支持 input/resize，
// 前端恢复时不会误触发 native 专属交互卡。abort 取消上游流（reader.cancel），
// 由完成路径把已累积内容收尾落盘。
function createApiProxyRunHandle(options = {}) {
  const getContent = typeof options.getContent === 'function' ? options.getContent : () => '';
  const cancelUpstream = typeof options.cancelUpstream === 'function' ? options.cancelUpstream : null;
  return {
    runId: normalizeString(options.runId),
    mode: 'api-proxy',
    provider: normalizeString(options.provider),
    ...(options.identity && typeof options.identity === 'object' ? options.identity : {}),
    sessionId: normalizeString(options.sessionId),
    projectDirName: normalizeString(options.projectDirName),
    projectPath: normalizeString(options.projectPath),
    ...
    writeInput() {
      const error = new Error('api_proxy_run_input_unsupported');
      error.code = 'api_proxy_run_input_unsupported';
      throw error;
    }
  };
}
...
async function handleChatRequest(ctx) {
  ...
  const payload = await readRequestBody(ctx.req, { maxBytes: 192 * 1024 * 1024 })
    .then((buf) => buf ? JSON.parse(buf.toString('utf8')) : null)
    .catch(() => null);
  ...
  if (isExplicitPureChatMode) {
    writeJson(ctx.res, 409, {
      ok: false,
      error: 'chat_harness_required',
      message: 'Chat 已迁移到原生 Harness，请刷新页面后继续对话。',
      endpoint: '/v0/webui/chat/sessions'
    });
    return true;
  }

  const pinnedChatSessionAccountRef = gateway
    ? readPinnedChatSessionAccountRef(ctx, sessionId, provider)
    : '';
  ...
  let apiProxyImages = [];
  let documents = [];
  let normalizedLegacyAttachments = [];
  let persistedAttachmentPaths = [];
  const rollbackLegacyAttachments = () => {
    cleanupPersistedAttachments(fs, persistedAttachmentPaths);
    persistedAttachmentPaths = [];
  };
  try {
    // Legacy chat carries documents as decoded text and images as data URLs.
    // Normalize the complete mixed batch before creating any directory or file;
    // this keeps the legacy and canonical upload contracts identical and makes
    // validation failure atomic from the caller's point of view.
    normalizedLegacyAttachments = normalizeLegacyChatAttachments({
      images: Array.isArray(images) ? images : [],
      documents: Array.isArray(requestedDocuments) ? requestedDocuments : []
    });
    documents = normalizedLegacyAttachments
      .filter((attachment) => attachment.kind === ATTACHMENT_KINDS.DOCUMENT)
      .map(({ kind, size, buffer, dataUrl, ...document }) => document);
  } catch (error) {
    writeJson(ctx.res, 400, canonicalizeChatPayload(ctx, provider, {
      ok: false,
      error: String(error && error.code || 'invalid_chat_attachments'),
      message: String((error && error.message) || error || 'invalid_chat_attachments')
    }));
    return true;
  }

  if (normalizedLegacyAttachments.length > 0) {
    try {
      // Work needs file paths for documents and original videos. One shared
      // materializer gives legacy and canonical uploads the same rollback
      // semantics when any later file, frame extraction, or DB step fails.
      const materialized = await materializeChatAttachments(normalizedLegacyAttachments, {
        fs,
        provider,
      ...
      persistedAttachmentPaths = materialized.map((item) => item.filePath);
      const documentPaths = materialized
        .filter((item) => item.attachment.kind === ATTACHMENT_KINDS.DOCUMENT)
        .map((item) => item.filePath);
      const preparedVideos = materialized
        .filter((item) => item.attachment.kind === ATTACHMENT_KINDS.VIDEO)
        .map((item) => item.prepared);
      persistedImagePaths = materialized
        .filter((item) => item.attachment.kind === ATTACHMENT_KINDS.IMAGE)
        .map((item) => item.filePath)
        .concat(preparedVideos.flatMap((video) => video.frames));
      apiProxyImages = buildApiProxyAttachmentImages(materialized, fs);
      normalizedPrompt = requestedChatMode === 'chat'
        ? appendDocumentText(normalizedPrompt, documents)
        : appendDocumentPathsToPrompt(normalizedPrompt, documentPaths);
      normalizedPrompt = appendVideoContextToPrompt(normalizedPrompt, preparedVideos, {
        toolsAvailable: requestedChatMode !== 'chat'
      });
      normalizedPrompt = appendImagePathsToPrompt(normalizedPrompt, persistedImagePaths);
    } catch (error) {
      rollbackLegacyAttachments();
      writeJson(ctx.res, 400, canonicalizeChatPayload(ctx, provider, {
        ok: false,
        error: String(error && error.code || 'invalid_chat_attachments'),
        message: String((error && error.message) || error || 'invalid_chat_attachments')
  ...
  // Codex native 会话在 spawn 前都要同步宿主配置：API-key 账号需要写入
  // 账号自己的 provider 段，OAuth 账号则必须移除上一次 API-key 会话留下的
  // aih_server auth command，否则 Codex 会用 OAuth 账号调用 auth 命令并报
  // “has no OPENAI_API_KEY credential”。同步保持幂等，失败仍由后续 native
  // 会话返回真实错误。
  if (provider === 'codex' && !useAihServerProfile) {
  ...
  if (runNativeInteractive) {
    ...
    const useHeadlessStream = (
      ...
      || provider === 'qodercn'
      || provider === 'grok'
      || ['codebuddy', 'codebuddycn', 'workbuddy', 'workbuddycn'].includes(provider)
    )
    ...
    try {
      ...
      if (stream !== false) {
        ...
        const createAttemptStream = (attemptIndex) => {
          ...
          // 首 token 前缓冲(压 agy/gemini PTY 暖机噪音),带 TTL/条数兜底,详见 createPreFirstTokenBuffer。
          const preFirstTokenBuffer = createPreFirstTokenBuffer(writeSse);
          const handleStreamEvent = (event) => {
            ...
            preFirstTokenBuffer.push(eventPayload);
          };
          ...
          return {
            attemptAccountRef,
            flush: preFirstTokenBuffer.flush,
            streamInstance
          };
        };
        ...
        const runAttempt = async (attemptIndex, existingAttempt = null) => {
          ...
          try {
            ...
            // 兜底：整轮都没出首 token 时(纯 thinking/暖机噪音),收尾把残余缓冲倒出。
            attempt.flush();
            return {
              result,
  ...
  // API-proxy has no native video part. Send real images plus committed video
  // frames through its existing image_url lane; never mislabel video bytes as
  // an image URL.
  const baseApiProxyMessages = buildApiProxyMessages(messages, apiProxyImages, {
    model,
    provider,
    documents,
    prompt
  ...
  if (
    ...
    try {
      const handled = await handleClaudeAnthropicCompatibleChat(ctx, {
        provider,
        accountRef,
        model,
      ...
      if (!handled || ctx.res.statusCode >= 400) rollbackLegacyAttachments();
      return handled;
    } catch (error) {
      rollbackLegacyAttachments();
      const failure = {
  ...
  // api-proxy 流式 run 句柄（注册进 run store 后，catch/完成路径据此注销并发布 turn 事件）。
  let apiProxyRun = null;
  // run 收尾：注销 + 发布 turn-completed/turn-failed。刷新后重连的页面靠 watch 收到
  // turn 事件重载会话（chatStore 已完成落盘），拿到完整内容。
  const settleApiProxyRun = (outcome, reason) => {
    if (!apiProxyRun) return;
    const settledRun = apiProxyRun;
    apiProxyRun = null;
    if (typeof unregisterNativeChatRun === 'function') {
      unregisterNativeChatRun(settledRun.runId);
    }
    publishNativeSessionEvent(ctx, {
      provider,
      ...
      type: outcome === 'completed' ? 'session:turn-completed' : 'session:turn-failed',
      reason,
      phase: outcome === 'completed' ? 'turn-completed' : 'turn-failed',
      at: Date.now(),
  ...
  try {
    ...
    // 只限制内层网关返回响应头的等待时间，并尊重更长的上游超时配置。
    // Codex 流由适配器逐事件转发；这里的超时预算不能替代真正的流式传输。
    const configuredProxyTimeoutMs = Number(options.upstreamTimeoutMs);
    const proxyTimeoutMs = Math.max(
      120000,
      Number.isFinite(configuredProxyTimeoutMs) && configuredProxyTimeoutMs > 0
    ...
    const response = await fetchWithTimeout(apiUrl, {
      ...
      headers: {
        ...
        'X-Provider': provider,
        ...((pinnedChatSessionAccountRef || (!useAihServerProfile ? accountRef : ''))
          ? { 'X-Account-Ref': pinnedChatSessionAccountRef || accountRef }
          : {})
      },
    ...
    if (stream) {
      const startedAt = Date.now();
      let firstTokenAt = 0;
      const upstreamAccountRef = readUpstreamAccountRef(response) || pinnedChatSessionAccountRef || accountRef;
      setUpstreamAccountRefHeader(ctx.res, upstreamAccountRef);
      // 网关流式末块可能携带 usage(protocol-stream-pipeline end()),留存供结束后持久化指标。
      ...
      // F6 断线韧性：api-proxy 纯聊天 run 注册进 run store（与 native 同表）。刷新后前端
      // GET /chat/runs 列出在途 run + 已累积内容快照（use-detached-run-recovery 恢复
      // "运行中"态并立即渲染部分内容）；完成/失败路径注销并发布 turn 事件驱动 watch 重载。
      let apiProxyContent = '';
      let apiProxyReader = null;
      apiProxyRun = createApiProxyRunHandle({
        runId: (typeof crypto.randomUUID === 'function'
          ? crypto.randomUUID()
          : `api-proxy-${startedAt}-${Math.random().toString(16).slice(2)}`),
        provider,
        identity: responseIdentity,
        accountRef: upstreamAccountRef,
        sessionId: chatSessionId,
        projectDirName: resolvedProjectDirName,
        projectPath,
        startedAt,
        getContent: () => apiProxyContent,
        cancelUpstream: () => {
          try {
            if (apiProxyReader && typeof apiProxyReader.cancel === 'function') {
              apiProxyReader.cancel();
            }
          } catch (_error) { /* best effort */ }
        }
      });
      if (typeof registerNativeChatRun === 'function') {
        registerNativeChatRun(apiProxyRun);
      }
      publishNativeSessionEvent(ctx, {
        provider,
        sessionId: chatSessionId,
        ...
        type: 'session:turn-started',
        reason: 'api_proxy_ready',
        phase: 'turn-started',
        at: startedAt,
        runId: apiProxyRun.runId
      });
      writeSse({
        type: 'ready',
        mode: 'api-proxy',
      ...
      if (!reader) {
        ...
        settleApiProxyRun('failed', 'api_proxy_stream_unavailable');
        rollbackLegacyAttachments();
        ctx.res.end();
        return true;
      }
      apiProxyReader = reader;
          ...
          while (boundary >= 0) {
            ...
            if (rawBlock) {
              ...
              if (payloadText && payloadText !== '[DONE]') {
                let parsed = null;
                try { parsed = JSON.parse(payloadText); } catch (_error) {}
                if (parsed && parsed.error) {
                  throw new Error(humanizeUpstreamError(JSON.stringify(parsed), { model, provider }));
                }
                if (parsed) {
                  ...
                  // 累积内容快照:detached 刷新后 /chat/runs 据此返回已有部分内容。
                  if (deltaText) apiProxyContent += deltaText;
        ...
        try {
          ...
          chatStore.appendMessageToChatSession(
            ...
            totalContent,
            {
              ...
              model,
              accountRef: upstreamAccountRef,
              metrics: buildChatTurnMetrics({ durationMs, ttftMs, usage: streamUsage })
            },
      ...
      adapter.finalize();
      settleApiProxyRun('completed', 'api_proxy_done');
      ctx.res.end();
    } else {
      const upstreamAccountRef = readUpstreamAccountRef(response) || pinnedChatSessionAccountRef || accountRef;
      setUpstreamAccountRefHeader(ctx.res, upstreamAccountRef);
    ...
    rollbackLegacyAttachments();
    settleApiProxyRun('failed', String((error && error.code) || 'api_proxy_failed'));
    const failure = {
      ok: false,
      error: 'chat_failed',
      message: String((error && error.message) || error || 'unknown')
...
module.exports = {
  ...
  createPreFirstTokenBuffer,
  buildChatTurnMetrics,

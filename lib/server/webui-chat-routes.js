...
async function handleNativeChatRunListRequest(ctx) {
  ...
  writeJson(ctx.res, 200, { ok: true, runs });
  return true;
}
...
// 仅 loopback 可达(webui-auth-gate 对本机对端自动信任;权限工具与 server 同机)。
...
async function handleChatRequest(ctx) {
  ...
  if (Array.isArray(images) && images.length > 0) {
    try {
      const profileDir = typeof getProfileDir === 'function'
        ? getProfileDir(provider, accountRef, { gateway: useAihServerProfile })
        : '';
  ...
  if (runNativeInteractive) {
    ...
    const useHeadlessStream = (provider === 'claude' || provider === 'codex' || provider === 'opencode')
        ...
        const createAttemptStream = (attemptIndex) => {
          ...
          const streamInstance = useCodexAppServerRunner
            ...
            : spawnNativeSessionStream({
              ...
              // 终端模式是常驻交互进程,不能在 transcript 一更新就判定「本轮完成」并杀/收编;
              // 由进程自身退出或用户关闭终端结束(与 slash 一致)。
              completeOnTranscriptUpdate: useOfficialNativeSession && !useHeadlessStream && !useTerminalMode,
              imagePaths: persistedImagePaths,
              model: nativeSessionModel,
              getProfileDir,
              ensureSessionStoreLinks,

...
function attachAbortableRequestClose(req, onClose) {
  if (!req || typeof req.on !== 'function' || typeof onClose !== 'function') return () => {};
...
async function handleClaudeAnthropicCompatibleChat(ctx, request) {
  ...
  attachAbortableRequestClose(ctx.req, closeStream);
...
async function handleChatRequest(ctx) {
  ...
  const useInteractiveCodexSessionInput = Boolean(
    String(provider || '').trim().toLowerCase() === 'codex'
    && sessionId
    && !useInteractiveNativeSlash
    && normalizedPrompt
  );
  const useInteractiveNativeMode = useInteractiveNativeSlash || useInteractiveCodexSessionInput;
  const nativeSessionModel = normalizeNativeSessionModel(provider, requestModel, apiKeyMode);
  ...
  if (!apiKeyMode && (sessionId || createSession) && normalizedPrompt) {
    try {
      ...
      if (stream !== false) {
        ...
        const createAttemptStream = (attemptIndex) => {
          ...
          const streamInstance = spawnNativeSessionStream({
            ...
            projectPath,
            prompt: useInteractiveNativeMode ? '' : normalizedPrompt,
            initialInput: useInteractiveNativeMode ? normalizedPrompt : '',
            interactiveCli: useInteractiveNativeMode,
            ...
            onEvent(event) {
              if (!event) return;
              if (!useInteractiveNativeMode && event.type === 'terminal-output') {
                return;
              }
              const eventPayload = {
                ...event,
        ...
        const runAttempt = async (attemptIndex, existingAttempt = null) => {
          ...
          try {
            ...
            const canRetry = !useInteractiveNativeMode
        ...
        runAttempt(0, initialAttempt).then(async ({ result, attemptAccountId }) => {
          ...
          const shouldRefreshPersistedProjects = Boolean(createSession && !sessionId && resolvedSessionId);
          if (shouldRefreshPersistedProjects) {
            try {
      ...
      if (createSession && !sessionId && resolvedSessionId) {
        try {
          await refreshProjectsSnapshotAfterNativeSession(ctx, provider, resolvedSessionId, resolvedProjectDirName);
        } catch (_error) {
          // best effort
  ...
  const chatRequest = {
    ...
    messages: injectProjectContextMessage(
      buildApiProxyMessages(messages, images),

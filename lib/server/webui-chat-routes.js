...
const { normalizeChatDocuments, persistChatDocuments, appendDocumentPathsToPrompt, appendDocumentText } = require('./chat-document-attachments');
...
async function handleChatRequest(ctx) {
  ...
  const payload = await readRequestBody(ctx.req, { maxBytes: 32 * 1024 * 1024 })
    .then((buf) => buf ? JSON.parse(buf.toString('utf8')) : null)
    .catch(() => null);
  ...
  let documents = [];
  try {
    documents = normalizeChatDocuments(requestedDocuments);
    if ((Array.isArray(images) ? images.length : 0) + documents.length > 8) {
      throw new Error('每次最多附加 8 个文件');
    }
    const documentPaths = persistChatDocuments(documents, {
      fs, provider, aiHomeDir: ctx.aiHomeDir,
      hostHomeDir: ctx.deps && ctx.deps.hostHomeDir, projectPath
    });
    ...
    writeJson(ctx.res, 400, { ok: false, error: 'invalid_chat_documents', message: error.message });
    return true;
  }

...
const ATTACHMENT_BODY_LIMIT = 192 * 1024 * 1024;
...
async function handleSessionCollection(ctx, service) {
  ...
  if (ctx.method === 'POST') {
    const input = createSessionDto(await readJsonBody(ctx));
    const session = input.policy && input.policy.workspaceMode === 'chat'
      ? await service.openChatSession(input) : await service.createSession(input);

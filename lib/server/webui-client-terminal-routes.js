...
async function handleWebUiClientTerminalRoutes(req, res, method, pathname, ctx = {}) {
  if (!['/v0/webui/terminals', '/v0/webui/toolkit/terminals'].includes(pathname)
    ...
    && pathname !== '/v0/webui/toolkit/terminals/open'
  ...
  if (method === 'GET' && (pathname === '/v0/webui/terminals' || pathname === '/v0/webui/toolkit/terminals')) {
    ...
    writeJson(routeCtx, 200, {
      ...
      // CLI 启动的项目目录选择以宿主 home 为默认项，前端需要真实路径做展示与回填。
      homeDir: resolveHostHomeDir({
        hostHomeDir: routeCtx.hostHomeDir || (routeCtx.deps && routeCtx.deps.hostHomeDir) || '',
        env: options.env,
        platform: (options.processObj && options.processObj.platform) || process.platform
      }),
      terminals
    });
    return true;
  ...
  if (pathname === '/v0/webui/toolkit/terminals/open') {
    const result = launchClientTerminal(body.terminalId, {
      ...options,
      title: body.title || 'AI Home 终端'
    });
    writeJson(routeCtx, result.ok ? 200 : errorStatus(result.error), result);
    return true;
  }

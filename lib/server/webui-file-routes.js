...
// provider 工作空间 + 临时目录：会话工具产出物(view_image 渲染图、中间文件)常落在这些空间之外的
// 项目目录,需放开只读授权。凭证类文件由上层硬拦截兜底。缓存以避免每次请求重复计算。
let _extraReadableRootsCache = null;
function resolveExtraReadableRoots() {
  if (_extraReadableRootsCache) return _extraReadableRootsCache;
  const os = require('node:os');
  const home = typeof os.homedir === 'function' ? os.homedir() : (process.env.HOME || process.env.USERPROFILE || '');
  const roots = [];
  if (home) {
    ['.claude', '.codex', '.gemini', '.opencode', '.antigravity', '.qwen', '.aih', '.ai_home']
      .forEach((dir) => roots.push(path.resolve(home, dir)));
  }
  const tmp = typeof os.tmpdir === 'function' ? os.tmpdir() : '';
  ['/tmp', '/private/tmp', tmp].forEach((entry) => {
    const resolved = String(entry || '').trim();
    if (resolved) roots.push(path.resolve(resolved));
  });
  _extraReadableRootsCache = Array.from(new Set(roots.filter(Boolean)));
  return _extraReadableRootsCache;
}
...
function resolveAuthorizedFileRequest(ctx, options = {}) {
  ...
  // 【授权扩展】provider 工作空间(~/.claude ~/.codex ~/.gemini 等)与临时目录(/tmp、系统 tmp)：
  // agent 工具产出的图片/中间文件(如 view_image 的渲染图)常落在这些空间,会话里要能预览。这些根
  // 放开只读授权;凭证类文件(.ssh/auth.json/oauth_creds/credentials.json/id_rsa)仍被上方硬拦截兜底。
  if (!isAuthorized) {
    for (const root of resolveExtraReadableRoots()) {
      if (root && (targetPath === root || isSubPath(root, targetPath))) {
        isAuthorized = true;
        break;
      }
    }

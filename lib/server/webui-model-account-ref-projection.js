 ...
 */
function mergePersistedAccountRefs(
  target,
  errorsTarget,
  byAccount,
  errorsByAccount,
  ...
  const scopedAccountRef = normalizeAccountScope(accountScope)?.accountRef || '';
  (Array.isArray(accountRefs) ? accountRefs : []).forEach((value) => {
    const accountRef = String(value || '').trim();
    if (!accountRef) return;
    // 账号详情页只能看到自己的缓存结果。extraAccountRefs 来自全量账号摘要，
    // 若这里不再次应用 scope，其他账号的持久化探测错误会被合并进当前响应。
    if (scopedAccountRef && accountRef !== scopedAccountRef) return;

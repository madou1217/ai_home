...
const { listProvidersByCapability, listProviderIds } = require('../provider-catalog');
...
// dirty 触发的 snapshot 重建合并窗口:上千会话的同步重建较重,原生会话持续写文件会频繁置 dirty,
// 这里把 dirty 重建合并到至多每 3s 一次,兼顾及时性与事件循环负载。
const PROJECTS_DIRTY_COALESCE_MS = 3_000;
// 每个项目在 /webui/projects 里最多下发的会话数(最近的优先);其余数量用 sessionTotal 表达。
const SESSION_PAYLOAD_CAP = 40;
...
const DEFAULT_WATCH_PROVIDERS = Object.freeze(listProvidersByCapability('sessionRuntime'));
const ACCOUNT_SCOPED_PROJECT_PROVIDERS = Object.freeze(listProvidersByCapability('accountSessionStore'));
const KNOWN_PROVIDER_IDS = new Set(listProviderIds());
...
function filterHiddenProjects(projects, deps = {}) {
  ...
  return (Array.isArray(projects) ? projects : []).filter((project) => {
    ...
    if (aiHomeDir) {
      const relative = path.relative(path.join(aiHomeDir, 'run', 'chat-workspaces'), normalizedProjectPath);
...
function buildProjectsSnapshot(hostProjects, deps = {}, options = {}) {
  const { fs, aiHomeDir } = resolveCacheDeps(deps);
  const requestedProjectPath = normalizeProjectPath(options.projectPath);
  const projectMap = new Map();
  // 每个已归并项目里已经出现过的会话 id（见下方去重注释）。
  const sessionIdByProject = new Map();
  ...
  for (const project of allProjects) {
    ...
    if (requestedProjectPath && resolvedProjectPath !== requestedProjectPath) continue;
    ...
    // 会话 id 在一个项目内就是会话身份，同一 id 只应出现一次。
    //
    // CodeBuddy 家族的国内/国际站各自是「两个产品共用一份会话存储」（WorkBuddy +
    // CodeBuddy 跑同一套 runtime，见 lib/sessions/session-reader-codebuddy.js），
    // 因此两个 Provider 会读出同一批会话。这里按 id 去重后只保留一份：Provider 顺序
    // 固定（codebuddy → codebuddycn → workbuddy → workbuddycn），胜出的总是该地区里
    // 自带 CLI 的那个 Provider，于是"看得见"与"续得上"同时成立，也不会因为两个
    // Provider 各报一次而在列表里出现重复会话。
    let seenSessionIds = sessionIdByProject.get(key);
    if (!seenSessionIds) {
      seenSessionIds = new Set();
      sessionIdByProject.set(key, seenSessionIds);
    }
    const sessions = Array.isArray(project && project.sessions) ? project.sessions : [];
  ...
  for (const opened of openedProjects) {
    ...
    if (requestedProjectPath && key !== requestedProjectPath) continue;
    if (projectMap.has(key)) {
      const existing = projectMap.get(key);
  ...
  return sortProjectsByLastActivityDesc(filterHiddenProjects(Array.from(projectMap.values()), { fs, aiHomeDir })
    .map((project) => {
      // 按 updatedAt 降序后截断到 SESSION_PAYLOAD_CAP:每次只下发最近的若干会话(运行中=近期活跃,
      // 自然落在截断集内),其余用 sessionTotal 表达数量、前端按需再拉。避免单个项目(可达数百会话)
      // 把 /webui/projects 撑到 MB 级、拖慢 wire+浏览器解析。
      const sorted = sortProjectSessionsByUpdatedAtDesc(project.sessions);
      const sessionTotal = sorted.length;
      return {
        ...project,
        sessionTotal,
        sessions: options.includeAllSessions || sessionTotal <= SESSION_PAYLOAD_CAP
          ? sorted
          : sorted.slice(0, SESSION_PAYLOAD_CAP)
      };
    }));
}
...
function collectHostProjectsWatchTargets(ctx) {
  ...
  const nestedDeps = ctx && typeof ctx.deps === 'object' && ctx.deps ? ctx.deps : {};
  const hostHome = String(
    ctx && ctx.hostHomeDir
    || nestedDeps.hostHomeDir
    || getRealHome()
    || ''
  ).trim();
  ...
  const aiHomeDir = String(ctx && ctx.aiHomeDir || nestedDeps.aiHomeDir || '').trim();
  const accounts = ctx && ctx.state && ctx.state.accounts || {};
  const accountWatchSpecs = {
    qoder: { relativePath: ['projects'], depth: 1 },
    qodercn: { relativePath: ['projects'], depth: 1 },
    grok: { relativePath: ['.grok', 'sessions'], depth: 3 },
    kiro: { relativePath: [], depth: 0 }
  };

  if (aiHomeDir) {
    for (const [provider, spec] of Object.entries(accountWatchSpecs)) {
      for (const account of Array.isArray(accounts[provider]) ? accounts[provider] : []) {
        const accountRef = String(account && account.accountRef || '').trim();
        if (!accountRef) continue;
        const runtimeDir = path.join(aiHomeDir, 'run', 'auth-projections', provider, accountRef);
        const sessionStorePath = path.join(runtimeDir, ...spec.relativePath);
        collectDirectoryWatchTargets(fs, sessionStorePath, spec.depth, targets);
      }
    }
  }
...
function inferProvidersFromWatchPath(hostHome, targetPath) {
  ...
  if (!normalizedHostHome || !normalizedTargetPath) {
    return DEFAULT_WATCH_PROVIDERS.slice();
  }

  const normalizedSegments = path.normalize(normalizedTargetPath).split(path.sep);
  const authProjectionsIndex = normalizedSegments.lastIndexOf('auth-projections');
  if (authProjectionsIndex >= 0) {
    const provider = String(normalizedSegments[authProjectionsIndex + 1] || '').trim().toLowerCase();
  ...
  // zcode 主会话落在宿主 ~/.zcode/cli/db/db.sqlite。
  if (relativePath === '.zcode' || relativePath.startsWith(`.zcode${path.sep}`)) {
    return ['zcode'];
  }
  // agy（antigravity）会话存于 .gemini/antigravity-cli|antigravity，必须在通用 .gemini 判断之前
  // 识别成 agy，否则被误判为 gemini。
  if (relativePath.startsWith(`.gemini${path.sep}antigravity-cli`)
    || relativePath.startsWith(`.gemini${path.sep}antigravity${path.sep}`)) {
    return ['agy'];
  }
...
async function refreshHostProjectsIndex(ctx, options = {}) {
  ...
  return runSingleFlightRefresh(cacheState, async () => {
    ...
    const nestedDeps = ctx && typeof ctx.deps === 'object' && ctx.deps ? ctx.deps : {};
    const readAllProjectsFromHost = ctx.readAllProjectsFromHost
      || nestedDeps.readAllProjectsFromHost
      || sessionReader.readAllProjectsFromHost;
    const readProjectsFromHostByProviders = ctx.readProjectsFromHostByProviders
      || nestedDeps.readProjectsFromHostByProviders
      || sessionReader.readProjectsFromHostByProviders;
    const readProviderProjects = (providers, readOptions = {}) => {
      const requested = Array.isArray(providers) ? providers : [];
      const accountScopedProviders = new Set(ACCOUNT_SCOPED_PROJECT_PROVIDERS);
      const regularProviders = requested.filter((provider) => !accountScopedProviders.has(provider));
      const projects = regularProviders.length
        ? readProjectsFromHostByProviders(regularProviders, readOptions)
        : [];
      const accounts = ctx && ctx.state && ctx.state.accounts || {};
      for (const provider of requested.filter((item) => accountScopedProviders.has(item))) {
        for (const account of Array.isArray(accounts[provider]) ? accounts[provider] : []) {
          const accountRef = String(account && account.accountRef || '').trim();
          if (!accountRef) continue;
          projects.push(...readProjectsFromHostByProviders([provider], {
            ...readOptions,
            accountRef,
            aiHomeDir: ctx.aiHomeDir || nestedDeps.aiHomeDir,
            hostHomeDir: ctx.hostHomeDir || nestedDeps.hostHomeDir
          }));
        }
      }
      return projects;
    ...
    if (canUseIncrementalRefresh && dirtyProviders.length > 0 && dirtyProviders.length < 3) {
      hostProjects = mergeHostProjectsByProvider(
        cacheState.snapshot,
        readProviderProjects(dirtyProviders),
        dirtyProviders
      );
    }
    ...
    if (canUseIncrementalRefresh && hintedProviders.length > 0) {
      ...
      const nextProjects = readProviderProjects(hintedProviders, {
    ...
    if (!hostProjects) {
      hostProjects = [
        ...
        ...readProviderProjects(ACCOUNT_SCOPED_PROJECT_PROVIDERS)
      ];
    }
...
async function ensureHostProjectsIndexCurrent(ctx, options = {}) {
  const cacheState = ensureHostProjectsIndexLoaded(ctx);
...
async function getHostProjectsIndex(ctx, options = {}) {
  const cacheState = await ensureHostProjectsIndexCurrent(ctx, options);
...
async function getCompleteProjectSnapshot(ctx, projectPath) {
  const normalizedProjectPath = normalizeProjectPath(projectPath);
  if (!normalizedProjectPath) return null;

  const cacheState = await ensureHostProjectsIndexCurrent(ctx);
  const projects = buildProjectsSnapshot(cacheState.snapshot, ctx, {
...
function shouldRefreshProjectsSnapshot(cacheState, options = {}) {
  if (options.forceRefresh) return true;
  if (!cacheState.updatedAt) return true;
  const sinceRefresh = Date.now() - Number(cacheState.lastRefreshAt || 0);
  // dirty 触发的重建做节流合并:snapshot 重建(buildProjectsSnapshot,上千会话)是同步重活,
  // 原生会话持续写文件会把 dirty 几乎每 tick 置真。若每个 dirty 都重建,事件循环被反复占满。
  // 合并到 DIRTY_COALESCE_MS 窗口内至多一次;真正的强制刷新走 forceRefresh。
  if (cacheState.dirty && sinceRefresh >= PROJECTS_DIRTY_COALESCE_MS) return true;
  return sinceRefresh >= PROJECTS_SNAPSHOT_TTL_MS;
}
...
function ensureProjectsSnapshotScheduler(ctx) {
  ...
  ensurePeriodicRefresh(cacheState, {
    shouldRefresh: shouldRefreshProjectsSnapshot,
    refresh: async () => {
      const result = await refreshProjectsSnapshot(ctx);
      syncHostProjectsWatchers(ctx);
      return result;
    },
...
function closeProjectsSnapshotScheduler(ctx) {
  if (!ctx || !ctx.state) return;
  clearCacheRefreshTimers(getProjectsCacheState(ctx.state));
  clearCacheRefreshTimers(getHostProjectsIndexState(ctx.state));
  resetHostProjectsWatchState(getHostProjectsWatchState(ctx.state));
}

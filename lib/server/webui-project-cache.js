...
async function refreshHostProjectsIndex(ctx, options = {}) {
  ...
  return runSingleFlightRefresh(cacheState, async () => {
    ...
    const readProviderProjects = (providers, readOptions = {}) => {
      ...
      const regularProviders = requested.filter((provider) => provider !== 'qoder' && provider !== 'qodercn');
      ...
      for (const provider of requested.filter((item) => item === 'qoder' || item === 'qodercn')) {
    ...
    if (!hostProjects) {
      hostProjects = [
        ...
        ...readProviderProjects(['qoder', 'qodercn'])
      ];
    }

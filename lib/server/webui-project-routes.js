...
async function handleGetProjectsRequest(ctx) {
  ...
  try {
    ...
    const projects = Array.from(projectMap.values())
      ...
      .sort((left, right) => {
        const leftUpdatedAt = left.sessions[0] ? Number(left.sessions[0].updatedAt) || 0 : Number(left.addedAt) || 0;
        const rightUpdatedAt = right.sessions[0] ? Number(right.sessions[0].updatedAt) || 0 : Number(right.addedAt) || 0;

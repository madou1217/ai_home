...
const { buildModelMetadataMap, getModelMetadataDeps } = require('./models-dev-metadata');
const {
...
async function handleGetModelsRequest(ctx) {
  ...
  try {
    ...
    const allUniqueModelIds = Array.from(new Set(
      Object.values(selectableByAccountRef).flat()
    )).map((id) => ({ id }));
    const metadata = buildModelMetadataMap(allUniqueModelIds, metadataDeps);

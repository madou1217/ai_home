...
function getProjectsWatchState(state) {
  if (!state.__webUiProjectsWatch) {
    state.__webUiProjectsWatch = {
      ...
      // 会话身份(provider:sessionId)→ hook 权威到期时间。收到 start/stop/progress 生命周期事件即
      // 认定该会话运行态由 hook 事件权威决定;粗粒度的「tmux 已 attach」扫描(persistentRunningKeys)
      // 在窗口内不得把它强行点亮,从而让 turn-completed 的熄灭立即生效、而非等下一次 1s 轮询兜底。
      hookAuthorityUntilByIdentity: new Map(),
...
function collectHookAuthorityIdentities(watchState, now = Date.now()) {
  const identities = new Set();
  const authority = watchState.hookAuthorityUntilByIdentity;
  if (!authority || typeof authority.entries !== 'function') return identities;
  for (const [identity, until] of [...authority.entries()]) {
    if (Number(until) > now) {
      identities.add(identity);
      continue;
    }
    authority.delete(identity);
  }
  return identities;
}
...
function collectVisibleRunningKeys(watchState, now = Date.now()) {
  ...
  const authoritativeIdentities = collectHookAuthorityIdentities(watchState, now);
  for (const key of watchState.persistentRunningKeys || []) {
    if (!key) continue;
    const { provider, sessionId } = parseRuntimeKey(key);
    const identity = buildSessionIdentity(provider, sessionId);
    // 该会话已有活的 hook 权威 → 运行态以 hook 驱动的 runningUntilByKey 为准(上一步已并入),
    // 不让「tmux attached」的粗粒度扫描把已熄灭的 dot 重新点亮。
    if (identity && authoritativeIdentities.has(identity)) continue;
    nextRunningKeys.add(key);
  }
  return nextRunningKeys;
}
...
function applyProjectRuntimeEvent(watchState, event = {}) {
  ...
  const identity = buildSessionIdentity(
    String(event.provider || '').trim().toLowerCase(),
  ...
  if (isRuntimeStopEvent(event)) {
    ...
    // 认领 hook 权威并保留到期窗口:turn-completed 后即便 tmux pane 仍 attached,
    // persistentRunningKeys 也不得在下次轮询前把 dot 重新点亮 → 熄灭立即生效。
    if (identity) watchState.hookAuthorityUntilByIdentity.set(identity, now + PROJECT_RUNTIME_STALE_MS);
  ...
  if (isRuntimeProgressEvent(event)) {
    ...
    // 进度事件只在「已有权威」时续期,不新建权威:避免漏收 start 时把一个正在运行但
    // runningUntilByKey 尚无键的会话误压制成熄灭。
    if (identity && watchState.hookAuthorityUntilByIdentity.has(identity)) {
      watchState.hookAuthorityUntilByIdentity.set(identity, now + PROJECT_RUNTIME_STALE_MS);
    }
    return emitRuntimeIfChanged(watchState);
  }
...
function stopProjectsWatchPoller(ctx, watchState) {
  ...
  watchState.hookAuthorityUntilByIdentity.clear();
  watchState.persistentRunningKeys = new Set();

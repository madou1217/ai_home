...
const PROVIDER_SESSION_SYNC_CAPABILITIES = Object.freeze({
  ...
  grok: Object.freeze({
    sourceHook: Object.freeze({
      ...
      configTarget: 'hooks.json',
      defaultEvents: Object.freeze(['SessionStart', 'UserPromptSubmit', 'Stop', 'StopFailure', 'SessionEnd']),
      note: 'Verified against docs.x.ai/build/features/hooks for the official x.ai Grok CLI (the same binary AI Home installs via x.ai/cli/install.sh) — not the unrelated superagent-ai/grok-cli community project.'
    }),
    sink: Object.freeze({
      available: false,
      kind: '',
      fallback: 'session-event-bus-watch-reader',
      note: 'No confirmed local Grok client refresh sink; not installed on this dev machine so end-to-end firing is unverified — treat as structural conformance to docs, not an observed-working integration.'
    }),
    flows: Object.freeze([
      buildSyncFlow(
        'cli-to-web',
        'available',
        'official-hook -> session-event-bus -> web-sse -> transcript-read',
        'CLI updates notify Web when Grok hooks are installed; Web still reads the real session transcript.'
      ),
      buildSyncFlow(
        'web-to-native',
        'fallback',
        'session-event-bus/watch-reader',
        'No confirmed Grok local client refresh sink is used.'
      ),
      buildSyncFlow(
        'native-to-web',
        'available',
        'official-hook -> session-event-bus -> web-sse -> transcript-read',
        'Native Grok CLI sessions can dirty Web through official hooks once installed.'
      )
    ])
  }),
  opencode: Object.freeze({
    sourceHook: Object.freeze({
      available: true,
      kind: 'plugin-bridge',
      configTarget: 'plugin.js',
      defaultEvents: Object.freeze(['UserPromptSubmit', 'Stop'])
    }),
    sink: Object.freeze({
      available: false,
      kind: '',
      fallback: 'session-event-bus-watch-reader',
      note: 'No confirmed OpenCode TUI/App refresh sink equivalent to Codex app-server notifications.'
    }),
    flows: Object.freeze([
      buildSyncFlow(
        'cli-to-web',
        'available',
        'plugin.js -> session-event-bus -> web-sse -> session-read',
        'OpenCode’s global plugin (run/serve/TUI all load it) notifies Web when installed; Web still reads the real session.'
      ),
      buildSyncFlow(
        'web-to-native',
        'fallback',
        'session-event-bus/watch-reader',
        'No confirmed OpenCode native refresh sink is used.'
      ),
      buildSyncFlow(
        'native-to-web',
        'available',
        'plugin.js -> session-event-bus -> web-sse -> session-read',
        'Native OpenCode sessions can dirty Web through the installed plugin.'
      )
    ])
  })
});

// 兜底:PROVIDER_SESSION_SYNC_CAPABILITIES 里没有手写条目的 provider(目前 grok/qoder/qodercn/
// kiro/kimi)——诚实地按 syncMode 派生,而不是假装它们完全没有能力。这些 provider 的官方 hook 尚未
// 在本仓库接入验证,所以只描述已确认的部分(hook 是否已知支持、有无轮询兜底),不编造 sink/flows 细节。
function getProviderSessionSyncCapabilities(provider) {
  const known = PROVIDER_SESSION_SYNC_CAPABILITIES[normalizeText(provider).toLowerCase()];
  if (known) return known;
  const syncMode = getProviderSessionSyncMode(provider);
  const pollingAvailable = syncMode === 'polling';
  return {
    sourceHook: { available: false, kind: '', note: 'No official session hook is wired for this provider yet.' },
    sink: { available: false, kind: '' },
    flows: [
      buildSyncFlow(
        'cli-to-web',
        pollingAvailable ? 'fallback' : 'unavailable',
        pollingAvailable ? 'session-event-bus file-watch/poll (500ms)' : 'none',
        pollingAvailable
          ? 'No official hook installed for this provider; Web still detects session changes via periodic file polling (not instant).'
          : 'No session file reader is wired for this provider; Web cannot detect session changes at all.'
      )
    ]
  };
}

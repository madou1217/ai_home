...
const { parseSshTarget, probeSshTarget } = require('../cli/services/node/bootstrap-probe');
...
async function handleWebUiSshHostRoutes(ctx) {
  ...
  const { fs: fsImpl, readRequestBody, writeJson, spawnImpl } = deps;
  ...
  if (method === 'POST' && pathname === '/v0/webui/ssh-connections/test') {
    ...
    try {
      ...
      // 如果提供了 connectionId，则自动从数据库补全所有连接和凭据属性
      ...
      if (connectionId) {
        ...
        if (saved) {
          ...
          if (body.port === undefined) port = saved.port || 22;
          if (!user) user = saved.user || '';
          if (body.authType === undefined) authType = saved.authType || 'agent';
          if (password === PASSWORD_MASK || !password) password = saved.password || '';
          if (privateKey === PASSWORD_MASK || !privateKey) privateKey = saved.privateKey || '';
        }
      }

...
const { detectNetworkLayer } = require('../cli/services/toolkit/network-layer-detector');
const {
  ToolkitConfigError,
...
async function handleWebUiToolkitRoutes(req, res, method, pathname, ctx = {}) {
  ...
  if (method === 'POST' && pathname === '/v0/webui/toolkit/apps/install') {
    try {
      ...
      if (!started.ok) {
        jsonResponse(res, started.error === 'confirmation_required' ? 428 : 400, {
          ok: false,
          error: started.error || 'app_install_unavailable'
        });
        return true;
      }

...
async function handleWebUiToolkitRoutes(req, res, method, pathname, ctx = {}) {
  ...
  // POST /v0/webui/toolkit/apps/:appId/open - open an installed Desktop client
  ...
  if (method === 'POST' && openAppMatch) {
    ...
    try {
      const result = openManagedDesktopApp(appId, buildToolkitOptions(ctx));
      if (result.ok) {
        jsonResponse(res, 200, result);
        return true;
      }
      if (result.error === 'desktop_not_installed') {
        const manager = getAppInstallJobManager(ctx);
        const installAvailable = Boolean(manager && typeof manager.canInstall === 'function'
          && manager.canInstall({ appId, kind: 'desktop', action: 'install' }));
        jsonResponse(res, 428, {
          ...result,
          installRequired: true,
          installAvailable,
          message: installAvailable
            ? '未检测到 Desktop 应用，请先安装后重试。'
            : '当前平台没有可用的自动安装器，请手动安装官方 Desktop 应用后重试。'
        });
        return true;
      }
      jsonResponse(res, result.error === 'unsupported_app' ? 400 : 500, result);
    } catch (error) {

...
const { createWebUiAccountAppLauncher } = require('./webui-account-routes');
...
async function handleWebUiToolkitRoutes(req, res, method, pathname, ctx = {}) {
  ...
  if (method === 'POST' && openAppMatch) {
    ...
    try {
      ...
      const result = selectedAccountRef || kind === 'cli'
        ? createWebUiAccountAppLauncher(toolkitOptions, provider, selectedAccountRef, action)
          .launchAccountApp({ provider, accountRef: selectedAccountRef, kind, action, terminalId: body.terminalId })
        : openManagedDesktopApp(appId, toolkitOptions);
      if (result.ok) {
      ...
      const status = result.error === 'account_not_found' ? 404
        : (result.error === 'unsupported_app' || result.error === 'unsupported_kind' || result.error === 'unsupported_provider' ? 400 : 409);
  ...
  if (method === 'POST' && pathname === '/v0/webui/toolkit/apps/install') {
    try {
      ...
      if (body.action) installInput.action = body.action;
  ...
  if (method === 'GET' && pathname === '/v0/webui/toolkit/environments') {
    try {
      const data = getEnvironmentsSummary(buildToolkitOptions(ctx));
      jsonResponse(res, 200, data);
    } catch (e) {
      jsonResponse(res, 500, { ok: false, error: e.message });
    }
  ...
  if (method === 'POST' && pathname === '/v0/webui/toolkit/environments/plan') {
    try {
      const body = await parseJsonBody(req);
      const result = planEnvironmentAction(body, buildToolkitOptions(ctx));
  ...
  if (method === 'POST' && pathname === '/v0/webui/toolkit/environments/execute') {
    try {
      const body = await parseJsonBody(req);
      const result = await executeEnvironmentAction(body, buildToolkitOptions(ctx));
      jsonResponse(res, serviceResultStatus(result), result);
    } catch (error) {
      sendToolkitError(res, error);
    }

...
async function handleWebUiZcodeCaptchaRoutes(ctx) {
  ...
  if (method === 'POST' && pathname.startsWith(CHALLENGE_PATH_PREFIX)) {
    ...
    if (action === 'complete') {
      ...
      // 透传求解浏览器的身份头，供重发时与桌面端「求解环境与请求环境共享应用
      // 身份」的行为保持同形。实测更正：405 {"code":3012} 是账号级 WAF/业务限制，
      // 并非 UA 指纹不符；此处对齐仅为行为一致性，不是 3012 的修复。
      ...
      const result = bridge.complete(id, {
        ...
        secChUa: reqHeaders['sec-ch-ua'],
        secChUaPlatform: reqHeaders['sec-ch-ua-platform'],
        secChUaMobile: reqHeaders['sec-ch-ua-mobile'],

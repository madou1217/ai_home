...
// bigmodel 实测 budget 边界宽松（budget == max_tokens 也可）；但 Anthropic 契约
// 要求 max_tokens > budget_tokens，且 api.z.ai 等兼容端点未必同样宽容——budget
// 必须严格夹在 max_tokens 内，不设下限（max_tokens=16 → budget=16）。
function resolveThinkingBudgetTokens(maxTokens, fallback = DEFAULT_THINKING_BUDGET_TOKENS) {
  ...
  return Math.max(1, Math.min(fallback, Math.round(cap)));
}

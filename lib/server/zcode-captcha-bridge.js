  ...
  function complete(id, input = {}) {
    ...
    settleChallenge(
      challenge,
      {
        ...
        verifyParam,
        region,
        // 求解浏览器的身份头，供重发时对齐验证方指纹（见 webui-zcode-captcha-routes.js）。
        userAgent: String(input.userAgent || '').trim(),
        secChUa: String(input.secChUa || '').trim(),
        secChUaPlatform: String(input.secChUaPlatform || '').trim(),
        secChUaMobile: String(input.secChUaMobile || '').trim(),

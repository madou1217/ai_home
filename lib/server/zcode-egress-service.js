...
const EGRESS_SUPPORTED_PROVIDERS = Object.freeze(['zcode']);
...
function describeAlreadyRunningWarning({ raced = false } = {}) {
  return raced
    ? '启动期间已有实例抢先运行，本次预备出口未被该实例加载；请在出口设置中重新实时应用'
    : 'ZCode 当前实例已运行；出口变更请在出口设置中实时应用';
}
...
function getAccountEgressRuntimeStatus(input = {}) {
  ...
  const zcodePid = resolveRunningZcodePid(input, lease?.pid);
  ...
  const hasLiveZcodeProcess = Number.isInteger(zcodePid) && zcodePid > 0;
  return {
    ok: true,
    ...
    runtime: {
      ...
      selectedNodeId,
      groupId,
      zcodePid: hasLiveZcodeProcess ? zcodePid : null,
      canRotate: Boolean(
...
async function applyStoredAccountEgressOnce(input = {}) {
  ...
  if (!active && !persisted) {
    untrackAccountHealth(input);
    if (input.launcher && typeof input.launcher.launchAccountApp === 'function') {
      return takeoverRunningDesktopWithStoredEgress(input);
    }
  ...
  if (binding) {
    ...
    const egress = resolved?.ok ? attachRunningZcodeLease(input, resolved) : resolved;
...
async function takeoverRunningDesktopWithStoredEgressOnce(input = {}) {
  ...
  try {
    ...
    return desktopTakeoverFailure({ reason: error?.message }, 'zcode_desktop_probe_failed');
  }
  if (preflight?.ok && preflight.status === APP_STATUS_LAUNCH_READY) {
    return { ok: true, applied: false, status: 'pending_launch' };
  }
  if (!preflight?.ok || preflight.status !== 'already_running') {
    return desktopTakeoverFailure(preflight, 'zcode_desktop_probe_failed');
  }
  ...
  if (!preparation.ok) {
    return {
      ok: false,
      applied: false,
      error: String(preparation.error || ZCODE_EGRESS_UNAVAILABLE),
  ...
  try {
    ...
    closed = { ok: false, error: 'zcode_desktop_close_failed', reason: error?.message };
  }
  if (!closed?.ok || !['closed', 'not_running'].includes(String(closed.status || ''))) {
    await cleanupTakeoverEgress(input, egress);
    return desktopTakeoverFailure(closed, 'zcode_desktop_close_failed');
  }
...
async function prepareAccountAppEgress(input = {}) {
  ...
  try {
    ...
    return {
      ok: false,
      error: ZCODE_EGRESS_BINDING_UNAVAILABLE,
      egress: null,
      egressPrepared: false,
      egressError: failure.error,
      reason: failure.reason,
      warning: describeEgressWarning(failure)
    };
  }
  if (!egress) {
    return {
...
async function launchAccountAppWithEgressOnce(input = {}) {
  ...
  if (!egressPreparation.ok) {
    return {
      result: {
        ok: false,
        error: String(egressPreparation.error || ZCODE_EGRESS_UNAVAILABLE),
  ...
  if (result?.ok && result.status === 'already_running') {
    egressWarning = mergeWarnings(egressWarning, describeAlreadyRunningWarning({ raced: true }));
  }
  return { result, egressWarning };
}
...
function describeEgressWarning(egress) {
  ...
  if (egress.preserveExisting) {
    return `${message}${reason}；为避免绕过账号绑定，本次保留现有 ZCode 原生设置并阻止启动`;
  }

...
const { listAccountRefRecords, resolveAccountRef } = require('./account-ref-store');
...
async function restorePersistedZcodeEgress(input = {}) {
  ...
  // state.json 记录已经建立过的固定端口；app_kv 绑定才是账号是否要求代理的
  // 权威事实。两者取并集：既恢复历史 endpoint，也覆盖数据库已恢复/同步但
  // runtime state 缺失的绑定账号。
  const accountRefs = new Set(Object.keys(state?.accounts || {}).filter(Boolean));
  let accountRecords = [];
  try {
    const listAccounts = typeof deps.listAccountRefRecords === 'function'
      ? deps.listAccountRefRecords
      : listAccountRefRecords;
    accountRecords = listAccounts(input.fs, input.aiHomeDir, '', { bestEffort: true }) || [];
  } catch {}
  const providersByAccountRef = new Map();
  for (const account of accountRecords) {
    const accountRef = String(account?.accountRef || '').trim();
    const provider = String(account?.provider || '').trim().toLowerCase();
    if (!accountRef || !isEgressSupportedProvider(provider)) continue;
    providersByAccountRef.set(accountRef, provider);
    try {
      if (readStoredBinding({ ...input, accountRef }, deps)) accountRefs.add(accountRef);
    } catch {
      // 损坏绑定仍进入恢复结果，由 applyStoredAccountEgress 产出可观察失败，
      // 不能把未知状态静默当成“未绑定”。
  ...
  for (const accountRef of accountRefs) {
    let result;
    let provider = providersByAccountRef.get(accountRef) || '';
    if (!provider) {
      try {

...
// 按显式 db 文件路径打开（不走 hostHome→.local/share/opencode 拼接）。
// 用于扫描 opencode 账号桥接目录下的 db（aih-opencode-runtime/opencode/opencode.db），
// 这类路径不符合共享库的目录布局，无法用 openOpenCodeDb(hostHome) 定位。
function openOpenCodeDbAtPath(dbPath, options = {}) {
  const DatabaseSync = getDatabaseSyncCtor();
  if (!DatabaseSync) {
    const error = new Error('node_sqlite_unavailable');
    error.code = 'node_sqlite_unavailable';
  ...
  const normalized = normalizeString(dbPath);
  if (!normalized) {
    const error = new Error('opencode_db_path_unavailable');
    error.code = 'opencode_db_path_unavailable';
    throw error;
  }
  if (!options.create && !fs.existsSync(normalized)) {
    const error = new Error('opencode_db_missing');
    error.code = 'opencode_db_missing';
    throw error;
  }
  return new DatabaseSync(normalized, { readOnly: Boolean(options.readOnly) });
}

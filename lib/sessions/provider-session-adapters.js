...
function pushRenderedMessage(messages, options) {
  const {
    role,
    blocks,
    timestamp,
  ...
  if (compactText(model)) message.model = compactText(model);
...
function readGeminiJsonlSessionMessages(filePath) {
  ...
  for (const record of readJsonlRecords(filePath)) {
    ...
    if (record.type === 'gemini') {
      pushRenderedMessage(messages, {
        role: 'assistant',
        blocks: geminiRecordToBlocks(record),
...
function extractAgySelectedModel(content) {
  const text = String(content || '');
  const settingsBlock = text.match(/<USER_SETTINGS_CHANGE>\s*([\s\S]*?)\s*<\/USER_SETTINGS_CHANGE>/i);
  const settingsText = settingsBlock && settingsBlock[1] ? settingsBlock[1] : text;
  const match = settingsText.match(
    /changed setting `Model Selection` from [\s\S]*? to ([\s\S]*?)(?:\.\s+No need to comment|\.\s+If reporting|$)/i
  );
  return compactText(match && match[1] ? match[1] : '');
}

// antigravity 会把系统通知（如 server 重启导致 subagent 停止）包装成 USER_INPUT 注入
// transcript，正文形如 “The following is a <SYSTEM_MESSAGE> not actually sent by the
// user. ... <SYSTEM_MESSAGE> [Message] timestamp=... content=... </SYSTEM_MESSAGE>”。
// 这类记录绝不能按用户消息渲染；提取内嵌 content 作为系统事件展示。
// agy 框架内部元通知——非用户输入、非模型回复,混进聊天气泡是噪声(用户会误以为是回答的一部分)。
// 典型:`<SYSTEM_MESSAGE> ... not actually sent by the user` 包裹,或
// "[Notice] All your subagents and background tasks have been stopped due to server restart..."。
function isAgyInternalSystemNotice(text) {
  const t = String(text || '');
  if (!t) return false;
  if (/not actually sent by the user/i.test(t)) return true;
  if (/^\s*\[Notice\]/i.test(t)) return true;
  if (/subagents?[\s\S]{0,60}(stopped|revived|background task)/i.test(t)) return true;
  return false;
}

function extractAgySystemMessage(content) {
  const text = String(content || '');
  if (!/<SYSTEM_MESSAGE>/i.test(text)) return '';
  if (!/not actually sent by the user/i.test(text)) return '';
  const block = text.match(/<SYSTEM_MESSAGE>\s*([\s\S]*?)\s*(?:<\/SYSTEM_MESSAGE>|$)/i);
  const body = block && block[1] ? block[1] : text;
  const contentMatch = body.match(/\bcontent=([\s\S]+)$/);
  return compactText(contentMatch ? contentMatch[1] : body);
}
...
function readAgySessionMessagesFromFile(filePath) {
  ...
  for (const record of readJsonlRecords(filePath)) {
    if (record.type === 'USER_INPUT') {
      const systemMessage = extractAgySystemMessage(record.content);
      if (systemMessage) {
        // agy 内部系统注入(`<SYSTEM_MESSAGE> ... not actually sent by the user`)——如
        // 「subagents/后台任务因 server restart 停止」之类的框架元信息,既非用户输入也非模型回复,
        // 混进聊天气泡是噪声(用户会误以为是回答的一部分)。直接跳过不渲染。
        continue;
      }
      flushAssistant();
      const selectedModel = extractAgySelectedModel(record.content);
      if (selectedModel) activeModel = selectedModel;
      const content = extractAgyUserContent(record.content);
      if (content) {
        const message = { role: 'user', content, timestamp: record.created_at };
    ...
    if (!assistantTimestamp && record.created_at) {
      assistantTimestamp = record.created_at;
    ...
    if (record.type === 'CHECKPOINT') {
      // antigravity 内部 planner checkpoint 标记（"{{ CHECKPOINT N }}" 等），对用户是纯噪声，
      // 不作为消息内容渲染——否则会污染会话回复与历史（尾部冒出 <checkpoint> 段）。
      continue;
    }
    ...
    if ((record.type === 'GENERIC' || record.type === 'SYSTEM_MESSAGE') && compactText(record.content)) {
      // SYSTEM_MESSAGE 原文带 “not actually sent by the user …” 包裹与
      // timestamp/sender/priority 元数据——剥出 content= 正文，读者只关心通知本身。
      const noticeText = extractAgySystemMessage(record.content) || record.content;
      // agy 框架内部元通知(subagents/server restart 之类)是噪声,不渲染进聊天。
      if (!isAgyInternalSystemNotice(noticeText)) {
        assistantBlocks.push({
          type: 'system_event',

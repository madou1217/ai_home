 ...
 * 账号侧也不需要持有会话副本：`projects` 是声明的共享条目，session-store 把沙箱里的
 * `projects` 做成指向宿主**地区存储**的软链接（一份物理数据），而且对 CodeBuddy 家族
 * **绝不迁移/复制**投影里已有的会话文件（见 session-store.js 的
 * SESSION_STORE_PRODUCT_LEVEL_PROVIDERS）。因此读宿主地区根就是读全部，
 * 与选中哪个账号无关——这正是"切换账号会话不受影响"的实现方式。
 */

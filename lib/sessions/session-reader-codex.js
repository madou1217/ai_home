...
// 列表标题只需要首个用户消息。绝不能为了展示标题读取整个 rollout（单文件可达数百 MB）。
const CODEX_SESSION_TITLE_MAX_BYTES = 1024 * 1024;
...
function readCodexSessionTitle(sessionFilePath) {
  ...
  try {
    forEachJsonlLineSync(sessionFilePath, (line) => {
      ...
      maxBytes: CODEX_SESSION_TITLE_MAX_BYTES,
      shouldStop: () => Boolean(title)
    });
  } catch (_error) {
    return '';
  }

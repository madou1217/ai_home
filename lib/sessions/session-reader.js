...
function readClaudeSessionMessages(sessionId, projectDirName) {
  ...
  try {
    const sessionPath = path.join(hostHome, '.claude', 'projects', projectDirName, `${sessionId}.jsonl`);
...
function readGeminiSessionMessages(sessionId, projectDirName) {
  ...
  try {
    // Gemini 会话存储在 ~/.gemini/tmp/{project}/chats/session-*.json
    const tmpDir = path.join(hostHome, '.gemini', 'tmp');
    if (!fs.existsSync(tmpDir)) return messages;

    // 在所有项目的 chats 下查找包含该 sessionId 的文件
    const findSession = () => {
      // 优先在 projectDirName 指定的目录找
      if (projectDirName) {
        const chatsDir = path.join(tmpDir, projectDirName, 'chats');

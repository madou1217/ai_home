...
function readCodexSessionTitle(sessionFilePath) {
  ...
  try {
    forEachJsonlLineSync(sessionFilePath, (line) => {
      ...
      if (
        nextTitle.startsWith('# AGENTS.md instructions')
        || nextTitle.startsWith('<environment_context')
        || nextTitle.startsWith('<user_instructions')
        || nextTitle.startsWith('<user_shell_command')
        || nextTitle.includes('<INSTRUCTIONS>')
      ) return;
    ...
    try {
      ...
      return rows
        .map((row) => {
          ...
          const title = String(nameEntry && nameEntry.thread_name || row && (row.title || row.first_user_message) || '').trim()

...
function readCodexThreadSnapshotFromStateDb(codexDir, options = {}) {
  ...
  for (const stateDbPath of stateDbPaths) {
    ...
    try {
      ...
      const spawnedChildIds = readCodexSpawnedChildIds(db);
  ...
  return createEmptyCodexThreadSnapshot();
}
...
// OpenCode 会话以宿主共享库为唯一来源；账号 runtime 只承载认证 projection。
function collectOpenCodeDbPaths(hostHome) {
  const dbPath = getOpenCodeDbPath(hostHome);
  return dbPath && fs.existsSync(dbPath) ? [dbPath] : [];
}
...
// 会话可能落在共享库或某个 profile 桥接库（见 collectOpenCodeDbPaths / readOpenCodeProjectsFromHostByPaths）。
// 只读共享库会对桥接库里的会话返回空历史（列表可见但打开是空的）。这里遍历所有候选库，返回首个有该
// 会话消息的结果（一个 sessionId 只存在于一个库）。
function readOpenCodeSessionMessages(sessionId, options = {}) {
  const id = String(sessionId || '').trim();
  if (!id) return [];
  let firstReadError = null;
  for (const dbPath of collectOpenCodeDbPaths(getRealHome())) {
    try {
      const messages = readOpenCodeMessagesFromDb(dbPath, id, options);
      if (messages.length > 0) return messages;
    } catch (error) {
      if (!firstReadError) firstReadError = error;
    }
  }
  if (options.throwOnError && firstReadError) throw firstReadError;
  return [];
}
...
function readOpenCodeSessionModel(sessionId) {
  ...
  for (const dbPath of collectOpenCodeDbPaths(getRealHome())) {
    ...
    try {
      ...
      db.exec('PRAGMA query_only = ON;');
      const columns = getSqliteTableColumns(db, 'session');
      if (!columns.has('id') || !columns.has('model')) continue;
...
function readSessionLastModel(provider, params = {}) {
  try {
...
function readSessionMessagesUncached(provider, params = {}, options = {}) {
  ...
  switch (provider) {
    case 'claude':
      return readClaudeSessionMessages(sessionId, projectDirName, options);
    case 'codex':
      return readCodexSessionMessages(sessionId, options);
    case 'gemini':
      return readGeminiSessionMessages(sessionId, projectDirName, options);
    case 'agy':
      return readAgySessionMessages(sessionId, options);
    case 'opencode':
      return readOpenCodeSessionMessages(sessionId, options);
    default:
      return [];
  }
}
...
function readSessionMessagesSnapshot(provider, params = {}) {
  const cursorBeforeRead = provider === 'codex'
    ? null
    : getSessionFileCursor(provider, params);
  const messages = readSessionMessages(provider, params, { throwOnError: true });

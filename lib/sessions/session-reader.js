  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      while ((match = regex.exec(content)) !== null) {
        addKnownPath(match[1]);
      }
    }
  } catch (e) { /* ignore */ }

  // 从 Codex 全局状态收集工作区根路径和排序线索
  try {
    ...
    if (fs.existsSync(globalStatePath)) {
      ...
      rootArrays.forEach((items) => {
        (Array.isArray(items) ? items : []).forEach(addKnownPath);
      });
      const threadWorkspaceHints = state['thread-workspace-root-hints'];
      if (threadWorkspaceHints && typeof threadWorkspaceHints === 'object') {
        Object.values(threadWorkspaceHints).forEach(addKnownPath);
      }
    }
  } catch (e) { /* ignore */ }
  ...
  try {
    ...
    if (fs.existsSync(sessionsDir)) {
      ...
      for (const sessionFile of sessionFiles) {
        ...
        const title = nameEntry?.thread_name || readCodexSessionTitle(sessionFile) || '未命名会话';

        if (title === 'Warmup' || title === '未命名会话') continue;

        const stats = fs.statSync(sessionFile);

        if (!projectSessionMap.has(normalizedCwd)) {
          projectSessionMap.set(normalizedCwd, []);
        }
        projectSessionMap.get(normalizedCwd).push({
          id: sessionId,
          title,

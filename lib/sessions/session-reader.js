...
const { CODEBUDDY_FAMILY_PROVIDERS, readCodebuddyProjects, readCodebuddySessionMessages, resolveCodebuddySessionPath } = require('./session-reader-codebuddy');
...
function readProjectsFromHostByProviders(providers, options = {}) {
  ...
  // CodeBuddy 家族（codebuddy / codebuddycn / workbuddy / workbuddycn）：同一站点的
  // WorkBuddy 与 CodeBuddy 跑的是同一套 runtime，历史按**地区**合并成一份（见
  // session-reader-codebuddy 的站点合并口径）。这里读的是宿主地区目录，不带账号
  // scope——切换选中账号不会改变可见历史。
  for (const provider of CODEBUDDY_FAMILY_PROVIDERS) {
    if (requestedProviders.has(provider)) projects.push(...readCodebuddyProjects(provider, options));
  }
...
function resolveSessionFilePath(provider, params = {}, options = {}) {
  ...
  switch (provider) {
    ...
    // codebuddy 家族：同地区 work/code 共用一份会话存储，入口不同但定位同一文件。
    case 'codebuddy':
    case 'codebuddycn':
    case 'workbuddy':
    case 'workbuddycn':
      return resolveCodebuddySessionPath(provider, sessionId, projectDirName, options);
    default:
      return '';
  }
}
...
function readSessionMessagesUncached(provider, params = {}, options = {}) {
  ...
  switch (provider) {
    ...
    case 'workbuddy':
    case 'workbuddycn':
      messages = readCodebuddySessionMessages(provider, sessionId, projectDirName, options);
      break;
    default:
      messages = [];
      break;

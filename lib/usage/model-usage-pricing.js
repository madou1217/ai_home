...
function normalizePricingRecord(model, value = {}) {
  ...
  const hasDeclaredCost = declaredCostFields.some((key) => (
    Object.prototype.hasOwnProperty.call(value, key)
    && optionalFiniteNumber(value[key]) !== null
  ));
  ...
  if (!hasDeclaredCost && contextCostTiers.length < 1) {
    return null;
  }
  return {
    model: modelId,

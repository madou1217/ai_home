...
function highWaterInteger(value) {
  const number = Number(value);
...
function materializeReadProjectionContext(db, forkDescriptors, attributions) {
  ...
  const insertAttribution = db.prepare(`
    INSERT INTO model_usage_read_attributions (
      scanner_id, proxy_id, model, account_provider, account_ref, cost_usd
    ) VALUES (?, ?, ?, ?, ?, ?)
  `);
  ...
  try {
    ...
    (Array.isArray(attributions) ? attributions : []).forEach((item) => {
      insertAttribution.run(
        ...
        normalizeProvider(item.accountProvider),
        normalizeString(item.accountRef),
...
function areCandidateObservationsCompatible(scanner, proxy) {
  ...
  const scannerAccountRef = normalizeString(scanner.account_ref);
  const proxyAccountRef = normalizeString(proxy.account_ref);
  if (scannerAccountRef && proxyAccountRef && scannerAccountRef !== proxyAccountRef) return false;
...
function partitionObservationCandidates(candidates) {
  ...
  ordered.forEach((candidate) => {
    union(
      getNode(scannerNodes, candidate.scannerId),
      getNode(proxyNodes, candidate.proxyId)
    );
  });

  const components = new Map();
  ordered.forEach((candidate) => {
    const root = findRoot(scannerNodes.get(candidate.scannerId));
...
function matchObservationCandidateComponent(candidates) {
  const ordered = (Array.isArray(candidates) ? candidates.slice() : []).sort(candidatePriority);
...
function matchObservationCandidates(candidates) {
  return partitionObservationCandidates(candidates)
    .flatMap(matchObservationCandidateComponent)
    .sort(candidatePriority);
}

function readProxyRows(db, query, options = {}) {
  ...
  return db.prepare(`
    SELECT id, provider, account_ref, session_id, source_kind, model,
  ...
  `).all(fromMs, toMs, highWaterInteger(options.recordHighWaterId)) || [];
}

function readScannerCandidates(db, proxies, options = {}) {
  ...
  return db.prepare(`
    ...
    CROSS JOIN model_usage_records AS s INDEXED BY idx_model_usage_timestamp
  ...
  `).all(
    ...
    ATTRIBUTION_WINDOW_MS,
    highWaterInteger(options.recordHighWaterId)
  ) || [];
}

function createAttributions(db, query, forkDescriptors, getPricingByModel, options = {}) {
  const proxies = readProxyRows(db, query, options);
  ...
  readScannerCandidates(db, proxies, options).forEach((candidateRow) => {
  ...
  const resolvePricing = typeof options.resolvePricing === 'function'
    ? options.resolvePricing
    : (model, provider, pricing) => matchModelPricing(model, pricing, provider);
  ...
  return matches.map((match) => {
    ...
    const pricing = resolvePricing(model, scanner.provider, getPricing());
    ...
    return {
      ...
      scannerTimestampMs: finiteInteger(scanner.timestamp_ms),
      proxyTimestampMs: finiteInteger(proxy.timestamp_ms),
      model,
      accountProvider: normalizeProvider(proxy.provider),
      accountRef: normalizeString(proxy.account_ref),
      costUsd
    };
  });
}
  ...
  return `
    ...
    canonical_usage AS (
      SELECT
        ...
        u.provider,
        CASE
          WHEN trim(u.account_ref) != '' THEN u.provider
          ELSE COALESCE(a.account_provider, '')
        END AS account_provider,
        CASE
          WHEN trim(u.account_ref) != '' THEN u.account_ref
          ELSE COALESCE(a.account_ref, '')
        END AS account_ref,
        CASE WHEN
...
function prepareCanonicalReadProjectionContext(db, query, options = {}) {
  const highWater = db.prepare(`
    SELECT
      (SELECT COALESCE(MAX(id), 0) FROM model_usage_records) AS record_high_water_id,
      (SELECT COALESCE(MAX(rowid), 0) FROM model_usage_prompt_events) AS prompt_high_water_rowid
  `).get() || {};
  const recordHighWaterId = highWaterInteger(highWater.record_high_water_id);
  const promptHighWaterRowId = highWaterInteger(highWater.prompt_high_water_rowid);
  ...
  const attributions = createAttributions(
    ...
    {
      recordHighWaterId,
      resolvePricing: options.resolvePricing
    }
  );
  return {
    attributions,
    forkDescriptors,
    recordHighWaterId,
    promptHighWaterRowId
  };
}

function createCanonicalReadProjection(db, query, options = {}) {
  const prepared = Object.prototype.hasOwnProperty.call(options, 'context')
    ? (options.context || {})
    : prepareCanonicalReadProjectionContext(db, query, options);
  const forkDescriptors = Array.isArray(prepared.forkDescriptors) ? prepared.forkDescriptors : [];
  const attributions = Array.isArray(prepared.attributions) ? prepared.attributions : [];
  const recordHighWaterId = highWaterInteger(prepared.recordHighWaterId);
  const promptHighWaterRowId = highWaterInteger(prepared.promptHighWaterRowId);
...
module.exports = {
  ...
  __private: {
    ...
    partitionObservationCandidates,

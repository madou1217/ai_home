...
function mergeLogEntry(record, entry) {
  ...
  if (entry.kind === REQUEST_CONTEXT_KIND) record.hasRequestContext = true;
...
function applyRequestSemantics(record) {
  if (record.endpoint === '/v1/models') {
    record.provider = 'gateway';
    record.model = 'model-catalog';
    record.reasoningEffort = 'not_applicable';
    record.requestType = 'sync';
    return record;
  }
  if (record.hasRequestContext && !record.reasoningEffort) {
...
function projectRequestLogText(text) {
  ...
  records.forEach(applyRequestSemantics);
  return records;
}

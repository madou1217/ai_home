...
// Kimi sessions live under <runtimeDir>/.kimi-code/sessions, where runtimeDir
// is either the host home or one auth-projection sandbox per account. Each
// runtimeDir owns its credentials, which is what account attribution keys on.
function listKimiUsageScanTargets({ fs, path, hostHomeDir, aiHomeDir }) {
  const targets = [{
  ...
  const projectionsRoot = aiHomeDir
    ...
    : '';
  if (!projectionsRoot || !pathExists(fs, projectionsRoot)) return targets;
  let entries = [];
  try {
  ...
  entries.forEach((entry) => {
    ...
    const runtimeDir = path.join(projectionsRoot, entry.name);
    targets.push({
      runtimeDir,
      sessionsRoot: path.join(runtimeDir, '.kimi-code', 'sessions')
    });
  });
  return targets;
}

function scanKimiFile({ fs, path, store, filePath, accountRef = '' }) {
  const stat = fs.statSync(filePath);
  const currentState = store.getFileState(filePath);
  const startOffset = stat.size < currentState.offset ? 0 : currentState.offset;
  const ctx = startOffset > 0 && currentState.scanContext ? currentState.scanContext : {};
  ...
  store.setFileState(filePath, {
    size: stat.size,
    offset: stat.size,
    scanContext: { sessionId, cwd, startedAtMs, updatedAtMs }
  });
  // Rows inserted before attribution became available (or before the account
  // was registered) keep account_ref = ''; backfill them for this file.
  if (attributedAccountRef && typeof store.backfillFileAccountRef === 'function') {
    store.backfillFileAccountRef({ provider: 'kimi', filePath, accountRef: attributedAccountRef });
  }
  return { records: inserted, prompts: promptsInserted };
}
...
function scanModelUsageSources(options = {}) {
  ...
  if (providers.has('kimi')) {
    const resolveKimiAccountRef = typeof options.resolveKimiAccountRef === 'function'
      ? options.resolveKimiAccountRef
      : null;
    const targets = listKimiUsageScanTargets({
      fs,
      path,
      hostHomeDir,
    ...
    targets.forEach((target) => {
      const files = listFilesRecursive(fs, path, target.sessionsRoot, (_full, name) => name === 'wire.jsonl');
      if (files.length === 0) return;
      let accountRef = '';
      try {
        accountRef = resolveKimiAccountRef
          ? String(resolveKimiAccountRef(target.runtimeDir) || '').trim()
          : '';
      } catch (_error) {
        accountRef = '';
      }
      addCounts(combined, scanProviderFiles({
        fs,
        path,
        store,
        files,
        scanFile: (filePath) => scanKimiFile({ fs, path, store, filePath, accountRef })
      }));
    });

...
// is either the host home or one auth-projection sandbox per account. Sandbox
// directories are named by accountRef (resolveAihRunPath), so the owning
// account comes straight from the path; host sessions stay unattributed.
  ...
  if (providers.has('kimi')) {
    ...
    targets.forEach((target) => {
      ...
      addCounts(combined, scanProviderFiles({
        ...
        store,
        files,
        scanFile: (filePath) => scanKimiFile({ fs, path, store, filePath, accountRef: target.accountRef })
      }));
    });

...
const { resolveZcodeUsageAttribution } = require('./zcode-session-ownership');
const { createCodexSessionOwnershipIndex } = require('./codex-session-ownership');
...
// Bound each scheduler tick at JSONL record boundaries. One oversized record
// may exceed this soft budget; the persisted cursor resumes on the next tick.
const CODEX_SCAN_BYTE_BUDGET = 4 * 1024 * 1024;
...
function normalizeProviderFilter(providers) {
  ...
  return set.size > 0 ? set : new Set(listProvidersByCapability('usageScan'));
}
...
function readJsonlFromOffset(fs, filePath, offset, onLine, maxBytes = Number.POSITIVE_INFINITY) {
  ...
  try {
    ...
    byteLimit = Number.isFinite(maxBytes)
      ? cursor + Math.max(0, Number(maxBytes))
      : Number.POSITIVE_INFINITY;
    // Finish the current record at the budget boundary. Rewinding a partial
    // record can otherwise stall forever when one JSONL line exceeds the budget.
    while (cursor < byteLimit || pendingBytes > 0) {
      const readSize = cursor < byteLimit ? Math.min(chunkSize, byteLimit - cursor) : chunkSize;
      const bytesRead = fs.readSync(fd, buffer, 0, readSize, cursor);
      if (!bytesRead) break;
      let segmentStart = 0;
      for (let index = 0; index < bytesRead; index += 1) {
        ...
        if (lineStart >= byteLimit) {
          return { offset: lineStart, hadTrailingLine: false, trailingLineStart: lineStart };
        }
...
function scanCodexFile({
  ...
  const initialSessionId = String(
    storedContext.sessionId
      || (firstPayload && firstPayload.id)
      || basenameWithoutExt(path, filePath)
      || ''
  ).trim();
  const requestedAccountRef = typeof resolveAccountRef === 'function'
    ? String(resolveAccountRef(initialSessionId, filePath) || '').trim()
    : '';
  const storedAccountRef = String(storedContext.attributedAccountRef || '').trim();
  // A newly discovered exact registry owner is allowed to rebuild an existing
  // transcript projection. If the registry later loses an owner, retain the
  // historical attribution instead of erasing a previously proven account.
  const rebuildAccountProjection = Number(currentState.offset) > 0
    && requestedAccountRef
    && requestedAccountRef !== storedAccountRef;
  ...
  const fullProjectionRebuild = rebuildForkProjection || rebuildAccountProjection;
  const startOffset = stat.size < currentState.offset || fullProjectionRebuild
    ? 0
    : currentState.offset;
  const ctx = startOffset > 0 && currentState.scanContext ? currentState.scanContext : {};
  let sessionId = String(ctx.sessionId || '').trim();
  let attributedAccountRef = requestedAccountRef || storedAccountRef;
  ...
  }, fullProjectionRebuild ? Number.POSITIVE_INFINITY : maxBytes);
  ...
  if (fullProjectionRebuild) {
    ...
    return {
      ...
      prompts: rebuilt.prompts,
      bytesRead: Math.max(0, readResult.offset - startOffset),
      filesDeferred: 0
    };
  }
  ...
  return {
    ...
    bytesRead: Math.max(0, readResult.offset - startOffset),
    filesDeferred: readResult.offset < stat.size ? 1 : 0
  };
}
...
function discoverZcodeUsageFile({ fs, path, hostHomeDir }) {
  // zcode 的 cli/db 是跨账号共享的（各投影软链回宿主同一份），只扫宿主库一次，
  // 避免按投影枚举造成同库重复记账。
  if (!hostHomeDir) return [];
  const dbPath = path.join(hostHomeDir, '.zcode', 'cli', 'db', 'db.sqlite');
  if (!pathExists(fs, dbPath)) return [];
  try {
...
// WAL 库的主文件 mtime 只在 checkpoint 时变化；把 -wal 一并纳入变化检测，
// 否则两次扫描之间的增量写入会被 mtime 跳过逻辑漏掉。
function readZcodeDbChangeFingerprint(fs, dbPath) {
  let size = 0;
  let mtimeMs = 0;
...
function scanZcodeUsageFile({ fs, path, store, filePath, accountRef, DatabaseSync }) {
  if (!pathExists(fs, filePath)) return { records: 0, prompts: 0 };
  const fingerprint = readZcodeDbChangeFingerprint(fs, filePath);
  if (fingerprint.size === 0) return { records: 0, prompts: 0 };
  const currentState = store.getFileState(filePath);
  const prevSize = Number(currentState.size) || 0;
  const prevMtime = Number(currentState.scanContext && currentState.scanContext.mtimeMs || 0);
  if (fingerprint.size <= prevSize && fingerprint.mtimeMs <= prevMtime) {
    return { records: 0, prompts: 0 };
  }

  const DatabaseSyncCtor = getDatabaseSyncCtor({ DatabaseSync });
  if (!DatabaseSyncCtor) return { records: 0, prompts: 0 };

  const normalizedAccountRef = isAccountRef(accountRef) ? accountRef : '';
  let db = null;
  try {
    db = new DatabaseSyncCtor(filePath, { readOnly: true });
    db.exec('PRAGMA query_only = ON;');
    ...
    store.setFileState(filePath, {
      ...
      scanContext: { mtimeMs: fingerprint.mtimeMs, error: String(error && error.message || error) }
    });
    return { records: 0, prompts: 0 };
  }

  try {
    if (!hasSqliteTable(db, 'model_usage')) {
      store.setFileState(filePath, {
        size: fingerprint.size,
        offset: fingerprint.size,
        scanContext: { mtimeMs: fingerprint.mtimeMs }
      });
      return { records: 0, prompts: 0 };
    }

    // 行级游标：model_usage 每笔推理一行、增长最快，指纹变化后只取
    // started_at >= 上次游标 的增量（重叠行靠 event_key 幂等去重），
    // 避免每次调度全表重扫。无游标（首扫/上游重置）则全量。
    const cursorStartedAt = Number(
      currentState.scanContext && currentState.scanContext.lastStartedAt || 0
    );
    const baseSql = `
      SELECT
        mu.id AS usage_id,
        mu.session_id AS session_id,
        mu.model_id AS model_id,
        mu.status AS status,
        mu.started_at AS started_at,
        mu.completed_at AS completed_at,
        mu.input_tokens AS input_tokens,
        mu.output_tokens AS output_tokens,
        mu.reasoning_tokens AS reasoning_tokens,
        mu.cache_creation_input_tokens AS cache_creation_input_tokens,
        mu.cache_read_input_tokens AS cache_read_input_tokens,
        mu.provider_total_tokens AS provider_total_tokens,
        mu.computed_total_tokens AS computed_total_tokens,
        mu.parent_user_message_id AS parent_user_message_id,
    ...
    `;
    const rows = cursorStartedAt > 0
      ? db.prepare(`${baseSql} WHERE mu.started_at >= ? ORDER BY mu.started_at ASC, mu.id ASC`).all(cursorStartedAt)
      : db.prepare(`${baseSql} ORDER BY mu.started_at ASC, mu.id ASC`).all();

    const records = [];
    const promptEvents = [];
    const promptMessageIds = new Set();
    const sessionsById = new Map();
    let cursorNextStartedAt = cursorStartedAt;

    for (const row of rows) {
      const sessionId = String(row && row.session_id || '').trim();
      const usageId = String(row && row.usage_id || '').trim();
      if (!usageId) continue;

      const input = toInt(row.input_tokens);
      const output = toInt(row.output_tokens);
      const reasoning = toInt(row.reasoning_tokens);
      const cacheRead = toInt(row.cache_read_input_tokens);
      const cacheCreation = toInt(row.cache_creation_input_tokens);
      const total = toInt(row.computed_total_tokens)
        || toInt(row.provider_total_tokens)
        || (input + output + reasoning + cacheRead + cacheCreation);
      if (!input && !output && !reasoning && !cacheRead && !cacheCreation && !total) continue;

      const model = String(row.model_id || '').trim().toLowerCase();
      if (!model) continue;

      const timestampMs = toTimestampMs(row.completed_at) || toTimestampMs(row.started_at);
      const rowStartedAt = toTimestampMs(row.started_at);
      if (rowStartedAt > cursorNextStartedAt) cursorNextStartedAt = rowStartedAt;
      const cwd = String(row.session_directory || row.session_path || '').trim();
      const promptMessageId = String(row.parent_user_message_id || '').trim();
      let sessionInfo = sessionsById.get(sessionId);
      if (!sessionInfo) {
        sessionInfo = {
          sessionId,
          cwd,
          project: inferProjectFromCwd(path, cwd, String(row.session_title || '').trim()),
          gitBranch: '',
          startedAtMs: toTimestampMs(row.session_time_created) || timestampMs,
          updatedAtMs: toTimestampMs(row.session_time_updated) || timestampMs,
      ...
      if (promptMessageId) {
        // parent_user_message_id 即本轮用户消息；去重后落 prompt 事件，
        // 驱动模型用量页的 prompt 统计与会话 promptCount。
        const promptKey = `${sessionId}\0${promptMessageId}`;
        if (!promptMessageIds.has(promptKey)) {
          promptMessageIds.add(promptKey);
          sessionInfo.promptMessageIds.add(promptMessageId);
          if (timestampMs) {
            promptEvents.push({
              provider: 'zcode',
              sessionId,
              timestampMs,
              eventKey: `zcode:prompt:${promptMessageId}`
            });
          }
        }
      }
      ...
      records.push({
        eventKey: `zcode:model_usage:${usageId}`,
        provider: 'zcode',
        sourceKind: 'session_db',
        sessionId,
        model,
        ...
        cwd: sessionInfo.cwd,
        gitBranch: '',
        ...(normalizedAccountRef ? { accountRef: normalizedAccountRef } : {})
      });
    }
    ...
    if (sessionsById.size > 0) {
      store.upsertSessions(Array.from(sessionsById.values()).map((s) => ({
        ...
        startedAtMs: s.startedAtMs,
        updatedAtMs: s.updatedAtMs,
        promptCount: s.promptMessageIds.size,
        version: s.version
      })));
    }
    store.setFileState(filePath, {
      ...
      scanContext: {
        ...
        lastStartedAt: cursorNextStartedAt,
...
function scanProviderFiles({ fs, path, store, files, scanFile, byteBudget = Number.POSITIVE_INFINITY }) {
  const out = {
    files: 0,
    records: 0,
    prompts: 0,
  ...
  let remainingBytes = Number.isFinite(byteBudget) ? Math.max(0, byteBudget) : Number.POSITIVE_INFINITY;
  files.forEach((filePath) => {
    ...
    try {
      const result = scanFile(filePath, remainingBytes) || {};
      out.files += 1;
      ...
      remainingBytes -= Number(result.bytesRead) || 0;
    } catch (_error) {
      out.skipped += 1;
    }
  });
...
function scanModelUsageSources(options = {}) {
  ...
  if (providers.has('codex')) {
    ...
    const codexOwnership = createCodexSessionOwnershipIndex({
      fs,
    ...
    const providerResult = scanProviderFiles({
      ...
      store,
      files,
      scanFile: (filePath, remainingBytes) => scanCodexFile({
        fs,
        path,
        store,
        filePath,
        reindexCodexForkHistory: options.reindexCodexForkHistory === true,
        resolveAccountRef: codexOwnership.resolveAccountRef,
        maxBytes: remainingBytes
      }),
      byteBudget: Number.isFinite(options.codexScanByteBudget)
        ? options.codexScanByteBudget
        : CODEX_SCAN_BYTE_BUDGET
    });
  ...
  if (providers.has('zcode')) {
    // 共享会话库的账号归属（单 OAuth 账号精确归属；多账号诚实降级为未归属）。
    const attribution = resolveZcodeUsageAttribution({
      fs,
      aiHomeDir: String(options.aiHomeDir || '').trim()
    });
    const files = discoverZcodeUsageFile({ fs, path, hostHomeDir });
    const providerResult = scanProviderFiles({
      fs,
      path,
      store,
      files,
      scanFile: (filePath) => scanZcodeUsageFile({
        fs,
        path,
        store,
        filePath,
    ...
    providerResult.attributionMode = attribution.mode;
    providerResult.attributionAccountCount = attribution.oauthAccountCount;
    result.providers.zcode = providerResult;
    addCounts(result, providerResult);
  }

...
function createModelUsageScanScheduler(options = {}) {
  ...
  async function runScanNow(reason = 'manual') {
    ...
    try {
      ...
      if (typeof options.onTokenUsageUpdated === 'function') {
        try {
          await refreshAccountTokenUsage(
            modelUsageService,
            options.onTokenUsageUpdated,
            { generatedAt: Date.now() }
          );
        } catch (error) {
          logWarn(`model token usage cache refresh failed (${reason}): ${formatError(error)}`);
        }
      }
      state.scanCount += 1;
      state.lastResult = {

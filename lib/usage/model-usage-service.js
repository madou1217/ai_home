...
} = require('./model-usage-request-details');
...
function normalizeQueryOptions(options = {}) {
  ...
  const trend = resolveModelUsageTrend({
    ...normalized,
    trendBucketMs: options.trendBucketMs,
    trendOriginMs: options.trendOriginMs
  });
  return {
    ...normalized,
...
function createModelUsageService(options = {}) {
  ...
  const requestLogReader = createRequestLogReader({
    fs,
  ...
  function getRequestDetails(query = {}) {
    const normalized = normalizeQueryOptions(query);
    const usageRows = withStore((store) => store.queryRequestUsage(normalized));
    return buildRequestDetails(usageRows, requestLogReader.read(), normalized);
  }
  ...
  function getBreakdown(query = {}) {
    return withStore((store) => store.queryBreakdown(normalizeQueryOptions(query)));
  }
  ...
  function getRequestDetailsAsync(query = {}) {
    return executeQueryAsync('getRequestDetails', query, getRequestDetails);
  }
  ...
  function getBreakdownAsync(query = {}) {
    return executeQueryAsync('getBreakdown', query, getBreakdown);
  }

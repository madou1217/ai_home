...
const { createKimiUsageAccountRefResolver } = require('./kimi-usage-account-ref');

const DEFAULT_PROVIDERS = Object.freeze(['codex', 'claude', 'gemini', 'agy', 'opencode', 'kimi']);
...
function createModelUsageService(options = {}) {
  ...
  const resolveKimiAccountRef = options.resolveKimiAccountRef === undefined
    ? createKimiUsageAccountRefResolver({ fs, path, aiHomeDir })

...
const {
  ...
  prepareCanonicalReadProjectionContext
} = require('./model-usage-read-projection');
...
function openModelUsageStore(options = {}) {
  ...
  if (!db) return null;
  ensureSchema(db);
  return createModelUsageStore(db, options);
}
...
function normalizeUsageRecord(input = {}, pricingByModel = {}, options = {}) {
  ...
  const pricing = typeof options.resolvePricing === 'function'
    ? options.resolvePricing(model, provider, pricingByModel)
    : matchModelPricing(model, pricingByModel, provider);
...
function normalizeQuery(input = {}) {
  ...
  const trend = resolveModelUsageTrend({
    fromMs,
    toMs,
    trendBucketMs: input.trendBucketMs,
    trendOriginMs: input.trendOriginMs
  });
  return {
    fromMs: fromMs || 0,
    toMs,
    ...
    trendBucketMs: trend.bucketMs,
    trendOriginMs: trend.originMs
  };
}

function parseAccountRefs(value) {
...
function compareUsageRows(left, right) {
  return (Number(right.costUsd) - Number(left.costUsd))
    || (Number(right.totalTokens) - Number(left.totalTokens))
    || (Number(right.calls) - Number(left.calls))
    || String(left.model || left.accountRef || '').localeCompare(String(right.model || right.accountRef || ''));
}
...
function createModelUsageStore(db, options = {}) {
  const modelPricingResolver = options.modelPricingResolver || null;
  function close() {
  ...
  function resolvePricing(model, provider, pricingByModel) {
    if (!modelPricingResolver || typeof modelPricingResolver.resolve !== 'function') {
      return matchModelPricing(model, pricingByModel, provider);
    }
    const activeCatalog = getActivePricingCatalog();
    const resolution = modelPricingResolver.resolve({
      sourceProviderId: String(activeCatalog && activeCatalog.sourceFamily || '').trim(),
      provider,
      model
    }, { pricingByModel });
    return resolution && resolution.pricing || null;
  }
  ...
  function normalizeUsageRecords(records = []) {
    ...
    return (Array.isArray(records) ? records : [])
      .map((record) => normalizeUsageRecord(record, pricingByModel, { resolvePricing }))
      .filter(Boolean);
  }
  ...
  function buildCostChanges(rows, pricingByModel) {
    const changes = [];
    rows.forEach((row) => {
      const pricing = resolvePricing(row.model, row.provider, pricingByModel);
      if (!pricing) return;
  ...
  function normalizeKimiFileProjectionInput(input = {}) {
    const sourceHash = String(input.sourceHash || '').trim();
    const filePath = String(input.filePath || '').trim();
    ...
    if (
      ...
      throw new Error('model_usage_kimi_file_projection_scope_invalid');
    }
    const eventPrefix = `kimi:file:${sourceHash}:`;
    const usageRecords = normalizeUsageRecords(input.usageRecords);
    const promptEvents = normalizePromptEvents(input.promptEvents);
    const sessionRecords = normalizeSessionRecords(input.sessionRecords);
    const fileState = normalizeFileState(filePath, input.fileState);
    ...
    const sessionsInScope = sessionRecords.length <= 1 && sessionRecords.every((record) => (
      record.provider === 'kimi' && record.sessionId === sessionId
    ));
    if (!usageInScope || !promptsInScope || !sessionsInScope) {
      throw new Error('model_usage_kimi_file_projection_scope_invalid');
    }
    return {
      eventPrefix,
      expectedFileState: Object.prototype.hasOwnProperty.call(input, 'expectedFileState')
        ? input.expectedFileState
        : undefined,
      fileState,
      promptEvents,
      sessionId,
  ...
  function assertExpectedKimiFileState(filePath, expectedState) {
    ...
    const currentContext = currentState.scanContext && typeof currentState.scanContext === 'object'
      ...
      : null;
    const expectedContext = expectedState.scanContext && typeof expectedState.scanContext === 'object'
      ? expectedState.scanContext
      : null;
    if (
      toSafeInteger(currentState.size) !== toSafeInteger(expectedState.size)
      || toSafeInteger(currentState.offset) !== toSafeInteger(expectedState.offset)
      || JSON.stringify(currentContext) !== JSON.stringify(expectedContext)
    ) {
      const error = new Error('model_usage_kimi_file_projection_stale');
      error.code = 'model_usage_kimi_file_projection_stale';
      throw error;
    }
  }

  function refreshKimiSessionPromptCounts(sessionIds = []) {
    const countPrompts = db.prepare(`
      SELECT COUNT(*) AS count FROM model_usage_prompt_events
      WHERE provider = 'kimi' AND session_id = ?
    `);
    const updatePromptCount = db.prepare(`
      UPDATE model_usage_sessions SET prompt_count = ?
      WHERE provider = 'kimi' AND session_id = ?
    `);
    Array.from(new Set(sessionIds.map((value) => String(value || '').trim()).filter(Boolean)))
      .forEach((sessionId) => {
        const row = countPrompts.get(sessionId) || {};
        updatePromptCount.run(toSafeInteger(row.count), sessionId);
      });
  }

  function appendKimiFileProjection(input = {}) {
    return runInImmediateTransaction(() => {
      const projection = normalizeKimiFileProjectionInput(input);
      assertExpectedKimiFileState(projection.fileState.filePath, projection.expectedFileState);
      const records = insertNormalizedUsageRecords(projection.usageRecords);
      const prompts = insertNormalizedPromptEvents(projection.promptEvents);
      const sessions = upsertNormalizedSessions(projection.sessionRecords.map((record) => ({
        ...record,
        promptCount: prompts
      })));
      writeFileState(projection.fileState);
      return { records, prompts, sessions };
    });
  }

  function replaceKimiFileProjection(input = {}) {
    return runInImmediateTransaction(() => {
      const projection = normalizeKimiFileProjectionInput(input);
      assertExpectedKimiFileState(projection.fileState.filePath, projection.expectedFileState);
      const eventGlob = `${projection.eventPrefix}*`;
      const affectedSessionIds = new Set([projection.sessionId]);
      db.prepare(`
        SELECT session_id FROM model_usage_records
        WHERE provider = 'kimi' AND event_key GLOB ?
        UNION
        SELECT session_id FROM model_usage_prompt_events
        WHERE provider = 'kimi' AND event_key GLOB ?
      `).all(eventGlob, eventGlob).forEach((row) => {
        const existingSessionId = String(row && row.session_id || '').trim();
        if (existingSessionId) affectedSessionIds.add(existingSessionId);
      });
      const usageDeleted = Number(db.prepare(`
        DELETE FROM model_usage_records
        WHERE provider = 'kimi' AND event_key GLOB ?
      `).run(`${projection.eventPrefix}*:usage`).changes) || 0;
      const promptsDeleted = Number(db.prepare(`
        DELETE FROM model_usage_prompt_events
        WHERE provider = 'kimi' AND event_key GLOB ?
      `).run(`${projection.eventPrefix}*:prompt`).changes) || 0;
      const records = insertNormalizedUsageRecords(projection.usageRecords);
      const prompts = insertNormalizedPromptEvents(projection.promptEvents);
      const sessions = upsertNormalizedSessions(projection.sessionRecords.map((record) => ({
        ...record,
      ...
      refreshKimiSessionPromptCounts(Array.from(affectedSessionIds));
      writeFileState(projection.fileState);
      return { usageDeleted, promptsDeleted, records, prompts, sessions };
    });
  }

  function replaceKimiTranscriptProjection(input = {}) {
    const fileStates = (Array.isArray(input.fileStates) ? input.fileStates : [])
      .map((state) => normalizeFileState(state && state.filePath, state))
      .filter((state) => state.filePath);
    if (fileStates.length === 0) {
      throw new Error('model_usage_kimi_projection_source_empty');
    }
    ...
    if (
      ...
      || fileStates.some((state) => !sourcePaths.includes(state.filePath))
    ) {
      throw new Error('model_usage_kimi_projection_scope_invalid');
    }
    const sourceHashes = new Set(sourcePaths.map((sourcePath) => stableHash(sourcePath)));
    const usageRecords = normalizeUsageRecords(input.usageRecords);
    const promptEvents = normalizePromptEvents(input.promptEvents);
    const sessionRecords = normalizeSessionRecords(input.sessionRecords);
    const matchesFileEvent = (eventKey, expectedKind) => {
      const match = String(eventKey || '').match(
        /^kimi:file:([0-9a-f]{16}):[0-9]+:(usage|prompt)$/
      );
      return Boolean(match && match[2] === expectedKind && sourceHashes.has(match[1]));
    };
    const usageInScope = usageRecords.every((record) => (
      record.provider === 'kimi'
      && record.sourceKind === 'session_jsonl'
      && matchesFileEvent(record.eventKey, 'usage')
    ));
    const promptsInScope = promptEvents.every((record) => (
      record.provider === 'kimi' && matchesFileEvent(record.eventKey, 'prompt')
    ));
    const sessionsInScope = sessionRecords.every((record) => record.provider === 'kimi');
    if (!usageInScope || !promptsInScope || !sessionsInScope) {
      throw new Error('model_usage_kimi_projection_scope_invalid');
    }

    return runInImmediateTransaction(() => {
      const affectedSessionIds = new Set(sessionRecords.map((record) => record.sessionId));
      const readAffectedSessions = db.prepare(`
        SELECT session_id FROM model_usage_records
        WHERE provider = 'kimi' AND source_kind = 'session_jsonl' AND event_key GLOB ?
        UNION
        SELECT session_id FROM model_usage_prompt_events
        WHERE provider = 'kimi' AND event_key GLOB ?
      `);
      ...
      const deletePrompts = db.prepare(`
        ...
        WHERE provider = 'kimi' AND event_key GLOB ?
      `);
      const deleted = { records: 0, prompts: 0, sessions: 0, fileStates: 0 };
      sourceHashes.forEach((sourceHash) => {
        const eventGlob = `kimi:file:${sourceHash}:*`;
        readAffectedSessions.all(eventGlob, eventGlob).forEach((row) => {
          const sessionId = String(row && row.session_id || '').trim();
          if (sessionId) affectedSessionIds.add(sessionId);
        });
        deleted.records += Number(deleteUsage.run(`${eventGlob}:usage`).changes) || 0;
        deleted.prompts += Number(deletePrompts.run(`${eventGlob}:prompt`).changes) || 0;
      });
      const deleteFileState = db.prepare('DELETE FROM model_usage_file_state WHERE path = ?');
      sourcePaths.forEach((sourcePath) => {
        deleted.fileStates += Number(deleteFileState.run(sourcePath).changes) || 0;
      });
      const records = insertNormalizedUsageRecords(usageRecords);
      const prompts = insertNormalizedPromptEvents(promptEvents);
      const sessions = replaceNormalizedSessions(sessionRecords);
      refreshKimiSessionPromptCounts(Array.from(affectedSessionIds));
      fileStates.forEach(writeFileState);
      return {
        deleted,
        records,
        prompts,
  ...
  function prepareDashboardQuery(query = {}) {
    const normalized = normalizeQuery(query);
    return prepareCanonicalReadProjectionContext(db, normalized, {
      getPricingByModel: getAllPricing,
      resolvePricing
    });
  }

  function buildReadProjection(query = {}, options = {}) {
    const normalized = normalizeQuery(query);
    return {
      normalized,
  ...
  function queryStatsFromProjection(projection, query = projection.normalized) {
    ...
    return {
      ...
      totalCostUsd: normalizeModelUsageCostUsd(row.total_cost_usd)
    };
  }
  ...
  function queryRequestUsage(query = {}) {
    const normalized = normalizeQuery(query);
    const clauses = [
      "request_id != ''",
      "source_kind GLOB 'server_*'",
    ...
    `).all(...args, normalized.limit) || []).map((row) => ({
      ...
      reasoningOutputTokens: Number(row.reasoning_output_tokens) || 0,
      totalTokens: Number(row.total_tokens) || 0,
      costUsd: normalizeModelUsageCostUsd(row.cost_usd),
      timestampMs: Number(row.timestamp_ms) || 0
    }));
  }

  function queryAccountTokenUsage(options = {}) {
    const dimensions = normalizeAccountTokenUsageDimensions(options.dimensions);
    const windows = resolveLocalAccountTokenUsageWindows(options.nowMs);
    const starts = {
      day: windows.dayStartMs,
      week: windows.weekStartMs,
      month: windows.monthStartMs,
      total: windows.totalStartMs
    };
    const fromMs = Math.min(...dimensions.map((dimension) => starts[dimension]));
    const aggregateColumns = dimensions.flatMap((dimension) => ([
      `COALESCE(SUM(CASE WHEN u.timestamp_ms >= ? THEN u.total_tokens ELSE 0 END), 0) AS ${dimension}_tokens`,
      `COALESCE(SUM(CASE WHEN u.timestamp_ms >= ? THEN u.cost_usd ELSE 0 END), 0) AS ${dimension}_cost_usd`
    ]));
    const pricingByModel = getAllPricing();

    // Account-scoped usage is emitted by the gateway with account_ref already
    // attached. Transcript rows historically had no account identity; kimi
    // wire files are the exception — the native session index records the
    // exact account projection that launched each shared physical session, so
    // the scanner attributes them at write time from that index. Other
    // providers' transcript rows
    // stay unattributed and are only used by the canonical read projection
    // for cross-source attribution; running that attribution here makes a
    // simple cache refresh scale with the entire transcript corpus and can
    // exceed the query-worker timeout. Aggregate the account-attributed
    // records directly while keeping the public model-usage service and
    // dimension contract shared by all callers.
    const rows = db.prepare(`
      ...
      WHERE u.account_ref != ''
        AND u.timestamp_ms BETWEEN ? AND ?
      GROUP BY u.account_ref, u.provider, u.model
    `).all(
      ...dimensions.flatMap((dimension) => [starts[dimension], starts[dimension]]),
      fromMs,
      windows.nowMs
    ) || [];
    ...
    rows.forEach((row) => {
      ...
      let account = accounts.get(accountRef);
      if (!account) {
        account = {
          totals: Object.fromEntries(dimensions.map((dimension) => [dimension, 0])),
          models: new Map()
        };
        accounts.set(accountRef, account);
      }

      const model = String(row.model || '').trim();
      const modelUsage = model ? (account.models.get(model) || {
        model,
      ...
      const pricing = model ? resolvePricing(model, row.provider, pricingByModel) : null;

      dimensions.forEach((dimension) => {
        const tokens = Math.max(0, Number(row[`${dimension}_tokens`]) || 0);
        const costKey = `${dimension}CostUsd`;
        account.totals[dimension] += tokens;
        if (!modelUsage) return;
        const currentTokens = modelUsage[dimension];
        modelUsage[dimension] += tokens;
        if (tokens <= 0) return;
        const costUsd = pricing
          ? Math.max(0, Number(row[`${dimension}_cost_usd`]) || 0)
          : null;
        modelUsage[costKey] = currentTokens === 0
          ? costUsd
          : modelUsage[costKey] === null || costUsd === null
            ? null
            : modelUsage[costKey] + costUsd;
      });
      if (modelUsage) account.models.set(model, modelUsage);
    });

    return Object.fromEntries(Array.from(accounts.entries()).map(([accountRef, account]) => {
      const models = Array.from(account.models.values())
        .filter((modelUsage) => dimensions.some((dimension) => modelUsage[dimension] > 0))
        .sort(compareAccountTokenUsageModels(dimensions));
      return [accountRef, { ...account.totals, models }];
    }));
  }

  function queryCostByModelFromProjection(projection, query = projection.normalized, options = {}) {
    ...
    return (db.prepare(`
      ...
      SELECT
        ...
        GROUP_CONCAT(DISTINCT NULLIF(u.account_ref, '')) AS account_refs,
        COALESCE(SUM(CASE WHEN u.account_ref = '' THEN 1 ELSE 0 END), 0) AS unattributed_calls
      FROM canonical_usage u
      WHERE ${filter.where}
    ...
    `).all(...projection.args, ...filter.args) || []).map((row) => {
      const accountRefs = parseAccountRefs(row.account_refs);
      const item = {
        ...
        totalTokens: Number(row.total_tokens) || 0,
        costUsd: normalizeModelUsageCostUsd(row.cost_usd),
        accountCount: accountRefs.length,
        unattributedCalls: Number(row.unattributed_calls) || 0
      };
      item.cacheHitRate = calculateCacheHitRate(item);
      if (options.includeAccountRefs) item.accountRefs = accountRefs;
      return item;
    });
  }
  ...
  function queryTrendFromProjection(projection, query = projection.normalized) {
    const filter = buildProjectedWhereClause(query, { tableAlias: 'u' });
    const trend = resolveModelUsageTrend(query);
    if (!trend.originMs || !trend.bucketMs) {
      return { fromMs: trend.fromMs, toMs: trend.toMs, bucketMs: 0, points: [] };
    }
    const rows = db.prepare(`
      WITH ${projection.ctes}
      SELECT
        CAST((u.timestamp_ms - ?) / ? AS INTEGER) AS bucket_index,
        COUNT(*) AS calls,
        COALESCE(SUM(u.input_tokens), 0) AS input_tokens,
        COALESCE(SUM(u.output_tokens), 0) AS output_tokens,
        COALESCE(SUM(u.cache_read_input_tokens), 0) AS cache_read_input_tokens,
    ...
    const rowsByStart = new Map(rows.map((row) => [
      trend.originMs + (Number(row.bucket_index) || 0) * trend.bucketMs,
      row
    ]));
    const points = listModelUsageTrendBucketStarts(trend).map((bucketStartMs) => {
      const row = rowsByStart.get(bucketStartMs) || {};
      const point = {
        bucketStartMs,
        calls: Number(row.calls) || 0,
        inputTokens: Number(row.input_tokens) || 0,
      ...
      point.cacheHitRate = calculateCacheHitRate(point);
      return point;
    });
    return {
      fromMs: trend.fromMs,
  ...
  function queryAccountBreakdownFromProjection(projection, query = projection.normalized) {
    const filter = buildProjectedWhereClause(query, {
      tableAlias: 'u',
      includeSession: true
    });
    const rows = db.prepare(`
      ...
      FROM canonical_usage u
      WHERE ${filter.where}
      GROUP BY u.account_provider, u.account_ref, u.provider, u.model
    `).all(...projection.args, ...filter.args) || [];
    const accounts = new Map();
    rows.forEach((row) => {
      const accountRef = String(row.account_ref || '').trim();
      const key = accountRef || '__unattributed__';
      let account = accounts.get(key);
      if (!account) {
        account = {
          accountRef,
          accountProvider: accountRef ? String(row.account_provider || '').trim() : '',
          calls: 0,
          inputTokens: 0,
          outputTokens: 0,
        ...
        && row.account_provider
        && account.accountProvider !== String(row.account_provider).trim()
      ) {
        account.accountProvider = '';
      }
      const model = {
      ...
      model.cacheHitRate = calculateCacheHitRate(model);
      ...
      [
        ...
        account[field] += Number(model[field]) || 0;
      });
    });
    return Array.from(accounts.values())
      .map((account) => ({
        ...
        cacheHitRate: calculateCacheHitRate(account),
        costUsd: normalizeModelUsageCostUsd(account.costUsd),
        models: account.models.sort(compareUsageRows)
      }))
      .sort(compareUsageRows);
  }

  function querySessionPartsFromProjection(projection, query = projection.normalized, options = {}) {
    const filter = buildProjectedWhereClause(query, { tableAlias: 'r' });
    const normalized = normalizeQuery(query);
    const limitResults = options.limit !== false;
    const rows = db.prepare(`
      WITH ${projection.ctes}
      SELECT
        r.provider,
        r.session_id,
        COALESCE(MAX(NULLIF(r.project, '')), '') AS usage_project,
        COALESCE(MAX(NULLIF(s.project, '')), '') AS session_project,
        COALESCE(MAX(NULLIF(r.cwd, '')), '') AS usage_cwd,
        COALESCE(MAX(NULLIF(s.cwd, '')), '') AS session_cwd,
        COALESCE(MAX(NULLIF(r.git_branch, '')), '') AS usage_git_branch,
        COALESCE(MAX(NULLIF(s.git_branch, '')), '') AS session_git_branch,
        MIN(r.timestamp_ms) AS usage_started_at_ms,
        COALESCE(MIN(NULLIF(s.started_at_ms, 0)), 0) AS session_started_at_ms,
        MAX(r.timestamp_ms) AS usage_updated_at_ms,
        COALESCE(MAX(NULLIF(s.updated_at_ms, 0)), 0) AS session_updated_at_ms,
        ...
        COALESCE(SUM(r.reasoning_output_tokens), 0) AS reasoning_output_tokens,
        COALESCE(SUM(r.total_tokens), 0) AS total_tokens,
        COALESCE(SUM(r.cost_usd), 0) AS cost_usd,
        GROUP_CONCAT(DISTINCT NULLIF(r.account_ref, '')) AS account_refs,
        COALESCE(SUM(CASE WHEN r.account_ref = '' THEN 1 ELSE 0 END), 0) AS unattributed_calls
      ...
      ${limitResults ? 'LIMIT ?' : ''}
    `).all(...projection.args, ...filter.args, ...(limitResults ? [normalized.limit] : [])) || [];
    const promptFilter = buildProjectedWhereClause(query, {
      tableAlias: 'p',
      includeModel: false
    });
    ...
    return rows.map((row) => ({
      ...
      usageProject: row.usage_project || '',
      sessionProject: row.session_project || '',
      usageCwd: row.usage_cwd || '',
      sessionCwd: row.session_cwd || '',
      usageGitBranch: row.usage_git_branch || '',
      sessionGitBranch: row.session_git_branch || '',
      usageStartedAtMs: Number(row.usage_started_at_ms) || 0,
      sessionStartedAtMs: Number(row.session_started_at_ms) || 0,
      usageUpdatedAtMs: Number(row.usage_updated_at_ms) || 0,
      sessionUpdatedAtMs: Number(row.session_updated_at_ms) || 0,
      ...
      totalTokens: Number(row.total_tokens) || 0,
      costUsd: normalizeModelUsageCostUsd(row.cost_usd),
      accountRefs: parseAccountRefs(row.account_refs),
      unattributedCalls: Number(row.unattributed_calls) || 0
    }));
  }

  function materializeSessionPart(part) {
    const item = {
      ...
      accountCount: Array.isArray(part.accountRefs) ? part.accountRefs.length : 0,
  ...
  function querySessionsFromProjection(projection, query = projection.normalized) {
    return querySessionPartsFromProjection(projection, query).map(materializeSessionPart);
  }
  ...
  function queryDashboard(query = {}) {
    ...
    const trend = queryTrendFromProjection(projection);
    const modelOptions = projection.normalized.model
      ...
      : models;
    return { stats, models, sessions, modelOptions, trend };
  }

  function queryDashboardShard(query = {}) {
    const projection = buildReadProjection(query, { context: query.projectionContext });
    const stats = queryStatsFromProjection(projection);
    const models = queryCostByModelFromProjection(projection, projection.normalized, {
      includeAccountRefs: true
    });
    const sessionParts = querySessionPartsFromProjection(projection, projection.normalized, { limit: false });
    const trend = queryTrendFromProjection(projection);
    const modelOptions = projection.normalized.model
      ? queryCostByModelFromProjection(projection, {
        ...projection.normalized,
      ...
      : models;
    return { stats, models, sessionParts, modelOptions, trend };
  }

  function queryBreakdown(query = {}) {
    const projection = buildReadProjection(query);
    const accounts = queryAccountBreakdownFromProjection(projection);
    const summary = {
      calls: 0,
      inputTokens: 0,
      outputTokens: 0,
    ...
    accounts.forEach((account) => {
      if (account.accountRef) summary.accountCount += 1;
      else summary.unattributedCalls += Number(account.calls) || 0;
      [
        'calls',
        'inputTokens',
        'outputTokens',
        ...
        summary[field] += Number(account[field]) || 0;
      });
      account.models.forEach((model) => {
        ...
        [
          ...
          aggregate[field] += Number(model[field]) || 0;
        });
        if (account.accountRef) aggregate.accountRefs.add(account.accountRef);
        else aggregate.unattributedCalls += Number(model.calls) || 0;
      });
    });
    summary.costUsd = normalizeModelUsageCostUsd(summary.costUsd);
    summary.cacheHitRate = calculateCacheHitRate(summary);
    return {
      summary,
      accounts,
      models: Array.from(models.values())
        .map((model) => {
          const item = {
            ...
            costUsd: normalizeModelUsageCostUsd(model.costUsd),
            accountCount: model.accountRefs.size,
            unattributedCalls: model.unattributedCalls
          };
          item.cacheHitRate = calculateCacheHitRate(item);
          return item;
        })
  ...
  return {
    ...
    appendKimiFileProjection,
    replaceKimiFileProjection,
    replaceKimiTranscriptProjection,
    prepareDashboardQuery,
    queryStats,
    queryRequestUsage,
    queryAccountTokenUsage,
    queryDashboard,
    queryDashboardShard,
    queryBreakdown,
    queryCostByModel,

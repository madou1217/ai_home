...
function createModelUsageStore(db) {
  ...
  function backfillFileAccountRef({ provider, filePath, accountRef } = {}) {
    const normalizedProvider = String(provider || '').trim();
    const normalizedRef = String(accountRef || '').trim();
    if (!normalizedProvider || !isAccountRef(normalizedRef)) return 0;
    const prefix = `${normalizedProvider}:file:${stableHash(filePath)}:`;
    const result = db.prepare(`
      UPDATE model_usage_records
      SET account_ref = ?
      WHERE provider = ? AND account_ref = '' AND substr(event_key, 1, ?) = ?
    `).run(normalizedRef, normalizedProvider, prefix.length, prefix);
  ...
  function queryAccountTokenUsage(options = {}) {
    ...
    // wire files are the exception — sandbox sessions live under
    // auth-projections/<provider>/<accountRef>, so the scanner attributes
    // them at write time from the path. Other providers' transcript rows

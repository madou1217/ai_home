...
function createModelUsageStore(db) {
  ...
  function queryAccountTokenUsage(options = {}) {
    ...
    // attached. Transcript rows intentionally have no account identity and
    // are only used by the canonical read projection for cross-source
    // attribution; running that attribution here makes a simple cache refresh
    // scale with the entire transcript corpus and can exceed the query-worker
    // timeout. Aggregate the account-attributed records directly while keeping
    // the public model-usage service and dimension contract shared by all
    // callers.

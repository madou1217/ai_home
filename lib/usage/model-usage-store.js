...
function createModelUsageStore(db) {
  ...
  function queryAccountTokenUsage(options = {}) {
    ...
    // wire files are the exception — the scanner attributes them at write
    // time from the owning .kimi-code home's OAuth identity. Other providers'
    // transcript rows stay unattributed and are only used by the canonical
    // read projection for cross-source attribution; running that attribution
    // here makes a simple cache refresh scale with the entire transcript
    // corpus and can exceed the query-worker timeout. Aggregate the
    // account-attributed records directly while keeping the public
    // model-usage service and dimension contract shared by all callers.

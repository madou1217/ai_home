...
function createModelUsageStore(db) {
  ...
  function queryAccountTokenUsage(options = {}) {
    ...
    const aggregateColumns = dimensions.map((dimension) => (
    ...
    const rows = db.prepare(`
      ...
      GROUP BY u.account_ref, u.model
    `).all(
      ...dimensions.map((dimension) => starts[dimension]),
      fromMs,
      windows.nowMs
    ) || [];
    ...
    rows.forEach((row) => {
      ...
      dimensions.forEach((dimension) => {
        ...
        if (modelUsage) modelUsage[dimension] += tokens;
      });

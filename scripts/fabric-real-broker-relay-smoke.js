...
Options:
  ...
  --management-key-file <path>
                        File containing the Server Management Key; the key is never printed.
...
function parseArgs(argv = [], env = process.env) {
  ...
  for (let index = 0; index < argv.length;) {
    ...
    if (token === '--management-key-file' || token.startsWith('--management-key-file=')) {
      ...
      options.managementKeyFile = path.resolve(String(next.value || '').trim());
      index += next.consumed;
      continue;
    }
...
async function main() {
  ...
  try {
    ...
    console.error('Usage: node scripts/fabric-real-broker-relay-smoke.js [--endpoint URL] [--server-id ID] [--management-key-file PATH]');
    process.exitCode = 1;
    return;
  }
  if (options.help) {

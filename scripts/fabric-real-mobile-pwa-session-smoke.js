...
Usage:
  AIH_MANAGEMENT_KEY=<key> npx --yes --package playwright node scripts/fabric-real-mobile-pwa-session-smoke.js [options]

Options:
  --endpoint <url>      AWS/current Server endpoint, default ${DEFAULT_ENDPOINT}.
  --client-endpoint <url>
                        Client API endpoint. Defaults to --endpoint.
  ...
  --existing-node       Use an already registered Fabric node instead of creating
                        a temporary relay node.
  --timeout-ms <n>     Relay/node wait timeout, default ${DEFAULT_TIMEOUT_MS}.
  --session-timeout-ms <n>
                        Mobile browser session wait timeout, default ${DEFAULT_SESSION_TIMEOUT_MS}.
  --headed             Show the browser window.
  -h, --help           Show this help.

The script opens a real Chromium mobile viewport and executes the session client
flow from inside the browser page with the Server Management Key. Temporary-node
mode creates only a real node join invite and starts a real local outbound relay.
It does not open a new product port and does not use old VPS targets.

When --existing-node is set, the script targets the provided Fabric node id
directly with the Server Management Key. It creates no client authorization
record and does not create a temporary relay node.
`);
}
...
function parseArgs(argv = [], env = process.env) {
  const envManagementKey = String(env.AIH_MANAGEMENT_KEY || '').trim();
  const options = {
    help: false,
    ...
    managementKeySource: envManagementKey ? 'env' : '',
  ...
  for (let index = 0; index < argv.length;) {
    ...
    if (token === '--management-key' || token.startsWith('--management-key=')) {
      ...
      options.managementKeySource = options.managementKey ? 'cli' : '';
      index += next.consumed;
      continue;
    }
  ...
  if (!options.help && options.sessionAccountRef && !isAccountRef(options.sessionAccountRef)) {
...
function redactReportSecrets(value, secrets = []) {
  const normalizedSecrets = [...new Set(
    secrets.map((secret) => String(secret || '')).filter(Boolean)
  )];
  ...
  if (Array.isArray(value)) {
    return value.map((item) => redactReportSecrets(item, normalizedSecrets));
  }
  if (!value || typeof value !== 'object') return value;
  return Object.fromEntries(
    Object.entries(value).map(([key, item]) => [key, redactReportSecrets(item, normalizedSecrets)])
  );
}
...
async function createNodeInviteUrl(input = {}, deps = {}) {
  ...
  if (!endpoint || !nodeId) throw new Error('invalid_node_invite_input');
...
async function prepareNodeViaApi(input = {}, deps = {}) {
...
async function waitForNodeOnline(endpoint, managementKey, nodeId, timeoutMs, deps = {}) {
...
function buildMobileBrowserInput(options, managementKey) {
...
async function mobileBrowserClientEvaluate(input) {
  const endpoint = String(input.endpoint || '').replace(/\/+$/, '');
  const managementKey = String(input.managementKey || '');
...
function resolveServerManagementKey(options = {}, nodeManagement = {}) {
  const explicit = String(options.managementKey || '').trim();
  if (explicit) {
    return {
      key: explicit,
      source: String(options.managementKeySource || '').trim() || 'option'
    };
  }
  const configured = nodeManagement.source === 'server-config'
    ? String(nodeManagement.key || '').trim()
    : '';
  if (configured) {
    return {
      key: configured,
...
async function runMobilePwaSessionSmoke(options = {}, deps = {}) {
  ...
  const resolveHostHome = deps.resolveExistingHostHome || resolveExistingHostHome;
  const resolveNodeManagement = deps.resolveExistingNodeManagementKey || resolveExistingNodeManagementKey;
  const hostHomeDir = resolveHostHome(options.hostHome, deps.env || process.env);
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const repoRoot = deps.repoRoot || path.resolve(__dirname, '..');
  const runBrowser = deps.runMobileBrowserClient || runMobileBrowserClient;
  const spawnProcess = deps.spawnAihProcess || spawnAihProcess;
  const waitForOnline = deps.waitForNodeOnline || waitForNodeOnline;
  const children = [];
  let nodeManagement = null;
  let serverManagement = null;
  let nodeInvite = null;
  let prepared = null;
  let online = null;
  try {
    nodeManagement = resolveNodeManagement(aiHomeDir, deps);
    serverManagement = resolveServerManagementKey(options, nodeManagement);
    ...
    if (options.existingNode === true) {
      const browserInput = buildMobileBrowserInput({
        ...
        endpoint,
        clientEndpoint
      }, serverManagement.key);
      const mobile = await runBrowser({
      ...
      return redactReportSecrets({
        ...
        nodeId: options.nodeId,
        managementKeySource: serverManagement.source,
        preparation: {
          mode: 'existing-node'
        },
        auth: {
          method: 'management-key',
          configured: true
        },
        mobile,
        children: []
      }, [serverManagement.key, nodeManagement.key]);
    }
    nodeInvite = await createNodeInviteUrl({
      endpoint,
      nodeId: options.nodeId,
      managementKey: serverManagement.key
    }, deps);
    prepared = await prepareNodeViaApi({
      nodeId: options.nodeId,
      joinUrl: nodeInvite.joinUrl,
    ...
    children.push(spawnProcess('relay-client', hostHomeDir, relayArgs, { repoRoot }));

    online = await waitForOnline(clientEndpoint, serverManagement.key, options.nodeId, options.timeoutMs, deps);
    const browserInput = buildMobileBrowserInput({
      ...options,
      endpoint,
      clientEndpoint
    ...
    return redactReportSecrets({
      ...
      nodeId: options.nodeId,
      managementKeySource: serverManagement.source,
      nodeManagementKeySource: nodeManagement.source,
      preparation: {
        mode: 'api',
        joinStatus: prepared.preparation.joinStatus,
        nodeInviteId: nodeInvite.nodeInviteId
      },
      relay: {
    ...
    return redactReportSecrets({
      ...
      nodeId: options.nodeId,
      managementKeySource: serverManagement && serverManagement.source || '',
      ...
      serverManagement && serverManagement.key,
      nodeManagement && nodeManagement.key
    ]);
  } finally {

...
Options:
  ...
  --endpoint <url>      Use an already running Server endpoint instead of starting temp servers.
  --client-endpoint <url>
                         Client API endpoint. Defaults to --endpoint; use a broker proxy base to
  ...
  --control-port <n>    Server port. Omit or pass 0 to auto-pick.
  ...
  --session-account-ref <ref>
                         Native provider accountRef for session smoke.
...
directories, starts a real "aih node relay connect" child process, authenticates
client requests with the Server Management Key, calls remote-node endpoints over
the relay, and prints a sanitized JSON report. It never prints management keys.

When --endpoint is provided, the script does not start any server or allocate
ports. It writes the test node record into the endpoint's host home,
...
When --node-join-url is provided with --endpoint, the script prepares the node
through that real HTTP API instead of sharing a host-home filesystem. This is
the cross-machine mode: the node process can run
on a different host from the Server as long as the endpoint URL is
...
function validateEndpointModeOptions(options = {}) {
  ...
  [options.nodeJoinUrl].filter(Boolean).forEach((value) => {
    try {
      ...
      throw new Error('--node-join-url must be a valid http(s) URL');
    }
  });
}
...
function validateSessionSmokeOptions(options = {}) {
  ...
  if (!options.sessionAccountRef) throw new Error('session smoke requires --session-account-ref');
  if (!isAccountRef(options.sessionAccountRef)) throw new Error('--session-account-ref must be a valid accountRef');
...
function usesApiEndpointPreparation(options = {}) {
  return Boolean(options.nodeJoinUrl);
}
...
async function waitForSessionRunCompletion(input = {}, deps = {}) {
  ...
  while (Date.now() < deadline) {
    const events = await fetchRunEvents(endpoint, input.managementKey, {
      nodeId: input.nodeId,
      runId: input.runId,
      cursor
    }, deps).catch((error) => ({
...
async function runRemoteNodeSessionSmoke(input = {}, deps = {}) {
  const {
    controlEndpoint,
    clientEndpoint,
    managementKey,
  ...
  try {
    while (Date.now() < deadline) {
      const events = await fetchRunEvents(apiEndpoint, managementKey, {
        nodeId: options.nodeId,
        runId,
        cursor
      }, deps);
      ...
      eventCounts = mergeEventCounts(eventCounts, batch);
      terminalTail = appendEventText(terminalTail, batch);
      const artifacts = await fetchNewArtifacts(apiEndpoint, managementKey, {
        nodeId: options.nodeId,
        events: batch,
        seenArtifactIds
      }, deps);
    ...
    quit = await postRunInput(apiEndpoint, managementKey, {
      nodeId: options.nodeId,
      runId,
      input: '/quit',
      appendNewline: true
    ...
    if (!cleanup.completed) {
      const abort = await postRunAbort(apiEndpoint, managementKey, {
        nodeId: options.nodeId,
        runId
      }, deps).catch((error) => ({
        status: 0,
    ...
    if (!quit && runId) {
      await postRunInput(controlEndpoint, managementKey, {
        nodeId: options.nodeId,
        runId,
        input: '/quit',
        appendNewline: true
...
function prepareStores(input) {
  ...
  return {
    node,
    managementKey: String(controlManagementKey || '').trim()
  };
}
...
async function runExistingEndpointRelaySmoke(rawOptions = {}, deps = {}) {
  ...
  const clientManagementKey = String(
    options.managementKey || (nodeManagement.source === 'server-config' ? nodeManagement.key : '')
  ).trim();
  if (!clientManagementKey) {
  ...
  try {
    const prepared = usesApiEndpointPreparation(options)
      ? await prepareExistingEndpointViaApi({
        ...
        managementKey: clientManagementKey,
        deps
      })
    ...
    managementKey = prepared.managementKey;
    ...
    if (hasSessionSmokeOptions(options)) {
      session = await runRemoteNodeSessionSmoke({
        controlEndpoint,
        clientEndpoint,
        managementKey,
        options

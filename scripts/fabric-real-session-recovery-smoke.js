...
Usage:
  AIH_FABRIC_BROKER_TOKEN=<token> node scripts/fabric-real-session-recovery-smoke.js [options]
...
Options:
  ...
  --session-account <id>   Native provider account id, default 1.
...
function parseArgs(argv = [], env = process.env) {
  ...
  for (let index = 0; index < argv.length;) {
    ...
    if (token === '--token-file' || token.startsWith('--token-file=')) {
      ...
      options.tokenFile = String(next.value || '').trim();
      index += next.consumed;
      continue;
    }
...
async function waitForDeviceNodeNotOnline(endpoint, token, nodeId, timeoutMs, deps = {}) {
  const deadline = Date.now() + timeoutMs;
  let latest = null;
  while (Date.now() < deadline) {
    latest = await fetchJson(`${endpoint}/v0/node-rpc/device-nodes`, {
...
async function requestDeviceApi(state, pathName, options = {}) {
...
async function startSession(state, options) {
  ...
  const response = await requestDeviceApi(state, '/v0/node-rpc/device-node-session-start', {
    method: 'POST',
    phase: 'start',
    timeoutMs: 15000,
    body: {
...
async function ackSession(state, cursor, phase = 'ack') {
  return requestDeviceApi(state, '/v0/node-rpc/device-node-session-ack', {
    method: 'POST',
    phase,
    body: {
      nodeId: state.nodeId,
...
async function attachSession(state, cursor, phase = 'attach') {
  return requestDeviceApi(state, '/v0/node-rpc/device-node-session-attach', {
    method: 'POST',
    phase,
    body: {
      nodeId: state.nodeId,
...
async function sendMessage(state, options) {
  ...
  return requestDeviceApi(state, '/v0/node-rpc/device-node-session-command', {
    method: 'POST',
    phase: 'message',
    body: {
      nodeId: state.nodeId,
...
async function interruptBroker(state, options, token, connect) {
...
async function runSessionRecoverySmoke(rawOptions = {}, deps = {}) {
  ...
  options.nodeId = normalizeNodeId(options.nodeId);
  options.interrupt = normalizeInterruptKind(options.interrupt);
  const token = readBrokerToken(options, deps);
  ...
  try {
    ...
    state.deviceToken = prepared.deviceToken;
    state.relayHandle = spawnProcess(
      'relay-client',
      hostHomeDir,
      createRelayArgs(options.localUrl, options.nodeId, nodeManagement),
    ...
    const interruption = options.interrupt === 'relay'
      ...
      : await interruptBroker(state, options, token, connect);
    ...
    const report = {
      ...
      preparation: prepared ? { devicePaired: Boolean(prepared.deviceToken) } : null,
      children: [state.relayHandle].filter(Boolean).map(summarizeChild)
    };
    const diagnosticsFile = writeDiagnosticsFile(options.diagnosticsFile, report, deps);
    if (diagnosticsFile) report.diagnostics.file = diagnosticsFile;
    ...
    if (!stop && state.runId && state.deviceToken) {
...
async function main() {
  ...
  try {
    ...
    console.error('Usage: node scripts/fabric-real-session-recovery-smoke.js [--broker-endpoint URL] [--token-file PATH]');
    process.exitCode = 1;
    return;
  }
  if (options.help) {

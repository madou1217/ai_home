...
Options:
  ...
  --management-key-file <path>
                          Remote file containing the Server Management Key (AIH_MANAGEMENT_KEY).
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--management-key-file' || arg.startsWith('--management-key-file=')) {
      if (arg.startsWith('--management-key-file=')) {
        options.managementKeyFile = arg.slice('--management-key-file='.length);
        continue;
      }
      if (!next) throw new Error('--management-key-file requires a value');
      options.managementKeyFile = next;
      index += 1;
      continue;
    }
    if (arg === '--broker-token-file' || arg.startsWith('--broker-token-file=')) {
      if (arg.startsWith('--broker-token-file=')) {
        options.managementKeyFile = arg.slice('--broker-token-file='.length);
        continue;
      }
  ...
  if (options.managementKeyFile && !path.posix.isAbsolute(options.managementKeyFile)) {
    throw new Error('--management-key-file must be an absolute remote path');
  }
  return options;
}
...
function buildRemoteEnvCommand(options) {
  ...
  if (options.managementKeyFile) {
    commands.push(
      `test -s ${shQuote(options.managementKeyFile)}`,
      `export AIH_MANAGEMENT_KEY="$(cat ${shQuote(options.managementKeyFile)})"`
    );
  }
  if (nodeHome) {

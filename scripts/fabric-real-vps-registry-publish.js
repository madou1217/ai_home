...
Usage:
  AIH_MANAGEMENT_KEY=<key> node scripts/fabric-real-vps-registry-publish.js --port 18482 --node-id vps-152-jp-v4 --name "VPS 152 Japan" --bandwidth-kbps 3072
...
Options:
  ...
  --management-key <key>   Server Management Key. Prefer AIH_MANAGEMENT_KEY to avoid argv leaks.
  -h, --help              Show this help.

The script authenticates with the server Management Key, runs
"aih fabric registry publish --from-server --relay-node", sends heartbeat,
runs a finite foreground registry agent smoke, reads registry and legacy node
views back, and prints a sanitized JSON report. It never prints the Management Key.
`);
}
...
function createManagementContext(options, deps = {}) {
  ...
  return { endpoint, aiHomeDir, managementKey };
}
...
async function runRealVpsRegistryPublish(options, deps = {}) {
  ...
  let agentProbeTransports = Array.isArray(options.agentProbeTransports) && options.agentProbeTransports.length > 0
    ...
    : [];
  const managementContext = createManagementContext(options, deps);
  const endpoint = managementContext.endpoint;
  ...
  const cli = spawn(execPath, [
    ...
    env: {
      ...process.env,
      AIH_MANAGEMENT_KEY: managementContext.managementKey
    },
    encoding: 'utf8'
  });
  ...
  const registry = await readJson(`${endpoint}/v0/fabric/registry`, managementContext.managementKey, deps.fetchImpl);
  const legacy = await readJson(`${endpoint}/v0/node-rpc/device-nodes`, managementContext.managementKey, deps.fetchImpl);
  ...
  return {
    ...
    name: options.name,
    managementAuth: 'verified',
    aiHomeDir: managementContext.aiHomeDir,

...
Authentication:
  Set AIH_MANAGEMENT_KEY in the environment. The key is sent only in the
  Authorization header and is never written to the diagnostics report.
...
function parseArgs(argv = [], env = process.env) {
  const options = {
    ...
    peerId: '',
    headed: false,
    managementKey: String((env && env.AIH_MANAGEMENT_KEY) || '').trim()
  };
...
async function createSignalingRoom(endpoint, deps = {}) {
  const response = await fetchJson(`${endpoint}/v0/fabric/webrtc/signaling/rooms`, {
    method: 'POST',
    headers: {
      ...(deps.managementKey ? { authorization: `Bearer ${deps.managementKey}` } : {}),
...
async function readRoomMessages(endpoint, roomId, deps = {}) {
  const response = await fetchJson(`${endpoint}/v0/fabric/webrtc/signaling/rooms/${encodeURIComponent(roomId)}/messages?since=0&limit=200`, {
    headers: deps.managementKey ? { authorization: `Bearer ${deps.managementKey}` } : {},
    timeoutMs: 10000,
    fetchImpl: deps.fetchImpl
  });
...
async function webrtcPeerEvaluate(input) {
  ...
  const authorizationHeaders = input.managementKey
    ? { authorization: `Bearer ${String(input.managementKey)}` }
    : {};
...
async function runWebrtcDatachannelSmoke(options = {}, deps = {}) {
  ...
  const signalingDeps = { ...deps, managementKey: options.managementKey };
  const room = await createSignalingRoom(options.endpoint, signalingDeps);
  ...
  try {
    ...
    const roomMessages = await readRoomMessages(options.endpoint, room.roomId, signalingDeps);
    await context.close();
...
async function runWebrtcCreateRoom(options = {}, deps = {}) {
  const room = await createSignalingRoom(options.endpoint, {
    ...deps,
    managementKey: options.managementKey
  });
...
async function runWebrtcPeerSmoke(options = {}, deps = {}) {
  ...
  try {
    ...
    const roomMessages = await readRoomMessages(options.endpoint, options.roomId, {
      ...deps,
      managementKey: options.managementKey
    });
    await context.close();

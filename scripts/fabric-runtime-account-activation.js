...
Options:
  ...
  --apply                 Run remote dry-run first, import accounts into app-state.db, then clear stale runtime blocks.
...
function buildRemoteManagementReloadCommand(options) {
  const script = [
    "const fs=require('node:fs')",
    "const path=require('node:path')",
    "const {readServerConfig}=require('./lib/server/server-config-store')",
    "const root=String(process.env.AIH_HOST_HOME||process.env.HOME||'')",
    "const aiHomeDir=path.join(root,'.ai_home')",
    "const key=String(readServerConfig({fs,aiHomeDir}).managementKey||'').trim()",
...
function buildRemoteRuntimeBlockClearCommand(options) {
  const script = [
    ...
    "if(!runtimeState){skipped.push({provider,accountRef,reason:'runtime_state_empty'});continue}",
    "const ok=service.clearRuntimeBlock(accountRef,provider,{status:String(row.status||'up').trim()||'up',configured:row.configured!==false,apiKeyMode:Boolean(row.apiKeyMode),authMode:String(row.authMode||'').trim(),displayName:String(row.displayName||'').trim(),evidence:'manual_admin_clear'})",
    "if(ok)cleared.push({provider,accountRef,reason:'manual_admin_clear'})",
    "else skipped.push({provider,accountRef,reason:'clear_rejected'})",
    "}",
    "}",
...
function buildRemoteRuntimeAuditCommand(options) {
  const script = [
    ...
    "const root=String(process.env.AIH_HOST_HOME||process.env.HOME||'')",
    "const aiHomeDir=path.join(root,'.ai_home')",
    "const dbFile=path.join(aiHomeDir,'app-state.db')",
    `const providers=${JSON.stringify(options.providers)}`,
    ...
    "const report={ok:true,mode:'remote-runtime-audit',readOnly:true,target:{remoteDir,aiHomeDir,dbFile,deployedGitHead:readText(path.join(remoteDir,'DEPLOYED_GIT_HEAD'))},providers:[],summary:{providers:providers.length,accountCount:0,stateRows:0,configured:0,runtimeBlocked:0,clearableRuntimeBlocks:0,dbPresent:fs.existsSync(dbFile)}}",
    "let db=null",
    "let accountRows=[]",
    "let stateRows=[]",
    "if(fs.existsSync(dbFile)){db=new DatabaseSync(dbFile,{readOnly:true});const placeholders=providers.map(()=>'?').join(',');if(providers.length>0&&tableExists(db,'account_refs'))accountRows=db.prepare(`SELECT provider, account_ref FROM account_refs WHERE provider IN (${placeholders}) ORDER BY provider, account_ref`).all(...providers);if(providers.length>0&&tableExists(db,'account_state'))stateRows=db.prepare(`SELECT provider, account_ref, status, configured, api_key_mode, auth_mode, runtime_state, updated_at FROM account_state WHERE provider IN (${placeholders}) ORDER BY provider, account_ref`).all(...providers)}",
    "for(const provider of providers){",
    "const providerAccounts=accountRows.filter((row)=>String(row.provider||'')===provider)",
    "const providerRows=stateRows.filter((row)=>String(row.provider||'')===provider)",
    "const stateRefSet=new Set(providerRows.map((row)=>String(row.account_ref||'')).filter(Boolean))",
    ...
    "if(runtimeState)clearable.push(String(row.account_ref||''))",
    "}",
    "const missingStateAccountRefs=providerAccounts.map((row)=>String(row.account_ref||'')).filter((ref)=>ref&&!stateRefSet.has(ref))",
    "report.providers.push({provider,accountCount:providerAccounts.length,stateRows:providerRows.length,configured,apiKeyMode,runtimeBlocked,clearableRuntimeBlocks:clearable.length,runtimeStatusCounts:toRows(runtimeStatusCounts),runtimeReasonCounts:toRows(runtimeReasonCounts),authModeCounts:toRows(authModeCounts),sampleClearableAccountRefs:clearable.slice(0,5),missingStateAccountRefs:missingStateAccountRefs.slice(0,10)})",
    "report.summary.accountCount+=providerAccounts.length;report.summary.stateRows+=providerRows.length;report.summary.configured+=configured;report.summary.runtimeBlocked+=runtimeBlocked;report.summary.clearableRuntimeBlocks+=clearable.length",
    "}",
    "if(db)db.close()",
...
function buildRemoteRegistryPublishCommand(options) {
  const script = [
    ...
    "const storedManagementKey=readRegistryAgentManagementKey(nodeId,{fs,aiHomeDir})",
    "const managementKey=String(readServerConfig({fs,aiHomeDir}).managementKey||'').trim()",
    "if(!storedManagementKey){console.log(JSON.stringify({ok:false,error:'management_key_missing'}));process.exit(2)}",
    "if(!managementKey){console.log(JSON.stringify({ok:false,error:'management_key_missing'}));process.exit(2)}",
    "const env={...process.env,AIH_MANAGEMENT_KEY:storedManagementKey||managementKey}",
...
function summarizeSessionStartResult(provider, marker, result) {
  ...
  return {
    ...
    accountRef: String(bodyResult.accountRef || ''),
...
function summarizeProviderAttempt(item) {
  ...
  return {
    ...
    accountRef: String(item && item.accountRef || ''),
...
function printReport(report, json) {
  ...
  if (report.remoteAudit) {
    ...
    console.log(`[fabric-runtime-account-activation] remote_audit deployed=${target.deployedGitHead || ''} db_present=${Boolean(summary.dbPresent)} state_rows=${Number(summary.stateRows) || 0} accounts=${Number(summary.accountCount) || 0} runtime_blocked=${Number(summary.runtimeBlocked) || 0} clearable=${Number(summary.clearableRuntimeBlocks) || 0}`);
    (Array.isArray(report.remoteAudit.providers) ? report.remoteAudit.providers : []).forEach((item) => {
      const reasons = (Array.isArray(item.runtimeReasonCounts) ? item.runtimeReasonCounts : [])
        ...
        .join(',');
      console.log(`[fabric-runtime-account-activation] remote_provider ${item.provider} accounts=${Number(item.accountCount) || 0} state_rows=${Number(item.stateRows) || 0} runtime_blocked=${Number(item.runtimeBlocked) || 0} clearable=${Number(item.clearableRuntimeBlocks) || 0}${reasons ? ` reasons=${reasons}` : ''}`);
    });
  ...
  try {
    ...
    if (revalidateOnly) {
      ...
      for (const provider of options.providers) {
        const maxSessionRuns = getProviderAccountCount(report.postClearAudit, provider) || 1;

#!/usr/bin/env node
/**
 * session-reader.js 按域拆分辅助脚本（一次性工具，用完即弃）
 *
 * 输入: lib/sessions/session-reader.js
 * 输出:
 *   - lib/sessions/session-reader-utils.js    共享纯工具
 *   - lib/sessions/session-reader-codex.js    codex 域
 *   - lib/sessions/session-reader-claude.js   claude 域
 *   - lib/sessions/session-reader-gemini.js   gemini 域
 *   - lib/sessions/session-reader-opencode.js opencode 域
 *   - lib/sessions/session-reader-other.js    agy/qoder/grok/kiro/zcode 域
 *   - 主文件保留: 编排层(readSessionMessages/Events/…)、缓存状态、常量、公共导出
 *
 * 安全约束:
...
const SRC = path.join(__dirname, '..', 'lib', 'sessions', 'session-reader.js');
...
const orchestrationNames = new Set([
  ...
  'readAccountScopedProjects',
  'resolveSessionFilePath',
  'resolveSessionResourceContext',
  'readSessionMessages',
...
'readSessionEvents',
  ...
  'canonicalizeSessionMessages',
  'readCachedSessionMessages',
  'touchSessionMessageCache',
  'pruneSessionMessageCache',
  'estimateSessionMessagesMemoryBytes',
  'getFileStatFingerprint',
  'getCodexStateCacheVersion',
  'getCodexDescriptorCacheVersion',
  'dependsOnCodexState',
  'readOpenCodeSessionModel',
  'collectKnownProjectPaths',
  'readCodexSessionEvents',
]);

// provider 域: 名称含 provider 关键字即归域
const domainKeywords = [
  { domain: 'codex', kw: /Codex/ },
  { domain: 'claude', kw: /Claude/ },
  { domain: 'gemini', kw: /Gemini/ },
  { domain: 'opencode', kw: /OpenCode/ },
  { domain: 'other', kw: /(^|[A-Z])(Agy|Qoder|Grok|Kiro|Zcode)/ },
];
// 域强制归属: 名称无关键字但实现属于特定 provider 基础设施
const forcedDomainNames = {
  collectCodexWorkspaceRoots: 'codex',
};

// 共享工具强制白名单: 无论被谁引用都留在 utils(通用基础能力)
const forcedUtilsNames = new Set([
  'forEachJsonlLineSync',
  'forEachJsonlLineRangeSync',
  'forEachJsonlLineSyncFromOffset',
  'getDatabaseSyncCtor',
]);
...
while (changed && iterations < 50) {
  ...
  for (const seg of segments) {
    ...
    if (dom !== 'codex' && dom !== 'claude' && dom !== 'gemini' && dom !== 'opencode' && dom !== 'other') continue;
...
// utils 函数引用 provider 域函数 → 不允许
for (const seg of segments) {
  if (domainOf(seg.name) !== 'utils') continue;
  for (const ref of refsOf[seg.name] || []) {
    const refDom = domainOf(ref);
    if (refDom === 'codex' || refDom === 'claude' || refDom === 'gemini' || refDom === 'opencode' || refDom === 'other' || refDom === 'orchestration') {
      problems.push(`utils ${seg.name} -> provider ${ref} (${refDom})`);
    }
  }
}
...
// ---------- 5. 生成各域文件 ----------
const domainOrder = ['utils', 'codex', 'claude', 'gemini', 'opencode', 'other'];
const outDir = path.join(__dirname, '..', 'lib', 'sessions');
...
for (const dom of domainOrder) {
  ...
  const utilsRequire = utilsRefs.length
    ? `const { ${utilsRefs.join(', ')} } = require('./session-reader-utils');`
    : '';
  files[dom] = {
...
const requireLines = domainOrder
  ...
  .map((d) => `const { ${files[d].members.join(', ')} } = require('./session-reader-${d}');`)
  .join('\n');
const utilsMembers = files.utils ? files.utils.members : [];
if (utilsMembers.length) {
  newMain += `const { ${utilsMembers.join(', ')} } = require('./session-reader-utils');\n`;
}
if (requireLines) newMain += requireLines + '\n';

// 保留段落(原样)
keepSegments.forEach((seg) => {
  newMain += lines.slice(seg.line - 1, seg.end).join('\n') + '\n';
});
...
for (const [dom, file] of Object.entries(files)) {
  const outPath = path.join(outDir, `session-reader-${dom}.js`);

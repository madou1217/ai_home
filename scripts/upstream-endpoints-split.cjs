#!/usr/bin/env node
/**
 * upstream-endpoints.js 按功能域拆分辅助脚本（一次性工具，用完即弃）
 *
 * 输入: lib/server/upstream-endpoints.js
 * 输出:
 *   - lib/server/upstream-endpoints-utils.js    共享纯工具/常量
 *   - lib/server/upstream-endpoints-models.js   模型列表域
 *   - lib/server/upstream-endpoints-path.js     上游 URL/路径解析域
 *   - lib/server/upstream-endpoints-headers.js  转发响应头域
 *   - lib/server/upstream-endpoints-sse.js      SSE 流处理域
 *   - lib/server/upstream-endpoints-usage.js    用量记录域
 *   - lib/server/upstream-endpoints-failure.js  失败策略域
 *   - lib/server/upstream-endpoints-transport.js 传输选择域
 *   - 主文件保留: 编排层(handleUpstreamModels/Passthrough)、常量、公共导出
 *
 * 安全约束:
...
const SRC = path.join(__dirname, '..', 'lib', 'server', 'upstream-endpoints.js');
...
const domainKeywords = [
  { domain: 'models', kw: /ModelList|ProviderByAccountRef/ },
  { domain: 'path', kw: /BaseUrl|Path|Url|QueryAndHash|MessagesPath|GenerateContentPath/ },
  { domain: 'headers', kw: /Header|ForwardHeader|RawUpstreamResponse/ },
  { domain: 'sse', kw: /Sse|SSE|Canonical|AsyncIterator|GeminiGenerateContentPiece|GeminiGenerateContentStream|CanonicalEventStream|TapCanonical|EmptyUpstreamResponseError|NonEmpty/ },
  { domain: 'usage', kw: /Usage|Payload|FirstObject|GetPayloadModel|JsonPayloads/ },
  { domain: 'failure', kw: /Failure|NetworkFailure|FinishReason|NoModelAccount|AliasRuntime|PolicyToAccount|TokenRefreshUnavailable/ },
  { domain: 'transport', kw: /Transport/ },
];
// 域强制归属: 名称无关键字但实现属于特定功能域
const forcedDomainNames = {
  buildProviderByAccountRef: 'models',
  buildModelListEntries: 'models',
  resolveProviderUpstream: 'path',
  baseUrlEndsWithPath: 'path',
  resolveProviderPath: 'path',
  isAnthropicCompatibleBaseUrl: 'path',
  stripUrlQueryAndHash: 'path',
  isClaudeMessagesPath: 'path',
  isOpenAIChatCompletionsPath: 'path',
  isGeminiGenerateContentPath: 'path',
  isGeminiStreamGenerateContentPath: 'path',
  shouldSkipForwardHeader: 'headers',
  normalizeHeaderValue: 'headers',
  isSafeHeaderValue: 'headers',
  sanitizeAccessToken: 'headers',
  sendRawUpstreamResponse: 'headers',
  writeGeneralUpstreamResponseHeaders: 'headers',
  writeUpstreamSseHeaders: 'headers',
  writeCodeAssistAnthropicSseHeaders: 'sse',
  writeCanonicalEventsAsAnthropicSse: 'sse',
  writeCanonicalEventStreamAsAnthropicSse: 'sse',
  isNonEmptyCanonicalAssistantEvent: 'sse',
  createEmptyUpstreamResponseError: 'sse',
  closeAsyncIterator: 'sse',
  replayPrimedAsyncIterator: 'sse',
  isNonEmptyGeminiGenerateContentPiece: 'sse',
  requireNonEmptyGeminiGenerateContentStream: 'sse',
  requireNonEmptyCanonicalEventStream: 'sse',
  tapCanonicalEventStream: 'sse',
  firstObject: 'usage',
  parseJsonPayloads: 'usage',
  parseSseJsonPayloads: 'usage',
  getPayloadModel: 'usage',
  extractModelUsageInputFromPayload: 'usage',
  extractModelUsageInputFromRaw: 'usage',
  recordSuccessfulModelUsage: 'usage',
  createModelUsageCapture: 'usage',
  describeUpstreamError: 'failure',
  isGlobalNetworkFailure: 'failure',
  withNetworkHint: 'failure',
  mapGeminiFinishReason: 'failure',
  selectAccountsForRequestModel: 'failure',
  writeNoModelAccountResponse: 'failure',
  shouldDeferAliasRuntimeFailure: 'failure',
  createAliasRuntimeFailureResult: 'failure',
  applyFailurePolicyToAccount: 'failure',
  createTokenRefreshUnavailablePolicy: 'failure',
  shouldUseCodeAssistAnthropicDirectTransport: 'transport',
  shouldUseOpenCodeGoApiTransport: 'transport',
};
...
const DOMAINS = new Set(['models', 'path', 'headers', 'sse', 'usage', 'failure', 'transport']);
...
// ---------- 5. 生成各域文件 ----------
const domainOrder = ['utils', 'models', 'path', 'headers', 'sse', 'usage', 'failure', 'transport'];
...
for (const dom of domainOrder) {
  ...
  const utilsRequire = utilsRefs.length
    ? `const { ${utilsRefs.join(', ')} } = require('./upstream-endpoints-utils');`
    : '';
  files[dom] = {
...
const requireLines = domainOrder
  ...
  .map((d) => `const { ${files[d].members.join(', ')} } = require('./upstream-endpoints-${d}');`)
  .join('\n');
const utilsMembers = files.utils ? files.utils.members : [];
if (utilsMembers.length) {
  newMain += `const { ${utilsMembers.join(', ')} } = require('./upstream-endpoints-utils');\n`;
}
...
for (const [dom, file] of Object.entries(files)) {
  const outPath = path.join(outDir, `upstream-endpoints-${dom}.js`);

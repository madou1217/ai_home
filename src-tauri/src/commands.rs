...
use crate::{
    ...
    server_discovery::{run_server_discovery, ServerDiscoveryInput, ServerDiscoveryResponse},
    server_http::{
        DesktopFrpRouteConfigureInput, DesktopHttpResponse, DesktopLanProfileAuthorizeInput,
        DesktopLanRoutesRefreshInput, DesktopManagementKeyRotateInput,
        DesktopOutboundRelayConfigureInput, DesktopRelayRouteTrustInput, DesktopRequestInput,
        DesktopStreamInput,
    },
...
#[tauri::command]
pub async fn desktop_discover_servers(
    input: ServerDiscoveryInput,
) -> NativeResult<ServerDiscoveryResponse> {
    run_blocking(move || run_server_discovery(input)).await
}

#[tauri::command]
pub async fn desktop_outbound_relays_configure(
    input: DesktopOutboundRelayConfigureInput,
    state: State<'_, AppState>,
) -> NativeResult<Value> {
    state.http.configure_outbound_relays(input).await
}

#[tauri::command]
pub async fn desktop_frp_route_configure(
    input: DesktopFrpRouteConfigureInput,
    state: State<'_, AppState>,
) -> NativeResult<Value> {
    state.http.configure_frp_route(input).await
}

#[tauri::command]
pub async fn desktop_relay_route_trust(
    input: DesktopRelayRouteTrustInput,
    state: State<'_, AppState>,
) -> NativeResult<Value> {
    state.http.trust_relay_route(input).await
}

#[tauri::command]
pub async fn desktop_lan_profile_authorize(
    input: DesktopLanProfileAuthorizeInput,
    state: State<'_, AppState>,
    app: AppHandle,
) -> NativeResult<ProfileResponse> {
    crate::tray::invalidate_before_profile_change();
    let profile = state.http.authorize_lan_profile(input).await?;
    crate::tray::refresh_after_profile_change(app);
    Ok(ProfileResponse {
        profile: Some(profile),
    })
...
#[tauri::command]
pub async fn desktop_lan_routes_refresh(
    input: DesktopLanRoutesRefreshInput,
    state: State<'_, AppState>,
) -> NativeResult<Value> {
    state.http.refresh_lan_routes(input).await
}

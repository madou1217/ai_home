...
fn normalize_endpoint_with_private_lan(
    value: &str,
    allow_private_lan_http: bool,
) -> NativeResult<String> {
    ...
    match url.scheme() {
        ...
        "http" if allow_private_lan_http && is_private_lan_host(host) => {}
        "http" => {
            return Err(NativeError::new(
                "insecure_endpoint",
                "远程 Server 必须使用 HTTPS；HTTP 仅允许已验证的私有局域网地址。",
                false,
            ));
        }
        _ => {
...
pub fn normalize_endpoint(value: &str) -> NativeResult<String> {
    normalize_endpoint_with_private_lan(value, false)
}

pub fn normalize_trusted_endpoint(value: &str) -> NativeResult<String> {
...
pub fn build_request_url(endpoint: &str, relative_path: &str) -> NativeResult<Url> {
    ...
    // trusted route from Keychain. Manual profile writes still go through the
    // stricter `normalize_endpoint` path.
    let normalized_endpoint = normalize_trusted_endpoint(endpoint)?;
...
mod tests {
    ...
    #[test]
    fn trusted_endpoint_allows_private_ip_http_only() {
        assert_eq!(
            normalize_trusted_endpoint("http://192.168.1.20:9527/").unwrap(),
            "http://192.168.1.20:9527"
        );
        assert_eq!(
            normalize_trusted_endpoint("http://10.0.0.8:9527").unwrap(),
            "http://10.0.0.8:9527"
        );
        assert!(normalize_trusted_endpoint("http://203.0.113.8:9527").is_err());
        assert!(normalize_trusted_endpoint("http://lan-host.local:9527").is_err());
    }

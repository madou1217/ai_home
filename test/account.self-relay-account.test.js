...
test('deleteSelfRelayAccounts deletes DB-backed AIH relays and keeps external localhost proxies', () => {
  ...
  try {
    ...
    assert.deepEqual(reconciled.sort(), [`claude/${claudeRelayRef}`, `codex/${codexRelayRef}`].sort());
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

test('deleteSelfRelayAccounts fails closed when provider resources are unresolved', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-self-relay-fail-closed-'));
  try {
    const aiHomeDir = path.join(root, '.ai_home');
    const accountRef = registerApiKeyAccount(aiHomeDir, 'codex', '7', {
      OPENAI_API_KEY: 'dummy',
      OPENAI_BASE_URL: 'http://127.0.0.1:8317/v1'
    });
    const runtimeDir = resolveAccountRuntimeDir(aiHomeDir, 'codex', accountRef);
    const resourcePath = path.join(runtimeDir, '.codex', 'sessions', 'late.jsonl');
    ...
    assert.equal(resolveAccountRef(fs, aiHomeDir, accountRef).accountRef, accountRef);

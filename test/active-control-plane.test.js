...
function createDeviceFetch(calls) {
  return async (url, init) => {
    ...
    if (requestUrl.endsWith('/v0/node-rpc/device-profile')) {
      return {
        ok: true,
        status: 200,
        json: async () => ({
          ...
          result: {
            device: {
              ...
              name: 'Phone',
              platform: 'ios',
              scopes: ['control-plane:read', 'nodes:read', 'accounts:read', 'sessions:read'],
              state: 'paired'
            },
...
test('active control plane context reports readiness and unavailable reasons', () => {
  ...
  const profiles = [
    createProfile('cp-unpaired'),
    createProfile('cp-ready', {
      state: 'paired',
      authState: 'paired',
      deviceToken: 'device-token'
    }),
    createProfile('cp-revoked', {
      state: 'revoked',
      authState: 'paired',
  ...
  assert.equal(
    activeControlPlane.resolveActiveControlPlaneContext(profiles, 'cp-unpaired').reason,
    'unpaired'
  );
  assert.equal(
    activeControlPlane.resolveActiveControlPlaneContext(profiles, 'cp-revoked').reason,
    'revoked'
  );
...
test('active control plane context skips a revoked stored server when another paired server exists', () => {
  ...
  selection.setActiveControlPlaneProfileId('cp-revoked', global.window.localStorage);

  const resolved = activeControlPlane.resolveActiveControlPlaneContext(profiles);

  assert.equal(resolved.profileId, 'cp-ready');
  assert.equal(resolved.profile.id, 'cp-ready');
...
test('active control plane refreshes selected device state with bearer token', async () => {
  global.window = { localStorage: createStorage() };
...
test('active control plane marks selected profile degraded when sync fails', async () => {
  ...
  assert.equal(degraded.deviceToken, 'device-token');
  assert.equal(degraded.lastError, 'control_plane_device_http_503');
  delete global.window;
});
...
test('active control plane reads remote node summaries without full device refresh', async () => {
  ...
  assert.equal(updated.sessionCount, 12);
  assert.equal(updated.lastError, '');
  assert.ok(updated.lastDeviceSyncAt > 0);
  delete global.window;
});

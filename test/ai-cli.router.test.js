...
test('`aih codex 7 down` updates account operational status', () => {
  const exits = [];
  const logs = [];
  const statusCalls = [];
  const originalLog = console.log;
  ...
  assert.deepEqual(statusCalls, [{ cliName: 'codex', id: '7', status: 'down' }]);
  assert.deepEqual(exits, [0]);
  assert.equal(logs.some((line) => line.includes('status set to down')), true);
});
...
test('`aih codex set-mobile` accepts api-key accounts', (t) => {
  ...
  const state = JSON.parse(fs.readFileSync(path.join(aiHomeDir, 'codex-desktop-hook-state.json'), 'utf8'));
  assert.deepEqual(exits, [0]);
  assert.equal(state.desktopAccountId, '10');
  assert.equal(logs.some((line) => line.includes('Set Codex Account ID 10 as Codex App account')), true);
});
...
test('`aih codex set-mobile` rejects accounts without usable auth', (t) => {
  ...
  assert.deepEqual(exits, [1]);
  assert.equal(errors.some((line) => line.includes('Codex App account requires a usable API Key or ChatGPT OAuth account')), true);

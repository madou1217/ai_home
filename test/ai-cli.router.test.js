...
function registerTestAccount(aiHomeDir, provider, cliAccountId, options = {}) {
  const accountRef = registerAccountIdentity(fs, aiHomeDir, {
    provider,
    cliAccountId,
    identitySeed: options.identitySeed || `test:ai-cli-router:${provider}:${cliAccountId}`
  }).accountRef;
  if (options.credentials !== false) {
    writeAccountCredentials(fs, aiHomeDir, accountRef, options.credentials || { AIH_TEST_CONFIGURED: '1' });
  }
...
function createRegisteredTestHome(t, provider = 'codex', cliAccountId = '12', options = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-ai-cli-router-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const aiHomeDir = path.join(root, '.ai_home');
  const profilesDir = path.join(aiHomeDir, 'profiles');
  const accountRef = registerTestAccount(aiHomeDir, provider, cliAccountId, options);
  return { root, aiHomeDir, profilesDir, accountRef };
}

function createRegisteredTestAccounts(t, provider = 'codex', cliAccountIds = ['1']) {
  const ids = Array.isArray(cliAccountIds) && cliAccountIds.length > 0 ? cliAccountIds : ['1'];
  const fixture = createRegisteredTestHome(t, provider, ids[0]);
  const accountRefs = { [ids[0]]: fixture.accountRef };
  ids.slice(1).forEach((cliAccountId) => {
    accountRefs[cliAccountId] = registerTestAccount(
      fixture.aiHomeDir,
      provider,
      cliAccountId
    );
  });
  return { ...fixture, accountRefs };
}
...
function codexDesktopStatePath(aiHomeDir) {
  return path.join(aiHomeDir, 'run', 'codex', 'desktop-hook-state.json');
}
...
test('`aih codex` without explicit action uses built-in AIH server profile', () => {
  ...
  assert.deepEqual(runCalls, [{ cliName: 'codex', id: '', forwardArgs: [] }]);
});
...
test('`aih claude` without explicit action uses built-in AIH server profile', () => {
  ...
  assert.deepEqual(exits, []);
  assert.deepEqual(runCalls, [{ cliName: 'claude', id: '', forwardArgs: [] }]);
});
...
test('`aih codex home` reports the shared host runtime without launching native CLI', (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-router-home-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const aiHomeDir = path.join(root, '.ai_home');
  const gatewayRuntimeDir = root;
  fs.mkdirSync(path.join(gatewayRuntimeDir, '.codex'), { recursive: true });
  ...
  try {
    runAiCliCommandRouter('codex', ['codex', 'home'], {
      processImpl: {
        env: {
          HOME: path.join(aiHomeDir, 'run', 'auth-projections', 'codex', 'acct_00000000000000000000'),
      ...
      aiHomeDir,
      HOST_HOME_DIR: root,
      readServerConfig: () => ({ host: '127.0.0.1', port: 9527, apiKey: 'sk-test' }),
      getProfileDir: (_provider, _accountRef, runtime) => runtime && runtime.gateway ? gatewayRuntimeDir : '',
      runCliPty: () => {
  ...
  assert.equal(output.includes(`CODEX_HOME=${path.join(root, '.codex')}`), true);
  assert.equal(fs.existsSync(path.join(aiHomeDir, 'run', 'internal')), false);
});
...
test('`aih gemini 1 home` prints ID-first diagnostics without launching native CLI', (t) => {
  ...
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const aiHomeDir = path.join(root, '.ai_home');
  const accountRef = registerTestAccount(aiHomeDir, 'gemini', '1');
  const profileDir = path.join(aiHomeDir, 'run', 'auth-projections', 'gemini', accountRef);
  ...
  try {
    runAiCliCommandRouter('gemini', ['gemini', '1', 'home'], {
      ...
      aiHomeDir,
      HOST_HOME_DIR: root,
      getProfileDir: (_provider, resolvedRef) => resolvedRef === accountRef ? profileDir : '',
      runCliPty: () => {
        throw new Error('unexpected_native_launch');
      }
    });
...
test('`aih codex login --no-browser` starts a pending login without reserving a CLI id', () => {
  const exits = [];
  const calls = [];
  const prepared = [];
  runAiCliCommandRouter('codex', ['codex', 'login', '--no-browser'], {
    ...
    extractActiveEnv: () => null,
    getNextId: () => {
      throw new Error('pending_login_must_not_reserve_cli_account_id');
    },
    createAccount: (cliName, cliAccountId = '') => {
      prepared.push({ cliName, cliAccountId });
      return true;
    },
    runCliPty: (cliName, accountRef, forwardArgs, isLogin, runtimeOptions) => {
      calls.push({ cliName, accountRef, forwardArgs, isLogin, runtimeOptions });
    }
  });

  assert.deepEqual(exits, []);
  assert.deepEqual(prepared, [{ cliName: 'codex', cliAccountId: '' }]);
  assert.equal(calls.length, 1);
  assert.deepEqual({
    ...calls[0],
    runtimeOptions: {
      ...calls[0].runtimeOptions,
      loginSessionId: '<generated>'
    }
  }, {
    cliName: 'codex',
    accountRef: '',
  ...
  assert.match(calls[0].runtimeOptions.loginSessionId, /^auth-/);
});

test('`aih codex set-default` persists accountRef without touching the desktop client', (t) => {
  ...
  try {
    runAiCliCommandRouter('codex', ['codex', 'set-default', '12'], {
      ...
      aiHomeDir,
      HOST_HOME_DIR: root,
      ensureSessionStoreLinks: (cliName, ref) => sessionSyncCalls.push({ cliName, accountRef: ref }),
  ...
  assert.deepEqual(sessionSyncCalls, [{ cliName: 'codex', accountRef }]);
  assert.equal(readDefaultAccountRef(fs, aiHomeDir, 'codex'), accountRef);
  assert.deepEqual(exits, [0]);
...
test('`aih codex set-mobile` writes desktop accountRef without changing the default account', (t) => {
  const { root, aiHomeDir, profilesDir, accountRef } = createRegisteredTestHome(
    t,
    'codex',
    '10009',
    {
  ...
  const previousAccountRef = registerTestAccount(aiHomeDir, 'codex', '10001', {
    credentials: false
  });
  const statePath = codexDesktopStatePath(aiHomeDir);
  writeJson(statePath, {
    enabled: true,
    traceFile: '/tmp/trace.jsonl',
    traceResponses: true,
    desktopAccountRef: previousAccountRef
  });
  ...
  assert.deepEqual(exits, [0]);
  assert.equal(state.desktopAccountRef, accountRef);
...
test('`aih codex unset-mobile` clears current Codex App account without user account id', (t) => {
  const { root, aiHomeDir, profilesDir, accountRef } = createRegisteredTestHome(t, 'codex', '10009');
  writeDefaultAccountRef(fs, aiHomeDir, 'codex', accountRef);
  const statePath = codexDesktopStatePath(aiHomeDir);
  writeJson(statePath, {
    enabled: true,
...
test('`aih codex set-mobile` rejects api-key accounts', (t) => {
  const { root, aiHomeDir, profilesDir } = createRegisteredTestHome(t, 'codex', '10', {
    credentials: {
      OPENAI_API_KEY: 'sk-test'
    }
  });
  ...
  assert.equal(fs.existsSync(codexDesktopStatePath(aiHomeDir)), false);
});

test('`aih codex set-mobile` rejects accounts without usable auth', (t) => {
  const { root, aiHomeDir, profilesDir } = createRegisteredTestHome(t, 'codex', '11', {
    credentials: false
  });
...
test('`aih codex set-default --restart-client` restarts desktop client as best effort after sync', (t) => {
...
test('`aih codex set-default --restart-client` reminds user to open desktop app first when no learned path exists', (t) => {
  const { root, aiHomeDir, profilesDir } = createRegisteredTestHome(t);
  const exits = [];
  const logs = [];
  const originalLog = console.log;
...
test('`aih codex set-default --restart-client` reports direct launch when desktop client was not running', (t) => {
  const { root, aiHomeDir, profilesDir } = createRegisteredTestHome(t);
  const exits = [];
  const logs = [];
  const originalLog = console.log;
...
test('`aih codex <id> --no-browser` logs in the same account when unconfigured', (t) => {
  const { root, aiHomeDir, profilesDir, accountRef } = createRegisteredTestHome(t);
  const calls = [];
  runAiCliCommandRouter('codex', ['codex', '12', '--no-browser'], {
    ...
    runCliPty: (cliName, ref, forwardArgs, isLogin, runtimeOptions) => {
      calls.push({ cliName, accountRef: ref, forwardArgs, isLogin, runtimeOptions });
    }
  });
  assert.deepEqual(calls, [{
    ...
    runtimeOptions: {
      cliAccountId: '12',
      loginSessionId: calls[0].runtimeOptions.loginSessionId,
      gateway: false
    }
  }]);
  assert.match(calls[0].runtimeOptions.loginSessionId, /^auth-/);
...
test('`aih codex <id> --no-browser` starts a pending login when target account is already configured', (t) => {
  const { root, aiHomeDir, profilesDir } = createRegisteredTestHome(t);
  const calls = [];
  const prepared = [];
  const logs = [];
  ...
  assert.equal(logs.some((line) => line.includes('already logged in') && line.includes('Starting a new no-browser OAuth login')), true);
  assert.deepEqual(prepared, [{ cliName: 'codex', cliAccountId: '' }]);
  assert.deepEqual(calls, [{
    cliName: 'codex',
    accountRef: '',
...
test('`aih codex <id>` ignores host config directories when the DB account is missing', (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-router-no-host-dir-fallback-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const aiHomeDir = path.join(root, '.ai_home');
  writeJson(path.join(root, '.codex', 'auth.json'), { tokens: { access_token: 'host-token' } });
  ...
  console.log = () => {};
  try {
    runAiCliCommandRouter('codex', ['codex', '1', '--version'], {
      processImpl: {
        env: {},
        stdout: { isTTY: false },
        exit: (code) => exits.push(code)
      ...
      createAccount: () => {
        throw new Error('unexpected_account_creation');
      },
      extractActiveEnv: () => null,
      runCliPty: () => {
        throw new Error('unexpected_native_launch');
  ...
  assert.equal(prompts[0].includes('Create Account 1'), true);
  assert.equal(fs.existsSync(path.join(aiHomeDir, 'app-state.db')), false);
});

test('`aih codex <id>` can launch an account that only exists in DB credentials', (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-router-db-account-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));

  const aiHomeDir = path.join(root, '.ai_home');
  const profilesDir = path.join(aiHomeDir, 'profiles');
  const profileDir = path.join(profilesDir, 'codex', '12');
  const accountRef = registerTestAccount(aiHomeDir, 'codex', '12', { credentials: false });
  ...
  const calls = [];
  runAiCliCommandRouter('codex', ['codex', '12', '--version'], {
    processImpl: { exit: () => {} },
    fs,
    PROFILES_DIR: profilesDir,
    aiHomeDir,
    HOST_HOME_DIR: root,
    askYesNo: () => {
      throw new Error('unexpected_create_prompt');
    },
    extractActiveEnv: () => null,
    getProfileDir: () => profileDir,
    checkStatus: () => ({ configured: true, accountName: 'API Key Configured' }),
    runCliPty: (cliName, ref, forwardArgs, isLogin, runtimeOptions) => {
      calls.push({ cliName, accountRef: ref, forwardArgs, isLogin, runtimeOptions });
    }
  });
  ...
  assert.deepEqual(calls, [{
    ...
    forwardArgs: ['--version'],
    isLogin: false,
    runtimeOptions: { cliAccountId: '12', loginSessionId: '', gateway: false }
  }]);
});

test('`aih gemini login api_key` stores credentials in DB without .aih_env.json', (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-router-gemini-db-login-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));

  const aiHomeDir = path.join(root, '.ai_home');
  const profilesDir = path.join(aiHomeDir, 'profiles');
  const profileDir = path.join(profilesDir, 'gemini', '1');
  const answers = ['gemini-key', ''];
  const calls = [];

  runAiCliCommandRouter('gemini', ['gemini', 'login', 'api_key'], {
    processImpl: { exit: () => {} },
    fs,
    PROFILES_DIR: profilesDir,
    aiHomeDir,
    HOST_HOME_DIR: root,
    readLine: { question: () => answers.shift() || '' },
    getNextId: () => {
      throw new Error('api_key_login_must_allocate_with_registration');
    },
    createAccount: () => {
      throw new Error('api_key_login_must_not_prepare_an_unregistered_account');
    },
    extractActiveEnv: () => null,
    getProfileDir: () => profileDir,
    checkStatus: () => ({ configured: true, accountName: 'API Key Configured' }),
  ...
  assert.equal(calls.length, 1);
  assert.deepEqual(readAccountCredentials(fs, aiHomeDir, calls[0].accountRef), {
    GEMINI_API_KEY: 'gemini-key'
  });
  assert.equal(fs.existsSync(path.join(profileDir, '.aih_env.json')), false);
  assert.deepEqual(calls, [{
    cliName: 'gemini',
    accountRef: calls[0].accountRef,
    forwardArgs: [],
    isLogin: false,
    runtimeOptions: { cliAccountId: '1', loginSessionId: '', gateway: false }
  }]);
});

test('`aih agy login api_key` stores access token in DB without .aih_env.json', (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-router-agy-db-login-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));

  const aiHomeDir = path.join(root, '.ai_home');
  const profilesDir = path.join(aiHomeDir, 'profiles');
  const profileDir = path.join(profilesDir, 'agy', '1');
  const answers = ['agy-token', ''];
  const calls = [];

  runAiCliCommandRouter('agy', ['agy', 'login', 'api_key'], {
    processImpl: { exit: () => {} },
    fs,
    PROFILES_DIR: profilesDir,
    aiHomeDir,
    ...
    extractActiveEnv: () => null,
    getProfileDir: () => profileDir,
    checkStatus: () => ({ configured: true, accountName: 'Access Token Configured' }),
    runCliPty: (cliName, accountRef, forwardArgs, isLogin, runtimeOptions) => {
      calls.push({ cliName, accountRef, forwardArgs, isLogin, runtimeOptions });
    }
  });
...
test('`aih agy usage 1 --refresh` maps refresh to a no-cache single account query', async (t) => {
  const { aiHomeDir, profilesDir } = createRegisteredTestHome(t, 'agy', '1');
  const exits = [];
  const calls = [];
...
test('`aih codex usage <id> --no-cache` forwards noCache query option', async (t) => {
  const { aiHomeDir, profilesDir } = createRegisteredTestHome(t, 'codex', '12');
  const exits = [];
  const calls = [];
...
test('`aih codex <id> usage --no-cache` forwards noCache query option', async (t) => {
  const { aiHomeDir, profilesDir } = createRegisteredTestHome(t, 'codex', '12');
  const exits = [];
  const calls = [];
...
test('`aih agy usage <id> --preflight` forwards local preflight query option', async (t) => {
  const { aiHomeDir, profilesDir } = createRegisteredTestHome(t, 'agy', '3');
  const exits = [];
  const calls = [];
...
test('`aih agy <id> usage --preflight` forwards local preflight query option', async (t) => {
  const { aiHomeDir, profilesDir } = createRegisteredTestHome(t, 'agy', '3');
  const exits = [];
  const calls = [];
...
test('`aih codex goal ...` forwards non-dash native subcommand to built-in AIH server profile', () => {
  ...
  runAiCliCommandRouter('codex', ['codex', 'goal', 'continue'], {
    ...
    extractActiveEnv: () => null,
    runCliPty: (cliName, accountRef, forwardArgs, isLogin, runtimeOptions) => {
      runCalls.push({ cliName, accountRef, forwardArgs, isLogin, runtimeOptions });
    }
  });
  assert.deepEqual(runCalls, [{
    cliName: 'codex',
    accountRef: '',
    forwardArgs: ['goal', 'continue'],
    isLogin: false,
    runtimeOptions: { cliAccountId: '', loginSessionId: '', gateway: true }
  }]);
});

test('`aih gemini --resume <session>` (no account id, no server profile) forwards to default account', (t) => {
  const { root, aiHomeDir, profilesDir, accountRef } = createRegisteredTestHome(t, 'gemini', '1');
  writeDefaultAccountRef(fs, aiHomeDir, 'gemini', accountRef);
  const runCalls = [];
  runAiCliCommandRouter('gemini', ['gemini', '--resume', '334e009d-267c-407d-be2e-253efd0df5ec'], {
    ...
    runCliPty: (cliName, ref, forwardArgs) => runCalls.push({ cliName, accountRef: ref, forwardArgs })
  });
  assert.deepEqual(runCalls, [{ cliName: 'gemini', accountRef, forwardArgs: ['--resume', '334e009d-267c-407d-be2e-253efd0df5ec'] }]);
});

test('`aih gemini plan ...` (no account id) forwards native subcommand to default account', (t) => {
  const { root, aiHomeDir, profilesDir, accountRef } = createRegisteredTestHome(t, 'gemini', '1');
  writeDefaultAccountRef(fs, aiHomeDir, 'gemini', accountRef);
  const runCalls = [];
  ...
  assert.deepEqual(runCalls, [{ cliName: 'gemini', accountRef, forwardArgs: ['plan', 'show'] }]);
});

test('`aih agy <id> exec ...` forwards native args after explicit account id', (t) => {
  const { root, aiHomeDir, profilesDir, accountRef } = createRegisteredTestHome(t, 'agy', '3');
  const runCalls = [];
  ...
  assert.deepEqual(runCalls, [{ cliName: 'agy', accountRef, forwardArgs: ['exec', '--foo', 'bar'] }]);
});

test('`aih codex sessions <id>` uses injected resolver for Windows psmux', (t) => {
  const { aiHomeDir, profilesDir, accountRef } = createRegisteredTestHome(t, 'codex', '12');
  const exits = [];
  const logs = [];
  const resolveCalls = [];
  const spawnCalls = [];
  ...
  const originalLog = console.log;
  const fsWithMissingPsmux = Object.create(fs);
  fsWithMissingPsmux.existsSync = (target) => (
    String(target) === psmuxPath ? false : fs.existsSync(target)
  );
  console.log = (msg) => logs.push(String(msg));
  try {
  ...
  assert.deepEqual(listCall.args.slice(0, 4), ['-u', '-L', `aih-codex-${accountRef}`, 'list-sessions']);
  assert.equal(logs.some((line) => line.includes('codex#12')), true);
});

test('`aih codex sessions <id>` can install missing Windows psmux then list sessions', (t) => {
  const { aiHomeDir, profilesDir, accountRef } = createRegisteredTestHome(t, 'codex', '12');
  let installed = false;
  const exits = [];
  const logs = [];
  ...
  const psmuxPath = 'C:\\Users\\madou\\AppData\\Local\\Microsoft\\WinGet\\Links\\psmux.exe';
  const originalLog = console.log;
  const fsWithPsmuxInstall = Object.create(fs);
  fsWithPsmuxInstall.existsSync = (target) => (
    String(target) === psmuxPath ? installed : fs.existsSync(target)
  );
  console.log = (msg) => logs.push(String(msg));
  try {
    runAiCliCommandRouter('codex', ['codex', 'sessions', '12'], {
...
test('`aih codex sessions <id>` respects disabled Windows psmux install prompt', (t) => {
  const { aiHomeDir, profilesDir } = createRegisteredTestHome(t, 'codex', '12');
  const exits = [];
  const logs = [];
  const spawnCalls = [];
  ...
  const fsWithMissingPsmux = Object.create(fs);
  fsWithMissingPsmux.existsSync = (target) => (
    String(target).toLowerCase().endsWith('psmux.exe') ? false : fs.existsSync(target)
  );
  console.log = (msg) => logs.push(String(msg));
  try {
    runAiCliCommandRouter('codex', ['codex', 'sessions', '12'], {
...
test('`aih codex sessions` hides accounts without live tmux sessions', (t) => {
  const { aiHomeDir, profilesDir, accountRefs } = createRegisteredTestAccounts(t, 'codex', ['1', '2']);
  ...
  try {
    runAiCliCommandRouter('codex', ['codex', 'sessions'], {
      ...
      spawnSync: (_command, args) => {
        if (tmuxSocket(args) === `aih-codex-${accountRefs['1']}`) {
          return {
            status: 0,
...
test('`aih codex sessions` reloads tmux config before listing live sessions', (t) => {
  const { aiHomeDir, profilesDir, accountRef } = createRegisteredTestHome(t, 'codex', '1');
  const spawnCalls = [];
  const exits = [];
  const logs = [];
  const originalLog = console.log;
  ...
  try {
    runAiCliCommandRouter('codex', ['codex', 'sessions', '1'], {
      ...
      spawnSync: (_command, args, options) => {
        ...
        if (args.includes('list-sessions') && tmuxSocket(args) === `aih-codex-${accountRef}`) {
...
test('`aih codex sessions` picker enters the selected tmux session in mirror mode', (t) => {
  ...
  runAiCliCommandRouter('codex', ['codex', 'sessions'], {
    ...
    spawnSync: (_command, args) => {
      ...
      if (tmuxSocket(args) === `aih-codex-${accountRefs['2']}`) {
        return {
          status: 0,
    ...
    runCliPty: (cliName, accountRef, forwardArgs, _isLogin, runtimeOptions) => {
      runCalls.push({
        cliName,
        accountRef,
        forwardArgs,
        cliAccountId: runtimeOptions.cliAccountId,
...
test('`aih codex sessions` picker opens a compatible session for legacy UTF-8 runtime rows', (t) => {
  ...
  runAiCliCommandRouter('codex', ['codex', 'sessions'], {
    ...
    runCliPty: (cliName, ref, forwardArgs, isLogin) => {
      runCalls.push({
        cliName,
        accountRef: ref,
        forwardArgs,
...
test('`aih codex sessions <id>` picker closes the selected tmux session', (t) => {
  ...
  assert.deepEqual(exits, [0]);
  assert.deepEqual(runCalls, []);
  assert.deepEqual(killCalls, [{ socket: `aih-codex-${accountRef}`, target: 'p-one' }]);
...
test('`aih codex sessions` async picker redraws from the block top without scrolling', async (t) => {
  ...
  const commandPromise = runAiCliCommandRouter('codex', ['codex', 'sessions'], {
    ...
    runCliPty: (cliName, ref, forwardArgs) => {
      runCalls.push({
        cliName,
        accountRef: ref,
        forwardArgs,
...
test('`aih codex sessions` uses sync picker by default on native Windows', (t) => {
  const { aiHomeDir, profilesDir, accountRefs } = createRegisteredTestAccounts(t, 'codex', ['1', '2']);
  const sep = persistentSession.SESSION_LIST_SEPARATOR;
  const keys = ['\x1b', '[B', '\r'];
  const fsWithSplitKeys = Object.create(fs);
  fsWithSplitKeys.readSync = (_fd, buffer) => {
    const key = keys.shift() || '\r';
    buffer.write(key);
  ...
  const writes = [];
  const runCalls = [];
  const env = { USERPROFILE: 'C:\\Users\\madou' };
  ...
  runAiCliCommandRouter('codex', ['codex', 'sessions'], {
    ...
    spawnSync: (_command, args) => {
      if (tmuxSocket(args) === `aih-codex-${accountRefs['1']}`) {
        return {
          status: 0,
          stdout: ['p-one', '1', '100', 'C:\\Users\\madou', '5555', 'codex', 'node'].join(sep)
        };
      }
      if (tmuxSocket(args) === `aih-codex-${accountRefs['2']}`) {
        return {
          status: 0,
          stdout: ['p-two', '1', '101', 'C:\\Users\\madou', '6666', 'codex', 'node'].join(sep)
        };
      }
      return { status: 0, stdout: '' };
    },
  ...
  assert.equal(writes.join('').includes('6666'), true);
});

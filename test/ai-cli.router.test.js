...
test('`aih codex login --no-browser` keeps the CLI id out of the PTY account-ref slot', () => {
  const exits = [];
  const calls = [];
...
test('`aih codex <id> --no-browser` creates new account when target account is already configured', (t) => {
  const { root, aiHomeDir, profilesDir } = createRegisteredTestHome(t);
  const calls = [];
  const logs = [];
  const originalLog = console.log;
...
test('`aih agy login api_key` stores access token in DB without .aih_env.json', (t) => {
  ...
  const aiHomeDir = path.join(root, '.ai_home');
  const profilesDir = path.join(aiHomeDir, 'profiles');
  const profileDir = path.join(profilesDir, 'agy', '2');
  ...
  assert.deepEqual(calls, [{
    ...
    forwardArgs: [],
    isLogin: false,
    runtimeOptions: { cliAccountId: '2', loginSessionId: '', gateway: false }
  }]);
});

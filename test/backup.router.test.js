...
function registerStoredAccount(aiHomeDir, provider, identitySeed, credentials = {}) {
  const registration = registerAccountIdentity(fs, aiHomeDir, { provider, identitySeed });
  if (credentials.env) {
    writeAccountCredentials(fs, aiHomeDir, registration.accountRef, credentials.env);
  }
  if (credentials.nativeAuth) {
    writeAccountNativeAuth(fs, aiHomeDir, registration.accountRef, credentials.nativeAuth);
  }
  return registration;
}
...
test('runBackupCommand generic export reports progress while writing flat account JSON files', async () => {
  ...
  try {
    const aiHomeDir = path.join(root, '.ai_home');
    registerStoredAccount(aiHomeDir, 'codex', 'oauth:codex:one@example.com', {
      nativeAuth: { auth: { email: 'one@example.com', tokens: { refresh_token: 'rt_1' } } }
    });
    registerStoredAccount(aiHomeDir, 'codex', 'oauth:codex:two@example.com', {
      nativeAuth: { auth: { email: 'two@example.com', tokens: { refresh_token: 'rt_2' } } }
    });
...
test('runBackupCommand generic export writes agy oauth account as flat JSON', async () => {
  ...
  try {
    const aiHomeDir = path.join(root, '.ai_home');
    registerStoredAccount(aiHomeDir, 'agy', 'oauth:agy:agy@example.com', {
      nativeAuth: {
        oauthToken: {
...
test('runBackupCommand provider-scoped codex export writes api-key account as flat JSON', async () => {
  ...
  try {
    ...
    const aiHomeDir = path.join(root, '.ai_home');
    registerStoredAccount(aiHomeDir, 'codex', 'api-key:codex:https://api.example.com/v1:sk-test', {
      env: {
...
test('runBackupCommand provider-scoped opencode export writes auth as flat JSON', async () => {
  ...
  try {
    ...
    const aiHomeDir = path.join(root, '.ai_home');
    registerStoredAccount(aiHomeDir, 'opencode', 'oauth:opencode:openai:sk-openai', {
      nativeAuth: { auth: { openai: { type: 'api', key: 'sk-openai' } } }
    });

...
test('`set-default` does not migrate encoded Windows absolute path entries into host codex dir', (t) => {
  const homeDir = mkTmpDir();
  t.after(() => fs.rmSync(homeDir, { recursive: true, force: true }));

  const toolDir = path.join(homeDir, '.ai_home', 'profiles', 'codex');
  const accountCodexDir = path.join(toolDir, '1', '.codex');
  ...
  fs.writeFileSync(path.join(accountCodexDir, 'auth.json'), '{"sandbox":"auth"}\n');

  const result = runCli(['codex', 'set-default', '1'], homeDir);
  assert.equal(result.status, 0, `stdout=${result.stdout}\nstderr=${result.stderr}`);

  assert.equal(
    fs.existsSync(path.join(homeDir, '.codex', encodedEntry)),
    false,
    'set-default must not promote encoded Windows absolute paths into the host codex store'
  );
  assert.equal(
    fs.existsSync(path.join(accountCodexDir, encodedEntry)),
    true,
    'unexpected encoded path entry should be left in the account sandbox for explicit cleanup'
  );
});

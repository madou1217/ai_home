...
test('thread 参数按 AIH 审批模式映射 Codex 原生权限', () => {
  assert.deepStrictEqual(
    protocol.buildThreadStartParams({ cwd: '/tmp/fast', approvalMode: 'bypass' }),
    { approvalPolicy: 'never', sandbox: 'danger-full-access', cwd: '/tmp/fast' }
  );
  assert.deepStrictEqual(
...
test('未知审批模式 fail closed，不降级成 bypass', () => {
  assert.throws(
    () => protocol.buildApprovalPolicyParams('future-mode'),
    (error) => error.code === 'codex_approval_mode_unsupported'
  );
});
...
test('turn/start 参数：text 输入 + localImage + model 覆盖', () => {
  ...
  assert.strictEqual(params.clientUserMessageId, 'run-1');
  assert.strictEqual(params.model, 'gpt-5.2-codex');
  assert.strictEqual(params.effort, 'high');
  assert.strictEqual(params.approvalPolicy, 'untrusted');
  assert.deepStrictEqual(params.sandboxPolicy, { type: 'workspaceWrite' });
  assert.deepStrictEqual(params.collaborationMode, {
    mode: 'default',
    settings: {
      model: 'gpt-5.2-codex',
...
test('plan turn 使用当前默认 Codex 的原生 collaborationMode', () => {
  const params = protocol.buildTurnStartParams({
    threadId: THREAD_ID,
    prompt: '先制定计划',
    model: 'gpt-5.3-codex',
  ...
  assert.strictEqual(params.effort, 'low');
  assert.strictEqual(params.approvalPolicy, 'untrusted');
  assert.deepStrictEqual(params.sandboxPolicy, { type: 'workspaceWrite' });
  assert.deepStrictEqual(params.collaborationMode, {
    mode: 'plan',
...
test('bypass turn 显式覆盖已加载 thread 的原生权限', () => {
  const params = protocol.buildTurnStartParams({
    threadId: THREAD_ID,
    prompt: '继续执行',
    model: 'gpt-5.3-codex',
  ...
  assert.deepStrictEqual(params.sandboxPolicy, { type: 'dangerFullAccess' });
});
...
test('通知映射：willRetry 保持 turn 活跃，终局 error 才失败', () => {
  const state = protocol.createTurnObserverState(THREAD_ID);

  assert.deepStrictEqual(
    protocol.mapServerNotification(notif('error', {
      threadId: THREAD_ID,
      turnId: 'turn-1',
  ...
  assert.strictEqual(state.failureMessage, '');
  ...
  assert.strictEqual(state.failureMessage, 'request rejected');
});

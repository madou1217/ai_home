...
test('removeCodexProjectRegistration removes only the matching TOML project table', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-codex-project-remove-'));
  const codexDir = path.join(root, '.codex');
  const configPath = path.join(codexDir, 'config.toml');
  const removedProject = '/tmp/project-to-remove';
  ...
  try {
    ...
    fs.writeFileSync(configPath, [
      ...
      'trust_level = "trusted"',
      '',
      '[projects."/Users/model/projects/keep"]',
      'trust_level = "trusted"',
      '',
      '[features]',
      'hooks = true',
    ...
    const first = removeCodexProjectRegistration(removedProject, { hostHomeDir: root });
    assert.equal(first.ok, true);
    assert.equal(first.updated, true);
    const content = fs.readFileSync(configPath, 'utf8');
    assert.doesNotMatch(content, /project-to-remove/);
    assert.match(content, /\[projects\."\/Users\/model\/projects\/keep"\]/);
    assert.match(content, /^\[features\]$/m);

    const second = removeCodexProjectRegistration(removedProject, { hostHomeDir: root });
    assert.equal(second.updated, false);

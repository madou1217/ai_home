...
`);
  ...
  const merged = mergeConfigs(hostConfig, accountOnlyConfig, {
    isApiKeyMode: true,
    openaiApiKey: 'dummy',
    codexVersion: '0.114.0'
  });
...
test('mergeConfigs replaces aih provider section without leaving duplicate keys behind', () => {
  ...
  assert.equal((providerSection[0].match(/^env_key = /gm) || []).length, 1);
...
test('mergeConfigs routes the CLI server alias through the canonical gateway provider key', () => {
  const hostProviderKey = getAihProviderKey();
  const serverProfileProviderKey = getAihProviderKey();
  const hostConfig = [
    'preferred_auth_method = "apikey"',
  ...
  assert.equal(hostProviderKey, serverProfileProviderKey);
  assert.equal((merged.match(new RegExp(`^\\[model_providers\\.${hostProviderKey}\\]$`, 'gm')) || []).length, 1);
  assert.match(merged, /^base_url = "http:\/\/127\.0\.0\.1:8317\/v1"$/m);
  assert.match(merged, /^env_key = "OPENAI_API_KEY"$/m);
  assert.doesNotMatch(merged, /server-key|host-token/);
});

test('scopeAccountOnlyConfig keeps only current account managed provider from stale account config', () => {
  const currentProvider = getAihProviderKey();
  const staleProvider = getAihProviderKey();
  ...
  assert.match(scoped.model_providers[0], /account\.example\.com/);
  assert.doesNotMatch(scoped.model_providers[0], /yesboss|name = "stale"/);
});

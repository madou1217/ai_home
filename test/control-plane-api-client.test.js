...
test('control plane api client sends bearer json requests without browser credentials', async () => {
  ...
  assert.deepEqual(
    await client.getJson('/v0/node-rpc/device-status', { requireToken: true }),
    { ok: true }
  );
  assert.deepEqual(
    await client.postJson('/v0/node-rpc/device-pair', { code: 'pair-code' }, {
      token: '',
      httpErrorPrefix: 'pair_http'
    }),
    { ok: true }
  );
  ...
  assert.equal(calls[0].headers.authorization, 'Bearer device-token');
  assert.equal(calls[0].headers.accept, 'application/json');
  assert.equal(calls[0].credentials, 'omit');
  assert.equal(calls[1].method, 'POST');
  assert.equal(calls[1].headers.authorization, undefined);
  assert.equal(calls[1].headers['content-type'], 'application/json');
  assert.equal(calls[1].body, JSON.stringify({ code: 'pair-code' }));
});

test('control plane api client validates token and preserves http error prefix', async () => {
  const { ControlPlaneApiClient } = loadControlPlaneApiClientModule();
  const client = new ControlPlaneApiClient({
    endpoint: 'https://control.example.com',
    fetchImpl: async () => {
  ...
  await assert.rejects(
    () => client.getJson('/v0/node-rpc/device-status', { requireToken: true }),
    /missing_control_plane_device_token/
  );
  assert.throws(
    () => client.buildEventStreamRequest('/v0/node-rpc/device-session-stream', { requireToken: true }),
    /missing_control_plane_device_token/
  );
  await assert.rejects(

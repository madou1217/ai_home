...
test('release asset staging publishes only one non-empty installer per platform kind', () => {
  ...
  try {
    ...
    assert.deepEqual(
      manifest.assets.map(({ fileName, kind }) => [kind, fileName]),
      [
        ['dmg', `AI.Home_${VERSION}_aarch64.dmg`],
        ['msi', `AI.Home_${VERSION}_x64_en-US.msi`],
        ['deb', `ai-home_${VERSION}_amd64.deb`],
        ['appimage', `ai-home_${VERSION}_amd64.AppImage`],
      ],
    );

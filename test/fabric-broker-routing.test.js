...
test('broker route allowlist stays narrow', () => {
  ...
  assert.equal(isFabricBrokerRouteAllowed('POST', '/v0/node-rpc/device-node-session-ack'), true);
  assert.equal(isFabricBrokerRouteAllowed('POST', '/v0/fabric/device-pair'), false);
...
test('broker connect proxies real HTTP requests over real WebSocket sockets', async (t) => {
  ...
  assert.equal(sessionStartPayload.result.authorization, 'Bearer management-secret');
  const sessionStartRequest = localRequests.find((entry) => entry.url === '/v0/node-rpc/device-node-session-start');
  assert.equal(sessionStartRequest.authorization, 'Bearer management-secret');
  ...
  assert.equal(sessionCommandPayload.result.authorization, 'Bearer management-secret');
  const sessionCommandRequest = localRequests.find((entry) => entry.url === '/v0/node-rpc/device-node-session-command');
  assert.equal(sessionCommandRequest.authorization, 'Bearer management-secret');

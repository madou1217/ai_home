...
const OPENCODE_ACCOUNT_REF = 'acct_22222222222222222222';
...
test('fabric closure verify command writes handoff file without raw reports', async (t) => {
  ...
  nodesReport.targetNode.managementKey = 'raw-management-key-secret';
  ...
  assert.equal(rawHandoff.includes('raw-management-key-secret'), false);
});
  ...
  codexGap.diagnostic = {
    accounts: {
      ...
      reasons: [{
        ...
        count: 1,
        sampleAccountRefs: ['acct_22222222222222222222']
      }]
    }
  };
...
test('fabric closure audit reports blocked instead of crashing when node inventory is unavailable', async () => {
  ...
  assert.equal(report.summary.nextActions.some((item) => item.includes('Configure a Server')), true);
  assert.equal(report.closurePlan.immediateNext.id, 'server-profile-required');
  ...
  assert.equal(report.failureLedger.executionDecision.decision, 'stop_configure_server');
  assert.equal(
    report.failureLedger.executionDecision.resumeWhen.some((item) => item.includes('ready Server profile')),
    true
  );
});

test('fabric closure audit reports session start errors instead of crashing on null step reports', async () => {
  const error = new Error('Remote Server is not configured');
  error.code = 'ready_server_profile_missing';

...
test('provider accounts credential handoff distinguishes API key and OAuth blockers', () => {
  ...
  assert.equal(agy.commands[0], 'aih fabric provider accounts reauth --provider agy --account-id 1 --endpoint https://control.example.com:9527 --json');
...
test('provider accounts reauth uses paired profile device token without printing it', async () => {
  const requests = [];
  ...
  assert.equal(requests[0].options.headers.authorization, 'Bearer secret-device-token');
  ...
  assert.match(text, /target_account_id: 42/);
  assert.match(text, /authorization_url: https:\/\/login\.example\.com\/oauth/);
  assert.doesNotMatch(JSON.stringify(report), /secret-device-token/);
  assert.doesNotMatch(text, /secret-device-token/);
});

test('provider accounts reauth parser accepts positional provider and account id', () => {
  const options = parseReauthArgs(['codex', '42', '--endpoint', 'https://control.example.com']);
...
test('provider accounts auth-job uses paired profile device token without printing it', async () => {
  const requests = [];
...
test('fabric command router routes provider accounts reauth JSON', async () => {
  ...
  await runFabricCommandRouter([
    ...
    runFabricProviderAccountsCommand: async (action, args) => {
      ...
      assert.deepEqual(args, ['--provider', 'codex', '--account-id', '42', '--json']);
      return {
        ok: true,
        json: true,
        exitOk: true,

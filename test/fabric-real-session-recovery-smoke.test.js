...
test('session recovery parser defaults to AWS broker and local default port', () => {
  ...
  assert.equal(options.managementKeyFile, '/tmp/aws-management-key');
});
...
test('runSessionRecoverySmoke resumes the same session after broker interruption', async () => {
  ...
  const result = await runSessionRecoverySmoke({
    ...
    serverId: 'm5-test',
    nodeId: 'm5-test-node',
    managementKey: 'aws-management-secret',
    ...
    connectFabricBroker: async (options) => {
      ...
      assert.equal(options.managementKey, 'aws-management-secret');
    ...
    resolveExistingNodeManagementKey: () => ({
      key: 'local-management-secret',
      source: 'server-config',
      passCliArg: false
    }),
    prepareExistingEndpointStores: () => ({
      managementKey: 'local-management-secret',
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      assert.equal(options.headers.authorization, 'Bearer local-management-secret');
  ...
  assert.equal(JSON.stringify(result).includes('aws-management-secret'), false);
  assert.equal(JSON.stringify(result).includes('local-management-secret'), false);

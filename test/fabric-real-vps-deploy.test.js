...
test('parseArgs accepts conservative real VPS deploy options', () => {
  ...
  const parsed = parseArgs([
    ...
    '--management-key-file', '/home/ubuntu/aih-fabric-current/.management-key',
    '--skip-build',
    '--skip-import',
    '--skip-start',
    '--dry-run'
  ...
  assert.equal(parsed.managementKeyFile, '/home/ubuntu/aih-fabric-current/.management-key');
...
test('parseArgs requires Management Key file to be an absolute remote path', () => {
  assert.throws(
    () => parseArgs([
      '--ssh',
      'ubuntu@example.com',
    ...
    /--management-key-file must be an absolute remote path/
  );
});
...
test('remote Node helpers keep runtime and AIH state scoped to remote deploy dir', () => {
  const options = {
    remoteDir: '/home/ubuntu/aih-fabric-current',
    nodeRuntimeArchive: '/tmp/node-v22.16.0-linux-x64.tar.xz',
    managementKeyFile: '/home/ubuntu/aih-fabric-current/.management-key'
  };
  ...
  assert.match(envCommand, /test -s '\/home\/ubuntu\/aih-fabric-current\/\.management-key'/);
  assert.match(envCommand, /export AIH_MANAGEMENT_KEY="\$\(cat '\/home\/ubuntu\/aih-fabric-current\/\.management-key'\)"/);
  ...
  assert.equal(
    ...
    "mkdir -p '/home/ubuntu/aih-fabric-current/.aih-host-home' && export AIH_HOST_HOME='/home/ubuntu/aih-fabric-current/.aih-host-home' && test -s '/home/ubuntu/aih-fabric-current/.management-key' && export AIH_MANAGEMENT_KEY=\"$(cat '/home/ubuntu/aih-fabric-current/.management-key')\" && export PATH='/home/ubuntu/aih-fabric-current/.node-runtime/node-v22.16.0-linux-x64/bin':'/home/ubuntu/aih-fabric-current/node_modules/.bin':$PATH; node --version"
  );
  assert.match(

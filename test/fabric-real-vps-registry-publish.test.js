...
test('parseArgs validates real VPS registry publish inputs', () => {
  ...
  assert.throws(() => parseArgs(['--port', '18482']), /--node-id is required/);
});
...
test('runRealVpsRegistryPublish prints only sanitized registry evidence', async (t) => {
  ...
  assert.equal(report.deviceId, 'device-registry-vps-152-jp-v4');
  ...
  assert.equal(spawnCalls[0].options.env.AIH_FABRIC_TOKEN.length > 20, true);
  assert.equal(spawnCalls[0].args.includes('--from-server'), true);
  assert.equal(spawnCalls[1].args.includes('heartbeat'), true);
  assert.equal(spawnCalls[1].options.env.AIH_FABRIC_TOKEN, spawnCalls[0].options.env.AIH_FABRIC_TOKEN);
  ...
  assert.equal(spawnCalls[2].options.env.AIH_FABRIC_TOKEN, spawnCalls[0].options.env.AIH_FABRIC_TOKEN);

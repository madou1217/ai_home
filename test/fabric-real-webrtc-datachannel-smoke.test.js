...
test('webrtc signaling smoke sends Management Key only in the Authorization header', async () => {
  const requests = [];
  const room = await createSignalingRoom('https://server.example.com', {
    ...
    fetchImpl: async (url, options) => {
      ...
      return {
        ok: true,
        status: 200,
        text: async () => JSON.stringify({ ok: true, result: { roomId: 'rtc_test' } })
      };
    }
  });

  assert.equal(room.roomId, 'rtc_test');
  assert.equal(requests[0].options.headers.authorization, 'Bearer management-secret');
  assert.equal(requests[0].url.includes('management-secret'), false);
});

...
test('parseFabricRegistryAgentArgs resolves the node token from app-state.db', (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-fabric-agent-token-db-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
  writeRegistryAgentManagementKey('home-node', 'db-token', { fs, aiHomeDir });
  ...
  assert.equal(options.managementKey, 'db-token');
  assert.equal(options.endpoint, 'https://server.example.com');
  assert.equal(options.nodeId, 'home-node');
});

test('parseFabricRegistryAgentArgs rejects runtime token-file fallback', () => {
  assert.throws(
    () => parseFabricRegistryAgentArgs([
      'https://server.example.com',
      '--node-id',
    ...
    { code: 'fabric_agent_management_key_file_not_allowed' }
  );
});
...
test('runFabricRegistryAgent reuses heartbeat sender without leaking credentials in events', async () => {
  ...
  assert.equal(calls[0].managementKey, 'management-secret');
...
test('runFabricRegistryAgent stops its heartbeat loop when aborted', async () => {
  const controller = new AbortController();
  let heartbeats = 0;
  let sleepSignal = null;
  const running = runFabricRegistryAgent([
    'https://server.example.com',
    '--management-key',
    'management-secret',
    '--node-id',
    ...
    postFabricRegistryHeartbeat: async () => {
      heartbeats += 1;
      return { ok: true, result: { registry: { counts: {} } } };
    },
    sleep: (_ms, context) => new Promise((resolve) => {
      sleepSignal = context.signal;
...
test('runFabricRegistryAgent exposes non-retryable heartbeat auth failures to its supervisor', async () => {
  const authError = new Error('unauthorized');
  authError.code = 'fabric_registry_heartbeat_failed';
  ...
  await assert.rejects(
    () => runFabricRegistryAgent([
      ...
      env: {},
      postFabricRegistryHeartbeat: async () => { throw authError; },

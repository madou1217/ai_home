...
test('persistent transport formatting excludes runtime diagnostic text', () => {
  assert.equal(formatPersistentTransportHeartbeat({
    kind: 'relay',
    health: 'degraded',
    lastError: 'Authorization: Bearer header-secret',
    promotion: {
      remoteRequestReady: false,
      mode: 'management-rpc'
    }
  }), 'relay=degraded,remote-request-ready=false,mode=management-rpc');
});

test('runFabricRegistryHeartbeat posts liveness without Management Key in payload', async () => {
  const requests = [];

...
test('runFabricRegistryPublish posts registry snapshot with bearer token', async () => {
  const requests = [];
...
test('runFabricRegistryPublish can derive runtimes from real server management accounts', async () => {
  ...
  assert.equal(requests[1].options.headers.authorization, 'Bearer device-token');

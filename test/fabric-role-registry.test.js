...
test('fabric role registry stores node roles projects runtimes relay metadata and mirrors legacy relay transport', (t) => {
  ...
  assert.equal(Boolean(readJsonValue(fs, aiHomeDir, FABRIC_REGISTRY_KEY)), true);
...
test('fabric role registry persists runtime diagnostics for inventory gap blockers', (t) => {
    ...
    runtimeDiagnostics: [
      ...
      },
      {
        ...
        accounts: {
          ...
          reasons: [{ reason: 'blocked_by_runtime_status:agy_not_signed_in', count: 1, sampleAccountRefs: ['acct_77777777777777777777'] }]
        }
      }
    ]
  }, deps);
  ...
  assert.deepEqual(node.runtimeGaps.find((gap) => gap.provider === 'agy').diagnostic.accounts.reasons, [
    { reason: 'blocked_by_runtime_status:agy_not_signed_in', count: 1, sampleAccountRefs: ['acct_77777777777777777777'] }
  ]);
  assert.deepEqual(

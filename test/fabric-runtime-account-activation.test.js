...
test('runActivation remote audit does not create or transfer an account archive', async () => {
  ...
  try {
    ...
    assert.match(commands[0], /account_state\.db/);
  ...
  try {
    ...
    const report = await runActivation(options, {
      ...
      runRemote: async (_options, command) => {
        ...
        const auditCount = remoteCommands.filter((entry) => String(entry).includes('account_state.db')).length;
        ...
        if (args.includes('start')) {
          ...
          return {
            ...
            stdout: JSON.stringify({
              ...
              result: { accepted: true, provider, accountId: '1', runId }
            }),
            stderr: ''
          };
        }
        ...
        if (args.includes('start')) {
          ...
          return {
            ...
            stdout: JSON.stringify({
              ok: true,
              http: { registryAuthorizedStatus: 200, sessionStartStatus: 200 },
              result: { provider: 'opencode', accountId: '1', runId: 'opencode-run' }
            }),
            stderr: ''
          };
        }
...
test('runActivation remote revalidation continues through blocked provider accounts', async () => {
  ...
  try {
        ...
        if (args.includes('start')) {
          ...
          return {
            ...
            stdout: JSON.stringify({
              ok: true,
              http: { registryAuthorizedStatus: 200, sessionStartStatus: 200 },
              result: { provider: 'claude', accountId: String(startCount), runId }
            }),
            stderr: ''
          };
        }
    ...
    assert.equal(report.sessionStarts[0].accountId, '2');
    assert.equal(report.sessionStarts[0].providerAttemptCount, 2);
    assert.equal(report.sessionStarts[0].providerAttemptLimit, 2);
    assert.deepEqual(report.sessionStarts[0].providerAttemptHistory.map((item) => item.accountId), ['1', '2']);
...
test('runActivation remote revalidation keeps provider blocked when pool ends with missing account', async () => {
  ...
  try {
        ...
        if (args.includes('start')) {
          ...
          return {
            ...
            stdout: JSON.stringify({
              ok: true,
              http: { registryAuthorizedStatus: 200, sessionStartStatus: 200 },
              result: { provider: 'claude', accountId: '1', runId: 'claude-run-1' }
            }),
            stderr: ''
          };
        }
    ...
    assert.equal(report.sessionStarts[0].providerAttemptCount, 2);
    assert.equal(report.sessionStarts[0].providerRuntimeBlocks.length, 1);
    assert.deepEqual(report.sessionStarts[0].providerAttemptHistory.map((item) => item.accountId), ['1', '']);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test('remote commands scope runtime state to AWS current dir and default port', () => {
  ...
  assert.match(reload, /server-config\.json/);
  ...
  assert.match(audit, /account_state\.db/);
  ...
  assert.match(publish, /aws-current-node\.token/);
  assert.match(publish, /server-config\.json/);
  assert.match(publish, /registry/);
  assert.match(publish, /publish/);
  assert.match(publish, /--from-server/);
  assert.match(publish, /AIH_FABRIC_TOKEN:token/);
  assert.match(publish, /AIH_MANAGEMENT_KEY:managementKey/);

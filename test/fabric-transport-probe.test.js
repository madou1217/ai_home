...
test('runFabricTransportProbe aborts an active tcp probe through the parent signal', async () => {
  const controller = new AbortController();
  const running = runFabricTransportProbe(['tcp://127.0.0.1:9527'], {
    signal: controller.signal,
    now: () => 1000,
    netImpl: createFakeNet(() => {})
  });
  ...
  assert.equal(result.ok, false);
  assert.equal(result.probes[0].status, 'aborted');
  assert.equal(result.probes[0].error, 'operation_aborted');
});

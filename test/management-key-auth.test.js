...
test('remote WebUI accepts only the Management Key while loopback remains local', () => {
  const loopback = authorizeWebUiRequest({
    req: { socket: { remoteAddress: '127.0.0.1' }, headers: {} },
    url: new URL('http://127.0.0.1:9527/v0/webui/projects'),
    requiredManagementKey: ''
  });
  assert.deepEqual(loopback, { ok: true, via: 'loopback' });

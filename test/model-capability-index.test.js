...
test('model capability index uses AGY quota snapshot as account model truth', () => {
  const state = {
    accounts: {
      codex: [],
      gemini: [],
  ...
  assert.deepEqual(listAvailableAccountRefsForModelProvider(index, 'claude-sonnet-4-6', 'agy'), []);
  assert.equal(modelHasRoutableProvider(index, 'gemini-3-flash-agent', 'agy'), true);
  assert.equal(modelHasRoutableProvider(index, 'claude-sonnet-4-6', 'agy'), false);
});

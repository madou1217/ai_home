...
test('model usage scanner applies provider-specific Codex, Claude, and Gemini rules', (t) => {
  ...
  try {
    ...
    assert.equal(stats.totalTokens, 115 + 13 + 112);
...
test('model usage API record normalizes OpenAI cached prompt tokens', () => {
  ...
  assert.match(record.eventKey, /^api:codex:req_1:7:/);
});
...
test('model usage service falls back to host .ai_home when aiHomeDir is omitted', (t) => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(root, '.ai_home', 'model-usage.db')), true);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test('codex scanner counts cumulative token deltas once despite repeated token_count emits', (t) => {
  const fixture = makeService(t);
  if (!fixture) return;
  const { root, service } = fixture;
  try {
    const sessionId = '22222222-2222-4222-8222-222222222222';
    const meta = (input, cached, output) => ({
      timestamp: '2026-06-04T08:00:00.000Z',
      type: 'event_msg',
      payload: {
        type: 'token_count',
        info: {
          ...
          // codex repeats the same last_token_usage on every ping — must be ignored.
          last_token_usage: { input_tokens: input, cached_input_tokens: cached, output_tokens: output }
        }
      }
    });
    writeJsonl(path.join(root, '.codex', 'sessions', '2026', '06', '04', `rollout-2026-06-04T08-00-00-${sessionId}.jsonl`), [
      ...
      meta(1000, 0, 50),   // turn 1: +1050
      meta(1000, 0, 50),   // duplicate emit → +0
      meta(1000, 0, 50),   // duplicate emit → +0
      meta(1500, 0, 80),   // turn 2: +530  (run-1 peak 1580)
      meta(300, 0, 10)     // cumulative reset (/compact): new run → +310
    ]);

    service.scan({ provider: 'codex' });
    const query = {
      provider: 'codex',
      fromMs: new Date(2026, 5, 4).getTime(),
      toMs: new Date(2026, 5, 5).getTime() - 1
    ...
    const stats = service.getStats(query);
    // naive per-event summing would inflate (1050+1050+1050+1580+310). Correct =
    // sum of run peaks: run-1 1580 (1500+80) + run-2 310 (post-reset 300+10).
    assert.equal(stats.totalCalls, 3, 'two advancing turns + one post-reset turn');
    assert.equal(stats.totalTokens, 1580 + 310);
    assert.equal(stats.outputTokens, 90);
    assert.equal(stats.inputTokens, 1800);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

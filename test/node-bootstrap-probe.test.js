...
test('buildLocalAssetInstallScript supports Windows local runtime bootstrap', () => {
  ...
  assert.doesNotMatch(script, /node-secret|device-token|managementKey.*node-secret/);
});
...
test('runNodeCommandRouter prints node usage for bootstrap apply help', async () => {
  ...
  assert.match(writes.join(''), /Mobile\/PWA\/tablet clients pair with a Control Plane/);
});

...
const TEST_RELAY_TIMEOUT_MS = 30_000;
...
async function waitForCondition(predicate, timeoutMs = TEST_RELAY_TIMEOUT_MS) {
...
function readJsonMessageWithTimeout(socket, timeoutMs = TEST_RELAY_TIMEOUT_MS, predicate = () => true) {
  return new Promise((resolve, reject) => {
...
test('runRelayLoop closes the active socket and stops reconnecting when aborted', async () => {
  const controller = new AbortController();
  const socket = createFakeRelaySocket();
  let attempts = 0;
  const running = runRelayLoop({
    url: new URL('ws://control.example.com/v0/relay/node?nodeId=nat-node'),
    nodeId: 'nat-node',
    managementKey: 'node-secret',
    heartbeatMs: 1000,
...
test('runNodeRelayConnect forwards relay management request to local server', async (t) => {
  ...
  assert.equal(await waitForCondition(() => registry.listRelaySessions().length === 1), true);
  const result = await requestRemoteManagement({
    ...
    method: 'GET',
    timeoutMs: TEST_RELAY_TIMEOUT_MS,
    audit: false
  }, {
    ...deps,
    relaySessionRegistry: registry,
...
test('runNodeRelayConnect forwards relay session stream from local SSE server', async (t) => {
  ...
  const result = await requestRelayManagementStream({
    ...
    method: 'GET',
    openTimeoutMs: TEST_RELAY_TIMEOUT_MS
  }, {
...
test('runNodeRelayConnect applies relay stream window before forwarding next chunk', async (t) => {
  ...
  assert.equal(await waitForCondition(() => registry.getRelaySession('window-node')), true);
  const session = registry.getRelaySession('window-node');
  const openedFramePromise = readJsonMessageWithTimeout(session.socket);
  ...
  const opened = await openedFramePromise;
  assert.equal(opened.type, 'relay.stream.opened');
  assert.equal(opened.ok, true);

  const firstChunk = await readJsonMessageWithTimeout(session.socket);
  ...
  const secondChunkPromise = readJsonMessageWithTimeout(session.socket);
  session.socket.send(JSON.stringify({
  ...
  const endFramePromise = readJsonMessageWithTimeout(session.socket);
  session.socket.send(JSON.stringify({
    type: 'relay.stream.ack',
    streamId: 'window-stream-1',
    credit: 1,
...
test('runNodeRelayConnect rejects non allowlisted relay stream route before local fetch', async (t) => {
  ...
  assert.equal(await waitForCondition(() => registry.getRelaySession('stream-deny-node')), true);
  const session = registry.getRelaySession('stream-deny-node');
  const framePromise = readJsonMessageWithTimeout(
    session.socket,
    TEST_RELAY_TIMEOUT_MS,
    (message) => message.type === 'relay.stream.error' && message.streamId === 'stream-1'
  );
  session.socket.send(JSON.stringify({
    type: 'relay.stream.open',
    streamId: 'stream-1',

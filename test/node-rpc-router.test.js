...
const { createRemoteNodeInvite } = require('../lib/server/remote/worker-join-invite');
...
function createManagementKey(name = 'management') {
  return `${name}-secret`;
}

const TEST_MANAGEMENT_KEY = createManagementKey();

function managementAuthorizationHeaders(managementKey = TEST_MANAGEMENT_KEY) {
...
function createDeps(overrides = {}) {
  ...
  if (typeof overrides.resolveSessionAccountRef === 'function') {
    deps.resolveSessionAccountRef = overrides.resolveSessionAccountRef;
  }
...
test('node rpc descriptor is public and does not leak configured keys', async () => {
  ...
  const handled = await handleNodeRpcRequest({
    ...
    requiredManagementKey: TEST_MANAGEMENT_KEY,
    deps: createDeps()
  });
  ...
  assert.equal(payload.result.capabilities.nodeRpc.includes('device-pair'), false);
  ...
  assert.equal(Object.hasOwn(payload.result.capabilities, 'devicePairing'), false);
...
test('node rpc client profile uses Management Key without leaking secrets', async (t) => {
  ...
  assert.equal(payload.result.server.service, 'aih-control-plane');
  assert.ok(payload.result.server.capabilities.nodeRpc.includes('device-profile'));
  assert.equal(Object.hasOwn(payload.result, 'device'), false);
  assert.doesNotMatch(res.body, new RegExp(TEST_MANAGEMENT_KEY));
});

test('node rpc client profile rejects missing and wrong Management Keys', async (t) => {
  ...
  assert.match(missingRes.body, /unauthorized_management/);

  const wrongRes = await requestProfile('Bearer wrong-token');
  assert.equal(wrongRes.statusCode, 401);
  assert.match(wrongRes.body, /unauthorized_management/);

  const validRes = await requestProfile(`Bearer ${TEST_MANAGEMENT_KEY}`);
  assert.equal(validRes.statusCode, 200);
  assert.doesNotMatch(validRes.body, new RegExp(TEST_MANAGEMENT_KEY));
});

test('node rpc device nodes lists sanitized node summaries without leaking connection secrets', async (t) => {
  ...
  const handled = await handleNodeRpcRequest({
    ...
    req: { headers: managementAuthorizationHeaders(TEST_MANAGEMENT_KEY) },
    res,
    options: {},
    state: {},
    requiredManagementKey: TEST_MANAGEMENT_KEY,
...
test('node rpc device status returns sanitized aggregate status without leaking local management details', async (t) => {
...
test('node rpc device status accepts the Management Key', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-status-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device status accepts the server Management Key', async () => {
  const res = createResCapture();
...
test('node rpc device accounts returns sanitized account summaries', async (t) => {
...
test('node rpc device provider account reauth starts oauth job through Management Key', async (t) => {
  ...
  const accountRef = upsertAccountRef(fs, aiHomeDir, {
    provider: 'codex',
    cliAccountId: '42',
    identitySeed: 'oauth:codex:node-rpc-reauth@example.com'
  });
  ...
  deps.getProfileDir = (provider, candidateRef) => path.join(aiHomeDir, 'run', 'auth-projections', provider, candidateRef);
  deps.getToolConfigDir = (provider, candidateRef) => path.join(aiHomeDir, 'run', 'auth-projections', provider, candidateRef, '.codex');
  ...
  assert.equal(payload.result.targetAccountRef, accountRef);
  ...
  assert.deepEqual(startedCalls.map((call) => ({
    ...
    accountRef: call.options.accountRef
  })), [{
    provider: 'codex',
...
test('node rpc device provider auth job get cancel and callback use Management Key', async (t) => {
  ...
  assert.doesNotMatch(getRes.body + cancelRes.body + callbackRes.body, new RegExp(TEST_MANAGEMENT_KEY));
});

test('node rpc device accounts accepts the Management Key', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-accounts-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device sessions returns sanitized session summaries', async (t) => {
...
test('node rpc device session stream emits safe SSE frames and clears poll timer on close', async (t) => {
  ...
  const req = createReqCapture(managementAuthorizationHeaders(TEST_MANAGEMENT_KEY));
...
test('node rpc device node sessions proxies safe list through Management Key bearer', async (t) => {
...
test('node rpc device node session catalog and attach proxy contract', async (t) => {
  ...
  assert.doesNotMatch(catalogRes.body + attachRes.body, new RegExp(TEST_MANAGEMENT_KEY));
...
test('node rpc device node sessions authorizes the Management Key before checking node existence', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-node-sessions-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device node session messages proxies safe snapshot through Management Key bearer', async (t) => {
...
test('node rpc device node session messages authorizes the Management Key before checking node existence', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-node-session-messages-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device node session input proxies typed write through Management Key bearer', async (t) => {
...
test('node rpc device node session input authorizes the Management Key before checking node existence', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-node-session-input-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device node session stream proxies remote node stream through Management Key bearer', async (t) => {
...
test('node rpc device node session stream authorizes the Management Key before checking node existence', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-node-session-stream-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device sessions accepts the Management Key', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-sessions-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device session messages authorizes the Management Key before checking session refs', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-session-messages-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device session events authorizes the Management Key before checking session refs', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-session-events-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device session stream authorizes the Management Key before checking session refs', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-session-stream-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc device nodes accepts the Management Key', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-device-nodes-management-key-'));
  t.after(() => fs.rmSync(aiHomeDir, { recursive: true, force: true }));
...
test('node rpc rejects unauthorized requests before status generation', async () => {
  ...
  assert.match(res.body, /unauthorized_management/);
});

...
test('runWebrtcLoop closes the active connection and stops reconnecting when aborted', async () => {
  const controller = new AbortController();
  const connection = createFakeConnection('session-1');
  let attempts = 0;
  const running = runWebrtcLoop({
    url: new URL('http://control.example.com/v0/fabric/webrtc/node/connect?nodeId=node-a'),
    nodeId: 'node-a',
    managementKey: 'node-secret',
    refreshMs: 60_000,
  ...
  assert.equal(connection.channel.closed, true);
  assert.equal(connection.peerConnection.closed, true);
  assert.equal(attempts, 1);

...
test('fetchOpenCodeModels reads opencode-go api key from account DB auth', async () => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-opencode-go-auth-'));
  try {
    const accountRef = upsertAccountRef(fs, aiHomeDir, {
      provider: 'opencode',
      cliAccountId: '1',
      identitySeed: 'oauth:opencode:models-test'
    });

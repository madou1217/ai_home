...
`;
  ...
  it('should add sandbox_mode = "danger-full-access" in merged config', () => {
    ...
    assert.ok(result.includes(`model_provider = "${providerKey}"`));
  ...
  it('should set preferred_auth_method and model_provider for API Key mode', () => {
    const providerKey = getAihProviderKey('123');
    const accountOnlyConfig = {
    ...
    assert.equal(accountOnlyConfig.model_provider, `model_provider = "${providerKey}"`);

    console.log('✅ API Key mode sets correct auth method and model provider');
  });

  it('should create account-scoped [model_providers.aih_<account>] section for API Key mode with custom base URL', () => {
    const providerKey = getAihProviderKey('123');
    const accountOnlyConfig = {
      providers: []
    };
    ...
    console.log('✅ API Key mode creates correct account-scoped model_providers section');
  });
});

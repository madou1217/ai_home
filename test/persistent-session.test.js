...
test('buildKillServerCommand kills the whole tmux server for the account socket', () => {
  ...
  assert.equal(cmd.socket, socket('codex', ACCOUNT_REF_1));
  assert.deepEqual(cmd.args, ['-u', '-L', socket('codex', ACCOUNT_REF_1), 'kill-server']);
...
test('mirror mode (-M / share): attach shared, never detach the other client', () => {
  ...
  const shared = persistentSession.buildTmuxLaunch({ command: 'claude', args: [] }, { cliName: 'claude', runtimeScope: ACCOUNT_REF_4, sessionName: base, share: true });
  const exclusive = persistentSession.buildTmuxLaunch({ command: 'claude', args: [] }, { cliName: 'claude', runtimeScope: ACCOUNT_REF_4, sessionName: base });

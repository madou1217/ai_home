...
const ACCOUNT_REF_3 = 'acct_00000000000000000003';
const ACCOUNT_REF_4 = 'acct_00000000000000000004';

function socket(provider, runtimeScope) {
  return persistentSession.deriveSocket(provider, runtimeScope);
}

test('deriveSocket is stable for account and gateway runtime scopes', () => {
  assert.equal(socket('claude', ACCOUNT_REF_1), 'aih-claude-' + ACCOUNT_REF_1);
  assert.equal(socket('claude', ACCOUNT_REF_1), 'aih-claude-' + ACCOUNT_REF_1);
  assert.notEqual(
    socket('claude', ACCOUNT_REF_1),
    socket('claude', ACCOUNT_REF_2)
  );
  assert.equal(socket('codex', persistentSession.GATEWAY_RUNTIME_SCOPE), 'aih-codex-gateway');
  assert.throws(() => socket('codex', '1'), /invalid_persistent_runtime_scope/);
});

test('deriveSessionName: stable project base by default, per-label when named', () => {
  // The stable cwd-derived name is a grouping base, not an implicit re-attach target.
...
test('buildSessionCompatibilityOptions keeps picker and launcher marker policy aligned', () => {
  assert.deepEqual(
    persistentSession.buildSessionCompatibilityOptions({
      cliName: 'codex',
      nativeWindowsPsmux: true,
...
test('deriveFreshSessionName preserves the project base and disambiguates concurrent launches', () => {
  const cwd = '/work/alpha';
  const base = persistentSession.deriveSessionName({ cwd: '/work/alpha' });
  const first = persistentSession.deriveFreshSessionName({ cwd, now: 36, pid: 72 });
  const second = persistentSession.deriveFreshSessionName({ cwd, now: 36, pid: 73 });

  assert.equal(first, `${base}-10-20`);
  assert.equal(second, `${base}-10-21`);
  assert.notEqual(first, second);
  assert.equal(first.startsWith(`${base}-`), true);
});
...
test('buildTmuxLaunch: default create uses new-session without attach or takeover flags', () => {
  const wrapped = persistentSession.buildTmuxLaunch(
    { command: '/usr/local/bin/claude', args: ['--model', 'opus'] },
    {
      ...
      cwd: '/work/project',
      sessionName: 'p-project-abc123-10-20',
      tmuxCommand: '/usr/bin/tmux',
      confPath: '/home/u/.aih/persist/tmux.conf'
    }
  );
  assert.equal(wrapped.command, '/usr/bin/tmux');
  assert.equal(wrapped.socket, socket('claude', ACCOUNT_REF_2));
  assert.equal(wrapped.session, 'p-project-abc123-10-20');
  assert.deepEqual(wrapped.args, [
    '-u', '-L', socket('claude', ACCOUNT_REF_2),
...
test('buildTmuxLaunch: explicit label yields a named concurrent window', () => {
  const wrapped = persistentSession.buildTmuxLaunch(
    { command: 'codex', args: [] },
    { cliName: 'codex', runtimeScope: ACCOUNT_REF_1, cwd: '/work/project', label: 'work2', tmuxCommand: 'tmux' }
  );
  assert.equal(wrapped.session, 's-work2');
  assert.deepEqual(wrapped.args, [
    '-u', '-L', socket('codex', ACCOUNT_REF_1),
    'new-session',
    '-e', `${persistentSession.UTF8_RUNTIME_MARKER_KEY}=${persistentSession.UTF8_RUNTIME_MARKER_VALUE}`,
    '-s', 's-work2',
    '-c', '/work/project',
...
test('buildTmuxLaunch: passes safe runtime env through tmux session env only', () => {
  ...
  assert.equal(wrapped.args.includes('-A'), false);
  assert.equal(wrapped.args.includes('-D'), false);
  assert.equal(wrapped.args.includes('OPENAI_API_KEY=sk-secret'), false);
});

test('buildTmuxLaunch: attachExisting attaches the exact session without launching the provider again', () => {
  const wrapped = persistentSession.buildTmuxLaunch(
    { command: '/usr/local/bin/claude', args: ['--model', 'opus'] },
    {
      cliName: 'claude',
  ...
  assert.deepEqual(wrapped.args, [
    '-u', '-L', socket('claude', ACCOUNT_REF_2),
    'attach-session', '-d', '-t', 'p-project-abc123-10-20'
  ]);
  assert.equal(wrapped.args.includes('new-session'), false);
  assert.equal(wrapped.args.includes('/usr/local/bin/claude'), false);
  assert.equal(wrapped.args.includes('--model'), false);
  assert.equal(wrapped.args.includes('opus'), false);
});

test('buildTmuxLaunch: detached restore keeps attach-or-create background semantics', () => {
  const wrapped = persistentSession.buildTmuxLaunch(
    { command: '/usr/local/bin/codex', args: ['resume'] },
    {
      cliName: 'codex',
      runtimeScope: ACCOUNT_REF_1,
      cwd: '/work/project',
...
test('buildSetEnvironmentCommands syncs only safe tmux env keys', () => {
  ...
  assert.deepEqual(commands.map((cmd) => cmd.args), [
    ['-u', '-L', socket('claude', ACCOUNT_REF_2), 'set-environment', '-g', 'LANG', 'en_US.UTF-8'],
    ['-u', '-L', socket('claude', ACCOUNT_REF_2), 'set-environment', '-g', 'LC_CTYPE', 'en_US.UTF-8'],
    ['-u', '-L', socket('claude', ACCOUNT_REF_2), 'set-environment', '-g', 'LC_ALL', 'en_US.UTF-8'],
    ['-u', '-L', socket('claude', ACCOUNT_REF_2), 'set-environment', '-g', 'CLAUDE_CODE_FORCE_SYNC_OUTPUT', '1'],
    ['-u', '-L', socket('claude', ACCOUNT_REF_2), 'set-environment', '-g', 'CLAUDE_CODE_DISABLE_VIRTUAL_SCROLL', '1'],
    [
      '-u',
      '-L',
      socket('claude', ACCOUNT_REF_2),
...
test('buildListSessionsCommand targets the account socket', () => {
  const cmd = persistentSession.buildListSessionsCommand({ cliName: 'claude', runtimeScope: ACCOUNT_REF_3, tmuxCommand: 'tmux' });
  const sep = persistentSession.SESSION_LIST_SEPARATOR;
  assert.equal(cmd.socket, socket('claude', ACCOUNT_REF_3));
  assert.deepEqual(cmd.args, [
    '-u',
    '-L', socket('claude', ACCOUNT_REF_3),
    'list-sessions',
    '-F', [
      '#{session_name}',
      '#{session_attached}',
...
test('buildTmuxLaunch uses strict create or exact attach with psmux', () => {
...
test('buildDetachClientCommand targets all clients attached to one safe session', () => {
  ...
  assert.equal(cmd.socket, 'aih-codex-gateway');
  ...
  assert.equal(
    persistentSession.buildDetachClientCommand({ cliName: 'codex', runtimeScope: ACCOUNT_REF_1, sessionName: 'bad name' }),
    null
  );
});
...
test('buildCapturePaneCommand targets one safe account session', () => {
  ...
  assert.equal(
    persistentSession.buildCapturePaneCommand({ cliName: 'codex', runtimeScope: ACCOUNT_REF_1, sessionName: 'bad name' }),
    null
  );
});
...
test('buildKillSessionCommand targets one safe account session', () => {
  ...
  assert.equal(cmd.socket, socket('codex', ACCOUNT_REF_1));
  assert.equal(cmd.session, 'p-ai-home-abc123');
  assert.deepEqual(cmd.args, [
    '-u',
    '-L',
  ...
  assert.equal(
    persistentSession.buildKillSessionCommand({ cliName: 'codex', runtimeScope: ACCOUNT_REF_1, sessionName: 'bad name' }),
    null
  );
});
...
test('buildSourceConfigCommand targets the account socket config reload', () => {
  const cmd = persistentSession.buildSourceConfigCommand({
    ...
    runtimeScope: ACCOUNT_REF_1,
    tmuxCommand: 'tmux',
    confPath: '/home/u/.ai_home/run/tmux/tmux.conf'
  });
  assert.equal(cmd.socket, socket('codex', ACCOUNT_REF_1));
  assert.deepEqual(cmd.args, [
    '-u',
    ...
    socket('codex', ACCOUNT_REF_1),
    'source-file',
    '/home/u/.ai_home/run/tmux/tmux.conf'
  ]);
...
test('ensureTmuxConf writes a psmux-compatible transparent config on native Windows', () => {
  ...
  assert.match(body, /set -g default-shell powershell/);
  assert.match(body, /set -g status off/);
  assert.match(body, /set -g remain-on-exit off/);
...
test('parseSessionList prefers pane current path over session path', () => {
  const sep = persistentSession.SESSION_LIST_SEPARATOR;
  ...
  assert.equal(session.path, 'C:\\Users\\madou\\projects\\feature\\ai_home');
});
...
test('parseSessionList carries persistent provider supervisor runtime marker', () => {
  const sep = persistentSession.SESSION_LIST_SEPARATOR;
  const [ready, legacy] = persistentSession.parseSessionList([
    [
      'p-ready',
  ...
  assert.equal(ready.providerSupervisorRuntimeChecked, true);
  assert.equal(ready.providerSupervisorRuntimeReady, true);
  assert.equal(persistentSession.isProviderSupervisorRuntimeReadySession(ready), true);
  assert.equal(legacy.providerSupervisorRuntimeChecked, true);
...
test('planPersistentSession always creates a fresh bare launch instead of reusing detached sessions', () => {
  const base = 'p-proj-ab12cd34';
  const plan = (sessions) => persistentSession.planPersistentSession(sessions, base, { intent: 'create' });

  // The caller already generated a collision-resistant fresh target. Create
  // remains strict even if a probe reports a collision; tmux must fail instead
  // of silently attaching to any existing provider process.
  assert.deepEqual(plan([]), { session: base, action: 'new' });
  assert.deepEqual(plan([{ name: base, attached: false }]), { session: base, action: 'new' });
  assert.deepEqual(plan([{ name: base, attached: true }]), { session: base, action: 'new' });
  assert.deepEqual(
    plan([{ name: base, attached: true }, { name: `${base}-2`, attached: false }]),
    { session: base, action: 'new' }
  );
});

test('planPersistentSession exact selection never creates or substitutes another session', () => {
  const base = 'p-proj-ab12cd34';

  assert.deepEqual(
    persistentSession.planPersistentSession([], base, { intent: 'select' }),
    { session: base, action: 'missing' }
  );
  assert.deepEqual(
    persistentSession.planPersistentSession(
      [{ name: base, attached: false, utf8RuntimeChecked: true, utf8Runtime: '' }],
      base,
      { intent: 'select', requireUtf8Runtime: true }
    ),
    { session: base, action: 'unavailable', reason: 'utf8-runtime' }
  );
  assert.throws(
    () => persistentSession.planPersistentSession([], base, { intent: 'typo' }),
    /invalid_persistent_session_intent/
  );
});
...
test('planPersistentSession opens a fresh compatible session for legacy UTF-8 runtime', () => {
  ...
  assert.deepEqual(
    persistentSession.planPersistentSession(
      [{ name: base, attached: false, utf8RuntimeChecked: true, utf8Runtime: '' }],
      base,
      { intent: 'upsert', requireUtf8Runtime: true }
    ),
...
test('planPersistentSession opens a fresh compatible session for legacy Claude render runtime', () => {
  ...
  assert.deepEqual(
    persistentSession.planPersistentSession(
      ...
      base,
      { intent: 'upsert', requireClaudeRenderRuntime: true }
    ),
    { session: `${base}-2`, action: 'new-compatible' }
  );
  assert.deepEqual(
...
test('planPersistentSession opens a fresh compatible session for legacy provider supervisor runtime', () => {
  const base = 'p-proj-ab12cd34';
  const ready = {
    name: `${base}-2`,
    attached: false,
    providerSupervisorRuntimeChecked: true,
    providerSupervisorRuntime: persistentSession.PROVIDER_SUPERVISOR_RUNTIME_MARKER_VALUE
  };
  ...
  assert.deepEqual(
    persistentSession.planPersistentSession(
      ...
      base,
      { intent: 'upsert', requireProviderSupervisorRuntime: true }
    ),
    { session: `${base}-2`, action: 'new-compatible' }
  );
  assert.deepEqual(
...
test('planPersistentSession opens a fresh session for completed dead panes', () => {
  ...
  assert.deepEqual(
    ...
    { session: `${base}-3`, action: 'new-completed' }
  );
});
...
test('planPersistentSession honors an explicit -S label (takeover when live)', () => {
  const named = 's-debug';
  const plan = (sessions) => persistentSession.planPersistentSession(
    sessions,
    named,
    { intent: 'upsert' }
  );
...
test('describeSessionList groups this-project vs others with runnable commands', () => {
  ...
  const v = persistentSession.describeSessionList(sessions, { cliName: 'claude', cliSelector: '4', cwd });
  assert.equal(v.hasCwd, true);
  // this-project rows
  assert.deepEqual(v.here.map((r) => r.command), [
    'aih claude sessions 4',  // automatic names must be selected explicitly
    'aih claude sessions 4',
  ...
  assert.deepEqual(v.others.map((r) => r.command), ['aih claude sessions 4']);
});
...
test('describeSessionList recommends takeover commands when shared attach is unavailable', () => {
  ...
  assert.deepEqual(noShareView.here.map((row) => row.command), [
    'aih codex sessions 1',
    'aih codex 1 -R -S debug'
  ]);
  assert.deepEqual(noShareView.others.map((row) => row.command), ['aih codex sessions 1']);
});
...
test('describeSessionList recommends compatible launch commands for legacy UTF-8 runtime rows', () => {
  ...
  const v = persistentSession.describeSessionList(sessions, { cliName: 'codex', cliSelector: '1', cwd });
...
test('describeSessionList recommends fresh launches for legacy psmux Codex launch runtime rows', () => {
  ...
  ], { cliName: 'codex', cliSelector: '1', cwd, shareLive: false });
...
test('describeSessionList recommends fresh launches for completed terminal screens', () => {
  ...
  ], { cliName: 'codex', cliSelector: '1', cwd });
...
test('mirror mode (-M / share): attach shared, never detach the other client', () => {
  ...
  assert.deepEqual(
    persistentSession.planPersistentSession(
      [{ name: base, attached: true }],
      base,
      { intent: 'select', share: true }
    ),
  ...
  assert.deepEqual(
    persistentSession.planPersistentSession(
      [{ name: base, attached: true }],
      base,
      { intent: 'select', share: false }
    ),
    { session: base, action: 'takeover' }
  );
  // buildTmuxLaunch: shared attach omits -d; exclusive attach detaches the old client.
  const shared = persistentSession.buildTmuxLaunch(
    { command: 'claude', args: [] },
    { cliName: 'claude', runtimeScope: ACCOUNT_REF_4, sessionName: base, share: true, attachExisting: true }
  );
  const exclusive = persistentSession.buildTmuxLaunch(
    { command: 'claude', args: [] },
    { cliName: 'claude', runtimeScope: ACCOUNT_REF_4, sessionName: base, attachExisting: true }
  );
  assert.equal(shared.args.includes('attach-session'), true);
  assert.equal(exclusive.args.includes('attach-session'), true);

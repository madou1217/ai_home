...
test('home-redirect: agy injects keychain bypass without provider-specific Codex behavior', () => {
...
test('opencode prepare discards stale runtime data instead of importing directory state', () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(sharedDataDir, 'log', 'opencode.log')), false);
    ...
    assert.equal(fs.existsSync(path.join(sharedDataDir, 'opencode.db')), false);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

test('opencode prepare prefers shared truth when both sides hold the same shared entry', () => {
  ...
  try {
    ...
    const sharedDataDir = path.join(hostHome, '.local', 'share', 'opencode');
    fs.mkdirSync(bridgeDataDir, { recursive: true });
    // 共享侧已有权威 db;账号本地又存了一个陈旧真实 db —— 应保留共享侧,丢弃账号本地副本(不分裂真相)。

...
test('link paths per provider: claude/codex link, opencode/agy managed, gemini skipped', () => {
  ...
  assert.equal(getSkillLinkPath(claude), path.join(claude.hostHomeDir, '.claude', 'skills', SKILL_NAME));
  assert.equal(getSkillLinkPath(codex), path.join(codex.hostHomeDir, '.codex', 'skills', SKILL_NAME));
  ...
  assert.equal(getManagedInstructionPath(agy), path.join(agy.hostHomeDir, '.gemini', 'GEMINI.md'));
...
test('ensureProviderSkillInstalled: claude installs link, opencode writes block, login skips', () => {
  ...
  assert.equal(fs.existsSync(path.join(claudeResult.target, 'SKILL.md')), true);
...
test('AIH_SKILL_INJECT=0 uninstalls both shapes', () => {
  ...
  assert.equal(fs.existsSync(claudeOff.target), false);

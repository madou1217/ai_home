...
test('remote node registry stores nodes and transports without credential payloads', (t) => {
  ...
  const registryText = JSON.stringify(readJsonValue(fs, aiHomeDir, REMOTE_REGISTRY_KEY));
  assert.doesNotMatch(registryText, /mgmt-secret/);
  assert.match(JSON.stringify(readJsonValue(fs, aiHomeDir, REMOTE_SECRET_KEY)), /mgmt-secret/);

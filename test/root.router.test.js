...
function createHarness(overrides = {}) {
  ...
  const deps = {
    ...
    updateContext: {},
    runBackgroundSupervisor: async () => ({ ok: true, stopped: true }),
    backgroundContext: {},
...
test('runCliRootRouter maps bare account id to unique tool router', async () => {
  ...
  const h = createHarness({
    ...
    listCliAccountRefRecords: (_fs, _aiHomeDir, provider) => (
      provider === 'codex' ? [{ cliAccountId: '123' }] : []
    ),
...
test('runCliRootRouter maps usage id shorthand to unique tool router', async () => {
  ...
  const h = createHarness({
    ...
    listCliAccountRefRecords: (_fs, _aiHomeDir, provider) => (
      provider === 'codex' ? [{ cliAccountId: '456' }] : []
    ),
    runAiCliCommandRouter: (cmd, args) => calls.push({ cmd, args })
  });
...
test('runCliRootRouter routes the private background supervisor entry', async () => {
  const calls = [];
  const backgroundContext = { aiHomeDir: '/tmp/.ai_home' };
  const h = createHarness({
    backgroundContext,
    runBackgroundSupervisor: async (context) => {
      calls.push(context);
      return { ok: true, stopped: true };
    }
  });

  await runCliRootRouter(['__background', 'run'], h.deps);

  assert.deepEqual(calls, [backgroundContext]);
  assert.equal(h.getExitCode(), 0);
  assert.equal(h.events.some((event) => event.startsWith('ai:')), false);
});

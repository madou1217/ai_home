...
test('runtime projection pruner keeps registered accountRefs and removes stale entries', (t) => {
  ...
  const result = pruneStaleAccountRuntimeProjections({ fs, path, aiHomeDir });

  assert.deepEqual(result, { removed: 4, kept: 3, failed: 0 });
  ...
  assert.equal(fs.existsSync(invalidAccountDir), false);
  assert.equal(fs.existsSync(unknownProviderDir), false);

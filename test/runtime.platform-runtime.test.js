...
test('collectCommandCandidates includes Windows PowerShell shims', (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-command-candidates-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const ps1Path = path.join(root, 'codex.ps1');
  ...
  assert.deepEqual(candidates, [ps1Path, cmdPath]);
});

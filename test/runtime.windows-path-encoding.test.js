...
test('normalizeHostPathForLookup maps Windows long paths to WSL mount paths', () => {
  assert.equal(
    normalizeHostPathForLookup('\\\\?\\C:\\Users\\madou\\projects\\feature\\ai_home\\', { platform: 'linux' }),
    '/mnt/c/Users/madou/projects/feature/ai_home'
  );
});

test('normalizeHostPathForLookup maps WSL mount paths to Windows paths on win32', () => {
  assert.equal(
    normalizeHostPathForLookup('/mnt/c/Users/madou/projects/feature/ai_home/', { platform: 'win32' }),
    'C:\\Users\\madou\\projects\\feature\\ai_home'
  );
});

test('buildHostPathLookupVariants includes Windows and WSL equivalents', () => {
  const variants = buildHostPathLookupVariants('/mnt/c/Users/madou/projects/feature/ai_home', {
    platform: 'linux'
  });
  assert.equal(variants.includes('/mnt/c/Users/madou/projects/feature/ai_home'), true);
  assert.equal(variants.includes('C:\\Users\\madou\\projects\\feature\\ai_home'), true);
  assert.equal(variants.includes('\\\\?\\C:\\Users\\madou\\projects\\feature\\ai_home'), true);
});

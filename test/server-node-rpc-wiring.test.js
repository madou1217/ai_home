...
function createServerDeps(aiHomeDir, processObj, relaySessionRegistry, overrides = {}) {
  return {
    ...
    getToolConfigDir: (provider, accountRef) => path.join(aiHomeDir, 'run', 'auth-projections', String(provider || ''), String(accountRef || '')),
    getProfileDir: (provider, accountRef) => path.join(aiHomeDir, 'run', 'auth-projections', String(provider || ''), String(accountRef || '')),
...
function seedRemoteNode(aiHomeDir) {
...
test('running Server rotates Management Key without loopback bypass or restart', async (t) => {
  ...
  const unauthenticated = await fetch(`${endpoint}/v0/webui/server-config/management-key/rotate`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ managementKey: newKey })
  });
  assert.equal(unauthenticated.status, 401);

  const rotated = await fetch(`${endpoint}/v0/webui/server-config/management-key/rotate`, {
    method: 'POST',
    headers: {
  ...
  assert.equal(rotated.status, 200, rotatedText);
  assert.equal(rotated.headers.get('cache-control'), 'no-store');
  const rotatedBody = JSON.parse(rotatedText);
  assert.equal(rotatedBody.ok, true);
  assert.equal(JSON.stringify(rotatedBody).includes(newKey), false);

  const rejectedOldKey = await fetch(`${endpoint}/v0/management/status`, {
    headers: { authorization: `Bearer ${oldKey}` }
  });
  assert.equal(rejectedOldKey.status, 401);

  const acceptedNewKey = await fetch(`${endpoint}/v0/management/status`, {
    headers: { authorization: `Bearer ${newKey}` }
  });
  assert.equal(acceptedNewKey.status, 200, await acceptedNewKey.text());
});

test('running Server protects loopback WebUI data with the Management Key', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-server-loopback-webui-auth-'));
  const port = await getFreePort();
  const endpoint = `http://127.0.0.1:${port}`;
  const processObj = createProcessCapture();
  const relaySessionRegistry = { closeAll() {} };
  ...
  for (const pathname of [
    ...
    assert.equal(response.status, 401, `${pathname} must reject an unauthenticated loopback client`);
    assert.equal((await response.json()).error, 'webui_unauthorized');
  }

  const wrongKey = await fetch(`${endpoint}/v0/webui/accounts`, {
    headers: { authorization: 'Bearer wrong-management-key' }
  });
  assert.equal(wrongKey.status, 401);

  const authorized = await fetch(`${endpoint}/v0/webui/management/status`, {
    headers: { authorization: `Bearer ${SERVER_MANAGEMENT_KEY}` }
  });
  assert.equal(authorized.status, 200, await authorized.text());

  const providerHook = await fetch(`${endpoint}/v0/webui/session-events/provider-hook`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({
      provider: 'codex',
      payload: {
        session_id: 'loopback-hook-session',
        cwd: '/tmp/loopback-hook-project',
...
test('server WebUI resolves SSH identity files from the configured SSH home', async (t) => {
  ...
  const sshDir = path.join(sshHomeDir, '.ssh');
  const identityFileName = 'server-wiring-test.pem';
  fs.ensureDirSync(sshDir);
  fs.writeFileSync(path.join(sshDir, identityFileName), 'test-private-key', { mode: 0o600 });
  const port = await getFreePort();
  ...
  const response = await fetch(`${controlEndpoint}/v0/webui/ssh-connections`, {
    method: 'POST',
    headers: {
...
test('server fabric descriptor advertises Management Key and removed client credential endpoints', async (t) => {
  ...
  await startLocalServer({
    ...
    createOutboundRelayManager: (input) => ({
      start: async (config) => { outboundStarts.push({ input, config }); },
      stop: async () => { outboundStops += 1; },
  ...
  assert.deepEqual(descriptor.result.auth.methods, ['management-key']);
  assert.equal(descriptor.result.auth.managementKeyConfigured, true);
  assert.equal(descriptor.result.capabilities.legacyControlPlane.nodeRpc.includes('device-pair'), false);
  ...
  const removedResponse = await fetch(`${controlEndpoint}/v0/fabric/device-pair`, {
    method: 'POST',
    headers: {
      authorization: `Bearer ${SERVER_MANAGEMENT_KEY}`,
      'content-type': 'application/json'
...
test('running Server exposes authenticated relay and FRP configuration APIs end to end', async (t) => {
  ...
  const relayResponse = await fetch(`${endpoint}/v0/webui/server-routes/relays`, {
    method: 'PUT',
    headers: {
      authorization: `Bearer ${SERVER_MANAGEMENT_KEY}`,
      'content-type': 'application/json'
    ...
    body: JSON.stringify({
      relays: [{
        ...
        name: 'Tokyo',
        enabled: true,
        managementKey: relayKey
      }]
    })
  });
  const relayText = await relayResponse.text();
  assert.equal(relayResponse.status, 200, relayText);
...
test('server fabric transport echo endpoint runs on the existing server listener', async (t) => {
  ...
  const rejected = await runFabricTransportEcho([
  ...
  assert.equal(rejected.failures[0].error, 'echo_upgrade_rejected');
...
test('server fabric webrtc signaling endpoint exchanges lab messages', async (t) => {
  ...
  const unauthenticated = await fetch(`${controlEndpoint}/v0/fabric/webrtc/signaling/rooms`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ name: 'Unauthorized node test' })
  });
  assert.equal(unauthenticated.status, 401);
...
test('server fabric registry uses Management Key and mirrors node-rpc views', async (t) => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-server-fabric-registry-'));
  const port = await getFreePort();
  const controlEndpoint = `http://127.0.0.1:${port}`;
  const processObj = createProcessCapture();
  const relaySessionRegistry = { closeAll() {} };
  const managementKey = SERVER_MANAGEMENT_KEY;
...
test('server node-rpc device nodes uses shared relay registry for mobile status', async (t) => {
  ...
  const port = await getFreePort();
  const controlEndpoint = `http://127.0.0.1:${port}`;
  const credential = seedRemoteNode(aiHomeDir);
  const processObj = createProcessCapture();
  ...
  const response = await fetch(`${controlEndpoint}/v0/node-rpc/device-nodes`, {
    headers: {
      authorization: `Bearer ${credential.managementKey}`
    }
  });
...
test('server node-rpc starts native session through injectable runtime service', async (t) => {
  ...
  await startLocalServer({
    ...
    startNativeDeviceSession(payload, runtimeDeps = {}) {
      const accountRef = payload.accountRef || runtimeDeps.resolveSessionAccountRef(payload);
      observed.start = { ...payload, accountRef };
      return {
        accepted: true,
        mode: 'native-session',
...
test('server node-rpc device node session start and run controls forward through relay management', async (t) => {
  ...
  const eventsResponse = await fetch(`${controlEndpoint}/v0/node-rpc/device-node-session-run-events?nodeId=home-win&runId=run-remote-1&cursor=0`, {
    headers: { authorization: `Bearer ${credential.managementKey}` }
  });
  assert.equal(eventsResponse.status, 200);
  const events = await eventsResponse.json();

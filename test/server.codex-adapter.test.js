...
test('codex adapter returns pool unavailable when 401 remains after refresh', async () => {
  const res = createResCapture();
  const account = { id: '10025', email: 'code3@example.com', accountId: 'acc_10025', accessToken: 'bad-token', refreshToken: 'bad-rt' };
  const state = {
  ...
  await handleCodexChatCompletions({
    ...
    deps: {
      chooseServerAccount: (pool, _state, _cursor, options = {}) => {
        ...
        return pool.find((item) => !excludeIds.has(String(item.id || '')) && Date.now() >= Number(item.cooldownUntil || 0)) || null;
      },
      pushMetricError: () => {},
      writeJson: (r, code, payload) => {
        r.statusCode = code;
  ...
  assert.deepEqual(failures, [{ id: '10025', reason: 'auth_invalid_reauth_required' }]);
...
test('codex adapter retries another account on deactivated workspace 402', async () => {
  ...
  const state = {
    accounts: {
      codex: [
        { id: '8', email: 'code8@example.com', accountId: 'acc_8', accessToken: 'deactivated-token' },
        { id: '9', email: 'code9@example.com', accountId: 'acc_9', accessToken: 'ok-token' }
      ]
    },
    cursors: { codex: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0 }
  ...
  await handleCodexChatCompletions({
    ...
    deps: {
      ...
      markProxyAccountFailure: (account, reason) => failures.push({ id: account.id, reason }),
      markProxyAccountSuccess: () => {},
      appendProxyRequestLog: () => {}
    }
  });

  assert.deepEqual(chosen, ['8', '9']);
  assert.deepEqual(failures, [{ id: '8', reason: 'deactivated_workspace' }]);
  assert.equal(res.statusCode, 200);
...
test('codex adapter retries another account when 200 SSE reports model capacity failure', async () => {
  ...
  const state = {
    accounts: {
      codex: [
        { id: '1', email: 'a@example.com', accountId: 'acc_1', accessToken: 'capacity-token' },
        { id: '2', email: 'b@example.com', accountId: 'acc_2', accessToken: 'ok-token' }
      ]
    },
    cursors: { codex: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0 }
  ...
  await handleCodexChatCompletions({
    ...
    deps: {
      ...
      markProxyAccountFailure: (account, reason, cooldownMs, threshold, opts) => {
        failures.push({ id: account.id, reason, opts });
        markProxyAccountFailure(account, reason, cooldownMs, threshold, opts);
      },
      markProxyAccountSuccess: () => {},
      appendProxyRequestLog: () => {}
  ...
  await handleCodexChatCompletions({
    ...
    deps: {
      chooseServerAccount: (pool, selectionState, cursorKey, options) => {
        ...
        if (account) chosen.push(account.id);
        return account;
      },
      pushMetricError: () => {},
      writeJson: (r, code, payload) => {
...
test('codex adapter logs stream disconnect account failure and retries another account', async () => {
  ...
  const state = {
    accounts: {
      codex: [
        { id: '1', email: 'a@example.com', accountId: 'acc_1', accessToken: 'disconnect-token' },
        { id: '2', email: 'b@example.com', accountId: 'acc_2', accessToken: 'ok-token' }
      ]
    },
    cursors: { codex: 0 },
...
test('codex adapter hides stream disconnect detail when all account attempts are exhausted', async () => {
  ...
  const disconnectDetail = 'stream disconnected before completion: An error occurred while processing your request. '
    + 'Please include the request ID 4d251fd0-862a-4b1f-90a3-fb3ed9629f18 in your message.';
  const account = { id: '1', email: 'a@example.com', accountId: 'acc_1', accessToken: 'disconnect-token' };
  const state = {
    accounts: { codex: [account] },
    cursors: { codex: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0 }

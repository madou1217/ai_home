...
test('patchAccountReadResponse does not use default account after mobile pointer is cleared', () => {
  ...
  fs.mkdirSync(accountDir, { recursive: true });
  fs.writeFileSync(path.join(codexHome, 'config.toml'), 'model_provider = "aih_10"\nmodel = "gpt-5.5"\n', 'utf8');
  fs.writeFileSync(path.join(tmpHome, '.ai_home', 'profiles', 'codex', '.aih_default'), '10038', 'utf8');
  fs.writeFileSync(path.join(accountDir, 'auth.json'), JSON.stringify({
    tokens: {
      access_token: createJwt({
    ...
    fs: {
      ...
      readFileSync(filePath) {
        if (String(filePath || '') === '/tmp/state.json') {
          return JSON.stringify({ enabled: true, desktopAccountId: '10038' });
        }
...
test('buildCodexAppServerSpawnEnv keeps host Codex home for API-key default instead of OAuth fallback', () => {
  ...
  const result = buildCodexAppServerSpawnEnv(fs, { enabled: true, desktopAccountId: '4' }, {
    processObj: {
      platform: 'darwin',
      env: {
        HOME: tmpHome,
  ...
  assert.equal(fs.existsSync(path.join(tmpHome, '.ai_home', 'codex-desktop-runtime', 'app-server-4', 'auth.json')), false);
});
...
test('buildCodexAppServerSpawnEnv keeps original env when no desktop account auth is available', () => {
  ...
  fs.writeFileSync(path.join(tmpHome, '.ai_home', 'profiles', 'codex', '.aih_default'), '10009', 'utf8');
  fs.writeFileSync(path.join(profileCodexHome, 'auth.json'), JSON.stringify({
    tokens: {
      access_token: createJwt({
        'https://api.openai.com/profile': { email: 'default@example.com' },
  ...
  assert.equal(fs.existsSync(path.join(tmpHome, '.ai_home', 'codex-desktop-runtime', 'app-server-10009', 'auth.json')), false);
});

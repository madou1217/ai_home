...
test('runServerCommand config shows secrets only when explicitly requested', async () => {
  const config = {
    host: '127.0.0.1',
    port: 9527,
    apiKey: 'client-secret',
  ...
  const textResult = await captureConsole(() => runServerCommand([
    'server', 'config', '--show-secrets'
  ], deps));
  assert.equal(textResult.result, 0);
  assert.match(textResult.logs.join('\n'), /api_key: client-secret/);
  assert.match(textResult.logs.join('\n'), /management_key: management-secret/);

  const jsonResult = await captureConsole(() => runServerCommand([
    'server', 'config', '--show-secrets', '--json'
  ], deps));
  assert.equal(jsonResult.result, 0);
  const payload = JSON.parse(jsonResult.logs.join('\n'));
  assert.equal(payload.config.apiKey, 'client-secret');
  assert.equal(payload.config.managementKey, 'management-secret');
});

...
test('daemon service restart reloads loaded launchd autostart with saved serve config', async () => {
  ...
  const commandForPid = (pid) => {
    ...
    if (pid === 2002 && newAlive) {
      return '/usr/local/bin/node /repo/lib/cli/app.js server serve --host 0.0.0.0 --port 9527 --api-key client-secret --management-key management-secret';
    }
    return '';
  };
  ...
  const daemon = createServerDaemonService({
    ...
    spawnSync(cmd, args) {
      ...
      if (cmd === 'launchctl') {
        ...
        if (args[0] === 'list') return launchdLoaded ? { status: 0, stdout: '', stderr: '' } : { status: 1, stdout: '', stderr: '' };
        if (args[0] === 'unload') {
          launchdLoaded = false;
          oldAlive = false;
  ...
  assert.equal(spawnCalls.length, 0);
  assert.equal(fs.readFileSync(pidFile, 'utf8').trim(), '2002');
  assert.equal(launchctlCalls.some((args) => args[0] === 'unload' && args[1] === launchdPlist), true);
  assert.equal(launchctlCalls.some((args) => args[0] === 'load' && args[1] === launchdPlist), true);
  assert.match(plist, /<string>--host<\/string>\s+<string>0\.0\.0\.0<\/string>/);
  assert.match(plist, /<string>--management-key<\/string>\s+<string>management-secret<\/string>/);
});
...
test('daemon service installs macOS launchd autostart with saved serve config', () => {
  const root = makeTempDir();
  const aiHomeDir = path.join(root, '.ai_home');
  const launchdPlist = path.join(root, 'Library', 'LaunchAgents', 'com.clawdcodex.ai_home.plist');
  ...
  assert.match(plist, /<string>server<\/string>\s+<string>serve<\/string>/);
  assert.match(plist, /<string>--port<\/string>\s+<string>9527<\/string>/);
  assert.equal(plist.includes('/usr/local/bin/node'), false);
  assert.equal(fs.existsSync(legacyLaunchdPlist), false);
  assert.equal(calls.some((call) => call.cmd === 'launchctl' && call.args[0] === 'unload' && call.args[1] === legacyLaunchdPlist), true);

...
test('provider executor snapshots always expose numeric concurrency', () => {
  const executor = createProviderExecutor('opencode');
  const snapshot = executor.snapshot();

  assert.equal(snapshot.name, 'opencode');
  assert.equal(snapshot.running, 0);
  assert.equal(snapshot.queued, 0);
  assert.equal(snapshot.maxConcurrency, 1);
  assert.equal(snapshot.queueLimit, 1);
});

...
test('management payloads prefer persisted runtime state over healthy in-memory account', () => {
  ...
  const state = {
    ...
    accounts: {
      codex: [],
      gemini: [
        { id: 'g1', provider: 'gemini', email: 'g@example.com', cooldownUntil: 0, authInvalidUntil: 0 }
      ],
      claude: []
    },
    metrics: {
  ...
  const accountStateIndex = {
    getAccountState(provider, accountId) {
      if (provider === 'gemini' && accountId === 'g1') {
        return {
          runtimeState: {
...
test('management accounts payload shows exhausted quota for free agy model quota cooldowns', () => {
  const now = Date.now();
  const runtimeState = {

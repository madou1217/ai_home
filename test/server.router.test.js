...
test('resolveRequestProvider respects explicit mode and model hint', () => {
  ...
  assert.equal(resolveRequestProvider({ provider: 'auto' }, { model: 'gemini-3.1-pro-high' }), 'agy');
...
test('resolveRequestProvider prefers account and cache model availability over codex fallback', () => {
  const state = {
    accounts: {
      codex: [{
        id: '10014',

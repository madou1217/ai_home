...
function startUpstreamStream(options = {}) {
  ...
  const model = String(options.model || 'claude-sonnet-4');
  const requestUrl = String(options.requestUrl || '/v1/messages');
  const res = options.res || createResCapture();
  const account = {
    accountRef: accountRef(`${provider}-oauth-stream`),
  ...
  const completion = handleUpstreamPassthrough({
    options: {
      ...
      maxAttempts: 1,
      failureThreshold: 1,
      logRequests: Boolean(options.logRequests)
    },
    state,
    req: {
      url: requestUrl,
    ...
    res,
    method: 'POST',
    bodyBuffer: Buffer.from(JSON.stringify({ model, stream: true, messages: [] })),
    requestJson: { model, stream: true, messages: [] },
    routeKey: `POST ${requestUrl}`,
    requestStartedAt: Date.now(),
    cooldownMs: 1000,
    deps: {
      chooseServerAccount: (pool) => pool[0],
      resolveRequestProvider: (serverOptions, requestJson, headers) => require('../lib/server/router').resolveRequestProvider(serverOptions, requestJson, headers),
      pushMetricError: () => {},
      ...
      fetchWithTimeout: async (_url, init, timeoutMs) => {
        ...
        return {
          ...
          arrayBuffer: options.arrayBuffer || (async () => {
            throw new Error('stream_must_not_buffer');
          })
        };
      },
      markProxyAccountFailure: options.markProxyAccountFailure || (() => {}),
      markProxyAccountSuccess: options.markProxyAccountSuccess || (() => {}),
      recordModelUsage: options.recordModelUsage,
      appendProxyRequestLog: options.appendProxyRequestLog || (() => {})
    }
  });
...
function chooseAvailableAccount(pool, _state, _cursorKey, options = {}) {
  ...
  return (pool || []).find((account) => {
    const ref = String(account && account.accountRef || '');
    if (ref && excludedRefs.has(ref)) return false;
    return now >= Number(account && account.cooldownUntil || 0);
  }) || null;
}

async function runClaudeRefreshFailureScenario(refreshResult, options = {}) {
  const res = createResCapture();
  const account = {
    accountRef: accountRef('claude-refresh-failure'),
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      ...
      refreshClaudeAccessToken: async (_selectedAccount, refreshOptions) => {
        refreshCalls += 1;
        return refreshOptions && refreshOptions.force
          ? refreshResult
          : { ok: true, refreshed: false, reason: 'not_due' };
      },
...
async function runAgyCodeAssistAuthScenario(options = {}) {
  const res = createResCapture();
  const transport = String(options.transport || 'chat');
  const transportConfig = {
    chat: {
      url: '/v1/chat/completions',
      requestJson: { model: 'agy-gemini-3-flash', messages: [] },
      requestMeta: { sessionKey: 'agy-auth-recovery-thread' },
      successPayload: { id: 'chatcmpl-agy-recovered', object: 'chat.completion', choices: [] }
    },
    anthropic: {
      url: '/v1/messages',
      requestJson: { model: 'claude-opus-4-6-thinking', messages: [{ role: 'user', content: 'ping' }] },
      requestMeta: {
        sessionKey: 'agy-auth-recovery-thread',
        providerProtocolRoute: AGY_ANTHROPIC_MESSAGES_ROUTE,
        sourceClientProtocol: 'anthropic_messages'
      },
      successPayload: {
        id: 'msg_agy_recovered',
        type: 'message',
        role: 'assistant',
    ...
    gemini: {
      url: '/v1beta/models/gemini-3-flash-agent:generateContent',
      requestJson: {
  ...
  assert.ok(transportConfig, `unsupported test transport: ${transport}`);
  const account = {
    accountRef: accountRef('agy-auth-recovery'),
  ...
  const runAdapter = async () => {
    adapterCalls += 1;
    if (options.succeedAfterRefresh && adapterCalls > 1) return transportConfig.successPayload;
    const error = new Error('HTTP 401 invalid OAuth credentials');
    error.code = 'HTTP_401';
    throw error;
  };
  const adapterDeps = {
    chat: { fetchGeminiCodeAssistChatCompletion: runAdapter },
    anthropic: { fetchCodeAssistAnthropicMessage: runAdapter },
    gemini: { fetchGeminiCodeAssistGenerateContent: runAdapter }
  }[transport];
  ...
  await handleUpstreamPassthrough({
    ...
    res,
    method: 'POST',
    bodyBuffer: Buffer.from(JSON.stringify(transportConfig.requestJson)),
    requestJson: transportConfig.requestJson,
    routeKey: `POST ${transportConfig.url}`,
    requestStartedAt: Date.now(),
    cooldownMs: 1000,
    requestMeta: transportConfig.requestMeta,
    deps: {
      chooseServerAccount: chooseAvailableAccount,
      pushMetricError: () => {},
      writeJson: (response, code, payload) => {
      ...
      refreshAgyAccessToken: async (selectedAccount, refreshOptions) => {
        if (!refreshOptions.force) return { ok: true, refreshed: false, reason: 'not_due' };
        forcedRefreshCalls += 1;
        const refreshResult = options.refreshResult || { ok: true, refreshed: true };
        if (refreshResult.ok && refreshResult.refreshed) selectedAccount.accessToken = 'fresh-token';
...
test('upstream passthrough strips hop-by-hop headers before fetch', async () => {
  ...
  const state = {
    accounts: { codex: [{ accountRef: accountRef('1'), email: 'a@example.com', accessToken: 'tok' }] },
    cursors: { codex: 0 },
  ...
  assert.equal(seenHeaders['x-aih-account-ref'], accountRef('1'));
...
test('upstream passthrough records the requested billing model instead of a response wire id', async () => {
  const res = createResCapture();
  const records = [];
  const successes = [];
  const state = {
    accounts: { gemini: [{ accountRef: accountRef('9'), email: 'g@example.com', accessToken: 'tok' }] },
    cursors: { gemini: 0 },
    metrics: {
      totalFailures: 0,
      totalSuccess: 0,
  ...
  await handleUpstreamPassthrough({
    ...
    requestJson: { model: 'gemini-3-flash-agent' },
    requestMeta: {
    ...
    deps: {
      ...
      markProxyAccountFailure: () => {},
      markProxyAccountSuccess: (account) => successes.push(account.accountRef),
  ...
  assert.equal(res.statusCode, 200);
  assert.equal(state.metrics.totalSuccess, 1);
  assert.deepEqual(successes, [accountRef('9')]);
  assert.equal(records.length, 1);
  assert.deepEqual({
    provider: records[0].provider,
    accountRef: records[0].accountRef,
...
test('upstream passthrough keeps a bare token-endpoint 401 recoverable after codex attempts are exhausted', async () => {
  const res = createResCapture();
  const accounts = [
    { accountRef: accountRef('10023'), email: 'code1@example.com', accessToken: 'bad-1', cooldownUntil: 0 },
    { accountRef: accountRef('10025'), email: 'code3@example.com', accessToken: 'bad-2', cooldownUntil: 0 }
  ];
  const state = {
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      ...
      markProxyAccountFailure: (account, reason, cooldownMs, threshold) => {
        failures.push({ accountRef: account.accountRef, reason });
  ...
  assert.deepEqual(failures, [
    { accountRef: accountRef('10023'), reason: 'token_refresh_refresh_http_401' },
    { accountRef: accountRef('10025'), reason: 'token_refresh_refresh_http_401' }
  ]);
  ...
  assert.match(
    String(body.detail || ''),
    /runtime:token_refresh_unavailable:token_refresh_refresh_http_401=2/u
  );
  assert.equal(
    body.availability.reasons.some((entry) => String(entry.reason || '').includes('auth_invalid')),
    false
  );
});
...
test('upstream passthrough honors explicit x-provider and x-account-id headers over model inference', async () => {
  ...
  const state = {
    accounts: {
      codex: [{ accountRef: accountRef('10000'), email: 'codex@example.com', accessToken: 'codex-token' }],
      claude: [
        { accountRef: accountRef('3'), email: 'claude-3@example.com', accessToken: 'claude-token-3', baseUrl: 'https://claude.example.com' },
        { accountRef: accountRef('4'), email: 'claude-4@example.com', accessToken: 'claude-token-4', baseUrl: 'https://claude.example.com' }
      ]
    },
...
test('upstream passthrough sends OAuth bearer headers for claude OAuth accounts', async () => {
  const res = createResCapture();
  let seenHeaders = null;
  let seenTimeoutMs = 0;
  const state = {
    ...
    deps: {
      ...
      fetchWithTimeout: async (_url, init, timeoutMs) => {
        ...
        return {
          status: 200,
          headers: new Map(),
          arrayBuffer: async () => zlib.gzipSync(Buffer.from('{"id":"msg_oauth","type":"message"}'))
        };
      },
      markProxyAccountFailure: () => {},
      markProxyAccountSuccess: () => {},
  ...
  assert.equal(res.statusCode, 200);
  assert.equal(seenHeaders.authorization, 'Bearer claude-oauth-token');
  assert.equal(seenHeaders['anthropic-version'], '2023-06-01');
  assert.equal(seenHeaders['anthropic-beta'], 'oauth-2025-04-20');
  assert.equal(Object.hasOwn(seenHeaders, 'x-api-key'), false);
  assert.equal(seenTimeoutMs, 3000);
  assert.deepEqual(JSON.parse(res.body.toString('utf8')), { id: 'msg_oauth', type: 'message' });
  assert.equal(res.headers['content-type'], 'application/json; charset=utf-8');
  assert.equal(Object.hasOwn(res.headers, 'content-encoding'), false);
});

test('upstream passthrough forwards a headerless Claude OAuth SSE chunk before upstream completion', async () => {
  let releaseTail;
  const tailGate = new Promise((resolve) => {
    releaseTail = resolve;
  });
  let resolveFirstChunk;
  const firstChunkWritten = new Promise((resolve) => {
    resolveFirstChunk = resolve;
  });
  const res = createResCapture();
  const originalWrite = res.write;
  res.write = function writeAndSignal(chunk) {
  ...
  let seenHeaders = null;
  let seenTimeoutMs = 0;
  const { completion } = startUpstreamStream({
    res,
    body,
    requestHeaders: { 'accept-encoding': 'gzip, br' },
    onFetch: (init, timeoutMs) => {
      seenHeaders = init.headers;
      seenTimeoutMs = timeoutMs;
    }
  });
  ...
  assert.match(res.body.toString('utf8'), /message_start/u);
  assert.equal(res.statusCode, 200);
  assert.equal(res.headers['content-type'], 'text/event-stream; charset=utf-8');
  assert.equal(Object.hasOwn(res.headers, 'content-encoding'), false);
  assert.equal(seenHeaders['accept-encoding'], 'identity');
  assert.equal(seenTimeoutMs, 5 * 60 * 1000);

  releaseTail();
  await completion;
  assert.match(res.body.toString('utf8'), /message_stop/u);
});

test('upstream passthrough incrementally decodes a headerless gzip Claude OAuth stream', async () => {
  const expected = [
    'event: message_start',
    'data: {"type":"message_start"}',
    '',
  ...
  const compressed = zlib.gzipSync(Buffer.from(expected));
  const body = {
    async *[Symbol.asyncIterator]() {
      yield compressed.subarray(0, Math.ceil(compressed.length / 2));
      yield compressed.subarray(Math.ceil(compressed.length / 2));
    }
  };

  const { completion, res } = startUpstreamStream({ body });
  await completion;
...
test('upstream passthrough keeps an already decoded SSE body when gzip header remains', async () => {
  const expected = 'event: message_stop\ndata: {"type":"message_stop"}\n\n';
  const body = {
    async *[Symbol.asyncIterator]() {
  ...
  const { completion, res } = startUpstreamStream({ body, upstreamHeaders });
  await completion;
...
test('upstream passthrough removes stale content encoding from generic SSE responses', async () => {
  const expected = 'data: {"choices":[]}\n\n';
  const body = {
    async *[Symbol.asyncIterator]() {
      yield Buffer.from(expected);
    }
  ...
  assert.equal(Object.hasOwn(res.headers, 'content-encoding'), false);
  assert.equal(Object.hasOwn(res.headers, 'connection'), false);
  assert.equal(Object.hasOwn(res.headers, 'keep-alive'), false);
  assert.equal(Object.hasOwn(res.headers, 'trailer'), false);
  assert.equal(Object.hasOwn(res.headers, 'transfer-encoding'), false);
});

test('upstream passthrough records final usage from a bounded SSE tail', async () => {
  const records = [];
  const body = {
    async *[Symbol.asyncIterator]() {
      yield Buffer.alloc(128 * 1024, 0x20);
  ...
  assert.equal(records.length, 1);
  assert.deepEqual(records[0].usage, { output_tokens: 9 });
  assert.equal(records[0].model, 'claude-sonnet-4');
});

test('upstream passthrough cancels a Claude OAuth SSE reader when the downstream closes', async () => {
  const res = Object.assign(new EventEmitter(), createResCapture());
  let pendingRead;
  let cancelled = false;
  let resolveFirstChunk;
  const firstChunkWritten = new Promise((resolve) => {
  ...
  const body = {
    getReader() {
      ...
      return {
        read() {
          ...
          if (reads === 1) {
            return Promise.resolve({
              ...
              value: Buffer.from('event: message_start\ndata: {"type":"message_start"}\n\n')
            });
          }
        ...
        cancel() {
          cancelled = true;
          if (pendingRead) pendingRead({ done: true, value: undefined });
          return Promise.resolve();
        },
        releaseLock() {}
      };
  ...
  const { completion, state } = startUpstreamStream({
    res,
    body,
    logRequests: true,
  ...
  assert.equal(requestLogs.at(-1).status, 499);
  assert.equal(requestLogs.at(-1).streamTransport, 'downstream_cancelled');
});
...
test('upstream passthrough resolves qwen model to claude by model availability in auto mode', async () => {
  ...
  const state = {
    accounts: {
      codex: [{ accountRef: accountRef('6'), email: 'codex@example.com', accessToken: 'codex-token' }],
      claude: [{
        id: '3',
        email: 'claude@example.com',
        accessToken: 'anthropic-key',
...
test('upstream passthrough does not reject OpenCode provider catalog without account mapping', async () => {
  ...
  const state = {
    accounts: {
      opencode: [{ accountRef: accountRef('1'), provider: 'opencode', accessToken: 'opencode-local' }]
    },
    cursors: { opencode: 0 },
    webUiModelsCache: {
      updatedAt: Date.now(),
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      ...
      fetchOpenCodeChatCompletion: async (_options, account, requestJson) => {
        called = true;
        assert.equal(account.accountRef, accountRef('1'));
...
test('upstream passthrough uses OpenCode Go transport from route instead of provider name', async () => {
  ...
  const state = {
    accounts: {
      claude: [{ accountRef: accountRef('3'), provider: 'claude', accessToken: 'transport-token' }]
    },
    cursors: { claude: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0, providerCounts: {}, providerSuccess: {}, providerFailures: {} }
  };
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      ...
      fetchOpenCodeChatCompletion: async (_options, account, requestJson) => {
        called = true;
        assert.equal(account.accountRef, accountRef('3'));
        assert.equal(requestJson.model, model);
        return {
...
test('upstream passthrough treats OpenCode AbortError as transient without first-failure cooldown', async () => {
  const res = createResCapture();
  const model = 'opencode-go/glm-5.2';
  const account = { accountRef: accountRef('1'), provider: 'opencode', accessToken: 'opencode-local', cooldownUntil: 0 };
  const route = createProviderProtocolRouteMeta(resolveDirectProviderProtocolRoute('openai_chat', 'opencode'));
  const state = {
...
test('upstream passthrough returns clear invalid_request when claude account uses anthropic-compatible endpoint with chat completions path', async () => {
  ...
  const state = {
    accounts: {
      claude: [{ accountRef: accountRef('3'), email: 'claude-3@example.com', accessToken: 'claude-token-3', baseUrl: 'https://dashscope.aliyuncs.com/apps/anthropic' }]
    },
    cursors: { claude: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0, providerCounts: {}, providerSuccess: {}, providerFailures: {} }
  };
...
test('upstream passthrough 404 does not mark account success and returns raw upstream body', async () => {
  const res = createResCapture();
  const state = {
    accounts: { claude: [{ accountRef: accountRef('1'), email: 'a@example.com', accessToken: 'tok', cooldownUntil: 0 }] },
    cursors: { claude: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0, providerFailures: {}, providerSuccess: {} }
  };
  let successCalls = 0;
...
test('upstream passthrough 429 applies model cooldown and retries another account before any downstream bytes are sent', async () => {
  const res = createResCapture();
  const accounts = [
    { accountRef: accountRef('1'), email: 'a@example.com', accessToken: 'tok-1', cooldownUntil: 0 },
    { accountRef: accountRef('2'), email: 'b@example.com', accessToken: 'tok-2', cooldownUntil: 0 }
  ];
  const state = {
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      chooseServerAccount: (pool, _state, _cursorKey, options = {}) => {
        const excluded = options.excludeAccountRefs || new Set();
        const next = pool.find((account) => !excluded.has(String(account.accountRef)));
        if (next) seen.push(String(next.accountRef));
        return next || null;
      },
      pushMetricError: () => {},
      writeJson: (r, code, payload) => {
  ...
  assert.deepEqual(seen, [accountRef('1'), accountRef('2')]);
  assert.equal(res.statusCode, 200);
  assert.equal(String(res.body), '{"ok":true}');
...
test('upstream passthrough can defer exhausted 429 to alias runtime fallback before writing response', async () => {
  ...
  assert.deepEqual(result.attemptedAccountRefs, [accountRef('1')]);
  assert.equal(res.statusCode, 0);
  assert.equal(String(res.body), '');
  assert.equal(state.metrics.totalFailures, 0);
});

test('upstream passthrough defers exhausted AGY 401 to the next alias candidate before writing response', async () => {
  const res = createResCapture();
  const account = {
    accountRef: accountRef('agy-auth-invalid'),
    provider: 'agy',
    email: 'agy@example.com',
  ...
  const result = await handleUpstreamPassthrough({
    ...
    res,
    method: 'POST',
    bodyBuffer: Buffer.from('{"model":"claude-opus-4-6-thinking","messages":[]}'),
    ...
    requestMeta: {
      ...
      aliasResolution: {
        ...
        requestedModel: 'claude-opus-4-8',
        aliasTarget: 'claude-opus-4-6-thinking'
      }
    },
    routeKey: 'POST /v1/chat/completions',
    requestStartedAt: Date.now(),
  ...
  assert.equal(result && result.error, 'auth_invalid_reauth_required');
  assert.equal(result && result.model, 'claude-opus-4-6-thinking');
  assert.deepEqual(result && result.attemptedAccountRefs, [account.accountRef]);
  assert.doesNotMatch(String(result && result.detail || ''), new RegExp(account.accountRef));
  assert.equal(res.statusCode, 0);
  assert.equal(String(res.body), '');
  assert.equal(state.metrics.totalFailures, 0);
});
...
test('upstream passthrough skips invalid token and retries with another account in same request', async () => {
  const res = createResCapture();
  const pool = [
    { accountRef: accountRef('1'), email: 'bad@example.com', accessToken: 'bad\ntoken' },
    { accountRef: accountRef('2'), email: 'ok@example.com', accessToken: 'good-token' }
  ];
  const state = {
    accounts: { codex: pool },
    cursors: { codex: 0 },
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      chooseServerAccount: (accounts, _state, _cursorKey, options = {}) => {
        const excluded = options.excludeAccountRefs || new Set();
        const next = accounts.find((acc) => !excluded.has(String(acc.accountRef)));
        if (next) seenAccountRefs.push(String(next.accountRef));
        return next || null;
      },
      pushMetricError: () => {},
      writeJson: (r, code, payload) => {
  ...
  assert.deepEqual(seenAccountRefs, [accountRef('1'), accountRef('2')]);
  assert.equal(res.statusCode, 200);
...
test('upstream passthrough refreshes codex token on 401 and retries same account', async () => {
  const res = createResCapture();
  const state = {
    accounts: { codex: [{ accountRef: accountRef('1'), email: 'a@example.com', accessToken: 'expired-token', refreshToken: 'rt_1' }] },
    cursors: { codex: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0 }
  };
  let upstreamCalls = 0;
...
test('upstream passthrough refreshes Claude OAuth token on 401 and retries same account', async () => {
  const res = createResCapture();
  const account = {
    accountRef: accountRef('claude-refresh'),
    email: 'claude@example.com',
    provider: 'claude',
    authType: 'oauth',
    apiKeyMode: false,
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      ...
      refreshClaudeAccessToken: async (selectedAccount, refreshOptions) => {
        if (!refreshOptions || !refreshOptions.force) {
          return { ok: true, refreshed: false, reason: 'not_due' };
        }
        forcedRefreshCalls += 1;
        selectedAccount.accessToken = 'fresh-oauth-token';
        return { ok: true, refreshed: true };
      },
      fetchWithTimeout: async (_url, init) => {
        const headers = init.headers || {};
        upstreamHeaders.push({ ...headers });
        if (String(headers.authorization || '').includes('expired-oauth-token')) {
          return {
            status: 401,
            headers: new Map([['content-type', 'application/json']]),
            arrayBuffer: async () => Buffer.from('{"error":"expired"}')
  ...
  assert.equal(upstreamHeaders[0].authorization, 'Bearer expired-oauth-token');
  assert.equal(upstreamHeaders[1].authorization, 'Bearer fresh-oauth-token');
  assert.equal(upstreamHeaders[1]['anthropic-beta'], 'oauth-2025-04-20');
  assert.equal(Object.hasOwn(upstreamHeaders[1], 'x-api-key'), false);
  assert.equal(res.statusCode, 200);
  assert.equal(String(res.body), '{"ok":true}');
  assert.equal(state.metrics.totalSuccess, 1);
});

test('Claude OAuth 401 keeps a transient token refresh failure recoverable', async () => {
  const result = await runClaudeRefreshFailureScenario({
    ok: false,
    refreshed: false,
    status: 500,
    reason: 'refresh_http_500',
    detail: 'token endpoint unavailable'
  });

  assert.equal(result.statusCode, 503);
  assert.equal(result.payload.error, 'upstream_failed');
  assert.match(result.payload.detail, /token_refresh_refresh_http_500/u);
  assert.notEqual(result.account.lastFailureKind, 'auth_invalid');
  assert.notEqual(result.account.lastFailureReason, 'auth_invalid_reauth_required');
});

test('Claude OAuth 401 marks an invalid refresh grant as requiring login', async () => {
  const result = await runClaudeRefreshFailureScenario({
    ok: false,
    refreshed: false,
    status: 400,
  ...
  assert.equal(result.payload.error, 'auth_invalid_reauth_required');
  assert.equal(result.account.lastFailureKind, 'auth_invalid');
  assert.equal(result.account.lastFailureReason, 'auth_invalid_reauth_required');
});

test('Claude API-key 401 remains an auth failure without invoking OAuth refresh', async () => {
  const result = await runClaudeRefreshFailureScenario({
    ok: false,
    refreshed: false,
    reason: 'not_oauth'
...
test('upstream passthrough uses Gemini Code Assist adapter for oauth-personal chat completions', async () => {
  ...
  const state = {
    accounts: {
      gemini: [{ accountRef: accountRef('g1'), email: 'g@example.com', accessToken: 'tok', authType: 'oauth-personal' }]
    },
    cursors: { gemini: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0 }
  };
...
test('upstream passthrough uses Code Assist adapter for agy provider', async () => {
  ...
  const state = {
    accounts: {
      agy: [{ accountRef: accountRef('a1'), provider: 'agy', email: 'agy@example.com', accessToken: 'tok', authType: 'oauth-personal' }]
    },
    cursors: { agy: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0 }
  };
...
test('upstream passthrough refreshes an expiring AGY token before Code Assist dispatch', async () => {
  const res = createResCapture();
  const account = {
    accountRef: accountRef('agy-refresh'),
    provider: 'agy',
    email: 'agy@example.com',
  ...
  await handleUpstreamPassthrough({
    ...
    requestStartedAt: Date.now(),
    cooldownMs: 1000,
    requestMeta: { sessionKey: 'agy-refresh-thread' },
    deps: {
      chooseServerAccount: (pool) => pool[0],
      pushMetricError: () => {},
      writeJson: (response, code, payload) => {
      ...
      refreshAgyAccessToken: async (selectedAccount, refreshOptions) => {
        refreshCalls += 1;
        assert.equal(refreshOptions.force, false);
        selectedAccount.accessToken = 'fresh-token';
        return { ok: true, refreshed: true };
      },
      fetchGeminiCodeAssistChatCompletion: async (_callOptions, selectedAccount) => {
        dispatchedToken = selectedAccount.accessToken;
        return { id: 'chatcmpl-agy-refreshed', object: 'chat.completion', choices: [] };
      },
      fetchWithTimeout: async () => {
        throw new Error('should_not_call_raw_passthrough');
      },
  ...
  assert.equal(dispatchedToken, 'fresh-token');
  assert.equal(res.statusCode, 200);
});

test('upstream passthrough force-refreshes AGY after a Code Assist 401 and retries the same account', async () => {
  const result = await runAgyCodeAssistAuthScenario({ succeedAfterRefresh: true });

  assert.equal(result.forcedRefreshCalls, 1);
  assert.equal(result.adapterCalls, 2);
  assert.equal(result.account.accessToken, 'fresh-token');
  assert.equal(result.statusCode, 200);
});

test('direct AGY Anthropic 401 force-refreshes and retries the same account', async () => {
  const result = await runAgyCodeAssistAuthScenario({
    transport: 'anthropic',
...
test('AGY Gemini generateContent 401 force-refreshes and retries the same account', async () => {
  const result = await runAgyCodeAssistAuthScenario({
    transport: 'gemini',
    succeedAfterRefresh: true
  });
...
test('AGY Code Assist 401 keeps a transient token refresh failure recoverable', async () => {
  const result = await runAgyCodeAssistAuthScenario({
    refreshResult: {
      ok: false,
      refreshed: false,
...
test('AGY Code Assist 401 marks an invalid refresh grant as requiring login', async () => {
  const result = await runAgyCodeAssistAuthScenario({
    refreshResult: {
      ok: false,
      refreshed: false,
...
test('AGY Code Assist retries only once when the refreshed token also returns 401', async () => {
  const result = await runAgyCodeAssistAuthScenario();
...
test('upstream passthrough uses native Gemini generateContent for Code Assist providers', async () => {
  ...
  const state = {
    accounts: {
      gemini: [{ accountRef: accountRef('g1'), provider: 'gemini', email: 'g@example.com', accessToken: 'tok', authType: 'oauth-personal' }]
    },
    cursors: { gemini: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0 }
  };
...
test('Gemini Code Assist generateContent stream retries another account when upstream returns no assistant content', async () => {
  const res = createResCapture();
  const accounts = [
    { accountRef: accountRef('a1'), provider: 'agy', email: 'empty@example.com', accessToken: 'tok-empty', authType: 'oauth-personal', cooldownUntil: 0 },
    { accountRef: accountRef('a2'), provider: 'agy', email: 'ok@example.com', accessToken: 'tok-ok', authType: 'oauth-personal', cooldownUntil: 0 }
  ];
  const state = {
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      ...
      fetchGeminiCodeAssistGenerateContentStream: async function* (_requestOptions, account) {
        ...
        if (account.accountRef === accountRef('a1')) {
          yield {
            model: 'gemini-3-flash-agent',
      ...
      markProxyAccountSuccess: (account) => successAccountRefs.push(account.accountRef),
      appendProxyRequestLog: () => {}
    }
  });

  const body = String(res.body);
  assert.deepEqual(attemptedAccountRefs, [accountRef('a1'), accountRef('a2')]);
  assert.equal(failureCalls.length, 0);
  assert.deepEqual(successAccountRefs, [accountRef('a2')]);
  assert.equal(res.statusCode, 200);
  assert.equal(res.headers['x-aih-server-account-ref'], accountRef('a2'));
...
test('direct AGY Anthropic adapter retries another account for model-scoped quota', async () => {
  const res = createResCapture();
  const accounts = [
    { accountRef: accountRef('a1'), provider: 'agy', email: 'agy1@example.com', accessToken: 'tok-1', authType: 'oauth-personal' },
    { accountRef: accountRef('a2'), provider: 'agy', email: 'agy2@example.com', accessToken: 'tok-2', authType: 'oauth-personal' }
  ];
  const state = {
    accounts: { agy: accounts },
    cursors: { agy: 0 },
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      chooseServerAccount: (pool, _cursorState, _cursorKey, selectionOptions = {}) => (
        pool.find((account) => !selectionOptions.excludeAccountRefs.has(String(account.accountRef)))
        || null
      ),
      pushMetricError: () => {},
      writeJson: (r, code, body) => {
...
test('direct AGY Anthropic adapter retries another account for model capacity without blocking account', async () => {
  const res = createResCapture();
  const accounts = [
    { accountRef: accountRef('a1'), provider: 'agy', email: 'agy1@example.com', accessToken: 'tok-1', authType: 'oauth-personal', cooldownUntil: 0 },
    { accountRef: accountRef('a2'), provider: 'agy', email: 'agy2@example.com', accessToken: 'tok-2', authType: 'oauth-personal', cooldownUntil: 0 }
  ];
  const state = {
    accounts: { agy: accounts },
    cursors: { agy: 0 },
...
test('direct AGY Anthropic adapter retries another account for unsupported location', async () => {
  const res = createResCapture();
  const accounts = [
    { accountRef: accountRef('a1'), provider: 'agy', email: 'blocked@example.com', accessToken: 'tok-1', authType: 'oauth-personal', cooldownUntil: 0 },
    { accountRef: accountRef('a2'), provider: 'agy', email: 'ok@example.com', accessToken: 'tok-2', authType: 'oauth-personal', cooldownUntil: 0 }
  ];
  const state = {
    accounts: { agy: accounts },
    cursors: { agy: 0 },
...
test('direct AGY Anthropic adapter covers full Code Assist pool when capacity retry exceeds configured attempts', async () => {
  ...
  const accounts = [
    { accountRef: accountRef('a1'), provider: 'agy', email: 'agy1@example.com', accessToken: 'tok-1', authType: 'oauth-personal', cooldownUntil: 0 },
    { accountRef: accountRef('a2'), provider: 'agy', email: 'agy2@example.com', accessToken: 'tok-2', authType: 'oauth-personal', cooldownUntil: 0 },
    { accountRef: accountRef('a3'), provider: 'agy', email: 'agy3@example.com', accessToken: 'tok-3', authType: 'oauth-personal', cooldownUntil: 0 },
    { accountRef: accountRef('a4'), provider: 'agy', email: 'agy4@example.com', accessToken: 'tok-4', authType: 'oauth-personal', cooldownUntil: 0 }
  ];
  const state = {
  ...
  assert.deepEqual(attemptedAccountRefs, [accountRef('a1'), accountRef('a2'), accountRef('a3'), accountRef('a4')]);
  ...
  await handleUpstreamPassthrough({
    ...
    deps: {
      ...
      fetchCodeAssistAnthropicMessageStream: async function* (_requestOptions, account) {
        attemptedAccountRefs.push(account.accountRef);
        yield { type: 'message_start', id: `msg_${account.accountRef}`, model: 'claude-4-6-thinking', created: 1770000000 };
        if (account.accountRef === accountRef('a1')) {
...
test('AGY Code Assist stream keeps tool_calls finish when STOP arrives after tool chunk', async () => {
  ...
  const state = {
    accounts: {
      agy: [{ accountRef: accountRef('agy1'), email: 'agy@example.com', accessToken: 'tok', authType: 'oauth-personal' }]
    },
    cursors: { agy: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0 }
  };
...
test('Gemini Code Assist 404 does not trigger cooldown and returns 404 upstream_failed', async () => {
  ...
  const state = {
    accounts: {
      gemini: [{ accountRef: accountRef('g1'), email: 'g@example.com', accessToken: 'tok', authType: 'oauth-personal', cooldownUntil: 0 }]
    },
    cursors: { gemini: 0 },
    metrics: { totalFailures: 0, totalSuccess: 0, totalTimeouts: 0 }
  };
...
test('Gemini Code Assist 429 triggers immediate cooldown and returns 429 upstream_failed', async () => {
  const res = createResCapture();
  const account = { accountRef: accountRef('g1'), email: 'g@example.com', accessToken: 'tok', authType: 'oauth-personal', cooldownUntil: 0 };
  const state = {

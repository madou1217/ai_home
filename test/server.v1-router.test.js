...
test('v1 router rejects aliased AGY Claude requests when refreshed catalog omits target model', async () => {
  ...
  const handled = await handleV1Request({
    ...
    state: {
      ...
      cursors: { agy: 0 },
      accounts: {
        codex: [{ id: 'c1', accountRef: V1_CODEX_REF_1, provider: 'codex', accessToken: 'codex-token' }],
        gemini: [],
        claude: [],
        agy: [{ id: 'a1', accountRef: V1_AGY_REF_1, provider: 'agy', accessToken: 'agy-token', authType: 'oauth-personal', availableModels: ['gemini-3.1-pro-preview'] }]
      }
    },
    requiredClientKey: '',
    cooldownMs: 1000,
...
test('v1 router falls back to lower-priority alias when higher-priority target is unavailable', async () => {
  const res = createResCapture();
  let seenRequest = null;
  ...
  assert.equal(handled, true);
  // 高优先级 target 不在目录,应自动回退到低优先级 target 并成功路由
  assert.equal(res.statusCode, 200);
  assert.ok(seenRequest);
  assert.equal(seenRequest.model, 'gemini-3.1-pro-preview');
});

test('v1 router retries lower-priority alias when higher-priority AGY target is exhausted at runtime', async () => {
  const res = createResCapture();
  const seenModels = [];
  const seenUrls = [];
  ...
  const handled = await handleV1Request({
    ...
    deps: {
      ...
      handleCodexModels: async () => {},
      handleCodexChatCompletions: async () => {
        throw new Error('runtime fallback alias should not use codex');
      },
      handleUpstreamModels: async () => {},
      handleUpstreamPassthrough: async (ctx) => {
        ...
        if (ctx.requestJson.model === 'claude-sonnet-4-6') {
          return {
            retryAliasCandidate: true,
            statusCode: 429,
            error: 'upstream_failed',
            detail: 'quota exhausted for claude-sonnet-4-6',
            provider: 'agy',
            model: ctx.requestJson.model,
            attemptedAccountIds: ['a1'],
            alias: {
        ...
        ctx.res.statusCode = 200;
        ctx.res.end(JSON.stringify(createGeminiGenerateContentResponse('gemini-3-flash-agent', 'ok')));
        return null;
      },
      chooseServerAccount: () => null,
      markProxyAccountSuccess: () => {},
  ...
  assert.equal(handled, true);
  assert.deepEqual(seenModels, ['claude-sonnet-4-6', 'gemini-3-flash-agent']);

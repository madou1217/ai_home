...
test('claude api credential legacy config migrates back to shared session store', (t) => {
  const root = mkTmpDir();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  ...
  const profilesDir = path.join(root, 'profiles');
  const hostClaudeDir = path.join(hostHomeDir, '.claude');
  const guestClaudeDir = path.join(profilesDir, 'claude', '7', '.claude');
  const legacyApiCredentialDir = path.join(profilesDir, 'claude', '7', '.claude-api-credential');
  const projectName = '-Users-model-projects-feature-ai-home';
  const projectPath = '/Users/model/projects/feature/ai_home';

  fs.mkdirSync(path.join(hostClaudeDir, 'projects', projectName), { recursive: true });
  fs.mkdirSync(path.join(legacyApiCredentialDir, 'projects', projectName), { recursive: true });

  fs.writeFileSync(
    path.join(hostClaudeDir, 'projects', projectName, 'host-session.jsonl'),
    '{"sessionId":"host-session"}\n',
    'utf8'
  );
  fs.writeFileSync(
    path.join(legacyApiCredentialDir, 'projects', projectName, 'legacy-session.jsonl'),
    '{"sessionId":"legacy-session"}\n',
    'utf8'
  );
  fs.writeFileSync(
    path.join(hostClaudeDir, 'history.jsonl'),
    '{"display":"host","sessionId":"host-session"}\n',
    'utf8'
  );
  fs.writeFileSync(
    path.join(legacyApiCredentialDir, 'history.jsonl'),
    '{"display":"legacy","sessionId":"legacy-session"}\n',
    'utf8'
  );
  fs.writeFileSync(path.join(hostClaudeDir, 'settings.json'), '{"theme":"dark"}\n', 'utf8');
  fs.writeFileSync(path.join(legacyApiCredentialDir, 'settings.json'), '{"theme":"auto"}\n', 'utf8');
  fs.writeFileSync(
    path.join(hostClaudeDir, '.claude.json'),
    JSON.stringify({
      customApiKeyResponses: { approved: ['host-key'], rejected: [] },
      projects: {
        [projectPath]: { hasCompletedProjectOnboarding: true }
      }
    }),
    'utf8'
  );
  fs.writeFileSync(
    path.join(legacyApiCredentialDir, '.claude.json'),
    JSON.stringify({
      customApiKeyResponses: { approved: ['legacy-key'], rejected: ['bad-key'] },
      projects: {
  ...
  const result = service.ensureSessionStoreLinks('claude', '7');

  assert.equal(result.migrated > 0, true);
  assert.equal(result.linked > 0, true);
  assert.ok(fs.lstatSync(path.join(guestClaudeDir, 'projects')).isSymbolicLink());
  assert.ok(fs.lstatSync(path.join(legacyApiCredentialDir, 'projects')).isSymbolicLink());
  assert.equal(
    fs.existsSync(path.join(hostClaudeDir, 'projects', projectName, 'legacy-session.jsonl')),
    true
  );
  assert.match(
    fs.readFileSync(path.join(hostClaudeDir, 'history.jsonl'), 'utf8'),
    /legacy-session/
  );

  const mergedClaudeJson = JSON.parse(fs.readFileSync(path.join(hostClaudeDir, '.claude.json'), 'utf8'));
  assert.deepEqual(mergedClaudeJson.customApiKeyResponses.approved, ['host-key', 'legacy-key']);
  assert.deepEqual(mergedClaudeJson.customApiKeyResponses.rejected, ['bad-key']);
  assert.deepEqual(mergedClaudeJson.projects[projectPath], {

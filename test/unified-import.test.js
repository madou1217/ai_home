...
function getSingleCredentialRecord(aiHomeDir, provider) {
  ...
  assert.equal(records.length, 1, `expected one ${provider} credential record`);
  return records[0];
}
...
test('runUnifiedImport imports standard JSON files such as sub2api-data', async () => {
  ...
  try {
    ...
    const codexRecord = getSingleCredentialRecord(aiHomeDir, 'codex');
    const agyRecord = getSingleCredentialRecord(aiHomeDir, 'agy');
    const codexEnv = codexRecord.env;
    const agyAuth = agyRecord.nativeAuth.oauthToken;
    const agyEmail = agyRecord.nativeAuth.email;
    assert.deepEqual(codexEnv, {
      OPENAI_API_KEY: 'sk-json',
      OPENAI_BASE_URL: 'https://json.example.com/v1'
    });
    assert.deepEqual(codexRecord.nativeAuth, {});
    ...
    const codexMetadata = readTransferMetadata(fs, aiHomeDir, codexRecord.accountRef).formats.sub2api;
...
test('runUnifiedImport imports current sub2api JSON without type and version header', async () => {
  ...
  try {
    ...
    assert.deepEqual(result.providers, ['codex']);
    const codexEnv = getSingleCredentialRecord(aiHomeDir, 'codex').env;
    assert.deepEqual(codexEnv, {
...
test('runUnifiedImport imports sub2api account TXT files', async () => {
  ...
  try {
    ...
    const codexAuth = getSingleCredentialRecord(aiHomeDir, 'codex').nativeAuth.auth;
    const claudeEnv = getSingleCredentialRecord(aiHomeDir, 'claude').env;
...
test('runUnifiedImport infers cpa zip folders with flat codex token JSON files', async () => {
  ...
  try {
    ...
    const importedAuth = getSingleCredentialRecord(aiHomeDir, 'codex').nativeAuth.auth;
    assert.equal(result.failedSources.length, 0);
    assert.equal(result.sourceResults.length, 1);
    assert.equal(result.sourceResults[0].imported, 1);
    assert.deepEqual(result.providers, ['codex']);
...
test('runUnifiedImport imports mixed flat sub2api zip roots through standard JSON importer', async () => {
  ...
  try {
    ...
    assert.equal(result.sourceResults[0].imported, 2);
    assert.deepEqual(result.providers, ['claude', 'codex']);
    assert.equal(getSingleCredentialRecord(aiHomeDir, 'codex').nativeAuth.auth.email, 'mixed-codex@example.com');
    assert.deepEqual(getSingleCredentialRecord(aiHomeDir, 'claude').env, {
...
test('runUnifiedImport imports mixed flat single-account JSON zip roots through standard JSON importer', async () => {
  ...
  try {
    ...
    assert.equal(getSingleCredentialRecord(aiHomeDir, 'codex').nativeAuth.auth.email, 'worker@example.com');
    assert.equal(getSingleCredentialRecord(aiHomeDir, 'claude').env.ANTHROPIC_API_KEY, 'sk-claude-flat');
    assert.equal(
      getSingleCredentialRecord(aiHomeDir, 'gemini').nativeAuth.oauthCreds.access_token,
      'gemini-access-flat'
    );
    assert.equal(
      getSingleCredentialRecord(aiHomeDir, 'agy').nativeAuth.oauthToken.token.access_token,
      'agy-access-flat'
    );
    assert.equal(fs.existsSync(path.join(aiHomeDir, 'profiles')), false);
  } finally {

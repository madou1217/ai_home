...
test('refreshIndexedStateForAccount clears stale agy auth block when OAuth creds are recoverable', () => {
  ...
  try {
    ...
    const service = createUsageAccountRuntimeService({
      ...
      accountStateService: {
        ...
        clearRuntimeBlock(provider, accountId, payload) {
          clears.push({ provider, accountId, payload });
          return true;
        },
        pruneMissing() {}
      },
    ...
    const result = service.refreshIndexedStateForAccount('agy', '1', { refreshSnapshot: false });

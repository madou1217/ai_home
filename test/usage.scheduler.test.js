...
test('threshold switch selects the best eligible account when threshold is crossed', () => {
  const result = thresholdSwitch.evaluateThresholdSwitch({
    ...
    thresholdPct: 90,
    accounts: [
      { accountId: 'acct-a', usagePct: 95, available: true, quotaBlocked: false },
      { accountId: 'acct-b', usagePct: 50, available: true, quotaBlocked: false },
      { accountId: 'acct-c', usagePct: 30, available: true, quotaBlocked: false }
    ]
  });
...
test('threshold switch remains on current account when usage is below threshold', () => {
  const result = thresholdSwitch.evaluateThresholdSwitch({
    ...
    thresholdPct: 90,
    accounts: [
      { accountId: 'acct-a', usagePct: 40, available: true, quotaBlocked: false },
      { accountId: 'acct-b', usagePct: 20, available: true, quotaBlocked: false }
    ]
  });

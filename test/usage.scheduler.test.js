...
test('threshold switch selects the best eligible account when threshold is crossed', () => {
  const result = thresholdSwitch.evaluateThresholdSwitch({
    currentAccountRef: 'acct-a',
    currentUsagePct: 95,
    thresholdPct: 90,
    accounts: [
      { accountRef: 'acct-a', usagePct: 95, available: true, quotaBlocked: false },
      { accountRef: 'acct-b', usagePct: 50, available: true, quotaBlocked: false },
      { accountRef: 'acct-c', usagePct: 30, available: true, quotaBlocked: false }
    ]
  });

  assert.equal(result.shouldSwitch, true);
  assert.equal(result.reason, 'threshold_crossed');
  assert.equal(result.toAccountRef, 'acct-c');
});
...
test('threshold switch remains on current account when usage is below threshold', () => {
  const result = thresholdSwitch.evaluateThresholdSwitch({
    ...
    thresholdPct: 90,
    accounts: [
      { accountRef: 'acct-a', usagePct: 40, available: true, quotaBlocked: false },
      { accountRef: 'acct-b', usagePct: 20, available: true, quotaBlocked: false }
    ]
  });

  assert.equal(result.shouldSwitch, false);
  assert.equal(result.reason, 'below_threshold');
  assert.equal(result.toAccountRef, '');
});
...
test('permission policy module exports stable API contract', () => {
  ...
  assert.equal(typeof permissionPolicy.resolvePolicyAiHomeDir, 'function');
...
test('permission policy persists and reloads full-access preference deterministically', () => {
  ...
  const loaded = permissionPolicy.loadPermissionPolicy({ fs, aiHomeDir: tempDir });

...
function createDeps(aiHomeDir) {
  return {
    ...
    getToolAccountIds(provider) {
      return listProviderAccountRefs(aiHomeDir, provider);
    },
    getToolConfigDir(provider, accountRef) {
      return path.join(aiHomeDir, 'run', 'auth-projections', provider, String(accountRef));
    },
    getProfileDir(provider, accountRef) {
      return path.join(aiHomeDir, 'run', 'auth-projections', provider, String(accountRef));
    },
...
function listProviderAccountRefs(aiHomeDir, provider) {
  return listCliAccountRefRecords(fs, aiHomeDir, provider).map((account) => account.accountRef);
}

function getOnlyAccountRef(aiHomeDir, provider) {
  const accountRefs = listProviderAccountRefs(aiHomeDir, provider);
  assert.equal(accountRefs.length, 1);
  return accountRefs[0];
}

function registerStoredAccount(aiHomeDir, input) {
  const registered = registerAccountIdentity(fs, aiHomeDir, {
    provider: input.provider,
    identitySeed: input.identitySeed,
    cliAccountId: input.cliAccountId
  });
  if (input.env) writeAccountCredentials(fs, aiHomeDir, registered.accountRef, input.env);
  if (input.nativeAuth) writeAccountNativeAuth(fs, aiHomeDir, registered.accountRef, input.nativeAuth);
  return registered.accountRef;
}

function readCodexAuth(aiHomeDir, accountRef) {
  const nativeAuth = readAccountNativeAuth(fs, aiHomeDir, accountRef);
  return nativeAuth.auth || {};
}
...
function makeSub2ApiCodexOauthBundle({ email, refreshToken, upstreamAccountId }) {
  return {
    type: 'sub2api-data',
    version: 1,
...
test('web ui account import accepts its own exported bundle content', async () => {
  ...
  try {
    ...
    const accountRef = getOnlyAccountRef(targetAiHomeDir, 'codex');
    const importedAuth = readCodexAuth(targetAiHomeDir, accountRef);
...
test('web ui account import accepts exported api key account bundle', async () => {
  ...
  try {
    ...
    registerStoredAccount(sourceAiHomeDir, {
      provider: 'codex',
      identitySeed: 'api-key:codex:https://api.openai.com/v1:source-key',
      env: {
    ...
    assert.equal(importBody.summary.created, 1);
    const accountRef = getOnlyAccountRef(targetAiHomeDir, 'codex');
    const importedEnv = readAccountCredentials(fs, targetAiHomeDir, accountRef);
    assert.equal(importedEnv.OPENAI_API_KEY, 'sk-test-api-key');
...
test('web ui account import ignores legacy AIH local accountId fields', async () => {
  ...
  try {
    ...
    const importBody = await readCompletedImportBody(importRes, deps);
    assert.equal(importBody.imported, 1);
    const [account] = listCliAccountRefRecords(fs, aiHomeDir, 'codex');
    assert.equal(account.cliAccountId, '1');
    const importedAuth = readCodexAuth(aiHomeDir, account.accountRef);
...
test('web ui account import stores sub2api metadata from manual JSON content', async () => {
  ...
  try {
    ...
    const importBody = await readCompletedImportBody(importRes, deps);
    assert.equal(importBody.imported, 1);
    const accountRef = getOnlyAccountRef(aiHomeDir, 'codex');
...
test('web ui account export skips credentialless DB accounts', async () => {
  const aiHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-web-account-transfer-skip-empty-'));
  try {
    const deps = createDeps(aiHomeDir);
    registerStoredAccount(aiHomeDir, {
      provider: 'claude',
      identitySeed: 'api-key:claude:credentialless',
      env: { ANTHROPIC_BASE_URL: 'https://api.anthropic.com' }
    });
    registerStoredAccount(aiHomeDir, {
      provider: 'claude',
      identitySeed: 'api-key:claude:transfer',
      env: {
...
test('web ui account export and import preserves agy oauth token account', async () => {
  ...
  try {
    ...
    const importBody = await readCompletedImportBody(importRes, targetDeps);
    assert.equal(importBody.imported, 1);
    const accountRef = getOnlyAccountRef(targetAiHomeDir, 'agy');
    const importedNativeAuth = readAccountNativeAuth(fs, targetAiHomeDir, accountRef);
    assert.equal(importedNativeAuth.oauthToken.token.access_token, 'agy-access-token');
    assert.equal(importedNativeAuth.oauthToken.token.refresh_token, 'agy-refresh-token');
    assert.equal(importedNativeAuth.email, 'agy@example.com');
  } finally {
    fs.rmSync(sourceAiHomeDir, { recursive: true, force: true });
    fs.rmSync(targetAiHomeDir, { recursive: true, force: true });
  }
...
test('web ui account export supports Antigravity Manager formats', async () => {
  ...
  try {
    ...
    registerStoredAccount(aiHomeDir, {
      provider: 'agy',
      identitySeed: 'oauth:agy:agy-export@example.com',
      nativeAuth: {
        oauthToken: {
          auth_method: 'oauth',
          token: { refresh_token: 'agy-refresh-export' }
        },
...
test('web ui account export returns CLIProxyAPI data without syncing host config', async () => {
  ...
  try {
    ...
    registerStoredAccount(aiHomeDir, {
      provider: 'codex',
      identitySeed: 'api-key:codex:https://cliproxy-web.example.com/v1:web-key',
      env: {
...
test('web ui account import does not use provider account_id as local accountRef', async () => {
...
test('web ui account import rejects same-provider oauth without email', async () => {
  ...
  try {
    ...
    assert.deepEqual(listProviderAccountRefs(aiHomeDir, 'codex'), []);
  } finally {
    fs.rmSync(aiHomeDir, { recursive: true, force: true });
  }
});
...
test('web ui account import keeps same email separate across oauth providers', async () => {
  ...
  try {
    ...
    const [codexAccountRef] = listProviderAccountRefs(aiHomeDir, 'codex');
    const [geminiAccountRef] = listProviderAccountRefs(aiHomeDir, 'gemini');
    assert.match(codexAccountRef, /^acct_[a-f0-9]{20}$/);
    assert.match(geminiAccountRef, /^acct_[a-f0-9]{20}$/);
    assert.notEqual(codexAccountRef, geminiAccountRef);
  } finally {
    fs.rmSync(aiHomeDir, { recursive: true, force: true });
  }
});
...
test('web ui account import dedupes api key accounts by provider url and key', async () => {
  ...
  try {
    ...
    assert.equal(importBody.summary.updated, 0);
    assert.equal(importBody.summary.skipped, 1);
    const accountRefs = listProviderAccountRefs(aiHomeDir, 'codex');
    assert.equal(accountRefs.length, 2);
    const baseUrls = accountRefs
      .map((accountRef) => readAccountCredentials(fs, aiHomeDir, accountRef).OPENAI_BASE_URL)
      .sort();
...
test('web ui account import path supports standard JSON files and auth hooks', async () => {
  ...
  try {
    ...
    const accountRef = getOnlyAccountRef(aiHomeDir, 'codex');
    const importedEnv = readAccountCredentials(fs, aiHomeDir, accountRef);
  ...
  try {
    ...
    const firstZipPayload = makeStoredZip([{
      ...
      content: JSON.stringify(makeSub2ApiCodexOauthBundle({
        ...
        upstreamAccountId: 'acc_multi_one'
      }))
    }]);
    const secondZipPayload = makeStoredZip([{
      ...
      content: JSON.stringify(makeSub2ApiCodexOauthBundle({
        ...
        upstreamAccountId: 'acc_multi_two'
      }))
    }]);
    const importRes = createResCapture();
    ...
    const accountRefs = listProviderAccountRefs(aiHomeDir, 'codex');
    assert.equal(accountRefs.length, 2);
    const importedAuths = accountRefs.map((accountRef) => readCodexAuth(aiHomeDir, accountRef));
...
test('web ui accounts list does not expose internal oauth identifiers', async () => {
  ...
  try {
    ...
    const accountRef = registerStoredAccount(aiHomeDir, {
      provider: 'codex',
    ...
    const state = {
      accounts: { codex: [accountState], agy: [], claude: [], gemini: [], opencode: [] }
    };
    const deps = {
      ...createDeps(aiHomeDir),
      accountStateIndex: {
        getAccountState: (candidateRef) => candidateRef === accountRef ? accountState : null,
        listStates: (provider) => provider === 'codex' ? [accountState] : []
      },

...
const { PROVIDER_ARTIFACTS } = require('../lib/account/native-auth-projection');
...
test('web ui file media preview serves authorized project images', async () => {
  ...
  try {
    ...
    addOpenedProject({ path: projectDir }, { fs, aiHomeDir });
    const res = createResCapture();
    const handled = await handleWebUIRequest({
      method: 'GET',
...
test('web ui file routes reject an unregistered projectPath authorization root', async (t) => {
  if (!fs.existsSync('/etc/hosts')) return;
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-untrusted-project-root-'));
  ...
  const handled = await handleWebUIRequest({
    ...
    url: new URL('http://localhost/v0/webui/fs/read?path=%2Fetc%2Fhosts&projectPath=%2Fetc'),
    req: { headers: {} },
    res,
    options: {},
    state: {},
    deps: { ...createBaseDeps(aiHomeDir), hostHomeDir }
  });

  assert.equal(handled, true);
  assert.equal(res.statusCode, 403);
  assert.equal(JSON.parse(res.body).error, 'unknown_project_root');
});

test('web ui file routes do not authorize broad AIH or temporary roots', async (t) => {
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-narrow-roots-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const aiHomeFile = path.join(aiHomeDir, 'runtime-note.txt');
  const tempFile = path.join(os.tmpdir(), `aih-unscoped-${Date.now()}.txt`);
  t.after(() => {
    fs.rmSync(hostHomeDir, { recursive: true, force: true });
    fs.rmSync(tempFile, { force: true });
  });
  fs.ensureDirSync(aiHomeDir);
  fs.writeFileSync(aiHomeFile, 'aih-internal', 'utf8');
  fs.writeFileSync(tempFile, 'unscoped-temp', 'utf8');

  for (const requestPath of [aiHomeFile, tempFile]) {
    const res = createResCapture();
    const handled = await handleWebUIRequest({
      method: 'GET',
      pathname: '/v0/webui/fs/read',
      url: new URL(`http://localhost/v0/webui/fs/read?path=${encodeURIComponent(requestPath)}`),
      req: { headers: {} },
      res,
      options: {},
      state: {},
    ...
    assert.equal(JSON.parse(res.body).error, 'outside_workspace', requestPath);
  }
});

test('web ui file preview offers canonical trust scopes and persists the selected folder', async (t) => {
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-file-trust-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const projectDir = path.join(hostHomeDir, 'projects', 'active-project');
  const sharedRoot = path.join(hostHomeDir, 'shared');
  const targetDir = path.join(sharedRoot, 'reports');
  const targetPath = path.join(targetDir, 'summary.md');
  const siblingPath = path.join(sharedRoot, 'assets', 'chart.txt');
  t.after(() => fs.rmSync(hostHomeDir, { recursive: true, force: true }));
  fs.ensureDirSync(projectDir);
  fs.ensureDirSync(targetDir);
  fs.ensureDirSync(path.dirname(siblingPath));
  fs.writeFileSync(targetPath, '# Summary\n', 'utf8');
  fs.writeFileSync(siblingPath, 'chart data', 'utf8');
  addOpenedProject({ path: projectDir }, { fs, aiHomeDir });
  const deps = { ...createBaseDeps(aiHomeDir), hostHomeDir };

  const deniedRes = createResCapture();
  const deniedHandled = await handleWebUIRequest({
    method: 'GET',
    pathname: '/v0/webui/fs/read',
    url: new URL(
      `http://localhost/v0/webui/fs/read?path=${encodeURIComponent(targetPath)}`
      + `&projectPath=${encodeURIComponent(projectDir)}`
    ),
    req: { headers: {} },
    res: deniedRes,
    options: {},
  ...
  const realTargetDir = path.dirname(realTargetPath);
  const realSharedRoot = path.dirname(realTargetDir);
  assert.equal(deniedBody.error, 'outside_workspace');
  assert.equal(deniedBody.authorization.required, true);
  assert.equal(deniedBody.authorization.filePath, realTargetPath);
  assert.deepEqual(
    deniedBody.authorization.candidates.map((candidate) => [candidate.scope, candidate.path]),
    [
      ['file_directory', realTargetDir],
      ['parent_directory', realSharedRoot]
    ]
  );

  const trustRes = createResCapture();
  const trustHandled = await handleWebUIRequest({
    method: 'POST',
    pathname: '/v0/webui/fs/trust',
    url: new URL('http://localhost/v0/webui/fs/trust'),
    req: { headers: {} },
    res: trustRes,
    options: {},
    state: {},
  ...
  assert.equal(trustHandled, true);
  assert.equal(trustRes.statusCode, 200);
  assert.equal(JSON.parse(trustRes.body).trustedRoot, realSharedRoot);
  const storedRoots = readJsonValue(fs, aiHomeDir, 'webui-file-trusted-roots');
  assert.equal(storedRoots.length, 1);
  assert.equal(storedRoots[0].path, realSharedRoot);
  assert.equal(Number.isFinite(storedRoots[0].trustedAt), true);

  for (const requestPath of [targetPath, siblingPath]) {
    const readRes = createResCapture();
    const readHandled = await handleWebUIRequest({
      method: 'GET',
      pathname: '/v0/webui/fs/read',
      url: new URL(
        `http://localhost/v0/webui/fs/read?path=${encodeURIComponent(requestPath)}`
        + `&projectPath=${encodeURIComponent(projectDir)}`
      ),
      req: { headers: {} },
      res: readRes,
    ...
    assert.equal(readHandled, true, requestPath);
    assert.equal(readRes.statusCode, 200, requestPath);
    assert.equal(readRes.headers['Cache-Control'], 'no-store, max-age=0', requestPath);
    assert.equal(readRes.headers.Pragma, 'no-cache', requestPath);
  }
});

test('web ui file access preflight uses the same trust policy as media preview', async (t) => {
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-file-access-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const imageDir = path.join(hostHomeDir, 'Downloads', 'screenshots');
  ...
  const deniedHandled = await handleWebUIRequest({
    method: 'GET',
    pathname: '/v0/webui/fs/access',
    url: new URL(`http://localhost/v0/webui/fs/access?path=${encodeURIComponent(imagePath)}`),
    req: { headers: {} },
    res: deniedRes,
    options: {},
    state: {},
  ...
  assert.equal(deniedHandled, true);
  assert.equal(deniedRes.statusCode, 403);
  assert.equal(JSON.parse(deniedRes.body).authorization.candidates[0].path, fs.realpathSync(imageDir));
});

test('web ui file trust refuses home-wide authorization and invalid scopes', async (t) => {
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-file-trust-scope-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const targetPath = path.join(hostHomeDir, 'home-note.txt');
  t.after(() => fs.rmSync(hostHomeDir, { recursive: true, force: true }));
  fs.ensureDirSync(aiHomeDir);
  fs.writeFileSync(targetPath, 'private home note', 'utf8');
  const deps = { ...createBaseDeps(aiHomeDir), hostHomeDir };
  ...
  await handleWebUIRequest({
    method: 'GET',
    pathname: '/v0/webui/fs/read',
    url: new URL(`http://localhost/v0/webui/fs/read?path=${encodeURIComponent(targetPath)}`),
    req: { headers: {} },
    res: deniedRes,
    options: {},
    state: {},
  ...
  assert.equal(deniedRes.statusCode, 403);
  assert.deepEqual(JSON.parse(deniedRes.body).authorization.candidates, []);

  for (const scope of ['file_directory', '../../arbitrary']) {
    const trustRes = createResCapture();
    const trustHandled = await handleWebUIRequest({
      method: 'POST',
      pathname: '/v0/webui/fs/trust',
      ...
      deps: {
        ...deps,
        readRequestBody: async () => Buffer.from(JSON.stringify({ path: targetPath, scope }), 'utf8')
      }
    });
    assert.equal(trustHandled, true, scope);
    assert.equal(trustRes.statusCode, 400, scope);
    assert.equal(JSON.parse(trustRes.body).error, 'unsafe_trust_scope', scope);
  }
  assert.equal(readJsonValue(fs, aiHomeDir, 'webui-file-trusted-roots'), null);
});

test('web ui file routes resolve deleted provider account aliases to native resources', async (t) => {
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-provider-native-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const sessionId = '287e94b0-5944-4d05-a614-d906797222dc';
  const nativeSessionDir = path.join(hostHomeDir, '.gemini', 'antigravity-cli', 'brain', sessionId);
  const nativeMarkdownPath = path.join(nativeSessionDir, 'real_concert_images.md');
  const nativeImagePath = path.join(nativeSessionDir, 'real_phoenix_1.jpg');
  ...
  t.after(() => fs.rmSync(hostHomeDir, { recursive: true, force: true }));
  fs.ensureDirSync(nativeSessionDir);
  fs.ensureDirSync(aiHomeDir);
  fs.writeFileSync(nativeImagePath, imageBytes);
  fs.writeFileSync(
    nativeMarkdownPath,
    `![concert](${legacyImagePath})\n`,
    'utf8'
  );
  ...
  const readHandled = await handleWebUIRequest({
    method: 'GET',
    pathname: '/v0/webui/fs/read',
    url: new URL(`http://localhost/v0/webui/fs/read?path=${encodeURIComponent(legacyMarkdownPath)}`),
    req: { headers: {} },
    res: readRes,
    options: {},
    state: {},
  ...
  assert.equal(readBody.path, nativeMarkdownPath);
  assert.equal(readBody.content, `![concert](${nativeImagePath})\n`);
  assert.equal(readBody.content.includes('.ai_home'), false);

  for (const aliasPath of [legacyImagePath, currentImagePath]) {
    const mediaRes = createResCapture();
    const mediaHandled = await handleWebUIRequest({
      method: 'GET',
      pathname: '/v0/webui/fs/media',
      url: new URL(`http://localhost/v0/webui/fs/media?path=${encodeURIComponent(aliasPath)}`),
      req: { headers: {} },
      res: mediaRes,
      options: {},
      state: {},
    ...
    assert.equal(mediaHandled, true);
    assert.equal(mediaRes.statusCode, 200, aliasPath);
    assert.equal(mediaRes.body, imageBytes.toString(), aliasPath);
  }

  assert.equal(fs.existsSync(path.join(aiHomeDir, 'profiles')), false);
  assert.equal(fs.existsSync(path.join(aiHomeDir, 'run', 'auth-projections')), false);
});

test('web ui file routes authorize AGY scratch artifacts', async (t) => {
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-agy-scratch-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const projectDir = path.join(hostHomeDir, 'projects', 'go-sorting');
  ...
  t.after(() => fs.rmSync(hostHomeDir, { recursive: true, force: true }));
  fs.ensureDirSync(projectDir);
  fs.ensureDirSync(path.dirname(scratchPath));
  fs.writeFileSync(scratchPath, 'package main\n', 'utf8');
  addOpenedProject({ path: projectDir }, { fs, aiHomeDir });
  ...
  const handled = await handleWebUIRequest({
    ...
    pathname: '/v0/webui/fs/read',
    url: new URL(
      `http://localhost/v0/webui/fs/read?path=${encodeURIComponent(scratchPath)}`
      + `&projectPath=${encodeURIComponent(projectDir)}`
    ),
    req: { headers: {} },
    res,
  ...
  assert.equal(body.path, scratchPath);
  assert.equal(body.content, 'package main\n');
});

test('web ui file routes authorize OpenCode resources', async (t) => {
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-opencode-native-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const nativePath = path.join(hostHomeDir, '.local', 'share', 'opencode', 'storage', 'message.txt');
  ...
  fs.writeFileSync(nativePath, 'shared opencode data', 'utf8');

  const res = createResCapture();
  const handled = await handleWebUIRequest({
    method: 'GET',
    pathname: '/v0/webui/fs/read',
    url: new URL(`http://localhost/v0/webui/fs/read?path=${encodeURIComponent(aliasPath)}`),
    req: { headers: {} },
    res,
    options: {},
    state: {},
  ...
  assert.equal(res.statusCode, 200);
  const body = JSON.parse(res.body);
  assert.equal(body.path, nativePath);
  assert.equal(body.content, 'shared opencode data');
});

test('web ui file routes reject provider configuration even when home is a registered project', async (t) => {
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-provider-config-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const opencodeConfigPath = path.join(hostHomeDir, '.config', 'opencode', 'opencode.json');
  const opencodeConfigAliasPath = path.join(
    aiHomeDir,
    'profiles',
    'opencode',
    '9',
  ...
  const configurationPaths = [
    ...
    path.join(hostHomeDir, '.claude', 'settings.json.aih-bak'),
    path.join(hostHomeDir, '.gemini', 'settings.json'),
    path.join(hostHomeDir, '.gemini', 'antigravity-cli', 'settings.json'),
    path.join(hostHomeDir, '.gemini', 'antigravity-cli', 'settings.local.json'),
    path.join(hostHomeDir, '.gemini', 'antigravity-cli', 'keybindings.json'),
    path.join(hostHomeDir, '.gemini', 'antigravity-cli', 'mcp_config.json')
  ];
  t.after(() => fs.rmSync(hostHomeDir, { recursive: true, force: true }));
  configurationPaths.forEach((filePath) => {
    fs.ensureDirSync(path.dirname(filePath));
    fs.writeFileSync(filePath, JSON.stringify({ apiKey: 'test-secret' }), 'utf8');
  });
  addOpenedProject({ path: hostHomeDir }, { fs, aiHomeDir });

  for (const requestPath of [...configurationPaths, opencodeConfigAliasPath]) {
    const configRes = createResCapture();
    const configHandled = await handleWebUIRequest({
      method: 'GET',
      pathname: '/v0/webui/fs/read',
      url: new URL(
        `http://localhost/v0/webui/fs/read?path=${encodeURIComponent(requestPath)}`
        + `&projectPath=${encodeURIComponent(hostHomeDir)}`
      ),
      req: { headers: {} },
      res: configRes,
      options: {},
    ...
    assert.equal(configHandled, true);
    assert.equal(configRes.statusCode, 403, requestPath);
    assert.equal(JSON.parse(configRes.body).error, 'permission_denied', requestPath);
  }
});

test('web ui file routes reject every provider credential artifact', async (t) => {
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-provider-auth-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const deps = { ...createBaseDeps(aiHomeDir), hostHomeDir };
  t.after(() => fs.rmSync(hostHomeDir, { recursive: true, force: true }));
  fs.ensureDirSync(aiHomeDir);

  for (const [provider, artifacts] of Object.entries(PROVIDER_ARTIFACTS)) {
    for (const artifact of artifacts) {
      const nativePath = path.join(hostHomeDir, ...artifact.path);
      ...
      for (const requestPath of [nativePath, projectedPath]) {
        for (const route of ['read', 'media']) {
          const res = createResCapture();
          const handled = await handleWebUIRequest({
            method: 'GET',
            pathname: `/v0/webui/fs/${route}`,
            url: new URL(`http://localhost/v0/webui/fs/${route}?path=${encodeURIComponent(requestPath)}`),
            req: { headers: {} },
            res,
            options: {},
            state: {},
          ...
          assert.equal(handled, true);
          assert.equal(res.statusCode, 403, `${provider}:${requestPath}:${route}`);
          assert.equal(JSON.parse(res.body).error, 'permission_denied');
        }
      }
    }
  }
  ...
  for (const requestPath of codexConfigPaths) {
    const res = createResCapture();
    const handled = await handleWebUIRequest({
      method: 'GET',
      pathname: '/v0/webui/fs/read',
...
test('web ui file routes authorize the real target instead of a symlink escape', async (t) => {
  if (!fs.existsSync('/etc/hosts')) return;
  const hostHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aih-webui-realpath-home-'));
  const aiHomeDir = path.join(hostHomeDir, '.ai_home');
  const resourceDirs = [
    path.join(hostHomeDir, '.gemini', 'antigravity-cli', 'brain', 'session-id'),
    path.join(hostHomeDir, '.gemini', 'antigravity-cli', 'scratch', 'go-sorting')
  ];
  t.after(() => fs.rmSync(hostHomeDir, { recursive: true, force: true }));

  for (const resourceDir of resourceDirs) {
    const linkPath = path.join(resourceDir, 'escaped-hosts.txt');
    ...
    const handled = await handleWebUIRequest({
      method: 'GET',
      pathname: '/v0/webui/fs/read',
      url: new URL(`http://localhost/v0/webui/fs/read?path=${encodeURIComponent(linkPath)}`),
      req: { headers: {} },
      res,
      options: {},
      state: {},
    ...
    assert.equal(handled, true, linkPath);
    assert.equal(res.statusCode, 403, linkPath);
    assert.equal(JSON.parse(res.body).error, 'outside_workspace', linkPath);
  }
});
...
test('web ui remove project deletes manual project from stored list', async () => {
  ...
  try {
    ...
    const codexConfigPath = path.join(aiHomeDir, '.codex', 'config.toml');
    assert.equal(fs.existsSync(codexConfigPath), true);
    assert.match(fs.readFileSync(codexConfigPath, 'utf8'), new RegExp(
      `\\[projects\\.${JSON.stringify(projectDir).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\]`
    ));
    ...
    assert.equal(fs.readFileSync(codexConfigPath, 'utf8').includes(projectDir), false);
  } finally {
    fs.rmSync(aiHomeDir, { recursive: true, force: true });
    fs.rmSync(projectDir, { recursive: true, force: true });
  }

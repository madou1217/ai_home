...
const { REMOTE_INVITES_KEY } = require('../lib/server/remote/worker-join-invite');
const { readJsonValue } = require('../lib/server/app-state-store');

function readStoredInvites(aiHomeDir) {
  const stored = readJsonValue(fs, aiHomeDir, REMOTE_INVITES_KEY);
  return Array.isArray(stored && stored.invites) ? stored.invites : [];
}
...
test('web ui remote node routes create invite without exposing stored code hash', async (t) => {
  ...
  assert.doesNotMatch(JSON.stringify(readStoredInvites(aiHomeDir)), new RegExp(created.code));
...
test('web ui remote node invite returns bootstrap script without secret material', async (t) => {
  ...
  await handleWebUIRequest({
    ...
    deps: createDeps(aiHomeDir, null, Buffer.from(JSON.stringify({
      ...
      obsoleteSecret: 'obsolete-secret-must-not-leak'
    }), 'utf8'))
  });
  ...
  assert.doesNotMatch(createRes.body, /must-not-leak|obsolete-secret-must-not-leak|codeHash/);
  assert.doesNotMatch(JSON.stringify(readStoredInvites(aiHomeDir)), /must-not-leak|obsolete-secret-must-not-leak/);
});
...
test('web ui remote node bootstrap plan previews Windows local console bootstrap without storing invite', async (t) => {
  ...
  assert.deepEqual(readStoredInvites(aiHomeDir), []);
});
...
test('web ui remote node bootstrap probe runs readonly ssh and tcp diagnostics', async (t) => {
  ...
  assert.doesNotMatch(payload.report.results[0].bootstrapScript.content, /obsoleteSecret|remote-secret/i);
  ...
  assert.doesNotMatch(payload.report.results[1].bootstrapScript.content, /obsoleteSecret|remote-secret/i);
});

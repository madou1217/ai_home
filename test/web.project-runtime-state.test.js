...
test('session UI identity never aliases the same native id across providers', async () => {
  const { isSameSession } = await loadProjectRuntimeState();
  assert.equal(isSameSession(
    { provider: 'codex', id: 'native-1' },
    { provider: 'claude', id: 'native-1' }
  ), false);
});
...
test('getVisibleProjectSessions respects collapsed and expanded limits', async () => {
  ...
  assert.equal(getVisibleProjectSessions(sessions, true, true, 10, sessions.length).length, 18);
});

test('getProjectProviderBadges keeps running provider icons visible in collapsed and expanded projects', async () => {
  ...
  assert.deepEqual(
    ...
    [
      { provider: 'codex', running: true },
      { provider: 'claude', running: false },
      { provider: 'gemini', running: true }
    ]
  );

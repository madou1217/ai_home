...
test('AGY session messages prefer recorded canonical turn models over transcript labels', async () => {
  const writes = [];
  const messages = [
  ...
  await handleGetSessionMessagesRequest({
    pathname: '/v0/webui/sessions/agy/session-1/messages',
    req: {
      headers: { host: '127.0.0.1:9527' },
      url: '/v0/webui/sessions/agy/session-1/messages'
    },
    res: {},
    deps: {
      readSessionMessagesSnapshot: () => ({ messages, cursor: 123 }),
      modelUsageService: {
        getNativeSessionModelTimeline(provider, sessionId) {
  ...
  assert.equal(writes[0].statusCode, 200);
  assert.deepEqual(
    writes[0].payload.messages.map((message) => message.model),
    Array(4).fill('gemini-3.6-flash-tiered')
  );
});

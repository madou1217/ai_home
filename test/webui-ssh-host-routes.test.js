...
test('web ui ssh-connections & workspaces CRUD, cascade delete, key/password scrubbing and SSH browse API testing', async (t) => {
  ...
  // ==========================================
  {
    ...
    assert.equal(data.result.bootstrapAction.remoteRunCommand.includes(detectedKeyPath), false);
    assert.equal(data.result.recommendation.includes(detectedKeyPath), false);

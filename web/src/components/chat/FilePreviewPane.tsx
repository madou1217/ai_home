...
function RenderedMarkdownPreview({ content, softWrap }: { content: string; softWrap: boolean }) {
  const wrapClassName = softWrap ? styles.filePreviewCodeSoftWrap : styles.filePreviewCodeNoWrap;
...
function RawMediaPreview({ mediaUrl }: { mediaUrl: string }) {
  return (
    <div className={styles.filePreviewRawMedia}>
      ...
      <Button size="small" href={mediaUrl} target="_blank" rel="noreferrer">
        打开原图
      </Button>
    </div>
  );
...
function FilePreviewPane({ path, content, mediaUrl, loading = false, error = '' }: Props) {
    ...
    <div className={styles.filePreviewPane}>
      ...
      <div className={styles.filePreviewViewport}>
        {previewMode === 'rendered' ? (
          <RenderedMarkdownPreview content={fileContent} softWrap={softWrap} />
        ) : (

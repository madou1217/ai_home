...
const MessageArea = ({
  ...
  const prevScrollInfo = useRef({ height: 0, firstMsgId: '' });
  ...
  useLayoutEffect(() => {
    ...
    } else if (firstMsgId !== prevScrollInfo.current.firstMsgId && prevScrollInfo.current.firstMsgId !== '') {
      // 加载更早的历史消息：保持当前相对滚动位置
      const heightDiff = el.scrollHeight - prevScrollInfo.current.height;
      if (heightDiff > 0) {
        el.scrollTop += heightDiff;
      }
    } else {
      // 后续更新（例如新消息追加或流式打字）：如果已经在底部附近，则平滑滚动跟随
      const distanceToBottom = prevScrollInfo.current.height - el.scrollTop - el.clientHeight;
      if (distanceToBottom < 300) {
        messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });
      }
    }

    prevScrollInfo.current = { height: el.scrollHeight, firstMsgId };
  }, [messages]);

...
import { ArrowDownOutlined, PlusOutlined, CloseOutlined, InfoCircleOutlined, DownOutlined, CodeOutlined } from '@ant-design/icons';
            ...
            <div className={styles.inputToolbarLeft}>
              ...
              {mobile ? (
                ...
                <Dropdown
                  ...
                  dropdownRender={(menu) => (
                    <div style={{ background: '#fff', borderRadius: 12, padding: '4px 0', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', minWidth: 180 }}>
                      {models.length === 0 ? (
                        <div style={{ padding: '12px 14px', textAlign: 'center', color: '#999', fontSize: 13 }}>
                          <div>{emptyModelHint}</div>
                          {!modelsLoading && selectedAccount && !isAihServerAccount(selectedAccount) ? (
                            <button
                              type="button"
                              onClick={() => history.push(`/accounts/${encodeURIComponent(selectedAccount.provider)}/${encodeURIComponent(selectedAccount.accountRef)}/models`)}
                              style={{ marginTop: 8, padding: '5px 12px', border: '1px solid #e2e2ee', borderRadius: 8, background: '#fff', color: 'var(--m-acc, #d97757)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}
                            >
                              去「账号模型管理」添加模型
                            </button>
                          ) : null}
              ...
              {onApprovalModeChange && !mobile ? (
                <Dropdown
                  ...
                  menu={{
                    items: ([
                      { key: 'bypass', label: '极速(默认)', hint: '不询问,直接执行' },
                      { key: 'confirm', label: '需确认', hint: '敏感操作弹审批卡' },
                      { key: 'plan', label: '计划模式', hint: '先出计划,确认后执行' }
                    ]).map((item) => ({
                      key: item.key,
                      onClick: () => onApprovalModeChange(item.key as 'bypass' | 'confirm' | 'plan'),
                      label: (
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '4px 2px' }}>
                          <span style={{ fontWeight: approvalMode === item.key ? 600 : 400 }}>{item.label}</span>
                ...
                >
                  <button type="button" className={styles.metaDropdownBtn} title="审批模式:运行中的敏感操作是否需要你确认">
                    <span>{approvalMode === 'plan' ? '计划' : approvalMode === 'confirm' ? '需确认' : '极速'}</span>
                    <DownOutlined style={{ fontSize: 10, color: '#9ca3af', marginLeft: 2 }} />
                  </button>
                </Dropdown>
              ) : null}
                ...
                <Dropdown
                  ...
                  menu={{
                    items: Object.entries(
                      ...
                      children: [
                        // 网关"全部账号+别名"作为该 provider 组的默认/首选项(agy/gemini 不支持,跳过)。
                        ...(supportsAihServer(provider) ? [(() => {
                          const isActive = isAihServerAccount(selectedAccount) && selectedAccount?.provider === provider;
                          return {
                            key: getGatewaySelectionScope(provider as Provider),
                            onClick: () => onAccountChange(makeAihServerAccount(provider as Provider)),
                            label: (
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 2px', color: isActive ? '#fff' : '#333' }}>
                                ...
                                <span style={{ fontWeight: isActive ? 600 : 400 }}>{AIH_SERVER_ACCOUNT_LABEL}</span>
                                <span style={{ fontSize: 10, background: isActive ? 'rgba(255,255,255,0.2)' : '#eef2ff', padding: '0 6px', borderRadius: 4, color: isActive ? '#fff' : '#4f46e5' }}>网关</span>
                              </div>
                            ),
                        ...
                        const isActive = !isAihServerAccount(selectedAccount)
                          && selectedAccount?.accountRef === acc.accountRef;
                        return {
                          key: acc.accountRef,
                  ...
                  <button type="button" className={styles.metaDropdownBtn}>
                    {selectedAccount ? (
                      <>
                        ...
                        <span>{isAihServerAccount(selectedAccount) ? AIH_SERVER_ACCOUNT_LABEL : formatChatAccountLabel(selectedAccount)}</span>

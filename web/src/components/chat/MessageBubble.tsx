...
function parseOaiMemCitations(value: string): ParsedCitation[] {
  const citations: ParsedCitation[] = [];
  const lines = value.split('\n');
  let inEntries = false;
  for (const line of lines) {
    if (line.trim() === '<citation_entries>') {
      inEntries = true;
      continue;
    }
    if (line.trim() === '</citation_entries>') {
    ...
    if (inEntries && line.trim()) {
      const match = line.match(/^([^:]+):([0-9-]+)\|note=\[(.+)\]$/);
      if (match) {
        citations.push({ path: match[1], lines: match[2], note: match[3] });
      }
    }
  }
  return citations;
...
/** 通用标签渲染块 */
const TagBlock = ({ name, value, orphanClose, mobile = false, onOpenFile, projectPath }: { name: string; value: string; orphanClose?: boolean; mobile?: boolean; onOpenFile?: (path: string) => void; projectPath?: string }) => {
  const [expanded, setExpanded] = useState(false);

  if (orphanClose) {
    return (
      <div className={`${styles.thinkingBlock} ${styles.tagBlockOrphan}`} style={{ padding: '4px 8px', display: 'inline-block', opacity: 0.7 }}>
        <span className={styles.thinkingHeader} style={{ padding: 0 }}>/{name}</span>
      </div>
    );
  }

  if (name === 'oai-mem-citation') {
    const citations = parseOaiMemCitations(value);
    return (
      <div className={styles.thinkingBlock}>
        <div className={styles.thinkingHeader} onClick={() => setExpanded(!expanded)}>
          <span style={{ fontSize: mobile ? 13 : 12 }}>{expanded ? '▼' : '▶'}</span>
          <span>{citations.length} memory citations</span>
        </div>
        {expanded && (
          <div className={styles.thinkingBody} style={{ padding: '12px' }}>
            {citations.map((cit, idx) => {
              // Resolve relative paths if projectPath is available
              const fullPath = (cit.path.startsWith('/') || cit.path.startsWith('~'))
                ? cit.path
                : (projectPath ? `${projectPath}/${cit.path}` : cit.path);

              return (
                <div key={idx} style={{ marginBottom: 12 }}>
                  <div
                    style={{ fontWeight: 500, color: '#333', cursor: 'pointer', display: 'flex', alignItems: 'baseline', gap: 6 }}
                    onClick={() => {
                      if (onOpenFile) onOpenFile(fullPath);
                    }}
                  >
                    <span style={{ textDecoration: 'underline', textUnderlineOffset: 2 }}>{basenameLike(cit.path)}</span>
                    <span style={{ color: '#999', fontSize: 12, fontWeight: 'normal', textDecoration: 'none' }}>lines {cit.lines}</span>
                  </div>
                  <div style={{ color: '#666', fontSize: 13, marginTop: 4 }}>{cit.note}</div>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div className={styles.thinkingBlock} onClick={() => isMultiline && setExpanded(!expanded)}>
      <div className={styles.thinkingHeader}>
        {isMultiline && <span style={{ fontSize: mobile ? 13 : 12 }}>{expanded ? '▼' : '▶'}</span>}
        <span>{name}</span>
        {!expanded && value && <span className={styles.thinkingPreview}>{preview}</span>}
      </div>
      {expanded && <div className={styles.thinkingBody}>{value}</div>}
    </div>
  );
  ...
  const handleOpenFile = useCallback((filePath: string) => {
    setTabs(prev => {
      if (!prev.find(t => t.path === filePath)) {
        return [...prev, { path: filePath, title: basenameLike(filePath), projectPath: resolvedProjectPath }];
      }
      return prev;
    });
            ...
            {hasLiveDetail ? (
              <div className={styles.pendingDetailBody}>
                {pendingDetailBlocks.map((block, idx) => {
                  if (block.type === 'tag') return <TagBlock key={idx} name={block.name} value={block.value} orphanClose={block.orphanClose} mobile={mobile} onOpenFile={handleOpenFile} projectPath={resolvedProjectPath} />;
                  ...
                  return <ReactMarkdown key={idx} remarkPlugins={[remarkGfm]} components={mdComponents}>{block.value}</ReactMarkdown>;
                })}
              </div>
            ) : null}
          </div>

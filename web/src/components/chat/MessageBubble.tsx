...
import { buildFileBackedImageUrl, getFilePreviewKind } from './file-preview-utils';
...
const MessageBubble = ({ message, provider, session, mobile = false }: Props) => {
  ...
  const hasStructuredUserBlocks = isUser && blocks.some((block) =>
    block.type === 'tag' && (isGoalContextTag(block.name, block.value) || block.name === 'request_user_input')
  );
  ...
  }, [handleOpenFile, mdComponents, mobile, renderedImages, resolvedProjectPath, toolImageFallbackMap]);

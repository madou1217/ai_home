...
export type ProviderBlock =
  ...
  // 子代理任务（opencode=task / claude=Task 等派生并行子代理的工具调用）：
  // 用任务描述做标题,多个并行子代理各自成卡、天然可区分。
  | {
      kind: 'subagent';
      description: string;
      prompt: string;
...
// task/Task 的入参形态：{description, prompt, ...}——是「派生子代理」而非计划清单。
...
function parseSubagentInvocation(name: string, body: string): SubagentInvocation | null {
  const toolName = String(name || '').trim().toLowerCase();
  if (toolName !== 'task' && toolName !== 'spawn_agent') return null;
  try {
    ...
    if (toolName === 'task') {
      return { description: description || '子代理任务', prompt };
    }
    return {
      description: description || '子代理任务',
      prompt,
      childSessionId: String(parsed?.child_session_id || '').trim(),
      agentNickname: String(parsed?.agent_nickname || '').trim(),
...
function classifyTool(name: string, body: string, result?: string): ProviderBlock {
  ...
  const subagent = parseSubagentInvocation(n, body);
  if (subagent) {
    return result === undefined
      ? { kind: 'subagent', ...subagent }
      : { kind: 'subagent', ...subagent, result };
  }

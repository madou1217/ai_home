...
export function readPersistedQueue(sessionKey) {
  ...
  try {
    const raw = window.sessionStorage.getItem(queueStorageKey(sessionKey));
...
export function writePersistedQueue(sessionKey, items) {
  ...
  try {
    ...
    if (list.length === 0) {
      window.sessionStorage.removeItem(queueStorageKey(sessionKey));
    } else {
      window.sessionStorage.setItem(queueStorageKey(sessionKey), JSON.stringify(list));
    }
  } catch (_error) { /* 配额/序列化失败忽略 */ }
}

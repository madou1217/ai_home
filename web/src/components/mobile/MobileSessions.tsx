...
interface Props {
  ...
  onRefresh: () => void;
  canCreate: boolean;
  lifecycleCapabilities: Partial<Record<Provider, ProviderSessionLifecycleCapability>>;
...
const MobileSessions = ({
  ...
  onRefresh, canCreate, lifecycleCapabilities, onRequestArchiveSession, onRequestRemoveProject
}: Props) => {
  ...
  useEffect(() => {
    ...
    const toFetch = visible
      .filter((session) => !fetchedRef.current.has(getSessionRunKey(session)))
      .map((session) => {
        const key = getSessionRunKey(session);
                  ...
                  <div>
                    <div className="msx-subs">
                      ...
                      {visible.map((session: Session) => {
                        ...
                        const identity = getSessionRunKey(session);
                        const active = isSameSession(selectedSession, session);
                        const meta = previews[identity] || {};
                        const model = meta.model || session.model;
                        const preview = meta.preview || session.preview;
                        const archiveAction = resolveArchiveAction(lifecycleCapabilities, session.provider);
                        const archiveDisabled = running || archiveAction.disabled;
                        return (
                          <div
                          ...
                          >
                            ...
                            <div className="msx-sbody">
                              ...
                              <div className="msx-metarow">
                                ...
                                {archiveAction.visible ? (
                                  <button
                                    className="msx-subact"
                                    title={archiveDisabled
                                      ? `原生归档不可用：${running ? 'session_lifecycle_active' : archiveAction.reason}`
                                      : '原生归档'}

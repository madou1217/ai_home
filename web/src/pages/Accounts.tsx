...
const Accounts = () => {
  ...
  const handleSetDefault = async (record: Account) => {
    try {
      ...
      message.success('默认账号已更新');
      loadAccounts();
    } catch (error: any) {
      message.error(error?.response?.data?.message || '设置默认账号失败');
    }
  };
  ...
  const handleSetMobile = async (record: Account) => {
    try {
      ...
      message.success('Codex App 账号已更新');
      loadAccounts();
    } catch (error: any) {
      message.error(error?.response?.data?.message || '设置 Codex App 账号失败');
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: Account) => {
        ...
        menuItems.push({
          key: 'set-default',
          label: record.isDefault ? '当前默认账号' : '设为默认账号',
          icon: record.isDefault ? <CheckCircleOutlined style={{ color: '#1677ff' }} /> : <CheckCircleOutlined />,
          disabled: Boolean(record.isDefault)
        });
        if (record.provider === 'codex') {
          menuItems.push({
            key: 'set-mobile',
            label: record.isMobile ? '当前 Codex App 账号' : '设为 Codex App 账号',
      ...
      >
        <Form form={editForm} layout="vertical">
          <Form.Item
            name="apiKey"
            label="API Key"
            extra="如不修改请留空。支持设置 API Key 以提升并发配额。"
          >

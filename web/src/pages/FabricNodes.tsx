...
function capabilityRows(view: FabricNodeInventoryItem, sshBindings: LocalSshBinding[]) {
  ...
  const sshHint = sshBindings.length > 0
    ...
    : (caps.sshBootstrap ? '能通过 SSH 远程开发' : '未配置：可在「SSH 开发机」页添加连接和工作区');
  return [
    {
      ok: caps.runtimeHost,
...
export default function FabricNodes() {
  ...
  const readyProfileKey = readyProfile
    ? `${readyProfile.id}:${readyProfile.updatedAt}:${isControlPlaneManagementKeyConfigured(readyProfile) ? 'key' : 'missing-key'}`
    : '';
    ...
    >
      {!readyProfile && (
        <Alert
          ...
          message="还没有连上服务器"
          description="节点总览需要先连上一个已配置 Management Key 的 Server 才能读取机器列表。"
          action={(

...
import { fetchAuthorizedWebUiResource } from '@/services/webui-auth-transport';
...
export default function FabricWebrtcDiagnostics() {
  ...
  const sendSignal = useCallback(async (nextRoomId: string, type: SignalType, payload: Record<string, unknown>) => {
    const response = await fetchAuthorizedWebUiResource(apiUrl(`/v0/fabric/webrtc/signaling/rooms/${encodeURIComponent(nextRoomId)}/messages`), {
      method: 'POST',
  ...
  const pollSignals = useCallback(async (activeRoomId: string) => {
    ...
    try {
      ...
      const response = await fetchAuthorizedWebUiResource(apiUrl(`/v0/fabric/webrtc/signaling/rooms/${encodeURIComponent(activeRoomId)}/messages?since=${since}&limit=100`));
  ...
  const createRoomAndStart = async () => {
    setBusy(true);
    try {
      const response = await fetchAuthorizedWebUiResource(apiUrl('/v0/fabric/webrtc/signaling/rooms'), {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
  ...
  return (
    ...
    >
      <StatisticCard.Group direction="row" style={{ marginBottom: 16 }}>
        ...
        <StatisticCard statistic={{ title: '信令状态', value: signalingState }} />

...
import { Alert, Button, Form, Input, Segmented, Space, Statistic, Tag, message } from 'antd';
import {
  ApiOutlined,
  DisconnectOutlined,
  LinkOutlined,
...
export default function FabricWebrtcLab() {
  ...
  return (
    <div className="fabric-webrtc-lab-page animate__animated animate__fadeIn animate__faster">
      <section className="fabric-webrtc-lab-header">
        <div>
          <span>AIH Fabric Lab</span>
          <h1>WebRTC DataChannel</h1>
          <p>短期信令房间只用于传输实验；默认工作流仍走已验证的 WSS fallback。</p>
        </div>
        <Space size={8} wrap>
          <Tag color={channelState === 'open' ? 'green' : 'gold'}>{channelState}</Tag>
          <Tag>{connectionState}</Tag>
          <Tag>{iceState}</Tag>
          <Tag>{iceGatheringState}</Tag>
          <Tag>{signalingState}</Tag>
        </Space>
      </section>

      <Alert
        className="fabric-webrtc-lab-alert"
        type="info"
        showIcon
        message="Lab transport"
        description="此页面记录 WebRTC 候选能力，不会把 WebRTC 设为生产默认 transport。跨 NAT 场景需要按实验记录配置 STUN/TURN。"
      />

      <div className="fabric-webrtc-lab-grid">
        <section className="fabric-webrtc-lab-panel fabric-webrtc-lab-panel--primary">
          <div className="fabric-webrtc-lab-panel-head">
            <div>
              <h2>信令房间</h2>
              <p>一端创建房间，另一端使用分享 URL 加入。</p>
            </div>
            <ApiOutlined />
          </div>
          <Form layout="vertical">
            <Form.Item label="Signal Endpoint">
          ...
          {shareUrl && (
            <div className="fabric-webrtc-lab-share">
              <span>{shareUrl}</span>
              <Button size="small" onClick={copyShareUrl}>复制</Button>
            </div>
          )}
          {room && (
            <div className="fabric-webrtc-lab-room-meta">
              <Tag>expires {new Date(room.expiresAt).toLocaleTimeString()}</Tag>
              <Tag>{room.peerCount} peers</Tag>
              <Tag>{room.messageCount} signals</Tag>
            </div>
          )}
        </section>

        <section className="fabric-webrtc-lab-panel">
          <div className="fabric-webrtc-lab-panel-head">
            <div>
              <h2>RTT</h2>
              <p>DataChannel 打点只统计应用层 ping/pong。</p>
            </div>
            <RadarChartOutlined />
          </div>
          <div className="fabric-webrtc-lab-stats">
            <Statistic title="samples" value={rttSummary.count} />
            <Statistic title="avg ms" value={rttSummary.avg} precision={2} />
            <Statistic title="p50 ms" value={rttSummary.p50} precision={2} />
            <Statistic title="p95 ms" value={rttSummary.p95} precision={2} />
          </div>
          <div className="fabric-webrtc-lab-stats fabric-webrtc-lab-stats--signals">
            <Statistic title="local candidates" value={localCandidateCount} />
            <Statistic title="remote candidates" value={remoteCandidateCount} />
            <Statistic title="signals" value={receivedSignalCount} />
            <Statistic title="queued" value={candidateQueueRef.current.length} />
          </div>
          <Space wrap>
      ...
      <section className="fabric-webrtc-lab-panel fabric-webrtc-lab-panel--wide">
        <div className="fabric-webrtc-lab-panel-head">
          <div>
            <h2>事件</h2>
            <p>用于 evidence 复盘，最多保留最近 80 条。</p>
          </div>
          <Tag>{peerId}</Tag>
        </div>
        <div className="fabric-webrtc-lab-log">
          {logs.length === 0 ? <span>暂无事件</span> : logs.map((line) => <code key={line}>{line}</code>)}
        </div>
      </section>
    </div>
  );

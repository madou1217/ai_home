...
import { DeleteOutlined, LinkOutlined, PlusOutlined, ReloadOutlined, SaveOutlined } from '@ant-design/icons';
...
const Settings = ({ section }: SettingsProps) => {
  ...
  const handleSaveServer = async (values: ServerConfig) => {
    ...
    try {
      ...
      if (managementKey) {
        await updateServerManagementKey(activeControlPlaneProfile, managementKey);
        syncSavedControlPlaneProfiles(activeControlPlaneProfile.id);
      }
  ...
  const handleSaveControlPlane = async (values: { endpoint?: string; name?: string; managementKey?: string }) => {
    ...
    try {
      ...
      const managementKey = String(values.managementKey || '').trim();
      if (!endpoint) {
        message.error('请输入有效的 Server URL');
        return;
      }
      ...
      if (!managementKey && !isControlPlaneManagementKeyConfigured(existing)) {
        message.error('请输入 Management Key');
        return;
      }
  ...
  const controlPlanesActions = (
    <Space size={8} wrap>
      ...
      <Button
        type="primary"
        icon={<PlusOutlined />}
        onClick={() => setControlPlaneAddModalOpen(true)}
      >
        添加 Server
      </Button>
    </Space>

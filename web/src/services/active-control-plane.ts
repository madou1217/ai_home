...
function getUnavailableReason(profile: ControlPlaneProfile | null): ActiveControlPlaneUnavailableReason {
  ...
  if (!isControlPlaneManagementKeyConfigured(profile)) return 'missing-key';
  if (profile.state === 'degraded') return 'degraded';
  if (profile.state === 'offline') return 'offline';
  return 'ready';
}
...
function createActiveControlPlaneError(reason: ActiveControlPlaneUnavailableReason) {
  const errorCode = reason === 'missing'
    ...
    : reason === 'degraded'
      ? 'active_control_plane_degraded'
      : reason === 'offline'
        ? 'active_control_plane_offline'
...
export function getActiveControlPlaneUnavailableText(
  ...
  if (reason === 'missing') return '未选择 Server';
  if (reason === 'degraded') return '当前 Server 连接异常';
  if (reason === 'offline') return '当前 Server 离线';
  if (reason === 'missing-key') return '当前 Server 缺少 Management Key';
  return '';
}

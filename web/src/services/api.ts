...
export const accountsAPI = {
  ...
  updateAccount: async (provider: string, accountId: string, data: { apiKey?: string; baseUrl?: string }): Promise<{ ok: boolean; account: Account }> => {
...
// 本地文件系统 API
export const fsAPI = {
  read: async (path: string, projectPath?: string): Promise<{ content: string; path: string; size: number; mtime: number }> => {
    const response = await api.get('/webui/fs/read', { params: { path, projectPath } });
    return response.data;
  }
};

...
function buildSessionRequestKey(url: string) {
  try {
    ...
    const page = kind === 'messages'
      ? `${parsed.searchParams.get('before') || 'tail'}:${parsed.searchParams.get('limit') || ''}`
      : (parsed.searchParams.get('cursor') || '0');
    return `${SESSION_REQUEST_SCOPE}:${provider}:${sessionId}:${projectDirName}:${kind}:${page}`;
  } catch (_error) {
    return '';
  }
}
...
function buildSessionSupersedeKey(url: string) {
  try {
    ...
    const [provider, sessionId, kind] = pathname.split('/').slice(-3);
    const projectDirName = parsed.searchParams.get('projectDirName') || '';
    const baseKey = `${SESSION_REQUEST_SCOPE}:${provider}:${sessionId}:${projectDirName}:${kind}`;
    if (kind !== 'messages') return baseKey;
    return `${baseKey}:${parsed.searchParams.get('before') || 'tail'}`;
  } catch (_error) {
    return '';
  }
}
...
export const sessionsAPI = {
  ...
  ): Promise<Record<string, { model?: string; preview?: string }>> => {
    if (!sessions || sessions.length === 0) return {};
    const response = await api.post<{ ok: boolean; previews?: Record<string, { model?: string; preview?: string }> }>(

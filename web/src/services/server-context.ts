 ...
 * 单一真相：激活 profile → { profile, endpoint, managementKey, isLocal }。
 * - isLocal: profile.endpoint 与当前页面同源（回环等价）→ 走本地 /v0/webui/*（完整 UI）。
 * - 远端：直连 profile.endpoint + 其 Management Key（/v0/node-rpc/device-*），不走本地代理。
...
function isSameOriginEndpoint(endpoint: string) {
  return typeof window !== 'undefined' && isSameServerOrigin(endpoint, window.location.origin);
}

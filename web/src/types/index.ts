...
export type GatewayAccount = Omit<Account, 'accountRef'> & {
  gateway: true;
  accountRef?: never;
};
...
export interface ChatStreamEvent {
  type: 'ready' | 'session-created' | 'delta' | 'thinking' | 'result' | 'done' | 'error' | 'terminal-output' | 'interactive-prompt' | 'interactive-prompt-cleared' | 'assistant_tool_call' | 'assistant_tool_result';
  delta?: string;
  thinking?: string;
  content?: string;
  text?: string;
